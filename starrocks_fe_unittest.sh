...
if [ "$GITHUB_PR_TARGET_BRANCH" == "main" ];then
    ...
    java -jar $ROOT/jacococli.jar report ./$jacoco_result --classfiles ./classes/ --html ./result --sourcefiles $PROJECT/fe/fe-core/src/main/java/ --encoding utf-8 --name fe-coverage
    ...
    while (( $pull_status != 0 ));do
        ...
        timeout 180 java -jar $ROOT/cover-checker-console-1.4.0-jar-with-dependencies.jar --cover $PROJECT/fe/fe-core/target/result/ --github-token 66e4c48809eb7e058eb73668b8c816867e6d7cbe  --repo StarRocks/starrocks --threshold 80 --github-url api.github.com  --pr ${GITHUB_PR_NUMBER} -type jacoco
        pull_status=$?
