...
#    ./docker-build.sh --fe --clean                       # clean and build Frontend
...
BUILD OPTIONS (passed to build.sh):
...
parse_args() {
    while [[ $# -gt 0 ]]; do
        case $1 in
            --fe|--be|--spark-dpp|--hive-udf|--clean|--enable-shared-data|--use-staros)
                BUILD_ARGS="$BUILD_ARGS $1"
                shift
                ;;
            --with-gcov|--without-gcov|--with-bench|--with-clang-tidy|--without-java-ext)
                BUILD_ARGS="$BUILD_ARGS $1"
                shift
                ;;
            --without-starcache|--without-tenann|--without-avx2|--disable-java-check-style)
                BUILD_ARGS="$BUILD_ARGS $1"
                shift
                ;;
            -j)
