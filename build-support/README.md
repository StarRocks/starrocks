...
- `build-support/gen_config_fwd_headers.py`: Generate or validate committed `be/src/common/config_<domain>_fwd.h` headers from `be/src/common/config.h`. By default it only rewrites files whose content changed, so unchanged headers keep their timestamps. Usage: `python3 build-support/gen_config_fwd_headers.py`, `python3 build-support/gen_config_fwd_headers.py --check`, or `python3 build-support/gen_config_fwd_headers.py --force`
