...
| **变量**                    | **默认值** | **是否支持动态修改** | **说明**                                                     |
| --------------------------- | ---------- | -------------------- | ------------------------------------------------------------ |
| enable_query_cache          | false      | 是                   | 指定是否开启 Query Cache。取值范围：`true` 和 `false`。`true` 表示开启，`false` 表示关闭。开启该功能后，只有当查询满足本文“适用场景”小节所述之条件时，才会启用 Query Cache。 |
...
| ---------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 明细模型         | 基表查询                                                     | 仅当增量版本含删除记录时不支持。其他情况下都支持。           |
| 单表物化视图查询 | 仅当查询的 GROUP BY、HAVING、或 WHERE 子句中引用聚合列时不支持。其他情况下都支持。 |                                                              |
| 聚合模型         | 基表查询或单表物化视图查询                                   | 仅在以下场景不支持：<ul><li>基表的 Schema 中含聚合函数 `replace`。</li><li>查询的 GROUP BY、HAVING、或 WHERE 子句中引用聚合列。</li><li>增量版本含删除记录。其他情况下都支持。</li></ul> |
...
#### 两个查询的第一次聚合语意等价复用 Query Cache 缓存结果
