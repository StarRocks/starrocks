...
- **外部数据目录物化视图**
  ...
  - 在 2.5.5 版本后，StarRocks 可以周期性刷新经常访问的 Hive 外部数据目录的元数据，达到感知数据更新的效果。您可以通过以下 FE 参数配置 Hive 元数据周期性刷新：

    | 配置名称                                                      | 默认值                        | 说明                                  |
    | ------------------------------------------------------------ | ---------------------------- | ------------------------------------ |
    | enable_background_refresh_connector_metadata                 | v3.0 为 true，v2.5 为 false   | 是否开启 Hive 元数据周期性刷新。开启后，StarRocks 通过轮询经常访问的 Hive 外部数据目录的 Hive Metastore 感知数据更新，并将信息缓存至对应外部数据目录的元数据中。`true` 代表开启，`false` 代表关闭。FE 静态参数，需要在 FE 配置文件 **fe.conf** 中修改。 |
    | background_refresh_metadata_interval_millis                  | 600000（10 分钟）             | 接连两次 Hive 元数据刷新之间的间隔。单位：毫秒。FE 动态参数，可以通过 [ADMIN SET FRONTEND CONFIG](../sql-reference/sql-statements/Administration/ADMIN%20SET%20CONFIG.md) 命令设置。 |
    | background_refresh_metadata_time_secs_since_last_access_secs | 86400（24 小时）              | Hive 元数据刷新任务过期时间。对于已被访问过的 Hive Catalog，如果超过该时间没有被访问，则停止刷新其元数据。对于未被访问过的 Hive Catalog，StarRocks 不会刷新其元数据。单位：秒。FE 动态参数，可以通过 [ADMIN SET FRONTEND CONFIG](../sql-reference/sql-statements/Administration/ADMIN%20SET%20CONFIG.md) 命令设置。 |
