...
- **同步刷新**：同步刷新方式下，在有数据被导入基表的时候，物化视图将同时更新。该刷新方式通过两阶段提交的方式保证物化视图和基表之间的数据强一致。当前版本中，仅单表物化视图支持同步刷新方式。除此之外，同步刷新模式的物化视图不支持直接查询。
...
您可以通过 SHOW ALTER MATERIALIZED VIEW 命令查看当前数据仓库中所有同步物化视图刷新任务。
...
> **注意**
>
> 您可以对异步刷新和手动刷新方式的物化视图手动调用物化视图，但不能通过该命令手动刷新同步刷新方式的物化视图。
