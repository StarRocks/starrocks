...
动态分区的时间粒度，取值为 `HOUR`、`DAY`、`WEEK`、`MONTH` 或 `YEAR` (自 v2.5.11 起)。时间粒度会决定动态创建的分区名后缀格式。  <br />取值为 `HOUR` 时，仅支持Datetime类型，动态创建的分区名后缀格式为 yyyyMMddHH，例如 `2020032101`。<br />取值为 `DAY` 时，动态创建的分区名后缀格式为 yyyyMMdd，例如 `20200321`。<br />取值为 `WEEK` 时，动态创建的分区名后缀格式为 yyyy_ww，例如 `2020_13` 代表 2020 年第 13 周。<br />取值为 `MONTH` 时，动态创建的分区名后缀格式为 yyyyMM，例如 `202003`。 <br />取值为 `YEAR` 时 (自 v2.5.11 起)，动态创建的分区名后缀格式为 yyyy，例如 `2020`。<br />**注意**<br />在 v2.5.11 之前版本，动态分区不支持时间粒度为 `YEAR`，手动和自动创建分区支持时间粒度为 `YEAR`。|
