...
public class CallStubGenerator {
    ...
    public static byte[] generateCallStubV(Class clazz, Method method) {
        ...
        desc.append("I");
        desc.append(Type.getDescriptor(clazz));
        ...
        batchCall.visitMethodInsn(INVOKEVIRTUAL, Type.getInternalName(clazz), method.getName(),
        ...
        batchCall.visitLocalVariable("obj", "L" + Type.getInternalName(clazz) + ";", null, l0, l5, 1);
        int padding = 2;
        for (int i = 0; i < parameters.length; ++i) {
            final Class<?> type = parameters[i].getType();
            batchCall.visitLocalVariable("var" + i, "L" + Type.getInternalName(type) + ";", null, l1, l5, i);
        }
