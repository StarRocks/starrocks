...
"""
This module is starrocks builtin functions
"""
...
//   https://github.com/apache/incubator-doris/blob/master/gensrc/script/gen_builtins_functions.py \n\
// \n\
...
def add_function(fn_meta_data, user_visible):
    ...
    assert 4 <= len(fn_meta_data) <= 6, \
            "Invalid function entry in starrocks_builtins_functions.py:\n\t" + repr(fn_meta_data)
    entry = {}
...
def generate_fe_registry_init(filename):
    ...
    for entry in starrocks_builtins_functions.non_null_result_with_null_param_functions:
...
for function in starrocks_builtins_functions.visible_functions:
    add_function(function, True)
for function in starrocks_builtins_functions.invisible_functions:
