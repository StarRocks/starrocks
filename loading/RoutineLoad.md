...
<<<<<<< HEAD
> - 空值用 `\\N` 表示。比如，数据文件一共有三列，其中某行数据的第一列、第三列数据分别为 `a` 和 `b`，第二列没有数据，则第二列需要用 `\\N` 来表示空值，写作 `a,\\N,b`，而不是 `a,,b`。`a,,b` 表示第二列是一个空字符串。
>>>>>>> 7be62748 (Add \\N descriptions to load docs (#4464))
=======
> - 空值用 `\N` 表示。比如，数据文件一共有三列，其中某行数据的第一列、第三列数据分别为 `a` 和 `b`，第二列没有数据，则第二列需要用 `\N` 来表示空值，写作 `a,\N,b`，而不是 `a,,b`。`a,,b` 表示第二列是一个空字符串。
>>>>>>> 14340a0a (suggest-users-to-manually-specify-null-values-in-csv-data-as-\N (#4526))
