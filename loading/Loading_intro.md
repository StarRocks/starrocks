...
异步导入是指您创建导入作业以后，StarRocks 直接返回作业创建结果。如果导入作业创建成功，StarRocks 会异步执行导入作业。但作业创建成功并不代表数据导入已经成功。您需要通过轮询的方式查看导入作业的状态，并且根据导入作业的状态来判断数据导入是否成功。如果导入作业创建失败，可以根据失败信息，判断是否需要重试。
...
| Routine Load       | 从 Apache Kafka® 导入实时数据流。                | 微批导入 MB 到 GB 级             | Kafka                                        | - CSV<br> - JSON          | 异步     |
