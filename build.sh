...
if [ ${FE_MODULES}x != ""x ]; then
    ...
    ${MVN_CMD} $addon_mvn_opts package -am -pl ${FE_MODULES} -DskipTests -Dmaven.clean.skip=true -Dproguard-cfg=${PROGUARD} -Dskip-proguard=${SKIP_PROGUARD} -T ${PARALLEL}
    cd ${STARROCKS_HOME}/java-extensions
...
if [ ${BUILD_FE} -eq 1 -o ${BUILD_SPARK_DPP} -eq 1 ]; then
    if [ ${BUILD_FE} -eq 1 ]; then
        ...
        cp -r -p ${STARROCKS_HOME}/fe/fe-core/target/multi-warehouse-ext.jar ${STARROCKS_OUTPUT}/fe/lib/
