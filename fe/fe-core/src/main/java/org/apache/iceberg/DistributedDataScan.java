...
public class DistributedDataScan
    ...
    private CloseableIterable<FileScanTask> planFileTasksRemotely(List<ManifestFile> deleteManifests) {
        ...
        try (Timer ignored = Tracers.watchScope(EXTERNAL, "ICEBERG.collectMetadata")) {
            ...
            collectJob.build();
            resultBatches = MetadataExecutor.executeDQL(collectJob);
        }
    ...
    private List<DataFile> deserializedMetadata(List<TResultBatch> resultBatches) {
    ...
    private PartitionData buildPartitionData(TIcebergMetadata thrift) {
        ...
        PartitionData partitionDataTemplate;
        try (Timer ignored = Tracers.watchScope(EXTERNAL, "ICEBERG.deserializePartitionData")) {
            partitionDataTemplate = partitionDataTemplates.computeIfAbsent(specId, k -> new PartitionData(spec.partitionType()));
            partitionValues = SerializationUtil.deserializeFromBytes(thrift.getPartition_data());
        }

        return new PartitionData(partitionDataTemplate, partitionValues);
    }
    ...
    private List<FileScanTask> toFileTasks(
        ...
        try (Timer ignored = Tracers.watchScope(EXTERNAL, "ICEBERG.toFileTasks")) {
