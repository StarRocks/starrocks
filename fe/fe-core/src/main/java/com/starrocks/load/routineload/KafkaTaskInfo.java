...
public class KafkaTaskInfo extends RoutineLoadTaskInfo {
    ...
    public TRoutineLoadTask createRoutineLoadTask() throws UserException {
        ...
        Database database = GlobalStateMgr.getCurrentState().getMetastore().getDb(routineLoadJob.getDbId());
        if (database == null) {
        ...
        Table tbl = GlobalStateMgr.getCurrentState().getMetastore().getTable(database.getId(), routineLoadJob.getTableId());
        if (tbl == null) {
