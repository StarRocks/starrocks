...
public class GlobalTransactionMgr implements MemoryTrackable {
    ...
    public TransactionStateSnapshot getLabelStatus(long dbId, String label) {
        return getLabelStatus(dbId, label, true);
    }

    // See DatabaseTransactionMgr#getLabelState(String, boolean): internal lookups made while serving one
    // recovered request pass false so the cache-hit metric counts requests rather than lookups.
    public TransactionStateSnapshot getLabelStatus(long dbId, String label, boolean countCacheHit) {
        try {
            DatabaseTransactionMgr dbTransactionMgr = getDatabaseTransactionMgr(dbId);
            return dbTransactionMgr.getLabelState(label, countCacheHit);
