...
class TxnTerminalStateCache {
    ...
    /**
     * Upper bound on the abort reason retained per record. The reason arrives from the client on the
     * thrift abort path and carries no length limit of its own, while this cache retains up to
     * {@code transaction_terminal_state_cache_num} records per database for {@code label_keep_max_second},
     * on every FE, and serializes all of them into the image. Bounding it here keeps that retention cost
     * predictable. The field is purely diagnostic: it is surfaced in the commit-failed message and in
     * status responses, and nothing parses it.
     */
    private static final int MAX_REASON_LENGTH = 1024;

    /**
     * @return the reason to retain. A {@code VISIBLE} outcome has no failure reason to report, so nothing
     * is kept for it; an {@code ABORTED} reason is truncated to {@link #MAX_REASON_LENGTH}.
     */
    private static String boundReason(TransactionStatus status, String reason) {
        if (status != TransactionStatus.ABORTED || reason == null || reason.isEmpty()) {
            return "";
        }
    ...
    private void insert(long txnId, String label, TransactionStatus status, String reason, long finishTime,
        ...
        Record r = new Record(txnId, label, status, boundReason(status, reason), finishTime, sourceType);
        byTxnId.put(txnId, r);
    ...
    /**
     * Approximate retained heap of this cache, for the FE memory tracker. Measured at roughly 350 bytes
     * per record with realistic labels and abort reasons, which covers the record itself, its strings,
     * and the entries in both the id map and the label index that hold it.
     */
    private static final long ESTIMATED_BYTES_PER_RECORD = 350;

    synchronized long estimateSize() {
        return (long) byTxnId.size() * ESTIMATED_BYTES_PER_RECORD;
    }
