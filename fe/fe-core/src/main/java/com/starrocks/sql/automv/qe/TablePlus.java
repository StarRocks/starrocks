...
public class TablePlus {
    ...
    public String getCreateTableSql() {
        ...
        List<String> columnDefinitions = columns.stream()
                .map(column -> column.toSqlWithoutAggregateTypeName(table.getIdToColumn())).collect(Collectors.toList());
    ...
    private TieredMap<Integer, ColumnAlias> getColumnAliases() {
        List<Column> columns = table.getFullSchema();
        return IntStream.range(0, columns.size()).boxed()
                .collect(TieredMap.toMap(Function.identity(), i -> ColumnAlias.of(null, columns.get(i).getName())));
    }
    ...
    public String getSelectSql(List<String> items, List<Op> conjuncts) {
        ...
        if (ObjectUtils.isNotEmpty(conjuncts)) {
            Optional<List<String>> optConditions =
                    OpUtil.conjunctsToSql(conjuncts, OpUtil.toOpToSqlConverter(getColumnAliases()));
            Preconditions.checkArgument(optConditions.isPresent());
            List<String> conditions = optConditions.get();
            printer.newLine().add("WHERE").newLine();
            printer.indentEnclose(() -> printer.addItemsWithNlDel("AND ", conditions));
        }
        return printer.getResult();
    }
}
