...
public class MergeIntoPlanner {
    ...
    public ExecPlan plan(MergeIntoStmt mergeIntoStmt, ConnectContext session) {
        return plan(mergeIntoStmt, session, AIProviderBindings.EMPTY);
    }

    public ExecPlan plan(MergeIntoStmt mergeIntoStmt, ConnectContext session, AIProviderBindings aiProviderBindings) {
        ...
        return createMergePlan(mergeIntoStmt, session, logicalPlan.getRootBuilder().getRoot(),
                columnRefFactory, outputColumns, colNames, icebergTable, requiredProperty, aiProviderBindings);
    }
