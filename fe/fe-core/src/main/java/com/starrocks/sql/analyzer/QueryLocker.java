...
public class QueryLocker {
    ...
    public QueryLocker(ConnectContext session, StatementBase statementBase) {
    ...
    private class TableCollector extends AstTraverser<Void, Void> {
        ...
        private void put(Pair<Database, Table> dbAndTable) {
            if (dbAndTable != null) {
                ...
                if (!dbs.containsKey(database.getId())) {
                    dbs.put(database.getId(), database);
                }

                if (!tables.containsKey(database.getId())) {
                    tables.put(database.getId(), new HashSet<>());
                }

                tables.get(database.getId()).add(table.getId());
            }
        }
    }
}
