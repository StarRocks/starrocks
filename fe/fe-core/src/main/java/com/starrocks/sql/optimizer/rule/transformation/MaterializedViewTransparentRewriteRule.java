...
public class MaterializedViewTransparentRewriteRule extends TransformationRule {
    ...
    private OptExpression doTransform(ConnectContext connectContext,
        ...
        MaterializationContext mvContext = MvRewritePreprocessor.buildMaterializationContext(context,
                mv, mvPlanContext, Sets.newHashSet(), ConstantOperator.TRUE, queryTables);

        OptExpression mvTransparentPlan = doGetMvTransparentPlan(mvContext, mvPlanContext, olapScanOperator, input, mv);
        Preconditions.checkState(mvTransparentPlan != null,
                "Build mv transparent plan failed: %s", mv.getName());
        ...
        mvTransparentPlan = mergeProjection(mvTransparentPlan, originalProjectionMap);

        // merge predicate
        if (olapScanOperator.getPredicate() != null) {
            MvUtils.addExtraPredicate(mvTransparentPlan, olapScanOperator.getPredicate());
        }

        // derive logical property
        deriveLogicalProperty(mvTransparentPlan);

        // mark transparent rewrite
        setOptScanOpsHavePushDown(mvTransparentPlan);
        return mvTransparentPlan;
    }
    ...
    public static void setOptScanOpsHavePushDown(OptExpression input) {
        ...
        scanOps.stream().forEach(op -> op.setOpRuleMask(op.getOpRuleMask() | Operator.OP_TRANSPARENT_MV_BIT));
    }

    /**
     * Wrap getTransparentPlan and getDefinedQueryPlan methods to ensure the transparent plan is returned if possible.
     */
    private OptExpression doGetMvTransparentPlan(MaterializationContext mvContext,
    ...
    private OptExpression getMvTransparentPlan(MaterializationContext mvContext,
        ...
        MVToRefreshPartitionInfo mvToRefreshPartitionInfo = mvContext.getMvToRefreshPartitionInfo();
        MVCompensation mvCompensation = getMvCompensation(mvContext, mvToRefreshPartitionInfo);
    ...
    private static MVCompensation getMvCompensation(MaterializationContext mvContext,
        ...
        Set<String> refTablePartitionNamesToRefresh = mvToRefreshPartitionInfo.getRefTableUpdatedPartitionNames();
        ...
        if (refBaseTable.isNativeTableOrMaterializedView()) {
            ...
            Map<String, Range<PartitionKey>> refTablePartitionNameWithRanges =
                    mvToRefreshPartitionInfo.getRefTablePartitionNameWithRanges();
