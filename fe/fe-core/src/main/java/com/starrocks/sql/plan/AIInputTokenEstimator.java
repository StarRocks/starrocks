...
final class AIInputTokenEstimator {
    // The request contains fixed chat framing in addition to the user prompt. This is a statistical reserve,
    // not provider-specific tokenization or an upper bound on billed tokens.
    ...
    private static AIInputTokenEstimate estimateCall(CallOperator call, OptExpression expression, boolean unpartitioned) {
        ...
        BigInteger perRow = text.bytes().setScale(0, RoundingMode.CEILING).toBigIntegerExact();
        if (descriptor.capability() == AICapability.CHAT) {
            perRow = perRow.add(CHAT_FRAMING_TOKENS);
        }
        return AIInputTokenEstimate.estimated(perRow.multiply(rows));
    }
    ...
    private static TextSize columnSize(ColumnRefOperator column, OptExpression expression,
        ...
        if (operator instanceof PhysicalOlapScanOperator scan && isBaseTableScan(scan)) {
            ...
            // Native string statistics are collected with CHAR_LENGTH. Do not treat characters as UTF-8 bytes.
