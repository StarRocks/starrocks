...
public class OutFileClause implements ParseNode {
    ...
    private boolean isSupportedTypeForParquetOutFile(Type type) {
        ...
        if (type.isMapType()) {
            ...
            return isSupportedTypeForParquetOutFile(mapType.getKeyType()) &&
                    isSupportedTypeForParquetOutFile(mapType.getValueType());
        }
    ...
    private void setBrokerProperties() {
        boolean outfileWithoutBroker = false;
        if (!properties.containsKey(PROP_BROKER_NAME)) {
            outfileWithoutBroker = true;
        }
        String brokerName = null;
        if (!outfileWithoutBroker) {
