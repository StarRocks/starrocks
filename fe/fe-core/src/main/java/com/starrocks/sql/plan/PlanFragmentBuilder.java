...
public class PlanFragmentBuilder {
    ...
    private static ExecPlan finalizeFragments(ExecPlan execPlan, TResultSinkType resultSinkType) {
        ...
        } else if (ConnectContext.get() != null && ConnectContext.get().getSessionVariable().isEnablePipelineAdaptiveDop()) {
            // TODO(lzh): support query cache for adaptive DOP.
            for (PlanFragment fragment : fragments) {
                if (fragment.canUseAdaptiveDop()) {
                    fragment.useAdaptiveDop();
                }
            }
        }
