...
public class AuthorizationAnalyzer {
    ...
    public static class AuthorizationAnalyzerVisitor implements AstVisitor<Void, ConnectContext> {
        ...
        public Void visitGrantRevokePrivilegeStatement(BaseGrantRevokePrivilegeStmt stmt, ConnectContext session) {
            ...
            try {
                ...
                if (objectType.equals(ObjectType.USER)) {
                    List<User> users = analyzeUserPrivToken(stmt);
                    for (User user : users) {
        ...
        public List<User> analyzeUserPrivToken(BaseGrantRevokePrivilegeStmt stmt) {
            List<User> userIdentities = new ArrayList<>();
            if (stmt.isGrantOnALL()) {
                ...
                for (User userIdentity : stmt.getUserPrivilegeObjectList()) {
                    AuthenticationAnalyzer.analyzeUser(stmt.getUser());
                    AuthenticationAnalyzer.checkUserExist(stmt.getUser(), true);
        ...
        public Void visitShowGrantsStatement(ShowGrantsStmt stmt, ConnectContext session) {
            if (stmt.getUser() != null) {
                ...
                User user = new User(userIdentity.getUser(), userIdentity.getHost(), userIdentity.isDomain());
                stmt.setUser(user);
            }
