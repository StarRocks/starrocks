...
public class BitmapRewriteEquivalent extends IAggregateRewriteEquivalent {
    ...
    private CallOperator makeBitmapUnionCountFunc(ScalarOperator arg0) {
        return new CallOperator(BITMAP_UNION_COUNT, StandardTypes.BIGINT,
                Arrays.asList(arg0), ExprUtils.getBuiltinFunction(BITMAP_UNION_COUNT, new Type[] {StandardTypes.BITMAP},
                        IS_IDENTICAL));
    }

    private CallOperator makeBitmapUnionFunc(ScalarOperator arg0) {
        return new CallOperator(BITMAP_UNION, StandardTypes.BITMAP,
                Arrays.asList(arg0), ExprUtils.getBuiltinFunction(BITMAP_UNION, new Type[] {StandardTypes.BITMAP},
                        IS_IDENTICAL));
    }

    private CallOperator makeBitmapCountFunc(ScalarOperator arg0) {
        return new CallOperator(FunctionSet.BITMAP_COUNT, StandardTypes.BIGINT,
                Arrays.asList(arg0), ExprUtils.getBuiltinFunction(FunctionSet.BITMAP_COUNT, new Type[] {StandardTypes.BITMAP},
                        IS_IDENTICAL));
    }
