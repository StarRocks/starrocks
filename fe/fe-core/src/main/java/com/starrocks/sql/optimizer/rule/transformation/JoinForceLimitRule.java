...
public class JoinForceLimitRule extends TransformationRule {
    ...
    public boolean check(OptExpression input, OptimizerContext context) {
        LogicalOperator join = (LogicalOperator) input.getOp();
        LogicalOperator left = (LogicalOperator) input.getInputs().get(0).getOp();
        LogicalOperator right = (LogicalOperator) input.getInputs().get(1).getOp();
        ...
        return (nodeLimit > leftLimit && left.getOpType() != OperatorType.LOGICAL_VALUES &&
                left.getOpType() != OperatorType.LOGICAL_LIMIT) ||
                (nodeLimit > rightLimit && right.getOpType() != OperatorType.LOGICAL_VALUES &&
                        right.getOpType() != OperatorType.LOGICAL_LIMIT);
    }
