...
public class ColumnDefAnalyzer {
    ...
    public static void analyze(ColumnDef columnDef, boolean isOlap) throws AnalysisException {
        ...
        TypeDef typeDef = columnDef.getTypeDef();
        Boolean isPartitionColumn = columnDef.isPartitionColumn();
        String charsetName = columnDef.getCharsetName();
        AggregateType aggregateType = columnDef.getAggregateType();
        AggStateDesc aggStateDesc = columnDef.getAggStateDesc();
        boolean isKey = columnDef.isKey();
        boolean isAllowNull = columnDef.isAllowNull();
        ColumnDef.DefaultValueDef defaultValueDef = columnDef.getDefaultValueDef();
        String defaultCharset = columnDef.getDefaultCharset();
        ...
        if (aggregateType != null && aggregateType != AggregateType.NONE) {
            ...
            if (!checkAggregationTypeCompatibility(aggregateType, type)) {
                throw new AnalysisException(
        ...
        if (type.isHllType()) {
            ...
            defaultValueDef = ColumnDef.DefaultValueDef.EMPTY_VALUE;
        }
    ...
    public static boolean checkAggregationTypeCompatibility(AggregateType aggregateType, Type type) {
