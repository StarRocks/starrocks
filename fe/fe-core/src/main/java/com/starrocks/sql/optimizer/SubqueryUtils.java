...
public class SubqueryUtils {
    ...
    public static CallOperator createCountRowsOperator() {
        Function count = getAggregateFunction(FunctionSet.COUNT, new Type[] {StandardTypes.BIGINT});
        return new CallOperator(FunctionSet.COUNT, StandardTypes.BIGINT, Lists.newArrayList(ConstantOperator.createBigint(1)),
                count, false);
    }

    public static CallOperator createCountRowsOperator(ScalarOperator column) {
        Function count = getAggregateFunction(FunctionSet.COUNT, new Type[] {StandardTypes.BIGINT});
        return new CallOperator(FunctionSet.COUNT, StandardTypes.BIGINT, Lists.newArrayList(column), count, false);
    }
