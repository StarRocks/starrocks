...
public class InsertPartitionEstimator {
    ...
    public static long estimatePartitionCount(InsertStmt insertStmt,
        ...
        // 1. For static partition INSERT, return 1
        ...
        if (hasWherePredicate) {
            long predicateEstimate = estimateFromPredicates(insertStmt, table, existingPartitionCount, ndv);
            if (predicateEstimate >= 0) {
                return predicateEstimate;
            }
            // Predicate estimation failed with WHERE clause, return -1
            return -1;
        }
