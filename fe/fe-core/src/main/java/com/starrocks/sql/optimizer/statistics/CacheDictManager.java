...
public class CacheDictManager implements IDictManager {
    ...
    private ColumnDict deserializeColumnDict(TStatisticData statisticData) throws AnalysisException {
        java.nio.ByteBuffer dictBuffer = statisticData.dict;
        HashMap<String, Integer> dict = Maps.newHashMap();
        while (dictBuffer.hasRemaining()) {
            dictBuffer.order(ByteOrder.LITTLE_ENDIAN);
            int code = dictBuffer.getInt();
            int stringSize = dictBuffer.getInt();
            byte[] bytes = new byte[stringSize];
            dictBuffer.order(ByteOrder.BIG_ENDIAN);
            dictBuffer.get(bytes);
            String stringContent = new String(bytes);
            dict.put(stringContent, code);
        }
        return new ColumnDict(dict, statisticData.meta_version);
    }
