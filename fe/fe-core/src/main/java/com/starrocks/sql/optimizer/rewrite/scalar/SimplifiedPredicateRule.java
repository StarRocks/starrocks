...
public class SimplifiedPredicateRule extends BottomUpScalarOperatorRewriteRule {
            ...
            case AND: {
                if (constantChildren.stream().anyMatch(d -> (!d.isNull() && !d.getBoolean()))) {
                    ...
                    if (constantChildren.stream().anyMatch(ConstantOperator::isNull)) {
                        ...
                        return ConstantOperator.createNull(StandardTypes.BOOLEAN);
                    }
                    // true and true
    ...
    public ScalarOperator visitLikePredicateOperator(LikePredicateOperator predicate,
        ...
        if (rightOp.getType() != StandardTypes.VARCHAR && rightOp.getType() != StandardTypes.CHAR) {
            return predicate;
        }
    ...
    private ScalarOperator simplifiedTimeFns(CallOperator call) {
        ...
        if (!call.getChild(1).isConstantRef() || !StandardTypes.INT.equals(call.getChild(1).getType())) {
            return call;
        }
        ...
        if (!child.getChild(1).isConstantRef() || !StandardTypes.INT.equals(child.getChild(1).getType())) {
            return call;
        }
    ...
    private static ScalarOperator simplifiedHourFromUnixTime(CallOperator call) {
        ...
        if (toDatetime != null) {
            ...
            if (args.size() == 1) {
                ...
                if (!StandardTypes.INT.equals(scale.getType()) || scale.isNull()) {
                    return call;
                }
                ...
                if (scaleVal == 0) {
                    ...
                    unixtimeArgForHour = new CallOperator(FunctionSet.DIVIDE, StandardTypes.BIGINT,
