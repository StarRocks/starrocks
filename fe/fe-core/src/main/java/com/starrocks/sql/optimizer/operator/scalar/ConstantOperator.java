...
public final class ConstantOperator extends ScalarOperator implements Comparable<ConstantOperator> {
    ...
    // return the next value of current value
    // for float/double/decimal/string/boolean, there is no previous value
    public Optional<ConstantOperator> nextValue() {
        if (type.isTinyint()) {
            if (getTinyInt() == Byte.MAX_VALUE) {
                return Optional.empty();
            } else {
                int next = getTinyInt() + (byte) 1;
                return Optional.of(ConstantOperator.createTinyInt((byte) next));
            }
        } else if (type.isSmallint()) {
            if (getSmallint() == Short.MAX_VALUE) {
                return Optional.empty();
            } else {
                int next = getSmallint() + 1;
                return Optional.of(ConstantOperator.createSmallInt((short) next));
            }
        } else if (type.isInt()) {
            if (getInt() == Integer.MAX_VALUE) {
                return Optional.empty();
            } else {
                return Optional.of(ConstantOperator.createInt(getInt() + 1));
            }
        } else if (type.isBigint()) {
            if (getBigint() == Long.MAX_VALUE) {
                return Optional.empty();
            } else {
                return Optional.of(ConstantOperator.createBigint(getBigint() + 1));
            }
        } else if (type.isLargeint()) {
            return Optional.of(ConstantOperator.createLargeInt(getLargeInt().add(BigInteger.valueOf(1))));
        } else if (type.isDateType()) {
            LocalDateTime dateLiteral = (LocalDateTime) value;
            if (dateLiteral.toLocalDate().isBefore(LocalDate.MAX)) {
                return Optional.of(ConstantOperator.createDate(dateLiteral.plus(1, ChronoUnit.DAYS)));
            } else {
                return Optional.empty();
            }
        } else if (type.isDatetime()) {
            LocalDateTime dateLiteral = (LocalDateTime) value;
            if (dateLiteral.isBefore(LocalDateTime.MAX)) {
                return Optional.of(ConstantOperator.createDatetime(dateLiteral.plus(1, ChronoUnit.SECONDS)));
            } else {
                return Optional.empty();
            }
        } else {
    ...
    // return the next value of current value
    // for float/double/decimal/string/boolean, there is no previous value
    public Optional<ConstantOperator> previousValue() {
        if (type.isTinyint()) {
            if (getTinyInt() == Byte.MIN_VALUE) {
                return Optional.empty();
            } else {
                int pre = getTinyInt() - (byte) 1;
                return Optional.of(ConstantOperator.createTinyInt((byte) pre));
            }
        } else if (type.isSmallint()) {
            if (getSmallint() == Short.MIN_VALUE) {
                return Optional.empty();
            } else {
                int pre = getSmallint() - 1;
                return Optional.of(ConstantOperator.createSmallInt((short) pre));
            }
        } else if (type.isInt()) {
            if (getInt() == Integer.MIN_VALUE) {
                return Optional.empty();
            } else {
                return Optional.of(ConstantOperator.createInt(getInt() - 1));
            }
        } else if (type.isBigint()) {
            if (getBigint() == Long.MIN_VALUE) {
                return Optional.empty();
            } else {
                return Optional.of(ConstantOperator.createBigint(getBigint() - 1));
            }
        } else if (type.isLargeint()) {
            return Optional.of(ConstantOperator.createLargeInt(getLargeInt().subtract(BigInteger.valueOf(1))));
        } else if (type.isDateType()) {
            LocalDateTime dateLiteral = (LocalDateTime) value;
            if (dateLiteral.toLocalDate().isAfter(LocalDate.MIN)) {
                return Optional.of(ConstantOperator.createDate(dateLiteral.minus(1, ChronoUnit.DAYS)));
            } else {
                return Optional.empty();
            }
        } else if (type.isDatetime()) {
            LocalDateTime dateLiteral = (LocalDateTime) value;
            if (dateLiteral.isAfter(LocalDateTime.MIN)) {
                return Optional.of(ConstantOperator.createDatetime(dateLiteral.minus(1, ChronoUnit.SECONDS)));
            } else {
                return Optional.empty();
            }
        } else {
