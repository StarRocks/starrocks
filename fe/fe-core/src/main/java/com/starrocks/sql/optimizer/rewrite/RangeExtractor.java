...
public class RangeExtractor extends ScalarOperatorVisitor<Void, Void> {
    ...
    public Void visitBinaryPredicate(BinaryPredicateOperator predicate, Void context) {
        if (predicate.getChild(1).isConstantRef() && predicate.getBinaryType() != BinaryPredicateOperator.BinaryType.NE
                && predicate.getBinaryType() != BinaryPredicateOperator.BinaryType.EQ_FOR_NULL) {

            if (predicate.getChild(0).getType().isStringType() &&
                    predicate.getBinaryType() != BinaryPredicateOperator.BinaryType.EQ) {
    ...
    public Void visitCompoundPredicate(CompoundPredicateOperator predicate, Void context) {
        ...
        if (predicate.isOr()) {
            ...
            for (Map.Entry<ScalarOperator, ValueDescriptor> entry : intersectMap.entrySet()) {
                if (leftMap.containsKey(entry.getKey())) {
                    ValueDescriptor l = leftMap.get(entry.getKey());
                    if (l instanceof RangeDescriptor && entry.getValue() instanceof RangeDescriptor) {
                        if (((RangeDescriptor) l).range.encloses(((RangeDescriptor) entry.getValue()).range)) {
                            descMap.remove(entry.getKey());
                        }
                    }
                }

                if (rightMap.containsKey(entry.getKey())) {
                    ValueDescriptor l = rightMap.get(entry.getKey());
                    if (l instanceof RangeDescriptor && entry.getValue() instanceof RangeDescriptor) {
                        if (((RangeDescriptor) entry.getValue()).range.encloses(((RangeDescriptor) l).range)) {
                            descMap.remove(entry.getKey());
                        }
                    }
                }
