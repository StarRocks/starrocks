...
public class MetaFunctions {
    ...
    public static Pair<Database, Table> inspectTable(TableName tableName) {
        Database db = GlobalStateMgr.getCurrentState().getStarRocksMetadata().mayGetDb(tableName.getDb())
                .orElseThrow(() -> ErrorReport.buildSemanticException(ErrorCode.ERR_BAD_DB_ERROR, tableName.getDb()));
        Table table = GlobalStateMgr.getCurrentState().getStarRocksMetadata().mayGetTable(tableName.getDb(), tableName.getTbl())
                .orElseThrow(() -> ErrorReport.buildSemanticException(ErrorCode.ERR_BAD_TABLE_ERROR, tableName));
        ConnectContext connectContext = ConnectContext.get();
        try {
    ...
    public static ConstantOperator inspectRelatedMv(ConstantOperator name) {
        ...
        if (table.isNativeTableOrMaterializedView()) {
            mayDb = GlobalStateMgr.getCurrentState().getStarRocksMetadata().mayGetDb(tableName.getDb());
        } else {
        ...
        try {
            ...
            for (MvId mv : SetUtils.emptyIfNull(relatedMvs)) {
                String mvName = GlobalStateMgr.getCurrentState().getStarRocksMetadata().mayGetTable(mv.getDbId(), mv.getId())
                        .map(Table::getName)
                        .orElse(null);
    ...
    public static ConstantOperator inspectAllPipes() {
        ...
        Database db = GlobalStateMgr.getCurrentState().getStarRocksMetadata().mayGetDb(connectContext.getDatabase())
