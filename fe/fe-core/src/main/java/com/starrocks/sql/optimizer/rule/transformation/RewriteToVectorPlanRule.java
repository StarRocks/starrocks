...
public class RewriteToVectorPlanRule extends TransformationRule {
    ...
    public boolean check(OptExpression input, OptimizerContext context) {
        ...
        if (input.getInputs().size() < 1 || !sessionVariable.isEnableUseANN() ||
                (sessionVariable.isEnableUseANN() && (sessionVariable.getANNType().equalsIgnoreCase(ANNAlgorithmType.TYPE_BRUTE)
                        || sessionVariable.getANNType().equalsIgnoreCase(ANNAlgorithmType.TYPE_DEFAULT)))) {
            return false;
        }
    ...
    public List<OptExpression> transform(OptExpression input, OptimizerContext context) {
        ...
        sessionVariable.setVectorLimitK(topNOperator.getLimit());
        LogicalScanOperator scanOperator = (LogicalScanOperator) input.getInputs().get(0).getOp();
        ...
        if (predicate != null) {
            newPredicate = findAndSetVectorRange(predicate, sessionVariable, isAscending);
            if (!sessionVariable.isEnableUseANN()) {
                return Lists.newArrayList(input);
            }
        }
        sessionVariable.setResultOrder(isAscending ? 0 : 1);
        ...
        if (functionName.equalsIgnoreCase(APPROX_L2_DISTANCE) && !isAscending ||
                functionName.equalsIgnoreCase(APPROX_COSINE_SIMILARITY) && isAscending ||
                    !sessionVariable.isEnableUseANN()) {
            sessionVariable.setEnableUseANN(false);
            return Lists.newArrayList(input);
        }
        sessionVariable.setIsRewrittenVectorPlan(true);
        if (sessionVariable.isUseIVFPQ()) {
            input.getInputs().get(0).getOp()
                    .setPredicate(newPredicate.isPresent() ? newPredicate.get() : null);
            return Lists.newArrayList(input);
        }

        OptExpression result = buildVectorSortScanOperator(topNOperator,
                (LogicalOlapScanOperator) scanOperator, context, newPredicate);
        return Lists.newArrayList(result);
    }

    public OptExpression buildVectorSortScanOperator(LogicalTopNOperator topNOperator,
            LogicalOlapScanOperator scanOperator, OptimizerContext context, Optional<ScalarOperator> newPredicate) {
        // bottom-up
        String distanceColumnName = context.getSessionVariable().getVectorDistanceColumnName();
        ...
        scanOperator.getProjection().getColumnRefMap().entrySet().stream()
                .forEach(entry -> {
                    if (FunctionSet.VECTOR_COMPUTE_FUNCTIONS.contains(entry.getKey().getName())
                        ...
                        projectMap.put(distanceColumnRefOperator, distanceColumnRefOperator);
                    } else {
                        projectMap.put(entry.getKey(), entry.getValue());
                    }
                });
        newScanOperator.setProjection(new Projection(projectMap));
        ...
        if (hasProjection) {
            topNOperator.getProjection().getColumnRefMap().entrySet().stream()
                    .forEach(entry -> {
                        if (FunctionSet.VECTOR_COMPUTE_FUNCTIONS.contains(entry.getKey().getName())) {
                            topNProjectMap.put(distanceColumnRefOperator, distanceColumnRefOperator);
                        } else {
        ...
        LogicalTopNOperator newTopNOperator = new LogicalTopNOperator(topNOperator.getLimit(),
                topNOperator.getPredicate(), hasProjection ? new Projection(topNProjectMap) : null,
    ...
    public Optional<ScalarOperator> findAndSetVectorRange(ScalarOperator operator,
                SessionVariable sessionVariable, boolean isAscending) {
        if (!sessionVariable.isEnableUseANN()) {
            return Optional.empty();
        }

        if (operator instanceof BinaryPredicateOperator && operator.getChild(1) instanceof ConstantOperator &&
                (operator.getChild(0).toString().contains("l2") || operator.getChild(0).toString().contains("cosine"))) {
            ...
            sessionVariable.setVectorRange(
            ...
            for (ScalarOperator child : operator.getChildren()) {
                Optional<ScalarOperator> newChild = findAndSetVectorRange(child, sessionVariable, isAscending);
