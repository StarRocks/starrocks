...
public abstract class BaseMaterializedViewRewriteRule extends TransformationRule implements IMaterializedViewRewriteRule {
    ...
    public List<OptExpression> doTransform(OptExpression queryExpression, OptimizerContext context) {
        ...
        List<Pair<OptExpression, MaterializationContext>> results = doTransform(mvCandidateContexts, queryExpression, context);

        // 4. update associated mvs' metrics
        ConnectContext connectContext = ConnectContext.get();
        if (isUpdateMaterializedViewMetrics(connectContext)) {
            for (Pair<OptExpression, MaterializationContext> pair : results) {
                MaterializedViewMetricsEntity mvEntity =
                        MaterializedViewMetricsRegistry.getInstance().getMetricsEntity(pair.second.getMv().getMvId());
                mvEntity.increaseQueryMatchedCount(1L);
            }
        }
        return results.stream().map(p -> p.first).collect(Collectors.toList());
    }
     ...
     */
    protected List<Pair<OptExpression, MaterializationContext>> doTransform(List<MaterializationContext> mvCandidateContexts,
                                                                            OptExpression queryExpression,
                                                                            OptimizerContext context) {
        List<Pair<OptExpression, MaterializationContext>> results = Lists.newArrayList();
