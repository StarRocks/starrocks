...
public final class AIModelConfigs {
    ...
    public static SystemChatConfig systemChatSnapshot(DefaultModelRequirement defaultModelRequirement) {
        ...
        endpoint = requireNonBlank(endpoint, "ai_default_chat_endpoint");
        validateEndpoint(endpoint);

        provider = requireNonBlank(provider, "ai_default_chat_provider");
        ...
        return new SystemChatConfig(endpoint, model, provider);
    }
    ...
    private static void validateEndpoint(String endpoint) {
        ...
        if (!httpsScheme || uri.getHost() == null || uri.getHost().isEmpty()
                || uri.getPort() > 65535 || (rawAuthority != null && rawAuthority.endsWith(":"))
