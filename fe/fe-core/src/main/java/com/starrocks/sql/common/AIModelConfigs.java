...
public final class AIModelConfigs {
    ...
    public static String configId(Function function, List<Expr> arguments) {
        return configId(function, arguments, AIProviderBindings.EMPTY);
    }

    public static String configId(Function function, List<Expr> arguments, AIProviderBindings bindings) {
        if (function.getAiModelSource() == TAIModelSource.PROVIDER) {
            return bindings.configurationId(providerName(arguments.get(getProviderArgument(function))));
        }
    ...
    public static ModelConfig fromProvider(AIProvider provider) {
        ...
        return new ModelConfig(type == AIProviderType.CHAT ? "CHAT" : "TEXT_EMBEDDING", endpoint, model,
                OPENAI_COMPATIBLE_PROVIDER, apiKey, TAIModelSource.PROVIDER, provider.getId(), timeout, dimensions);
    }
    ...
    /** Immutable query snapshot. Credentials are excluded from diagnostic rendering. */
    public record ModelConfig(String capability, String endpoint, String model, String provider, String apiKey,
                              TAIModelSource source, String providerId, Integer timeoutMs, Integer dimensions) {
        public ModelConfig(String capability, String endpoint, String model, String provider) {
            this(capability, endpoint, model, provider, null, TAIModelSource.SYSTEM, null, null, null);
        }
        ...
        public String toString() {
            return "ModelConfig[capability=" + capability + ", source=" + source + ", providerId=" + providerId + "]";
        }
        ...
        public TAIModelConfiguration toThrift() {
            ...
            if (source == TAIModelSource.PROVIDER) {
                ...
                result.setProvider_id(providerId);
            }
