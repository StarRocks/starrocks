...
public class AlterTableClauseAnalyzer implements AstVisitor<Void, ConnectContext> {
    ...
    public Void visitModifyTablePropertiesClause(ModifyTablePropertiesClause clause, ConnectContext context) {
        ...
        if (properties.containsKey(PropertyAnalyzer.PROPERTIES_COLOCATE_WITH)) {
            ...
            if (!properties.get(PropertyAnalyzer.PROPERTIES_IO_MERGE).equalsIgnoreCase("true") &&
                    !properties.get(PropertyAnalyzer.PROPERTIES_IO_MERGE).equalsIgnoreCase("false")) {
                ErrorReport.reportSemanticException(ErrorCode.ERR_COMMON_ERROR,
                        "Property " + PropertyAnalyzer.PROPERTIES_IO_MERGE +
                                " must be bool type(false/true)");
            }
            ...
            boolean ioMerge = properties.get(
                            PropertyAnalyzer.PROPERTIES_IO_MERGE).equalsIgnoreCase("true");
            OlapTable olapTable = (OlapTable) table;
            if (ioMerge == olapTable.isIOMerge()) {
                String msg = String.format("table: %s io_merge is %s, nothing need to do",
                        olapTable.getName(), ioMerge);
                ErrorReport.reportSemanticException(ErrorCode.ERR_COMMON_ERROR, msg);
            }
            
            if (!olapTable.allowUpdateIOMerge()) {
                ErrorReport.reportSemanticException(ErrorCode.ERR_COMMON_ERROR,
                            "Property " + PropertyAnalyzer.PROPERTIES_IO_MERGE +
