...
public class PruneSubColumnsRule implements TreeRewriteRule {
    ...
    public OptExpression rewrite(OptExpression root, TaskContext taskContext) {
        if (!taskContext.getOptimizerContext().getSessionVariable().getEnablePruneComplexTypes()) {
            return root;
        }
        SubColumnAccessPathComputer computer = new SubColumnAccessPathComputer();
        return computer.visit(root, null);
    }
    ...
    private static class SubColumnAccessPathComputer extends OptExpressionVisitor<OptExpression, Void> {
        ...
        public OptExpression visit(OptExpression optExpression, Void context) {
            ...
            for (int i = 0; i < optExpression.getInputs().size(); i++) {
                optExpression.setChild(i, visit(optExpression.inputAt(i), context));
            }
        ...
        public OptExpression visitLogicalTableScan(OptExpression optExpression, Void context) {
            ...
            for (ColumnRefOperator ref : scanColumns) {
                ...
                accessPaths.add(normalizer.normalizePath(ref, columnName));
                paths.add(normalizer.normalizePath(ref, columnName));
            }
    ...
    private static class ComplexColumnAccessPathNormalizer extends ScalarOperatorVisitor<Void, Void> {
        ...
        private ColumnAccessPath normalizePath(ColumnRefOperator root, String columnName) {
            List<AccessPath> paths = allAccessPaths.stream().filter(path -> path.root().equals(root))
                    .sorted(Comparator.comparingInt(o -> o.paths.size()))
