...
public class PercentileRewriteEquivalent extends IAggregateRewriteEquivalent {
    ...
    private CallOperator makePercentileUnion(ScalarOperator replace) {
        Function unionFn = ExprUtils.getBuiltinFunction(FunctionSet.PERCENTILE_UNION, new Type[] { StandardTypes.PERCENTILE },
                IS_IDENTICAL);
        Preconditions.checkState(unionFn != null);
        return new CallOperator(PERCENTILE_UNION, StandardTypes.PERCENTILE, Arrays.asList(replace), unionFn);
    }
    ...
    private CallOperator makePercentileApproxRaw(ScalarOperator replace, ScalarOperator arg1) {
        Function approxRawFn = ExprUtils.getBuiltinFunction(FunctionSet.PERCENTILE_APPROX_RAW,
                new Type[] { StandardTypes.PERCENTILE, StandardTypes.DOUBLE }, Function.CompareMode.IS_IDENTICAL);
        Preconditions.checkState(approxRawFn != null);
        // percentile_approx_raw(percentile_union(input), arg1)
        return new CallOperator(PERCENTILE_APPROX_RAW, StandardTypes.DOUBLE, Arrays.asList(replace, arg1), approxRawFn);
    }
    ...
    private CallOperator makeRollupFunc(ScalarOperator replace, ScalarOperator arg1) {
        Function unionFn = ExprUtils.getBuiltinFunction(FunctionSet.PERCENTILE_UNION, new Type[] { StandardTypes.PERCENTILE },
                IS_IDENTICAL);
        Preconditions.checkState(unionFn != null);
        CallOperator rollup = new CallOperator(PERCENTILE_UNION, StandardTypes.PERCENTILE, Arrays.asList(replace), unionFn);

        Function approxRawFn = ExprUtils.getBuiltinFunction(FunctionSet.PERCENTILE_APPROX_RAW,
                new Type[] { StandardTypes.PERCENTILE, StandardTypes.DOUBLE }, Function.CompareMode.IS_IDENTICAL);
        Preconditions.checkState(approxRawFn != null);
        // percentile_approx_raw(percentile_union(input), arg1)
        return new CallOperator(PERCENTILE_APPROX_RAW, StandardTypes.DOUBLE, Arrays.asList(rollup, arg1), approxRawFn);
    }
