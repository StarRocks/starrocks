...
public class DecodeRewriter extends OptExpressionVisitor<OptExpression, ColumnRefSet> {
    ...
    private OptExpression insertStructuredDecodeNode(
        ...
        // For struct, we need to decode them manually by decoding their fields first and then reconstructing them using
        // a PhysicalProjectOperator.
        ...
        for (int sid : info.inputStringColumns.getColumnIds()) {
            ...
            if (context.stringRefToDicts.containsKey(sid)) {
                dictMap.put(dictRef.getId(), context.stringRefToDicts.get(sid));
                    ...
                    && context.getFieldUseStringRefMap(stringRef) != null) {
                context.getFieldUseStringRefMap(stringRef).values().forEach(c -> {
                    if (context.stringRefToDicts.containsKey(c.getId())) {
                        ColumnRefOperator cDict = context.stringRefToDictRefMap.get(c);
                        dictMap.put(cDict.getId(),  context.stringRefToDicts.get(c.getId()));
                    }
                });
            } else {
    ...
    private Map<Integer, ScalarOperator> computeDictExpr(ColumnRefSet fragmentUseDictExprs) {
        ...
        for (int sid : fragmentUseDictExprs.getColumnIds()) {
            ...
            if (strRef.getType().isStructType()) {
                ...
                Preconditions.checkNotNull(subFields);
