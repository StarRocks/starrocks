...
/**
 * BottomUpRewriteTask performs a bottom-up rewrite logic operators pass.
 * <p>
 * This class modify from CMU noisepage project BottomUpRewrite class
 */
public class BottomUpRewriteTask extends OptimizerTask {
    private final Group group;
    private final RuleSetType ruleSetType;
    private boolean childrenHaveOptimized = false;

    BottomUpRewriteTask(TaskContext context,
                        Group group,
                        RuleSetType ruleSetType,
                        boolean childrenHaveOptimized) {
        super(context);
        this.group = group;
        this.ruleSetType = ruleSetType;
        this.childrenHaveOptimized = childrenHaveOptimized;
    }
    ...
    public void execute() {
        ...
        if (!childrenHaveOptimized) {
            pushTask(new BottomUpRewriteTask(context, group, ruleSetType, true));
            for (Group childGroup : curGroupExpression.getInputs()) {
                pushTask(new BottomUpRewriteTask(context, childGroup, ruleSetType, false));
            }
        }
        ...
        for (Rule rule : validRules) {
            ...
            while (extractExpr != null) {
                ...
                if (!newExpressions.isEmpty()) {
                    ...
                    pushTask(new BottomUpRewriteTask(context, group, ruleSetType, false));
                    return;
                }
                extractExpr = binder.next();
            }

            curGroupExpression.setRuleExplored(rule);
        }
    }
}
