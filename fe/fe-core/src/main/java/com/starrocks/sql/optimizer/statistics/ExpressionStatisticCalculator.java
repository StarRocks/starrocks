        ...
        private ColumnStatistic binaryExpressionCalculate(CallOperator callOperator, ColumnStatistic left,
            ...
            switch (funcName) {
                ...
                case FunctionSet.DATE_TRUNC:
                    String fmt = callOperator.getChild(0).toString().toLowerCase(Locale.ROOT);
                    minValue = right.getMinValue() / INTERVAL_MAP.get(fmt);
                    maxValue = right.getMaxValue() / INTERVAL_MAP.get(fmt);
                    break;
                default:
                    return ColumnStatistic.unknown();
            }
