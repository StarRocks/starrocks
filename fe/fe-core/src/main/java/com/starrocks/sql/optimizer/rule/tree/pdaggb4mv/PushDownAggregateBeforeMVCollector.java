 ...
 */
class PushDownAggregateBeforeMVCollector extends OptExpressionVisitor<Void, AggregatePushDownBeforeMVContext> {
    private static final Logger LOG = LogManager.getLogger(PushDownAggregateBeforeMVCollector.class);
    ...
    private static final List<String> WHITE_FNS = ImmutableList.of(FunctionSet.MAX, FunctionSet.MIN,
            FunctionSet.SUM, FunctionSet.HLL_UNION, FunctionSet.BITMAP_UNION, FunctionSet.PERCENTILE_UNION, FunctionSet.COUNT);
    ...
    private final Map<LogicalAggregationOperator, List<AggregatePushDownBeforeMVContext>> allRewriteContext = Maps.newHashMap();

    public PushDownAggregateBeforeMVCollector(TaskContext taskContext) {
    ...
    Map<LogicalAggregationOperator, List<AggregatePushDownBeforeMVContext>> getAllRewriteContext() {
        return allRewriteContext;
    }

    public void collect(OptExpression root) {
        process(root, new AggregatePushDownBeforeMVContext());
    }

    @Override
    public Void visit(OptExpression optExpression, AggregatePushDownBeforeMVContext context) {
        // forbidden push down
        for (OptExpression input : optExpression.getInputs()) {
            process(input, AggregatePushDownBeforeMVContext.EMPTY);
        }
        return null;
    }

    private Void processChild(OptExpression optExpression, AggregatePushDownBeforeMVContext context) {
        for (OptExpression input : optExpression.getInputs()) {
    ...
    private void process(OptExpression opt, AggregatePushDownBeforeMVContext context) {
    ...
    @Override
    public Void visitLogicalFilter(OptExpression optExpression, AggregatePushDownBeforeMVContext context) {
    ...
    @Override
    public Void visitLogicalProject(OptExpression optExpression, AggregatePushDownBeforeMVContext context) {
        if (isInvalid(optExpression, context)) {
            return visit(optExpression, context);
        }
    ...
    @Override
    public Void visitLogicalAggregate(OptExpression optExpression, AggregatePushDownBeforeMVContext context) {
        ...
        context = new AggregatePushDownBeforeMVContext();
        context.setAggregator(aggregate);
        return processChild(optExpression, context);
    }

    @Override
    public Void visitLogicalJoin(OptExpression optExpression, AggregatePushDownBeforeMVContext context) {
        if (isInvalid(optExpression, context)) {
            return visit(optExpression, context);
        }
        ...
        AggregatePushDownBeforeMVContext leftContext = splitJoinAggregate(optExpression, context, 0);
        AggregatePushDownBeforeMVContext rightContext = splitJoinAggregate(optExpression, context, 1);
    ...
    private AggregatePushDownBeforeMVContext splitJoinAggregate(OptExpression optExpression,
        ...
        childContext.groupBys.values().forEach(c -> allGroupByColumns.union(c.getUsedColumns()));
        ColumnRefSet allAggregateColumns = new ColumnRefSet();
        childContext.aggregations.values().forEach(c -> allAggregateColumns.union(c.getUsedColumns()));
...
//        }
        ...
        allRewriteContext.put(childContext.origAggregator, list);


        // just push down after join
        return AggregatePushDownBeforeMVContext.EMPTY;
    }

    @Override
    public Void visitLogicalUnion(OptExpression optExpression, AggregatePushDownBeforeMVContext context) {
        if (isInvalid(optExpression, context)) {
            return visit(optExpression, context);
        }

        List<PushDownAggregateBeforeMVCollector> collectors = Lists.newArrayList();
        ...
        for (int i = 0; i < optExpression.getInputs().size(); i++) {
            ...
            PushDownAggregateBeforeMVCollector collector = new PushDownAggregateBeforeMVCollector(this.taskContext);
            collectors.add(collector);
            collector.process(optExpression.inputAt(i), childContext);
        }

        // collect push down aggregate context
        List<List<AggregatePushDownBeforeMVContext>> allChildRewriteContext = Lists.newArrayList();
        for (PushDownAggregateBeforeMVCollector childCollector : collectors) {
        ...
        for (PushDownAggregateBeforeMVCollector collector : collectors) {
        ...
        for (List<AggregatePushDownBeforeMVContext> childContexts : allChildRewriteContext) {
    ...
    @Override
    public Void visitLogicalCTEAnchor(OptExpression optExpression, AggregatePushDownBeforeMVContext context) {
        process(optExpression.inputAt(1), context);
        process(optExpression.inputAt(0), AggregatePushDownBeforeMVContext.EMPTY);
        return null;
    }

    @Override
    public Void visitLogicalTableScan(OptExpression optExpression, AggregatePushDownBeforeMVContext context) {
        // do nothing
        return visit(optExpression, context);
    }

    private boolean checkStatistics(AggregatePushDownBeforeMVContext context, ColumnRefSet groupBys, Statistics statistics) {
