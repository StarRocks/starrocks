...
public class MetadataMgr {
    ...
    public void cleanTemporaryTables(UUID sessionId) {
        TemporaryTableMgr temporaryTableMgr = GlobalStateMgr.getCurrentState().getTemporaryTableMgr();
        Session session = temporaryTableMgr.getSession(sessionId);
        if (session == null) {
            return;
        }

        Map<Long, Map<String, Long>> temporaryTables = session.getAllTemporaryTables();
        temporaryTables.forEach((databaseId, tables) -> {
            ...
            tables.forEach((tableName, tableId) -> {
                try {
                    ...
                    temporaryTableMgr.dropTemporaryTable(sessionId, databaseId, tableName);
                } catch (DdlException e) {
                    LOG.error("Failed to drop temporary table {}.{} in session {}",
        ...
        temporaryTableMgr.removeSession(sessionId);
    }
