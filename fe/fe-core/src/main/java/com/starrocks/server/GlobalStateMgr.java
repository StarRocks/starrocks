...
public class GlobalStateMgr {
    ...
    private ReentrantLock journalLock;
    ...
    public boolean usingBDBMetastore = false;
    ...
    /*
    public TabletInvertedIndexIface getTabletInvertedIndex() {
        if (usingBDBMetastore) {
            return new TabletInvertedIndexV2();
        } else {
    ...
    public StarRocksMetadata getStarRocksMetadata() {
        return starRocksMetadata;
    }
    ...
    private GlobalStateMgr(boolean isCkptGlobalState, NodeMgr nodeMgr) {
        ...
        this.starRocksMetadata = new StarRocksMetadata();
        ...
        this.metadataMgr = new MetadataMgr(starRocksMetadata, temporaryTableMgr, connectorMgr, connectorTblMetaInfoMgr);
    ...
    protected void initJournal() throws JournalException, InterruptedException {
        ...
        /*
        if (Config.enable_bdb_metastore) {
            BDBMetaStore bdbMetaStore = new BDBMetaStore();
            long editLogReplayId = bdbMetaStore.getEditLogReplayId();
            if (editLogReplayId >= journal.getMaxJournalId()) {
                usingBDBMetastore = true;
                LOG.info("Using BDB MetaStore");
            } else {
                usingBDBMetastore = false;
                LOG.info("Using Local MetaStore");
            }
        }
         */
    }

    public ReentrantLock getJournalLock() {
        return journalLock;
    }

    public long getNextVisibleJournalId() {
        return journalWriter.getNextVisibleJournalId();
    }
    ...
    private void transferToLeader() {
        ...
        try {
            ...
            if (Config.enable_bdb_metastore) {
                ...
                if (editLogReplayer.load(maxJournalId)) {
                    usingBDBMetastore = true;
                    LOG.info("Using BDB Metastore");
                } else {
                    LOG.info("Using Local Metastore");
    ...
    public void saveKV() {
        localMetastore.saveKV();
        localMetastore.buildTabletIndex();
        //create all tablet index
    }
    ...
    public AlterJobMgrIface getAlterJobMgrIFace() {
        if (isCheckpointThread()) {
            return alterJobMgr;
        } else {
            return null;
