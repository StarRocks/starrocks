...
public class SharedDataStorageVolumeMgr extends StorageVolumeMgr {
    ...
    protected List<List<Long>> getBindingsOfBuiltinStorageVolume() {
        ...
        List<Long> dbIds = GlobalStateMgr.getCurrentState().getStarRocksMetadata().getDbIdsIncludeRecycleBin().stream()
        ...
        for (Long dbId : dbIds) {
            ...
            dbBindings.add(dbId);
            try {
                List<Table> tables = GlobalStateMgr.getCurrentState().getStarRocksMetadata().getTablesIncludeRecycleBin(db);
