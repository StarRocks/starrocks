...
public class LocalMetastore implements ConnectorMetadata, MVRepairHandler, MemoryTrackable {
    ...
    public void addSubPartitions(Database db, OlapTable table, Partition partition,
        ...
        try {
            ...
            addSubPartitionsInternal(db, table, partition, numSubPartition, null, warehouseId);
        } finally {
            table.setAutomaticBucketing(false);
        }
    }

    public void addSubPartitions(Database db, OlapTable table, Partition partition, int numSubPartition,
                                 String[] subPartitionNames, long warehouseId) throws DdlException {
        try {
            table.setAutomaticBucketing(true);
            addSubPartitionsInternal(db, table, partition, numSubPartition, subPartitionNames, warehouseId);
        } finally {
            table.setAutomaticBucketing(false);
        }
    }

    private void addSubPartitionsInternal(Database db, OlapTable table, Partition partition, int numSubPartition,
                                          String[] subPartitionNames, long warehouseId) throws DdlException {
        OlapTable olapTable;
        OlapTable copiedTable;
    ...
    private void buildPartitionsConcurrently(long dbId, OlapTable table, List<PhysicalPartition> partitions,
        ...
        try {
            ...
            LOG.info("build partitions concurrently for {}, all tasks finished, took {}ms",
                    table.getName(), System.currentTimeMillis() - startTime);
    ...
    public void onCreate(Database db, Table table,
