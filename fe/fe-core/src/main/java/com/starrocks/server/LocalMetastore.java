...
public class LocalMetastore implements ConnectorMetadata {
        ...
        for (Map.Entry<Long, MaterializedIndex> entry : indexMap.entrySet()) {
            ...
            TabletMeta tabletMeta =
                    new TabletMeta(db.getId(), table.getId(), partitionId, indexId, indexMeta.getSchemaHash(),
                            storageMedium, table.isCloudNativeTable());
        ...
        try {
            if (countDownLatch.await(timeout, TimeUnit.SECONDS)) {
                ...
                for (Map.Entry<Long, Long> mark : firstThree) {
                    ...
                    sb.append('(');
                    DataNode dataNode = stateMgr.getClusterInfo().getBackend(mark.getKey());
                    sb.append(dataNode != null ? dataNode.getHost() : "N/A");
                    sb.append(") ");
                }
    ...
    private void createLakeTablets(OlapTable table, long partitionId, long shardGroupId, MaterializedIndex index,
            ...
            throws DdlException {
        Preconditions.checkArgument(table.isCloudNativeTable());
    ...
    public void createMaterializedView(CreateMaterializedViewStatement stmt)
        ...
        try {
            ...
            if (materializedView.isLakeMaterializedView()) {
    ...
    public void initDefaultCluster() {
        ...
        final List<DataNode> defaultClusterDataNodes = systemInfoService.getBackends();
        for (DataNode dataNode : defaultClusterDataNodes) {
            backendList.add(dataNode.getId());
        }
    ...
    public void replayUpdateClusterAndBackends(BackendIdsUpdateInfo info) {
        for (long id : info.getBackendList()) {
            final DataNode dataNode = stateMgr.getClusterInfo().getBackend(id);
            final Cluster cluster = defaultCluster;
            cluster.removeBackend(id);
            dataNode.setDecommissioned(false);
            dataNode.clearClusterName();
            dataNode.setBackendState(DataNode.BackendState.free);
        }
    }
                ...
                for (Partition partition : info.getPartitions()) {
                    ...
                    for (MaterializedIndex mIndex : partition.getMaterializedIndices(
                        ...
                        TabletMeta tabletMeta = new TabletMeta(db.getId(), olapTable.getId(),
                                partitionId, indexId, schemaHash, medium, olapTable.isCloudNativeTable());
                        for (Tablet tablet : mIndex.getTablets()) {
                            long tabletId = tablet.getId();
                            invertedIndex.addTablet(tabletId, tabletMeta);
