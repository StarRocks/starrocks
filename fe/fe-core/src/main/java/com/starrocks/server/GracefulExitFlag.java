...
public class GracefulExitFlag {
    ...
    public static boolean shouldAcceptNewRequest() {
        long elapsed = elapsedSinceGracefulExit();
        return elapsed < 0 || elapsed < httpAcceptWindowNanos();
    }
    ...
    public static boolean isDrainWindowElapsed() {
        ...
        long acceptWindowNanos = httpAcceptWindowNanos();
    ...
    // Nanoseconds elapsed since SIGUSR1, or -1 when graceful exit has not started (BEGIN_NANO not
    // set). Callers branch on the sign of the return.
    private static long elapsedSinceGracefulExit() {
    ...
    private static long httpAcceptWindowNanos() {
        return TimeUnit.NANOSECONDS.convert(Config.graceful_exit_http_accept_window_ms, TimeUnit.MILLISECONDS);
    }
    ...
    private static void flipHttpRejectingIfNeeded() {
        ...
        if ((System.nanoTime() - begin) >= httpAcceptWindowNanos()) {
            httpRejecting = true;
        }
    }
}
