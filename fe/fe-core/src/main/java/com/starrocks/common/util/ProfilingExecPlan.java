...
public class ProfilingExecPlan {
    ...
    public static ProfilingExecPlan buildFrom(ExecPlan execPlan) {
        ...
        // Pre-executed fragments (e.g. the LookUp fragment generated by GLM's FETCH node)
        // are sent to the BE and appear in the runtime profile, but are not included in
        // getFragments(). Register their plan nodes so ExplainAnalyzer can bind the BE
        // profile entries to FE plan elements and avoid a NPE on element == null.
        for (PlanFragment fragment : execPlan.getPreExecutedFragments()) {
            PlanNode planRoot = fragment.getPlanRoot();
            ProfilingFragment lightFragment = new ProfilingFragment(visitSink(fragment.getSink()),
                    visitNode(execPlan, planRoot, visitedElements));
            profilingPlan.addFragment(lightFragment);
        }
