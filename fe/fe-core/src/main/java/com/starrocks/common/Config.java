...
public class Config extends ConfigBase {
    ...
    @ConfField(mutable = true, comment = "After a Sample-Based Tablet Pre-Split reshard job reaches "
            ...
            + "SPREAD placement is eventually consistent: StarOS places the new shards together on a "
            + "single node, then its periodic balancer spreads them across nodes over the next few "
            + "seconds; a load that plans immediately after the split finds them still clustered on "
            + "one node and opens one load channel (ChannelNum=1), serializing the write to that node. "
            + "The wait polls the "
