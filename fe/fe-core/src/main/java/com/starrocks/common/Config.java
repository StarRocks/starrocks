...
public class Config extends ConfigBase {
     ...
     * The cleaner wakes up every minute and re-reads this value, so a change takes effect without a restart.
     */
    @ConfField(mutable = true)
    ...
    @ConfField(mutable = true, comment =
            "Cluster-wide ceiling (ms) on how long a bookmark reference's lease may run before "
                    ...
                    + "renewal (or the acquire time when never renewed). Because it runs from the "
                    + "last renewal, it also bounds how long a bookmark that stops being renewed can hold "
