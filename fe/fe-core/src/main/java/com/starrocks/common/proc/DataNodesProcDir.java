...
//   https://github.com/apache/incubator-doris/blob/master/fe/fe-core/src/main/java/org/apache/doris/common/proc/DataNodesProcDir.java
...
public class DataNodesProcDir implements ProcDirInterface {
    ...
    static {
        ImmutableList.Builder<String> builder = new ImmutableList.Builder<String>()
                .add("DataNodeId").add("IP").add("HeartbeatPort")
    ...
    public ProcResult fetchResult() throws AnalysisException {
        ...
        final List<List<String>> backendInfos = getClusterDataNodeInfos();
    ...
    public static List<List<String>> getClusterDataNodeInfos() {
        ...
        List<Long> backendIds = clusterInfoService.getDataNodeIds(false);
        if (backendIds == null) {
            return backendInfos;
        }
        ...
        List<List<Comparable>> comparableDataNodeInfos = new LinkedList<>();
        for (long backendId : backendIds) {
            ...
            watch.start();
            long tabletNum = GlobalStateMgr.getCurrentInvertedIndex().getTabletNumByDataNodeId(backendId);
            watch.stop();
            ...
            backendInfo.add(new Gson().toJson(backend.getDataNodeStatus()));
            ...
            if (RunMode.allowCreateLakeTable()) {
                ...
                long workerId = GlobalStateMgr.getCurrentState().getStarOSAgent().getWorkerIdByDataNodeId(backendId);
                backendInfo.add(String.valueOf(workerId));
            }

            comparableDataNodeInfos.add(backendInfo);
        }
        ...
        comparableDataNodeInfos.sort(comparator);

        for (List<Comparable> backendInfo : comparableDataNodeInfos) {
