...
public class SSLChannelImp implements SSLChannel {
    ...
    public SSLChannelImp(SSLEngine sslEngine, Transport transport) {
    ...
    public int readAll(ByteBuffer dstBuf) throws IOException {
        ...
        while (dstBuf.remaining() > 0) {
            ...
            peerAppData.clear();
            peerNetData.clear();
            int bytesRead = transport.read(peerNetData);
            if (bytesRead > 0) {
                peerNetData.flip();
            ...
            switch (handshakeStatus) {
                case NEED_UNWRAP:
                    if (!haveCachedPeerNetData) {
                        peerNetData.clear();
                        if (transport.read(peerNetData) < 0) {
