...
public class UserProperty implements Writable {
    ...
    public void readFields(DataInput in) throws IOException {
        ...
        if (getCurrentCatalogJournalVersion() < FeMetaVersion.VERSION_43) {
            ...
            if (getCurrentCatalogJournalVersion() >= 1) {
        ...
        // load cluster
        if (getCurrentCatalogJournalVersion() >= FeMetaVersion.VERSION_12) {
            if (in.readBoolean()) {
        ...
        if (getCurrentCatalogJournalVersion() >= FeMetaVersion.VERSION_21) {
