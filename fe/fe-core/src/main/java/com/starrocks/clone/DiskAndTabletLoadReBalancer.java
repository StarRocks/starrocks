...
public class DiskAndTabletLoadReBalancer extends Rebalancer {
    ...
    private OlapTable getOlapTableById(long dbId, long tblId) {
        ...
        try {
            ...
            return (OlapTable) globalStateMgr.getStarRocksMetadata().getTableIncludeRecycleBin(db, tblId);
        } finally {
    ...
    private int getPartitionTabletNumOnBePath(long dbId, long tableId, long partitionId, long indexId, long beId,
        ...
        try {
            ...
            Partition partition = globalStateMgr.getStarRocksMetadata().getPartitionIncludeRecycleBin(table, partitionId);
            if (partition == null) {
                return 0;
            }
    ...
    private boolean isTabletUnhealthy(long dbId, OlapTable olapTable, Long tabletId,
        ...
        try (AutoCloseableLock ignore =
            ...
            Partition partition = globalStateMgr.getStarRocksMetadata()
            ...
            LocalTablet tablet = (LocalTablet) GlobalStateMgr.getCurrentState().getMetastore().getTablet(index, tabletId);
            if (tablet == null) {
                return true;
            }

            short replicaNum = globalStateMgr.getStarRocksMetadata()
    ...
    private Map<Pair<Long, Long>, PartitionStat> getPartitionStats(TStorageMedium medium, boolean isLocalBalance,
        ...
        long lockStart;
        List<Long> dbIds = globalStateMgr.getStarRocksMetadata().getDbIdsIncludeRecycleBin();
        DATABASE:
        for (Long dbId : dbIds) {
            Database db = globalStateMgr.getStarRocksMetadata().getDbIncludeRecycleBin(dbId);
            if (db == null) {
            ...
            try {
                TABLE:
                for (Table table : globalStateMgr.getStarRocksMetadata().getTablesIncludeRecycleBin(db)) {
                    ...
                    for (Partition partition : globalStateMgr.getStarRocksMetadata()
                        ...
                        if (partitionChecked % partitionBatchNum == 0) {
                            ...
                            lockStart = System.nanoTime();
                            if (globalStateMgr.getStarRocksMetadata().getDbIncludeRecycleBin(dbId) == null) {
                                continue DATABASE;
                            }
                            if (globalStateMgr.getStarRocksMetadata().getTableIncludeRecycleBin(db, olapTbl.getId()) == null) {
                                continue TABLE;
                            }
                        ...
                        DataProperty dataProperty = globalStateMgr.getStarRocksMetadata()
                        ...
                        int replicationFactor = globalStateMgr.getStarRocksMetadata()
