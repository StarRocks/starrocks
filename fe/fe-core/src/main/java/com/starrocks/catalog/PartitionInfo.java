...
public class PartitionInfo implements Cloneable, Writable, GsonPreProcessable, GsonPostProcessable {
    ...
    public void write(DataOutput out) throws IOException {
        ...
        for (Map.Entry<Long, DataProperty> entry : idToDataProperty.entrySet()) {
            ...
            out.writeLong(idToExternalCoolDownSyncedTimeMs.get(entry.getKey()));
            out.writeLong(idToExternalCoolDownConsistencyCheckTimeMs.get(entry.getKey()));
            out.writeLong(idToExternalCoolDownConsistencyCheckDifference.get(entry.getKey()));
        }
    }
    ...
    public void readFields(DataInput in) throws IOException {
        ...
        for (int i = 0; i < counter; i++) {
            ...
            if (GlobalStateMgr.getCurrentStateStarRocksMetaVersion() >= StarRocksFEMetaVersion.VERSION_5) {
                idToExternalCoolDownSyncedTimeMs.put(partitionId, in.readLong());
                idToExternalCoolDownConsistencyCheckTimeMs.put(partitionId, in.readLong());
                idToExternalCoolDownConsistencyCheckDifference.put(partitionId, in.readLong());
            }
        }
    }
