...
public class ColumnStatsUsageSystemTable extends SystemTable {
    ...
    public ColumnStatsUsageSystemTable() {
        ...
        super(SystemId.COLUMN_STATS_USAGE, NAME, TableType.SCHEMA,
                builder()
                        ...
                        .column("USAGE", StandardTypes.STRING, "use case of this column stats")
                        .column("LAST_USED", StandardTypes.DATETIME, "last time when using this column stats")

                        .column("CREATED", StandardTypes.DATETIME, "create time of this column stats")
                        .build(),
    ...
    private static List<ScalarOperator> columnUsageToScalar(ColumnUsage columnUsage) {
        ...
        result.add(ConstantOperator.createNullableObject(names.map(x -> x.first.getDb()).orElse(null), StandardTypes.VARCHAR));
        result.add(ConstantOperator.createNullableObject(names.map(x -> x.first.getTbl()).orElse(null), StandardTypes.VARCHAR));
        result.add(ConstantOperator.createNullableObject(names.map(x -> x.second.getId()).orElse(null), StandardTypes.VARCHAR));
