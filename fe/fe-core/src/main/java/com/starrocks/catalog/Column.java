...
public class Column implements Writable, GsonPreProcessable, GsonPostProcessable {
    ...
    public void gsonPostProcess() throws IOException {
        ...
        for (DefaultExpr expression : new DefaultExpr[] {defaultExpr, originDefaultExpr}) {
            if (expression != null && expression.hasExprObject() && expression.getExprObject() != null) {
                try {
                    Expr validatedExpr = ColumnDefAnalyzer.validateComplexTypeDefaultValue(
                            type, expression.getExprObject(), true);
                    expression.setExprObject(validatedExpr);
                } catch (Exception e) {
