...
public class PartitionAccessTimeStore {
    ...
    public static final String TABLE = "_statistics_.partition_access_time";
    public static final String LOAD_ALL_SQL =
            "SELECT db_id, table_id, partition_id, last_access_time_ms FROM " + TABLE;

    public static String buildUpsertSql(List<TPartitionAccessTimeEntry> entries) {
        StringJoiner values = new StringJoiner(", ");
        for (TPartitionAccessTimeEntry e : entries) {
            values.add("(" + e.getDb_id() + ", " + e.getTable_id() + ", "
                    + e.getPartition_id() + ", " + e.getAccess_time_ms() + ")");
        }
    ...
    public void upsert(List<TPartitionAccessTimeEntry> entries) {
        ...
        for (List<TPartitionAccessTimeEntry> chunk : ListUtils.partition(entries, chunkSize)) {
     ...
     */
    private static List<TPartitionAccessTimeEntry> parseEntries(List<TResultBatch> batches) {
        ...
        for (TResultBatch batch : ListUtils.emptyIfNull(batches)) {
            for (ByteBuffer buffer : batch.getRows()) {
                try {
                    ...
                    e.setDb_id(data.get(0).getAsLong());
                    e.setTable_id(data.get(1).getAsLong());
                    e.setPartition_id(data.get(2).getAsLong());
                    e.setAccess_time_ms(data.get(3).getAsLong());
                    out.add(e);
                } catch (Exception e) {
                    LOG.debug("failed to parse partition access time row: {}", e.getMessage());
                }
            }
        }
        return out;
