...
import static com.starrocks.statistic.StatsConstants.PARTITIONS_SCAN_TABLE_NAME;
...
public class StatisticsMetaManager extends FrontendDaemon {
    ...
    private boolean createPartitionsScanTable(ConnectContext context) {
        LOG.info("create partitions_scan table start");
        TableName tableName = new TableName(STATISTICS_DB_NAME, PARTITIONS_SCAN_TABLE_NAME);
        try {
            int defaultReplicationNum = AutoInferUtil.calDefaultReplicationNum();
            String sql = String.format("CREATE TABLE %s\n" +
                    ...
                    "`scan_count` double sum NOT NULL COMMENT \"\"\n" +
                    ") ENGINE = OLAP \n" +
                    "AGGREGATE KEY(`dt`,`catalog_name`,`db_name`,`table_name`,`partition_name`)\n" +
                    "PARTITION BY date_trunc('day', `dt`)\n" +
                    "DISTRIBUTED BY HASH(`catalog_name`, `db_name`, `table_name`, `partition_name`) BUCKETS 10\n" +
                    "PROPERTIES (\n" +
                    "\"replication_num\" = \"%s\",\n" +
            ...
            LOG.warn("Failed to create partitions_scan table", e);
            return false;
        }
        LOG.info("create partitions_scan table done");
        return checkTableExist(PARTITIONS_SCAN_TABLE_NAME);
    }
    ...
    private boolean createTable(String tableName) {
        ...
        try (ConnectContext.ScopeGuard guard = context.bindScope()) {
            ...
            } else if (PARTITIONS_SCAN_TABLE_NAME.equals(tableName)) {
                return createPartitionsScanTable(context);
            } else {
    ...
    protected void runAfterCatalogReady() {
        ...
        refreshStatisticsTable(PARTITIONS_SCAN_TABLE_NAME);
        ...
        GlobalStateMgr.getCurrentState().getPartitionsScanMgr().alterTtlPartitionsScan();
