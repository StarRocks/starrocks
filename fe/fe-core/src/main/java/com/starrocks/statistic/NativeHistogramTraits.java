...
final class NativeHistogramTraits extends HistogramCollectTraits {
    // Fork-local (#59167, VARCHAR up to 2 GiB): oversized values blow up the histogram aggregation
    // state, so they are excluded from collection. Filtering them trades histogram accuracy for
    // collection stability on affected columns.
    // TODO: Preserve accurate statistics for oversized values without retaining them in aggregation state.
    private static final int MAX_HISTOGRAM_VARCHAR_LENGTH_BYTES = 1024 * 1024;
    ...
    private static final String MCV_STATISTIC_TEMPLATE =
            "select cast(version as INT), cast(db_id as BIGINT), cast(table_id as BIGINT), " +
                    ...
                    "WHERE $columnName is not null $columnLengthFilter " +
                    "GROUP BY $columnName " +
    ...
    private static final String QUERY_HISTOGRAM_STATISTIC_TEMPLATE =
            "SELECT cast(" + StatsConstants.STATISTIC_HISTOGRAM_VERSION + " as INT)," +
                    ...
                    "   FROM `$dbName`.`$tableName` $sampleClause " +
                    "   WHERE $randFilter and $columnName is not null $columnLengthFilter $MCVExclude" +
    ...
    private static final String QUERY_HISTOGRAM_WITH_HLL_NDV_STATISTIC_TEMPLATE =
            "SELECT cast(" + StatsConstants.STATISTIC_HISTOGRAM_VERSION + " as INT)," +
                    ...
                    " FROM `$dbName`.`$tableName`$hllColumnFilter;";
    ...
    private static final String BUCKET_BOUNDARIES_TEMPLATE =
            "SELECT cast(" + StatsConstants.STATISTIC_HISTOGRAM_VERSION + " as INT) as version," +
                    ...
                    " and $columnName is not null $columnLengthFilter $MCVExclude" +
                    " ORDER BY $columnName LIMIT $totalRows) t";
    ...
    String buildMcvQuery(String columnName) {
        ...
        putColumnLengthFilter(context, columnName, columnTypeOf(columnName));
    ...
    String buildHistogramQuery(double sampleRatio, Long bucketNum, Map<String, String> mostCommonValues,
        ...
        putColumnLengthFilter(context, columnName, columnType);
    ...
    @VisibleForTesting
    String buildHllNdvQuery(String buckets, String columnName, Type columnType) {
        VelocityContext context = buildBaseContext(db, table, catalogName, columnName);
    ...
    /**
     * Fork-local: excludes oversized VARCHAR values from histogram collection (#59167).
     *
     * <p>Kept inside this class rather than widening {@link HistogramCollectTraits}, so the
     * divergence from upstream stays in one file - the external flavour deliberately has no such
     * filter. {@code columnLengthFilter} extends an existing WHERE clause; {@code hllColumnFilter}
     * is a whole WHERE clause, for the one template that has none.
     */
    private void putColumnLengthFilter(VelocityContext context, String columnName, Type columnType) {
        if (!columnType.isVarchar()) {
            context.put("columnLengthFilter", "");
            context.put("hllColumnFilter", "");
            return;
        }

        String quotedColumnName = StatisticUtils.quoting(table, columnName);
        String lengthFilter = String.format("LENGTH(%s) <= %d", quotedColumnName, MAX_HISTOGRAM_VARCHAR_LENGTH_BYTES);
        context.put("columnLengthFilter", "and " + lengthFilter);
        context.put("hllColumnFilter", String.format(" WHERE %s is not null and %s", quotedColumnName, lengthFilter));
    }

    /** The declared type of {@code columnName}, which the traits SQL hooks do not all carry. */
    private Type columnTypeOf(String columnName) {
        int index = job.getColumnNames().indexOf(columnName);
        return job.getColumnTypes().get(index);
    }
