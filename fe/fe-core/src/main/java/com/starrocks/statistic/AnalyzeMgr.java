...
public class AnalyzeMgr implements Writable {
    ...
    public void updateLoadRows(TransactionState transactionState) {
        ...
        if (attachment instanceof RLTaskTxnCommitAttachment) {
            ...
            if (!transactionState.getTableIdList().isEmpty()) {
                ...
                // Check if this is an UPDATE operation by checking if StatisticsCollectionTrigger was called with
                // UPDATE type
                // For UPDATE operations, loadRows represents modified rows, not inserted rows
                DmlType dmlType = getDmlTypeFromTransactionState(transactionState);
    ...
    private DmlType getDmlTypeFromTransactionState(TransactionState transactionState) {
        // Try to get DmlType from transaction state label
        // For UPDATE operations, the label is generated by MetaUtils.genUpdateLabel()
        // which contains "update" keyword in the format: "update_<execution_id>"
        String label = transactionState.getLabel();
        if (label != null) {
            String lowerLabel = label.toLowerCase();
            if (lowerLabel.startsWith("update_") || lowerLabel.contains("_update_")) {
                return DmlType.UPDATE;
            }
        }
        // Default to INSERT_INTO if we can't determine
        return DmlType.INSERT_INTO;
    }
