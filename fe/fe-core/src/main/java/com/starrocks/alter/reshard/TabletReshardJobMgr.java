...
public class TabletReshardJobMgr extends FrontendDaemon implements GsonPostProcessable {
    ...
    // Jobs waiting for their WAL callback are intentionally absent from tabletReshardJobs, but they
    // still consume admission capacity. Guard both the transient count and publication transfer with
    // this monitor so a concurrent submitter observes each job in exactly one of the two places.
    private final transient Object capacityAdmissionLock = new Object();
    private transient long walPendingParallelTablets;
    ...
    public void addTabletReshardJob(TabletReshardJob tabletReshardJob) throws StarRocksException {
        ...
        long reservedParallelTablets = reserveTabletReshardCapacity(tabletReshardJob);
        ...
        try {
            ...
            releaseTabletReshardCapacity(reservedParallelTablets);
            throw e;
        }

        try {
            GlobalStateMgr.getCurrentState().getEditLog().logJsonObjectOrThrow(
                    OperationType.OP_UPDATE_TABLET_RESHARD_JOB_LOG,
                    tabletReshardJob,
                    wal -> publishReservedTabletReshardJob(tabletReshardJob, reservedParallelTablets));
        } catch (InterruptedException e) {
            releaseTabletReshardCapacity(reservedParallelTablets);
            throw rollbackAfterJournalFailure(tabletReshardJob, e, true);
        } catch (JournalWriteException e) {
            releaseTabletReshardCapacity(reservedParallelTablets);
            throw rollbackAfterJournalFailure(tabletReshardJob, e, false);
    ...
    private StarRocksException rollbackAfterJournalFailure(
            TabletReshardJob job, Exception cause, boolean interruptedFailure) {
        StarRocksException failure = new StarRocksException(
                "Failed to journal tablet reshard job: " + cause.getMessage(), cause);
        // Definite journal failures must leave the catalog reservation rolled back even when the
        // journal path restored the interrupt bit. Clear it only for cleanup, then restore it for
        // the caller after every cleanup outcome.
        boolean wasInterrupted = Thread.interrupted();
        boolean restoreInterrupt = interruptedFailure || wasInterrupted;
        try {
            job.rollbackInit();
    ...
    private long reserveTabletReshardCapacity(TabletReshardJob tabletReshardJob) throws StarRocksException {
        ...
        long parallelTablets = tabletReshardJob.getParallelTablets();
        synchronized (capacityAdmissionLock) {
            long currentParallelTablets = getTotalParallelTablets() + walPendingParallelTablets;
            if (currentParallelTablets > 0
                    && parallelTablets + currentParallelTablets > Config.tablet_reshard_max_parallel_tablets) {
            ...
            walPendingParallelTablets += parallelTablets;
        }
        return parallelTablets;
    }

    private void releaseTabletReshardCapacity(long parallelTablets) {
        synchronized (capacityAdmissionLock) {
            walPendingParallelTablets -= parallelTablets;
        }
    }

    private void publishReservedTabletReshardJob(TabletReshardJob job, long parallelTablets) {
        synchronized (capacityAdmissionLock) {
            tabletReshardJobs.put(job.getJobId(), job);
            walPendingParallelTablets -= parallelTablets;
        }
    }
