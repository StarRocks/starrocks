...
public class DynamicTabletJob implements Writable {
    ...
    protected void setJobState(JobState jobState) {
        ...
        // Update finished time at the end of each job state,
        // so that leader and follower can share the same time as visible version time.
    ...
    protected void runPreparingJob() {
        try (LockedObject<OlapTable> lockedTable = getLockedTable(LockType.READ)) {
            ...
            for (var physicalPartitionEntry : physicalPartitionContexts.entrySet()) {
                ...
                long commitVersion = physicalPartitionContext.getPhysicalPartition().getCommittedVersion();
    ...
    private void updateNextVersion() {
        try (LockedObject<OlapTable> lockedTable = getLockedTable(LockType.WRITE)) {
            OlapTable olapTable = lockedTable.get();
            for (var physicalPartitionEntry : physicalPartitionContexts.entrySet()) {
                PhysicalPartition oldpPhysicalPartition = olapTable
                        .getPhysicalPartition(physicalPartitionEntry.getKey());
                if (oldpPhysicalPartition == null) {
                    continue;
                }
                PhysicalPartitionContext physicalPartitionContext = physicalPartitionEntry.getValue();
                PhysicalPartition newPhysicalPartition = physicalPartitionContext.getPhysicalPartition();

                long nextVersion = oldpPhysicalPartition.getNextVersion() + 1;
                oldpPhysicalPartition.setNextVersion(nextVersion);
    ...
    private void replacePhysicalPartitions() {
        try (LockedObject<OlapTable> lockedTable = getLockedTable(LockType.WRITE)) {
            ...
            for (var physicalPartitionEntry : physicalPartitionContexts.entrySet()) {
                ...
                long commitVersion = newPhysicalPartition.getCommittedVersion();
                Preconditions.checkState(commitVersion == oldpPhysicalPartition.getVisibleVersion() + 1,
                        "commit version: " + commitVersion + ", visible version: "
                                + oldpPhysicalPartition.getVisibleVersion());

                oldpPhysicalPartition.setVisibleVersion(commitVersion, finishedTimeMs);
                newPhysicalPartition.setVisibleVersion(commitVersion, finishedTimeMs);

                olapTable.replacePhysicalPartition(oldpPhysicalPartition.getId(), newPhysicalPartition, true);
            }
        }
    }
    ...
    private void addTabletsToInvertedIndex() {
        Map<Long, TStorageMedium> partitionStorageMediums = new HashMap<>();
        try (LockedObject<OlapTable> lockedTable = getLockedTable(LockType.READ)) {
            OlapTable olapTable = lockedTable.get();
            PartitionInfo partitionInfo = olapTable.getPartitionInfo();
            for (PhysicalPartitionContext physicalPartitionContext : physicalPartitionContexts.values()) {
                PhysicalPartition physicalPartition = physicalPartitionContext.getPhysicalPartition();
                DataProperty dataProperty = partitionInfo.getDataProperty(physicalPartition.getParentId());
                if (dataProperty == null) {
                    continue;
                }

                partitionStorageMediums.put(physicalPartition.getParentId(), dataProperty.getStorageMedium());
            }
        }
        ...
        for (PhysicalPartitionContext physicalPartitionContext : physicalPartitionContexts.values()) {
            PhysicalPartition physicalPartition = physicalPartitionContext.getPhysicalPartition();
            TStorageMedium storageMedium = partitionStorageMediums.get(physicalPartition.getParentId());
    ...
    private void registerDynamicTablets() {
        ...
        for (PhysicalPartitionContext physicalPartitionContext : physicalPartitionContexts.values()) {
            long visibleVersion = physicalPartitionContext.getPhysicalPartition().getCommittedVersion();
