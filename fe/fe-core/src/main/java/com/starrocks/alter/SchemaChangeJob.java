...
//   https://github.com/apache/incubator-doris/blob/master/fe/fe-core/src/main/java/org/apache/doris/alter/SchemaChangeJob.java
...
public class SchemaChangeJob extends AlterJob {
    ...
    public void getJobInfo(List<List<Comparable>> jobInfos, OlapTable tbl) {
        ...
        for (Long indexId : changedIndexIdToSchemaVersion.keySet()) {
            ...
            jobInfo.add(TimeUtils.longToTimeString(createTime));
            jobInfo.add(TimeUtils.longToTimeString(finishedTime));
            jobInfo.add(tbl.getIndexNameById(indexId) == null ? FeConstants.null_string :
                    tbl.getIndexNameById(indexId)); // index name
            jobInfo.add(indexId);
            ...
            jobInfo.add(cancelMsg);
            if (state == JobState.RUNNING) {
                jobInfo.add(indexProgress.get(indexId) == null ? FeConstants.null_string :
                        indexProgress.get(indexId)); // progress
            } else {
    ...
    public synchronized void readFields(DataInput in) throws IOException {
        ...
        if (getCurrentCatalogJournalVersion() >= FeMetaVersion.VERSION_39) {
            if (in.readBoolean()) {
