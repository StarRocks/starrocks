...
public class SystemHandler extends AlterHandler {
    ...
    public synchronized void process(List<AlterClause> alterClauses, String clusterName, Database dummyDb,
        ...
        if (alterClause instanceof AddBackendClause) {
            ...
            if (!Strings.isNullOrEmpty(destClusterName)
                    && GlobalStateMgr.getCurrentState().getLocalMetastore().getCluster(destClusterName) == null) {
            ...
            GlobalStateMgr.getCurrentState().getNodeMgr().addFrontend(FrontendNodeType.OBSERVER, clause.getHost(), clause.getPort());
        } else if (alterClause instanceof DropObserverClause) {
            DropObserverClause clause = (DropObserverClause) alterClause;
            GlobalStateMgr.getCurrentState().getNodeMgr().dropFrontend(FrontendNodeType.OBSERVER, clause.getHost(), clause.getPort());
        } else if (alterClause instanceof AddFollowerClause) {
            AddFollowerClause clause = (AddFollowerClause) alterClause;
            GlobalStateMgr.getCurrentState().getNodeMgr().addFrontend(FrontendNodeType.FOLLOWER, clause.getHost(), clause.getPort());
        } else if (alterClause instanceof DropFollowerClause) {
            DropFollowerClause clause = (DropFollowerClause) alterClause;
            GlobalStateMgr.getCurrentState().getNodeMgr().dropFrontend(FrontendNodeType.FOLLOWER, clause.getHost(), clause.getPort());
    ...
    @Override
    public void replayInitJob(AlterJob alterJob, GlobalStateMgr catalog) {
    ...
    @Override
    public void replayFinish(AlterJob alterJob, GlobalStateMgr catalog) {
    ...
    @Override
    public void replayCancel(AlterJob alterJob, GlobalStateMgr catalog) {
        throw new NotImplementedException();
    }
}
