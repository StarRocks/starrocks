...
public class TaskManager {
    ...
    private final Map<Long, Task> normalTaskMap;
    ...
    public TaskManager() {
        normalTaskMap = Maps.newConcurrentMap();
    ...
    public void dropTasks(List<Long> taskIdList, boolean isReplay) {
        ...
        try {
            for (long taskId : taskIdList) {
                Task task = normalTaskMap.get(taskId);
                if (task == null) {
                ...
                switch (task.getType()) {
                    case NORMAL:
                        normalTaskMap.remove(task.getId());
                        break;
                    case PERIODICAL:
    ...
    public List<Task> showTasks(String dbName) {
        ...
        if (dbName == null) {
            taskList.addAll(normalTaskMap.values());
            taskList.addAll(periodTaskMap.values());
        } else {
            taskList.addAll(normalTaskMap.values().stream()
                    .filter(u -> u.getDbName().equals(dbName)).collect(Collectors.toList()));
            taskList.addAll(periodTaskMap.values().stream()
                    .filter(u -> u.getDbName().equals(dbName)).collect(Collectors.toList()));
        }
        return taskList;
    }
