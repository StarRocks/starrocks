...
public class TaskManager {
    ...
    public static final long TASK_NO_SCHEDULE = -4L;
    public static final long UNKNOWN_TASK_TYPE = -5L;
    ...
    private final Map<Long, Task> periodTaskMap;
    ...
    public TaskManager() {
        ...
        periodTaskMap = Maps.newConcurrentMap();
        ...
        try {
            ...
            switch (task.getTaskType()) {
                case MANUAL:
                    ...
                    break;
                case PERIODICAL:
                    if (periodTaskMap.containsKey(task.getId())) {
                        return DUPLICATE_CREATE_TASK;
                    }
                    Schedule schedule = task.getSchedule();
                    if (schedule == null) {
                        return TASK_NO_SCHEDULE;
                    }
                    LocalDateTime startTime = LocalDateTime.ofInstant(Instant.ofEpochMilli(schedule.getStartTime()),
                            TimeUtils.getTimeZone().toZoneId());
                    Duration duration = Duration.between(LocalDateTime.now(), startTime);
                    ...
                    ScheduledFuture<?> future = periodScheduler.scheduleAtFixedRate(() ->
                            ...
                            schedule.getTimeUnit());
                    periodFutureMap.put(task.getId(), future);
                    periodTaskMap.put(task.getId(), task);
                    break;
                default:
            ...
            for (long taskId : taskIdList) {
                ...
                switch (task.getTaskType()) {
                    case MANUAL:
                        ...
                        break;
                    case PERIODICAL:
                        ScheduledFuture<?> future = periodFutureMap.get(taskId);
                        if (future != null) {
                        ...
                        nameToTaskMap.remove(task.getName());
                        periodTaskMap.remove(task.getId());
                        periodFutureMap.remove(task.getId());
                        break;
                    default:
                        LOG.warn("drop taskId {} failed because unknown task type", taskId);
                }
            }
                ...
                switch (task.getTaskType())  {
                    ...
                    case PERIODICAL:
                        ...
                        if (schedule == null) {
                            ...
                            LOG.warn("clean up a null schedule period task: {}", task.getName());
                            continue;
                        }
                        if (schedule.isActive()) {
                            continue;
