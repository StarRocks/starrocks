...
public class ExecuteOption {
     ...
     */
    public boolean isMergeableWith(ExecuteOption option) {
        if (!isMergeRedundant || !option.isMergeRedundant) {
            return false;
        }
        if (CollectionUtils.sizeIsEmpty(this.taskRunProperties)
                && CollectionUtils.sizeIsEmpty(option.taskRunProperties)) {
            return true;
        }
        return Objects.equals(this.getTaskRunComparableProperties(), option.getTaskRunComparableProperties());
    }
    ...
    public Map<String, String> getTaskRunComparableProperties() {
        ...
        for (Map.Entry<String, String> e : taskRunProperties.entrySet()) {
            if (TaskRun.COMPARABLE_PROPERTIES.contains(e.getKey())) {
