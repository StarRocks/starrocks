...
public class UnityCatalogClient implements UnityCatalogApi {
    ...
    private static final String PATH_METASTORE_SUMMARY = "/api/2.1/unity-catalog/metastore_summary";
    ...
    public UnityCatalogTypes.TableInfo getTable(String fullName) {
        // UC three-level names are a single path segment with dots; addPathSegment preserves them.
        HttpUrl url = resolve(PATH_TABLES).newBuilder().addPathSegment(fullName).build();
    ...
    public UnityCatalogTypes.MetastoreSummary getMetastoreSummary() {
        HttpUrl url = resolve(PATH_METASTORE_SUMMARY);
        return executeGet(url, UnityCatalogTypes.MetastoreSummary.class);
    }
