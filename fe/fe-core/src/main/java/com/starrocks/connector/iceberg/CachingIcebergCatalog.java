...
public class CachingIcebergCatalog implements IcebergCatalog {
    ...
    public void refreshTable(String dbName, String tableName) {
        ...
        if (tables.getIfPresent(identifier) == null) {
            ...
            BaseTable current = (BaseTable) getTable(dbName, tableName);
            BaseTable update = (BaseTable) delegate.getTable(dbName, tableName);
            String currentLocation = current.operations().current().metadataFileLocation();
            String updateLocation = update.operations().current().metadataFileLocation();
    ...
    public void refreshCatalog() {
        for (TableIdentifier identifier : tables.asMap().keySet()) {
            refreshTable(identifier.namespace().level(0), identifier.name());
        }
    }
}
