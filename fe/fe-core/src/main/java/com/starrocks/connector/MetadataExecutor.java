...
public class MetadataExecutor {
    ...
    public static List<TResultBatch> executeDQL(MetadataCollectJob job) {
        ...
        if (!sqlResult.second.ok()) {
            throw new SemanticException("Metadata query fail | Error Message [%s] | QueryId [%s] | SQL [%s]",
                    context.getState().getErrorMessage(), DebugUtil.printId(context.getQueryId()), job.getSql());
        } else {
