...
public class PartitionPhase implements SearchPhase {
    ...
    private OpenSearchShardPartitions shardPartitions;
    private Map<String, OpenSearchNodeInfo> nodesInfo;

    public PartitionPhase(OpenSearchRestClient client) {
        this.client = client;
    }
    ...
    public void execute(SearchContext context) throws StarRocksConnectorException {
        ...
        if (!context.wanOnly()) {
            ...
            for (int i = 0; i < seeds.length; i++) {
                nodesInfo.put(String.valueOf(i), new OpenSearchNodeInfo(String.valueOf(i), seeds[i]));
            }
        }
    }
