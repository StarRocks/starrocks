...
public class IcebergDLFRESTCatalog implements IcebergCatalog {

    private static final Logger LOG = LogManager.getLogger(IcebergDLFRESTCatalog.class);

    private static final String KEY_CLIENT_CREDENTIALS_PROVIDER_DLF_TOKEN_PATH = "client.credentials-provider.dlf.token-path";

    private final Configuration conf;
    private final RESTCatalog delegate;

    public IcebergDLFRESTCatalog(String name, String ramUser, Configuration conf, Map<String, String> properties) {
        this.conf = conf;
        Map<String, String> copiedProperties = Maps.newHashMap();
        // fixed value
        copiedProperties.put(AuthProperties.AUTH_TYPE, AuthProperties.AUTH_TYPE_SIGV4);
        copiedProperties.put(AuthProperties.SIGV4_DELEGATE_AUTH_TYPE, AuthProperties.AUTH_TYPE_NONE);
        copiedProperties.put(CatalogProperties.METRICS_REPORTER_IMPL, IcebergMetricsReporter.class.getName());
        copiedProperties.put(AwsProperties.REST_SIGNING_NAME, "DlfNext");
        copiedProperties.put(AwsClientProperties.CLIENT_CREDENTIALS_PROVIDER,
                "org.apache.iceberg.rest.credentials.DlfLocalFileCredentialsProvider");
        copiedProperties.put(CatalogProperties.FILE_IO_IMPL, DlfFileIO.class.getName());
        copiedProperties.put(KEY_CLIENT_CREDENTIALS_PROVIDER_DLF_TOKEN_PATH, DlfUtil.getMetaToken(ramUser));

        // init from catalog properties
        if (Strings.isNullOrEmpty(properties.get(CatalogProperties.WAREHOUSE_LOCATION))) {
            throw new StarRocksConnectorException("The property %s must be set.", CatalogProperties.WAREHOUSE_LOCATION);
        } else {
            copiedProperties.put(CatalogProperties.WAREHOUSE_LOCATION, properties.get(CatalogProperties.WAREHOUSE_LOCATION));
        }
        if (Strings.isNullOrEmpty(properties.get(CatalogProperties.URI))) {
            throw new StarRocksConnectorException("The property %s must be set.", CatalogProperties.URI);
        } else {
            copiedProperties.put(CatalogProperties.URI, properties.get(CatalogProperties.URI));
        }
        if (Strings.isNullOrEmpty(properties.get(AwsProperties.REST_SIGNER_REGION))) {
            throw new StarRocksConnectorException("The property %s must be set.", AwsProperties.REST_SIGNER_REGION);
        } else {
            copiedProperties.put(AwsProperties.REST_SIGNER_REGION, properties.get(AwsProperties.REST_SIGNER_REGION));
        }

        if (!Strings.isNullOrEmpty(properties.get(AwsProperties.REST_ACCESS_KEY_ID))) {
            copiedProperties.put(AwsProperties.REST_ACCESS_KEY_ID, properties.get(AwsProperties.REST_ACCESS_KEY_ID));
        }
        if (!Strings.isNullOrEmpty(properties.get(AwsProperties.REST_SECRET_ACCESS_KEY))) {
            copiedProperties.put(AwsProperties.REST_SECRET_ACCESS_KEY, properties.get(AwsProperties.REST_SECRET_ACCESS_KEY));
        }
