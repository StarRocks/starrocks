...
public class OAuth2SecurityConfig {
    ...
    private IcebergRESTCatalog.Security security = IcebergRESTCatalog.Security.NONE;
...
class OAuth2SecurityConfigBuilder {
    public static OAuth2SecurityConfig build(final Map<String, String> properties) {
        ...
        if (strSecurity.equals(IcebergRESTCatalog.Security.NONE.name())) {
            ...
            return new OAuth2SecurityConfig().setSecurity(IcebergRESTCatalog.Security.OIDC);
        }

        OAuth2SecurityConfig config = new OAuth2SecurityConfig();
        config = config.setSecurity(IcebergRESTCatalog.Security.OAUTH2)
