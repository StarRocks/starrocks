 ...
 * {@link CloudConfiguration} by routing each cloud through the existing
 * {@link CloudConfigurationFactory} entry points
 ...
 * about into the property keys that the factory already understands, and the
...
public final class UnityCatalogCredentialTranslator {
    ...
    public static CloudConfiguration toCloudConfiguration(UnityCatalogTypes.TemporaryTableCredentials creds,
        ...
        if (creds.azureUserDelegationSas != null
            ...
            if (endpoint != null) {
                Map<String, String> props = new HashMap<>();
                props.put(AZURE_ADLS2_SAS_TOKEN, creds.azureUserDelegationSas.sasToken);
                ...
                // The Azure props above are already keyed with the AZURE_ADLS2_* constants the
                // provider chain understands; route directly through buildCloudConfigurationForStorage
                // so AzureCloudConfigurationProvider picks them up.
