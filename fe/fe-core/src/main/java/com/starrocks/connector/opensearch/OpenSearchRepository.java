...
/**
 * It is responsible for loading all OpenSearch external table's meta-data such as `fields`, `partitions` periodically,
 * playing the `repo` role at StarRocks On OpenSearch
 */
public class OpenSearchRepository extends FrontendDaemon {

    private static final Logger LOG = LogManager.getLogger(OpenSearchRepository.class);

    private Map<Long, EsTable> esTables;

    private Map<Long, OpenSearchRestClient> esClients;

    public OpenSearchRepository() {
        super("opensearch-repository", Config.es_state_sync_interval_second * 1000);
    ...
    public void registerTable(EsTable esTable) {
        ...
        esClients.put(esTable.getId(),
                new OpenSearchRestClient(esTable.getSeeds(), esTable.getUserName(), esTable.getPasswd(), esTable.sslEnabled()));
    ...
    protected void runAfterCatalogReady() {
        for (EsTable esTable : esTables.values()) {
            OpenSearchRestClient esClient = esClients.get(esTable.getId());
            if (esClient == null) {
                LOG.warn(String.format("EsTable[%s] existed, but OpenSearchClient not existed now, need retry.", esTable));
                continue;
            }
            ...
            try {
                ...
                LOG.warn(String.format("Thread %s: Exception happens when fetch index [%s] meta " +
                                "data from remote opensearch cluster. Table info: [%s]",
