...
import static com.starrocks.connector.opensearch.OpenSearchUtil.getFromJSONArray;
import static com.starrocks.connector.opensearch.OpenSearchUtil.readTree;
...
public class OpenSearchRestClient {
    ...
    // Phase 2/3: Extended constructor for SSL/auth support
    public OpenSearchRestClient(String[] nodes, String username, String password, boolean sslEnabled) {
        // Phase 1: Ignore SSL/auth parameters, use basic HTTP
        this(nodes);
    }
    ...
    public Map<String, OpenSearchNodeInfo> getHttpNodes() throws StarRocksConnectorException {
        Map<String, Map<String, Object>> nodesData = get("_nodes/http", "nodes");
        if (nodesData == null) {
            return Collections.emptyMap();
        }
        Map<String, OpenSearchNodeInfo> nodesMap = new HashMap<>();
        for (Map.Entry<String, Map<String, Object>> entry : nodesData.entrySet()) {
            OpenSearchNodeInfo node = new OpenSearchNodeInfo(entry.getKey(), entry.getValue());
            if (node.hasHttp()) {
    ...
    public OpenSearchMajorVersion version() throws StarRocksConnectorException {
        Map<String, Object> result = get("/", null);
        if (result == null) {
            throw new StarRocksConnectorException("Unable to retrieve OpenSearch cluster info.");
        }
        Map<String, String> versionBody = (Map<String, String>) result.get("version");
        return OpenSearchMajorVersion.parse(versionBody.get("number"));
    }
    ...
    public OpenSearchShardPartitions searchShards(String indexName) throws StarRocksConnectorException {
        ...
        return OpenSearchShardPartitions.findShardPartitions(indexName, searchShards);
    }
    ...
    private String execute(String path) throws StarRocksConnectorException {
        ...
        OkHttpClient client = NETWORK_CLIENT;
        ...
        for (int i = 0; i < retrySize; i++) {
            ...
            if (LOG.isTraceEnabled()) {
                LOG.trace("opensearch rest client request URL: {}", currentNode + "/" + path);
            }
            try {
    ...
    private <T> T parseContent(String response, String key) {
        ...
        try {
            ...
            LOG.error("parse opensearch response failure: [{}]", response);
    ...
    private Set<String> getIndices() {
        ...
        if (Objects.isNull(response)) {
            throw new StarRocksConnectorException("opensearch indexes are null, maybe this is error");
        }

        OpenSearchIndex[] indices = getFromJSONArray(response, OpenSearchIndex[].class);
        return Arrays.asList(indices).stream()
