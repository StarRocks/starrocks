...
public class CachingDeltaLakeMetastore extends CachingMetastore implements IDeltaLakeMetastore {
    ...
    public long estimateSize() {
        return Estimator.estimate(tableNameLockMap) +
                Estimator.estimate(databaseNamesCache.asMap()) +
                Estimator.estimate(tableNamesCache.asMap()) +
                Estimator.estimate(databaseCache.asMap()) +
                Estimator.estimate(tableCache.asMap()) +
                Estimator.estimate(tableSnapshotCache.asMap()) +
                Estimator.estimate(lastAccessTimeMap) +
                delegate.estimateSize();
    }
}
