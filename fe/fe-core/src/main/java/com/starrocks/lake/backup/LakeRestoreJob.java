    ...
    protected void prepareAndSendSnapshotTasks(Database db) {
        for (IdChain idChain : fileMapping.getMapping().keySet()) {
            ...
            try {
                ...
                DataNode dataNode = GlobalStateMgr.getCurrentSystemInfo().getBackend(tablet.getPrimaryBackendId());
                LakeTableSnapshotInfo info = new LakeTableSnapshotInfo(db.getId(), idChain.getTblId(),
                        ...
                        dataNode.getId(), tbl.getSchemaHashByIndexId(index.getId()), -1);
                snapshotInfos.put(idChain.getTabletId(), dataNode.getId(), info);
