...
public class KeyMgr {
    ...
    public void initDefaultMasterKey() throws InvalidConfException {
        ...
        try {
            if (defaultMasterKeySpec.isEmpty()) {
                if (idToKey.size() != 0) {
                    ...
                    throw new InvalidConfException("default_master_key removed in config");
                }
            } else {
                ...
                if (idToKey.size() == 0) {
                    ...
                    if (!masterKey.equals(masterKeyFromConfig)) {
                        // masterKey changed
                        LOG.error("default_master_key changed");
                        throw new InvalidConfException("default_master_key changed");
                    }
                }
                if (idToKey.size() == 1) {
                    // setup first KEK
