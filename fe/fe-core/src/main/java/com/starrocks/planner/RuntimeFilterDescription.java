...
public class RuntimeFilterDescription {
    ...
    public Expr getProbeExpr() {
        return probeExpr;
    }

    public void setProbeExpr(Expr probeExpr) {
    ...
    public void setBuildCardinality(long value) {
        ...
        if (isColocateOrBucketShuffle()) {
            ...
            // 2. for rf generated by BUCKET_SHUFFLE/COLOCATE JOIN, adopts rf if row count / numAliveBackends < buildMin
            int numBackends = ConnectContext.get() != null ? ConnectContext.get().getAliveBackendNumber() : 1;
            numBackends = Math.max(1, numBackends);
            buildCardinality = Math.max(1, buildCardinality / numBackends);
        }
    }

    public long getBuildCardinality() {
        ...
        if (probeNodeId >= 0) {
            ...
            if (hasRemoteTargets) {
                if (ConnectContext.get() == null ||
                        waitTimeMs != ConnectContext.get().getSessionVariable().getGlobalRuntimeFilterWaitTimeout()) {
