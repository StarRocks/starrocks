...
public abstract class JoinNode extends PlanNode implements RuntimeFilterBuildNode {
    ...
    protected void buildSelfRuntimeFilters(IdGenerator<RuntimeFilterId> runtimeFilterIdIdGenerator) {
    ...
    protected void pushDownSelfRuntimeFilters(DescriptorTable descTbl, boolean isForce) {
        SessionVariable sessionVariable = ConnectContext.get().getSessionVariable();
        // join may be 1-N relation, the value will be negative, but it's ok
        double selectivity = getChild(0).getCardinality() == 0 ? 0 :
                Math.max(Math.min(1.0 * getCardinality() / getChild(0).getCardinality(), 1.0), 0);
        if (!isForce && !isEffectiveFilter(sessionVariable, getChild(1).getCardinality(), selectivity)) {
            return;
        }
        for (RuntimeFilterDescription rf : unPushDownRuntimeFilters) {
            if (getChild(0).pushDownRuntimeFilters(descTbl, rf, rf.getProbeExpr(), probePartitionByExprs)) {
    ...
    public void buildRuntimeFilters(IdGenerator<RuntimeFilterId> runtimeFilterIdIdGenerator, DescriptorTable descTbl) {
        buildSelfRuntimeFilters(runtimeFilterIdIdGenerator);
        pushDownSelfRuntimeFilters(descTbl, false);
    }
    ...
    public boolean pushDownRuntimeFilters(DescriptorTable descTbl, RuntimeFilterDescription description, Expr probeExpr,
        ...
        if ((probeExpr instanceof SlotRef) && isInnerOrSemiJoin) {
            boolean pushedDown = pushDownRuntimeFiltersForChild(descTbl, description, probeExpr, partitionByExprs, 0);
            if (pushDownRuntimeFiltersForChild(descTbl, description, probeExpr, partitionByExprs, 1)) {
                ...
                pushDownSelfRuntimeFilters(descTbl, true);
                return true;
            }
            if (pushedDown) {
                return true;
        ...
        // equalJoinConjuncts
        if (super.pushDownRuntimeFilters(descTbl, description, probeExpr, partitionByExprs)) {
