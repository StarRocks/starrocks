...
//   https://github.com/apache/incubator-doris/blob/master/fe/fe-core/src/main/java/org/apache/doris/planner/AnalyticPlanner.java
...
public class AnalyticPlanner {
    ...
    private boolean isSortSlot(List<Expr> sortExprs, SlotDescriptor slotDesc) {
        for (Expr expr : sortExprs) {
            if (expr instanceof SlotRef) {
                SlotRef slotRef = (SlotRef) expr;
                if (slotRef.getDesc().equals(slotDesc)) {
                    return true;
                }
            }
        }
    ...
    private SortInfo createSortInfo(
        ...
        for (TupleId tid : input.getTupleIds()) {
            ...
            for (SlotDescriptor inputSlotDesc : tupleDesc.getSlots()) {
                ...
                // Avoid repeatedly materialized same slot
                if (isSortSlot(sortExprs, inputSlotDesc)) {
                    continue;
                }
                // TODO(zc)
