...
>>>>>>> 46ccb07 ([BugFix] Skip order-by hints when the TopN filter probes a column-less slot (#78527))
    ...
    public static OlapScanNode createOlapScanNodeByLocation(
            ...
            ComputeResource computeResource) {
        OlapScanNode olapScanNode = new OlapScanNode(id, desc, planNodeName, -1);
    ...
    private boolean canUseMultiVersionCache() {
        switch (olapTable.getKeysType()) {
            ...
            case DUP_KEYS:
            case AGG_KEYS: {
                List<Column> columns = olapTable.getSchemaByIndexId(index.indexId);
    ...
    protected void toNormalForm(TNormalPlanNode planNode, FragmentNormalizer normalizer) {
        ...
        // schemaId == indexId and only diverges once a schema change assigns a new schema, and
        ...
        if (selectedIndexId != -1) {
            MaterializedIndexMeta selectedIndexMeta = olapTable.getIndexMetaByIndexId(selectedIndexId);
            if (selectedIndexMeta != null && selectedIndexMeta.getSchemaId() != selectedIndexId) {
            ...
            for (PhysicalPartition physicalPartition : partition.getSubPartitions()) {
                ...
                if (materializedIndex == null) {
                    throw new RuntimeException(String.format("Materialized index with meta id %d " +
                            ...
                            index.indexId, partition.getName(), physicalPartition.getId(), olapTable.getName()));
                }
        ...
        for (Long tabletId : scanTabletIds) {
            ...
            if (!tabletToPartitionMap.containsKey(tabletId)) {
                throw new RuntimeException(String.format("Invalid tablet id: '%d'. The tablet may have been dropped. " +
                        ...
                        tabletId, selectedPartitionIds, olapTable.getName(), index.indexId));
            }
