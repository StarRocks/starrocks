...
//   https://github.com/apache/incubator-doris/blob/master/fe/fe-core/src/main/java/org/apache/doris/http/action/DataNodeAction.java
...
public class DataNodeAction extends WebBaseAction {
    private static final Logger LOG = LogManager.getLogger(DataNodeAction.class);

    public DataNodeAction(ActionController controller) {
        super(controller);
        // TODO Auto-generated constructor stub
    }

    public static void registerAction(ActionController controller) throws IllegalArgException {
        controller.registerHandler(HttpMethod.GET, "/backend", new DataNodeAction(controller));
    }
    ...
    public void executeGet(BaseRequest request, BaseResponse response) {
        ...
        appendKnownDataNodesInfo(response.getContent());

        getPageFooter(response.getContent());
        writeResponse(request, response);
    }

    private void appendKnownDataNodesInfo(StringBuilder buffer) {
        ...
        for (DataNode backend : backendMap.values()) {
            ...
            backendInfo.add("host: " + backend.getHost()
                    ...
                    + ", state: " + backend.getDataNodeState()
                    + ", start_time: " + TimeUtils.longToTimeString(backend.getLastStartTime())
                    + ", last_report_tablets_time: " + backend.getDataNodeStatus().lastSuccessReportTabletsTime
        ...
        // set result
        buffer.append("<h2>Known DataNodes(" + backendMap.size() + ")</h2>");
