...
/**
 * {@code POST /api/collections}. Body:
 * {@code {"contextbase": "...", "name": "...", "collection_type": "knowledge", "properties": {...}}}.
 */
public class CreateCollectionAction extends RestBaseAction {

    private static final Gson GSON = new Gson();
    private static final Type MAP_TYPE = new TypeToken<Map<String, Object>>() {
    }.getType();

    public CreateCollectionAction(ActionController controller) {
        super(controller);
    }

    public static void registerAction(ActionController controller) throws IllegalArgException {
        controller.registerHandler(HttpMethod.POST, "/api/collections", new CreateCollectionAction(controller));
    }
    ...
    protected void executeWithoutPassword(BaseRequest request, BaseResponse response)
        ...
        try {
            ...
            if (Strings.isNullOrEmpty(contextBase) || Strings.isNullOrEmpty(name)) {
                sendResult(request, response,
                        new RestBaseResult("\"contextbase\" and \"name\" are required"));
                return;
            }
            ContextRestAuth.checkOnContextBase(ConnectContext.get(), contextBase, ContextRestAuth.BaseAction.USAGE);
            String collectionType = (String) payload.getOrDefault("collection_type", "knowledge");
            ...
            long id = GlobalStateMgr.getCurrentState().getContextMgr()
                    .createCollection(contextBase, name, collectionType, properties, ifNotExists);
            CreatedResult result = new CreatedResult();
            result.id = id;
            result.contextbase = contextBase;
            result.name = name;
