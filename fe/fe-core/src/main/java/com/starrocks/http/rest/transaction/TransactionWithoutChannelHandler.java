...
public class TransactionWithoutChannelHandler implements TransactionOperationHandler {
    ...
    private TransactionResult handlePrepareTransaction(Database db, String label) throws StarRocksException {
        ...
        // Decision only, so it does not count as a cache hit; commitEvictedTransactionByLabel counts it
        // once when it builds the terminal response.
        TransactionStatus status = GlobalStateMgr.getCurrentState().getGlobalTransactionMgr()
                .getLabelStatus(dbId, label, false).getStatus();
