...
public class ColocateMetaService {
    ...
    public static class UpdateGroupAction extends ColocateMetaBaseAction {
        ...
        public void executeInLeaderWithAdmin(BaseRequest request, BaseResponse response)
            ...
            Database db = GlobalStateMgr.getCurrentState().getStarRocksMetadata().getDbIncludeRecycleBin(groupId.dbId);
            if (db == null) {
            ...
            try {
                OlapTable table = (OlapTable) globalStateMgr.getStarRocksMetadata().getTableIncludeRecycleBin(db, tableId);
                if (table == null) {
