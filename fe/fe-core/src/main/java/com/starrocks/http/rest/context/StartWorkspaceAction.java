...
/**
 * {@code POST /api/workspaces/start}. Body:
 * {@code {"qualified_name": "<cb.col.ws>", "collection_id": 0, "properties": {...}}}.
 *
 * <p>Idempotent per Doc 2 §8.5: existing workspaces resume (return their id); missing workspaces
 * are created. The {@code resumed} field tells the caller which path was taken so an agent can
 * decide whether to replay session state or start fresh.
 */
public class StartWorkspaceAction extends RestBaseAction {

    private static final Gson GSON = new Gson();
    private static final Type MAP_TYPE = new TypeToken<Map<String, Object>>() {
    }.getType();

    public StartWorkspaceAction(ActionController controller) {
        super(controller);
    }

    public static void registerAction(ActionController controller) throws IllegalArgException {
        controller.registerHandler(HttpMethod.POST, "/api/workspaces/start", new StartWorkspaceAction(controller));
    }
    ...
    protected void executeWithoutPassword(BaseRequest request, BaseResponse response)
        ...
        try {
            ...
            if (workspace != null) {
                resp.id = workspace.getId();
                resp.resumed = true;
            } else {
                resp.id = mgr.createWorkspace(qualifiedName, collectionId.longValue(), properties, true);
                resp.resumed = false;
                workspace = mgr.getWorkspace(qualifiedName);
            }

            WorkspaceRestSupport.Summary summary = WorkspaceRestSupport.summarizeWorkspaces(
                    workspace == null ? Collections.emptyList() : Collections.singletonList(workspace))
                    .get(resp.id);
            if (summary != null) {
                resp.memory = summary.memory;
                resp.scratch = summary.scratch;
                resp.output = summary.output;
