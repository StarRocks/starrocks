...
public class StarRocksMetadata implements ConnectorMetadata {
    ...
    public void createDb(String dbName, Map<String, String> properties) throws DdlException, AlreadyExistsException {
        ...
        try {
            if (localMetastore.getDb(dbName) != null) {
                ...
                localMetastore.createDb(createDbInfo);
            }
        } finally {
    ...
    public void dropDb(String dbName, boolean isForceDrop) throws DdlException, MetaNotFoundException {
        ...
        try {
            ...
            DropDbInfo dropInfo = new DropDbInfo(db.getFullName(), isForceDrop);
            localMetastore.dropDb(dropInfo);
    ...
    public void renameDatabase(AlterDatabaseRenameStatement stmt) throws DdlException {
        GlobalStateMgr globalStateMgr = GlobalStateMgr.getCurrentState();
        MetaStore localMetastore = globalStateMgr.getMetastore();
    ...
    public List<String> listDbNames() {
        return GlobalStateMgr.getCurrentState().getMetastore().listDbNames();
    }

    @Override
    public Database getDb(String name) {
        return GlobalStateMgr.getCurrentState().getMetastore().getDb(name);
    }

    @Override
    public Database getDb(long dbId) {
        return GlobalStateMgr.getCurrentState().getMetastore().getDb(dbId);
    }
    ...
    public boolean createTable(CreateTableStmt stmt) throws DdlException {
        ...
        Table table = tableFactory.createTable(GlobalStateMgr.getCurrentState().getMetastore(), db, stmt);
    ...
    public void onCreate(Database db, Table table, String storageVolumeId, boolean isSetIfNotExists)
        ...
        try {
            ...
            GlobalStateMgr.getCurrentState().getMetastore().createTable(createTableInfo);
            table.onCreate(db);
        } finally {
            locker.unLockDatabase(db.getId(), LockType.WRITE);
        }
    ...
    public void dropTemporaryTable(String dbName, long tableId, String tableName,
                                   boolean isSetIfExists, boolean isForce) throws DdlException {
        Database db = getDb(dbName);
        if (db == null) {
            ErrorReport.reportDdlException(ErrorCode.ERR_BAD_DB_ERROR, dbName);
        }
        ...
        try {
            ...
            db.unprotectDropTemporaryTable(tableId, isForce, false);
            DropInfo info = new DropInfo(db.getId(), table.getId(), -1L, isForce);
    ...
    public void truncateTable(TruncateTableStmt truncateTableStmt, ConnectContext context) throws DdlException {
        ...
        try {
            ...
            buildPartitionTablets(db, copiedTbl, newPartitions.stream().map(Partition::getSubPartitions)
        ...
        try {
            ...
            TruncateTableInfo info = new TruncateTableInfo(db.getId(), olapTable.getId(), newPartitions,
                    truncateEntireTable);
            GlobalStateMgr.getCurrentState().getMetastore().truncateTable(info);
    ...
    public void alterTableComment(Database db, Table table, AlterTableCommentClause clause) {
        ...
        GlobalStateMgr.getCurrentState().getMetastore().alterTable(log);
    ...
    public void renameTable(Database db, Table table, TableRenameClause tableRenameClause) throws DdlException {
        ...
        GlobalStateMgr.getCurrentState().getMetastore().renameTable(tableInfo);
        LOG.info("rename table[{}] to {}, tableId: {}", oldTableName, newTableName, olapTable.getId());
    }

    @Override
    public List<String> listTableNames(String dbName) {
        return GlobalStateMgr.getCurrentState().getMetastore().listTableNames(dbName);
    }

    @Override
    public Table getTable(String dbName, String tblName) {
        return GlobalStateMgr.getCurrentState().getMetastore().getTable(dbName, tblName);
    }

    public Table getTable(Long dbId, Long tableId) {
        return GlobalStateMgr.getCurrentState().getMetastore().getTable(dbId, tableId);
    }
    ...
    public void createMaterializedView(CreateMaterializedViewStatement stmt)
        ...
        try {
            ...
            if (isNonPartitioned) {
                ...
                buildPartitionTablets(db, materializedView, new ArrayList<>(partition.getSubPartitions()),
                        materializedView.getWarehouseId());
    ...
    public void dropMaterializedView(DropMaterializedViewStmt stmt) throws DdlException, MetaNotFoundException {
        ...
        if (table instanceof MaterializedView) {
            ...
            GlobalStateMgr.getCurrentState().getAlterJobMgr().getMaterializedViewHandler().processDropMaterializedView(stmt);
        }
    }
    ...
    private void addPartitions(ConnectContext ctx, Database db, String tableName, List<PartitionDesc> partitionDescs,
        ...
        try {
            ...
            buildPartitionTablets(db, copiedTable, partitionList.stream().map(Partition::getSubPartitions)
    ...
    public void dropPartition(Database db, Table table, DropPartitionClause clause) throws DdlException {
        ...
        if (clause.getPartitionName() != null) {
            ...
            GlobalStateMgr.getCurrentState().getMetastore().dropPartition(info);
            ...
            GlobalStateMgr.getCurrentState().getMetastore().dropPartitions(info);
    ...
    public void renamePartition(Database db, Table table, PartitionRenameClause renameClause) throws DdlException {
        ...
        TableInfo tableInfo = TableInfo.createForPartitionRename(db.getId(), olapTable.getId(), partition.getId(),
                newPartitionName);
        GlobalStateMgr.getCurrentState().getMetastore().renamePartition(tableInfo);
    ...
    public void replaceTempPartition(Database db, String tableName, ReplacePartitionClause clause) throws DdlException {
        ...
        try {
            ...
            GlobalStateMgr.getCurrentState().getMetastore().replaceTempPartition(info);
    ...
    public void setPartitionVersion(AdminSetPartitionVersionStmt stmt) {
        ...
        try {
            ...
            GlobalStateMgr.getCurrentState().getMetastore().setPartitionVersion(
    ...
    private void addSubPartitions(Database db, OlapTable table, Partition partition,
        ...
        // build partitions
        buildPartitionTablets(db, copiedTable, subPartitions, warehouseId);
        ...
        try {
            ...
            for (PhysicalPartition subPartition : subPartitions) {
                // add sub partition
                GlobalStateMgr.getCurrentState().getMetastore().addPhysicalPartition(partition, subPartition);
            ...
            GlobalStateMgr.getCurrentState().getMetastore().addSubPartitionLog(infos);
        } finally {
            locker.unLockDatabase(db.getId(), LockType.WRITE);
        }
    }
    ...
    public void buildPartitionTablets(Database db, OlapTable table, List<PhysicalPartition> partitions, long warehouseId)
            throws DdlException {
        ...
        try {
            ...
            if (status == Replica.ReplicaStatus.BAD || status == Replica.ReplicaStatus.OK) {
                if (replica.setBadForce(status == Replica.ReplicaStatus.BAD)) {
                    ...
                    GlobalStateMgr.getCurrentState().getMetastore().setReplicaStatus(log);
                    LOG.info("set replica {} of tablet {} on backend {} as {}.",
                            replica.getId(), tabletId, backendId, status);
                }
            }
        } finally {
            locker.unLockDatabase(db.getId(), LockType.WRITE);
