...
public class AuthorizationMgr {
    ...
    protected void revokeFromUser(
            ...
            List<PrivilegeType> privilegeTypes,
            List<PEntryObject> objects,
            User user) throws PrivilegeException {
        userWriteLock();
        try {
            UserIdentity userIdentity = new UserIdentity(user.getUser(), user.getHost(), user.isDomain());
            UserPrivilegeCollectionV2 collection = getUserPrivilegeCollectionUnlocked(userIdentity);
    ...
    protected void grantRoleToUser(List<String> parentRoleName, User user) throws PrivilegeException {
        userWriteLock();
        try {
            UserIdentity userIdentity = new UserIdentity(user.getUser(), user.getHost(), user.isDomain());
    ...
    protected void revokeRoleFromUser(List<String> roleNameList, User user) throws PrivilegeException {
        userWriteLock();
        try {
            UserIdentity userIdentity = new UserIdentity(user.getUser(), user.getHost(), user.isDomain());
                ...
                if (!parentRoleNameList.isEmpty()) {
                    return Lists.newArrayList(userIdentity.toString(), null,
                            AstToSQLBuilder.toSQL(new GrantRoleStmt(parentRoleNameList,
                                    new User(userIdentity.getUser(), userIdentity.getHost(), userIdentity.isDomain()),
                                    NodePosition.ZERO)));
                }
                return null;
            } finally {
