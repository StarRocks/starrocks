...
public class CachingHiveMetastore implements IHiveMetastore {
    public static final long NOT_CACHE = 0;
    public static final long NOT_EVICT = -1;
    public static final long NOT_REFRESH = -1;
    ...
    protected LoadingCache<HivePartitionName, HivePartitionStatistics> partitionStatsCache;
    protected LoadingCache<HiveTableName, HivePartitionStatistics> tableStatsCache;

    public static CachingHiveMetastore cachingHiveMetastore(IHiveMetastore metastore, Executor executor,
    ...
    public static CachingHiveMetastore memoizeMetastore(IHiveMetastore metastore, long maxSize) {
    ...
    protected CachingHiveMetastore(IHiveMetastore metastore, Executor executor, long expireAfterWriteSec,
        ...
        if (enableListNamesCache) {
            ...
            partitionKeysCache = newCacheBuilder(NOT_CACHE, NOT_CACHE, NOT_CACHE)
                    .build(asyncReloading(CacheLoader.from(this::loadPartitionKeys), executor));
            databaseNamesCache = newCacheBuilder(NOT_CACHE, NOT_CACHE, NOT_CACHE)
                    .build(asyncReloading(CacheLoader.from(this::loadAllDatabaseNames), executor));
            tableNamesCache = newCacheBuilder(NOT_CACHE, NOT_CACHE, NOT_CACHE)
                    .build(asyncReloading(CacheLoader.from(this::loadAllTableNames), executor));
        }
        ...
        partitionCache = newCacheBuilder(expireAfterWriteSec, NOT_REFRESH, maxSize)
                .build(asyncReloading(new CacheLoader<HivePartitionName, Partition>() {
                    @Override
                    public Partition load(HivePartitionName key) {
                        return loadPartition(key);
                    }

                    @Override
                    public Map<HivePartitionName, Partition> loadAll(Iterable<? extends HivePartitionName> partitionKeys) {
        ...
        partitionStatsCache = newCacheBuilder(expireAfterWriteSec, NOT_REFRESH, maxSize)
                .build(asyncReloading(new CacheLoader<HivePartitionName, HivePartitionStatistics>() {
                    @Override
                    public HivePartitionStatistics load(HivePartitionName key) {
                        return loadPartitionStatistics(key);
                    }

                    @Override
                    public Map<HivePartitionName, HivePartitionStatistics> loadAll(Iterable<? extends HivePartitionName> partitionKeys) {
    ...
    public Map<String, HivePartitionStatistics> getPartitionsStatistics(Table table, List<String> partitionNames) {
        ...
        Map<HivePartitionName, HivePartitionStatistics> statistics = getAll(partitionStatsCache, hivePartitionNames);
    ...
    public Map<String, HivePartitionStatistics> getPresentPartitionsStatistics(List<HivePartitionName> partitions) {
    ...
    private HivePartitionStatistics loadPartitionStatistics(HivePartitionName hivePartitionName) {
    ...
    private Map<HivePartitionName, HivePartitionStatistics> loadPartitionsStatistics(Iterable<? extends HivePartitionName> partitionNames) {
        HivePartitionName hivePartitionName = Iterables.get(partitionNames, 0);
        Table table = getTable(hivePartitionName.getDatabaseName(), hivePartitionName.getTableName());
        ...
        return partitionsStatistics.entrySet().stream().collect(Collectors.toMap(
                entry -> HivePartitionName.of(hivePartitionName.getDatabaseName(), hivePartitionName.getTableName(), entry.getKey()),
                Map.Entry::getValue
        ));
    }
