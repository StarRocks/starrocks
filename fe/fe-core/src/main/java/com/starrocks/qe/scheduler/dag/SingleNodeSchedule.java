...
// all at once execution schedule for Single node
// use batch interface to speed prepare phase
// don't support incremental scan range deploy
public class SingleNodeSchedule extends AllAtOnceExecutionSchedule {
    @Override
    public void schedule(Coordinator.ScheduleOption option) throws RpcException, StarRocksException {
        List<FragmentInstanceExecState> fragmentInstanceExecStateList = new ArrayList<>();
        for (List<ExecutionFragment> executionFragments : dag.getFragmentsInTopologicalOrderFromRoot()) {
            ...
            deployState.getThreeStageExecutionsToDeploy()
                    .forEach(t -> t.forEach(a -> fragmentInstanceExecStateList.add(a)));
        }
