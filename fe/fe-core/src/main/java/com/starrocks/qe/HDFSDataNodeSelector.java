...
public class HDFSDataNodeSelector implements DataNodeSelector {
    ...
    // be host -> bes
    Multimap<String, ComputeNode> hostToDataNodes = HashMultimap.create();
    private final ScanNode scanNode;
    private final List<TScanRangeLocations> locations;
    private final FragmentScanRangeAssignment assignment;
    private final Set<Long> usedDataNodeIDs;
    private final Map<TNetworkAddress, Long> addressToDataNodeId;
    ...
    public HDFSDataNodeSelector(ScanNode scanNode, List<TScanRangeLocations> locations,
                               FragmentScanRangeAssignment assignment, Map<TNetworkAddress, Long> addressToDataNodeId,
                               Set<Long> usedDataNodeIDs, ImmutableCollection<ComputeNode> computeNodes,
        ...
        this.addressToDataNodeId = addressToDataNodeId;
        this.usedDataNodeIDs = usedDataNodeIDs;
    ...
    public void computeScanRangeAssignment() throws Exception {
        ...
        for (ComputeNode computeNode : computeNodes) {
            ...
            hostToDataNodes.put(computeNode.getHost(), computeNode);
        }
        if (hostToDataNodes.isEmpty()) {
        ...
        if (forceScheduleLocal) {
            for (int i = 0; i < locations.size(); ++i) {
                ...
                for (final TScanRangeLocation location : scanRangeLocations.getLocations()) {
                    Collection<ComputeNode> servers = hostToDataNodes.get(location.getServer().getHostname());
    ...
    private void recordScanRangeAssignment(ComputeNode node, TScanRangeLocations scanRangeLocations) {
        ...
        usedDataNodeIDs.add(node.getId());
        addressToDataNodeId.put(address, node.getId());
