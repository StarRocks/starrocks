...
public class FinishedState implements JobState {
    @Override
    public JobStateType getType() {
        return JobStateType.FINISHED;
    }

    public static final class GotoFinished implements TransitionAction {
        @Override
        public JobState transition(StateContext ctx, JobState fromState, JobEvent event) {
            ctx.releaseSharingSlot();
