...
public class CoordinatorPreprocessor {
    ...
    void prepareFragments() {
        ...
        if (LOG.isDebugEnabled()) {
            ...
            for (Map.Entry<Long, DataNode> entry : idToBackend.entrySet()) {
                ...
                LOG.debug("backend: {}-{}-{}", backendID, dataNode.getHost(), dataNode.getBePort());
            }
    ...
    private void recordUsedBackend(TNetworkAddress addr, Long backendID) {
        if (this.queryOptions.getLoad_job_type() == TLoadJobType.STREAM_LOAD &&
            ...
            DataNode dataNode = idToBackend.get(backendID);
            bePortToBeWebServerPort.put(addr, new TNetworkAddress(dataNode.getHost(), dataNode.getHttpPort()));
        }
    ...
    private String backendInfosString(boolean chooseComputeNode) {
        if (chooseComputeNode) {
            ...
            for (Map.Entry<Long, DataNode> entry : this.idToBackend.entrySet()) {
                Long backendID = entry.getKey();
                DataNode dataNode = entry.getValue();
                infoStr.append(String.format("[%s alive: %b inBlacklist: %b] ", dataNode.getHost(),
                        dataNode.isAlive(), SimpleScheduler.isInBlacklist(backendID)));
            }
