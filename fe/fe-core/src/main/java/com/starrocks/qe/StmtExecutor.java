...
public class StmtExecutor {
    ...
    public void handleDMLStmt(ExecPlan execPlan, DmlStmt stmt) throws Exception {
        ...
        if (parsedStmt instanceof InsertStmt && ((InsertStmt) parsedStmt).isOverwrite()
                && !((InsertStmt) parsedStmt).hasOverwriteJob() && !(targetTable instanceof IcebergTable)) {
            ...
            transactionId =
                    GlobalStateMgr.getCurrentGlobalTransactionMgr()
                            .beginRemoteTransaction(externalTable.getSourceTableDbId(),
                                    ...
                                    authenticateParams);
        } else if (targetTable.shouldSkipSinkTxn()) {
            // schema or iceberg table does not need txn
        } else {
        ...
        try {
            ...
            if (targetTable instanceof ExternalOlapTable) {
                ...
                context.getGlobalStateMgr().getMetadataMgr().finishSink(
                        catalogName, dbName, tableName, commitInfos);
                txnStatus = TransactionStatus.VISIBLE;
