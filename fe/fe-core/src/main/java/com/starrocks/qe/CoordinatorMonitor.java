    ...
    private class DeadBackendAndComputeNodeChecker extends Thread {
        ...
        private void cancelCoordinatorsUsing(List<Long> deadBackendIDs) {
            ...
            for (Coordinator coord : coordinators) {
                // A QueryInfo can be registered without a coordinator, so the list may hold nulls.
                if (coord != null && usesAnyDeadBackend(coord, deadBackendIDs)) {
        ...
        private boolean usesAnyDeadBackend(Coordinator coord, List<Long> deadBackendIDs) {
            try {
                return deadBackendIDs.stream().anyMatch(coord::isUsingBackend);
            } catch (Throwable e) {
                // One coordinator that cannot answer must not stop the sweep from reaching the others.
        ...
        private void cancelAsync(Coordinator coord) {
