...
public class CancellingState implements JobState {
    @Override
    public JobStateType getType() {
        return JobStateType.CANCELLING;
    }

    public static final class GotoCancelling implements TransitionAction {
        @Override
        public JobState transition(StateContext ctx, JobState fromState, JobEvent event) {

            Preconditions.checkState(event instanceof CancelJobEvent,
                    "The transition GotoCancelling cannot handle event {}", event);
            CancelJobEvent cancelEvent = (CancelJobEvent) event;

            ctx.cancelInternal(cancelEvent.getReason(), event.getStatus(), null);
