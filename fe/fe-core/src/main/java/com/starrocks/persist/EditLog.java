...
public class EditLog {
    ...
    /**
     * Throw-path variant used by tablet-reshard admission. An interrupt before enqueue is a definite
     * failure, while an interrupt after enqueue cannot abandon the durability outcome or skip the WAL
     * applier. The leader WAL fence covers serialization, enqueue, commit, and apply.
     */
    public void logJsonObjectOrThrow(short op, Object obj, WALApplier applier)
            throws JournalWriteException, InterruptedException {
        try {
            enterGate();
        } catch (EditLogException e) {
            throw new JournalWriteException(JournalWriteException.Reason.ADMISSION_CLOSED, e.getMessage(), e);
        }
        try {
            JournalTask task = submitRaw(op, new Writable() {
                @Override
            ...
            try {
                ...
                if (e.getCause() instanceof JournalWriteException journalWriteException) {
                    throw journalWriteException;
                }
                throw new JournalWriteException(
                        JournalWriteException.Reason.WRITER_ABORTED, e.getMessage(), e);
            }
            if (applier != null) {
                applier.apply(obj);
            }
