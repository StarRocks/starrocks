...
public class StarRocksFEServer {
    ...
    private static void handleGracefulExit() {
        ...
        Signal.handle(new Signal("USR1"), sig -> {
            ...
            // becomes visible, so the exempt set is fixed at shutdown start. The isGracefulExit()
            // guard stops a repeated SIGUSR1 from re-snapshotting mid-drain (which would admit
            // post-signal transactions into the exempt set).
    ...
    private static void waitForDraining() throws InterruptedException {
        ...
        // T0: HealthAction returns 500 (HTTP probe) and stopAccept closes the MySQL port (TCP probe).
        // JDBC idle connections close from the start. HTTP ExecuteSqlAction still admits requests and
        // HTTP keep-alives stay until graceful_exit_http_accept_window_ms elapses, then 503 + close.
        // Drain waits for HTTP rejecting + activeHttpRequests==0, JDBC/HTTP contexts gone, leader
        // running txns (leader only), and min_graceful_exit_time_second.
        ...
        while (true) {
            ...
            // Compute the running-transaction count only when this node is the leader.
            int runningTxnNums = isLeader
                    ? GlobalStateMgr.getCurrentState().getGlobalTransactionMgr().getRunningTxnNums()
                    : 0;
            boolean hasRunningTxnsToDrain = isLeader && runningTxnNums > 0;

            if (connectScheduler.isDrained() && !hasRunningTxnsToDrain
                    && System.nanoTime() - stopAcceptTimeNano
                    > TimeUnit.SECONDS.toNanos(Config.min_graceful_exit_time_second)
                    && GracefulExitFlag.trySealHttpDrained()) {
                break;
            }

            if (totalConns > 0) {
                LOG.info("waiting for {} connections to drain", totalConns);
            } else if (hasRunningTxnsToDrain) {
                LOG.info("waiting for {} running transactions to drain", runningTxnNums);
            } else if (GracefulExitFlag.isHttpRejecting()) {
