 ...
 *
 * <p>A published profile is named {@code <prefix><yyyyMMdd-HHmmss>.html.tar.gz}. Files that a collection is
 * still working on carry an extra suffix - {@code .tmp.html} while the profiler dumps, {@code .tar.gz.tmp}
 * while the archive is written - and must never end in {@code .html.tar.gz}: ProcProfileAction and
 * ProcProfileFileAction select downloadable profiles by exactly that suffix, so a half-written file that
 * matched it would be listed and served.
 *
...
final class ProcProfileFiles {
    ...
    static boolean isPublishedArchive(String fileName) {
        return fileName.endsWith(PUBLISHED_SUFFIX);
    }
}
