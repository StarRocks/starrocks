...
public class ReplicationMgr extends FrontendDaemon {
    ...
    private final Map<Long, ReplicationJob> runningJobs = Maps.newConcurrentMap();

    @SerializedName(value = "finishedJobs")
    private final Map<Long, ReplicationJob> finishedJobs = Maps.newConcurrentMap();

    @SerializedName(value = "abortedJobs")
    private final Map<Long, ReplicationJob> abortedJobs = Maps.newConcurrentMap();
    ...
    protected void runAfterCatalogReady() {
        ...
        for (ReplicationJob job : runningJobs.values()) {
            ...
            if (state.equals(ReplicationJobState.FINISHED)) {
                toRemovedJobs.add(job);
                finishedJobs.put(job.getTableId(), job);
    ...
    public void addReplicationJob(TTableReplicationRequest request) throws UserException {
        ...
        if (runningJobs.putIfAbsent(job.getTableId(), job) != null) {
            throw new AlreadyExistsException("Replication job on table: " + job.getTableId() + " is already exist");
        }
    ...
    public void finishReplicationTask(ReplicationTask task, TFinishTaskRequest request) {
        ...
        job.finishReplicationTask(task, request);
    }
    ...
    public void replayReplicationJob(ReplicationJob replicationJob) {
        ...
        if (replicationJob.getState().equals(ReplicationJobState.FINISHED)) {
            finishedJobs.put(replicationJob.getTableId(), replicationJob);
    ...
    public void load(SRMetaBlockReader reader) throws SRMetaBlockEOFException, IOException, SRMetaBlockException {
        ...
        finishedJobs.putAll(replicationMgr.finishedJobs);
