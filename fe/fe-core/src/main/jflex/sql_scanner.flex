...
import com.starrocks.analysis.SqlParserSymbols;
...
%{
    ...
    static {
        ...
        keywordMap.put("costs", new Integer(SqlParserSymbols.KW_COSTS));
        ...
        keywordMap.put("decimal32", new Integer(SqlParserSymbols.KW_DECIMAL32));
        keywordMap.put("decimal64", new Integer(SqlParserSymbols.KW_DECIMAL64));
        keywordMap.put("decimal128", new Integer(SqlParserSymbols.KW_DECIMAL128));
        ...
        keywordMap.put("percentile", new Integer(SqlParserSymbols.KW_PERCENTILE));
        keywordMap.put("percentile_union", new Integer(SqlParserSymbols.KW_PERCENTILE_UNION));
        ...
        keywordMap.put("swap", new Integer(SqlParserSymbols.KW_SWAP));
...
SingleQuoteHexStringLiteral = [xX]\'([a-fA-F0-9]{2})*\'
...
{SingleQuoteHexStringLiteral} {
  String l = yytext().substring(2, yytext().length()-1);
  try {
    /* HEX string like: x'616263' stands for 'abc' */
    l = new String(Hex.decodeHex(l.toCharArray()));
  } catch (DecoderException e) {
    return newToken(SqlParserSymbols.UNMATCHED_STRING_LITERAL, yytext());
  }
  return newToken(SqlParserSymbols.STRING_LITERAL, l);
}
...
{DoubleLiteral} {
  ...
  try {
    ...
    int precision = DecimalLiteral.getRealPrecision(decimal_val);
    int scale = DecimalLiteral.getRealScale(decimal_val);
    int integerPartWidth = precision - scale;
    if (integerPartWidth > 38) {
        return newToken(SqlParserSymbols.FLOATINGPOINT_LITERAL, decimal_val.doubleValue());
    }
  } catch (NumberFormatException e) {
    return newToken(SqlParserSymbols.NUMERIC_OVERFLOW, yytext());
  }
