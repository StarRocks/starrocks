...
LEFT OUTER JOIN (join-predicate [1: v1 = 1 AND 4: v4 = 2] post-join-predicate [11: countRows IS NOT NULL != true])
    ...
    EXCHANGE BROADCAST
        AGGREGATE ([GLOBAL] aggregate [{11: countRows=sum(11: countRows)}] group by [[]] having [null]
            EXCHANGE GATHER
                AGGREGATE ([LOCAL] aggregate [{11: countRows=sum(14: count_v7)}] group by [[]] having [null]
                    META SCAN (columns{14})
[end]
...
LEFT OUTER JOIN (join-predicate [1: v1 = 1 AND 4: v4 = 2] post-join-predicate [null])
    CROSS JOIN (join-predicate [null] post-join-predicate [null])
        ...
        EXCHANGE BROADCAST
            SCAN (columns[4: v4] predicate[null])
    EXCHANGE BROADCAST
        AGGREGATE ([GLOBAL] aggregate [{12: countRows=sum(12: countRows)}] group by [[]] having [null]
            EXCHANGE GATHER
                AGGREGATE ([LOCAL] aggregate [{12: countRows=sum(15: count_v7)}] group by [[]] having [null]
                    META SCAN (columns{15})
[end]
