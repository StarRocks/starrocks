...
order by
    value desc ;
[plan-1]
TOP-N (order by [[21: sum(20: expr) DESC NULLS LAST]])
    TOP-N (order by [[21: sum(20: expr) DESC NULLS LAST]])
        CROSS JOIN (join-predicate [null] post-join-predicate [21: sum(20: expr) > 43: expr])
            AGGREGATE ([GLOBAL] aggregate [{21: sum(20: expr)=sum(20: expr)}] group by [[1: PS_PARTKEY]] having [null]
                EXCHANGE SHUFFLE[1]
            ...
            EXCHANGE BROADCAST
                AGGREGATE ([GLOBAL] aggregate [{42: sum(41: expr)=sum(41: expr)}] group by [[]] having [null]
                    EXCHANGE GATHER
                ...
                EXCHANGE SHUFFLE[1]
                    INNER JOIN (join-predicate [2: PS_SUPPKEY = 7: S_SUPPKEY AND 15: N_NATIONKEY = 10: S_NATIONKEY] post-join-predicate [null])
                        ...
                        EXCHANGE BROADCAST
                            SCAN (columns[7: S_SUPPKEY, 10: S_NATIONKEY] predicate[null])
            EXCHANGE BROADCAST
                AGGREGATE ([GLOBAL] aggregate [{42: sum(41: expr)=sum(42: sum(41: expr))}] group by [[]] having [null]
                    EXCHANGE GATHER
                        AGGREGATE ([LOCAL] aggregate [{42: sum(41: expr)=sum(41: expr)}] group by [[]] having [null]
                            INNER JOIN (join-predicate [23: PS_SUPPKEY = 28: S_SUPPKEY AND 36: N_NATIONKEY = 31: S_NATIONKEY] post-join-predicate [null])
                                CROSS JOIN (join-predicate [null] post-join-predicate [null])
                                    SCAN (columns[23: PS_SUPPKEY, 24: PS_AVAILQTY, 25: PS_SUPPLYCOST] predicate[null])
                                    EXCHANGE BROADCAST
