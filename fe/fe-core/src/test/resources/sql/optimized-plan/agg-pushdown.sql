...
[result]
AGGREGATE ([GLOBAL] aggregate [{7: sum=sum(8: sum)}] group by [[]] having [null]
    EXCHANGE GATHER
        INNER JOIN (join-predicate [1: v1 = 4: v4] post-join-predicate [null])
            AGGREGATE ([GLOBAL] aggregate [{8: sum=sum(1: v1)}] group by [[1: v1]] having [null]
                SCAN (columns[1: v1] predicate[1: v1 IS NOT NULL])
            EXCHANGE SHUFFLE[4]
                AGGREGATE ([GLOBAL] aggregate [{}] group by [[4: v4]] having [null]
...
[result]
AGGREGATE ([GLOBAL] aggregate [{7: sum=sum(9: sum), 8: max=max(10: max)}] group by [[]] having [null]
    EXCHANGE GATHER
        INNER JOIN (join-predicate [1: v1 = 4: v4] post-join-predicate [null])
            AGGREGATE ([GLOBAL] aggregate [{9: sum=sum(2: v2)}] group by [[1: v1]] having [null]
                SCAN (columns[1: v1, 2: v2] predicate[1: v1 IS NOT NULL])
            EXCHANGE SHUFFLE[4]
                AGGREGATE ([GLOBAL] aggregate [{10: max=max(5: v5)}] group by [[4: v4]] having [null]
...
[result]
AGGREGATE ([GLOBAL] aggregate [{7: sum=sum(9: sum), 8: max=max(10: max)}] group by [[3: v3]] having [null]
    EXCHANGE SHUFFLE[3]
        LEFT OUTER JOIN (join-predicate [1: v1 = 4: v4] post-join-predicate [null])
...
[result]
AGGREGATE ([GLOBAL] aggregate [{7: sum=sum(9: sum), 8: max=max(10: max)}] group by [[3: v3, 6: v6]] having [null]
    EXCHANGE SHUFFLE[3, 6]
        LEFT OUTER JOIN (join-predicate [1: v1 = 4: v4] post-join-predicate [null])
            AGGREGATE ([GLOBAL] aggregate [{9: sum=sum(2: v2)}] group by [[1: v1, 3: v3]] having [null]
                SCAN (columns[1: v1, 2: v2, 3: v3] predicate[null])
            EXCHANGE SHUFFLE[4]
                AGGREGATE ([GLOBAL] aggregate [{10: max=max(5: v5)}] group by [[4: v4, 6: v6]] having [null]
...
[result]
AGGREGATE ([GLOBAL] aggregate [{7: stddev=stddev(2: v2), 8: max=max(9: max)}] group by [[]] having [null]
    EXCHANGE GATHER
        LEFT OUTER JOIN (join-predicate [1: v1 = 4: v4] post-join-predicate [null])
            SCAN (columns[1: v1, 2: v2] predicate[null])
            EXCHANGE SHUFFLE[4]
                AGGREGATE ([GLOBAL] aggregate [{9: max=max(5: v5)}] group by [[4: v4]] having [null]
                    SCAN (columns[4: v4, 5: v5] predicate[null])
[end]
...
AGGREGATE ([GLOBAL] aggregate [{8: sum=sum(9: sum)}] group by [[7: expr]] having [null]
    EXCHANGE SHUFFLE[7]
        INNER JOIN (join-predicate [1: v1 = 4: v4] post-join-predicate [null])
            ...
            EXCHANGE SHUFFLE[4]
                AGGREGATE ([GLOBAL] aggregate [{}] group by [[4: v4, 6: v6]] having [null]
...
[sql]
select max(v2 + 3), sum(v2 + 3) from t0 join t1 on t0.v1 = t1.v4;
[result]
AGGREGATE ([GLOBAL] aggregate [{9: sum=sum(11: sum), 8: max=max(10: max)}] group by [[]] having [null]
    EXCHANGE GATHER
        INNER JOIN (join-predicate [1: v1 = 4: v4] post-join-predicate [null])
            AGGREGATE ([GLOBAL] aggregate [{10: max=max(12: add), 11: sum=sum(13: add)}] group by [[1: v1]] having [null]
                SCAN (columns[1: v1, 2: v2] predicate[1: v1 IS NOT NULL])
            EXCHANGE SHUFFLE[4]
                AGGREGATE ([GLOBAL] aggregate [{}] group by [[4: v4]] having [null]
                    SCAN (columns[4: v4] predicate[4: v4 IS NOT NULL])
...
[sql]
select max(v2 + v3), sum(v2 + 3) from t0 join t1 on t0.v1 = t1.v4;
[result]
AGGREGATE ([GLOBAL] aggregate [{9: max=max(11: max), 10: sum=sum(12: sum)}] group by [[]] having [null]
    EXCHANGE GATHER
        INNER JOIN (join-predicate [1: v1 = 4: v4] post-join-predicate [null])
...
[sql]
select sum(v2) from (select * from t0 union all select * from t1) x;
[result]
AGGREGATE ([GLOBAL] aggregate [{10: sum=sum(11: sum)}] group by [[]] having [null]
    EXCHANGE GATHER
        UNION
            AGGREGATE ([GLOBAL] aggregate [{12: sum=sum(2: v2)}] group by [[]] having [null]
                EXCHANGE GATHER
                    SCAN (columns[2: v2] predicate[null])
            AGGREGATE ([GLOBAL] aggregate [{13: sum=sum(5: v5)}] group by [[]] having [null]
                EXCHANGE GATHER
                    SCAN (columns[5: v5] predicate[null])
[end]

[sql]
select sum(v2) from (select t0.* from t0 join t1 on t0.v1 = t1.v4 union all select * from t1) x;
[result]
...
[sql]
select sum(v2) from (select t0.* from t0 join t1 on t0.v1 = t1.v4 where abs(t0.v2) = 1 union all select * from t1) x;
[result]
AGGREGATE ([GLOBAL] aggregate [{13: sum=sum(14: sum)}] group by [[]] having [null]
    EXCHANGE GATHER
        UNION
...
[sql]
select sum(v2) from (select t0.* from t0 join t1 on t0.v1 = t1.v4 union all select * from t1) x where abs(x.v2) = 2;
[result]
AGGREGATE ([GLOBAL] aggregate [{13: sum=sum(14: sum)}] group by [[]] having [null]
    EXCHANGE GATHER
        UNION
    ...
    EXCHANGE GATHER
        UNION
            ...
            INNER JOIN (join-predicate [11: v8 = 8: v5] post-join-predicate [null])
                ...
                AGGREGATE ([GLOBAL] aggregate [{}] group by [[8: v5]] having [null]
                    EXCHANGE SHUFFLE[8]
                        SCAN (columns[8: v5] predicate[abs(8: v5) = 2])
[end]
...
[result]
AGGREGATE ([GLOBAL] aggregate [{22: sum=sum(23: sum)}] group by [[]] having [null]
    EXCHANGE GATHER
        UNION
            UNION
                AGGREGATE ([GLOBAL] aggregate [{26: sum=sum(2: v2)}] group by [[]] having [null]
                    EXCHANGE GATHER
                        SCAN (columns[2: v2] predicate[null])
                AGGREGATE ([GLOBAL] aggregate [{27: sum=sum(5: v5)}] group by [[]] having [null]
                    EXCHANGE GATHER
                        SCAN (columns[5: v5] predicate[null])
            UNION
                AGGREGATE ([GLOBAL] aggregate [{28: sum=sum(11: v5)}] group by [[]] having [null]
                    EXCHANGE GATHER
                        SCAN (columns[11: v5] predicate[null])
                AGGREGATE ([GLOBAL] aggregate [{29: sum=sum(14: v8)}] group by [[]] having [null]
                    EXCHANGE GATHER
                        SCAN (columns[14: v8] predicate[null])
[end]
...
where x.v2 + x.v5 = 1;
[result]
AGGREGATE ([GLOBAL] aggregate [{19: sum=sum(20: sum)}] group by [[]] having [null]
    EXCHANGE GATHER
        INNER JOIN (join-predicate [8: v2 = 17: v5 AND add(8: v2, 17: v5) = 1] post-join-predicate [null])
            ...
            EXCHANGE BROADCAST
                UNION
                    AGGREGATE ([GLOBAL] aggregate [{}] group by [[11: v5]] having [null]
                        EXCHANGE SHUFFLE[11]
                            SCAN (columns[11: v5] predicate[11: v5 IS NOT NULL])
                    AGGREGATE ([GLOBAL] aggregate [{}] group by [[14: v8]] having [null]
                        EXCHANGE SHUFFLE[14]
...
[sql]
select sum(v2) from t0, t1;
[result]
AGGREGATE ([GLOBAL] aggregate [{7: sum=sum(8: sum)}] group by [[]] having [null]
    CROSS JOIN (join-predicate [null] post-join-predicate [null])
        AGGREGATE ([GLOBAL] aggregate [{8: sum=sum(2: v2)}] group by [[]] having [null]
            EXCHANGE GATHER
                SCAN (columns[2: v2] predicate[null])
        EXCHANGE BROADCAST
