...
INNER JOIN (join-predicate [1: v1 = 4: v4 AND 2: v2 = 8: case] post-join-predicate [null])
    ...
    EXCHANGE SHUFFLE[4]
        AGGREGATE ([GLOBAL] aggregate [{7: max=max(5: v5)}] group by [[4: v4]] having [CASE 7: max IS NULL WHEN true THEN null WHEN false THEN 7: max END IS NOT NULL]
            SCAN (columns[4: v4, 5: v5] predicate[4: v4 IS NOT NULL])
[end]
...
INNER JOIN (join-predicate [1: v1 = 4: v4 AND 2: v2 = 8: case] post-join-predicate [null])
    SCAN (columns[1: v1, 2: v2, 3: v3] predicate[1: v1 IS NOT NULL AND 2: v2 IS NOT NULL])
    EXCHANGE SHUFFLE[4]
        AGGREGATE ([GLOBAL] aggregate [{7: max=max(5: v5)}] group by [[4: v4]] having [CASE 7: max IS NULL WHEN true THEN 7: max WHEN false THEN null END IS NOT NULL]
            SCAN (columns[4: v4, 5: v5] predicate[4: v4 IS NOT NULL])
[end]
...
LEFT OUTER JOIN (join-predicate [1: v1 = 4: v4] post-join-predicate [2: v2 = CASE 7: count IS NULL WHEN true THEN 0 WHEN false THEN 7: count END])
    ...
    EXCHANGE SHUFFLE[4]
        AGGREGATE ([GLOBAL] aggregate [{7: count=count(5: v5)}] group by [[4: v4]] having [null]
            SCAN (columns[4: v4, 5: v5] predicate[null])
[end]
...
INNER JOIN (join-predicate [1: v1 = 4: v4 AND 2: v2 = 8: case] post-join-predicate [null])
    SCAN (columns[1: v1, 2: v2, 3: v3] predicate[1: v1 IS NOT NULL AND 2: v2 IS NOT NULL])
    EXCHANGE SHUFFLE[4]
        AGGREGATE ([GLOBAL] aggregate [{7: count=count(5: v5)}] group by [[4: v4]] having [CASE 7: count IS NULL WHEN true THEN 7: count WHEN false THEN 0 END IS NOT NULL]
            SCAN (columns[4: v4, 5: v5] predicate[4: v4 IS NOT NULL])
[end]
