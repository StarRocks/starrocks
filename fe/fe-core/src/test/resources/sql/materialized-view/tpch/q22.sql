                            ...
                            EXCHANGE BROADCAST
                                ASSERT LE 1
                                    AGGREGATE ([GLOBAL] aggregate [{103: sum=sum(103: sum), 104: count=sum(104: count)}] group by [[]] having [null]
                                        EXCHANGE GATHER
                                            AGGREGATE ([LOCAL] aggregate [{103: sum=sum(86: c_sum), 104: count=sum(85: c_count)}] group by [[]] having [null]
                                                SCAN (mv[customer_agg_mv1] columns[83: c_acctbal, 84: substring_phone, 85: c_count, 86: c_sum] predicate[83: c_acctbal > 0.00 AND 84: substring_phone IN (21, 28, 24, 32, 35, 34, 37)])
                        EXCHANGE SHUFFLE[23]
