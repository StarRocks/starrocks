...
TOP-N (order by [[10: l_returnflag ASC NULLS FIRST, 11: l_linestatus ASC NULLS FIRST]])
    TOP-N (order by [[10: l_returnflag ASC NULLS FIRST, 11: l_linestatus ASC NULLS FIRST]])
        AGGREGATE ([GLOBAL] aggregate [{129: count=sum(110: count_discount), 130: count=sum(113: count_order), 120: sum=sum(105: sum_qty), 121: sum=sum(105: sum_qty), 122: count=sum(106: count_qty), 123: sum=sum(107: sum_base_price), 124: sum=sum(107: sum_base_price), 125: sum=sum(111: sum_disc_price), 126: count=sum(108: count_base_price), 127: sum=sum(112: sum_charge), 128: sum=sum(109: sum_discount)}] group by [[103: l_returnflag, 104: l_linestatus]] having [null]
