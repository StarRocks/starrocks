...
AGGREGATE ([GLOBAL] aggregate [{27: sum=sum(27: sum)}] group by [[]] having [null]
    EXCHANGE GATHER
        AGGREGATE ([LOCAL] aggregate [{27: sum=sum(26: expr)}] group by [[]] having [null]
            SCAN (mv[lineitem_mv] columns[95: l_quantity, 99: l_shipinstruct, 100: l_shipmode, 108: p_brand, 109: p_container, 111: p_size, 116: l_saleprice] predicate[99: l_shipinstruct = DELIVER IN PERSON AND 100: l_shipmode = AIR OR 100: l_shipmode = AIR REG AND 108: p_brand = Brand#11 AND 111: p_size <= 10 AND 109: p_container = MED BAG OR 109: p_container = MED BOX OR 109: p_container = MED PACK OR 109: p_container = MED PKG AND 95: l_quantity <= 25 AND 95: l_quantity >= 15 OR 108: p_brand = Brand#21 AND 111: p_size <= 15 AND 109: p_container = LG BOX OR 109: p_container = LG CASE OR 109: p_container = LG PACK OR 109: p_container = LG PKG AND 95: l_quantity <= 35 AND 95: l_quantity >= 25 OR 108: p_brand = Brand#45 AND 111: p_size <= 5 AND 109: p_container = SM BOX OR 109: p_container = SM CASE OR 109: p_container = SM PACK OR 109: p_container = SM PKG AND 95: l_quantity <= 15 AND 95: l_quantity >= 5 AND 111: p_size >= 1])
[end]
