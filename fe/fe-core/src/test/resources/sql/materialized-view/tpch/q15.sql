        ...
        INNER JOIN (join-predicate [1: s_suppkey = 12: l_suppkey] post-join-predicate [null])
            ...
            EXCHANGE SHUFFLE[12]
                INNER JOIN (join-predicate [25: sum = 44: max] post-join-predicate [null])
                    AGGREGATE ([GLOBAL] aggregate [{144: sum=sum(82: sum_disc_price)}] group by [[76: l_suppkey]] having [144: sum IS NOT NULL]
                        SCAN (mv[lineitem_agg_mv2] columns[76: l_suppkey, 77: l_shipdate, 82: sum_disc_price] predicate[77: l_shipdate >= 1995-07-01 AND 77: l_shipdate < 1995-10-01])
                    EXCHANGE BROADCAST
                                ...
                                AGGREGATE ([GLOBAL] aggregate [{44: max=max(44: max)}] group by [[]] having [null]
                                    EXCHANGE GATHER
                                        AGGREGATE ([LOCAL] aggregate [{44: max=max(43: sum)}] group by [[]] having [null]
                                            AGGREGATE ([GLOBAL] aggregate [{143: sum=sum(82: sum_disc_price)}] group by [[76: l_suppkey]] having [null]
                                                SCAN (mv[lineitem_agg_mv2] columns[76: l_suppkey, 77: l_shipdate, 82: sum_disc_price] predicate[77: l_shipdate >= 1995-07-01 AND 77: l_shipdate < 1995-10-01])
[end]
