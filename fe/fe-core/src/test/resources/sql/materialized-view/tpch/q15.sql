        ...
        INNER JOIN (join-predicate [1: s_suppkey = 12: l_suppkey] post-join-predicate [null])
            ...
            EXCHANGE SHUFFLE[12]
                INNER JOIN (join-predicate [25: sum = 44: max] post-join-predicate [null])
                    AGGREGATE ([GLOBAL] aggregate [{158: sum=sum(158: sum)}] group by [[50: l_suppkey]] having [158: sum IS NOT NULL]
                        EXCHANGE SHUFFLE[50]
                            AGGREGATE ([LOCAL] aggregate [{158: sum=sum(59: sum_disc_price)}] group by [[50: l_suppkey]] having [null]
                                SCAN (mv[lineitem_agg_mv2] columns[50: l_suppkey, 51: l_shipdate, 59: sum_disc_price] predicate[51: l_shipdate >= 1995-07-01 AND 51: l_shipdate < 1995-10-01 AND 51: l_shipdate >= 1995-01-01 AND 51: l_shipdate < 1996-01-01 AND 59: sum_disc_price IS NOT NULL])
                    EXCHANGE BROADCAST
                                ...
                                AGGREGATE ([GLOBAL] aggregate [{44: max=max(44: max)}] group by [[]] having [null]
                                    EXCHANGE GATHER
                                        AGGREGATE ([LOCAL] aggregate [{44: max=max(43: sum)}] group by [[]] having [null]
                                            AGGREGATE ([GLOBAL] aggregate [{169: sum=sum(169: sum)}] group by [[50: l_suppkey]] having [null]
                                                EXCHANGE SHUFFLE[50]
                                                    AGGREGATE ([LOCAL] aggregate [{169: sum=sum(59: sum_disc_price)}] group by [[50: l_suppkey]] having [null]
