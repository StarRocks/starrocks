...
TOP-N (order by [[2: s_name ASC NULLS FIRST]])
    TOP-N (order by [[2: s_name ASC NULLS FIRST]])
        RIGHT SEMI JOIN (join-predicate [13: ps_suppkey = 1: s_suppkey] post-join-predicate [null])
            EXCHANGE SHUFFLE[13]
                INNER JOIN (join-predicate [12: ps_partkey = 28: l_partkey AND 13: ps_suppkey = 29: l_suppkey AND cast(14: ps_availqty as decimal(38, 3)) > multiply(0.5, 43: sum)] post-join-predicate [null])
