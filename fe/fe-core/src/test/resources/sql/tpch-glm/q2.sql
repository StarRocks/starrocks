                ...
                TOP-N (order by [[1: P_PARTKEY ASC NULLS FIRST]])
                    EXCHANGE SHUFFLE[1]
                        INNER JOIN (join-predicate [11: S_SUPPKEY = 20: PS_SUPPKEY] post-join-predicate [null])
                            FETCH (columns[[17: S_COMMENT,11: S_SUPPKEY,12: S_NAME,13: S_ADDRESS,15: S_PHONE,16: S_ACCTBAL]])
                                INNER JOIN (join-predicate [14: S_NATIONKEY = 25: N_NATIONKEY] post-join-predicate [null])
                                    SCAN (columns[60: ROW_ID, 14: S_NATIONKEY] predicate[null])
                                    EXCHANGE BROADCAST
                                        FETCH (columns[[25: N_NATIONKEY,26: N_NAME]])
                                            INNER JOIN (join-predicate [27: N_REGIONKEY = 30: R_REGIONKEY] post-join-predicate [null])
                                                SCAN (columns[27: N_REGIONKEY, 61: ROW_ID] predicate[null])
                                                EXCHANGE BROADCAST
                                                    SCAN (columns[30: R_REGIONKEY, 31: R_NAME] predicate[31: R_NAME = AMERICA])
                                            LookUp
                                LookUp
                            EXCHANGE SHUFFLE[20]
                                FETCH (columns[[20: PS_SUPPKEY,22: PS_SUPPLYCOST]])
                                    INNER JOIN (join-predicate [1: P_PARTKEY = 19: PS_PARTKEY] post-join-predicate [null])
                                        ...
                                        EXCHANGE SHUFFLE[19]
                                            SCAN (columns[19: PS_PARTKEY, 62: ROW_ID] predicate[null])
                                    LookUp
[end]
