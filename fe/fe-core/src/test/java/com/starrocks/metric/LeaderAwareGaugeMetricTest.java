...
public class LeaderAwareGaugeMetricTest {
    ...
    public void testLeaderAwareMetricLeaderOutputLeader(@Mocked GlobalStateMgr globalStateMgr) {
        ...
        createMetrics();
        {
            ...
            // _metricLong{is_master="true"} 3
            String output = visitor.build();
            Assert.assertTrue(output, output.contains("_metricLong{is_master=\"true\"} 3"));
        }
        {
            metricDoubleValue = -10.3;
            MetricVisitor visitor = new PrometheusMetricVisitor("");
            visitor.visit(metricDouble);
            // _metricDouble{is_master="true"} -10.3
            String output = visitor.build();
            Assert.assertTrue(output, output.contains("_metricDouble{is_master=\"true\"} -10.3"));
        }
    }
    ...
    public void testLeaderAwareMetricLeaderOutputNonLeader(@Mocked GlobalStateMgr globalStateMgr) {
        ...
        createMetrics();
        {
            ...
            MetricVisitor visitor = new PrometheusMetricVisitor("");
            visitor.visit(metricLong);
            // _metricLong{is_master="false"} 0
            String output = visitor.build();
            Assert.assertTrue(output, output.contains("_metricLong{is_master=\"false\"} 0"));
        }
        {
            metricDoubleValue = -10.3;
            MetricVisitor visitor = new PrometheusMetricVisitor("");
            visitor.visit(metricDouble);
            // _metricDouble{is_master="false"} -1.5
            String output = visitor.build();
            Assert.assertTrue(output, output.contains("_metricDouble{is_master=\"false\"} -1.5"));
        }
    }
}
