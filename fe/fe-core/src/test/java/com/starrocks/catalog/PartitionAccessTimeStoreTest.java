...
public class PartitionAccessTimeStoreTest {
    ...
    public void testBuildUpsertSql() {
        String sql = PartitionAccessTimeStore.buildUpsertSql(
                Lists.newArrayList(entry(1, 2, 3, 111L), entry(1, 2, 4, 222L)));
    ...
    public void testLoadAllDecodesJsonRows() {
        ...
        List<TPartitionAccessTimeEntry> all = new PartitionAccessTimeStore().loadAll();
        Assertions.assertEquals(2, all.size());
        Assertions.assertEquals(1L, all.get(0).getDb_id());
        Assertions.assertEquals(2L, all.get(0).getTable_id());
        Assertions.assertEquals(100L, all.get(0).getPartition_id());
        Assertions.assertEquals(111L, all.get(0).getAccess_time_ms());
        Assertions.assertEquals(300L, all.get(1).getPartition_id());
        Assertions.assertEquals(222L, all.get(1).getAccess_time_ms());
    }
