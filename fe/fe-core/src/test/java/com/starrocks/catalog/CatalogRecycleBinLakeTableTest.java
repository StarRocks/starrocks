...
public class CatalogRecycleBinLakeTableTest {
    ...
    private static void waitTableToBeDone(CatalogRecycleBin recycleBin, long id, long time) {
        // For Lake Tables with new partition-level deletion, we need to process partitions
        while (recycleBin.isDeletingTable(id) || recycleBin.isLakeTablePartitionsDeletionInProgress(id)) {
            recycleBin.erasePartition(time);
            if (recycleBin.isDeletingTable(id) && recycleBin.isDeletingTableDone(id)) {
                recycleBin.eraseTable(time);
            }
            try {
                Thread.sleep(100);
    ...
    private static void waitAllTablesToBeDone(CatalogRecycleBin recycleBin, List<Long> ids, long time) {
        // For Lake Tables with new partition-level deletion, tables may use lakeTableToPartitions instead of asyncDeleteForTables
        ids.forEach(x -> Assertions.assertTrue(
                recycleBin.isDeletingTable(x) || recycleBin.isLakeTablePartitionsDeletionInProgress(x)));
        ...
        while (doingCount > 0) {
            ...
            recycleBin.erasePartition(time);
            doingCount = ids.stream().filter(x ->
                    (recycleBin.isDeletingTable(x) && !recycleBin.isDeletingTableDone(x)) ||
                    recycleBin.isLakeTablePartitionsDeletionInProgress(x)).count();
            try {
                Thread.sleep(100);
            } catch (Exception ignore) {
                // nothing to do
