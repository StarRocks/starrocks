...
public class InsertOverwriteJobRunnerTest {
    ...
    public void testReplayInsertOverwrite() {
        Database database = GlobalStateMgr.getCurrentState().getMetastore().getDb("insert_overwrite_test");
        Table table = GlobalStateMgr.getCurrentState().getMetastore().getTable(database.getFullName(), "t1");
