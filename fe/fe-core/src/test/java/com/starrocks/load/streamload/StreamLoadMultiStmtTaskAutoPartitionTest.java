...
public class StreamLoadMultiStmtTaskAutoPartitionTest {
    ...
    @Test
    public void testReadonlyFirstTableIsRejectedBeforeRegistration() throws Exception {
        ...
        StreamLoadMultiStmtTask multiTask = beginMultiStmtTask("multi_stmt_readonly_first_table");
        long txnId = multiTask.getTxnId();
        try {
            ...
            Assertions.assertEquals(ErrorCode.ERR_BEGIN_TXN_FAILED, error.getErrorCode());
            Assertions.assertNull(txnMgr.getTransactionState(db.getId(), txnId));
            Assertions.assertTrue(txnMgr.getExplicitTxnState(txnId).getTransactionState().getTableIdList().isEmpty());
    ...
    @Test
    public void testReadonlySecondTableIsRejectedBeforeActivation() throws Exception {
        OlapTable readonlyTable = getTable("random_bucket");
        new MockUp<FailoverGroupMgr>() {
            @Mock
            public boolean isSecondaryReadonly(long dbId, List<Long> tableIdList) {
                return tableIdList.contains(readonlyTable.getId());
            }
        };

        StreamLoadMultiStmtTask multiTask = beginMultiStmtTask("multi_stmt_readonly_second_table");
        long txnId = multiTask.getTxnId();
        try {
            tryLoad(multiTask, "transaction_simple");
            TransactionState txnState = txnMgr.getTransactionState(db.getId(), txnId);
            ErrorReportException error = Assertions.assertThrows(ErrorReportException.class,
                    () -> multiTask.tryLoad(0, "random_bucket", new TransactionResult()));
            Assertions.assertEquals(ErrorCode.ERR_BEGIN_TXN_FAILED, error.getErrorCode());
            Assertions.assertEquals(List.of(getTable("transaction_simple").getId()), txnState.getTableIdList());
            Assertions.assertEquals(1, multiTask.getTasks().size());
        } finally {
            rollback(multiTask);
        }
        Assertions.assertEquals(TransactionStatus.ABORTED,
