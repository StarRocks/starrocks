...
/*
 * Mocked DataNode
 * A mocked DataNode has 3 rpc services.
 ...
 *
 * Better to create a mocked DataNode from MockedDataNodeFactory.
 * In MockedDataNodeFactory, there default rpc service for above 3 rpc services.
 */
public class MockedDataNode {
    ...
    private final MockPDataNodeService pbService;

    public MockedDataNode(String host) throws Exception {
        this.host = host;
        ...
        pbService = new MockPDataNodeService();
        ...
        new MockUp<BrpcProxy>() {
            @Mock
            protected synchronized PDataNodeService getDataNodeService(TNetworkAddress address) {
                return pbService;
            }
        };
    ...
    private static class MockBeThriftClient extends BackendService.Client {
        ...
        private final TBackend tDataNode;
        private long reportVersion = 0;
        private final LeaderImpl master = new LeaderImpl();

        public MockBeThriftClient(MockedDataNode backend) {
            super(null);

            tDataNode = new TBackend(backend.getHost(), backend.getBeThriftPort(), backend.getHttpPort());
            new Thread(() -> {
                while (true) {
                    try {
                        TAgentTaskRequest request = taskQueue.take();
                        TFinishTaskRequest finishTaskRequest = new TFinishTaskRequest(tDataNode,
    ...
    private static class MockPDataNodeService implements PDataNodeService {
