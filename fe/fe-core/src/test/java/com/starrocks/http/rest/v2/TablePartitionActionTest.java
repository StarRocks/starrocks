...
public class TablePartitionActionTest extends StarRocksHttpTestCase {
    ...
    public void testNonOlapTable() throws Exception {
        ...
        try (Response response = networkClient.newCall(request).execute()) {
            RestBaseResultV2<PagedResult<TablePartitionView>> resp = parseResponseBody(response.body().string());
    ...
    public void testOlapTable() throws Exception {
        ...
        try (Response response = networkClient.newCall(request).execute()) {
            ...
            PagedResult<TablePartitionView> tablePartition = resp.getResult();

            List<TablePartitionView> partitionItems = tablePartition.getItems();
            assertEquals(PARTITION_SIZE, partitionItems.size());

            {
                TablePartitionView partition = partitionItems.get(0);
    ...
    private static RestBaseResultV2<PagedResult<TablePartitionView>> parseResponseBody(String body) {
        try {
            System.out.println("resp: " + body);
            return GsonUtils.GSON.fromJson(
                    body,
                    new TypeToken<RestBaseResultV2<PagedResult<TablePartitionView>>>() {
                    }.getType());
        } catch (Exception e) {
