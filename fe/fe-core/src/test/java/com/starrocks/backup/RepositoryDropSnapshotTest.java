    ...
    public void testRepositoryDropSnapshotMethodsExist() {
        ...
        for (String testCase : testCases) {
            ...
            if (testCase == null || testCase.trim().isEmpty()) {
                ...
                Assertions.assertTrue(result.getErrMsg().contains("Snapshot name cannot be empty"));
            } else {
        ...
        for (String[] testCase : testCases) {
            ...
            switch (scenario) {
                ...
                case "empty_timestamp":
                    ...
                    Assertions.assertFalse(result.ok());
                    Assertions.assertTrue(result.getErrMsg().contains("Operator and timestamp cannot be empty"));
                    break;
                case "invalid_operator":
                    // Lines 358-359: Invalid operator validation
                    Assertions.assertFalse(result.ok());
                    Assertions.assertTrue(result.getErrMsg().contains("Invalid operator"));
                    break;
                case "list_failure":
                    // Lines 366-368: listSnapshots failure
                    Assertions.assertFalse(result.ok());
                    Assertions.assertTrue(result.getErrMsg().contains("Failed to list snapshots"));
                    break;
                case "no_snapshots":
                    // Lines 436-437: No snapshots found
                    Assertions.assertFalse(result.ok());
                    Assertions.assertTrue(result.getErrMsg().contains("No snapshots found matching the timestamp criteria"));
                    break;
                case "partial_failure":
                    // Lines 431-433: Partial failure
                    Assertions.assertFalse(result.ok());
                    Assertions.assertTrue(result.getErrMsg().contains("Deleted 1 snapshots, but 1 deletions failed"));
                    break;
                case "success":
                case "success_ge":
