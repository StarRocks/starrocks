...
public class StatisticsSQLTest extends PlanTestBase {
    ...
    public static void beforeClass() throws Exception {
        ...
        OlapTable t0 = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("stat0");
        t0StatsTableId = t0.getId();
    }

    @Test
    public void testSampleStatisticsSQL() throws Exception {
        Table t0 = GlobalStateMgr.getCurrentState().getMetastore().getDb("test").getTable("stat0");
    ...
    public void testFullStatisticsSQLWithStruct() throws Exception {
        Table t0 = GlobalStateMgr.getCurrentState().getMetastore().getDb("test").getTable("struct_a");
        Database db = GlobalStateMgr.getCurrentState().getMetastore().getDb("test");
        List<Long> pids = t0.getPartitions().stream().map(Partition::getId).collect(Collectors.toList());
    ...
    public void testEscapeFullSQL() throws Exception {
        Table t0 = GlobalStateMgr.getCurrentState().getMetastore().getDb("test").getTable("escape0['abc']");
        Database db = GlobalStateMgr.getCurrentState().getMetastore().getDb("test");
        List<Long> pids = t0.getPartitions().stream().map(Partition::getId).collect(Collectors.toList());
    ...
    public void testQuota() {
        Table t0 = GlobalStateMgr.getCurrentState().getMetastore().getDb("test").getTable("complex_table");
