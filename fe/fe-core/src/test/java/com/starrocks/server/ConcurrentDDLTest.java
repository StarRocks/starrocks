...
public class ConcurrentDDLTest {
    ...
    public void testConcurrentCreatingColocateTables() throws InterruptedException {
        ...
        Database db = GlobalStateMgr.getServingState().getMetastore().getDb("test");
        ...
        for (long threadId : threadIds) {
            table = GlobalStateMgr.getCurrentState().getMetastore().getTable(db.getFullName(), "test_tbl_" + threadId);
            ...
            Thread controlThread = new Thread(() -> {
                ...
                while (times < 5) {
                    try {
                        ...
                        Database db = GlobalStateMgr.getCurrentState().getMetastore().getDb("concurrent_test_db");
