...
public class MaterializedViewSSBTest extends MaterializedViewTestBase {
    ...
    public void testMvRewrite() throws Exception {
        starRocksAssert.withMaterializedView("CREATE MATERIALIZED VIEW `mv_leftjoin`\n" +
                ...
                "\"storage_medium\" = \"HDD\"\n" +
                ")\n" +
                "AS SELECT `lineorder`.`lo_orderkey`, `lineorder`.`lo_linenumber`, `lineorder`.`lo_custkey`," +
        ...
        String plan = getFragmentPlan(query);
        PlanTestBase.assertContains(plan, "mv_leftjoin");
    }
}
