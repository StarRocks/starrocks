...
class SelectStmtWithCaseWhenTest {
    ...
    private static Stream<Arguments> caseWhenWithNullableCol() {
        ...
        String[][] testCases = new String[][] {
                ...
                {"in ('D','E')", "(5: ship_code < 90) OR (5: ship_code IS NULL), (5: ship_code < 80) OR (5: ship_code IS NULL)"},
                {"in ('E')", "(5: ship_code < 90) OR (5: ship_code IS NULL), (5: ship_code < 80) OR (5: ship_code IS NULL)"},
                {"in (NULL)", "0:EMPTYSET"},
                {"in ('F')", "0:EMPTYSET"},
                {"in ('A','B','C','D','E')",
                        "  0:OlapScanNode\n" +
                ...
                {"in ('A','B','C','D','E','F')",
                        "  0:OlapScanNode\n" +
                                ...
                                "     cardinality: 1"},
                {"not in ('A','B', 'C')",
                        "(5: ship_code < 90) OR (5: ship_code IS NULL), (5: ship_code < 80) OR (5: ship_code IS NULL)"},
    ...
    private static Stream<Arguments> ifTests() {
        String[][] testCases = new String[][] {
                ...
                {"select * from test.t0 where if(ship_code is null or ship_code > 2, 2, 1) != 2",
                        "((5: ship_code IS NOT NULL) AND (5: ship_code <= 2)) OR"},
                ...
                {"with tmp as (select ship_mode, if(ship_code > 4, 1, 0) as layer0, " +
                        ...
                        "(13: ship_code IS NULL) OR (13: ship_code < 1), (13: ship_code <= 4) OR (13: ship_code IS NULL)",
                        "((13: ship_code < 1) OR (13: ship_code > 4)) OR ((13: ship_code >= 1) AND (13: ship_code <= 4) IS NULL)"
                },
