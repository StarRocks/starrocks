...
public class LowCardinalityTest extends PlanTestBase {
    ...
    public void testExtractProject() throws Exception {
        ...
        assertContains(plan, "2:AGGREGATE (update finalize)\n" +
                "  |  output: max(16: upper), min(16: upper), max(15: S_ADDRESS), sum(10: expr)\n" +
                "  |  group by: \n" +
                "  |  \n" +
                "  1:Project\n" +
                "  |  <slot 10> : CAST(1: S_SUPPKEY AS BIGINT) + 1\n" +
    ...
    public void testJoinWithMinMaxAgg() throws Exception {
        ...
        String plan = getCostExplain(sql);
        assertContains(plan, "  1:AGGREGATE (update finalize)\n" +
                "  |  aggregate: max[([91: S_ADDRESS, VARCHAR, true]); args: VARCHAR; " +
                "result: VARCHAR; args nullable: true; result nullable: true]\n" +
                "  |  group by: [90: S_NAME, CHAR, false]\n" +
                "  |  cardinality: 1\n" +
                "  |  column statistics: \n" +
