...
class WindowSkewToMergeSortRuleTest extends PlanTestBase {
    ...
    public void setUp() {
        ...
        connectContext.getSessionVariable().setForceWindowMergeSort(false);
    ...
    @Test
    void testForceVariableBypassesStatistics() throws Exception {
        connectContext.getSessionVariable().setForceWindowMergeSort(true);
        // No statistics injected at all - force mode should not care
        storage().refreshColumnStatistics(table(), ALL_COLUMNS, true);
    ...
    @Test
    void testForceVariableWithNonSkewedStats() throws Exception {
        connectContext.getSessionVariable().setForceWindowMergeSort(true);
        setNonSkewedStatsForS();

        String plan = getCostPlan(BASIC_WINDOW_SQL);
        // Force mode overrides statistics
        assertRuleApplied(plan);
    }
    ...
    @Test
    void testForceVariableTakesPrecedenceOverSplitWindowSkewToUnion() throws Exception {
    ...
    @Test
    void testMultiPartitionSingleSkewedColumnTriggers() throws Exception {
        connectContext.getSessionVariable().setEnableWindowSkewMergeSort(true);
        ...
        String plan = getCostPlan(sql);
        // Skew on any individual partition column is sufficient to trigger the rewrite.
        assertRuleApplied(plan);
    }
