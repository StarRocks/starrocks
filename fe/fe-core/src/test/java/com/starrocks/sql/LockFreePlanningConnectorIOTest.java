...
public class LockFreePlanningConnectorIOTest extends ConnectorPlanTestBase {
    ...
    private void probeConnectorCalls() {
        ...
        new MockUp<MetadataMgr>() {
            ...
            public Table getTable(Invocation invocation, ConnectContext context, String catalogName, String dbName,
                                  String tblName) {
                if (!CatalogMgr.isInternalCatalog(catalogName) && !calledFromTheAnalyzer()) {
    ...
    /**
     * Whether this call came from the analyzer resolving the statement's own tables, rather than from the
     * work this change moves off the lock.
     *
     * <p>On this branch the analyzer still resolves an external table with the lock held -- #68467 moved
     * that pre-pass ahead of the lock and is only on branch-4.1 and later -- so counting it would fail
     * these tests on a violation that belongs to a different fix. What this change is responsible for is
     * everything the authorization check and the optimizer ask the connector afterwards: statistics,
     * partition names and file lists, all still sampled.
     */
    private static boolean calledFromTheAnalyzer() {
        return StackWalker.getInstance().walk(frames -> frames.anyMatch(
                frame -> frame.getClassName().startsWith(QueryAnalyzer.class.getName())));
    }
