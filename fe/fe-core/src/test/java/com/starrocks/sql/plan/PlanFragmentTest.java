...
public class PlanFragmentTest extends PlanTestBase {
    ...
    public void testWindowFunctionTest() throws Exception {
        ...
        Assert.assertTrue(
                plan.contains("decimal_literal:TDecimalLiteral(value:0), output_scale:-1, use_vectorized:true, has_nullable_child:false, is_nullable:false, is_monotonic:true)"));
    }
    ...
    public void testEquivalenceTest() throws Exception {
        ...
        Assert.assertTrue(plan.contains("0:OlapScanNode\n" +
                "     TABLE: t0\n" +
                "     PREAGGREGATION: ON\n" +
                "     PREDICATES: CAST(2: v2 AS VARCHAR) = 'zxcv'"));
        Assert.assertTrue(plan.contains("  1:OlapScanNode\n"
                + "     TABLE: t0\n"
                + "     PREAGGREGATION: ON\n"
                + "     PREDICATES: CAST(5: v2 AS VARCHAR) = 'zxcv'\n"));
    }
    ...
    public void testEqStringCast() throws Exception {
        ...
        String plan = getFragmentPlan(sql);
        System.out.println(plan);
        Assert.assertTrue(plan.contains("CAST(1: v1 AS VARCHAR) = 'a'\n"));
    }
