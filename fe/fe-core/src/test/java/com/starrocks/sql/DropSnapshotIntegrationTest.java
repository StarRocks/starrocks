...
public class DropSnapshotIntegrationTest {
    ...
    public void testDropSnapshotSqlParsing() {
        ...
        try {
            ...
            Assertions.assertEquals(1, statements.size(), "Should parse exactly one statement");
            Assertions.assertTrue(statements.get(0) instanceof DropSnapshotStmt, "Should be DropSnapshotStmt");
            
            DropSnapshotStmt stmt = (DropSnapshotStmt) statements.get(0);
            Assertions.assertEquals("repo", stmt.getRepoName(), "Repository name should match");
            Assertions.assertNotNull(stmt.getWhere(), "WHERE clause should not be null");
            
            System.out.println("✓ Basic DROP SNAPSHOT parsing test passed");
        } catch (Exception e) {
            Assertions.fail("Failed to parse basic DROP SNAPSHOT statement: " + e.getMessage());
        }
    }
    ...
    public void testDropSnapshotWithTimestamp() {
        ...
        try {
            ...
            Assertions.fail("Failed to parse DROP SNAPSHOT with timestamp: " + e.getMessage());
        }
    }
    ...
    public void testDropSnapshotWithCompoundCondition() {
        ...
        try {
            ...
            DropSnapshotStmt stmt = (DropSnapshotStmt) statements.get(0);
            Assertions.assertEquals("Repository name should match", "repo", stmt.getRepoName());
            Assertions.assertNotNull(stmt.getWhere(), "WHERE clause should not be null");
            
            System.out.println("✓ DROP SNAPSHOT with compound condition parsing test passed");
        } catch (Exception e) {
            Assertions.fail("Failed to parse DROP SNAPSHOT with compound condition: " + e.getMessage());
        }
    }
    ...
    public void testDropSnapshotToSql() {
        ...
        try {
            ...
            Assertions.assertNotNull(generatedSql, "Generated SQL should not be null");
            Assertions.assertTrue(generatedSql.contains("DROP SNAPSHOT"), "Generated SQL should contain DROP SNAPSHOT");
            Assertions.assertTrue(generatedSql.contains("repo"), "Generated SQL should contain repository name");
            ...
            Assertions.fail("Failed to convert DROP SNAPSHOT statement to SQL: " + e.getMessage());
        }
    }
    ...
    public void testDropSnapshotAstProperties() {
        ...
        try {
            ...
            Assertions.assertEquals("test_repo", stmt.getRepoName(), "Repository name should match");
            Assertions.assertNotNull(stmt.getWhere(), "WHERE clause should not be null");
            
            // Test setter methods
            stmt.setSnapshotName("manual_snapshot");
            Assertions.assertEquals("manual_snapshot", stmt.getSnapshotName(), "Snapshot name should be set");
            
            stmt.setTimestamp("2024-01-01-12-00-00");
            Assertions.assertEquals("2024-01-01-12-00-00", stmt.getTimestamp(), "Timestamp should be set");
            
            stmt.setTimestampOperator("<=");
            Assertions.assertEquals("<=", stmt.getTimestampOperator(), "Timestamp operator should be set");
            
            // Test snapshot names list
            stmt.addSnapshotName("snapshot1");
            stmt.addSnapshotName("snapshot2");
            Assertions.assertEquals(2, stmt.getSnapshotNames().size(), "Should have 2 snapshot names");
            Assertions.assertTrue(stmt.getSnapshotNames().contains("snapshot1"), "Should contain snapshot1");
            Assertions.assertTrue(stmt.getSnapshotNames().contains("snapshot2"), "Should contain snapshot2");
            
            System.out.println("✓ DROP SNAPSHOT AST properties test passed");
        } catch (Exception e) {
            Assertions.fail("Failed to test DROP SNAPSHOT AST properties: " + e.getMessage());
        }
    }
    ...
    public void testDropSnapshotVariousFormats() {
        ...
        for (String sql : testSqls) {
            try {
                List<StatementBase> statements = SqlParser.parse(sql, 0);
                Assertions.assertEquals(1, statements.size(), "Should parse exactly one statement for: " + sql);
                Assertions.assertTrue(statements.get(0) instanceof DropSnapshotStmt, "Should be DropSnapshotStmt for: " + sql);
                
                DropSnapshotStmt stmt = (DropSnapshotStmt) statements.get(0);
                Assertions.assertNotNull(stmt.getRepoName(), "Repository name should not be null for: " + sql);
                
                System.out.println("✓ Parsed successfully: " + sql);
            } catch (Exception e) {
                Assertions.fail("Failed to parse SQL: " + sql + " - " + e.getMessage());
            }
        }
    ...
    public void testDropSnapshotInvalidSyntax() {
        ...
        for (String sql : invalidSqls) {
            try {
                ...
                Assertions.fail("Should have failed to parse invalid SQL: " + sql);
            } catch (Exception e) {
    ...
    public void testDropSnapshotVisitorPattern() {
        ...
        try {
            ...
            Assertions.assertEquals("Visitor should return expected result", "drop_snapshot_visited", result);
            
            System.out.println("✓ DROP SNAPSHOT visitor pattern test passed");
        } catch (Exception e) {
            Assertions.fail("Failed to test visitor pattern: " + e.getMessage());
        }
    }

    // Helper visitor class for testing
    private static class TestVisitor implements com.starrocks.sql.ast.AstVisitorExtendInterface<String, Void> {
        @Override
