...
public class DistributedEnvPlanTestBase extends PlanTestBase {
    ...
    public static void beforeClass() throws Exception {
        ...
        OlapTable t0 = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("region");
        setTableStatistics(t0, 5);

        OlapTable t5 = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("nation");
        setTableStatistics(t5, 25);

        OlapTable t1 = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("supplier");
        setTableStatistics(t1, 10000 * scale);

        OlapTable t4 = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("customer");
        setTableStatistics(t4, 150000 * scale);

        OlapTable t6 = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("part");
        setTableStatistics(t6, 200000 * scale);

        OlapTable t2 = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("partsupp");
        setTableStatistics(t2, 800000 * scale);

        OlapTable t3 = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("orders");
        setTableStatistics(t3, 1500000 * scale);

        OlapTable t7 = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("lineitem");
        setTableStatistics(t7, 6000000 * scale);

        OlapTable t8 = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("lineitem_partition");
        setTableStatistics(t8, 6000000 * scale);

        OlapTable testAllType = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("test_all_type");
        setTableStatistics(testAllType, 6000000);

        OlapTable lineorderNewL =
                (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("lineorder_new_l");
        setTableStatistics(lineorderNewL, 1200018434);

        OlapTable datesN = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("dates_n");
        setTableStatistics(datesN, 2556);

        OlapTable skewTable =
                (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("skew_table");
