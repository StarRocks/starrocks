...
public class FoldConstantsRuleTest {
    ...
    public void testNonDeterministicFunctionsNotFolded() {
        ...
        // Helper method to test a non-deterministic function
        testNonDeterministicFunction(FunctionSet.UUID, VarcharType.VARCHAR, 1);
        testNonDeterministicFunction(FunctionSet.RAND, TypeFactory.createType(PrimitiveType.DOUBLE), 2);
        testNonDeterministicFunction(FunctionSet.RANDOM, TypeFactory.createType(PrimitiveType.DOUBLE), 3);
    }
    
    private void testNonDeterministicFunction(String functionName, Type returnType, int uniqueId) {
        CallOperator call = new CallOperator(
                functionName, 
                returnType, 
                Lists.newArrayList(ConstantOperator.createInt(uniqueId)) // unique ID added by translator
        );
        
        Function fn = new Function(new FunctionName(functionName),
                new Type[] {IntegerType.INT}, returnType, false);
        ...
        ScalarOperator result = rule.apply(call, new ScalarOperatorRewriteContext());
