...
public class MvRewriteCaseWhenGroupByTest extends MVTestBase {
    ...
    @Test
    public void testCaseWhenRewriteRetainsPushedDownHaving() throws Exception {
        starRocksAssert.withTable("CREATE TABLE test_cw_having (id INT NULL, aux INT NULL, v BIGINT SUM NULL) " +
                "AGGREGATE KEY(id, aux) DISTRIBUTED BY HASH(id) BUCKETS 1 " +
                "PROPERTIES ('replication_num'='1')");
        String previousMode = connectContext.getSessionVariable().getMaterializedViewRewriteMode();
        boolean previousMultiStages = connectContext.getSessionVariable().isEnableMaterializedViewMultiStagesRewrite();
        try {
            executeInsertSql("INSERT INTO test_cw_having VALUES (1,0,10),(1,1,20),(2,0,30)");
            createAndRefreshMv("CREATE MATERIALIZED VIEW test_cw_having_mv " +
                    "DISTRIBUTED BY HASH(id) BUCKETS 1 REFRESH MANUAL PROPERTIES ('replication_num'='1') AS " +
                    "SELECT id, aux, v, CASE WHEN id=1 THEN 'a' ELSE 'b' END AS flag FROM test_cw_having");
            ...
            String plan = getFragmentPlan("SELECT id, aux, SUM(v) FROM test_cw_having " +
                    "WHERE CASE WHEN id=1 THEN 'a' ELSE 'b' END='a' GROUP BY id, aux HAVING SUM(v)>15");
            PlanTestBase.assertContains(plan, "TABLE: test_cw_having_mv", "v > 15");
        } finally {
            connectContext.getSessionVariable().setMaterializedViewRewriteMode(previousMode);
            connectContext.getSessionVariable().setEnableMaterializedViewMultiStagesRewrite(previousMultiStages);
            dropMv(DB_NAME, "test_cw_having_mv");
            starRocksAssert.dropTable("test_cw_having");
        }
    }

    @Test
    public void testCaseWhenRewriteAfterPartitionPredicatePruning() throws Exception {
        starRocksAssert.withTable("CREATE TABLE test_cw_partition (id INT, pt DATE NOT NULL, v BIGINT) " +
                "DUPLICATE KEY(id) PARTITION BY RANGE(pt) " +
                "(PARTITION p1 VALUES [('2025-12-12'), ('2025-12-13')), " +
                "PARTITION p2 VALUES [('2025-12-13'), ('2025-12-14')), " +
                "PARTITION p3 VALUES [('2025-12-14'), ('2025-12-15'))) " +
                "DISTRIBUTED BY HASH(id) BUCKETS 1 " +
                "PROPERTIES ('replication_num'='1')");
        boolean previousRewrite = connectContext.getSessionVariable().isEnableMaterializedViewRewrite();
        try {
            cluster.runSql(DB_NAME, "INSERT INTO test_cw_partition PARTITION(p1) VALUES (1,'2025-12-12',10)");
            cluster.runSql(DB_NAME, "INSERT INTO test_cw_partition PARTITION(p2) VALUES (2,'2025-12-13',20)");
            cluster.runSql(DB_NAME, "INSERT INTO test_cw_partition PARTITION(p3) VALUES (1,'2025-12-14',30)");
            createAndRefreshMv("CREATE MATERIALIZED VIEW test_cw_partition_mv " +
                    "DISTRIBUTED BY HASH(pt) BUCKETS 1 REFRESH MANUAL PROPERTIES ('replication_num'='1') AS " +
                    "SELECT pt, CASE WHEN id=1 THEN 'a' WHEN id=2 THEN 'b' ELSE 'c' END AS flag, SUM(v) AS total " +
                    "FROM test_cw_partition " +
                    "GROUP BY pt, CASE WHEN id=1 THEN 'a' WHEN id=2 THEN 'b' ELSE 'c' END");
            String query = "SELECT pt, SUM(v) FROM test_cw_partition " +
                    "WHERE pt >= '2025-12-12' AND pt < '2025-12-14' " +
                    "AND CASE WHEN id=1 THEN 'a' WHEN id=2 THEN 'b' ELSE 'c' END='a' GROUP BY pt";
            connectContext.getSessionVariable().setEnableMaterializedViewRewrite(false);
            String basePlan = getFragmentPlan(query);
            PlanTestBase.assertContains(basePlan, "partitions=2/3");
            PlanTestBase.assertNotContains(basePlan, "pt >=", "pt <");

            connectContext.getSessionVariable().setEnableMaterializedViewRewrite(true);
            String plan = planForceMv(query);
            PlanTestBase.assertContains(plan, "TABLE: test_cw_partition_mv", "flag = 'a'",
                    "pt >= '2025-12-12'", "pt < '2025-12-14'");
        } finally {
            connectContext.getSessionVariable().setEnableMaterializedViewRewrite(previousRewrite);
            dropMv(DB_NAME, "test_cw_partition_mv");
            starRocksAssert.dropTable("test_cw_partition");
        }
    }
