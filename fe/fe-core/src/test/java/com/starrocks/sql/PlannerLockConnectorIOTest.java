    ...
    private void probeConnectorCalls() {
        new MockUp<MetadataMgr>() {
            ...
            public List<String> listPartitionNames(Invocation invocation, String catalogName, String dbName,
                                                  String tableName, ConnectorMetadatRequestContext requestContext) {
    ...
    public void testConnectorMetadataStaysOutOfTheLockForEveryMixedShape() throws Exception {
        ...
        try {
            for (String sql : List.of(
                    ...
                    //
                    // Iceberg rather than JDBC: what abstains here is an immutable external table
                    // (CopyUnsafeTablesCollector.IMMUTABLE_EXTERNAL_TABLES), so a JDBC table is still
                    // copy-unsafe and still holds the lock for the whole phase -- as it did before this
                    // change. Widening the abstention to every table in an external catalog is #78613's
                    // job, not this one's; cover JDBC here once that has landed.
                    "SELECT * FROM test.t0 JOIN iceberg0.unpartitioned_db.t0 ON true",
                    "SELECT * FROM test.t0 JOIN hive0.partitioned_db.lineitem_par ON true "
                            + "JOIN iceberg0.unpartitioned_db.t0 ON true")) {
                underLock.clear();
                getFragmentPlan(sql);
                Assertions.assertFalse(underLock.isEmpty(),
