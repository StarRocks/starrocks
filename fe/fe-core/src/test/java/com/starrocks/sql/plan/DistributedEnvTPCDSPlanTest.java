...
public class DistributedEnvTPCDSPlanTest extends TPCDSPlanTestBase {
    ...
    public static void beforeClass() throws Exception {
        ...
        OlapTable customerAddress =
                (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("customer_address");
        setTableStatistics(customerAddress, 1000000);

        OlapTable customer = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("customer");
        setTableStatistics(customer, 2000000);

        OlapTable storeSales = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("store_sales");
        setTableStatistics(storeSales, 287997024);

        OlapTable dateDim = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("date_dim");
        setTableStatistics(dateDim, 73048);

        OlapTable item = (OlapTable) globalStateMgr.getMetastore().getDb("test").getTable("item");
