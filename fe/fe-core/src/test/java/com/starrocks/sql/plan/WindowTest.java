...
public class WindowTest extends PlanTestBase {
    ...
    public void testLagWindowFunction() throws Exception {
        ...
        // Third parameter is a string literal; FE harmonizes types (prefers third arg type STRING),
        // so this should analyze successfully and produce a plan.
        String acceptSql = "select lag(id_datetime, 1, '2020-01-01xxx') over(partition by t1c) from test_all_type;";
        String acceptPlan = getThriftPlan(acceptSql);
        assertContains(acceptPlan, "lag(8: id_datetime, 1, CAST('2020-01-01xxx' AS DATETIME))");

        // Prefer third parameter type: VARCHAR
        sql = "select lag(id_datetime, 1, cast('2020-01-02' as varchar(32))) over(order by t1c) from test_all_type";
        plan = getThriftPlan(sql);
        assertContains(plan, "lag");

        // Non-constant offset should fail
        String invalidOffsetSql = "select lag(id_datetime, t1c, '2020-01-01') over(partition by t1c) from test_all_type";
        SemanticException e2 = assertThrows(SemanticException.class, () -> getThriftPlan(invalidOffsetSql));
        Assertions.assertTrue(e2.getMessage()
                .contains("The offset parameter of LEAD/LAG must be a constant positive integer"), e2.getMessage());
    }
    ...
    public void testLeadAndLagFunction() throws Exception {
        ...
        starRocksAssert.query(sql).explainContains("functions: [, lead(3: k3, 3, CAST(abs(3: k3) AS INT)), ]");
    ...
    public void testRuntimeFilterPushWithRightPartition() throws Exception {
        ...
        assertContains(plan, "1:SORT\n"
                ...
                + "     table: t0, rollup: t0\n"
                + "     preAggregation: on\n"
                + "     Predicates: [1: v1, BIGINT, true] = 1, [1: v1, BIGINT, true] IS NOT NULL\n"
