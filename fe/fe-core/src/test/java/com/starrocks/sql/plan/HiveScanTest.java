...
public class HiveScanTest extends ConnectorPlanTestBase {
    ...
    @Test
    public void testHiveRewriteSimpleAggToHdfsScanBug() throws Exception {
        connectContext.getSessionVariable().setEnableRewriteSimpleAggToHdfsScan(true);

        // positive cases.
        {
            String[] sqlString = {
                    "select l_shipdate, count(*) from customer_union_view where l_shipdate = '1998-01-02' group by 1",
                    "select count(*) from customer_union_view",
            };
            for (int i = 0; i < sqlString.length; i++) {
                String sql = sqlString[i];
