...
public class AnalyzeCreateTableTest {
    ...
    @Test
    public void testPartitionByRangeExprTable() throws Exception {
        String sql = "CREATE TABLE `bill_detail` (\n" +
        ...
        String firstParamErrorSql = "CREATE TABLE `bill_detail` (\n" +
                "  `day` datetime \n" +
                ") ENGINE=OLAP \n" +
                "PRIMARY KEY(`day`)\n" +
                "PARTITION BY RANGE(cast(substr(day, 0, 13) as datetime))\n" +
                ...
                "\"in_memory\" = \"false\",\n" +
                "\"storage_format\" = \"DEFAULT\");";
        String exceptError = "Getting analyzing error from line 5, column 24 to line 5, column 41. Detail message: " +
                "Unsupported partition expression substr for column day type DATETIME, Cause: The first parameter " +
                "of substr must be a VARCHAR.";
        analyzeFail(firstParamErrorSql, exceptError);

        String seccondParamErrorSql = "CREATE TABLE `bill_detail` (\n" +
                "  `bill_code` varchar(200) NOT NULL DEFAULT \"\" COMMENT \"\"\n" +
                ") ENGINE=OLAP \n" +
                "PRIMARY KEY(`bill_code`)\n" +
                "PARTITION BY RANGE(cast(substr(bill_code, '3', 13) as bigint))\n" +
                ...
                "\"storage_format\" = \"DEFAULT\"\n" +
                ");";
        exceptError = "Getting analyzing error from line 5, column 24 to line 5, column 49. Detail message: " +
        ...
        analyzeFail(seccondParamErrorSql, exceptError);

        String thirtParamErrorSql = "CREATE TABLE `bill_detail` (\n" +
                "  `bill_code` varchar(200) NOT NULL DEFAULT \"\" COMMENT \"\"\n" +
                ") ENGINE=OLAP \n" +
                "PRIMARY KEY(`bill_code`)\n" +
                "PARTITION BY RANGE(cast(substr(bill_code, 3, '13') as bigint))\n" +
        ...
        analyzeFail(thirtParamErrorSql, exceptError);
    }
}
