...
public class ClusterSnapshotTest {
    ...
    public void testOperationOfAutomatedSnapshot() throws DdlException {
        ...
        GlobalStateMgr.getCurrentState().getClusterSnapshotMgr().createAutomatedSnaphot(job, false);
    ...
    public void testReplayClusterSnapshotLog() {
        ...
        logSnapshot.setCreateSnapshot(clusterSnapshotMgr.createNewAutomatedSnapshotJob());
        GlobalStateMgr.getCurrentState().getClusterSnapshotMgr().replayLog(logCreate);
    ...
    public void testCheckpointCoordination() {
        ...
        context.setImageDir("/root/meta/", true);
        context.setImageDir("/root/meta/starmgr", false);
        ...
        for (Boolean f : flags) {
            ...
            // case 1: normal case, cross execution
            {
                ...
                long consistentId1 = context.tryToAcquireCheckpointId(f1, true);
                long consistentId2 = context.tryToAcquireCheckpointId(f2, true);
                Assert.assertTrue(consistentId1 != ClusterSnapshotCheckpointContext.INVALID_JOURANL_ID);
                Assert.assertTrue(consistentId2 != ClusterSnapshotCheckpointContext.INVALID_JOURANL_ID);
                Assert.assertTrue(context.uploadImageForSnapshot(f1));
                Assert.assertTrue(context.uploadImageForSnapshot(f2));
                context.updateRoundIdAndMarkJobFinishedFromLaggard(f1);
                context.updateRoundIdAndMarkJobFinishedFromLaggard(f2);
                Assert.assertTrue(GlobalStateMgr.getCurrentState().getClusterSnapshotMgr().getAutomatedSnapshot() != null);
                dropAutomatedSnapshotRequest(false);
            }
            ...
            // case 3: normal case, peer is super slow
            {
                ...
                Assert.assertTrue(context.tryToAcquireCheckpointId(f1, true) ==
                                  ClusterSnapshotCheckpointContext.INVALID_JOURANL_ID);
                Assert.assertTrue(context.tryToAcquireCheckpointId(f1, true) ==
                                  ClusterSnapshotCheckpointContext.INVALID_JOURANL_ID);
                Assert.assertTrue(context.tryToAcquireCheckpointId(f1, true) ==
                                  ClusterSnapshotCheckpointContext.INVALID_JOURANL_ID);
            ...
            // case 4: error case, double upload failure, cross execution
            {
                ...
                Assert.assertTrue(context.tryToAcquireCheckpointId(f2, true) !=
                                  ClusterSnapshotCheckpointContext.INVALID_JOURANL_ID);
                Assert.assertTrue(context.tryToAcquireCheckpointId(f1, true) !=
                                  ClusterSnapshotCheckpointContext.INVALID_JOURANL_ID);
                Assert.assertTrue(context.uploadImageForSnapshot(f1));
            ...
            // case 5: error case, leader upload failure, peer success
            {
                ...
                // reset by peers, and begin a new round
                Assert.assertTrue(context.tryToAcquireCheckpointId(f2, true) !=
                                  ClusterSnapshotCheckpointContext.INVALID_JOURANL_ID);
                Assert.assertTrue(context.tryToAcquireCheckpointId(f1, true) !=
                                  ClusterSnapshotCheckpointContext.INVALID_JOURANL_ID);
