...
public class AlterTest {
    ...
    public static void tearDown() throws Exception {
        ...
        try {
            ...
            GlobalStateMgr.getCurrentState().getStarRocksMeta().dropTable(dropTableStmt);
        } catch (Exception ex) {
    ...
    private static void createMaterializedView(String sql) throws Exception {
        ...
        GlobalStateMgr.getCurrentState().getStarRocksMeta().createMaterializedView(createMaterializedViewStatement);
    }
    ...
    private static void dropMaterializedView(String sql) throws Exception {
        ...
        GlobalStateMgr.getCurrentState().getStarRocksMeta().dropMaterializedView(dropMaterializedViewStmt);
    }
    ...
    private static void cancelRefreshMaterializedView(String sql, boolean expectedException) throws Exception {
        ...
        try {
            GlobalStateMgr.getCurrentState().getStarRocksMeta().cancelRefreshMaterializedView(cancelRefresh);
            if (expectedException) {
                Assert.fail();
            }
        } catch (Exception e) {
    ...
    private static void alterTableWithNewParserAndExceptionMsg(String sql, String msg) throws Exception {
        AlterTableStmt alterTableStmt = (AlterTableStmt) UtFrameUtils.parseStmtWithNewParser(sql, connectContext);
        try {
            GlobalStateMgr.getCurrentState().getStarRocksMeta().alterTable(connectContext, alterTableStmt);
        } catch (Exception e) {
    ...
    public void testRenameMaterializedView() throws Exception {
        ...
        MaterializedView materializedView = (MaterializedView) GlobalStateMgr.getCurrentState().getStarRocksMeta().
    ...
    public void testCouldNotFindMaterializedView() throws Exception {
        ...
        OlapTable table = (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta()
                .getDb("test").getTable("testTable1");
    ...
    public void testConflictAlterOperations() throws Exception {
        Database db = GlobalStateMgr.getCurrentState().getStarRocksMeta().getDb("test");
        OlapTable tbl = (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta().getTable(db.getFullName(), "tbl1");
        ...
        OlapTable tbl2 = (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta().getTable(db.getFullName(), "tbl2");
    ...
    public void testBatchUpdatePartitionProperties() throws Exception {
        Database db = GlobalStateMgr.getCurrentState().getStarRocksMeta().getDb("test");
        OlapTable tbl4 = (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta().getTable(db.getFullName(), "tbl4");
    ...
    public void testDynamicPartitionDropAndAdd() throws Exception {
        ...
        Database db = GlobalStateMgr.getCurrentState().getStarRocksConnector().getDb("test");
        OlapTable tbl = (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksConnector().getTable(db.getFullName(), "tbl3");
    ...
    */
    ...
    public void testDynamicPartitionTableMetaFailed() throws Exception {
        ...
        String createOlapTblStmt = "CREATE TABLE test.`" + tableName + "` (\n" +
                ...
                ");";
        createTable(createOlapTblStmt);
        OlapTable olapTable = (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta()
                .getDb("test").getTable(tableName);
    ...
    public void testSwapTable() throws Exception {
        ...
        OlapTable replace1 =
                (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta().getTable(db.getFullName(), "replace1");
        OlapTable replace2 =
                (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta().getTable(db.getFullName(), "replace2");
        ...
        replace1 = (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta().getTable(db.getFullName(), "replace1");
        replace2 = (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta().getTable(db.getFullName(), "replace2");
        Assert.assertEquals(1,
                    replace1.getPartition("replace1").getMaterializedIndices(MaterializedIndex.IndexExtState.VISIBLE)
                                .size());
        Assert.assertEquals(3,
    ...
    public void testCatalogAddPartitionsDay() throws Exception {
        ...
        Table table = GlobalStateMgr.getCurrentState().getStarRocksMeta().getDb("test")
    ...
    public void testAddPhysicalPartition() throws Exception {
        ...
        StarRocksAssert.utCreateTableWithRetry(createTableStmt);
        Database db = GlobalStateMgr.getCurrentState().getStarRocksMeta().getDb("test");
        OlapTable table = (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta().getDb("test")
                .getTable("test_partition");
        ...
        GlobalStateMgr.getCurrentState().getStarRocksMeta().addSubPartitions(db, table, partition.get(), 1,
                WarehouseManager.DEFAULT_WAREHOUSE_ID);
        Assert.assertEquals(partition.get().getSubPartitions().size(), 2);
        Assert.assertEquals(table.getPhysicalPartitions().size(), 2);

        GlobalStateMgr.getCurrentState().getStarRocksMeta().addSubPartitions(db, table, partition.get(), 2,
                WarehouseManager.DEFAULT_WAREHOUSE_ID);
    ...
    public void testAddRangePhysicalPartition() throws Exception {
        ...
        GlobalStateMgr.getCurrentState().getStarRocksMeta().addSubPartitions(db, table, partition, 1,
                WarehouseManager.DEFAULT_WAREHOUSE_ID);
        Assert.assertEquals(table.getPhysicalPartitions().size(), 4);
        ...
        GlobalStateMgr.getCurrentState().getStarRocksMeta().addSubPartitions(db, table, partition, 2,
                WarehouseManager.DEFAULT_WAREHOUSE_ID);
    ...
    public void testModifyPartitionBucket() throws Exception {
        ...
        GlobalStateMgr.getCurrentState().getStarRocksMeta().alterTable(ctx, alterTableStmt);
    }
    ...
    public void testAddSingleItemListPartition() throws Exception {
        ...
        AddPartitionClause addPartitionClause =
                new AddPartitionClause(partitionDesc, null, new HashMap<>(), false);
        OlapTable table =
                (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta().getTable(db.getFullName(), "test_partition");
        AlterTableClauseAnalyzer analyzer = new AlterTableClauseAnalyzer(table);
        analyzer.analyze(Util.getOrCreateConnectContext(), addPartitionClause);
        GlobalStateMgr.getCurrentState().getStarRocksMeta()
                .addPartitions(Util.getOrCreateConnectContext(), db, "test_partition", addPartitionClause);
    ...
    public void testSingleItemPartitionPersistInfo() throws Exception {
        ...
        Database db = GlobalStateMgr.getCurrentState().getStarRocksMeta().getDb("test");
        OlapTable table = (OlapTable) GlobalStateMgr.getCurrentState().getStarRocksMeta()
                .getTable(db.getFullName(), "test_partition");
        ListPartitionInfo partitionInfo = (ListPartitionInfo) table.getPartitionInfo();
    ...
    public void testAddSingleListPartitionSamePartitionNameShouldThrowError() throws Exception {
        ...
        Table table = GlobalStateMgr.getCurrentState().getStarRocksMeta().getTable(db.getFullName(), "test_partition_1");
        AlterTableClauseAnalyzer analyzer = new AlterTableClauseAnalyzer(table);
        analyzer.analyze(Util.getOrCreateConnectContext(), addPartitionClause);
    ...
    public void testAddSingleListPartitionSamePartitionValueShouldThrowError() throws Exception {
        ...
        PartitionDesc partitionDesc = new SingleItemListPartitionDesc(false, "p3", values, new HashMap<>());
        AddPartitionClause addPartitionClause = new AddPartitionClause(partitionDesc, null, new HashMap<>(), false);
        Table table = GlobalStateMgr.getCurrentState().getStarRocksMeta().getTable(db.getFullName(), "test_partition_3");
        AlterTableClauseAnalyzer analyzer = new AlterTableClauseAnalyzer(table);
        analyzer.analyze(Util.getOrCreateConnectContext(), addPartitionClause);
    }
    ...
    public void testAddMultiItemListPartitionSamePartitionValueShouldThrowError() throws Exception {
        ...
        Table table = GlobalStateMgr.getCurrentState().getStarRocksMeta().getTable(db.getFullName(), "test_partition_4");
        AlterTableClauseAnalyzer analyzer = new AlterTableClauseAnalyzer(table);
        analyzer.analyze(Util.getOrCreateConnectContext(), addPartitionClause);
    }
    ...
    public void testCreateTemporaryPartitionInBatch() throws Exception {
        ...
        CreateTableStmt createTableStmt = (CreateTableStmt) UtFrameUtils.parseStmtWithNewParser(createSQL, ctx);
        StarRocksAssert.utCreateTableWithRetry(createTableStmt);
        Database db = GlobalStateMgr.getCurrentState().getStarRocksMeta().getDb("test2");
        ...
        Table table = GlobalStateMgr.getCurrentState().getStarRocksMeta().getDb("test2")
    ...
    public void testAlterMvWithResourceGroup() throws Exception {
        ...
        GlobalStateMgr.getCurrentState().getStarRocksMeta().alterMaterializedView(alterTableStmt);
        ...
        Assert.assertThrows("resource_group not_exist_rg does not exist.",
                ...
                () -> GlobalStateMgr.getCurrentState().getStarRocksMeta().alterMaterializedView(alterTableStmt2));
        sql = "ALTER MATERIALIZED VIEW mv2\n" +
                "set (\"resource_group\" =\"mv_rg\" )";
        AlterMaterializedViewStmt alterTableStmt3 =
                (AlterMaterializedViewStmt) UtFrameUtils.parseStmtWithNewParser(sql, ctx);
        GlobalStateMgr.getCurrentState().getStarRocksMeta().alterMaterializedView(alterTableStmt3);
        Assert.assertEquals("mv_rg", mv.getTableProperty().getResourceGroup());
        ...
        AlterMaterializedViewStmt alterTableStmt4 =
                (AlterMaterializedViewStmt) UtFrameUtils.parseStmtWithNewParser(sql, ctx);
        GlobalStateMgr.getCurrentState().getStarRocksMeta().alterMaterializedView(alterTableStmt4);
