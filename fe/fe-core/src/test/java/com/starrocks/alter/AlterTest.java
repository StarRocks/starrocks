...
//        alterTable(stmt, true);
    ...
    */
    ...
    public void testCatalogAddColumns() throws Exception {
        OlapTable tbl = (OlapTable) GlobalStateMgr.getCurrentState().getDb("default_cluster:test")
                .getTable("tbl1");
        ...
        Assert.assertEquals(tbl.getColumns().size(), 5);
    }
    ...
    public void testCatalogDropColumn() throws Exception {
        ...
        stmt = "alter table test.tbl1 add column k3 bigint after k2";
        alterTableWithNewParser(stmt, false);
        waitSchemaChangeJobDone(false, tbl);

        stmt = "alter table test.tbl1 drop column k3";
        alterTableWithNewParser(stmt, false);
        waitSchemaChangeJobDone(false, tbl);

        Assert.assertEquals(tbl.getColumns().size(), 3);
    }
    ...
    public void testCatalogModifyColumn() throws Exception {
        ...
        stmt = "alter table test.tbl1 add column k3 int after k2";
        alterTableWithNewParser(stmt, false);
        waitSchemaChangeJobDone(false, tbl);
    ...
    public void testCatalogReorderColumns() throws Exception {
        ...
        Assert.assertEquals("ORDER BY `k1`, `k2` FROM `testRollup`", clause.toString());
    }
}
