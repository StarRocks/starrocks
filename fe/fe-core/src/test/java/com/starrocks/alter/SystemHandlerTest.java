    ...
    public void testAddBackendIntoCNGroup() throws StarRocksException {
        ...
        }
        {
            ...
            RuntimeException exception = Assert.assertThrows(RuntimeException.class,
                    () -> systemHandler.process(Lists.newArrayList(clause), null, null));
            Assert.assertTrue(exception.getMessage().contains(ErrorCode.ERR_CN_GROUP_NOT_IMPLEMENTED.formatErrorMsg()));
            ConnectContext connCtx = ConnectContext.get();
            Assert.assertEquals(ErrorCode.ERR_CN_GROUP_NOT_IMPLEMENTED, connCtx.getState().getErrorCode());
        }
    }
