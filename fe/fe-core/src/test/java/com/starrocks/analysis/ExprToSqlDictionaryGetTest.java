 ...
 *
 * <p>Backport note: on this branch the printer lives in {@code DictionaryGetExpr.toSqlImpl()}
 * rather than in the {@code ExprExplainVisitor} the trunk change touched, so the assertions go
 * through {@code Expr#toSql()}. The cases and the expected argument shape are unchanged.
 */
public class ExprToSqlDictionaryGetTest {
    ...
    private static void assertArgs(int keySize, boolean nullIfNotExist, boolean explicitFlagChild) {
        String sql = dictGet(keySize, nullIfNotExist, explicitFlagChild).toSql();
