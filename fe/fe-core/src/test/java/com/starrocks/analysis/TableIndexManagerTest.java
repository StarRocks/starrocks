...
public class TableIndexManagerTest extends PlanTestBase {

    private final TableIndexManager manager = TableIndexManager.getInstance();
    ...
    public void testCreateIndex() {
        IndexDef indexDef = new IndexDef("inverted_index", Collections.singletonList("f1"), IndexType.INVERTED, "",
                new HashMap<String, String>() {{
                    put("tmp_test_param", "1");
                }});
        Table table = connectContext.getGlobalStateMgr().getDb("test").getTable("test_index_tbl");
        Index indexFromDef = manager.createIndexFromDef(table, indexDef);
    ...
    public void testCheckInvertedIndex() {
        ...
        Assertions.assertThrows(
                ...
                () -> TableIndexManager.checkInvertedIndexValid(c1, null),
                "The inverted index can only be build on column with type of scalar type.");

        Column c2 = new Column("f2", Type.STRING, true);
        Assertions.assertThrows(
                SemanticException.class,
                () -> TableIndexManager.checkInvertedIndexValid(c2, new HashMap<String, String>() {{
        ...
        Assertions.assertThrows(
                SemanticException.class,
                () -> TableIndexManager.checkInvertedIndexValid(c3, new HashMap<String, String>() {{
                    put(IMP_LIB.name().toLowerCase(Locale.ROOT), InvertedIndexImpType.CLUCENE.name());
    ...
    public void testCreateIndexFromStmt() {
        ...
        Index index1 = new Index("idx1", Collections.singletonList(c1.getName()), IndexType.INVERTED, "", Collections.emptyMap());
        Index index2 = new Index("idx2", Collections.singletonList(c2.getName()), IndexType.INVERTED, "", Collections.emptyMap());

        OlapTable table = (OlapTable) connectContext.getGlobalStateMgr().getDb("test").getTable("test_index_tbl");

        Assertions.assertTrue(manager.createIndexesFromCreateStmt(Collections.emptyList(), table).getIndexes().isEmpty());

        index1.setIndexId(100);
        Assertions.assertThrows(IllegalArgumentException.class,
                () -> manager.createIndexesFromCreateStmt(Arrays.asList(index1, index2), null),
                "All status of index indexId should be consistent");
        index1.setIndexId(-1);


        TableIndexes tableIndexes =
                Assertions.assertDoesNotThrow(() -> manager.createIndexesFromCreateStmt(Arrays.asList(index1, index2), null));
        Assertions.assertTrue(tableIndexes.getIndexes().stream().allMatch(index -> index.getIndexId() >= 0));
        index1.setIndexId(-1);
        index2.setIndexId(-1);

        table.setMaxIndexId(10);
        manager.createIndexesFromCreateStmt(Arrays.asList(index1, index2), table);
