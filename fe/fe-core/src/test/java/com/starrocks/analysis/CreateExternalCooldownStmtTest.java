...
public class CreateExternalCooldownStmtTest {
    ...
    @Test
    public void testBasicCreateExternalCooldown() throws Exception {
        ConnectContext ctx = starRocksAssert.getCtx();
        ctx.setExecutionId(UUIDUtil.toTUniqueId(UUIDUtil.genUUID()));
        String createExternalCooldownSQL = "COOLDOWN TABLE tbl1";
        CreateExternalCooldownStmt stmt = (CreateExternalCooldownStmt) UtFrameUtils.parseStmtWithNewParser(
        ...
        String createSQL2 = "COOLDOWN TABLE tbl1 PARTITION START ('2020-01-01 00:00:00') END ('2020-01-13 00:00:00')";
        CreateExternalCooldownStmt stmt2 = (CreateExternalCooldownStmt) UtFrameUtils.parseStmtWithNewParser(
                createSQL2, ctx);
        ExternalCooldownAnalyzer.analyze(stmt2, ctx);
        Assert.assertEquals(TEST_DB_NAME, stmt2.getTableName().getDb());
        Assert.assertEquals(TEST_TBL_NAME, stmt2.getTableName().getTbl());
        Assert.assertNotNull(stmt2.getPartitionRangeDesc());
        Assert.assertFalse(stmt2.isForce());

        String createSQL3 = "COOLDOWN TABLE tbl1 PARTITION START ('2020-01-01 00:00:00') END ('2020-01-13 00:00:00') FORCE";
        CreateExternalCooldownStmt stmt3 = (CreateExternalCooldownStmt) UtFrameUtils.parseStmtWithNewParser(
                createSQL3, ctx);
        ExternalCooldownAnalyzer.analyze(stmt3, ctx);
        Assert.assertEquals(TEST_DB_NAME, stmt3.getTableName().getDb());
        Assert.assertEquals(TEST_TBL_NAME, stmt3.getTableName().getTbl());
        Assert.assertNotNull(stmt3.getPartitionRangeDesc());
        Assert.assertTrue(stmt3.isForce());
        String s = "COOLDOWN TABLE `test`.`tbl1` PARTITION START ('2020-01-01 00:00:00') END ('2020-01-13 00:00:00') FORCE";
        Assert.assertEquals(s, stmt3.toSql());
        Task task = TaskBuilder.buildExternalCooldownTask(stmt3);
        Assert.assertNotNull(task);
    }

    @Test
    public void testCreateExternalCooldownOnNonOlapTable() throws Exception {
        ConnectContext ctx = starRocksAssert.getCtx();
        ctx.setExecutionId(UUIDUtil.toTUniqueId(UUIDUtil.genUUID()));
        String createExternalCooldownSQL = "COOLDOWN TABLE iceberg0.partitioned_transforms_db.t0_day";
        CreateExternalCooldownStmt stmt = (CreateExternalCooldownStmt) UtFrameUtils.parseStmtWithNewParser(
                createExternalCooldownSQL, ctx);
        ExternalCooldownAnalyzer.analyze(stmt, ctx);
        try {
            TaskBuilder.buildExternalCooldownTask(stmt);
        } catch (Exception e) {
            Assert.assertTrue(e instanceof SemanticException);
            Assert.assertTrue(e.getMessage().contains("only support cooldown for olap table"));
        }
    }
}
