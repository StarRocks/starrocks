...
public class CTASAutoTabletTest {
    ...
    public void testCTASAutoTablet() throws Exception {
        ...
        try {
            ...
            // ctas1
            table = (OlapTable) GlobalStateMgr.getCurrentState().getMetastore().getTable(db.getFullName(), "ctas1");
            if (table == null) {
                return;
            }
            for (Partition partition : table.getPartitions()) {
            ...
            // ctas2
            table = (OlapTable) GlobalStateMgr.getCurrentState().getMetastore().getTable(db.getFullName(), "ctas2");
            if (table == null) {
                return;
            }
            for (Partition partition : table.getPartitions()) {
