...
public class StartSchedulingTest extends SchedulerTestBase {
    ...
    public void testDeploySuccess() throws Exception {
        ...
        Assert.assertEquals(JobStateType.EXECUTING, scheduler.getState());
    }
    ...
    public void testDeployFutureThrowException() throws Exception {
        ...
        deployFuture.setRef(mockFutureWithException(new ExecutionException("test execution exception", new Exception())));
        ...
        DefaultScheduler scheduler = getScheduler(sql);
        Assert.assertThrows("test interrupted exception", UserException.class, () -> scheduler.startScheduling(true));

        // The deployed executions haven't reported.
        Assert.assertFalse(scheduler.isDone());
        Assert.assertEquals(JobStateType.FAILING, scheduler.getState());
        ...
        Assert.assertTrue(executionDAG.getExecutionIndexesInJob().size() < executionDAG.getInstanceIds().size());
    ...
    public void testDeployTimeout() throws Exception {
        ...
        try {
            ...
            String sql = "select count(1) from lineitem t1 JOIN [shuffle] lineitem t2 using(l_orderkey)";
            DefaultScheduler scheduler = getScheduler(sql);
            Assert.assertThrows("deploy query timeout", UserException.class, () -> scheduler.startScheduling(true));
