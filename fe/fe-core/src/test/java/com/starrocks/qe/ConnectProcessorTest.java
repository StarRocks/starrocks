...
public class ConnectProcessorTest extends DDLTestBase {
    ...
    @ParameterizedTest
    @ValueSource(strings = {
            "ALTER AI PROVIDER m SET ('api_key'='do-not-expose')",
            "ALTER AI PROVIDER m SET (' api_key '='do-not-expose')",
            "ALTER AI PROVIDER m SET ('\\api_key'='do-not-expose')",
            "ALTER AI PROVIDER m SET ('\\tapi_key\\n'='do-not-expose')",
            "ALTER AI PROVIDER m SET (' endpoint '='https://user:do-not-expose@models.example.test/v1')",
            "ALTER AI PROVIDER m SET ('end\\point'='https://user:do-not-expose@models.example.test/v1')",
            "ALTER AI PROVIDER m SET (\" end\\point \"=\"https://models.example.test/v1?token=do-not-expose\")",
            "ALTER AI PROVIDER m SET ('endpoint'='https://user:do-not-expose@models.example.test/v1')",
            "ALTER AI PROVIDER m SET ('endpoint'='https:\\/\\/user:do-not-expose@models.example.test/v1')",
            "ALTER AI PROVIDER m SET ('endpoint'='https://user:part''do-not-expose@models.example.test/v1')",
            "ALTER AI PROVIDER m SET ('endpoint' /* note */ = 'https://user:do-not-expose@models.example.test/v1')",
            "ALTER AI PROVIDER m SET ('endpoint'='https://models.example.test/v1?token=do-not-expose')",
            "ALTER AI PROVIDER m SET ('endpoint'='https://models.example.test/v1#do-not-expose')",
    ...
    public void testAuditPreservesSafeEndpointProperty() throws Exception {
        ...
        try {
            Config.enable_audit_sql = true;
            Config.enable_sql_desensitize_in_log = false;
            String sql = "ALTER AI PROVIDER m SET ('endpoint'='https://models.example.test/v1')";
            StatementBase statement = com.starrocks.sql.parser.SqlParser.parseSingleStatement(sql, 0);
            Method formatStmt = ConnectProcessor.class.getDeclaredMethod("formatStmt", String.class, StatementBase.class);
            formatStmt.setAccessible(true);
            Assertions.assertEquals(sql, formatStmt.invoke(new ConnectProcessor(myContext), sql, statement));
            Assertions.assertEquals("AI provider does not exist", ConnectProcessor.auditedErrorMessage(
                    "AI provider does not exist", sql));
        } finally {
            Config.enable_audit_sql = originalAuditSql;
            Config.enable_sql_desensitize_in_log = originalDesensitize;
        }
