...
public class TrinoFunctionTransformTest extends TrinoTestBase {
    ...
    public void testArrayFnTransform() throws Exception {
        ...
        assertPlanContains(sql, "array_concat(2: c1, CAST(3: c2 AS ARRAY<VARCHAR>), " +
                "CAST(ARRAY<tinyint(4)>[1,2] AS ARRAY<VARCHAR>), CAST(ARRAY<tinyint(4)>[3,4] AS ARRAY<VARCHAR>))");

        sql = "select concat(c2, 2) from test_array";
        assertPlanContains(sql, "array_concat(3: c2, CAST(ARRAY<tinyint(4)>[2] AS ARRAY<INT>))");
    ...
    public void testArrayFnWithLambdaExpr() throws Exception {
        ...
        assertPlanContains(sql, "array_filter(ARRAY<tinyint(4)>[5,-6,NULL,7], " +
                "array_map(<slot 2> -> <slot 2> > 0, ARRAY<tinyint(4)>[5,-6,NULL,7]))");

        sql = "select filter(array[5, NULL, 7, NULL], x -> x IS NOT NULL);";
        assertPlanContains(sql, "array_filter(ARRAY<tinyint(4)>[5,NULL,7,NULL], " +
                "array_map(<slot 2> -> <slot 2> IS NOT NULL, ARRAY<tinyint(4)>[5,NULL,7,NULL]))");

        sql = "select array_sort(array[1, 2, 3])";
        assertPlanContains(sql, "array_sort(ARRAY<tinyint(4)>[1,2,3])");
    }
