...
public class IcebergRESTCatalogAuthRecoveryTest {
    ...
    public void setUp() {
        ...
        // buildViewProperties puts getFeVersion() into an ImmutableMap, which rejects the null default
        new MockUp<Frontend>() {
            @Mock
    ...
    public void testFailedRetryPropagatesAfterSingleRebuild() {
        ...
        Assertions.assertThrows(StarRocksConnectorException.class, () -> catalog.getDB("db1"));
        new Verifications() {
            {
                restCatalog.initialize(anyString, (Map<String, String>) any);
                times = 2;
    ...
    public void testWrappedOperationsPassThroughOnSuccess() {
        ...
        IcebergRESTCatalog catalog = newOAuth2CredentialCatalog();
        Assertions.assertNotNull(catalog.getTable("db1", "t1"));
        Assertions.assertFalse(catalog.tableExists("db1", "t1"));
        Assertions.assertTrue(catalog.listAllDatabases().isEmpty());
        catalog.createDB("db2", null);
        Assertions.assertEquals("s3://bucket/db1", catalog.getDB("db1").getLocation());
        Assertions.assertDoesNotThrow(() -> catalog.dropDB("db1"));
        Assertions.assertTrue(catalog.listTables("db1").isEmpty());
        Assertions.assertTrue(catalog.createTable("db1", "t1", null, null, null, ImmutableMap.of()));
        Assertions.assertFalse(catalog.dropTable("db1", "t1", true));
        catalog.renameTable("db1", "t1", "t2");
        Assertions.assertNotNull(catalog.getView("db1", "v1"));
        Assertions.assertFalse(catalog.dropView("db1", "v1"));
        Assertions.assertEquals("s3://bucket/db1",
                catalog.loadNamespaceMetadata(Namespace.of("db1")).get("location"));
        new Verifications() {
            {
                restCatalog.initialize(anyString, (Map<String, String>) any);
    ...
    public void testCreateViewRecoversFromNotAuthorized(@Mocked ViewBuilder viewBuilder,
        ...
        IcebergRESTCatalog catalog = newOAuth2CredentialCatalog();
        Assertions.assertTrue(catalog.createView("iceberg_catalog", definition, false));
        new Verifications() {
            {
                restCatalog.initialize(anyString, (Map<String, String>) any);
                times = 2;
    ...
    public void testCreateOrReplaceViewRecoversFromWrappedNotAuthorized(@Mocked ViewBuilder viewBuilder,
        ...
        IcebergRESTCatalog catalog = newOAuth2CredentialCatalog();
        Assertions.assertTrue(catalog.createView("iceberg_catalog", definition, true));
        new Verifications() {
            {
                restCatalog.initialize(anyString, (Map<String, String>) any);
                times = 2;
    ...
    public void testAlterViewRecoversFromNotAuthorized(@Mocked ViewBuilder viewBuilder,
        ...
        IcebergRESTCatalog catalog = newOAuth2CredentialCatalog();
        Assertions.assertTrue(catalog.alterView(currentView, definition));
        new Verifications() {
            {
                restCatalog.initialize(anyString, (Map<String, String>) any);
                times = 2;
