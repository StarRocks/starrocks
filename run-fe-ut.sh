...
if [ ${COVERAGE} -eq 1 ]; then
    ...
    if [ $DRY_RUN -eq 0 ]; then
        ...
        ${MVN_CMD} test -DargLine="-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=$STARROCKS_HOME/log/fe.$DATE.hprof -XX:ErrorFile=$STARROCKS_HOME/log/java/hs_err_pid%p.log" -DfailIfNoTests=false -DtrimStackTrace=false -D test="$TEST_NAME" -Dsurefire.printSummary=true -T $PARALLEL
    fi
fi
