...
- Follower
  - 只有元数据读取权限，无写入权限。通过异步同步并且回放 Leader 的元数据日志来同步数据。
  - 参与 Leader 选主，必须有半数以上的 Follower 节点存活才能进行选主。

- Observer
  - 主要用于扩展集群的查询并发能力，可选部署。
  - 不参与选主，不会增加集群选主压力。
  - 通过异步同步并且回放 leader 的元数据日志来同步数据。
...
由于一张表被切分成了多个 Tablet，在数据导入时，可以并发写入到指定的 Tablet；在数据查询时，可以根据查询条件自动路由到相应的 Tablet。充分利用多机、多核提供的计算能力。同时由于数据可以直达指定的 Tablet，实现最大程度的本地计算，减少网络开销和传输时延。您也可以利用 StarRocks 的数据切分方式，将高并发请求压力分摊到多个物理节点，通过增加物理节点的方式来扩展系统的高并发能力。
