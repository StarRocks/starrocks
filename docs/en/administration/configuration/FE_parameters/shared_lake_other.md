...
- Unit: -
- Is mutable: Yes
- Description: Whether the bookmark metadata functions (`bookmark_create` / `bookmark_renew` / `bookmark_release`) are enabled. These are debug/test-only, OPERATE-privileged, leader-only functions and are gated off by default. The `holder` argument is a caller-supplied label, not an authenticated identity: anyone able to call these functions can release, or re-lease at a shorter TTL, a reference another holder created, and `HOLDER_ID` is readable from `information_schema.table_bookmark_references`.
- Introduced in:
