Introduction
In the actual deployment and operation of StarRocks, understanding and properly using the logging system is critical for troubleshooting, performance analysis, and system tuning. This article provides a detailed overview of the log file types, typical content, configuration methods, as well as log rolling and retention strategies for both the Frontend (FE) and Backend (BE or CN) components of StarRocks.
The information of this document is based on StarRocks Version 3.5.x.
...
- sys_log_roll_mode: Log rotation mode. Default is SIZE-MB-1024, meaning a new log file will be created when the current one reaches 1024MB. Together with sys_log_roll_interval, this indicates that FE logs can be rotated either daily or based on file size
...
You can check fe.out in the following scenarios:
...
- sys_log_level: Log level, default is INFO
- sys_log_dir: Log storage directory, default is ${STARROCKS_HOME}/log
- sys_log_roll_mode: Log rotation mode, default is SIZE-MB-1024, meaning a new log file is created when the current one reaches 1024MB
