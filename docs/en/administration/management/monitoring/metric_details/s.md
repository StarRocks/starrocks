...
- Unit: ms
- Type: Summary
- Labels: `execution_mode` (`legacy_fse` or `fse`), `is_leader`
- Description: Time in milliseconds to apply an ALTER TABLE column change, per statement. The `execution_mode` label indicates how the change was applied: `fse` is the current Fast Schema Evolution (FSE), applied immediately while the statement runs; `legacy_fse` is the legacy FSE path, which runs in the background and is usually far slower. `legacy_fse` occurs only in shared-data clusters with `cloud_native_fast_schema_evolution_v2` disabled (enabled by default, which uses `fse`). Reported only by the Leader FE (`is_leader="true"`). Includes the 0.75/0.95/0.98/0.99/0.999 quantiles plus `_sum` and `_count`.

## `starrocks_fe_alter_column_operation_total`

- Unit: Count
- Type: Cumulative
- Labels: `type` (`add`, `drop`, or `modify`), `is_leader`
