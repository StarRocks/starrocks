...
- Unit: Rows
- Is mutable: Yes
- Description: Software prefetch distance (in rows) used by both the aggregation probe and the hash-join probe: the probe fetches data this many rows ahead to hide memory latency on large hash tables. Setting it to `0` disables software prefetch. The default 16 works for most workloads; raise it for large hash tables that spill to DRAM, where a longer lookahead hides more latency. Lowering it has no real effect, since small cache-resident tables are already excluded by the L2 ratio. Each path additionally gates prefetch on L2 residency via its own ratio (`agg_prefetch_l2_ratio`, `join_probe_prefetch_l2_ratio`).
- Introduced in: -
