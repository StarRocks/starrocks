...
- Description: When true, most external BE HTTP endpoints require HTTP Basic Auth. Credentials are verified by RPC to the FE leader using the `checkAuth` Thrift method, so the user/password store on the FE side (including LDAP / security-integration) is the source of truth. The public probe / observability endpoints `/api/health`, `/metrics`, and `/metrics/memory` are authentication-only (AuthN-only): when `enable_http_auth=true` they require valid Basic credentials but do **not** require any RBAC privilege, so any authenticated user may call them; when `enable_http_auth=false` they remain anonymous. The following stay fully exempt regardless of this flag:
