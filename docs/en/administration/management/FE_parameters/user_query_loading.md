...
- Unit: -
- Is mutable: Yes
- Description: Maximum number of new tablets one source tablet may be split into when the split drags a full UNSHARE rewrite behind it, that is, on a range-distributed primary-key table whose `ORDER BY` key differs from the primary key. Such a split cannot range-filter the parent's shared segments, so every child is rewritten in full; a wide fan-out multiplies that read amplification. The value is further clamped by `tablet_reshard_max_split_count`. A value of 1 or less disables this extra clamp.
- Introduced in: -
...
- Unit: -
- Is mutable: Yes
- Description: Maximum number of source tablets one split job may take when the split drags a full UNSHARE rewrite behind it. This bounds how much data a single UNSHARE compaction rewrites, and therefore how long it holds the partition's only compaction slot away from size-tiered compaction. The largest tablets are selected first. A value of 0 or less means the compute node count of the warehouse.
- Introduced in: -
...
- Unit: Seconds
- Is mutable: Yes
- Description: Quiet period after the previous tablet reshard job on a table finishes before an automatic split may be triggered again, for tables whose split drags a full UNSHARE rewrite behind it. This gives size-tiered compaction a window to drain the small files that accumulated while the partition's compaction slot was held. A value of 0 or less disables the wait.
- Introduced in: -
