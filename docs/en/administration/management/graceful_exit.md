...
- When `graceful_exit_wait_for_frontend_heartbeat` is `true`, it keeps accepting new requests until whichever comes first: the admission delay after it has sent the shutdown heartbeat (a heartbeat response marked SHUTDOWN), or the fallback deadline measured from the start of the graceful shutdown.
...
- Description: If true, the BE waits until it has sent the shutdown heartbeat (a heartbeat response marked SHUTDOWN, which the FE is expected to observe) and then keeps accepting new requests for a `graceful_exit_reject_delay_ms` window before rejecting them. If false, the BE starts rejecting new requests as soon as graceful shutdown begins, without waiting for the shutdown heartbeat. Already-begun transactions continue to be accepted during the drain window regardless of this setting.
- Default: true
...
- Description: Delay (in ms) after the BE/CN has sent the shutdown heartbeat (a heartbeat response marked SHUTDOWN, which the FE is expected to observe) before the node starts rejecting new requests. During this window the node keeps accepting and running new requests as a healthy node, giving the FE time to stop scheduling new fragments to it.
- Default: 10000
- How to apply: Modify it in the BE/CN configuration file or update it dynamically.
...
- Description: Absolute upper bound (in ms) from the start of a graceful exit before the BE/CN rejects new requests, even if the FE never observes the shutdown heartbeat. Guards against unbounded acceptance when the FE never or very late observes the shutdown.
- Default: 15000
- How to apply: Modify it in the BE/CN configuration file or update it dynamically.
