...
2. View the specific compaction status using `show proc '/compactions'`:
   ...
   * If `read_remote_sec` is too large (taking more than 40% of the total compaction time), check the disk size on the machine and also check the cache quota through `SHOW BACKENDS` DataCacheMetrics.
   * If `write_remote_sec` is too large (taking more than 80% of the total compaction time), S3 write may be too slow. This can be verified through shared-data grafana metrics with keywords `single upload latency` and `multi upload latency`.
