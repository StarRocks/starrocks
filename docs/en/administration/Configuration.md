...
|  expr_children_limit                     | -    | 10000        | The maximum number of child expressions allowed in an expression. |
...
| create_table_max_serial_replicas         | -    | 128          | The maximum number of replicas to create serially. If the actual replica count exceeds this, replicas will be created concurrently. Try to reduce this config if table creation takes a long time to complete. |
...
|statistic_collect_concurrency             | -    |  3  | The maximum number of manual collection tasks that can run in parallel. The value defaults to 3, which means you can run a maximum of three manual collection tasks in parallel. <br/>If the value is exceeded, incoming tasks will be in the PENDING state, waiting to be scheduled.|
...
| disable_load_job                        | -    | FALSE                                           | Whether to disable loading when the cluster encounters an error. This prevents any loss caused by cluster errors. The default value is `FALSE`, indicating that loading is not disabled. `TRUE` indicates loading is disabled and the cluster is in read-only state.|
...
| http_worker_threads_num              | 0                 | Number of worker threads for http server to deal with http requests. For a negative or 0 value, the number of threads will be twice the number of cpu cores. Introduced in: 2.5.18，3.0.10，3.1.7，3.2.2.                                                                                 |
...
|pindex_major_compaction_limit_per_disk|1|N/A | The maximum concurrency of compaction on a disk. This addresses the issue of uneven I/O across disks due to compaction. This issue can cause excessively high I/O for certain disks. Introduced in: 3.0.9|
