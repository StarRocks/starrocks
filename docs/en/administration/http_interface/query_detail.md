...
The **query detail** API returns recent query execution details that are cached in FE memory. Query detail records are
collected only when the FE configuration `enable_collect_query_detail_info` is enabled.
...
| `is_request_all_frontend` | No (v2 only) | `false` | When `true`, the FE queries other alive frontends and merges their results. |
