...
| max_replication_data_size_per_job_in_gb   | The data size threshold at which the migration tool triggers data synchronization tasks. Unit: GB. Multiple data synchronization tasks will be triggered if the size of the data to be migrated exceed this value. The default value is `-1`, meaning no limit is imposed. You can set this parameter to restrict the data size of each task if the table to be migrated has a large data volume. |
