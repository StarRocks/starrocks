...
Each time FE creates a new metadata image after completing a metadata checkpoint, it automatically creates a snapshot. The name of the snapshot is generated by the system, following the format `automated_snapshot_{timestamp}`.
...
1. **(Optional)** If the storage location (storage volume) that stores the cluster snapshot have changed, all files under the original storage path must be copied to the new path. To achieve this, you must place the configuration file **cluster_snapshot.yaml** under the directory **fe/conf** of all FE nodes. For the template of **cluster_snapshot.yaml**, see [Appendix](#appendix).
...
3. Start other FE nodes.

   ```Bash
   ./fe/bin/start_fe.sh --helper <leader_ip>:<leader_edit_log_port> --daemon
   ```

4. Start CN nodes **after cleaning the local cache**.

   ```Bash
   ./be/bin/start_cn.sh --daemon
   ```

If you have added **cluster_snapshot.yaml** in the step 1, the node and storage volumes will be re-configured in the new cluster according to the information in the file. The information of the original cluster will be automatically deleted or replaced.
...
cluster_snapshot:
    ...
    # Example: s3://defaultbucket/test/f7265e80-631c-44d3-a8ac-cf7cdc7adec811019/meta/image/automated_cluster_snapshot_1704038400000
...
# If this section is not specified, the node information of the original cluster will be retained after recovery.
