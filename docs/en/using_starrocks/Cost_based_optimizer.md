...
- For tables with large data volume, the following restrictions apply:
  ...
  - Only statistics of partitions whose update time is later than the time of the precious collection task are collected. Statistics of partitions with no data change are not collected.
...
| statistic_auto_collect_sample_threshold  | DOUBLE	| 0.3   | The statistics health threshold for triggering automatic sampled collection. If the health threshold is lower than this threshold, automatic sampled collection is triggered. |
...
| statistic_collect_too_many_version_sleep | LONG | 600000 | The sleep time of automatic collection tasks if the table on which the collection task runs has too many data versions. Unit: seconds. Default value: 600000 (10 minutes).  |
