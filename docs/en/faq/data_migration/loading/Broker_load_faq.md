...
The amount of time taken to write the data exceeds the upper limit, causing a timeout error. To resolve this issue, modify the settings of the [session variable](../../../sql-reference/System_variable.md) `query_timeout` and the [BE configuration item](../../../administration/configuration/BE_parameters/BE_parameters.md#configure-be-static-parameters) `streaming_load_rpc_max_alive_time_sec` based on your business requirements.
