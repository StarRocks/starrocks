...
2. Add the new Follower FE node to the cluster by executing the following SQL.
   ...
   > - If you want to add Observer FE nodes, execute `ALTER SYSTEM ADD OBSERVER "<fe_address>:<edit_log_port>"=`. For detailed instructions, see [ALTER SYSTEM - FE](../sql-reference/sql-statements/Administration/cluster-management/nodes_processes/ALTER_SYSTEM.md).
