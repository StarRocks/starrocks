...
- `default`: the default value returned if no matching row is found. If this parameter is not specified, NULL is the default. `default` supports any expression whose type is compatible with `expr`, since version 3.6, default no longer has to be a constant—it can be a column name.
