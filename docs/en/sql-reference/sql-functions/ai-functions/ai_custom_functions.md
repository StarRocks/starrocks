...
The existing per-type default-provider mechanism is unchanged. Setting a default provider does not reroute SYSTEM `ai_complete`, [text helpers](ai_text_functions.md), or [ai_embed](ai_embed.md). Their existing overloads, FE configuration, and BE-local credentials remain unchanged; an explicit SYSTEM `model` argument still means a remote model name.
