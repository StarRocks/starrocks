...
The provider and model must support any supplied option, including `dimensions`. To select a registered AI provider instead of SYSTEM configuration, use [ai_custom_embedding](ai_custom_functions.md). Setting a default AI provider does not change `ai_embed` routing; its explicit `model` argument still selects a remote model name.
