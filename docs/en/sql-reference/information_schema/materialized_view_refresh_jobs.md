...
| RUN_AS_USER                        | User identity the refresh runs as. With creator-based authorization (the default, `mv_use_creator_based_authorization=true`) this is the materialized view's creator; with root-based authorization it is `'root'@'%'`. The same setting also governs the identity used to build the materialized view's plan cache for query rewrite. See [`mv_use_creator_based_authorization`](../../administration/management/FE_parameters/shared_lake_other.md). |
