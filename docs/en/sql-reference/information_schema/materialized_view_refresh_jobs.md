...
| REFRESH_MODE_REASON_TABLE | The base table that drove the mode decision, as `catalog.database.table`. Empty when no single base table drove it: a reason the backend reports while reading changes identifies a tablet rather than a table. |
