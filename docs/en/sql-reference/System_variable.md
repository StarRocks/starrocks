...
* **Scope**: Session
* **Description**: Controls which node types the fragment scheduler may choose to run computation fragments on. Valid values (case-insensitive) are:
  * COMPUTE_NODES_ONLY — schedule fragments only on Compute Nodes (default)
  * ALL_NODES — allow scheduling on both Compute Nodes and Backend nodes
  The session variable is stored as a String but interpreted against the enum SessionVariableConstants.ComputationFragmentSchedulingPolicy. The setter upper-cases the value and validates it; an invalid value causes IllegalArgumentException listing legal values. The getter returns the corresponding enum (or COMPUTE_NODES_ONLY when unset/invalid). This variable is consulted by scheduling and planning components (e.g., WorkerProvider, CandidateWorkerProvider, HDFSBackendSelector, CoordinatorPreprocessor) and interacts with related flags such as prefer_compute_node to influence worker selection, locality, and resource placement for query fragments.
* **Default**: COMPUTE_NODES_ONLY.name()
* **Data Type**: String
* **Introduced in**: 3.2.7
...
* **Description**: When set to true for the current session, the analyzer will pre-plan and retain the materialized-view query during CREATE MATERIALIZED VIEW for views using an incremental refresh scheme. MaterializedViewAnalyzer.planMVQuery checks this session flag (isEnableIncrementalRefreshMV()) and, if the view uses an IncrementalRefreshSchemeDesc, proceeds to build logical and optimized physical plans (it sets the MV planner, transforms the relation, and invokes the optimizer) and keeps the plan in memory to support incremental-refresh planning/optimization. This is a session-scoped planning toggle only; it controls planner behavior and does not by itself change refresh execution semantics or persist beyond the session. (Accessors: isEnableIncrementalRefreshMV / setEnableIncrementalRefreshMv.)
* **Default**: false
* **Data type**: boolean
* **Scope**: Session
* **Introduced in**: 3.2.0
...
* **Description**: Controls the Large IN predicate optimization. When true, the parser (AstBuilder) will represent very large IN lists as raw constant payloads (instead of many expression nodes) once the element count reaches large_in_predicate_threshold. The optimizer rule LargeInPredicateToJoinRule will then convert such LargeInPredicate instances into a RawValues table plus a LEFT_SEMI_JOIN or NULL_AWARE_LEFT_ANTI_JOIN (for NOT IN), reducing FE memory usage and planning time for extremely large constant lists (e.g., 100k+). The rule performs checks (no OR compound predicates, only one LargeInPredicate per query); if transformation is unsupported or fails, a LargeInPredicateException is thrown and upper layers (ConnectProcessor / StmtExecutor) retry the query from the parser stage with enable_large_in_predicate disabled to preserve correctness. Use large_in_predicate_threshold to tune the switch point.
* **Scope**: Session
* **Default**: true
* **Data Type**: boolean
* **Introduced in**: -
...
* **Description**: Session-scoped boolean that enables the optimizer to generate/allow one‑phase local aggregation using a local shuffle operator (pattern: Scan -> LocalShuffle -> OnePhaseAgg) and to prune a Shuffle Exchange into a local shuffle when safe. When enabled, the cost model may prefer a one‑phase local aggregation on single‑node deployments (single backend + compute node) and the planner (PruneShuffleDistributionNodeRule, EnforceAndCostTask, CostModel) can replace or favor local‑shuffle plans. Effectively reduces network/redistribution overhead for eligible OLAP scan → global aggregation patterns. Constraints and notes: it only applies when the pipeline engine is enabled and the cluster is a single backend+compute node; it is not applied to operators with multiple inputs (e.g., joins) or to non‑query flows that require "assign scan ranges per driver sequence" (some planners temporarily disable this variable for DELETE/UPDATE/INSERT and during certain MV optimizations). The variable is consulted by the optimizer and cost model to influence plan generation and costing.
* **Default**: true
* **Data type**: Boolean
...
* **Scope**: Session
* **Description**: When enabled, allows the planner/fragment finalization phase to mark eligible pipeline fragments for runtime adaptive DOP (degree-of-parallelism). It is effective only when the pipeline engine is enabled (isEnableRuntimeAdaptiveDop() returns true only if enable_pipeline_engine is true and this flag is on). During PlanFragmentBuilder.finalizeFragments(), FE checks each fragment: if fragment.canUseRuntimeAdaptiveDop() is true the fragment.enableAdaptiveDop() is invoked so the fragment can adjust its pipeline parallelism at runtime. Enabling runtime adaptive DOP disables group execution and also turns off pipeline-level multi-partitioned runtime filters (these features are incompatible). This session flag is controlled via setEnableRuntimeAdaptiveDop(...) and interacts with the hidden tuning parameters runtime_adaptive_dop_max_block_rows_per_driver_seq and runtime_adaptive_dop_max_output_amplification_factor, which bound when and how much adaptive DOP can change workload characteristics.
* **Default**: false
* **Data Type**: boolean
* **Introduced in**: 3.2.0
...
* **Description**: Integer threshold (count of constants) used by the parser to decide whether to convert a regular IN predicate into a LargeInPredicate optimization. When enable_large_in_predicate is true and the number of literal elements in an IN list (string or integer) is >= this threshold, the parser builds a LargeInPredicate that carries raw constant values (avoiding construction of many Expr nodes). LargeInPredicate is intended to reduce FE parse/analyze/planner/deploy overhead and is later rewritten (when possible) to a left semi/anti join by the optimizer. If the optimization is unsupported or fails during planning/execution, the query may be retried with enable_large_in_predicate disabled. (Used in AstBuilder.visitInStringList / visitInIntegerList, optimizer rewrite and retry logic.)
* **Scope**: Session
* **Default**: 100000
* **Data Type**: int
* **Introduced in**: -
...
* **Scope**: Session
* **Description**: Controls which bucket-sequence traversal algorithm ColocatedBackendSelector uses to assign bucket sequences to backends. When the value is <= 0, ColocatedBackendSelector always uses the NormalBucketSequenceIterator. Otherwise FE computes:
  - numTotalBuckets (sum of replica counts for the first replica of each bucket),
  - backends.size() (number of distinct backends),
  - hasMultiReplications (whether any bucket has >1 replica).
  If hasMultiReplications && numTotalBuckets <= maxBucketsPerBeToUseBalancerAssignment * backends.size(), ColocatedBackendSelector switches to BalancerBucketSequenceIterator which produces a more even assignment across backends. BalancerIterator.reduce skew at the cost of higher CPU: its useBackend() is O(numBucketsPerBe), so this algorithm is chosen only when per-backend bucket count is small enough. See ColocatedBackendSelector.createBucketIterator and BalancerBucketSequenceIterator for details.
* **Default**: 6
* **Data type**: int
* **Introduced in**: 3.2.0
...
* **Description**: Session-scoped (invisible) parameter that caps the maximum number of rows per driver-sequence block used by the runtime adaptive DOP (degree-of-parallelism) mechanism. When the pipeline engine and runtime adaptive DOP are enabled and a fragment opts into runtime adaptive DOP, the FE sets TAdaptiveDopParam.max_block_rows_per_driver_seq from this session variable and sends it to the BE. The BE uses this cap as an upper bound in its adaptive-DOP estimation to limit per-driver-sequence work units and avoid overly large blocks from skewing parallelism decisions. Adjust this per-session to tune how aggressive adaptive DOP will scale parallelism for large driver sequences.
* **Default**: 4096L * 4 (16384)
* **Data Type**: long
* **Introduced in**: 3.2.0

### runtime_adaptive_dop_max_output_amplification_factor

* **Description**: Session-level limit on the output amplification factor used by the runtime adaptive DOP (degree-of-parallelism) mechanism. When a query fragment uses runtime adaptive DOP (fragment.isUseRuntimeAdaptiveDop()) and the pipeline engine is enabled, the FE reads this session variable and populates TAdaptiveDopParam.max_output_amplification_factor that is sent to executors. The setting is effective only when set to a positive value; a value of 0 disables this limit. Use this to bound how much the runtime adaptive DOP logic is allowed to increase output production relative to expectations to control resource amplification during adaptive parallelism decisions.
* **Scope**: Session
* **Default**: 0
* **Data Type**: long
* **Introduced in**: 3.2.0
