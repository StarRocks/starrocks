...
* **Description**: Whether a constant array subscript pushed down to a PostgreSQL JDBC catalog is corrected for each value's own array lower bound. A PostgreSQL array carries its lower bound per value, and reading one does not retain it: the driver hands over a plain array and StarRocks rebases it to position 1. When this variable is `false` (Default), the subscript is pushed down as the plain `a[k]`, which assumes a lower bound of 1. That holds for every array PostgreSQL builds for itself, but on a value stored with another lower bound the pushed-down subscript returns a different element than the same subscript evaluated in StarRocks, with no error and no warning: on a value stored as `[0:2]={zero,one,two}`, `items[1]` returns `one` remotely where StarRocks returns `zero`, and on `[5:7]={a,b,c}` it returns NULL where StarRocks returns `a`. Set this variable to `true` if your PostgreSQL tables hold arrays with a lower bound other than 1 and you need the push-down to answer exactly what StarRocks answers; the subscript is then evaluated against `array_lower()` per row and guarded on `array_length()`. This variable governs only the SQL that is emitted. Which expressions are pushed down at all is the same either way, so turning it on or off never changes whether a query reads the whole array column.
* **Default**: false
* **Data type**: Boolean

### enable_jdbc_array_subscript_push_down

* **Description**: Whether a constant array subscript over a PostgreSQL JDBC catalog column -- `items[1]`, or the equivalent `element_at(items, 1)` -- is evaluated by PostgreSQL instead of the whole array column being read back and indexed in StarRocks. Valid values: `true` (Default) and `false`. Set this variable to `false` to roll the push-down back: the subscript is then not pushed down on either path -- a subscript in a `WHERE` clause keeps its predicate in StarRocks, and a subscript in the `SELECT` list is not folded into the remote query -- and the whole array column is read back and indexed locally, as it was before the push-down existed. Two consequences of turning it off are worth stating. First, [`enable_jdbc_array_lower_bound_correction`](#enable_jdbc_array_lower_bound_correction) then has no effect at all, because it chooses between two renderings of a pushed-down subscript and no subscript is pushed down; there are therefore two ways to get exactly what local evaluation answers -- set this variable to `false`, or leave it `true` and set `enable_jdbc_array_lower_bound_correction` to `true`, which keeps the push-down. Second, a column that holds a multidimensional value is read directly again, so a query over it fails with the unsupported-array error instead of receiving NULL from PostgreSQL.
### enable_jdbc_column_statistics

* **Description**: Whether a JDBC catalog may feed the optimizer per-column statistics read from the source database's own catalog, such as the number of distinct values, the fraction of NULLs, and the average column width. Valid values: `true` (Default) and `false`. Only columns the source actually describes are affected. A column the source left out is left unknown; a dialect that describes no columns at all -- MySQL and ClickHouse report a row count and nothing else -- keeps the type-ratio estimate it has always been given. Setting it to `false` puts every column back on that estimate, which is where JDBC catalogs stood before this feature. The table row count is not affected by this variable.
* **Default**: true
* **Data type**: Boolean

### enable_jdbc_runtime_filter_push_down

* **Description**: Whether to push the runtime filter built by a join into the remote SQL of a JDBC catalog scan, as an `IN` list of the exact values the build side produced, rendered into the statement as SQL literals rather than bound as parameters, so that the remote database returns only the rows that can match. Valid values: `true` and `false`. Only a Local RF reaches the scan, so the join must be a Broadcast Hash Join with the JDBC table on the probe side; a Shuffle Join builds a Global RF, which is not pushed down. No filter is built at all when the build side produces more rows than `max_pushdown_conditions_per_column` (`1024` in `be.conf`). Supported join key types are `TINYINT`, `SMALLINT`, `INT`, and `BIGINT` on every dialect, together with string keys whose remote source type is known to be a text type -- on PostgreSQL a VARCHAR key needs no such proof. On PostgreSQL only, `FLOAT` and `DATE` keys are supported as well, and `DOUBLE` and `DATETIME` keys are supported when the remote source type is known to be `double precision` (or `float8`) and `timestamp without time zone` respectively. A `timestamp with time zone` column is never pushed down: StarRocks reads it as a wall clock in the query's time zone, and the two distinct instants of a repeated daylight-saving hour become the same wall clock, so the remote `IN` would return fewer rows rather than the same ones. For the same reason `DOUBLE` and `DATETIME` keys keep their filters local on the derived table an aggregation, projection or join push down produces, where the remote source types are no longer known; `FLOAT` and `DATE` still push down there. Any other key keeps its filter local, including `TIME`, `CHAR`, `DECIMAL`, `JSON`, and a column StarRocks mapped to DECIMAL(38,18) from a PostgreSQL `numeric` that was declared without a precision. A string key whose values include a character outside the Basic Multilingual Plane keeps its filter local as well: the handoff that carries the finished statement to the JDBC bridge is specified for modified UTF-8, in which such a character is a six-byte surrogate pair, so its four-byte UTF-8 form would arrive cut short -- changing which rows match, and able to end the statement in the middle of a literal. This covers emoji and also CJK Extension B, which holds rare characters used in personal names. A string key whose values include a backslash keeps its filter local too: a value's own single quote is escaped by doubling it, which is correct only while the remote reads a backslash as an ordinary character, and `standard_conforming_strings` on PostgreSQL and `NO_BACKSLASH_ESCAPES` on MySQL are session settings the BE cannot see. A scan that carries a row limit is never filtered remotely: the remote statement applies its WHERE before the limit, while the local plan applies the filter after it, so pushing it down would return different rows rather than fewer. That restriction is a correctness requirement, not a tuning choice. To check what a query actually did, read the `RUNTIME FILTER PUSH DOWN` line in `EXPLAIN VERBOSE` output, which reports whether the FE authorized the scan, together with the `PushdownRuntimeFilters` and `PushdownRuntimeFilterSkipped` counters in the query profile, which report what the BE rendered into the remote statement and why any filter was dropped.
* **Default**: false
* **Data type**: Boolean

### enable_jdbc_topn_push_down

* **Description**: Whether to push eligible `ORDER BY ... LIMIT` operations to PostgreSQL JDBC catalogs. Legacy external JDBC tables created with a `RESOURCE` keep TopN in StarRocks. Valid values: `true` and `false`. Supports `TINYINT`, `SMALLINT`, `INT`, and `BIGINT` sort keys, including matching results of pushed aggregations. The following PostgreSQL column types are also supported when their source type is known: `date` and `timestamp without time zone`; `boolean`; `text` and `varchar`; and `numeric`/`decimal` declared with a precision of 38 or less. Supported dates and timestamps range from AD 0001 through 9999. Temporal results of pushed expressions or aggregations are not supported. `text` and `varchar` keys are sorted remotely under `COLLATE "C"` to match the byte order StarRocks applies, which can prevent PostgreSQL from using an index built under another collation. The remote ORDER BY replaces the local TopN instead of feeding it, and the scan preserves the order it received its rows in. Queries with OFFSET, an existing scan limit, or filters or aggregations that must execute locally are not pushed down. Floating-point, `char`/`bpchar`, `enum`, `timestamp with time zone`, and `numeric` sort keys wider than a precision of 38 are not supported. Upgrade the FE, the BE/CN nodes, and their JDBC bridges together. A BE/CN that predates this feature reorders the rows it reads, and no local TopN remains to restore the order; date/time ordering additionally requires the bridge's lossless PostgreSQL temporal reader.
* **Default**: true
* **Data type**: Boolean
...
* A partition that automatic partitioning creates during the load itself is spread too. It is not in the plan, so its tablets are handed to the load afterwards, over the create-partition RPC: they are spread over the same number of nodes the load resolved when it was planned, picked from the compute nodes alive at the moment the partition is created -- a long load can outlive the node list it was planned against. Such a partition normally starts with a single tablet, because the data boundaries that would have split it have not been seen yet, so it is exactly the shape this setting exists for.
* **Default**: auto
* **Type**: String
* **Scope**: Session
* **Introduced in**: v4.2
