...
* **Description**: Controls the adaptive fallback for bucket-aware execution on data lake tables (such as Iceberg tables). When a shuffle-requiring aggregation reads a bucket-partitioned table and the estimated number of buckets that survive partition and predicate pruning is smaller than this value multiplied by the number of alive workers, the optimizer stops using bucket-aware one-stage aggregation for that scan and falls back to the ordinary shuffle (multi-stage) aggregation plan to restore parallelism. This prevents severe skew when, for example, an equality filter on the bucket source column prunes the query down to a single bucket while the query groups by a high-cardinality column. Setting this variable to `0` disables the fallback. Larger values make the fallback more aggressive. This variable takes effect only when `enable_bucket_aware_execution_on_lake` is `true`.
* **Default**: 1.0
* **Data type**: Double
* **Introduced in**: v4.2
