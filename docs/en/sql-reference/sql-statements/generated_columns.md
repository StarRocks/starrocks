...
- The expressions for generated column cannot reference other generated columns or [auto-increment columns](table_bucket_part_index/auto_increment.md), but the expressions can reference multiple regular columns.
...
When modifying a generated column, StarRocks needs to rewrite all the data, which is time-consuming and resource-intensive. If it is unavoidable to use ALTER TABLE to modify generated columns, it is recommended to evaluate the cost and time involved in advance.
