...
| partial_update_mode | No | Specifies the mode for partial updates. Valid values: `row`, `column`, `auto`, `flexible`, and `flexible_row`. <ul><li> The value `row` (default) means partial updates in row mode, which is more suitable for real-time updates with many columns and small batches.</li><li>The value `column` means partial updates in column mode, which is more suitable for batch updates with few columns and many rows. In such scenarios, enabling the column mode offers faster update speeds. For example, in a table with 100 columns, if only 10 columns (10% of the total) are updated for all rows, the update speed of the column mode is 10 times faster.</li><li>The value `auto` (experimental) means StarRocks automatically chooses row mode or column mode for each load based on the update width and the read/write sensitivity of the workload. For JSON-formatted partial updates on a shared-data Primary Key table, `auto` also enables per-row heterogeneous column sets (see `flexible` below); where per-row column sets cannot apply (a shared-nothing table, a table with an AUTO_INCREMENT column, a `merge_condition`, or a `columns` mapping that contains expressions), `auto` keeps its previous behavior and applies one homogeneous partial update.</li><li>The value `flexible` (experimental; shared-data/cloud-native Primary Key tables only) allows each row in a JSON-formatted partial update to update a different subset of columns (heterogeneous partial update). Columns that a row omits keep their existing values. This mode is applied through the column mode (sparse) write path.</li><li>The value `flexible_row` (experimental; shared-data only) provides the same per-row heterogeneous update semantics as `flexible`, but is applied through row mode (the whole row is rewritten).</li></ul>**NOTE**<br/>`auto`, `flexible`, and `flexible_row` are experimental features.<br/>`flexible` and `flexible_row` are supported only on shared-data (cloud-native) Primary Key tables. They cannot be used together with `merge_condition` or with expressions in the `columns` parameter, and are not supported on a table that has an AUTO_INCREMENT column.<br/>If a partial update touches a column that has a GIN (inverted) index, StarRocks automatically applies the update in row mode to keep the index consistent.<br/>The sparse write path underlying `column`/`auto`/`flexible` is disabled by default and is controlled by the BE configuration item `enable_sparse_dcg`. |
