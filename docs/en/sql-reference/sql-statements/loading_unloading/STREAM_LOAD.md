...
STREAM LOAD allows you to load data from a local file system or a streaming data source. After you submit a load job, the system synchronously runs the job, and returns the result of the job after the job finishes. You can determine whether the job is successful based on the job result. For information about the application scenarios, limits, and supported data file formats of Stream Load, see [Loading from a local file system via Stream Load](../../../loading/StreamLoad.md).
...
:::note
- After you load data into a native table by using Stream Load, the data of the materialized views that are created on that table is also updated.
- You can load data into native tables only as a user who has the INSERT privilege on those tables. If you do not have the INSERT privilege, follow the instructions provided in [GRANT](../account-management/GRANT.md) to grant the INSERT privilege to the user that you use to connect to your cluster.
:::
...
Specify the username and password of the account that you use to connect to your cluster. This is a required parameter. If you use an account for which no password is set, you need to input only `<username>:`.
...
Specifies the URL of the table. Syntax:

<TableURL />

### data_desc

Describes the data file that you want to load. The `data_desc` descriptor can include the data file's name, format, column separator, row separator, destination partitions, and column mapping against the table. Syntax:
...
| partitions | No       | The partitions into which you want to load the data file. By default, if you do not specify this parameter, the system loads the data file into all partitions of the table. |
...
| columns    | No       | The column mapping between the data file and the table.<br/>If the fields in the data file can be mapped in sequence onto the columns in the table, you do not need to specify this parameter. Instead, you can use this parameter to implement data conversions. For example, if you load a CSV data file and the file consists of two columns that can be mapped in sequence onto the two columns, `id` and `city`, of the table, you can specify `"columns: city,tmp_id, id = tmp_id * 100"`. For more information, see the "[Column mapping](#column-mapping)" section in this topic. |
...
| Parameter        | Required | Description                                                  |
| ---------------- | -------- | ------------------------------------------------------------ |
| label            | No       | The label of the load job. If you do not specify this parameter, the system automatically generates a label for the load job.<br/>The system does not allow you to use one label to load a data batch multiple times. As such, the system prevents the same data from being repeatedly loaded. For label naming conventions, see [System limits](../../System_limit.md).<br/>By default, the system retains the labels of load jobs that were successfully completed over the most recent three days. You can use the [FE parameter](../../../administration/configuration/FE_parameters/FE_parameters.md) `label_keep_max_second` to change the label retention period. |
| where            | No       | The conditions based on which the system filters the pre-processed data. The system loads only the pre-processed data that meets the filter conditions specified in the WHERE clause. |
...
| timeout          | No       | The timeout period of the load job. Valid values: `1` to `259200`. Unit: second. Default value: `600`.<br/>**NOTE**In addition to the `timeout` parameter, you can also use the [FE parameter](../../../administration/configuration/FE_parameters/FE_parameters.md) `stream_load_default_timeout_second` to centrally control the timeout period for all Stream Load jobs in your cluster. If you specify the `timeout` parameter, the timeout period specified by the `timeout` parameter prevails. If you do not specify the `timeout` parameter, the timeout period specified by the `stream_load_default_timeout_second` parameter prevails. |
| strict_mode      | No       | Specifies whether to enable the [strict mode](../../../loading/strict_mode.md). Valid values: `true` and `false`. Default value: `false`.  The value `true` specifies to enable the strict mode, and the value `false` specifies to disable the strict mode. |
...
| partial_update_mode | No | Specifies the mode for partial updates. Valid values: `row`, `column`, `auto`, `flexible`, and `flexible_row`. <ul><li> The value `row` (default) means partial updates in row mode, which is more suitable for real-time updates with many columns and small batches.</li><li>The value `column` means partial updates in column mode, which is more suitable for batch updates with few columns and many rows. In such scenarios, enabling the column mode offers faster update speeds. For example, in a table with 100 columns, if only 10 columns (10% of the total) are updated for all rows, the update speed of the column mode is 10 times faster.</li><li>The value `auto` (experimental) lets StarRocks choose row mode or column mode for each load based on the update width and the read/write sensitivity of the workload when the BE configuration item `enable_sparse_dcg` is enabled; with that item at its default (`false`), `auto` behaves exactly as it did before SDCG was introduced (row mode). Only when the configuration item `enable_sparse_dcg` is enabled on both the FE and the BE, `auto` also enables per-row heterogeneous column sets (see `flexible` below) for JSON-formatted partial updates on a shared-data Primary Key table; where per-row column sets cannot apply (the configuration item is disabled, a shared-nothing table, a table with an AUTO_INCREMENT column, a `merge_condition`, or a `columns` mapping that contains expressions), `auto` keeps its previous behavior and applies one homogeneous partial update.</li><li>The value `flexible` (experimental; shared-data/cloud-native Primary Key tables only; requires `enable_sparse_dcg` to be enabled on both the FE and the BE) allows each row in a JSON-formatted partial update to update a different subset of columns (heterogeneous partial update). Columns that a row omits keep their existing values. This mode is applied through the column mode (sparse) write path.</li><li>The value `flexible_row` (experimental; shared-data only; requires the same configuration items as `flexible`) provides the same per-row heterogeneous update semantics as `flexible`, but is applied through row mode (the whole row is rewritten).</li></ul>**NOTE**<br/>Values are case-sensitive.<br/>`auto`, `flexible`, and `flexible_row` are experimental features.<br/>`flexible` and `flexible_row` are accepted only when the FE configuration item `enable_sparse_dcg` and the BE configuration item of the same name are both set to `true`; otherwise the load is rejected. With these configuration items at their default (`false`), `row`, `column`, and `auto` behave exactly as they did before `flexible` was introduced.<br/>`flexible` and `flexible_row` are supported only on shared-data (cloud-native) Primary Key tables. They cannot be used together with `merge_condition` or with expressions in the `columns` parameter, and are not supported on a table that has an AUTO_INCREMENT column.<br/>When `enable_sparse_dcg` is enabled on both the FE and the BE and a partial update touches a column that has a GIN (inverted) index, StarRocks automatically applies the update in row mode to keep the index consistent.<br/>The sparse write path underlying `column`/`auto`/`flexible` is controlled by the BE configuration item `enable_sparse_dcg` and is disabled by default. |
| merge_condition  | No       | Specifies the name of the column you want to use as the condition to determine whether updates can take effect. The update from a source record to a destination record takes effect only when the source data record has a greater or equal value than the destination data record in the specified column. The system supports conditional updates since v2.5. <br/>**NOTE**<br/>The column that you specify cannot be a primary key column. Additionally, only tables that use the Primary Key table support conditional updates. |

## Column mapping

### Configure column mapping for CSV data loading

If the columns of the data file can be mapped one on one in sequence to the columns of the table, you do not need to configure the column mapping between the data file and the table.

If the columns of the data file cannot be mapped one on one in sequence to the columns of the table, you need to use the `columns` parameter to configure the column mapping between the data file and the table. This includes the following two use cases:
...
After the load job finishes, the system returns the job result in JSON format. Example:
...
If the load job fails, the system also returns `ErrorURL`. Example:
...
Your database `test_db` contains a table named `table1`. The table consists of three columns, which are `col1`, `col2`, and `col3` in sequence.
...
Your database `test_db` contains a table named `table2`. The table consists of three columns, which are `col1`, `col2`, and `col3` in sequence.
...
Your database `test_db` contains a table named `table3`. The table consists of three columns, which are `col1`, `col2`, and `col3` in sequence.
...
Your database `test_db` contains a table named `table4`. The table consists of three columns, which are `col1`, `col2`, and `col3` in sequence.
...
Your database `test_db` contains a table named `table5`. The table consists of three columns, which are `col1`, `col2`, and `col3` in sequence.
...
Your database `test_db` contains a table named `table6`. The table consists of three columns, which are `col1`, `col2`, and `col3` in sequence.
...
Your database `test_db` contains a table named `table7`. The table consists of two HLL-type columns, which are `col1` and `col2` in sequence.
...
Your database `test_db` contains a table named `table8`. The table consists of two BITMAP-type columns, which are `col1` and `col2`, in sequence.
...
Your database `test_db` contains a table named `table9`. The table consists of three columns, which are `col1`, `col2`, and `col3` in sequence.
...
Your database `test_db` contains a table named `table10`. The table consists of three columns, which are `col1`, `col2`, and `col3` in sequence.
...
Your database `test_db` contains a table named `tbl1`, whose schema is as follows:
...
The system performs the following steps to match and process JSON data:
