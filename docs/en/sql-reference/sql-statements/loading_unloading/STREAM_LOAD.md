...
| fill_default_on_absent_key | No | Specifies what a column takes when a row's JSON object has no key for it. Valid values: `true` and `false`. Default value: `false`.<br/>By default such a column is loaded as `NULL`, which overrides the column's `DEFAULT`. Set this parameter to `true` to load the column's `DEFAULT` instead. A key that is present with a value of `null` is a supplied value and is still loaded as `NULL`, whatever this parameter is set to.<br/>**NOTE**<br/>This parameter only applies when `format` is `json`. It cannot be used together with `partial_update`, which already keeps a column the load did not supply at its stored value.<br/>A constant `DEFAULT` is filled, including a complex one such as `ARRAY<INT> DEFAULT [1, 2]`, and so are the `current_timestamp()`, `now()`, `uuid()` and `uuid_numeric()` default functions. The following are not filled and keep the existing `NULL` behavior: a column with no `DEFAULT`; a column whose `DEFAULT` is any other expression; an `AUTO_INCREMENT` column; a column computed by a mapping expression in `columns`; a `JSON`, `ARRAY`, `MAP` or `STRUCT` column that this load reads as text, which happens when the column is referenced by a mapping expression in `columns`, when a generated column's expression reads it, or while a schema change on it is in progress, because those declared defaults have no faithful text form; a key column on a Primary Key, Unique Key or Aggregate table, because filling it would give every row missing that key the same key and the rows would merge into one another; and a `REPLACE_IF_NOT_NULL` column on an Aggregate table, because an absent key is how that column keeps its stored value. A Duplicate Key table's key is only a sort key, so it is filled like any other column.<br/>It cannot be used together with `jsonpaths`. Resolving a path cannot distinguish a key that is absent from one that is present with a `null` value, so nothing could be filled there, and the load is rejected rather than silently doing nothing.<br/>The parameter applies to every column the load supplies, which is the whole table schema when no `columns` header is given, minus the columns listed above.<br/>Both the FE and the BE must support this parameter. A version that does not will ignore it without reporting an error, so to confirm that it took effect, check for `"FillDefaultOnAbsentKey": true` in the load result. This confirmation is reported for ordinary and transaction Stream Load. It is not reported for Merge Commit loads, nor for a transaction Stream Load that uses channels, because those do not receive the plan result the confirmation is read from. Such loads still apply the parameter, they simply do not echo it. |
