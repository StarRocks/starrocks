...
| partial_update_mode | No | Specifies the mode for partial updates. Valid values: `row`, `column`, `flexible`, and `flexible_row`. <ul><li> The value `row` (default) means partial updates in row mode, which is more suitable for real-time updates with many columns and small batches.</li><li>The value `column` means partial updates in column mode, which is more suitable for batch updates with few columns and many rows. In such scenarios, enabling the column mode offers faster update speeds. For example, in a table with 100 columns, if only 10 columns (10% of the total) are updated for all rows, the update speed of the column mode is 10 times faster.</li><li>The value `flexible` (experimental) means a flexible partial update: each row of a JSON-formatted load updates only the columns present in that row, and the columns that the row omits keep their current values. A row that inserts a new primary key gets the default values of the columns it omits. A column present in a row with the value `null` is set to `NULL`. The load is applied in column mode. Set `partial_update` to `true` and list in `columns` every column that the rows may contain.</li><li>The value `flexible_row` (experimental) has the same semantics as `flexible`, but the load is applied in row mode, which rewrites the whole row.</li></ul>**NOTE**<br/>Values are case-sensitive.<br/>`flexible` and `flexible_row` are supported only for Stream Load into Primary Key tables in shared-data clusters, and only when the FE configuration item `enable_flexible_partial_update` and the BE configuration item of the same name are both set to `true`. Otherwise, the load fails. Enable them only after all BEs and CNs of the cluster run a version that supports flexible partial update: a node of an earlier version would apply such a load as a plain partial update and set the omitted columns to `NULL`.<br/>`flexible` and `flexible_row` cannot be combined with `merge_condition`, `jsonpaths`, or a `columns` mapping that contains expressions, and are not supported for range-distributed tables, tables whose sort key (ORDER BY) contains a non-primary-key column, tables with an AUTO_INCREMENT column or a generated column, tables with a column named `__cset__`, transaction stream load, or merge commit.<br/>When several rows of one load have the same primary key, each column of the key takes the value of the last of these rows that includes the column. A delete row (`__op` = 1) drops the rows of its key before it, and if it is the last row of its key, the key is deleted. |
