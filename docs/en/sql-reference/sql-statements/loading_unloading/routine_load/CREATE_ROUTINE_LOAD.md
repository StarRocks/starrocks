...
**Required**: No\
**Description**: Specifies whether the BE skips a Kafka message whose JSON is malformed, instead of failing the load task. Valid values: `true` and `false`. Default value: `false`. <br />Without it, such a message makes every task that reads it fail with `parse error` and the offset never advances: with `pause_on_fatal_parse_error` = `false` the job stays `RUNNING` while consumer lag grows without bound, with `true` it is paused. With it, the BE drops exactly that one message (uncompressed JSON only) and loads nothing from it, not even the fields that precede the point where a truncated document breaks (the one exception is a message larger than a whole chunk, thousands of rows, whose leading rows were already handed to the next stage before the defect was found; those cannot be recalled): it is counted as one filtered row, written to the error log with its topic, partition and offset, logged as a BE `WARNING`, and counted in the BE metric `starrocks_be_routine_load{type="skipped_malformed_messages"}`. The rows in the same batch load normally and the offset advances past the bad message. <br />Notes:<ul><li>Both shapes of malformed JSON are skipped and each counts as exactly one filtered row: a message that is not JSON at all (for example `abcd`, rejected before any row is read), and a message whose document is only found to be truncated while its rows are being read (for example `{"a": 1, "b": }`: the row that fails and the message it came from are counted once, not twice).</li><li>Only defects in the JSON that is actually read are detected: the document itself, the object selected by `json_root`, and each element when `strip_outer_array` is set. A structural defect elsewhere (for example in a sibling field next to the `json_root` object) is not seen by the parser, and even inside the read part the parser reports only what it encounters while reading (a missing value inside a nested `json_root` object, for example, can pass as a differently shaped object). Such a message is loaded exactly as it is without this option.</li><li>Skipped messages count toward the error thresholds, which react differently. `max_error_number` is a job-level total: once exceeded, the task that crossed it still commits and the job is paused. `max_filter_ratio` is checked per task: once exceeded, the task fails, nothing from it is committed, and it is retried from the same offset, so a job that opts in should keep `max_filter_ratio` high (for example `1.0`) and rely on `max_error_number` as the circuit breaker.</li><li>`pause_on_fatal_parse_error` may be set as well; the BE skips first, so it only acts if a parse error still reaches the FE (for example from a BE that predates this option).</li><li>Has no effect on Stream Load, Broker Load or compressed streams, which keep failing on malformed JSON.</li></ul>
