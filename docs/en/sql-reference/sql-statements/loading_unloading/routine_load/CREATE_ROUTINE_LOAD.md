...
| pause_on_fatal_parse_error          | NO | Specifies whether to automatically pause the job upon encountering unrecoverable data parsing errors. Valid values: `true` and `false`. Default value: `true`. This parameter is supported since v3.4.2. <br/>Such parsing errors are typically caused by illegal data formats, such as:<br /><ul><li>Importing a JSON array without setting strip_outer_array.</li><li>Importing JSON data, but the Kafka message contains illegal JSON, such as abcd.</li></ul>  **NOTE** In previous versions, such errors would trigger continuous retries. After upgrading to v3.4.2 or above, the behavior changed from retrying to automatically pausing the job, resulting in a behavioral change. |
