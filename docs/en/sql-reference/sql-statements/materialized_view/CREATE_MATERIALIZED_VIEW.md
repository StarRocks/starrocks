...
- `auto_refresh_partitions_limit`: The number of most recent materialized view partitions that need to be refreshed when a materialized view refresh is triggered. You can use this property to limit the refresh range and reduce the refresh cost. However, because not all the partitions are refreshed, the data in the materialized view may not be consistent with the base table. Default: `-1`. When the value is `-1`, all partitions will be refreshed. When the value is a positive integer N, StarRocks sorts the existing partitions in chronological order, and refreshes the current partition and N-1 most recent partitions. If the number of partitions is less than N, StarRocks refreshes all existing partitions. If there are dynamic partitions created in advance in your materialized view, StarRocks refreshes all pre-created partitions. This property is not supported on a materialized view whose `EFFECTIVE_REFRESH_MODE` is `INCREMENTAL` or `AUTO`, because incremental maintenance requires a complete baseline. To batch the initial full refresh of such a view, use `partition_refresh_number` instead; refreshes after that process only the base table changes and are not affected by either property.
...
- If you omit `distribution_desc`, StarRocks uses range distribution only in shared-data mode when `enable_range_distribution` and `enable_mv_range_distribution` are both enabled. In all other cases, StarRocks falls back to hash distribution over all target key columns.
...
- A range-distributed partition starts with a single tablet, so its first refresh would otherwise be written by one node. StarRocks therefore pre-splits the tablets that refresh writes into, deriving the boundaries from the internal row-id column instead of sampling any data. This applies while the view's row ids are generated by the storage engine and none has been allocated for the view yet, which is the case for the first refresh of a newly created view, and while that refresh targets a single partition. It is best-effort: any other case is refreshed without pre-splitting and the reason is recorded under `starrocks_fe_tablet_pre_split_eligibility_skipped`. Set `enable_tablet_pre_split_for_mv_refresh` to `false` to turn it off.
