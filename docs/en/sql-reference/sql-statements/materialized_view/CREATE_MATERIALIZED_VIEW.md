...
- `mv_rewrite_staleness_second`: If the materialized view's last complete refresh was confirmed within the time interval specified in this property (measured from the start time of that refresh), this materialized view can be used directly for query rewrite, regardless of whether the data in the base tables changes. Otherwise, StarRocks checks whether the base tables have been updated to determine whether the materialized view can be used for query rewrite. Note that a refresh covering only part of the partitions (for example, a manual `REFRESH ... PARTITION START ... END`) does not renew this freshness baseline. Unit: Second. This property is supported from v3.0.
