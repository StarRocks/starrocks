...
| last_refresh_mode_reason_table | The base table that drove the mode decision, as `catalog.database.table`. Empty when no single base table drove it. A reason the backend reports while reading changes identifies a tablet rather than a table, so it is named only where the frontend can narrow the tablet down to one base table on its own: `CHANGE_CAPTURE_DISABLED` can only come from a primary-key base table whose changes the refresh had to read, and is named when exactly one base table fits. |
