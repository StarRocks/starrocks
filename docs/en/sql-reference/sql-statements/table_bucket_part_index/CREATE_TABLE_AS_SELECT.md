...
| column_name       | No          | The name of a column in the new table. You do not need to specify the data type for the column. StarRocks automatically specifies an appropriate data type for the column. StarRocks converts FLOAT and DOUBLE data into DECIMAL(38,9) data. For CHAR, VARCHAR, and STRING data, StarRocks creates VARCHAR columns and preserves the source length when it is explicitly specified or can be inferred. If the source length cannot be inferred, StarRocks uses the current maximum OLAP VARCHAR length, 2147482624. |
