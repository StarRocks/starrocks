...
| ------------- | ------------ | ----------------------------------------------------------- |
| db_name       | No           | Name of the database to which the Routine Load job belongs. |
| JobName       | No           | Name of the Routine Load job.                               |

## Output

| **Parameter**        | **Description**                                              |
| -------------------- | ------------------------------------------------------------ |
| TaskId               | Globally unique ID of the load task, automatically generated by StarRocks. |
| TxnId                | ID of the transaction to which the load task belongs.        |
...
| Timeout              | Timeout period for the load task, controlled by the FE parameter [`routine_load_task_timeout_second`](../../../administration/FE_configuration.md#routine_load_task_timeout_second) and the `task_timeout_second` parameter in [job_properties](./CREATE_ROUTINE_LOAD.md#job_properties) for the Routine Load job. |
| BeId                 | ID of the BE that executes the load task.                    |
| DataSourceProperties | The load task's progress (measured in the offset) of consuming messages in partitions of the topic. |
| Message              | Information returned for the load task, including task error information. |
