...
| `SET (...)`     | `"key" = "value"` pairs to merge. Allowed keys are the same as for the provider's `TYPE` (see [`CREATE AI PROVIDER`](./CREATE_AI_PROVIDER.md)). `protocol` can be changed to any supported value. |
