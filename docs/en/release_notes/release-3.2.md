...
- Changed the default value of the BE/CN configuration item `starlet_use_star_cache` to `true`. This indicates that Block Cache is enabled by default in shared-data clusters. If, before upgrading, you have manually configured the BE/CN configuration item `starlet_cache_evict_high_water`, for example, to `X`，you must configure the BE/CN configuration item `starlet_star_cache_disk_size_percent` as `1.0-X` (For example, if you have set `starlet_cache_evict_high_water` to `0.3` before upgrading, then you must set `starlet_star_cache_disk_size_percent` to `0.7`). This will ensure that both file cache and block cache use the same disk capacity. [#38200](https://github.com/StarRocks/starrocks/pull/38200)
