...
- Changed the default value of the BE/CN configuration item `starlet_use_star_cache` to `true`. This indicates that Block Cache is enabled by default in shared-data clusters. If, before upgrading, you have manually configured the BE/CN configuration item `starlet_cache_evict_high_water` to `X`，you must configure the BE/CN configuration item `starlet_star_cache_disk_size_percent` to `1.0 - X`. For example, if you have set `starlet_cache_evict_high_water` to `0.3` before upgrading, you must set `starlet_star_cache_disk_size_percent` to `0.7`. This ensures that both file cache and Block Cache will not exceed the disk capacity limit. [#38200](https://github.com/StarRocks/starrocks/pull/38200)
