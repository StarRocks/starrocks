...
- StarRocks supports using `s3_compatible_fs_list` to configure a schema for S3-compatible object storage (the AWS SDK is used), and supports using `fallback_to_hadoop_fs_list` to configure a schema for HDFS (the JAR package provided by the vendor is used). [#41612](https://github.com/StarRocks/starrocks/pull/41612)
