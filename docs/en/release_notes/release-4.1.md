...
- `pipeline_enable_large_column_checker` is set to `true` (enabled) by default. [#72798](https://github.com/StarRocks/starrocks/pull/72798)
- The COW (copy-on-write) optimization is disabled. [#73480](https://github.com/StarRocks/starrocks/pull/73480)
...
- Added a `schema` parameter to `FILES()` for specifying an explicit schema to read. [#72033](https://github.com/StarRocks/starrocks/pull/72033)
...
- Reduced lock contention in `CachingIcebergCatalog` by switching the locking from catalog level to table level. [#73079](https://github.com/StarRocks/starrocks/pull/73079)
...
- Invalid JIT IR for CASE WHEN with mixed float/int when and result types. [#74382](https://github.com/StarRocks/starrocks/pull/74382)
- A FlatJson crash when subwriters receive no appends. [#73730](https://github.com/StarRocks/starrocks/pull/73730)
- Crashes in shared-data lake replication file-copy. [#73666](https://github.com/StarRocks/starrocks/pull/73666)
- A potential out-of-bounds access caused by partitioned join. [#74315](https://github.com/StarRocks/starrocks/pull/74315)
- Added a null-check on rowset before reading gtid in schema change. [#74855](https://github.com/StarRocks/starrocks/pull/74855)
- `count_combine(nullable_col)` silently counting NULL rows like `count(*)`. [#74029](https://github.com/StarRocks/starrocks/pull/74029)
- Aggregation using type-mismatched aggregate functions. [#74159](https://github.com/StarRocks/starrocks/pull/74159)
- `NullableColumnUnaryFunction` losing a decimal's scale when all values are NULL. [#73789](https://github.com/StarRocks/starrocks/pull/73789)
- Partition TopN potentially losing a child's output column. [#72848](https://github.com/StarRocks/starrocks/pull/72848)
- Derived dictionary translations for expressions where `f(null) != null`. [#69376](https://github.com/StarRocks/starrocks/pull/69376)
- `extract` reading the wrong host from a URL. [#63542](https://github.com/StarRocks/starrocks/pull/63542)
- JSON load partial-append for nested types. [#73715](https://github.com/StarRocks/starrocks/pull/73715)
- Used null-safe equality for Iceberg equality delete columns. [#67321](https://github.com/StarRocks/starrocks/pull/67321)
- PostgreSQL and Oracle JDBC type mapping issues. [#70842](https://github.com/StarRocks/starrocks/pull/70842) [#71412](https://github.com/StarRocks/starrocks/pull/71412)
- Paimon DATE partition columns with NULL values, and Paimon primary key columns being incorrectly marked as non-nullable. [#73950](https://github.com/StarRocks/starrocks/pull/73950) [#71660](https://github.com/StarRocks/starrocks/pull/71660)
- `__ROW_ID__` group keys for SELECT DISTINCT and positional GROUP BY in incremental materialized views. [#74493](https://github.com/StarRocks/starrocks/pull/74493) [#74030](https://github.com/StarRocks/starrocks/pull/74030)
- Rejected aggregate materialized view rewrite with a HAVING predicate, and rejected HAVING with aggregates in incremental materialized views at creation time. [#73610](https://github.com/StarRocks/starrocks/pull/73610) [#74054](https://github.com/StarRocks/starrocks/pull/74054)
- An NPE in nested materialized view refresh. [#73644](https://github.com/StarRocks/starrocks/pull/73644)
- Detects Iceberg/external materialized view schema drift on refresh. [#73770](https://github.com/StarRocks/starrocks/pull/73770)
- IVM materialized view refresh now surfaces strict-load errors instead of silently dropping rows. [#73938](https://github.com/StarRocks/starrocks/pull/73938)
- Restored `SHOW CREATE MATERIALIZED VIEW` for sync materialized views. [#73396](https://github.com/StarRocks/starrocks/pull/73396)
- A memory leak introduced by the UDAF cache. [#74025](https://github.com/StarRocks/starrocks/pull/74025)
- Memory accounting in `OlapTableSink`. [#73807](https://github.com/StarRocks/starrocks/pull/73807)
- A data race on `MaterializedIndexMeta` `updateSchemaBackendId`. [#74412](https://github.com/StarRocks/starrocks/pull/74412)
- A data race on the spill writer's `auto_flush` flag when entering parallel merge mode. [#73616](https://github.com/StarRocks/starrocks/pull/73616)
- A race condition in `TabletSinkSender::_send_chunk_by_node`. [#73820](https://github.com/StarRocks/starrocks/pull/73820)
- A `set_thread_name` race on data dir load threads. [#73862](https://github.com/StarRocks/starrocks/pull/73862)
...
- Ranger policy rewrite for Hive views. [#73265](https://github.com/StarRocks/starrocks/pull/73265)
...
- FE startup failure during ADLS2 shared-data disaster recovery caused by `AZURE_PATH_KEY` validation. [#73509](https://github.com/StarRocks/starrocks/pull/73509)
- A session variable override caused by `SET WAREHOUSE` in statistics tasks. [#74385](https://github.com/StarRocks/starrocks/pull/74385)
...
- Memory leak when caching the materialized view plan context. [#72300](https://github.com/StarRocks/starrocks/pull/72300)
...
- Aggregate-join-pushdown materialized view rewrite on Iceberg base tables. [#71856](https://github.com/StarRocks/starrocks/pull/71856)
...
- A memory leak in `DefaultValueColumnIterator` for complex types. [#71142](https://github.com/StarRocks/starrocks/pull/71142)
- A memory leak caused by `shared_ptr` cycle between `BatchUnit` and `FetchTaskContext`. [#71126](https://github.com/StarRocks/starrocks/pull/71126)
...
- Fixed materialized view refresh bug in the case of multiple tables. [#61763](https://github.com/StarRocks/starrocks/pull/61763)
- Fixed incorrect materialized view recycle time if force refreshed. [#68673](https://github.com/StarRocks/starrocks/pull/68673)
- Fixed all-null value handling bug in sync materialized view. [#69136](https://github.com/StarRocks/starrocks/pull/69136)
- Fixed duplicate column id error when querying materialized view after fast schema change ADD COLUMN. [#71072](https://github.com/StarRocks/starrocks/pull/71072)
...
- `DROP FUNCTION IF EXISTS` ignoring `ifExists` flag. [#69216](https://github.com/StarRocks/starrocks/pull/69216)
...
- `array_map` crash when processing null literal array. [#70629](https://github.com/StarRocks/starrocks/pull/70629)
