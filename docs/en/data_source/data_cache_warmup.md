...
- `TOTAL_CACHE_USAGE`: The space usage of the data cache after this warmup task is complete. This metric can be used to assess whether the data cache has sufficient space.

### Warm up specified columns with filter conditions

You can also perform more fine-grained warmup by specifying columns and predicates to reduce data cache usage, as shown in the following case:
...
- `AVG_READ_CACHE_TIME`: the average lookup time for each file in the data cache.
