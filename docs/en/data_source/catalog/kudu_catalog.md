...
| kudu.client.default-operation-timeout-ms | No       | The timeout, in milliseconds, that the Kudu client uses for regular (non-administrative) operations, primarily data scans. StarRocks also propagates this value into the scan tokens sent to the BEs, so it bounds the per-request timeout the BEs use when scanning Kudu tablet servers. The value must be a positive integer. Defaults to `30000` (30 seconds), which matches the kudu-client built-in default. |
| kudu.client.default-admin-operation-timeout-ms | No       | The timeout, in milliseconds, that the Kudu client uses for administrative operations against the Kudu master, such as listing tables, opening tables, and fetching table schemas and statistics. This applies to FE-side metadata requests only. The value must be a positive integer. Defaults to `30000` (30 seconds), which matches the kudu-client built-in default. |
