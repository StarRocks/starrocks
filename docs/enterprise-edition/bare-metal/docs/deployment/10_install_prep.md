...
## Determine the StarRocks cluster details

After installing the Manager, you will either deploy a new StarRocks cluster or manage an existing StarRocks cluster with CelerData Manager.
...
### Enable SSH password-free login for all nodes

  :::note
  When you deploy CelerData Manager, you need to use SSH and Python to transfer files between CelerData Manager and StarRocks cluster nodes. Therefore, you must enable **SSH password-free login for all nodes.** If you encounter the `permission denied` issue, check the SSH configuration.
  :::
...
Obtain `CelerData-EE-x.x.x.tar.gz` of the required version from your sales manager and decompress this package.
