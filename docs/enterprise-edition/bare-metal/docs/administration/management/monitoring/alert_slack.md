...
This topic describes how to integrate Slack with CelerData Manager via webhook to receive alerts from your CelerData cluster.
...
6. Navigate to your workspace, and invite the bot to the channel via which you want CelerData Manager to push alerts by sending `/invite {your app name}` (for example, `/invite @test_app`) in the channel.

   ![Slack-6](../../../_assets/manager/slack_6.png)

7. On the page of the channel, click the More Actions button on the top-right corner, and select **Open channel details**. On the channel detail tab, copy the **Channel ID**.

   ![Slack-7](../../../_assets/manager/slack_7.png)

## Configure CelerData Manager

Follow these steps to configure CelerData Manager:
...
Now you can receive alerts from CelerData Manager from your Slack channel.
