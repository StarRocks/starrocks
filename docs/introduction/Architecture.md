...
FEs are responsible for metadata management, client connection management, query planning, and query scheduling. Each FE stores and maintains a complete copy of metadata in its memory, which guarantees indiscriminate services among the FEs. FEs can work as followers and observers based on your configurations. Followers can elect a leader according to the Paxos-like BDB JE protocol.
