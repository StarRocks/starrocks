...
6. (Optional) Check the status of the CN nodes by executing the following SQL.
   ...
   *************************** 1. row ***************************
   ...
   After CNs are properly started, you need to set the system variables `SET prefer_compute_node = true;` and `SET use_compute_nodes = -1;` to scale computing resources during queries. For more information, see [System variables](../reference/System_variable.md#descriptions-of-variables).
