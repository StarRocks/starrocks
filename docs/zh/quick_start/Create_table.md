...
StarRocks 表中支持多种字段类型，除以上示例中已经列举的字段类型，还支持 [BITMAP 类型](../using_starrocks/distinct_values/Using_bitmap.md)，[HLL 类型](../using_starrocks/distinct_values/Using_HLL.md)，[ARRAY 类型](../sql-reference/data-types/semi_structured/Array.md)，字段类型介绍详见 [数据类型章节](../sql-reference/data-types/numeric/BIGINT.md)。
