...
> 注意：排序列在建表时应定义在其他列之前。排序键详细描述以及不同类型表中排序键的设置方法请参考 [排序键](../table_design/indexes/Prefix_index.md)。
...
StarRocks 默认会给 Key 列创建前缀索引加速查询，具体规则见 [前缀索引](../table_design/indexes/Prefix_index.md)。支持的索引类型有 [Bitmap 索引](../table_design/indexes/Bitmap_index.md)，[Bloomfilter 索引](../table_design/indexes/Bloomfilter_index.md) 等。
