...
- **刷新时修复**：在刷新物化视图时会尝试自动执行以上的修复命令，重建物化视图再刷新
- **自动修复**： 后台线程会尝试自动修复 Inactive 的物化视图，您可以通过 `ADMIN SET FRONTEND CONFIG('enable_mv_automatic_active_check'='false')` 关闭此功能。此功能自 3.1.4 和 3.2.0 版本开始支持。
