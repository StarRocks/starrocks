...
自 StarRocks v3.1.7 和 v3.2.3 版本起，StarRocks 引入 Block Cache 功能用以加速存算分离集群中的查询。该功能将取代早期版本中的 File Cache 功能。Block Cache 以 Block（MB 级别）为单位，按需从远端存储中加载数据。相较之下，File Cache 无论查询需要读取多少数据，都需要在后台线程加载整个数据文件。

相比 File Cache，Block Cache 具有以下优势：
...
自 v3.2.3 版本起，StarRocks 默认启用 Block Cache。如果您需要在 v3.1 版本集群中使用 Block Cache，或此前曾手动禁用了此功能，需执行以下流程启用 Block Cache。

执行以下语句在集群运行时动态启用 Block Cache：
...
如需永久启用 Block Cache，需要将以下配置添加到 CN 配置文件 **cn.conf** 中，并重新启动 CN 节点：
...
您可以通过以下 CN（BE）配置项配置 Block Cache：
...
StarRocks 提供了多种监控 Block Cache 的指标。
...
如需永久禁用 Block Cache，需要将以下配置添加到 CN 配置文件 **cn.conf** 中，并重新启动 CN 节点：
...
- 如果云原生表的 `datacache.enable` 属性设置为 `false`，则不会为该表启用 Block Cache。
...
# 将 <starlet_cache_path> 替换为集群的 Block Cache 目录，例如 /usr/be/storage/starlet_cache
...
### Q1：为什么通过 `du`、`ls` 命令看到的 Block Cache 目录占用的空间远大于实际的数据量？

Block Cache 的磁盘占用空间代表的是历史上的最高水位，与当前实际缓存的数据量没有关系。例如，假设导入了 100 GB 的数据，之后进行了 Compaction，数据量变成了 200 GB，然后又进行了数据 GC，数据量变成了 100 GB，那么 Block Cache 的磁盘占用空间仍然保持在最高水位 200 GB，但 Block Cache 内部的实际缓存数据量是 100 GB。

### Q2：Block Cache 是否会自动淘汰？

不会。Block Cache 只有在达到磁盘使用上限（默认设置下为磁盘容量的 80%）时才会开始淘汰数据。淘汰过程并不会实际删除数据，只是将旧缓存的磁盘位置标记为空，后续的新缓存写入时会覆盖旧缓存。因此，即使发生淘汰，磁盘占用空间也不会下降，不会影响实际使用。

### Q3：为什么磁盘水位一直维持在配置的最高点，不下降？

参考 Q2。Block Cache 的淘汰机制不会实际删除数据，只是将旧缓存标记为可覆盖，因此磁盘占用空间不会下降。

### Q4：为什么 DROP TABLE 后，远程存储显示表文件已删除，但 Block Cache 实际存储的数据量没有下降？

DROP TABLE 不会触发 Block Cache 删除缓存的数据。被删除表的缓存会随着时间的推移，通过 Block Cache 的内部 LRU 逻辑逐渐淘汰，不影响实际使用。

### Q5：为什么配置了 80% 的磁盘容量，实际使用却达到了 90% 甚至更多？

Block Cache 的磁盘容量使用是精确的，不会超过配置中设置的容量。磁盘实际使用超过 80% 可能是由以下原因导致的：
...
您可以在磁盘根目录或各个子目录执行 `du -h . -d 1` 查看具体占用空间的目录，然后删除意外的占用部分。或者，也可以通过配置 `starlet_star_cache_disk_size_percent` 调小 Block Cache 可使用的磁盘容量。
