...
| REFRESH_MODE                       | 物化视图所配置的刷新模式。有效值：`PCT`、`INCREMENTAL` 和 `AUTO`。如果物化视图已被删除，则为 `NULL`。注意这是**配置值**，不是某次刷新实际采用的模式：bootstrap 首刷以及 INCREMENTAL 规划失败回退 PCT 时，实际模式与本列不同。某次刷新实际采用的模式请查询 `information_schema.task_runs` 的 `get_json_string(EXTRA_MESSAGE, '$.refreshMode')`。 |
...
| REFRESH_MODE_REASON_TABLE | 导致该模式决策的基表,格式为 `catalog.database.table`。没有单一基表导致时为空。由 BE 在读取变更时报出的原因定位到的是 tablet 而非表,因此只在 FE 能自行把该 tablet 收窄到唯一一张基表时才填:`CHANGE_CAPTURE_DISABLED` 只可能来自「本次刷新需要读取其变更的主键基表」,当恰好只有一张基表符合时填入该表。 |
