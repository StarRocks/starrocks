...
| RESOURCE_GROUP                     | 刷新作业所使用的资源组。该值为物化视图所配置的 `session.resource_group`；未配置时返回 `default_mv_wg`。 |
| CREATOR                            | 创建物化视图的用户（任务的所有者）。                         |
| SUBMIT_USER                        | 提交刷新作业的用户。对于手动刷新，该值为发起刷新的用户；对于周期性刷新或基表变更触发的刷新，则由系统提交。 |
| RUN_AS_USER                        | 刷新所使用的运行用户身份。                                   |
| SUBMIT_TIME                        | 作业提交的时间（第一个 task run 的创建时间）。               |
| REFRESH_STATE                      | 作业的状态，由最后一个 task run 汇总而来。有效值：`PENDING`、`RUNNING`、`FAILED`、`SUCCESS`、`MERGED` 和 `SKIPPED`。 |
...
| REFRESH_TRIGGER                    | 刷新的触发方式，由物化视图的刷新方案推导而来。有效值：`MANUAL`、`SCHEDULED`、`ON_BASE_TABLE_CHANGE` 和 `NONE`。如果物化视图已被删除，则为 `UNKNOWN`。 |
| REFRESH_MODE                       | 物化视图所配置的刷新模式。有效值：`AUTO`、`PCT` 和 `INCREMENTAL`。如果物化视图已被删除，则为 `NULL`。 |
| IMV_SOURCE_VERSION_RANGE           | 增量刷新所消费的源版本范围的 JSON。不适用时返回 `{}`。       |
| IMV_SOURCE_TIMESTAMP_RANGE         | 增量刷新所消费的源时间戳范围的 JSON。不适用时返回 `{}`。     |
| IMV_SOURCE_PINNED_SNAPSHOT_ID_MAP  | 增量刷新所固定的源快照 ID 的 JSON。不适用时返回 `{}`。       |
| FAILED_TASK_RUN_ID                 | 作业中失败 task run 的 ID。如果没有 task run 失败，则为 `NULL`。 |
...
:::note
该视图没有持久化存储。其数据行在查询时由 `task_runs` 派生而来，因此记录的保留时长遵循 `task_runs` 历史记录的设置。
:::
