...
注意此处的 `user_identity` 语法与 [CREATE USER](./CREATE_USER.md) 章节中的相同。且必须为使用 `CREATE USER` 创建过的 `user_identity`。否则会报错用户不存在。如果不指定 `user_identity`，则会修改当前用户的密码。可以通过 `SHOW GRANTS;` 查看当前用户。
