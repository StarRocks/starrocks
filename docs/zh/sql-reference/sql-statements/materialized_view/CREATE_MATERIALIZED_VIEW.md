...
- `ON_CHANGE`: 基表变更触发模式。每当基表数据发生变化时，物化视图会自动刷新。自 v26.2 起，不带 `EVERY` 的 `REFRESH ASYNC` 不再支持，请改用 `REFRESH ON_CHANGE`。v26.2 之前创建的物化视图不受影响，其 `SHOW CREATE MATERIALIZED VIEW` 输出为 `ON_CHANGE`。
...
- `partition_refresh_strategy`：单次刷新中物化视图的刷新策略。当值为`adaptive`，会根据基表分区的数据量来自行判断此次刷新需要刷新的分区数，此策略会极大地提高刷新效率。如不指定该属性，默认是`strict`, 即单次刷新完全由`partition_refresh_number`来控制。取值为 `force` 时每次刷新都会刷新全部分区；对 `EFFECTIVE_REFRESH_MODE` 为 `INCREMENTAL` 或 `AUTO` 的物化视图不生效，这类视图按定义就是依据基表变更增量维护的。以 `refresh_mode` 为 `AUTO` 创建、但因定义无法增量维护而定型为 `PCT` 的视图，`force` 仍然生效。
...
- 如果省略 `distribution_desc`，仅当处于存算分离模式且 `enable_range_distribution` 与 `enable_mv_range_distribution` 均已启用时，StarRocks 才使用 Range 分布。其他情况下，StarRocks 回退到基于目标表全部 Key 列的哈希分布。
...
增量刷新读取的是每次导入产生的变更，因此基表自上次成功刷新以来产生的每个版本都会保留在基表上，直到某次刷新消费它。被消费掉的版本会被释放。

保留版本会占用存储，因此存算分离集群中 StarRocks 基表上这样保留的版本有保留上限：[`bookmark_reference_max_ttl_ms`](../../../administration/configuration/FE_parameters/shared_lake_other.md#bookmark_reference_max_ttl_ms)，默认 3 天。物化视图若在该上限内没有成功刷新，就会丢失下次刷新本该读取的那些版本，从而无法再进行增量刷新：

- `INCREMENTAL` 下，刷新失败，物化视图被置为 inactive，需要删除并重建才能恢复。
- `AUTO` 下，刷新回退为 `PCT` 全量刷新，之后的刷新恢复为增量。

请让物化视图无法成功刷新的每个时间窗都短于该保留上限：

- 调度刷新周期设置为小于该上限。
- 两次手动刷新的间隔小于该上限。
- 手动置为 inactive 的持续时长小于该上限。
- 在该上限内处理掉刷新失败。
