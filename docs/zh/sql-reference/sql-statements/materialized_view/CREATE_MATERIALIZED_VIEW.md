...
`refresh_mode = INCREMENTAL` 模式下可使用的聚合函数被限制在一个经过端到端验证的白名单内。聚合函数不在白名单上时，CREATE MATERIALIZED VIEW 会在分析阶段抛出明确的错误。

| 函数                                      | 支持的入参类型                            | 备注                                                            |
|-------------------------------------------|-------------------------------------------|----------------------------------------------------------------|
| `COUNT(*)` / `COUNT(col)`                 | 任意类型                                  |                                                                |
| `SUM(col)`                                | 整数 / 浮点 / DECIMAL                     |                                                                |
| `AVG(col)`                                | 整数 / 浮点                               | 暂不支持 DECIMAL，待 BE state union 修复。                     |
| `MIN(col)` / `MAX(col)`                   | 整数 / 浮点 / DATE / DATETIME             | 暂不支持 VARCHAR / STRING，待 combinator 类型推导修复。        |
| `BOOL_OR(col)`                            | boolean                                   |                                                                |
| `APPROX_COUNT_DISTINCT(col)` / `NDV(col)` | 任意类型                                  |                                                                |

以下聚合形态在 CREATE 阶段会被显式拒绝：

- **distinct 聚合**（`COUNT(DISTINCT)`、`SUM(DISTINCT)` 等）：与基于 state 的增量刷新机制根本不兼容，重写过程会丢弃 `DISTINCT` 标记，state union 阶段无法保留 distinct 语义。
- **`AVG(DECIMAL)`**：已知 BE state union bug；修复前任何一次增量刷新后物化视图会静默返回 `0`。
- **`MIN(VARCHAR)` / `MAX(VARCHAR)`**：已知 FE combinator 类型扩展 bug；刷新时抛出 `IllegalArgumentException`。
- **`ARRAY_AGG`**：已知 FE combinator 类型 bug；刷新时 thrift 序列化失败。
- **`HLL_UNION` / `BITMAP_UNION` / `GROUP_CONCAT`**：这些函数的 combinator 暂未注册。
- **窗口函数**（`ROW_NUMBER`、`RANK`、`LAG`、`LEAD` 等）：不在增量刷新模型范围内。
- **`PERCENTILE_*`、`STDDEV_*`、`VAR_*`、`MAP_AGG`、`RETENTION`、`WINDOW_FUNNEL`** 等：不在白名单上。

后续随着底层 BE / combinator 问题被修复，白名单会逐步放开；升级时请关注 release notes 了解新放开的组合。
