...
- 如果使用模拟身份的身份验证方法访问 Google Storage（从 v3.5.1 版开始支持），请设置以下 PROPERTIES：
...
- 如果使用基于 S3 协议以及 IAM User 的身份验证方法访问 Google Storage，请设置以下 PROPERTIES：
  ...
  :::tip 
  Google Storage 支持使用 [XML API](https://docs.cloud.google.com/storage/docs/interoperability)，设置使用 AWS S3 语法。在这种情况下，必须将 `TYPE` 设置为 `S3`，并将 `LOCATIONS` 设置为与 S3 协议兼容的存储位置。
  :::
