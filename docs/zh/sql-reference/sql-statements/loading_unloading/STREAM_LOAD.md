...
| partial_update_mode | 否 | 指定部分更新的模式。有效值：`row`、`column` 和 `flexible_row`。<ul><li> 值 `row`（默认）表示行模式的部分更新，更适合列数较多、批量较小的实时更新场景。</li><li>值 `column` 表示列模式的部分更新，更适合列数较少、行数较多的批量更新场景。在此类场景中，启用列模式可提供更快的更新速度。例如，在一张有 100 列的表中，如果只更新 10 列（占总列数的 10%）的所有行，列模式的更新速度是行模式的 10 倍。</li><li>值 `flexible_row`（实验功能）表示灵活部分更新：JSON 格式导入的每一行只更新该行中出现的列，该行未出现的列保留当前值。插入新主键的行，其未出现的列取列的默认值。在行中出现且值为 `null` 的列会被更新为 `NULL`。导入以行模式执行，即重写整行。需要将 `partial_update` 设置为 `true`，并在 `columns` 中列出各行可能出现的所有列。</li></ul>**说明**<br/>取值区分大小写。<br/>`flexible_row` 仅支持通过 Stream Load 导入存算分离集群的主键表，并且需要同时将 FE 配置项 `enable_flexible_partial_update` 和同名的 BE 配置项设置为 `true`，否则导入失败。请在集群所有 BE 和 CN 都升级到支持灵活部分更新的版本之后再开启：更早版本的节点会把这样的导入当作普通部分更新执行，把未出现的列更新为 `NULL`。<br/>`flexible_row` 不能与 `merge_condition`、`jsonpaths` 或包含表达式的 `columns` 映射同时使用，也不支持 Range 分布的表、排序键（ORDER BY）包含非主键列的表、包含 AUTO_INCREMENT 列或生成列的表、包含名为 `__cset__` 的列的表、事务 Stream Load 和 Merge Commit。取值 `flexible`（列模式的对应取值）为保留值，暂不支持。<br/>如果一次导入中有多行主键相同，该主键的每一列取这些行中最后一个包含该列的行的值。删除行（`__op` = 1）会丢弃同一主键在它之前的行；如果删除行是该主键的最后一行，则删除该主键。 |
