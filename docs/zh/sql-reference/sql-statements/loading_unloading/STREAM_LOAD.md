...
| envelope          | 否           | 指定 JSON 数据的 CDC Envelope 格式。有效值：`debezium`。默认不设置（即无 Envelope 包装）。设置为 `debezium` 时，StarRocks 将每条 JSON 消息解析为 Debezium CDC 事件，消息中须包含 `op` 字段（`c`=insert、`u`=update、`d`=delete、`r`=快照读取）以及 `after` 字段（c/u/r 操作）或 `before` 字段（d 操作），用于承载实际行数据。`payload` 为 `null` 的 tombstone 消息将被跳过。不能与 `json_root` 或 `strip_outer_array` 同时使用。 |
