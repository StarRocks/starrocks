...
| pause_on_fatal_parse_error          | 否           | 遇到不可恢复的数据解析错误是否自动暂停作业，取值包括 `true` 和 `false`，默认值为 `true`。该配置自 v3.4.2 开始支持。这类解析错误通常由于非法格式数据导致，比如：<ul><li>导入 JSON array，但是没有设置 `strip_outer_array`。</li><li>导入 JSON 数据，但 Kafka message 是非法的 JSON，如 `abcd`。</li></ul>**说明**<br />之前版本遇到这种错误会不断重试，升级到 v3.4.2 及之后版本由重试变为自动暂停，存在行为变化。|
