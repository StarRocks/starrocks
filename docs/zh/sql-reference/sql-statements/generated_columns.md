...
不过需要注意的是，导入数据至具有生成列的表时，因为需要额外计算出表达式的结果并写入生成列，耗时和开销可能会有所增大。
...
大部分情况中，您是在建表后确定查询时频繁使用的表达式，因此往往在建表后添加生成列。因此 StarRocks 对建表后添加生成列的操作做了特别的优化。添加生成列时 StarRocks 不需要重写全部数据，只需要写入新增的生成列的数据，并且关联到存量的物理文件上即可，大大提高了建表后添加生成列的效率。
