...
description: "CREATE ANALYZE 创建自定义自动采集任务进行 CBO 统计信息采集。"
---
...
- PROPERTIES: 采集任务的自定义参数。如果不配置，则采用 `fe.conf` 中的默认配置。任务实际执行中使用的 PROPERTIES，可以通过 [SHOW ANALYZE JOB](SHOW_ANALYZE_JOB.md) 中的 `Properties` 列查看。
...
[SHOW ANALYZE JOB](SHOW_ANALYZE_JOB.md)：查看创建的自定义自动采集任务。
