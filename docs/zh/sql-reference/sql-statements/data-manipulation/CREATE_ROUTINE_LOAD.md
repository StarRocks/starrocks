...
| timezone                  | 否           | 该参数的取值会影响所有导入涉及的、跟时区设置有关的函数所返回的结果。受时区影响的函数有 strftime、alignment_timestamp 和 from_unixtime 等，具体请参见[设置时区](../../../administration/management/timezone)。导入参数 `timezone` 设置的时区对应[设置时区](../../../administration/management/timezone)中所述的会话级时区。 |
