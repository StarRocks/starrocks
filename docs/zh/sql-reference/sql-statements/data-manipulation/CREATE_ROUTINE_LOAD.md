...
| failure_pause_interval_second | 否           | Routine Load 导入作业中每个 Routine Load 导入任务持续失败重试时间，单位为秒。当导入作业的任一任务持续失败超过该值时，任务将会转为 PAUSED 状态。对于由于持续失败而暂停的任务，请务必检查任务失败原因，避免长时间无法同步数据导致数据源数据过期。该参数仅针对单个 Routine Load 导入作业。当未配置 `failure_pause_interval_second` 时，StarRocks 则使用 FE 动态参数 `routine_load_failure_pause_interval_second` 来控制该行为。|
