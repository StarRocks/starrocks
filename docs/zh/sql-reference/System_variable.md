...
* **描述**: 用于将某个外部标识绑定到当前查询。可以在执行查询前通过 `SET SESSION custom_query_id = 'my-query-id';` 设置。查询结束后该值会被重置。该值可以用于 `KILL QUERY 'my-query-id'`。在审计日志中可以作为 `customQueryId` 字段找到该值。
* **默认值**: ""
* **类型**: String
* **引入版本**: v3.4.0
...
* **作用域**: Session
* **描述**: 控制 ColocatedBackendSelector 用于将桶序列分配到后端的遍历算法。当该值 <= 0 时，ColocatedBackendSelector 始终使用 NormalBucketSequenceIterator。否则 FE 会计算：
  - numTotalBuckets（每个 bucket 首个副本的副本数之和），
  - backends.size()（不同后端的数量），
  - hasMultiReplications（是否存在副本数 >1 的 bucket）。
  如果 hasMultiReplications 且 numTotalBuckets <= maxBucketsPerBeToUseBalancerAssignment * backends.size()，ColocatedBackendSelector 会切换到 BalancerBucketSequenceIterator，该迭代器能在后端间产生更均匀的分配。BalancerIterator 以更高的 CPU 代价减少倾斜：其 useBackend() 的复杂度为 O(numBucketsPerBe)，因此仅在每个后端的 bucket 数足够小时才会选择该算法。详情参见 ColocatedBackendSelector.createBucketIterator 和 BalancerBucketSequenceIterator。
* **默认值**: 6
* **类型**: int
* **引入版本**: 3.2.0
