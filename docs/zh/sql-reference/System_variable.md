...
### enable_local_first_tablet_write (v4.2 及以后)

* **描述**：仅在存算分离模式下生效。开启后，导入把每一行写入 sink 实例所在计算节点上的 delta writer，而不是发送给该 tablet 归属的那一个节点。这样既把单个 tablet 的写入分散到整个集群，也省掉了这些数据本来要走的网络传输。该变量仅对开启了 `file_bundling` 的存算分离表生效，并且只在分区的 tablet 数量少于 Warehouse 中存活计算节点数量时生效——超过之后分桶级别的并行度已经可以占满集群，此时再写本地只会白白丢掉读侧的缓存局部性。开启它需要承担一个前提：相同键的行会落到不同节点、彼此之间没有先后顺序，因此单个事务内依赖重复键到达顺序的导入，其胜出者是不确定的。事务之间的顺序不受影响。
* **默认值**：false
* **类型**：Boolean
* **粒度**：Session
* **引入版本**：v4.2
