...
* **描述**：使用缓存的 Delta Lake 快照行数作为优化器统计，避免在规划阶段提前枚举全部文件。默认关闭。已收集的内部统计仍优先使用；显式开启 `enable_delta_lake_column_statistics` 时仍收集完整的元数据统计。缓存未命中时，优化器使用 `default_statistics_output_row_count`，列统计为未知值，因此可能改变 Join 和聚合计划，建议验证代表性查询后再启用。缓存行数来自文件元数据，是估算值，可能包含被删除向量隐藏的行，不能用于限制实际扫描或直接返回精确的 `COUNT(*)` 结果。
* **默认值**：false
* **类型**：Boolean

缓存属于各 FE 的各个 Catalog，最多保存 1,000 个快照摘要，条目在一小时未访问后过期。表路径、Delta 表 ID 和快照版本共同隔离条目。只有完整且无过滤的文件枚举才会写入缓存；有过滤、已取消、失败或只消费部分文件的扫描不会发布摘要。关闭此开关执行无过滤查询可以预热缓存。缓存未命中不会启动后台统计收集。
