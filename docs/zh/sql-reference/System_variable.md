...
* 描述：是否启用数据湖（如 Iceberg 表）查询的 bucket_aware 执行。启用后，系统通过利用分桶信息来优化查询执行，减少数据 shuffle 并提高性能。此优化对分桶表的 Join 和 Aggregation 特别有效。
* 默认值：true
* 数据类型：Boolean
* 引入版本：v4.0
...
* 描述：数据湖表查询的分桶分配模式。此变量控制查询执行期间且 bucket_aware 执行起作用时如何将分桶分配给工作节点。有效值：
