...
* **范围**：Session
* **描述**：builtin GIN 全文 `score()` 相关性排序使用的 BM25 长度归一化参数 `b`。取值越大，对长文档的惩罚越强。必须在 [0, 1] 范围内。
* **默认值**：0.75
* **类型**：Double
* **引入版本**：-
...
* **范围**：Session
* **描述**：builtin GIN 全文 `score()` 相关性排序使用的 BM25 词频饱和参数 `k1`。取值越大，重复词项对得分的提升越持续。必须为有限的非负数。
* **默认值**：1.2
* **类型**：Double
* **引入版本**：-
...
* **描述**：向量查询在使用 Tablet 内并行扫描时，切分点是否落在 Segment 边界上。向量索引以 Segment 为单位检索，因此切分点落在 Segment 内部时，切出的每个子任务都会在各自的行范围上把整个索引重新检索一遍。该变量约束的是切分点的位置，不改变采用哪种切分方式，也不影响不使用向量索引的查询。设置为 `false` 会退回按行数切分，索引检索次数随子任务数成倍增加。
* **默认值**：`true`
* **数据类型**：Boolean
* **作用域**：Session
