...
- 数据分布：在数据量大且 GROUP BY 的维度列基数高的情况下，数据计算将使用更多内存。不建议在此情况下使用 HLL。建议在不使用 GROUP BY 的 count distinct 或对低基数维度列进行 GROUP BY 时使用。
...
3. 通过以下三种方式聚合数据：（不进行聚合时，直接查询基表的速度可能与使用 approx_count_distinct 一样慢）
   ...
   -- c. 创建另一张表来计算 UV。插入数据并通过 hll_hash 对其他列进行测试以生成 HLL 列。
