...
提供商和模型必须支持传入的选项，包括 `dimensions`。如需选择已注册的 AI provider 而非 SYSTEM 配置，请使用 [ai_custom_embedding](ai_custom_functions.md)。设置默认 AI provider 不改变 `ai_embed` 的路由；显式 `model` 参数仍选择远端模型名。
