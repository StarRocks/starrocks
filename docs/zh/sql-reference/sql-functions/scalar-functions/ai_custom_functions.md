...
语句在规划时统一捕获所引用的 provider，所有 AI 执行节点使用这些捕获值。后续 ALTER 或 DROP 不改变正在执行的计划；新查询或下一次预处理 EXECUTE 会捕获当前元数据。同名删除重建会产生不同的 provider 身份。删除 provider 不会取消正在执行的查询。
...
已有的按类型默认 provider 机制保持不变。设置默认 provider 不会改变 SYSTEM `ai_complete`、[文本辅助函数](ai_functions.md) 或 [ai_embed](ai_embed.md) 的路由。它们原有的重载、FE 配置和 BE 本地凭证保持不变；SYSTEM 显式 `model` 参数仍表示远端模型名。
