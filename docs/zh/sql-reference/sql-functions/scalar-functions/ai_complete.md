...
- `model`：为本次调用选择模型的 VARCHAR 表达式。省略时，StarRocks 使用
  [`ai_default_chat_model`](../../../administration/management/FE_parameters/user_query_loading.md#ai_default_chat_model)。显式模型可以逐行变化，但常量模型不能是空字符串或仅包含空白字符。
...
| 参数 | 默认值 | 要求 |
|------|--------|------|
| [`ai_default_chat_endpoint`](../../../administration/management/FE_parameters/user_query_loading.md#ai_default_chat_endpoint) | 空字符串 | 必填。聊天补全端点的完整 HTTPS POST URL。 |
| [`ai_default_chat_model`](../../../administration/management/FE_parameters/user_query_loading.md#ai_default_chat_model) | 空字符串 | 仅传入 prompt 的重载必须配置；如果每次调用都显式指定模型，则可不配置。 |
| [`ai_default_chat_provider`](../../../administration/management/FE_parameters/user_query_loading.md#ai_default_chat_provider) | 空字符串 | 必填。唯一有效值为 `openai_compatible`。 |
...
- 独立 task timeout 在 task 创建后保持不变，重试不会重新计时；Query 取消和 deadline 更新会在整个异步执行期间持续生效。超时、工作线程和调度粒度控制参见其他
  [BE AI 函数配置项](../../../administration/management/BE_parameters/query_loading.md#ai_function_request_timeout_ms)。
