...
按升序对数组元素进行排序。从 v4.1 开始，`array_sort` 支持 Lambda 比较器。
...
`lambda_comparator`：一个可选的 lambda 函数比较器，格式为 `(x,y)->expr(x,y)`。lambda 比较器：
...
- 当不使用 lambda 比较器时，此函数按升序对数组元素进行排序。
- `NULL` 值放置在返回数组的开头。
- 返回数组的元素与输入数组的元素具有相同的数据类型。

### Lambda 比较器排序

- 使用 lambda 比较器时，排序顺序由表达式结果决定：
  - 对于布尔返回值：`true` 表示 x 应在 y 之前
  - 对于数字返回值：负数表示 x 在 y 之前

- lambda 比较器必须满足严格弱序要求：
