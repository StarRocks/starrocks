...
自 4.1 版本起，StarRocks 支持创建 View Function（视图函数），允许用户使用 SQL 表达式创建函数。

View Function 是一种轻量级的函数定义方式，它将一个 SQL 表达式封装为可重用的函数，在查询时动态展开为实际的 SQL 表达式。
...
View Function 可以与 StarRocks 内置函数自由组合：
...
View Function 在查询优化阶段被展开为实际的 SQL 表达式，具有以下性能特点：
