...
输入可以是来自任何源的紧凑 theta 草图列：通过 [`ds_theta_accumulate`](./ds_theta_accumulate.md) 构建、由成对标量集合运算 [`ds_theta_union`](../scalar-functions/ds_theta_union.md) / [`ds_theta_intersect`](../scalar-functions/ds_theta_intersect.md) / [`ds_theta_a_not_b`](../scalar-functions/ds_theta_a_not_b.md) 产生，或从 Parquet/Iceberg 作为原始 `VARBINARY` 加载的外部草图。
