...
- 对名称的要求，包括数据目录名 (catalog)、数据库名、表名、视图名、异步物化视图名、分区名、列名、索引名、用户名、角色名、仓库名 (repository)、资源名 (resource)、存储卷名 (Storage Volume)、Pipe 名等：
  ...
  - 通过 FE 配置项 `enable_table_name_case_insensitive`，可以控制数据目录名、数据库名、表名、视图名、异步物化视图名是否大小写**不敏感**。当前默认对表名大小写**敏感**。
    - 开启该配置后，相关名称将统一按照小写存储，所有SQL命令中涉及的上述名称都会自动转为小写进行处理。
    - 此功能仅支持新集群开启，且新集群首次启动时会记录该配置项状态。**集群后续的重启、升降级、切主等操作均不会改变该属性**。
    - 该配置项一旦记录，无法通过任何命令修改，修改时会提示错误。
    - 如果 Leader 节点启动时，FE 配置文件中的该属性与集群首次启动时记录的状态不一致，则该 FE 节点会启动失败。
    - 此功能暂时不支持对 JDBC Catalog 和 JDBC 外表的名称大小写不敏感处理。对于有 JDBC/ODBC 等相关需求的用户，禁止开启该功能。
