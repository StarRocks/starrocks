...
描述：所有计算节点中，该 Operator 的峰值内存。该指标仅对于部分物化算子有意义，例如聚合、排序、Join 等。而对于 Project、Scan 等算子无意义，因为内存在当前算子分配，在后续算子释放，对于当前算子来说，峰值内存就等同于累计分配的内存。在 v3.1.7（包含），v3.2.2（包含） 及之前版本中，该指标的含义是：所有 PipelineDriver 中，该 Operator 的峰值内存。
