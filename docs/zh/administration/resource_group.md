...
- `spill_mem_limit_threshold`: 资源组在当前 BE 节点触发落盘的内存占用阈值（百分比）。取值范围：(0,1)，默认值为 1，即不生效。该参数自 v3.1.7 版本引入。
  ...
  - 如果开启自动落盘功能且查询命中资源组（包括所有系统内建资源组）后，该查询使用的内存超过以下任意一项，都将触发中间结果落盘：
    - 当前资源组内所有查询使用的内存超过 `当前 BE 节点内存上限 * mem_limit * spill_mem_limit_threshold` 时
    - 当前查询占用 `query_mem_limit` 的 80% 时
