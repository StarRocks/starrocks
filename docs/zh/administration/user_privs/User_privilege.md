...
StarRocks 支持使用用户密码登录或外部认证作（LDAP、OIDC、OAuth 2.0）为用户认证方式。有关 StarRocks 认证方式的更多信息，请参阅 [用户认证](./auth/ldap_authentication.md)。有关创建用户的更多操作说明，请参阅 [CREATE USER](../../sql-reference/sql-statements/account-management/CREATE_USER.md)。
