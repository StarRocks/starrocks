...
创建用户时，通过 `IDENTIFIED WITH authentication_jwt [AS '{xxx}']` 指定认证方法为 JWT。`{xxx}` 是用户的 JWT 属性。您可以在 FE 配置文件中配置默认的 JWT 属性，您需要手动修改所有配置文件并重启所有 FE 使之生效。配置默认属性后，当您省略 [AS '{xxx}'] 部分时，StarRocks 会使用您配置的默认属性。
