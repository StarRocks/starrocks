...
迁移 Range 分布表之前，请确认源集群和目标集群的部署版本都包含 FE Range 拓扑及精确边界分裂桥接功能，以及以下两个存算分离服务端修复：

- 旧数据版本元数据文件不存在时，分裂后的子 Tablet 可以使用当前元数据及其数据版本作为 UUID 文件去重基线。
- 共享对象和捆绑 Segment 只复制一次，物理路径正确，并保留其逻辑偏移量和大小。

只有部署版本包含上述全部依赖后，才能进行端到端验证或生产迁移。
...
此流程仅支持通过分裂收敛。如果收敛需要目标集群执行合并，流程将以失败关闭方式停止，包括源集群发生合并、目标集群存在独有边界、Range 交叉或目标拓扑比源拓扑更细的情况。Range Colocate 布局也不受支持。目标集群启用透明数据加密时，不支持首次复制由分裂后多个 Range Tablet 共享的物理文件；目标集群上已存在的文件仍可复用其现有加密元数据。
