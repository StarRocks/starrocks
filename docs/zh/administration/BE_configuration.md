...
- 含义：存算分离集群下，主键表 Compaction 任务中允许的最大输入 Rowset 数量。从 v3.2.4 和 v3.1.10 版本开始，该参数默认值从 `5` 改为 `1000`。存算分离主键表在新的 Sized-tiered Compaction 策略下，不需要通过限制每次 Compaction 的 Rowset 个数来降低写放大，因此调大该值。
- 默认值：1000
- 引入版本：v3.1.8、v3.2.3
