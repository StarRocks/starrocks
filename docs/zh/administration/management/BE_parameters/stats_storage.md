...
- 单位：-
- 是否动态：是
- 描述：是否将已加载字典的 ZSTD 解压上下文保留在一个小的线程局部集合中，而不是从共享上下文池中借用。从共享池借用的上下文在归还时会被重置，这会清除粘性的 `refDDict`，导致每读一个页都要把字典重新加载到一个冷上下文中。该参数仅影响 Segment 中带 ZSTD 压缩字典的列的读取，其他列的读路径不受影响。以开关形式暴露，便于在生产环境中无需回滚即可关闭该优化。
- 引入版本：v4.2
