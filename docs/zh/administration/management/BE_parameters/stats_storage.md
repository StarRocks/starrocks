...
- 单位：-
- 是否动态：是
- 描述：是否将 BinaryPlainPage（用于 string/varchar 列以及字典页）页尾的偏移数组以逐值增量（即字符串长度）方式存储，而非绝对偏移。绝对偏移单调递增，在 LZ4 下几乎压不动；增量长度对于长度接近固定的字符串近乎常数，压缩效果好得多，而未压缩的页尾大小保持不变。压缩后列大小的减少量约等于偏移页尾的大小（每行约 4 字节），对高基数字符串列更明显。增量页通过页尾中的一个标志位自描述，因此开启前写入的页（绝对偏移）保持原有的零拷贝读取路径，并且在开关切换后仍可正常读取。该配置仅影响写入侧；读取侧按页自动识别格式。
- 引入版本：v4.2.0
