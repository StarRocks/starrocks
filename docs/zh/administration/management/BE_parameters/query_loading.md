...
- 单位：MB/s
- 是否动态：是
- 描述：实验性。落盘目标的有效写吞吐，供列级 codec 选择器（`spill_enable_codec_selector`）在编解码 CPU 开销与节省的磁盘字节之间权衡。它是一个策略输入，而非实测值：其含义是「1 纳秒的编解码 CPU 折合多少字节的 I/O」。填低于真实吞吐的值会提高磁盘空间的权重——例如落盘卷接近容量上限时填 `20`，用 CPU 换更小的落盘体积；填高则偏向轻量编码、节省 CPU。

  该值只需量级正确，且曲线是不对称的：填低会过度压缩，但多花的 CPU 仍换回真实的 I/O 节省；填高则停止压缩，既丢掉 I/O 节省又照付采样成本。最优取值取决于本次落盘是大到真实走盘、还是小到能被页缓存吸收。落盘压缩基准实测（20 个数据集、缓冲写），以「相对不压缩节省的落盘时间比例」计：1 GB 落盘（被页缓存吸收）时 `50`/`100`/`200` 分别节省 32.5%/34.0%/34.6%；8 GB 落盘（真实走盘）时分别节省 45.6%/44.2%/40.6%，`800` 节省 34.6%。默认值 `100` 在两种情形下都与各自最优值相差约 1.5 个点以内。填低的代价小于填高：过度压缩仍换回真实的 I/O 节省，而压缩不足既丢掉节省又照付采样成本。
- 引入版本：-
...
- 单位：-
- 是否动态：是
- 描述：实验性。启用可插拔的落盘列级 codec 选择器（v2 落盘序列化格式），在落盘时按列自适应选择编码。为 `false` 时，落盘沿用旧的 `spill_encode_level` 驱动的编码，行为不变。
- 引入版本：-
