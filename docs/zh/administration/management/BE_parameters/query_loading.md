...
- 单位：行
- 是否动态：是
- 描述：由聚合探测和哈希连接探测共用的软件预取距离（以行数为单位）：探测会提前指定行数预取数据，以在大型哈希表上隐藏内存延迟。设置为 `0` 将禁用软件预取。默认值 16 适用于大多数工作负载；当哈希表大到溢出 DRAM 时可调高，此时更长的前瞻能隐藏更多延迟。调低没有实际效果，因为可驻留缓存的小表已被 L2 比率排除。每条路径还通过各自的比率（`agg_prefetch_l2_ratio`、`join_probe_prefetch_l2_ratio`）根据 L2 驻留情况进行门控。
- 引入版本：-
...
- 单位：-
- 是否动态：是
- 描述：以 L2 缓存大小的比例控制哈希连接探测在何时启用软件预取：仅当连接哈希表增长超过 `join_probe_prefetch_l2_ratio × L2` 时才启用预取。对于较小的表，预取会拖慢探测，因此被禁用。降低该值可在更小的表上启用预取，提高该值则仅在更大的表上启用；`0` 表示始终预取。默认值 0.4 是首次测得预取产生净收益的临界点。负数或非有限比率会禁用预取。
- 引入版本：-
