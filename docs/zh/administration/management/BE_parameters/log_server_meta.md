...
- 单位：毫秒
- 是否动态：否
- 描述：内部堆中存在待处理定时任务时，bRPC `TimerThread` 在获得一次消费定时器桶并评估是否执行条件清理的机会之前的最长休眠时间。有效范围为 `[0, 2147483647]`。`0` 表示禁用周期性唤醒，恢复 bRPC 自身「休眠到最近任务运行时间」的行为；此时远早于其截止时间被取消调度的任务会一直占用槽位直到该截止时间。正值可为截止时间很远的定时器提供更频繁的回收机会，但由于仍需满足清理阈值和堆增长条件，因此不保证在所配置的间隔内完成任务槽位回收。该设置在进程级别生效，因此较小的正值可能增加所有 bRPC `TimerThread` 实例的唤醒次数和 CPU 使用量。
- 引入版本：-
