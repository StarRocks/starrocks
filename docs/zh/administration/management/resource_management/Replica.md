...
管理 StarRocks 集群中的数据副本。

本文含两个部分——[存算一体](#存算一体)和[存算分离](#存算分离)。

## 存算一体

### 概述

对于存算一体集群中的内表，StarRocks 采用多副本策略来保证数据的高可用性。当您创建表时，必须通过配置 `replication_num` （默认值：`3`）属性（Property）指定表的副本数量。发起导入事务后，数据会同时导入到指定数量的副本中，即在数据写入多数副本后，事务才会返回成功。详情请参考[导入数据安全等级（Write quorum）](#导入数据安全等级write-quorum)。StarRocks 允许您设置导入更少的副本即可返回导入成功，从而实现更好的导入性能。
...
从 v4.1 起，StarRocks 支持在存算分离集群中对云原生表的数据副本进行修复。

### 概述

在存算分离架构中，为了降低存储成本，数据通常以**单副本模式**存储在远端存储系统中，例如对象存储或 HDFS。与传统的存算一体架构不同，该设计无法依赖多副本机制在数据文件丢失时自动完成恢复。

因此，当 FE 当前生效的元数据版本引用了远端存储中已不存在的元数据文件或数据文件时，数据导入和查询操作将因 “File not found” 错误而失败，严重情况下可能导致业务不可用。

文件丢失通常可能由以下原因引起：

- **误删除**：由于运维操作失误，误删了对象存储中的文件。
- **一致性问题**：在极端情况下，存储系统可能发生延迟一致或元数据丢失。
- **软件缺陷**：系统 Bug 导致文件被提前清理。

传统基于快照的恢复机制通常耗时较长、成本较高，不适合用于生产环境中的快速恢复。

为此，StarRocks 提供了一种**低成本、秒级恢复**的修复机制。系统通过扫描历史元数据版本，识别最近一个所有相关文件均完整存在的健康版本，并将 Tablet 的元数据回滚至该版本。该机制通过牺牲少量最新数据，快速恢复表的可用性。

#### 工作机制

该功能是对存算分离集群中云原生表的 `ADMIN REPAIR TABLE` 语句的扩展，主要包括以下机制：

1. **自动探测**

   FE 协调 CN，以倒序并分批的方式探测 Tablet 的历史元数据版本。

2. **确定性路径推导**

   元数据文件路径可通过确定性规则直接推导，无需执行代价高且低效的对象存储 List Objects 操作。

3. **多策略恢复决策**

   支持两种恢复策略：**严格一致性（Strict Consistency）** 和 **最大可用性（Maximum Availability）**，以满足不同业务需求。

4. **元数据重置能力**

   当 Tablet 的所有历史元数据均不可用时，系统支持创建空 Tablet（Empty Tablet Recovery），以避免少量损坏的 Tablet 阻塞整个表的可用性。

#### 核心优势

- **极速恢复**

  仅修改 KB/MB 级别的元数据，无需任何数据拷贝操作，即使是 PB 级表也可实现秒级恢复。

- **运维成本低**

  无需额外副本，也不依赖高成本的对象存储 API 调用。

### 检查 Tablet 状态

在执行修复前，可以通过 [ADMIN SHOW TABLET STATUS](../../../sql-reference/sql-statements/cluster-management/tablet_replica/ADMIN_SHOW_TABLET_STATUS.md) 语句查看存算分离表或云原生物化视图中各 Tablet 的文件完整性，确认是否存在元数据或数据文件缺失。

**仅存算分离集群的云原生表和云原生物化视图支持此语句。**
...
| Status               | Tablet 状态，取值见上方状态说明。                                        |
| MissingDataFileCount | 缺失的数据文件数量。`Status` 为 `MISSING_DATA` 时有值。                    |
| MissingDataFiles     | 缺失的数据文件列表。受 `max_missing_data_files_to_show` 限制，默认展示最多 5 个。 |
...
使用带 `PROPERTIES` 的 `ADMIN REPAIR TABLE` 语句控制恢复行为。
...
| 参数名                         | 类型      | 默认值   | 说明                                                                                                                                                                                                       |
| --------------------------- | ------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| enforce_consistent_version  | Boolean | True  | 是否强制分区内所有分片回滚至一致版本。若设置为`true`，系统将在历史中搜索对所有分片有效的版本进行回滚，确保分区内数据版本一致。若设置为`false`，则允许分区内每个分片回滚至其最新可用有效版本。不同分片版本可能不一致，但此方式可最大化数据保留率。 |
| allow_empty_tablet_recovery | Boolean | False | 是否允许通过创建空分片进行恢复。该项仅在 `enforce_consistent_version` 为 `false` 时生效。若设置为 `true`，当某些分片的全部版本元数据缺失但至少存在一个分片的有效元数据时，系统将尝试创建空分片来填补缺失版本。若所有分片的所有版本元数据均丢失，则无法进行恢复。 |
| dry_run                     | Boolean | False | 是否仅返回修复计划而不实际执行修复。若设置为 `true`，系统将评估每个分区的可修复性并返回修复计划，但不会执行任何实际的元数据回滚操作。可用于在正式修复前预览修复效果。详见[预览修复计划（Dry Run）](#预览修复计划dry-run)。 |

### 示例

#### 示例 1：严格一致性恢复（推荐用于强一致性场景）

将分区 `p20250101` 中的所有 Tablet 恢复到最近一个完整且一致的版本。
...
允许分区 `p20250101` 中的每个 Tablet 回滚到其各自最近的有效版本，即使不同 Tablet 之间版本不一致。
...
当部分 Tablet 的元数据完全缺失时，允许将其恢复为空 Tablet。
...
在正式执行修复前，您可以使用 `dry_run` 模式预览修复计划，了解每个分区的可修复性和各分片将回滚到的版本，而不会实际修改任何元数据。

**仅存算分离集群的云原生表支持 `dry_run` 模式。**
...
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| PartitionId      | 分区 ID。                                                                                                                                       |
| VisibleVersion   | 当前可见版本号。                                                                                                                                   |
| RepairStatus     | 修复状态：`NORMAL`（所有分片正常，无需修复）、`RECOVERABLE`（存在丢失文件但可修复）、`UNRECOVERABLE`（存在丢失文件且无法修复）、`UNKNOWN`（探测过程中发生异常）。 |
| TabletRecoverInfo | JSON 数组，列出每个分片将回滚到的版本号。仅在 `RepairStatus` 为 `RECOVERABLE` 时有值。                                                                 |
| ErrorMsg         | 错误信息。仅在 `RepairStatus` 为 `UNRECOVERABLE` 或 `UNKNOWN` 时有值。                                                                              |
...
确认修复计划无误后，去掉 `dry_run` 参数（或将其设置为 `false`）执行实际修复。

### 限制与建议

- 存算分离集群的副本修复功能从 v3.5 开始支持。
- 当前仅存算分离集群的云原生表支持通过 `PROPERTIES` 配置恢复策略。
- 在对物化视图执行恢复操作时，请注意：
  - 异步物化视图在恢复后需要手动刷新。
  - 同步物化视图必须使用**严格一致性恢复**，以确保基表与 Rollup 数据的一致性。
- 恢复过程为尽力而为（Best-effort）。根据可用的历史版本以及所选恢复策略，可能会丢失少量最新数据。
