...
- 单位：字节
- 类型：累积
- 标签：`delete_type` (`position`、`deletion_vector` 或 `metadata`)
...
- 类型：累积
- 标签：`delete_type` (`position`、`deletion_vector` 或 `metadata`)
- 描述：Iceberg `DELETE` 任务的总执行时间（毫秒）。每个任务的持续时间在其结束后添加。`delete_type` 区分删除方法。
...
- 标签：
  ...
  - `delete_type` (`position`、`deletion_vector` 或 `metadata`)
- 描述：针对 Iceberg 表的 `DELETE` 任务总数。无论任务成功或失败，每当任务结束时，该指标都会增加 1。`delete_type` 区分删除方法：`position`（生成位置删除文件）、`deletion_vector`（写入 Iceberg V3 deletion vector）和 `metadata`（元数据级别删除）。
