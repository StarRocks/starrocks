...
- 单位: -
- 是否可变: Yes
- 描述: 控制用于授权物化视图后台操作的用户身份，涵盖刷新任务以及构建用于查询改写的执行计划缓存。当设置为 `true` 时（默认值，即基于创建者的授权），这些操作以物化视图创建者的身份进行授权，并激活该创建者的所有角色。当设置为 `false` 时（即基于 root 的授权），这些操作以 `'root'@'%'` 的身份进行授权，即物化视图的执行计划将以 ROOT 权限构建。若需兼容使用 LDAP 认证的集群，可将其设置为 `false`。
- 引入版本: v3.5.5
