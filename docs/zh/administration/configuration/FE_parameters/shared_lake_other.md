...
- 单位: -
- 是否可变: Yes
- 描述: 对于按 `date_trunc(granularity, col)`（粒度为 `day` 或更粗）分区的范围物化视图，是否镜像基表实际（可能为混合粒度）的分区边界，而不是将更粗的基表分区展开为固定粒度的单元格。启用后，已合并为月的历史基表分区将保留为一个月的物化视图分区，而近期的天分区仍保留为天分区，从而每个基表分区对应一个物化视图分区。对于宽度恰好为一个粒度单元的基表分区（包括离散和多 union 场景），该行为与默认行为一致。对于不对齐的输入会回退到默认映射器。
- 引入版本: -
