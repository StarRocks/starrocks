...
- 单位: -
- 是否可变: Yes
- 描述: FE 在绑定服务端口之前，是否等待启动期的物化视图激活任务执行完毕。这些任务会占满共享 plan cache 线程池的全部线程，因此设置为 `false` 时，FE 会在该线程池上排队的其他工作（包括 plan cache 构建）仍被阻塞在激活任务之后时就开放端口。设置为 `true` 时，正在重启的 FE 会等到启动期激活执行完毕后才接受连接，这些构建随后即可以完整并发度运行。请注意该等待仅覆盖激活：plan cache 构建是单独排队的，不在等待范围内，因此端口开放后立即发起的查询，仍可能遇到 plan 尚未构建完成的物化视图。该参数是启动就绪门的 kill switch，而非用于灰度启用该功能的开关。该参数虽然可以动态修改，但 FE 只在启动路径上读取一次：运行时修改不影响当前进程，只在下次重启时生效。当 `enable_mv_post_image_reload_cache` 设置为 `false` 时，该参数不生效，因为此时物化视图 reload 完全同步执行，不会提交任何异步任务供门等待。
- 引入版本: v4.2.0
...
- 单位: 毫秒
- 是否可变: Yes
- 描述: 下一条 gtid 将越过当前批次上界时，FE 向前预留的毫秒窗口。小于 `1` 的值按 `1` 处理。窗口还必须落在 snowflake 剩余的 42 位时间戳内；再大则发号失败，而不会把预留上界算溢出。合法范围内值越大，元数据 journal 写入越少。副作用是切主时未用完的预留作废，gtid 的时间戳最多向前跳这整个窗口。
- 引入版本: -
...
- 单位: 毫秒
- 是否可变: Yes
- 描述: 集群级别的 bookmark reference 单次租约时长上限。无论 reference 自身的 TTL 如何，清理任务都会在租约超过该上限后回收 reference。`<= 0` 表示不启用该上限。reference 的实际租约取该上限与其自身 TTL 中较小的一个，任一侧 `<= 0` 均按不限处理，因此自身不设 TTL 的 reference 同样受该上限约束。起算点为最近一次续租（从未续租则为 acquire 时间）——持续续租的持有者可以让 reference 一直存活。由于清理任务每轮都会重新读取该配置，调低它会缩短**已经授予**的租约：距上次续租的时长一旦达到新上限，reference 立即被回收，续租间隔长于该上限的持有者会在下次续租之前就丢掉 bookmark。`bookmark_renew` 的返回值只在该次调用的时刻成立。由于该上限从最后一次续约开始计时，它同时决定了一个不再续约的 bookmark 最多能在回收站中保留被替代(reshard 之前)的 index generation 多久：reference 被回收后，该 generation 即可被清除，因此 bookmark 持有者泄漏或物化视图停止刷新都不会无限期占住 shard。
- 引入版本:
...
- 单位: -
- 是否可变: Yes
- 描述: 是否启用 bookmark 元数据函数(`bookmark_create` / `bookmark_renew` / `bookmark_release`)。这些函数仅用于调试/测试，默认关闭，需要 OPERATE 权限且只能在 leader 节点上执行。注意：`holder` 参数只是调用方自报的标签，并非经过鉴权的身份——任何能调用这些函数的人都可以释放、或以更短的 TTL 重新租用他人创建的引用，而 `HOLDER_ID` 可从 `information_schema.table_bookmark_references` 读到。
- 引入版本:
