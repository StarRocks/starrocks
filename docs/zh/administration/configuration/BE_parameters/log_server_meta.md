...
- 单位：-
- 是否动态：否
- 描述：bRPC `TimerThread` 对内部堆中的已取消调度任务执行清理时所需的最小任务数。有效范围为 `[1, 2147483647]`。为了均摊扫描开销，bRPC 还会等待堆增长到当前清理基线的约两倍。该基线会在清理后重置，并随着任务执行导致的堆缩小而降低。清理会将任务槽位归还给 bRPC 池以供复用，有助于限制内存继续增长，但不保证进程 RSS 会降低。增大该值可降低清理频率，但可能保留更多临时内存。
- 引入版本：-
...
- 单位：毫秒
- 是否动态：否
- 描述：内部堆中存在待处理定时任务时，bRPC `TimerThread` 在获得一次消费定时器桶并评估是否执行条件清理的机会之前的最长休眠时间。有效范围为 `[0, 2147483647]`。`0` 表示禁用周期性唤醒，并保持休眠到最近任务运行时间的默认行为。正值可为截止时间很远的定时器提供更频繁的回收机会，但由于仍需满足清理阈值和堆增长条件，因此不保证在所配置的间隔内完成任务槽位回收。该设置在进程级别生效，因此较小的正值可能增加所有 bRPC `TimerThread` 实例的唤醒次数和 CPU 使用量。
- 引入版本：-
