...
- 单位：-
- 是否动态：否
- 描述：系统日志分卷的模式。有效值包括 `TIME-DAY`、`TIME-HOUR` 和 `SIZE-MB-` 大小。默认值表示日志被分割成大小为 1GB 的日志卷。若配置值无法解析，BE 会在日志中报告该值并改用默认值。
- 引入版本：-
...
- 单位：Bytes
- 是否动态：是
- 描述：RPC payload 压缩允许的最大输入大小。如果序列化后的 chunk 大小大于或等于该值，StarRocks 会将该 payload 视为不适合进行 RPC 压缩。具体行为由 `enable_rpc_compress_overflow_skip` 控制：为 `true` 时跳过压缩并以未压缩形式发送；为 `false` 时返回 `InternalError`。将该值设置为 0 或负数可禁用该 RPC 策略上限。
- 引入版本：-
...
- 单位：-
- 是否动态：是
- 描述：控制序列化的 chunk 大小达到 `rpc_compress_max_input_size` 或超过压缩 codec 允许的最大输入大小时的行为。设置为 `true`（默认）时，StarRocks 跳过压缩并以未压缩形式发送数据；如果启用了对应的 verbose row 级别日志，可能会记录跳过压缩的信息。设置为 `false` 时，StarRocks 返回 `InternalError` 并中止 RPC。
- 引入版本：-
...
- 单位：-
- 是否动态：是
- 描述：ThreadPool 的工作线程是否吞掉任务抛出的异常并继续执行下一个任务。设置为 `false`（默认）时，任务体外层没有 catch 语句，任务抛出的异常找不到处理者，会在抛出点终止 BE 进程。设置为 `true` 时，异常以 ERROR 级别记入日志，进程级指标 [`threadpool_task_exception_total`](../../management/monitoring/metric_details/t-z.md#threadpool_task_exception_total) 加一，工作线程继续执行下一个任务，进程得以存活。注意工作线程存活并不意味着任务是异常安全的：如果任务通过 `DeferOp` 或析构函数发出完成信号，而记录结果的语句被跳过，等待方会把该任务读作成功，因为未赋值的 `Status` 读出来就是 OK。此时故障不会报错，而是产生错误的结果。仅在需要缓解崩溃循环时将该项设置为 `true`，并预期相关故障会变为静默。
- 引入版本：v4.2.0
