...
- 单位：-
- 是否动态：是
- 描述：是否为字符串（CHAR/VARCHAR）列启用基于前缀的 Zonemap 索引。对于非键列，最小值/最大值会截断到由 `string_prefix_zonemap_prefix_len` 配置的前缀长度。如果 CHAR/VARCHAR 列中存在任一超过 1 MiB 的值，无论该配置如何设置，StarRocks 都不会为整个 Segment 中的该列写入 ZoneMap。
- 引入版本：-
...
- 单位：毫秒
- 是否动态：是
- 描述：每个同步缓存调用方等待正在进行的向量索引加载完成的最长时间。等待超时后，该调用方会收到缓存未命中，使查询路径可以回退到暴力检索。已经开始的加载不会被取消，而是继续在后台执行。后续每个调用方都会按配置值重新开始自己的等待。小于或等于 `0` 时不等待。运行时修改对之后开始的等待生效。
- 引入版本：v4.2.0
...
- 单位：-
- 是否动态：是
- 描述：是否通过精确暴力计算避免 top-k 向量检索返回结果不足。设置为 `true` 时，如果谓词或运行时过滤器在 Segment 级 ANN 检索之后执行，StarRocks 会直接改走精确计算；如果带预过滤条件的 ANN 检索返回行数小于 `min(k, 匹配候选数)`，StarRocks 会对匹配候选项进行精确重算。设置为 `false` 时，即使部分谓词或运行时过滤器只能在 ANN 候选生成后执行，StarRocks 仍保留 ANN 物理执行路径，并且不会触发基于结果数量的精确重算，因此最终结果可能少于 `k` 行。基于结果数量的回退仅作用于 top-k 检索，不作用于范围检索，因为范围检索返回较少结果可能表示没有更多候选项满足指定半径。
- 引入版本：-
