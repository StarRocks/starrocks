...
- 单位：-
- 是否动态：是
- 描述：Segment 写入时，是否把 short key index 和所有列的 ordinal index 放在紧邻 segment footer 之前的一段连续区域内，而不是把每列的 ordinal index 写在该列数据页之后。页级 zone map 仍放在列数据旁，谓词扫描读取 zone map 和数据时可以复用同一个 Data Cache block。ordinal index 紧邻 footer，可以复用解析 footer 时读入的 Data Cache block，减少冷查对象存储时的离散读取。该配置仅影响写入侧。所有会写出 segment footer 的路径都会生成该区域，包括纵向 Compaction 和部分列更新重写。两种布局都能被任意版本的 BE/CN 读取，并可在同一张表中共存。
- 引入版本：v4.2.0
