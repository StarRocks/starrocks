...
StarRocks 主要由两种类型的组件组成：FE 节点和 BE/CN 节点。每个节点必须单独部署在物理机或虚拟机上。
...
CN 节点负责存算分离集群中的数据缓存和 SQL 执行。

您可以通过增加 CN 节点数量以弹性扩展计算资源。

## CPU 和内存

通常，FE 服务不会消耗大量的 CPU 和内存资源。建议您为每个 FE 节点分配 8 个 CPU 内核和 16 GB RAM。

与 FE 服务不同，如果您的应用程序需要在大型数据集上处理高度并发或复杂的查询，BE/CN 服务可能会使用大量 CPU 和内存资源。因此，建议您为每个 BE/CN 节点分配 16 个 CPU 内核和 64 GB RAM。
...
存算分离集群中的原始数据存储在远程存储中。您可以根据需要随时扩展存储空间。

CN 节点的本地磁盘用于缓存热数据，以加速查询。您可以根据日常业务场景中的热数据量来估算所需的本地磁盘空间。
