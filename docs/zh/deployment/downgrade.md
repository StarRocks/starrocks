...
- **重大版本降级**
  ...
  您只能将 StarRocks v4.1 集群降级至 v4.0.5 及更高版本。

  :::warning

  **降级说明**

  - 将 StarRocks 升级至 v4.1 后，请勿降级至 v4.0.5 之前的任何 v4.0 版本。

    由于 v4.1 引入了数据布局的内部变更（与 Tablet 分割和数据分布机制相关），升级至 v4.1 的集群生成的元数据和存储结构可能与早期版本不完全兼容。因此，从 v4.1 降级仅支持降至 v4.0.5 或更高版本。不支持降级至 v4.0.5 之前的版本。此限制源于早期版本在解析 Tablet 布局和分布元数据时的向后兼容性约束。
