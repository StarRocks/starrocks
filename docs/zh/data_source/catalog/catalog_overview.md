...
- **External catalog**: 外部数据目录，用于连接外部 metastore。在 StarRocks 中，您可以通过 external catalog 直接查询外部数据，无需进行数据导入或迁移。当前支持创建以下类型的 external catalog：
  - [Hive catalog](../catalog/hive_catalog.md)：用于查询 Hive 里的数据。
  - [Iceberg catalog](../catalog/iceberg_catalog.md)：用于查询 Iceberg 里的数据。
  - [Hudi catalog](../catalog/hudi_catalog.md)：用于查询 Hudi 里的数据。
  - [Delta Lake catalog](../catalog/deltalake_catalog.md)：用于查询 Delta Lake 里的数据。
  - [JDBC catalog](../catalog/jdbc_catalog.md)：用于查询 JDBC 数据源里的数据。
  - [Elasticsearch catalog](../catalog/elasticsearch_catalog.md)：用于查询 Elasticsearch 里的数据。该特性自 3.1 版本起支持。
  - [Paimon catalog](../catalog/paimon_catalog.md)：用于查询 Paimon 里的数据。该特性自 3.1 版本起支持。
