...
JDBC Catalog 自 3.0 版本开始支持 MySQL、PostgreSQL，自 3.2.9、3.3.1 版本开始支持 Oracle 和 SQLServer。自 3.3.0 开始支持 ClickHouse（试验性）。自 4.2 开始支持 BigQuery（试验性）。
...
| driver_class | JDBC 驱动程序的类名称。以下是常见数据库引擎支持的 JDBC 驱动程序类名称：<ul><li>MySQL：`com.mysql.jdbc.Driver`（MySQL 5.x 及之前版本）、`com.mysql.cj.jdbc.Driver`（MySQL 6.x 及之后版本）</li><li>PostgreSQL: `org.postgresql.Driver`</li><li>Oracle: `oracle.jdbc.driver.OracleDriver`</li><li>BigQuery: `com.simba.googlebigquery.jdbc.Driver` 或 `com.simba.googlebigquery.jdbc42.Driver`</li></ul> |
| schema_resolver | （可选）显式指定要使用的 Schema Resolver。有效值：`postgresql`、`mysql`、`oracle`、`sqlserver`、`clickhouse`、`bigquery`。当使用非标准 JDBC 驱动程序且无法通过驱动类名自动检测时，请使用此参数。如果未指定，StarRocks 将根据 `driver_class` 参数自动检测相应的 Resolver。 |
...
BigQuery 使用基于 OAuth 的认证方式，而非用户名/密码。认证参数直接在 `jdbc_uri` 字符串中传入。`user` 和 `password` 属性不使用，但必须出现在 DDL 中（设置为空字符串）。

Simba BigQuery JDBC 驱动程序不在 Maven Central 上提供。请从 [Google Cloud](https://cloud.google.com/bigquery/docs/reference/odbc-jdbc-drivers) 下载，并将其托管在 FE 和 BE/CN 节点可访问的 URL 上，然后将 `driver_url` 设置为该 URL。

常见 `jdbc_uri` 认证方式：

| OAuthType | 认证方式 | 额外参数 |
| :-------- | :------- | :------- |
| `0` | 服务账号密钥文件 | `OAuthServiceAcctEmail`、`OAuthPvtKeyPath` |
| `3` | 应用默认凭据 | 无 |

示例 URI：`jdbc:bigquery://https://www.googleapis.com/bigquery/v2:443;ProjectId=my-project;OAuthType=3`
...
自 v4.2 起，StarRocks 支持通过 [`native_query`](../../sql-reference/sql-functions/table-functions/native_query.md) 表函数，使用数据库原生 `SELECT` 语句查询 JDBC 数据。
