...
| DestID | 目标节点 ID。 |
| PartType | 数据分布模式，包括：UNPARTITIONED、RANDOM、HASH_PARTITIONED、BUCKET_SHUFFLE_HASH_PARTITIONED 和 CONNECTOR_SINK_SKEW_HASH_PARTITIONED。 |
| SerializeChunkTime | 序列化 chunk 所花费的时间。 |
| SerializedBytes | 序列化数据的大小。 |
| ShuffleChunkAppendCounter | 当 PartType 为 HASH_PARTITIONED、BUCKET_SHUFFLE_HASH_PARTITIONED 或 CONNECTOR_SINK_SKEW_HASH_PARTITIONED 时，Chunk Append 操作的次数。 |
| ShuffleChunkAppendTime | 当 PartType 为 HASH_PARTITIONED、BUCKET_SHUFFLE_HASH_PARTITIONED 或 CONNECTOR_SINK_SKEW_HASH_PARTITIONED 时，Chunk Append 操作所花费的时间。 |
| ShuffleHashTime | 当 PartType 为 HASH_PARTITIONED、BUCKET_SHUFFLE_HASH_PARTITIONED 或 CONNECTOR_SINK_SKEW_HASH_PARTITIONED 时，计算哈希所花费的时间。 |
...
| RpcCount | RPC 的总次数。 |
| SkewRebalancePassCount | 倾斜分区 rebalancer 实际执行的 rebalance pass 次数。仅当全局阈值 `connector_sink_skew_rebalance_min_data_processed` 跨越且真正跑了一次 pass 时才递增；门控未通过的轻量检查本身不计入。仅在 PartType 为 CONNECTOR_SINK_SKEW_HASH_PARTITIONED 时输出。 |
| SkewRebalanceSpreadEvents | rebalancer 真正将某个热分区的 task 分配扩展（加一个 task）的次数。均匀负载下保持为 0。仅在 PartType 为 CONNECTOR_SINK_SKEW_HASH_PARTITIONED 时输出。 |
| SkewRebalanceMaxAssignedTasks | 单个 logical partition 历史上被分配过的 writer task 最大数量。若没有发生 spread 则为 1。仅在 PartType 为 CONNECTOR_SINK_SKEW_HASH_PARTITIONED 时输出。 |
| SkewRebalanceSpreadPartitionCount | 历史上至少被扩展过一次（即新增过一个 task）的 logical partition 个数。仅在 PartType 为 CONNECTOR_SINK_SKEW_HASH_PARTITIONED 时输出。 |
| SkewRebalanceDataProcessed | rebalancer 在该 operator 实例上累计观测到的总字节数（单位：字节），用于核对驱动 rebalance 决策的实际数据量。仅在 PartType 为 CONNECTOR_SINK_SKEW_HASH_PARTITIONED 时输出。 |
