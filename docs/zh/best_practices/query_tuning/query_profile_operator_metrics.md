...
| RpcCount | RPC 的总次数。 |
| SkewRebalancePassCount | 倾斜分区 rebalancer 的 `rebalance()` 检查点调用总次数（每 chunk 一次）。大部分调用是 no-op，仅当累积字节跨过全局阈值 `connector_sink_skew_rebalance_min_data_processed` 时才会真正执行 rebalance pass。仅在 PartType 为 CONNECTOR_SINK_SKEW_HASH_PARTITIONED 时输出。 |
