...
| CompressTime | 压缩时间。 |
| CompressedInputBytes | 实际送入压缩器的序列化数据（压缩前）的大小。被自适应压缩策略跳过的 chunk 不计入其中。`CompressedBytes / CompressedInputBytes` 即为压缩率，`SerializedBytes - CompressedInputBytes` 即为未被压缩的数据大小。 |
