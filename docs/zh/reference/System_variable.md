...
是否开启资源组粒度的[查询队列](../administration/management/resource_management/resource_management.mdx)。
...
该变量的具体含义请参阅 [BE 配置项](../administration/management/management.mdx#配置-be-动态参数)中 `max_pushdown_conditions_per_column` 的说明。该变量默认值为 -1，表示使用 `be.conf` 中的配置值。如果设置大于 0，则忽略 `be.conf` 中的配置值。

### max_scan_key_num

该变量的具体含义请参阅 [BE 配置项](../administration/management/management.mdx#配置-be-动态参数)中 `max_scan_key_num` 的说明。该变量默认值为 -1，表示使用 `be.conf` 中的配置值。如果设置大于 0，则忽略 `be.conf` 中的配置值。
