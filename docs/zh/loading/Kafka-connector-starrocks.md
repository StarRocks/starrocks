...
- 相比于 Routine Load 仅支持导入 CSV、JSON、Avro 格式的格式数据，Kafka connector 支持导入更丰富的数据格式。只要数据能通过 Kafka connect 的 converters 转换成 JSON 和 CSV 格就可以通过 Kafka connector 导入，例如 Protobuf 格式的数据。
