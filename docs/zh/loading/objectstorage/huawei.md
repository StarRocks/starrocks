...
description: "从华为云 OBS 导入数据到 StarRocks，支持 Broker Load 和 INSERT 导入方式。"
---
...
另外，Broker Load 还支持在导入过程中做数据转换、以及通过 UPSERT 和 DELETE 操作实现数据变更。请参见[导入过程中实现数据转换](../Etl_in_loading.md)和[通过导入实现数据变更](../Load_to_Primary_Key_tables.md)。
...
从 3.1 版本起，StarRocks 新增支持使用 INSERT 语句和 `FILES` 关键字直接从 AWS S3 导入特定格式的数据文件，避免了需事先创建外部表的麻烦。参见 [INSERT > 通过 FILES 关键字直接导入外部数据文件](../InsertInto.md#通过-insert-into-select-以及表函数-files-导入外部数据文件)。
