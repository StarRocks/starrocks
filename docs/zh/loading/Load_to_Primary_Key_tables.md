...
主键模型还支持部分列更新（Partial Updates），并且针对不同的数据更新场景，提供了行模式和列模式两种部分列更新，在不影响查询性能的同时，尽可能地降低部分列更新的开销，从而能够保证更新的实时性。行模式比较适用于小批量的实时更新场景，列模式适用于大批量的批处理更新场景。

> **注意**
>
> 在部分更新模式下，如果要更新的行不存在，那么 StarRocks 会插入新的一行，并自动对缺失的列填充默认值。

以下以 CSV 格式的数据文件为例进行说明。
...
- 通过 Stream Load 导入：
  ...
  > - 如果需要使用列模式部分更新，则需要设置 `partial_update_mode` 为 `column`。
