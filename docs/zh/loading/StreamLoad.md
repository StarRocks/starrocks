...
通过 SHOW LOAD 语句或者 curl 命令来查看导入作业的进度。

有关 SHOW LOAD 的用法，参见 [SHOW LOAD](../sql-reference/sql-statements/data-manipulation/SHOW_LOAD.md)。
...
例如，可以通过如下命令查看 `mydatabase` 数据库中标签为 `label_local` 的导入作业的执行情况：

```Bash
curl --location-trusted -u <username>:<password> \
    'http://<fe_host>:<fe_http_port>/api/mydatabase/_load_info?label=label_local'
```

命令执行后，以 JSON 格式返回导入作业的结果信息 `jobInfo`，如下所示：

```JSON
{"jobInfo":{"dbName":"default_cluster:mydatabase","tblNames":["table1_simple"],"label":"label_local","state":"FINISHED","failMsg":"","trackingUrl":""},"status":"OK","msg":"Success"}%
```
