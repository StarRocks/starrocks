...
可动态修改的 FE 配置 `arrow_flight_proxy_backpressure_timeout_ms` 限制代理结果流连续等待下游客户端就绪的时间。默认值为 `300000` 毫秒（5 分钟），适用于 FE 到 BE 和 FE 到 FE 的代理结果流。每次等待下游就绪时重新计时；这不是查询或传输的总超时时间，也不包括读取上游数据的时间。如果客户端 RPC 的截止时间更早，该截止时间仍然有效。

等待超时后，代理会取消上游流、释放结果传输工作线程，并向客户端返回 `DEADLINE_EXCEEDED`。配置值必须为正整数，最大为 `2147483647` 毫秒。配置为零或负数时，新代理结果流将被拒绝并返回 `INTERNAL`，而不是禁用超时。配置修改无需重启 FE，对新打开的代理结果流生效；已有流继续使用启动时读取的配置值。
