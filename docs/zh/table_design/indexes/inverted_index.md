...
StarRocks 底层数据是按列存储在数据文件。每个数据文件会包含基于索引列的全文倒排索引。索引列的每一数据行（文本）会被分割为一个一个词，然后每个分词后的词语都会被视为一个索引项，与包含该词语的数据文件行号构成映射关系。目前支持的分词方式有英文分词、中文分词、多语言分词、不分词。
...
进行全文搜索时，StarRocks 会根据查询条件中的关键词，找到全文倒排索引中包含关键词的索引项，然后快速找到关键词所在数据文件行号。
