...
​
在 StarRocks中使用 SSB 100G 的测试数据集验证 Bitmap 索引加速查询的效果，测试结果如下：

- Bitmap 只有在高基数的场景下才会有比较明显的性能的提升（在此测试数据集中，基数达到 100000 数量级可以看到比较明显的性能提升）。
- 高基数的单列与多列组合的高基数都可以带来性能提升。
- 低基数的场景下，使用 Bitmap 索引基本没有性能提升甚至导致性能下降。
