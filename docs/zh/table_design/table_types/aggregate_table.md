...
建表时需要定义聚合键并且为 value 列指定聚合函数。当多条数据具有相同的聚合键时，value 列会进行聚合。并且支持单独定义排序键，如果查询的过滤条件包含排序键，则 StarRocks 能够快速地过滤数据，提高查询效率。
...
例如需要分析某一段时间内，来自不同城市的用户，访问不同网页的总次数。则可以将网页地址 `site_id`、日期 `date` 和城市代码 `city_code` 作为聚合键，将访问次数 `pv` 作为value 列，并为 value 列 `pv` 指定聚合函数为 SUM。
...
- 建表时，仅支持为 value 列创建 Bitmap 索引、Bloom filter 索引。
