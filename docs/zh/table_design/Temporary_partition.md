...
- 使用 `DROP` 命令直接删除数据库或表后，您可以在限定时间内通过 [RECOVER](../sql-reference/sql-statements/table_bucket_part_index/RECOVER.md) 命令恢复该数据库或表，但临时分区无法被恢复。
- 使用 `ALTER` 命令删除正式分区后，您可以在限定时间内通过 [RECOVER](../sql-reference/sql-statements/table_bucket_part_index/RECOVER.md) 命令恢复。正式分区与临时分区不相关联，操作正式分区不会对临时分区产生影响。
- 使用 `ALTER` 命令删除临时分区后，您无法通过 [RECOVER](../sql-reference/sql-statements/table_bucket_part_index/RECOVER.md) 命令恢复。
