...
- 调整 BE/CN 配置项 `starlet_use_star_cache` 默认值为 `true`，在存算分离模式下默认开启块缓存，如果升级前配置了 BE/CN 参数 `starlet_cache_evict_high_water` 为 X，需要同时配置 BE/CN 参数 `starlet_star_cache_disk_size_percent` 为 1.0-X（如果 X 是 0.3，那么 1.0-X 就是 0.7），确保文件缓存和块缓存使用的磁盘容量一致。[#38200](https://github.com/StarRocks/starrocks/pull/38200)
