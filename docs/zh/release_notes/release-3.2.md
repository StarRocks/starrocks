...
- 调整 BE/CN 配置项 `starlet_use_star_cache` 默认值为 `true`，即在存算分离模式下默认开启 Block Cache。如果您在升级前将 BE/CN 参数 `starlet_cache_evict_high_water` 配置为 `X`，则需要同时将 BE/CN 参数 `starlet_star_cache_disk_size_percent` 配置为 `1.0-X`。例如，如果您将 `starlet_cache_evict_high_water` 设置为 0.3，则还需要设置 `starlet_star_cache_disk_size_percent` 为 0.7），以确保文件缓存和 Block Cache 不会超过磁盘容量上限。[#38200](https://github.com/StarRocks/starrocks/pull/38200)
