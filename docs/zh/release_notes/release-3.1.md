...
- 新增支持主键表，暂不支持持久化索引。
- 支持自增列属性 [AUTO_INCREMENT](https://docs.starrocks.io/docs/sql-reference/sql-statements/table_bucket_part_index/auto_increment/)，提供表内全局唯一 ID，简化数据管理。
- 支持[导入时自动创建分区和使用分区表达式定义分区规则](https://docs.starrocks.io/docs/table_design/data_distribution/expression_partitioning/)，提高了分区创建的易用性和灵活性。
