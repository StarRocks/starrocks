...
- 通过 `seq -w 0 n` 拆分数据文件。
- 通过 `curl -XPOST http://be_host:http_port/api/update_config?streaming_load_max_mb=<file_size>` 调整 [BE 配置项](../../../administration/configuration/BE_parameters/BE_parameters.md) `streaming_load_max_mb` 的取值来扩大文件大小上限。
