...
## Running link testing

It is very difficult to test for broken links with tools other than
the built-in Docusaurus test. Unfortunately, this test uses more RAM
than we have on our laptops if we build the docs for all versions.

There is a second Dockerfile that loops over the branches (doc versions)
and runs the test on each branch.
