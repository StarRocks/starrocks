...
// The docs directory structure changed when the sidebar became autogenerated.
// Versioned docs built from branches that predate that change render this
// component without a `layout` prop, and must keep the pre-restructure URLs.
const LEGACY_URLS = {
...
function forLayout(list, layout) {
  if (layout === 'v2') return list;
  return list.map((f) => ({...f, url: LEGACY_URLS[f.url] ?? f.url}));
}
...
export default function Features({language, layout}) {
  if (language == "Chinese") {
    return (
      <section className={styles.features}>
        <div className="container">
          <div className="row">
            {forLayout(ChineseFeatureList, layout).map((props, idx) => (
    ...
    return (
      <section className={styles.features}>
        <div className="container">
          <div className="row">
            {forLayout(JapaneseFeatureList, layout).map((props, idx) => (
              <Feature key={idx} {...props} />
            ))}
          </div>
        </div>
    ...
    return (
      <section className={styles.features}>
        <div className="container">
          <div className="row">
            {forLayout(EnglishFeatureList, layout).map((props, idx) => (
              <Feature key={idx} {...props} />
            ))}
          </div>
        </div>
