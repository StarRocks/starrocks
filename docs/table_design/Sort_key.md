...
>**NOTICE**
>
> For an Aggregate Key table, columns for which `agg_type` is specified are key columns, and those for which `agg_type` is not specified are value columns. See [CREATE TABLE](../sql-reference/sql-statements/data-definition/CREATE%20TABLE.md). In the preceding example, `site_id` and `city_code` are specified as sort key columns, and therefore you must specify `agg_type` for `user_id` and `pv`.
