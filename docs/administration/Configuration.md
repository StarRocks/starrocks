...
- **Unit**: -
- **Default**: TRUE
- **Description**: Whether to synchronously execute the apply task at the publish phase of a load transaction. This parameter is applicable only to Primary Key tables. Valid values: <ul><li>`true` (default): The apply task is synchronously executed at the publish phase of a load transaction. It means that the load transaction is reported as successful only after the apply task is completed, and the loaded data can truly be queried. It can improve query performance and stability when frequently load large batches of data into Primary Key tables but may significantly increase load latency.</li><li>`false`: The apply task is asynchronously executed at the publish phase of a load transaction. It means that the load transaction is reported as successful after the apply task is submitted, but the loaded data cannot be immediately queried. In this case, concurrent queries need to wait for the apply task to complete or time out before they can continue, which may affect query performance and stability.</li></ul>
