...
| load_process_max_memory_limit_bytes | 107374182400 | The maximum size limit of memory resources can be taken up by all load process on a BE node. Its value is the smaller one between `mem_limit * load_process_max_memory_limit_bytes` and `load_process_max_memory_limit_bytes`. It is the actual available import memory threshold, reaching which a disk swipe will be triggered.  |
| load_process_max_memory_limit_percent | 30 | The maximum percentage limit of memory resources can be taken up by all load process on a BE node. Its value is the smaller one between `mem_limit * load_process_max_memory_limit_percent` and `load_process_max_memory_limit_bytes`. It is the actual available import memory threshold, reaching which the swipe will be triggered. |
