...
Template of **cluster_snapshot.yaml** for cross-region recovery:
...
cluster_snapshot:
    ...
    # NOTE: It must be identical with that in the original cluster.
    storage_volume_name: my_s3_volume
    # Set the snapshot scope to external
    snapshot_scope: external

# Information of the storage volume that stores the original cluster snapshot.
# It must be the storage volume defined in the `cluster_snapshot` section.
snapshot_storage_volume:
  - name: my_s3_volume
    type: S3
    # The location of the storage volume.
    # Example: s3://defaultbucket/test/
    location: <storage_volume_location>
    comment: my s3 volume
    properties:
      - key: aws.s3.region
        value: us-east-1
...
storage_volumes:
  # Example for S3-compatible storage volume.
    # NOTE: The name of the storage volume must be identical with that of `snapshot_storage_volume`.
  - name: my_s3_volume
    type: S3
    # NOTE: The location of the storage volume must NOT be identical with that of `snapshot_storage_volume`.
    location: <new_storage_volume_location>
    comment: my s3 volume
    properties:
      - key: aws.s3.region
        value: us-east-1
