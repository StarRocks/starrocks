...
| key.converter                       | NO      |           Key converter used by Kafka connect cluster                                                           | This parameter specifies the key converter for the sink connector (Kafka-connector-starrocks), which is used to deserialize the keys of Kafka data. The default key converter is the one used by Kafka connect cluster. You can also specify a key converter. |
| value.converter                     | NO      |    Value converter used by Kafka connect cluster                             | This parameter specifies the value converter for the sink connector (Kafka-connector-starrocks), which is used to deserialize the values of Kafka data. The default value converter is the one used by Kafka connect cluster. You can also specify a value converter. |
