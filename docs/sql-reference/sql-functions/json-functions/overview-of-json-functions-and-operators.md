...
| [json_each](./json-query-and-processing-functions/json_each.md) | Expands the top-level elements of a JSON object into key-value pairs. | SELECT * FROM tj_test, LATERAL JSON_EACH(j); | See the example in [json_each](./json-query-and-processing-functions/json_each.md). |
...
| [json_length](../../sql-functions/json-functions/json-processing-functions/json_length.md) | Returns the length of a JSON document.  | SELECT json_length('{"Name": "Alice"}'); |  1  |
