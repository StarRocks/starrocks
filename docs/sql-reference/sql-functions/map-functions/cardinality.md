...
Returns the number of elements in a MAP value. It is the alias of [map_size()](map_size.md). MAP is an unordered collection of key-value pairs, for example, `{"a":1, "b":2}`. One key-value pair constitutes one element, for example, `{"a":1, "b":2}` contains two elements.
