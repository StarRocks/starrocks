...
- **AUTO_INCREMENT**：specifies an `AUTO_INCREMENT` column. The data types of the `AUTO_INCREMENT` columns' must be BIGINT. The auto-incremented ID starts from 1 and increases by 1. For more information about `AUTO_INCREMENT` columns, see [AUTO_INCREMENT](../../sql-statements/auto_increment.md).
