...
- `mv_rewrite_staleness_second`:  The `mv_rewrite_staleness_second` option is used for MV Rewrite and helps you use the MV when processing large, frequently changing datasets. We define the refresh time for each table partition based on its `getVisibleVersionTime` for StarRocks tables and `getModifiedTime` for external tables. We define the MV's refresh time as the maximum refresh time of its referred base tables. Without `mv_rewrite_staleness_second`, the MV needs to be refreshed and cannot be used for MV rewrite when one of the referred base tables is refreshed. However, with `mv_rewrite_staleness_second`, the MV can be used for MV rewrite when the MV's refresh time is less than mv_rewrite_staleness_second.
- `colocate_with`: The colocation group can be specified for a Materialized View, just like when creating a table. See [Colocate Join](../../../using_starrocks/Colocate_join) for further information.
- `unique_constraints`/`foreign_key_constraints`:  Define the constraints of materialized view's related tables to rewrite queries in view delta join. NOTE: The Unique Key constraints and Foreign Key constraints are only used for query rewrite. Foreign Key constraint checks are not guaranteed when data is loaded into the table. You must ensure the data loaded into the table meets the constraints. See [Asynchronous materialized view - Rewrite queries in View Delta Join scenario](../../../using_starrocks/Materialized_view.md#rewrite-queries-in-view-delta-join-scenario) for further information.

> **CAUTION**
> - `PROPERTIES` can be modified by `ALTER MATERIALIZED VIEW` command, see [ALTER MATERIALIZED VIEW](./ALTER%20MATERIALIZED%20VIEW) for further information.
