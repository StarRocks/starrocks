...
Returns the CREATE TABLE statement that was used to create a given table. In versions earlier than v3.0, the SHOW CREATE TABLE statement requires you to have the `SELECT_PRIV` permission on the table. Since v3.0, the SHOW CREATE TABLE statement requires you to have the `SELECT_PRIV` permission on the table. Note that from v3.0 onwards you can use the SHOW CREATE TABLE statement to view the CREATE TABLE statements of the tables that are managed by an external catalog and are stored in Apache Hive™, Apache Iceberg, or Apache Hudi.
