...
| PARTITION    | The name of the target partition(s) to load data in. It must be set to partitions that exist in the target table. If it is not specified, the data will be inserted into all partitions. Otherwise, the data will be inserted only into the specified partition(s). |
