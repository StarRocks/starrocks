...
- `NEGATIVE`
  ...
  > **NOTE**
  >
  > This parameter is valid only when the StarRocks table uses the Aggregate Key table and all its value columns are computed by the `sum` function.
...
If you choose an S3-compatible storage system, such as MinIO, configure `StorageCredentialParams` as follows:
