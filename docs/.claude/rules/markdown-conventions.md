...
1. Create the file under `en/`, in the directory matching the sidebar section it belongs to. The sidebar is autogenerated from the directory tree — there is no sidebar file to edit.
2. Set `sidebar_position` in its frontmatter to place it within that section. Existing pages are spaced by 10, so use the gaps.
3. Create matching files under `zh/` and `ja/` at the same relative path. If a `ja/` page does not yet exist for that section, note it in your completion summary and ask the user whether to create it.

Adding a new category means adding a directory plus a `_category_.yml` in all three language trees. See "Sidebar Configuration" in `docs/AGENTS.md`.
