...
* Iceberg external tables can be used to query only the following types of data:
  * Iceberg v1 (Analytic Data Tables) tables. From v3.0 onwards, ORC-formatted Iceberg v2 (Row-level Deletes) tables are supported. For the differences between Iceberg v1 tables and Iceberg v2 tables, see [Iceberg Table Spec](https://iceberg.apache.org/spec/).
