...
| user                        | No       | Empty         | The username that is used to log in to the Elasticsearch cluster with HTTP basic authentication enabled. Make sure you have access to paths such as `/cluster/state/ nodes/http` and the index. |
