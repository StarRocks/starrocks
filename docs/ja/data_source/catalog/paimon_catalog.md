...
Append-only テーブルでは、`WHERE` と `LIMIT` を持つスキャンの候補ファイルを段階的に計画できます。フィルタリング後の行数が不足している場合、StarRocks はファイルの取得を続け、実行側で LIMIT に達すると後続の計画を停止します。例:

```SQL
SELECT * FROM <table_name> WHERE <column_name> > 100 LIMIT 10;
```

この最適化には、コネクタの増分スキャン範囲が有効である必要があります。小さい LIMIT では、バッチごとに要求するスキャン範囲の数も減少します。Manifest のバッチと実行中のスキャンタスクによる先読みは発生するため、一致する行がないクエリではすべての候補ファイルを調べる場合があります。主キーテーブル、削除ベクトル、データ進化、および SDK のクエリ認可では、マージとアクセス制御のセマンティクスを維持するため、Paimon ネイティブの split 計画を使用します。
