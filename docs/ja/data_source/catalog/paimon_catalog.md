...
また、Paimon catalog を基に [INSERT INTO](../../sql-reference/sql-statements/loading_unloading/INSERT.md) を使用して、Paimon からデータを直接変換してロードすることもできます。
...
| パラメータ                | 必須 | 説明                                                  |
| ------------------------ | ---- | ---------------------------------------------------- |
| paimon.catalog.type      | はい | Paimon クラスターで使用するメタストアのタイプ。有効な値: `filesystem`、`hive`、`jdbc`。 |
| paimon.catalog.warehouse | はい | Paimon データのウェアハウスストレージパス。 |
| uri                    | いいえ | JDBC メタストアデータベースの接続 URI。`paimon.catalog.type` を `jdbc` に設定した場合に必須。例: `jdbc:mysql://host:3306/paimon_metastore`。 |
| jdbc.user               | いいえ | JDBC メタストアデータベースに接続するためのユーザー名。`paimon.catalog.type` が `jdbc` の場合に使用します。 |
| jdbc.password           | いいえ | JDBC メタストアデータベースに接続するためのパスワード。`paimon.catalog.type` が `jdbc` の場合に使用します。 |
| catalog-key             | いいえ | 同じ JDBC メタストアデータベースを共有する複数の Paimon catalog を分離するために使用される catalog key。デフォルト値: `jdbc`。 |
| hive.metastore.uris      | いいえ | Hive メタストアの URI。形式: `thrift://<metastore_IP_address>:<metastore_port>`。`paimon.catalog.type` を `hive` に設定した場合に必須。Hive メタストアで高可用性 (HA) が有効になっている場合、複数のメタストア URI を指定し、カンマ (`,`) で区切ることができます。例: `"thrift://<metastore_IP_address_1>:<metastore_port_1>,thrift://<metastore_IP_address_2>:<metastore_port_2>,thrift://<metastore_IP_address_3>:<metastore_port_3>"`。 |
...
Paimon クラスターのメタストアとして JDBC メタストアを選択する場合、以下のようなコマンドを実行します。
...
JDBC メタストアのパラメータを以下の表に示します。

| パラメータ                   | 必須 | 説明                                                  |
| ---------------------------- | ---- | ---------------------------------------------------- |
| uri                          | はい | JDBC メタストアデータベースの接続 URI。例: `jdbc:mysql://host:3306/paimon_metastore`。 |
| jdbc.user                    | いいえ | JDBC メタストアデータベースに接続するためのユーザー名。  |
| jdbc.password                | いいえ | JDBC メタストアデータベースに接続するためのパスワード。  |
| catalog-key                  | いいえ | 同じ JDBC メタストアデータベースを共有する複数の Paimon catalog を分離するために使用される catalog key。デフォルト値: `jdbc`。 |
...
また、外部 catalog の作成ステートメントをクエリするには、[SHOW CREATE CATALOG](../../sql-reference/sql-statements/Catalog/SHOW_CREATE_CATALOG.md) を使用できます。以下の例では、`paimon_catalog_fs` という名前の Paimon catalog の作成ステートメントをクエリします。
...
1. [SHOW DATABASES](../../sql-reference/sql-statements/Database/SHOW_DATABASES.md) を使用して、Paimon クラスター内のデータベースを表示します。
...
2. [SET CATALOG](../../sql-reference/sql-statements/Catalog/SET_CATALOG.md) を使用して、現在のセッションで目的の catalog に切り替えます。
   ...
   または、[USE](../../sql-reference/sql-statements/Database/USE.md) を使用して、目的の catalog 内で直接アクティブなデータベースを指定することもできます。
