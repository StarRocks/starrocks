...
JDBC catalog は v3.0 から MySQL と PostgreSQL を、v3.2.9 と v3.3.1 から Oracle と SQLServer を、v3.3.0 から ClickHouse (実験的) を、v4.2 から BigQuery (実験的) をサポートしています。
...
| driver_class      | JDBC ドライバーのクラス名。一般的なデータベースエンジンの JDBC ドライバークラス名は以下の通りです：<ul><li>MySQL: `com.mysql.jdbc.Driver` (MySQL v5.x およびそれ以前) および `com.mysql.cj.jdbc.Driver` (MySQL v6.x およびそれ以降)</li><li>PostgreSQL: `org.postgresql.Driver`</li><li>Oracle: `oracle.jdbc.driver.OracleDriver`</li><li>BigQuery: `com.simba.googlebigquery.jdbc.Driver` または `com.simba.googlebigquery.jdbc42.Driver`</li></ul> |
| schema_resolver   | (オプション) 使用するスキーマリゾルバーを明示的に指定します。有効な値：`postgresql`、`mysql`、`oracle`、`sqlserver`、`clickhouse`、`bigquery`。ドライバークラス名から自動検出できない非標準の JDBC ドライバーを使用する場合にこのパラメーターを使用します。指定しない場合、StarRocks は `driver_class` パラメーターに基づいて適切なリゾルバーを自動検出します。 |
...
BigQuery はユーザー名/パスワードの代わりに OAuth ベースの認証を使用します。認証パラメーターは `jdbc_uri` 文字列に直接渡します。`user` および `password` プロパティは使用しませんが、DDL に存在する必要があります（空文字列に設定してください）。

Simba BigQuery JDBC ドライバーは Maven Central では提供されていません。[Google Cloud](https://cloud.google.com/bigquery/docs/reference/odbc-jdbc-drivers) からダウンロードして FE および BE/CN ノードからアクセスできる URL にホストし、`driver_url` にその URL を設定してください。

一般的な `jdbc_uri` 認証モード：

| OAuthType | 認証方式 | 追加パラメーター |
| :-------- | :------- | :--------------- |
| `0` | サービスアカウントキーファイル | `OAuthServiceAcctEmail`、`OAuthPvtKeyPath` |
| `3` | アプリケーションデフォルト認証情報 | なし |

URI 例：`jdbc:bigquery://https://www.googleapis.com/bigquery/v2:443;ProjectId=my-project;OAuthType=3`
...
v4.2 以降、StarRocks は [`native_query`](../../sql-reference/sql-functions/table-functions/native_query.md) テーブル関数を使用して、データベースネイティブの `SELECT` 文で JDBC データをクエリできます。
