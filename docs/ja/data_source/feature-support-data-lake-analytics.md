...
- インポートやパーティションの追加削除によって生成された新しいデータファイルはすぐに表示され、手動での更新は必要ありません。
  - **設定**： Catalog プロパティ `iceberg_meta_cache_ttl_sec` を `0` に設定すると、StarRocks はクエリごとに新しいスナップショットをフェッチします。
...
v4.2 以降でサポートされています（実験的）。Simba BigQuery JDBC ドライバーが必要です。

#### データ型の対応

| BigQuery    | StarRocks           | 備考 |
| :---------- | :------------------ | :--- |
| BOOL        | BOOLEAN             | Simba ドライバーは `BIT` または `BOOLEAN` として報告することがあります |
| INT64       | BIGINT              | |
| FLOAT64     | DOUBLE              | |
| NUMERIC     | DECIMAL(p, s)       | 精度 ≤ 38 |
| BIGNUMERIC  | VARCHAR             | 精度 > 38；DECIMAL(38, x) に収まらない場合 |
| STRING      | VARCHAR             | |
| BYTES       | VARBINARY           | |
| DATE        | DATE                | |
| TIME        | TIME                | |
| DATETIME    | DATETIME            | 民間時間、タイムゾーン変換なし |
| TIMESTAMP   | DATETIME            | UTC エポック；セッションクエリタイムゾーンに変換 |
| JSON        | JSON                | |
| STRUCT      | VARCHAR             | シリアル化された文字列表現 |
| ARRAY       | VARCHAR             | シリアル化された文字列表現 |
| GEOGRAPHY   | VARCHAR             | |
| INTERVAL    | VARCHAR             | |
