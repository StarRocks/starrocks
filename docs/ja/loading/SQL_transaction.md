...
- 同じ理由により、同一トランザクション内で既に変更されたテーブルに対する部分列更新（主キーテーブルの一部の列のみを書き込む INSERT ステートメント）は許可されません。部分列更新は各行を補完するためにテーブルの他の列を暗黙的に読み取る必要があり、これは先行ステートメントの未コミットの変更を読み取ることになります。違反するとエラーが発生します。
