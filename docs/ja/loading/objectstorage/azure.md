...
ただし、INSERT+`FILES()` メソッドは現在、Parquet、ORC、CSV ファイル形式のみをサポートしています。そのため、JSON などの他のファイル形式のデータをロードする必要がある場合や、[データロード中に DELETE などのデータ変更を行う](../Load_to_Primary_Key_tables.md)場合は、Broker Load を使用できます。
...
このトピックの例では、Shared Key 認証方法を使用します。ADLS Gen2 からデータを読み取る権限があることを確認するために、[Azure Data Lake Storage Gen2 > Shared Key (ストレージアカウントのアクセスキー)](../../integrations/csp_auth/authenticate_to_azure_storage.md#service-principal-1) を読み、設定する必要がある認証パラメータを理解することをお勧めします。
...
利用可能なすべての認証方法については、[Azure クラウドストレージへの認証](../../integrations/csp_auth/authenticate_to_azure_storage.md)を参照してください。
...
- [SELECT](../../sql-reference/sql-statements/table_bucket_part_index/SELECT/SELECT.md) を使用して Azure から直接データをクエリする。
