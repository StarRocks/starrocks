...
description: "Google Cloud Storage（GCS）から INSERT+FILES()、Broker Load、Spark Load を使用してデータをロードする方法。"
...
ただし、INSERT+`FILES()` メソッドは現在、Parquet、ORC、および CSV ファイル形式のみをサポートしています。そのため、JSON などの他のファイル形式のデータをロードする必要がある場合や、[データロード中に DELETE などのデータ変更を行う](../Load_to_Primary_Key_tables.md)場合は、Broker Load を利用できます。
...
利用可能なすべての認証方法については、[Google Cloud Storage への認証](../../integrations/csp_auth/authenticate_to_gcs.md)を参照してください。
...
- [SELECT](../../sql-reference/sql-statements/table_bucket_part_index/SELECT/SELECT.md) を使用して GCS から直接データをクエリする。
