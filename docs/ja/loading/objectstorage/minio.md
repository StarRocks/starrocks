...
description: "MinIO から INSERT+FILES()、Broker Load、Spark Load を使用してデータをロードする方法。"
...
ただし、INSERT+`FILES()` メソッドは現在、Parquet、ORC、および CSV ファイル形式のみをサポートしています。そのため、JSON などの他のファイル形式のデータをロードする必要がある場合や、[データロード中に DELETE などのデータ変更を行う](../Load_to_Primary_Key_tables.md)必要がある場合は、Broker Load を使用できます。
...
- [SELECT](../../sql-reference/sql-statements/table_bucket_part_index/SELECT/SELECT.md) を使用して MinIO から直接データをクエリする。
