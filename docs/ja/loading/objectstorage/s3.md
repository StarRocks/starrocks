...
description: "AWS S3 から INSERT+FILES()、Broker Load、Spark Load、Pipe を使用してデータをロードする方法（IAM およびインスタンスプロファイル認証オプション付き）。"
...
このトピックの例では、IAM ユーザーに基づく認証を使用しています。AWS S3 からデータを読み取る権限を持っていることを確認するために、[IAM ユーザーに基づく認証の準備](../../integrations/csp_auth/authenticate_to_aws_resources.md) を読み、適切な [IAM ポリシー](../../sql-reference/aws_iam_policies.md) を設定した IAM ユーザーを作成する手順に従うことをお勧めします。
...
利用可能なすべての認証方法については、[AWS リソースへの認証](../../integrations/csp_auth/authenticate_to_aws_resources.md) を参照してください。
...
- [SELECT](../../sql-reference/sql-statements/table_bucket_part_index/SELECT/SELECT.md) を使用して S3 から直接データをクエリする。
