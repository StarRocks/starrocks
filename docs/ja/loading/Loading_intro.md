...
- INSERT INTO SELECT:
  - INSERT INTO SELECT FROM `<table_name>`: 内部または外部テーブルでのクエリ結果をテーブルに追加。
...
**ビジネスシナリオ:** Spark クラスターを使用して HDFS に保存された Apache Hive™ テーブルのバッチデータロード。
...
### [Kafka コネクタ](Kafka-connector-starrocks.md)

**ビジネスシナリオ:** Kafka からのストリームデータロード。

### [Spark コネクタ](Spark-connector-starrocks.md)

**ビジネスシナリオ:** Spark からのバッチデータロード。

### [Flink コネクタ](Flink-connector-starrocks.md)
...
| **データソース** | **利用可能なロードオプション**                                |
| --------------- | ------------------------------------------------------------ |
| Apache Flink®   | <ul><li>[Flink コネクタ](Flink-connector-starrocks.md)</li><li>[Stream Load トランザクションインターフェース](Stream_Load_transaction_interface.md)</li></ul> |
| Apache Kafka®   | <ul><li>(ストリーミング) [Kafka コネクタ](Kafka-connector-starrocks.md)</li><li>(ストリーミング) [Routine Load](RoutineLoad.md)</li><li>[Stream Load トランザクションインターフェース](Stream_Load_transaction_interface.md)</li></ul> **注意**<br/>ソースデータがマルチテーブルジョインや ETL 操作を必要とする場合、Flink を使用してデータを読み取り、事前処理を行い、その後 [Flink コネクタ](Flink-connector-starrocks.md) を使用してデータを StarRocks にロードできます。 |
| Apache Spark™   | <ul><li>[Spark コネクタ](Spark-connector-starrocks.md)</li><li>[Spark Load](SparkLoad.md)</li></ul> |

### データレイク

| **データソース** | **利用可能なロードオプション**                                |
| --------------- | ------------------------------------------------------------ |
| Apache Hive™    | <ul><li>(バッチ) [Hive catalog](../data_source/catalog/hive_catalog.md) を作成し、その後 [INSERT INTO SELECT FROM `<table_name>`](InsertInto.md#insert-data-from-an-internal-or-external-table-into-an-internal-table) を使用。</li><li>(バッチ) [Spark Load](https://docs.starrocks.io/docs/loading/SparkLoad/)。</li></ul> |
| Apache Iceberg  | (バッチ) [Iceberg catalog](../data_source/catalog/iceberg_catalog.md) を作成し、その後 [INSERT INTO SELECT FROM `<table_name>`](InsertInto.md#insert-data-from-an-internal-or-external-table-into-an-internal-table) を使用。 |
| Apache Hudi     | (バッチ) [Hudi catalog](../data_source/catalog/hudi_catalog.md) を作成し、その後 [INSERT INTO SELECT FROM `<table_name>`](InsertInto.md#insert-data-from-an-internal-or-external-table-into-an-internal-table) を使用。 |
| Delta Lake      | (バッチ) [Delta Lake catalog](../data_source/catalog/deltalake_catalog.md) を作成し、その後 [INSERT INTO SELECT FROM `<table_name>`](InsertInto.md#insert-data-from-an-internal-or-external-table-into-an-internal-table) を使用。 |
| Elasticsearch   | (バッチ) [Elasticsearch catalog](../data_source/catalog/elasticsearch_catalog.md) を作成し、その後 [INSERT INTO SELECT FROM `<table_name>`](InsertInto.md#insert-data-from-an-internal-or-external-table-into-an-internal-table) を使用。 |
| Apache Paimon   | (バッチ) [Paimon catalog](../data_source/catalog/paimon_catalog.md) を作成し、その後 [INSERT INTO SELECT FROM `<table_name>`](InsertInto.md#insert-data-from-an-internal-or-external-table-into-an-internal-table) を使用。 |

StarRocks は v3.2 以降、[unified catalogs](https://docs.starrocks.io/docs/data_source/catalog/unified_catalog/) を提供しており、Hive、Iceberg、Hudi、Delta Lake のデータソースからのテーブルを統合データソースとして取り扱うことができます。

### 内部および外部データベース

| **データソース**                                              | **利用可能なロードオプション**                                |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| StarRocks                                                    | (バッチ) [StarRocks external table](../data_source/External_table.md#starrocks-external-table) を作成し、その後 [INSERT INTO VALUES](InsertInto.md#insert-data-via-insert-into-values) を使用して少量のデータレコードを挿入するか、[INSERT INTO SELECT FROM `<table_name>`](InsertInto.md#insert-data-from-an-internal-or-external-table-into-an-internal-table) を使用してテーブルのデータを挿入。<br/>**注意**<br/>StarRocks external tables はデータの書き込みのみをサポートし、データの読み取りはサポートしていません。 |
| MySQL                                                        | <ul><li>(バッチ) [JDBC catalog](../data_source/catalog/jdbc_catalog.md)（推奨）または [MySQL external table](../data_source/External_table.md#deprecated-mysql-external-table) を作成し、その後 [INSERT INTO SELECT FROM `<table_name>`](InsertInto.md#insert-data-from-an-internal-or-external-table-into-an-internal-table) を使用。</li><li>(ストリーミング) [SMT、Flink CDC コネクタ、Flink、および Flink コネクタ](Flink_cdc_load.md) を使用。</li></ul> |
| その他のデータベース（Oracle、PostgreSQL、SQL Server、ClickHouse、TiDB など） | <ul><li>(バッチ) [JDBC catalog](../data_source/catalog/jdbc_catalog.md)（推奨）または [JDBC external table](../data_source/External_table.md#external-table-for-a-jdbc-compatible-database) を作成し、その後 [INSERT INTO SELECT FROM `<table_name>`](InsertInto.md#insert-data-from-an-internal-or-external-table-into-an-internal-table) を使用。</li><li>(ストリーミング) [SMT、Flink CDC コネクタ、Flink、および Flink コネクタ](loading_tools.md) を使用。</li></ul> |