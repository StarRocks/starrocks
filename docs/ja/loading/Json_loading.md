...
* Stream Load: テキストファイルに保存された JSON データには、stream load を使用してインポートします。
* Routine Load: Kafka 内の JSON データには、routine load を使用してインポートします。
...
`format: json` パラメータは、インポートデータのフォーマットを実行することを可能にします。`jsonpaths` は、対応するデータインポートパスを実行するために使用されます。

関連パラメータ:

* jsonpaths: 各カラムの JSON パスを選択
* json\_root: JSON が解析を開始するカラムを選択
* strip\_outer\_array: 最外の配列フィールドを切り取る
* strict\_mode: インポート中のカラム型変換を厳密にフィルタリング
...
インポート中に k1 を 100 倍する ETL 操作が実行され、カラムは `Jsonpath` によって元のデータと一致します。
...
欠落しているカラムについては、カラム定義が nullable であれば `NULL` が追加され、または `ifnull` によってデフォルト値を追加できます。
...
2. 配列形式の JSON が複数行のデータを表すが、JSON フォーマットエラーのために JSON の解析に失敗した場合、エラー行は 1 だけ増加します（解析に失敗したため、StarRocks は実際にどれだけの行のデータが含まれているかを判断できず、エラーデータを 1 行として記録することしかできません）。

### Canal を使用して MySQL から StarRocks へインクリメンタル同期 binlogs をインポート

[Canal](https://github.com/alibaba/canal) は Alibaba のオープンソースの MySQL binlog 同期ツールであり、これを通じて MySQL データを Kafka に同期できます。データは Kafka で JSON 形式で生成されます。ここでは、routine load を使用して Kafka のデータを MySQL とインクリメンタルデータ同期する方法を示します。

* MySQL には、次のテーブル作成ステートメントを持つデータテーブルがあります。
...
server_id=1 # MySQL レプリケーションには定義が必要で、canal の slaveId と重複しないようにします
~~~
...
# ...
# kafka/rocketmq クラスター設定: 192.168.1.117:9092,192.168.1.118:9092,192.168.1.119:9092
...
`show routine load` でインポートジョブのステータスとエラーメッセージを確認します。