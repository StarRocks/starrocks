...
要するに、Shared Key 認証を実践する場合、次の情報を収集する必要があります。
...
このメソッドは v3.2 以降で利用可能で、現在は Parquet および ORC ファイル形式のみをサポートしています。
...
これは前の例の続きです。前のクエリは CREATE TABLE AS SELECT (CTAS) でラップされ、スキーマ推測を使用してテーブル作成を自動化します。つまり、StarRocks はテーブルスキーマを推測し、作成したいテーブルを作成し、データをテーブルにロードします。Parquet ファイルを使用する場合、列名は Parquet 形式に含まれているため、`FILES()` テーブル関数を使用してテーブルを作成する際に列名と型を指定する必要はありません。

> **NOTE**
>
> スキーマ推測を使用する場合の CREATE TABLE の構文では、レプリカ数を設定することはできません。StarRocks 共有なしクラスタを使用している場合は、テーブルを作成する前にレプリカ数を設定してください。以下の例は、3 つのレプリカを持つシステムの例です：
>
> ```SQL
...
この例では、テーブルがどのようにクエリされるか、および Parquet ファイル内のデータに関する知識に基づいてテーブルを作成します。Parquet ファイル内のデータに関する知識は、Azure でファイルを直接クエリすることで得られます。
...
- 予想されるクエリタイプに基づいて、ソートキーとバケッティング列は `UserID` 列に設定されています。このデータに対する使用ケースは異なるかもしれないので、ソートキーとして `ItemID` を `UserID` に追加または代わりに使用することを決定するかもしれません。
...
手動でテーブルを作成します（Azure からロードしたい Parquet ファイルと同じスキーマを持つテーブルをお勧めします）：
...
スキーマを表示し、`FILES()` テーブル関数によって生成された推測スキーマと比較できるようにします：
...
先ほど作成したスキーマと、`FILES()` テーブル関数を使用して以前に推測されたスキーマを比較します。以下を確認してください：
...
非同期の Broker Load プロセスは、Azure への接続を確立し、データを取得し、StarRocks にデータを保存する役割を果たします。
