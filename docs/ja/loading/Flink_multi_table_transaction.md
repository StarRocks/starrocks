...
- Default: `-1` (auto)
- Unit: ms
- Description: パーティションごとのチャンク切り替え間の最小間隔（ms）。この間隔内では、ソーストランザクションを 1 つの Stream Load にまとめ、HTTP リクエスト数を抑制します。`-1` の場合は `min(1000, max(500, sink.buffer-flush.interval-ms/4))` が自動的に適用され、正の値を指定するとその値で上書きされます。
...
- Unit: Bytes
- Description: 時間間隔に基づくチャンク切り替えは、あるリージョンのアクティブチャンクに少なくともこのバイト数が蓄積されている場合にのみ実行されます。これにより、低ボリュームのパーティションでは、複数のソーストランザクションを 1 つの Stream Load にまとめ、多数の小さなリクエストが発生するのを防ぎます。半分まで埋まった場合のヘッドルーム、バッファサイズによるメモリプレッシャー、またはパーティションが最後に切り替えられてから `sink.buffer-flush.interval-ms` が経過した場合は、サイズしきい値に関係なく切り替えが強制されます。そのため、大きなしきい値を設定しても、継続的な低ボリュームのストリームはおおむねフラッシュ間隔で切り替えられ、無期限に保持されることはありません。`<=0` に設定するとサイズゲートが無効になり、従来のインターバルごとの切り替え動作に戻ります。
...
- Default: `0` (unlimited)
- Unit: Bytes
- Description: `txnEnd` を待機している間に、1 つのシンクサブタスクがバッファリングできる処理中のソーストランザクションデータ量のハード上限（バイト単位）。`txnEnd` より前にフラッシュできないデータについて、Writer がブロックすることはありません（制限事項 7 を参照）。そのため、1 つのソーストランザクションが `2 × buffer-size` を超えて増大する可能性があります。このオプションを使用するとその増大を制限し、上限を超えた場合は明確なエラーでジョブを失敗させます。`0` は上限なしを意味し、TaskManager のヒープのみが上限となります。
...
- **大規模なソーストランザクションにおける `activeChunk` のメモリ使用量の増加**: マルチテーブルモードでは、チャンクサイズによる内部切り替えが無効化されています（クリーンなトランザクション境界の不変条件を維持するため）。そのため、`activeChunk` は次の `txnEnd` が到着するまで増大する可能性があります。`sink.transaction.multi-table.buffer-size`（ソフト上限）と `2 × buffer-size`（`blockIfCacheFull` によるハード上限）は、バッファのうち*フラッシュ可能な*部分のみを制限します。フリーズされたチャンクとすべてのクリーンなパーティションはこの制限の対象外です。`txnEnd` より前にバッファから排出できないデータ（処理中のトランザクション自体、および同じパーティション内でロックステップルールによって保持される兄弟テーブルの完了済み行）については、Writer は待機しません。バッファがハード上限に達し、フラッシュ可能なデータが何もない場合、Writer は WARN（`Write-block cap ... reached with nothing flushable`）をログに出し、`txnEnd` までバッファリングを継続します。その後、パーティションが切り替えられて排出されます。そのため、1 つのソーストランザクションが `2 × buffer-size` を超える可能性があります。そのサイズは `sink.transaction.multi-table.max-txn-bytes`（デフォルトは無制限）と TaskManager のヒープによってのみ制限されます。ヒープサイズは、想定される最大ソーストランザクションサイズ × 1 つの TaskManager 上のシンクサブタスク数を基準に設定してください。ヒーププレッシャーよりも明確な失敗を優先する場合は、`max-txn-bytes` を設定してください。また、サイズに上限のないトランザクション（バルクブートストラップ）は上流で分割してください。切り替え時に、フリーズされたチャンクがロード本文の上限（デフォルトでは `sink.chunk-limit` の 3 GB と `sink.transaction.multi-table.buffer-size` の小さい方）を超えている場合は、同じ共有ラベルを使用して複数のロードリクエストに再分割されます。そのため、単一の行自体がこの上限を超える場合を除き、単一の `/api/transaction/load` 本文がこの上限を超えることはありません。
...
- 原因：ソーストランザクションと、`txnEnd` まで保持される兄弟テーブルの完了済み行の合計が `2 × buffer-size` を超えています。Writer はブロックせず、`txnEnd` までバッファリングを継続します。
- 解決策：大きなソーストランザクションでは想定される動作です。`txnEnd` が到着するとデータがコミットされます。TaskManager のヒープを監視してください。ハード失敗を優先する場合は `sink.transaction.multi-table.max-txn-bytes` を設定するか、上流でトランザクションを分割してください。

#### `would exceed sink.transaction.multi-table.max-txn-bytes`

- 原因：ソーストランザクションが設定されたハード上限を超えています。
- 解決策：`max-txn-bytes` を引き上げる（または無制限にするため `0` に設定する）か、上流でソーストランザクションを分割してください。
