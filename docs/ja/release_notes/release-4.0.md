...
ファイルバンドリングを有効化した後、クラスターを v3.5.2 以降にダウングレードできます。クラスタが v3.5.2 より前のバージョンにダウングレードされた場合、ファイルバンドリングを有効にしているテーブルを読み書きすることはできません。ファイルバンドリングは、v4.0 以降で作成されたテーブルに対してデフォルトで有効化されています。
