...
主キーテーブルは、v4.0 以降で全文インバーテッドインデックスをサポートしています。

全文インバーテッドインデックスは、共有データクラスタではまだサポートされていません。
...
:::note
テーブルに全文インバーテッドインデックスを作成する際は、そのテーブルの `replicated_storage` 機能を無効化する必要があります。
- v4.0 以降では、インデックスを作成する際、この機能は自動的に無効化されます。
- v4.0 以前のバージョンでは、テーブルのプロパティ `replicated_storage` を `false` に手動で設定する必要があります。
:::
...
- `parser` パラメータはトークン化方法を指定します。サポートされている値と説明は以下の通りです:
  - `none` (デフォルト): トークン化なし。インデックス化された列のデータ全体が全文インバーテッドインデックスが構築されるときに単一のインデックス項目として扱われます。
  - `english`: 英語トークン化。このトークン化方法は通常、非アルファベット文字でトークン化します。また、大文字の英語文字は小文字に変換されます。したがって、クエリ条件のキーワードは大文字の英語ではなく小文字の英語である必要があります。
  - `chinese`: 中国語トークン化。このトークン化方法は、CLucene の [CJK Analyzer](https://lucene.apache.org/core/6_6_1/analyzers-common/org/apache/lucene/analysis/cjk/package-summary.html) を使用してトークン化します。
  - `standard`: 多言語トークン化。このトークン化方法は、文法に基づくトークン化（[Unicode Text Segmentation algorithm](https://unicode.org/reports/tr29/) に基づく）を提供し、ほとんどの言語や混合言語のケースに対応します。例えば、このトークン化方法は中国語と英語が共存する場合にそれらを区別できます。英語をトークン化した後、大文字の英語文字を小文字に変換します。したがって、クエリ条件のキーワードは大文字の英語ではなく小文字の英語である必要があります。
...
- クエリ中、キーワードは `%` を使用してあいまいに一致させることができ、形式は `%keyword%` です。ただし、キーワードは単語の一部を含む必要があります。例えば、キーワードが <code>starrocks&nbsp;</code> の場合、スペースを含むため `starrocks` という単語に一致しません。
...
- クエリ条件の `MATCH` プレディケートはプッシュダウンプレディケートとして使用されなければならないため、WHERE 句にあり、インデックス化された列に対して実行される必要があります。

    以下のテーブルとテストデータを例に取ります:
    ...
    - クエリ文の `MATCH` プレディケートが実行される列 `value_test` がインデックス化された列ではないため、クエリが失敗します。
...
全文インバーテッドインデックスがインデックス化された列をトークン化しない場合、つまり `'parser' = 'none'` の場合、以下にリストされたクエリ条件のすべてのプッシュダウンプレディケートを使用して、全文インバーテッドインデックスを使用してデータフィルタリングを行うことができます:
