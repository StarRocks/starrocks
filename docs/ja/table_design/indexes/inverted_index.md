...
全文インバーテッドインデックスを作成した後、システム変数 `enable_gin_filter` が有効になっていることを確認する必要があります。これにより、インバーテッドインデックスがクエリを高速化できます。また、インデックスカラムの値がトークン化されているかどうかを考慮して、どのクエリが高速化されるかを判断する必要があります。
