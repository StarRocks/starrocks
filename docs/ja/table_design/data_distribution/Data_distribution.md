...
| ------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| ランダム分散               | ランダムバケット法                                             | テーブル全体が1つのパーティションと見なされます。テーブル内のデータは異なるバケットにランダムに分配されます。これは、レンジ分散が有効でない場合のデフォルトのデータ分散方法です（レンジ分散を参照）。 |
| ハッシュ分散              | ハッシュバケット法                                             | テーブル全体が1つのパーティションと見なされます。テーブル内のデータは、ハッシュ関数を使用してデータのバケッティングキーのハッシュ値に基づいて対応するバケットに分配されます。 |
| レンジ分散                | レンジベースのバケット法                                        | v4.1 以降でサポートされています。共有データモードでは、`DISTRIBUTED BY` 句を指定しない場合のデフォルトの分散方法です。テーブル全体が 1 つのパーティションと見なされます。テーブル内のデータは、テーブルキーの値の範囲に基づいて、対応するバケットに分散されます。 |
| レンジ+ランダム分散       | <ol><li>レンジパーティション化</li><li>ランダムバケット法 </li></ol> | <ol><li>テーブル内のデータは、パーティション列の値が属する範囲に基づいて対応するパーティションに分配されます。 </li><li>パーティション内のデータは異なるバケットにランダムに分配されます。 </li></ol> |
| レンジ+ハッシュ分散       | <ol><li>レンジパーティション化</li><li>ハッシュバケット法 </li></ol> | <ol><li>テーブル内のデータは、パーティション列の値が属する範囲に基づいて対応するパーティションに分配されます。</li><li>パーティション内のデータは、ハッシュ関数を使用してデータのバケッティングキーのハッシュ値に基づいて対応するバケットに分配されます。</li></ol> |
| リスト+ランダム分散       | <ol><li>式に基づくパーティション化またはリストパーティション化</li><li>ランダムバケット法 </li></ol> | <ol><li>テーブル内のデータは、パーティション列の値リストに基づいてパーティション化されます。</li><li>パーティション内のデータは異なるバケットにランダムに分配されます。</li></ol> |
...
#### ランダム分散
  ...
  -- キータイプ、ORDER BY、バケッティング方法のいずれも設定されていないため、デフォルトでランダム分散の Duplicate Key テーブルが作成されます。
  ```

  :::note
  共有データモードでは、キータイプまたは `ORDER BY` 句を明示的に指定し、`DISTRIBUTED BY` 句を指定しない場合、代わりにデフォルトでレンジベースのバケット法が使用されます。[レンジベースのバケット法](#range-based-bucketing)を参照してください。
  :::
...
- **レンジ分散**
  ...
  v4.1 以降、StarRocks は**レンジベースの分散セマンティクス**をサポートしており、これは FE 設定の `enable_range_distribution` によって制御されます。データはキー列のデータ範囲に従ってシーケンス化され、各タブレットには特定の範囲のデータが含まれます。

  :::note
  共有データモードでは、レンジ分散は**デフォルトで有効**であり、FE 設定 `enable_range_distribution` によって制御されます。`false` に設定すると、このデフォルトを無効にできます。この設定は共有なしモードでは効果がありません。
  :::
  ...
  PRIMARY KEY (user_id);
  -- バケット法が設定されていないため、キー列が指定されている場合でも、レンジベースのバケット法が使用されます。
  ```
...
サポートされているバケッティング方法は、[ランダムバケット法](#random-bucketing-since-v31) (v3.1以降)、[ハッシュバケット法](#hash-bucketing)、および [レンジベースのバケット法](#range-based-bucketing) (v4.1以降) です。
...
- レンジベースのバケット法: v4.1 以降、FE 設定 `enable_range_distribution` を有効にすると、キータイプまたは `ORDER BY` 句を明示的に指定し、`DISTRIBUTED BY` 句を指定しないテーブルでは、それらの列の範囲に基づいて同一パーティション内のデータがバケットに分散されます。Tablet は自動分割・統合に対応しており、データ偏りを緩和できます。共有データモードでは、これはデフォルトで有効です。`enable_range_distribution` を `false` に設定すると無効化できます。この設定は共有なしモードでは効果がありません。
...
[レンジパーティション化](./dynamic_partitioning.md)は、時系列データや連続した数値データなどのシンプルな連続データを保存するのに適しています。
...
[動的パーティション化](dynamic_partitioning.md#動的パーティション化)関連のプロパティは、テーブル作成時に設定されます。StarRocksは、データの新鮮さを確保するために、新しいパーティションを事前に自動的に作成し、期限切れのパーティションを削除します。これにより、パーティションのタイムトゥリブ (TTL) 管理が実現されます。
...
StarRocksは、パーティション内のデータをすべてのバケットにランダムに分配します。これは、データサイズが小さく、クエリパフォーマンスの要件が比較的低いシナリオに適しています。バケッティング方法を設定しない場合、StarRocksはデフォルトでランダムバケット法を使用し、バケット数を自動的に設定します。共有データモードでは、キータイプまたは `ORDER BY` 句を指定し、`DISTRIBUTED BY` 句を指定しないテーブルは、代わりにデフォルトでレンジベースのバケット法を使用します。[レンジベースのバケット法](#range-based-bucketing)を参照してください。
...
- キータイプ（AGGREGATE KEY / UNIQUE KEY / PRIMARY KEY / DUPLICATE KEY）が明示的に指定され、かつ `DISTRIBUTED BY` が指定されていない場合、デフォルトでレンジ分散が適用されます。
...
- 多くの場合、テーブル作成はカラム定義と `ORDER BY` のみで完結します。
- `DISTRIBUTED BY` を明示的に指定する必要がありません。

**データスキューへの対応**

- Tablet は自動的に分割・統合され、データ分布を動的に再調整することでスキューを緩和します。詳細は [ALTER TABLE - Split or merge tablets](../../sql-reference/sql-statements/table_bucket_part_index/ALTER_TABLE.md#split-or-merge-tablets) を参照してください。

**データローカリティと分散のバランス**

- 小規模テナントのデータは単一の BE/CN に集約され、ローカリティを最大化します。
- 大規模テナントのデータは複数の BE/CN に分散され、スケーラビリティと性能を確保します。

**時間パーティションへの非依存**

- データ管理において時間パーティションは必須ではありません。
- 単一パーティションで大量データを保持可能であり、パーティションごとのデータ量に大きな差を持たせることもできます。これにより、要件に応じた柔軟なパーティション設計が可能です。

#### 注意事項

- 本機能は v4.1 以降でサポートされます。共有データモードでは、FE 設定 `enable_range_distribution` によって制御され、デフォルトで有効です。
...
```SQL
-- DUPLICATE KEY を指定すると、キー列の範囲に基づいて分散されます。
...
-- ORDER BY を指定すると重複キーテーブル作成され、ソートキーの範囲で分散されます。
...
-- DUPLICATE KEY と ORDER BY の両方が未指定の場合、ランダムバケット方式の重複キーテーブル作成されます。
...
**主キーテーブル / 集計テーブル / ユニークキーテーブル：**

```SQL
-- Primary Key テーブルの例：
-- キータイプとキー列を指定すると、キー列の範囲で分散されます。
