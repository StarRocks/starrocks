...
| レンジ分散                | レンジベースのバケット法                                        | v4.1 以降でサポートされています。テーブル全体が 1 つのパーティションと見なされます。テーブル内のデータは、テーブルキーの値の範囲に基づいて、対応するバケットに分散されます。 |
...
- **レンジ分散**
  ...
  :::note
  このセマンティックを有効にするには、FE の設定 `enable_range_distribution` を `true` に設定する必要があります。
  :::
...
- レンジベースのバケット法: v4.1 以降、FE 設定 `enable_range_distribution` を有効にすると、キータイプまたは `ORDER BY` 句を明示的に指定し、`DISTRIBUTED BY` 句を指定しないテーブルでは、それらの列の範囲に基づいて同一パーティション内のデータがバケットに分散されます。Tablet は自動分割・統合に対応しており、データ偏りを緩和できます。このセマンティクスはデフォルトでは無効です。
...
- 本機能は v4.1 以降でサポートされ、デフォルトでは無効です。
- 利用するには、FE 設定 `enable_range_distribution` を `true` に設定する必要があります。
