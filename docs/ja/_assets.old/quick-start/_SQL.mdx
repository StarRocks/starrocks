これらのクエリは、SQLクライアントで実行できます。すべてのクエリは `quickstart` データベースを使用します。

```sql
USE quickstart;
```

#### NYCでは1時間あたりの事故件数はどれくらいですか？
...
以下は出力の一部です。1月6日と7日を詳しく見ているのは、これが祝日でない週の月曜日と火曜日だからです。元旦を見るのは、ラッシュアワーの通常の朝を示しているとは言えないでしょう。
...
月曜日または火曜日の朝のラッシュアワーでは約40件の事故が発生し、17:00時頃も同様です。
...
視界が悪い（0から1.0マイルの間）ときの事故件数を見てみましょう。この質問に答えるために、2つのテーブルをDATETIME列でジョインします。
...
視界が悪いときの1時間あたりの事故の最大件数は129件です。考慮すべき点は複数あります：
...
氷点下の気温の結果は少し驚きました。寒い1月の日曜日の朝に市内であまり交通がないと思っていました。[weather.com](https://weather.com/storms/winter/news/northeast-storm-rain-snow-wind) を見ると、その日は多くの事故が発生した大きな嵐があったことがわかりました。データにもその様子が見られます。
