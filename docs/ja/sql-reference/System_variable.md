...
* **スコープ**: Session
* **説明**: builtin GIN 全文検索の `score()` 関連性ランキングで使う BM25 の長さ正規化パラメータ `b`（FE 設定項目 `enable_experimental_bm25` で有効化）。値が大きいほど長いドキュメントへのペナルティが強くなります。範囲は [0, 1] である必要があります。
* **デフォルト**: 0.75
* **データ型**: Double
* **導入バージョン**: -
...
* **スコープ**: Session
* **説明**: builtin GIN 全文検索の `score()` 関連性ランキングで使う BM25 の語頻度飽和パラメータ `k1`（FE 設定項目 `enable_experimental_bm25` で有効化）。値が大きいほど、繰り返される語がスコアを増加させ続けます。有限かつ非負の数である必要があります。
* **デフォルト**: 1.2
* **データ型**: Double
* **導入バージョン**: -
