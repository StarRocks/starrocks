...
* **説明**: データレイクテーブル（例: Iceberg テーブル）に対する Bucket-aware 実行の適応的フォールバックを制御します。シャッフルを必要とする集計クエリがバケットパーティション化されたテーブルを読み取る際、パーティションおよび述語プルーニング後に残ると推定されるバケット数が、この値と稼働中のワーカー数の積を下回る場合、オプティマイザはそのスキャンに対して Bucket-aware の単一ステージ集計の使用をやめ、通常のシャッフル（マルチステージ）集計プランにフォールバックして並列度を回復します。例えば、バケットソース列に対する等価フィルタによってクエリが単一バケットに絞り込まれ、かつ高カーディナリティ列でグループ化される場合の深刻なスキューを防ぎます。`0` に設定するとフォールバックは無効になります。値が大きいほどフォールバックは積極的になります。この変数は `enable_bucket_aware_execution_on_lake` が `true` の場合にのみ有効です。
* **デフォルト**: 1.0
* **タイプ**: Double
* **導入バージョン**: v4.2
