...
### enable_local_first_tablet_write (v4.2 以降)

* **説明**: 共有データモードでのみ有効です。有効にすると、ロードは各行を、Tablet が割り当てられている単一のノードに送るのではなく、その sink インスタンスが既に動作しているコンピュートノード上の delta writer に書き込みます。これにより 1 つの Tablet の書き込みがクラスタ全体に分散され、同時にそれらの行が本来必要としていたネットワークホップもなくなります。`file_bundling` が有効なクラウドネイティブテーブルにのみ適用され、さらにパーティションの Tablet 数が Warehouse の稼働中コンピュートノード数より少ない間だけ有効です。それを超えるとバケットレベルの並列度でクラスタを埋められるため、ローカルに書いても読み取り側のキャッシュ局所性を失うだけになります。有効化する際は 1 つの前提を受け入れることになります: 同じキーを持つ行は異なるノードに分かれて順序を持たないため、単一トランザクション内で重複キーの到着順に依存するロードでは、どの行が残るかが不定になります。トランザクション間の順序は影響を受けません。
* **デフォルト値**: false
* **型**: Boolean
* **スコープ**: Session
* **導入バージョン**: v4.2

### lake_local_first_write_bytes_per_node (v4.2 以降)

* **説明**: 共有データモードでのみ有効で、`enable_local_first_tablet_write` が有効な場合にのみ作用します。Tablet の書き込み集合に次のノードを追加する前に、1 つのコンピュートノードへ割り当てるロードのバイト数です。ノード数はロードの推定サイズをこの値で割ったもので (整数除算のため、1 ノード分がまるごと確保できて初めてノードが追加されます)、最終的に使用される並列度は、その値、FE 設定項目 `lake_local_first_write_max_nodes`、および稼働中コンピュートノード数の最小値になります。例えば 10 GB のロードはデフォルトの 2 GB で 5 ノードとなり、3 ノードの Warehouse ではさらに 3 に制限されます。書き込みの分散は無償ではありません: Tablet のノードリストにある各ノードはそれぞれ自身の segment を書き出し、自身の部分トランザクションログを生成します。さらに open/close は、そのノードが最終的に行を保持するかどうかにかかわらずリスト内のすべてのノードに到達します。したがって小さなロードを広く分散させると、ノードごとに segment とログが余分に増えるだけで得るものはわずかです。サイズは、`INSERT` ではオプティマイザの推定値から、Broker Load では解決済みのファイルリストから取得します。どちらも利用できない場合、ノード数は `lake_local_first_write_max_nodes` のみに委ねられます。サイズが不明であることは、サイズが小さいことを意味しないためです。0 に設定すると、サイズに基づく算出を無効にします。
