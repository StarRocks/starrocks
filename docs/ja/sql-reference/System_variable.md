...
* **説明**: クライアントがクエリ結果メタデータ内の文字列最大長に依存し、その長さが実際の値より小さいことで一部の BI ツールで空値や切り詰めが発生する場合に、拡張されたフォールバック長を使用するかどうか。`false` の場合、このような列はメタデータ上の長さを `64` にフォールバックします。`true` の場合、CHAR 列は `255`、その他の文字列列は `1048576`（1 MB）にフォールバックします。
* **デフォルト**: false
