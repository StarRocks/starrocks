...
* **説明**: paimon-cpp ネイティブ Reader が 1 回のリモート読み取りで取得する連続 DataCache Block の最大数です。値を増やすとリモート要求の断片化を抑えられますが、一時バッファーが大きくなります。
* **デフォルト**: 64。DataCache Block がデフォルトの 256 KiB の場合は 16 MiB に相当します
* **データ型**: Integer
* **値の範囲**: [1, 256]
* **導入バージョン**: v4.2
