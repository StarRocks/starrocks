...
* **説明**: Delta Lake のファイルタスク列挙とスキャン範囲の消費を並行して実行します。デフォルトでは無効です。有効時は、各増分ファイルソースが現在のバッチと先読みバッチを一つずつ保持します。各バッチは最大 256 ファイルタスクで、推定タスクデータ量が 1 MiB に達すると読み取りを停止します。単一の大きなタスクはこの推定値を超える場合があります。この制限には Delta Kernel のチェックポイントやログ再生のメモリは含まれません。この設定はオプティマイザの統計収集を省略しません。
* **デフォルト**: false
* **データ型**: Boolean

各カタログは二つの先読みスレッドと、最大 128 バッチジョブのキューを使用します。スレッドはバッチを生成すると戻り、消費を待ちません。ジョブの送信が拒否された場合、消費スレッドがそのバッチを同期的に読み取ります。スキャンが LIMIT に達するかソースが閉じられると、保留中の作業をキャンセルし、実行中の読み取りを中断します。リソースの解放には、基盤の I/O が戻るまで待つ必要がある場合があります。クエリの再試行では新しいファイルソースを作成します。クエリトレースには `DELTA_LAKE.prefetchRead`、`DELTA_LAKE.prefetchWait`、`DELTA_LAKE.prefetchFallback` が含まれます。
