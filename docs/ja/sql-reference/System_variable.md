...
* **スコープ**: Session
* **説明**: テキストが完全に一致する派生テーブルを 1 つの共有 CTE にまとめるかどうか。同じサブクエリを複数回記述するクエリ（TPC-DS Q65 や Q44 など）では、これを行わないと出現ごとに再スキャン・再計算されます。有効（`true`）にすると、重複した派生テーブルはセマンティック解析の前に 1 つの CTE へ書き換えられ、ユーザーが `WITH` 句を書いた場合とまったく同じ結果になります。その CTE を実体化して共有するか、各箇所へインライン展開し直すかは、既存の CTE コストモデル（`cbo_cte_reuse_rate` を参照）が判断します。`rand()` や `any_value()` などの非決定的関数を使う、内部のいずれかの階層に `LIMIT` を持つ、出力列名を変更している、`LATERAL` 結合の中にある、または `WITH` 句で定義された名前を参照するサブクエリは対象外です。`?` パラメータを使う文は全体がスキップされます。さらに、関連するマテリアライズドビューを持つテーブルを読むクエリでは、この書き換えは一切適用されません。マテリアライズドビューの書き換えはクエリを AST で識別するため書き換え後のクエリは一致しなくなり、通常はマテリアライズドビューの方が効果が大きいためです。
* **デフォルト**: true
* **データタイプ**: boolean
* **導入バージョン**: v4.2.0
