...
* **説明**: Delta Lake のファイルを計画時にすべて列挙する代わりに、キャッシュされたスナップショット行数をオプティマイザ統計として使用します。デフォルトでは無効です。収集済みの内部統計が優先され、`enable_delta_lake_column_statistics` を明示的に有効にすると、完全なメタデータ統計の収集が行われます。キャッシュミス時には `default_statistics_output_row_count` と未知の列統計を使用します。結合や集約の計画が変わる可能性があるため、有効化前に代表的なクエリを検証してください。キャッシュ行数はファイルメタデータに基づく推定値で、削除ベクトルで隠された行を含む場合があります。実際のスキャンの制限や正確な `COUNT(*)` の結果には使用されません。
* **デフォルト**: false
* **データ型**: Boolean

キャッシュは各 FE の各カタログに属し、最大 1,000 件のスナップショット概要を保持します。エントリは一時間アクセスがないと期限切れになります。テーブルの場所、Delta テーブル ID、スナップショットバージョンで分離されます。フィルタなしの完全なファイル列挙のみがキャッシュを更新します。フィルタ付き、キャンセル、失敗、部分的なスキャンは概要を公開しません。この設定を無効にしてフィルタなしのクエリを実行すると、キャッシュを準備できます。キャッシュミスはバックグラウンドの統計収集を開始しません。
