...
### tablet_write_local_first (v4.2 以降)

* **説明**: 共有データモードでのみ有効で、`tablet_write_parallelism` が 1 つの Tablet を複数のコンピュートノードに分散させている場合にのみ意味を持ちます。`true` の場合、自ノードがその Tablet の書き込みノードの 1 つであり、そのチャネルがバックプレッシャー状態でない限り、ロードは各 sink インスタンスの行をそのインスタンスが動作しているコンピュートノードに保持します。これにより、それらの行が本来必要としていたネットワークホップがなくなります。ローカルチャネルが満杯の間は他のノードへの分散にフォールバックするため、sink が単一インスタンスで動作するロード (例: Stream Load) でも 1 台のマシンに集中せず分散されます。`false` の場合、行は常に Tablet のノード間でラウンドロビンに分散されます。
* **デフォルト値**: true
* **型**: Boolean
* **スコープ**: Session
* **導入バージョン**: v4.2

### tablet_write_parallelism (v4.2 以降)

* **説明**: 共有データモードでのみ有効です。1 つのロードトランザクション内で**同一の** Tablet に並列で書き込めるコンピュートノードの数です。デフォルト値の `1` では、Tablet は割り当てられたコンピュートノードだけが書き込むため、Tablet が 1 つしかないパーティションはロード全体が 1 台のマシンに固定されます。`1` より大きい値を指定すると、その Tablet の行をその数のノードに分散できます。`-1` は Warehouse 内のすべての稼働中コンピュートノードを意味し、これは `tablet_write_local_first` が各 sink インスタンスの行を自ノードに保持するために必要な設定です。この変数は `file_bundling` が有効な `DUPLICATE KEY` のクラウドネイティブテーブルにのみ適用され、さらにパーティションの Tablet 数が Warehouse の稼働中コンピュートノード数より少ない間だけ有効です。バケットレベルの並列度でクラスタを埋められるようになると、この設定は自動的に適用されなくなります。
* **デフォルト値**: 1
* **型**: Int
* **スコープ**: Session
* **導入バージョン**: v4.2
