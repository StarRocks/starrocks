...
### enable_insert_partial_update（日本語）

* **説明**：Primary Key テーブルの INSERT 文に対して部分更新（Partial Update）を有効にするかどうかを指定します。`true`（デフォルト）に設定すると、INSERT 文がテーブル内のすべての非生成列ではなく一部の列のみを指定した場合、StarRocks は部分更新を実行し、指定された列のみを更新し、その他の既存値は保持します。`false` に設定すると、指定されていない列には既存値を保持せず、デフォルト値が使用されます。本機能は、Primary Key テーブルで特定の列のみを更新し、他の列に影響を与えたくない場合に特に有用です。
* **デフォルト値**：true
