...
* **説明**: データレイクテーブル（例: Iceberg テーブル）での Bucket-aware 実行のフォールバックを調整します。シャッフルが必要な集計クエリで、バケット分割されたテーブルを読み取る場合に使います。パーティションと述語のプルーニング後に残る推定バケット数が、この値と稼働中のワーカー数を掛けた数より少ない場合、オプティマイザはそのスキャンで Bucket-aware の単一ステージ集計を使いません。代わりに、通常のシャッフルを使うマルチステージ集計プランに切り替えて、並列度を回復します。例えば、バケットソース列の等価フィルタでクエリが単一バケットに絞られ、高カーディナリティ列でグループ化される場合に、深刻なスキューを防ぎます。`0` に設定すると、フォールバックは無効になります。値が大きいほど、より積極的にフォールバックします。この変数は、`enable_bucket_aware_execution_on_lake` が `true` の場合にのみ有効です。
* **デフォルト**: 1.0
* **タイプ**: Double
* **導入バージョン**: v4.2
