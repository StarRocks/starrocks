...
* **説明**: StarRocks が Paimon Global Index を使用する方法を制御します。有効な値は `0`、`1`、`2` です。`0` は Global Index を無効にし、通常の Paimon Split 計画を使用します。`1` は Paimon Java SDK が Split 計画時にインデックスを評価します。`2` は StarRocks が 2 段階の分散プランでインデックスを評価し、その結果をデータ Split の計画時に Paimon へ渡します。分散インデックスの評価に失敗した場合、StarRocks はクエリの意味を変更せず、通常の Paimon Split 計画にフォールバックします。
* **デフォルト**: 1
* **データ型**: Integer
* **導入バージョン**: v4.2
