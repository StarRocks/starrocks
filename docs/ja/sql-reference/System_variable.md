...
* **説明**: `enable_reduce_cast_varchar_length_inheritance` が有効な場合に、どの修復実装を優先するかを制御します。`true` の場合、FE は cast 削除後に既存の child `VARCHAR` 型の長さをその場で変更します。`false` の場合、child の type 参照を新しい `VARCHAR(N)` 型に置き換えます。現在は mutation パスがデフォルトの優先実装です。
* **デフォルト**: true
* **データ型**: Boolean
* **導入バージョン**: v3.5.16, v4.0.9
...
* **説明**: `ReduceCastRule` が同一型の `VARCHAR -> VARCHAR` cast を削除した後、再利用される planner `Expr` の `type` と `originType` を書き換え後の `VARCHAR(N)` に同期するかどうかを制御します。この変数は主に、type mutation パスを無効にした場合の代替修復経路として使用します。
* **デフォルト**: false
* **データ型**: Boolean
* **導入バージョン**: v3.5.16, v4.0.9
