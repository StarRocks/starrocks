...
* **説明**: Join ランタイムフィルターによる動的パーティションプルーニングを有効にするかどうか。shared-nothing 構成のみ対応します。有効にすると、BE は Join ランタイムフィルターに一致しないパーティションをスキップするか、それらのパーティションの以降のスキャンを停止します。通常の RANGE パーティションの最初の列と LIST パーティション列に対応し、Join のプローブは式や辞書エンコーディングで書き換えられていないパーティション列そのものである必要があります。
* **デフォルト**: `false`
* **データ型**: Boolean
