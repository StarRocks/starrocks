...
* **スコープ**: セッション  
* **説明**: CTEプロデューサーツリーのノード数に対する閾値を制御し、その閾値を超えた場合にCTEを完全に変換/インライン化する代わりに強制的に再利用する動作をトリガーします。オプティマイザはプロデューサーノード数（`cteContext.getCteNodeCount`から）を計測し、その数が設定された閾値を超えると RelationTransformer は既に登録されたCTE式を再利用する consume 演算子を作成します（プロデューサープラン全体を訪問して構築することを回避します）。これにより非常に大きなCTEプロデューサーツリーに対するオプティマイザ時間を短縮できます。値を 0 にすると強制再利用動作が無効になり、プロデューサーのサイズにかかわらず通常の変換/インライン化が強制されます。  
* **デフォルト**: 2000  
* **データ型**: int  
* **導入バージョン**: v3.5.3
...
* **説明**: セッション専用の文字列で、無効にするオプティマイザルールの enum 名をカンマ区切りで列挙します。各名前は RuleType enum 定数と完全一致（大文字小文字を区別）する必要があり、無効化できるのは名前が `TF_`（変換ルール）または `GP_`（グループ結合ルール）で始まるルールのみです。ステートメントアナライザ（`SetStmtAnalyzer.validateCboDisabledRules`）はリストを検証し、未知の名前や `TF_`/`GP_` でない名前を拒否します。実行時には `OptimizerOptions.applyDisableRuleFromSessionVariable` が `cbo_disabled_rules` を読み取り、`parseDisabledRules` が項目を分割・トリムし、有効な enum 名を RuleType に変換して対応するルールスイッチをクリアすることでオプティマイザがそれらのルールをスキップするようになります。未知の名前はオプティマイザの解析経路で警告ログを出して無視される場合がありますが、不正な値を SET しようとすると意味的エラーが発生します。ルールを無効にしないには空文字列を使用してください。例:
...
* **説明**: オプティマイザが有益と判断した場合に `LOGICAL_INTERSECT` の各入力ブランチにグローバル DISTINCT（集約）を挿入しようとする CBO の変換を有効にします。有効にすると、`IntersectAddDistinctRule` は各子に対して `LogicalAggregationOperator` (AggType.GLOBAL) を生成し、統計推定を呼び出して、推定される Distinct 行数を元の子の行数と `StatisticsEstimateCoefficient.MEDIUM_AGGREGATE_EFFECT_COEFFICIENT` を用いて比較します。集約は、推定された distinct 行数が正で元の行数より十分小さい場合（つまり estimatedAggRows > 0、originalRows > 0、および estimatedAggRows * coefficient <= originalRows）のみ保持されます。無効にすると、ルールは DISTINCT を挿入せずに元の `INTERSECT` を返します。これはセッションレベルのトグルで、`SessionVariable.isCboEnableIntersectAddDistinct()` / `setCboEnableIntersectAddDistinct(...)` を通じてアクセスします。
* **スコープ**: セッション
* **デフォルト**: `true`
* **データ型**: boolean
...
* **説明**: 文字列列に対して辞書エンコード（low-cardinality）表現を用いるようにジョインを書き換えるセッショントグル。有効にすると、オプティマイザ（`DecodeRewriter.visitPhysicalHashJoin` を参照）はジョインの ON 条件、投影、述語において高コストな文字列比較を避けるために文字列列を辞書 ID に置き換えようと試みます。コレクタ（`DecodeCollector.visitPhysicalJoin`）もこのフラグを参照して low-cardinality 列の結合等価グループを抽出するかを決定します。なおコレクタは現状ブロードキャスト型ジョインにのみこの最適化を適用し（両側が shuffle の場合はリライトをスキップします）、その点に注意してください。無効にした場合、ジョイン関連の low-cardinality リライトは無効化され、関係する ON 列にはリライトを防ぐマークが付与されます。また `SessionVariable.isEnableLowCardinalityOptimizeForJoin()` からも参照できます。
* **スコープ**: Session
* **デフォルト**: `true`
* **タイプ**: boolean
* **導入バージョン**: -
...
* **説明**: グローバルな DISTINCT 集約を二段階（ローカル + グローバル）集約に変換する際に、オプティマイザが使用するセッションレベルの閾値（行数上限）。`SplitTwoPhaseAggRule` の書き換え中に、クエリが LIMIT を持つ純粋な GROUP BY であり、クエリの LIMIT がこの変数より小さい場合、オプティマイザはその LIMIT をローカル集約段階に押し下げ（ローカル集約の limit をクエリの LIMIT に設定）してグローバル段階で処理するデータ量を削減します。具体的にはルールは `aggOp.getLimit() &lt; context.getSessionVariable().cboPushDownDistinctLimit()` と同等の条件をチェックし、非デフォルトの localAggLimit を設定するかどうかを決定します。小さい値はこのプッシュダウンを抑制し、大きい値はより積極的にします。使用箇所は `SplitTwoPhaseAggRule` を参照してください。
* **スコープ**: Session
* **デフォルト**: `4096`
* **データタイプ**: long
...
* **スコープ**: Session
* **説明**: オプティマイザが TopN オペレータを下位のプランノードにプッシュダウンする際に考慮する最大の TopN LIMIT。オプティマイザはこのセッション変数に対して TopN の LIMIT をチェックします（例: `PushDownTopNToPreAggRule`、`PushDownTopNBelowUnionRule`、`PushDownTopNBelowOuterJoinRule` のルール）。TopN に LIMIT がないか、またはその LIMIT がこの値より大きい場合はプッシュダウンは行われません。この変数は数値の LIMIT チェックのみを制御し、OFFSET の存在、集約結果への依存、結合/ユニオンの制約など他のルール条件は個々の変換ルールによって引き続き適用されます。この値を 0 に設定すると、通常の正の LIMIT に対する TopN プッシュダウンが事実上無効になります。
* **デフォルト**: `1000`
* **タイプ**: long
* **導入バージョン**: v3.2.0
...
* **説明**: セッションスコープのサーバー照合順序名。明示的な照合順序が指定されていない場合に、SQLレイヤーが文字列比較、ソート（ORDER BY）、パターンマッチ（LIKE）およびその他の照合に依存する操作で使用するデフォルトの照合順序を設定します。SessionVariable オブジェクト（`collationServer`）に格納され、`character_set_server`、`collation_connection`、`collation_database` と連携して動作します。許容される値は MySQL スタイルの照合識別子（例：`utf8_general_ci`）です。変更は現在のセッションに対するロケール依存の動作にのみ影響します。
* **スコープ**: Session
...
* **タイプ**: String
* **説明**: クエリプランナー/スケジューラが計算フラグメントを配置する際に選択できるノードタイプを制御します。有効な値は大文字小文字を区別せずに扱われます:
  * `compute_nodes_only` — フラグメントを compute ノードのみにスケジュールします（デフォルト）
  * `all_nodes` — compute ノードと BE/backend ノードの両方へのスケジューリングを許可します
  セッションの setter は値を検証します（SetStmtAnalyzer）; SET 文の実行時にサポートされていない値が指定されると SemanticException を発生させます。SessionVariable の setter も検証を行い、無効な名前の場合は IllegalArgumentException を投げます。有効な値は enum SessionVariableConstants.ComputationFragmentSchedulingPolicy にマップされ大文字で格納されます。getter は対応する enum を返し、格納値が欠落または不明な場合は `COMPUTE_NODES_ONLY` にフォールバックします。
* **導入バージョン**: v3.2.7
...
### custom_query_id (セッション)

* **説明**: 現在のクエリに外部識別子を紐付けるために使用されます。クエリ実行前に `SET SESSION custom_query_id = 'my-query-id';` を使って設定できます。値はクエリ終了後にリセットされます。この値は `KILL QUERY 'my-query-id'` に渡すことができます。監査ログでは `customQueryId` フィールドとして確認できます。
* **デフォルト**: ""
* **データ型**: String
* **導入バージョン**: v3.4.0
...
* **スコープ**: セッション
* **説明**: `true` に設定すると、コストベース最適化器はすべての結合リオーダリングフェーズをスキップします。具体的には、プランナーは `ReorderJoinRule`、結合変換ルール、外部結合変換ルール、または通常リオーダリングが許可されている場合に追加される結合の可換性展開を適用しません。`QueryOptimizer` と `SPMOptimizer` のコードパスはこのフラグをチェックし、`innerCrossJoinNode < `CBO_MAX_REORDER_NODE`` や外部結合のリオーダリング可否などの条件を評価する際にリオーダリングを保護します。最適化器に元の結合順序を保持させたい（デバッグ、安定したプラン、またはリオーダーが悪いプランを生む場合など）ときに使用します。`SessionVariable` に用意されている補助メソッドには `isDisableJoinReorder()`, `disableJoinReorder()`, `enableJoinReorder()`, `enableJoinReorder(boolean)` があります。
* **デフォルト**: `false`
* **データタイプ**: boolean
* **導入バージョン**: v3.2.0
...
* **スコープ**: セッション
* **説明**: カーディナリティ保存型の結合に対する CBO 駆動のテーブルプルーニングを有効にします。true に設定すると、オプティマイザ（QueryOptimizer と SPMOptimizer の両方）は、CBO プルーン結合タイプの結合ノード数が < 10 の場合に memo 最適化ルールセットへ CboTablePruneRule を追加します。このフラグは memo 最適化中にプルーニングルールを考慮するかどうかのみを制御し、プルーニングルールの内部しきい値や結合再順序の振る舞い自体は変更しません。RBO 版を有効にするには `enable_rbo_table_prune` を使用し、UPDATE 文でのプルーニングを制御するには `enable_table_prune_on_update` を使用してください。
* **デフォルト**: `false`
* **タイプ**: boolean
* **導入バージョン**: v3.2.0
...
* **スコープ**: セッション
* **説明**: ウィンドウ関数上で計算される DISTINCT 集約を、等価な null-safe-equality ジョインプランに変換するオプティマイザのリライトを制御します。有効にすると、QueryOptimizer は準備的なリライト（merge/separate project ルールと論理プロパティ導出）を実行し、DistinctAggregationOverWindowRule を適用して、プラン中に存在する DISTINCT aggregation-over-window パターン（LogicalWindowOperator が存在する場合）を、より効率的であったりさらなる最適化を可能にするジョインベースの実装に置き換えます。無効にすると、オプティマイザはこのリライトをスキップし、DISTINCT-over-window 式をそのままにします。この変数は、フレーム付きウィンドウ固有の最適化を制御する `optimize_distinct_agg_over_framed_window` と相互作用します。
* **デフォルト**: `true`
* **タイプ**: boolean
* **導入バージョン**: -
...
* **スコープ**: Session
* **説明**: 互換性のあるフラグメントを colocated 実行グループにまとめてシャッフルを削減し、colocated 実行を可能にするプランナー最適化を有効にします。有効な場合、PlanFragmentBuilder は ExecGroups を colocate グループとしてマークし、colocate join のために左右のグループをマージし、集約ステージに対して（ローカルシャッフルがない限り）group execution を有効にし、セット演算に対する colocate 挙動を設定します。この変数はプラン構築全体（例：aggregation、join、set-operation の処理）で参照されます。注意: ランタイムの適応 DOP が有効な場合、ランタイム DOP は join の probe が build を待つ必要があり group execution と競合するため、暗黙的に無効になります。関連するセッション設定: `group_execution_group_scale`, `group_execution_max_groups`, `group_execution_min_scan_rows`.
* **デフォルト**: `true`
* **タイプ**: boolean
...
* **スコープ**: セッション
* **説明**: true に設定すると、セッションはロードジョブ終了後にランタイムプロファイルの生成と収集を要求します。セッション変数はロードコーディネータによってチェックされます（例: StreamLoadTask は `coord.isEnableLoadProfile()` を使用して `collectProfile(false)` を呼ぶかどうかを判断します）。この変数はストリーム/ロード操作中のプロファイル関連の挙動を制御します（DefaultCoordinator、StreamLoadMgr、StreamLoadPlanner、QueryRuntimeProfile、StatisticUtils で使用されます）。このオプションを有効にすると FE はロードのために追加のプロファイリング情報を収集/アップロードし、追加の作業および I/O が発生します。これは他のプロファイル変数で制御される通常のクエリプロファイリングには影響しません。
* **デフォルト**: `false`
* **データタイプ**: boolean
* **導入バージョン**: v3.2.0
...
* **スコープ**: セッション
* **説明**: 有効にすると、最適化器は PredicateReorderRule を適用して、演算子の述語内にある結合条件（AND）を推定選択性に基づいて並べ替えます。ルールは子の統計情報から、あるいは PhysicalOlapScanOperator の場合はテーブル列統計から Statistics オブジェクトを構築し、DefaultPredicateSelectivityEstimator を使って各結合条件の選択性を推定します。より選択性の高い述語（推定選択性が低いもの）が先に評価されるよう結合条件をソートし、新しい複合 AND 述語を生成します。この書き換えは述語が `CompoundPredicateOperator` の場合にのみ実行され、`PredicateReorderRule.rewrite(...)` を `SessionVariable.isEnablePredicateReorder()` でガードして呼び出します。必要な統計が存在しないか述語が複合でない場合はリオーダーは行われません。`SessionVariable` は `isEnablePredicateReorder()`, `enablePredicateReorder()`, `disablePredicateReorder()` のアクセサを提供します。
* **デフォルト**: `false`
* **データタイプ**: boolean
* **導入バージョン**: v3.2.0
...
* **スコープ**: セッション
* **説明**: 有効にすると、クエリオプティマイザでカーディナリティ保存型の結合に対するルールベースのテーブルプルーニングを有効化します。QueryOptimizer は pruneTables() 内でこのフラグをチェックし、true でかつプルーニング可能な結合が存在する場合、パーティションプルーニング、プロジェクションマージ、`UniquenessBasedTablePruneRule`、ジョイン再順序（`enable_table_prune_on_update` が設定されている場合はオプションで `PrimaryKeyUpdateTableRule`）の一連のリライトルールを実行し、最後に `RboTablePruneRule` を適用して対象となる結合の不要なテーブル入力を削除または簡素化します。これを有効にすると、オプティマイザコンテキストでの結合同値導出（setEnableJoinEquivalenceDerive(false) の呼び出し）が無効化され、最適化時の結合述語／同値の導出方法が変わります。ルールベース（ツリーリライト）によるプルーニングがカーディナリティ保存型結合パターンに対してより良い結果を生むケース向けのオプションです。
* **デフォルト**: `false`
* **タイプ**: boolean
* **導入バージョン**: v3.2.0
...
* **スコープ**: Session
* **説明**: 対象となる単純な集約クエリを、ベースデータをスキャンせずにメタデータ（例: zonemap/カラム統計）に対する LogicalMetaScan + aggregation に書き換えるオプティマイザ変換を有効にします。有効にすると、RewriteSimpleAggToMetaScanRule は MIN/MAX/COUNT（および COLUMN_SIZE / COLUMN_COMPRESSED_SIZE）のようなクエリを MetaScan と SUM ベースの集約に置き換え、適切な DUPLICATE_KEY OLAP テーブルに対して I/O を大幅に削減することがあります。ルールは次のすべてが成立する場合にのみ適用されます:
  * 対象テーブルが DUP_KEYS で削除がないこと；
  * GROUP BY キーがなく、HAVING がなく、クエリレベルのフィルタがなく、LIMIT がないこと；
  * 使用される集約が MIN、MAX、COUNT（非 DISTINCT）、COLUMN_SIZE、または COLUMN_COMPRESSED_SIZE のみであること；
  * アグリゲータの引数が式を含まない単一のプリミティブカラムであり、zonemap インデックスが存在すること（min/max の場合、文字列/複雑型は対象外）；
  * パーティションプルーニングがなく、互換性のないスキーマ変更がないこと（共有データモードでスキーマ進化がある場合は min/max は無効化される）。
  注意: 一部のコンポーネント（例: ヒント付きのベースラインプランを生成する SPMPlanBuilder）は、安定したプラン生成を保証するために一時的にこの変数を無効にします。
* **デフォルト**: `true`
* **データタイプ**: boolean
* **導入バージョン**: v3.2.0
...
* **スコープ**: Session
* **説明**: aggregate 式 sum(expr +/- const) を sum(expr) +/- count(expr_other) * const に変換するオプティマイザのリライトを制御します。これは行ごとの projection コストを回避するための変換です。ルールは Aggregate が `SUM` である場合にのみ適用され、`DISTINCT` ではなく、DecimalV2 でも適用されません。SUM の引数に対する事前集約プロジェクションが少なくとも一つの定数オペランドを含む `ADD` または `SUBTRACT` 呼び出しである必要があります。リライタは以下を行います:
    * 基になる型が完全に互換性があり一致する SUM シグネチャが存在する場合、冗長なキャストを削除する、
    * 新しい集約を生成する（非定数部分に対する SUM と、乗算に必要な引数に対する COUNT）、
    * 書き換えられない元の集約は保持する。
  グループ化された集約に対しては、ユニークな集約関数の数が減らない（コストが増加する）場合は書き換えをスキップします。オプティマイザはリライトを試みる前に `context.getSessionVariable().isEnableRewriteSumByAssociativeRule()` をチェックします。
* **デフォルト**: `true`
* **データタイプ**: boolean
* **導入バージョン**: v3.2.0
...
* **スコープ**: セッション
* **説明**: 有効にすると、パイプライン実行エンジンが適格なプランフラグメントに対してランタイム適応型並列度（DOP）を有効にできるようになります。プラン構築中（PlanFragmentBuilder内）に、クエリキャッシュが使用されておらず `enable_runtime_adaptive_dop` が true の場合、`canUseRuntimeAdaptiveDop()` を報告するフラグメントには `enableAdaptiveDop()` が適用され、実行時に観測された実行メトリクスに基づいて DOP を調整できるようになります。この設定は `enable_pipeline_engine` も true の場合にのみ有効です。このオプションを有効にすると、自動的に `enable_pipeline_level_multi_partitioned_rf` が無効になります（setter がそのフラグをクリアします）。この変数の値は監査のためクエリのランタイムプロファイルに記録されます。
* **デフォルト**: `false`
* **タイプ**: boolean
* **導入バージョン**: v3.2.0
...
* **スコープ**: Session
* **説明**: システム変数 `innodb_read_only` として公開されるセッションスコープのブールフラグです。セッションオブジェクト（`SessionVariable` 内のフィールド `innodbReadOnly`）に格納され、`isInnodbReadOnly()` / `setInnodbReadOnly()` でアクセス可能です。この変数は、MySQL クライアントやツールとの互換性のためにセッションが InnoDB を読み取り専用として扱うべきかどうかを示します。StarRocks はここでストレージ層の強制を自動的に行うわけではなく、読み取り専用のセマンティクスを尊重するには他のコンポーネントが明示的にこのセッションフラグを確認する必要があります。
* **デフォルト**: `true`
* **タイプ**: boolean
* **導入バージョン**: v3.2.0
...
* **スコープ**: Session
* **説明**: オプティマイザで使用する low-cardinality 最適化の実装を選択するセッションレベルのトグル。有効にするとオプティマイザは `LowCardinalityRewriteRule` に実装された V2 リライトを適用します（このルールは decode 候補を収集し、decode/encode 演算子を使ってプランを書き換えます）。無効にするとオプティマイザはレガシーパス（`AddDecodeNodeForDictStringRule`）にフォールバックするか、V2 固有のリライトをスキップします。プランナーは `isEnableLowCardinalityOptimize()` とこの変数の両方をチェックして V2 リライトを実行するか判断します。セッションごとに `setUseLowCardinalityOptimizeV2` で変更可能です。
* **デフォルト**: `true`
* **データ型**: boolean
* **導入バージョン**: v3.3.0, v3.4.0, v3.5.0
...
* **範囲**: セッション
* **説明**: フロントエンドに、スキャンレンジにマークを付けてバックエンドがローカルディスクキャッシュをスキップするよう指示するセッションレベルのブール値です。`true` に設定すると、OlapScanNode（TScanRange/TInternalScanRange を構築する際）が BE に送る各スキャンレンジに `skip_disk_cache` フラグを設定し、バックエンドのストレージエンジンがそのクエリに対してローカルディスクキャッシュの読み取りや充填を回避します。これはセッションごとに適用され、そのセッションの後続スキャンに影響します。通常、キャッシュをバイパスして新鮮な読み取りを行うかキャッシュ汚染を避けるために `skip_page_cache` と併用されます。
* **デフォルト**: `false`
* **タイプ**: boolean
...
* **デフォルト**: `7`
* **タイプ**: int
* **説明**: スピルファイルに書き込まれるデータに適用されるエンコード/圧縮レベルを制御します。ビットの意味は `transmission_encode_level` と同じ規約に従います:
  * bit 2 (value 2) — 整数ライクな列に対して整数型エンコーディング(streamvbyte)を有効にする。
  * bit 4 (value 4) — バイナリ/文字列列に対して圧縮(lz4)を有効にする。
  * bit 1 (value 1) — 適応エンコーディングを有効にする（圧縮率に基づいて列ごとに決定）。
  解釈の例:
  * `7` (デフォルト) = 1|2|4: 整数列および文字列/バイナリ列を適応的にエンコードする（圧縮率 < 0.9 を基準に適応的に判断）。
  * `6` = 2|4: 整数および文字列/バイナリのエンコーディングを強制する（適応判断なし）。
  * `2` または `4`: それぞれ整数エンコーディングのみ、または文字列エンコーディングのみを選択的に有効にする。
  この変数はスピリングが有効なとき（`enable_spill` を参照）に意味を持ち、セッションから読み取られてスピルされたデータのエンコード方法を決定し、ディスク/IO コストを削減し CPU と IO のトレードオフを行うために使用されます。追加の実装詳細と根拠については `transmission_encode_level` を参照してください。
* **導入バージョン**: v3.2.0
...
* **説明**: データ交換（例：RPC／exchange）中にカラムごとに使用する転送エンコードを制御します。変数はビットマスクです:
  * ビット1（`1`）: 適応型エンコーディングを有効にする（測定された圧縮比に基づいてエンコーディングを選択）、
  * ビット2（`2`）: 整数および整数互換型を StreamVByte でエンコードする、
  * ビット4（`4`）: バイナリ／文字列カラムを LZ4 で圧縮する。
  例: `7` (1|2|4) は数値と文字列の両方に対して適応型エンコーディングを有効にし、エンコード比が &lt; 0.9 のときにエンコードを選択します。`6` (2|4) は数値と文字列のエンコードを強制します。JSON および object 型のカラムはまだサポートされていません。この設定はセッションごとの転送動作に影響し、`TRANSMISSION_ENCODE_LEVEL` として公開されます。
* **スコープ**: Session
* **デフォルト**: `7`
* **データタイプ**: int
* **導入バージョン**: v3.2.0
