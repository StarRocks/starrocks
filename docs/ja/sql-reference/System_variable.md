...
* **スコープ**: Session
* **説明**: builtin GIN 全文検索の `score()` 関連性ランキングで使う BM25 の長さ正規化パラメータ `b`。値が大きいほど長いドキュメントへのペナルティが強くなります。範囲は [0, 1] である必要があります。
* **デフォルト**: 0.75
* **データ型**: Double
* **導入バージョン**: -
...
* **スコープ**: Session
* **説明**: builtin GIN 全文検索の `score()` 関連性ランキングで使う BM25 の語頻度飽和パラメータ `k1`。値が大きいほど、繰り返される語がスコアを増加させ続けます。有限かつ非負の数である必要があります。
* **デフォルト**: 1.2
* **データ型**: Double
* **導入バージョン**: -
...
* **説明**: CHANGES クエリが読み取ったデータをコンピュートノードのキャッシュに書き込むかどうかを指定します。CHANGES クエリはテーブルの行レベルの変更をバージョン範囲にわたって返すため、テーブルの最新バージョンにはもう含まれていないデータも読み取ります。有効な値：
  * `auto`（デフォルト）：システムが判断します。現在は `always` と同じ動作です。
  * `always`：通常のクエリと同様にデータをキャッシュします。
  * `never`：データキャッシュを埋めずに読み取ります。大規模なバックフィルが通常のクエリの依存するデータをキャッシュから追い出すことを防ぎます。
* **スコープ**: Session
* **デフォルト**: `auto`
* **タイプ**: 文字列
* **導入バージョン**: v26.2
...
* **説明**: 非同期マテリアライズドビューの増分更新時に、マテリアライズドビュー全体ではなく、今回の更新差分で変更された基底テーブルのパーティションに対応するマテリアライズドビューのパーティションのみをスキャンするかどうか。クラウドネイティブな基底テーブルにのみ適用され、その他の基底テーブル、初回更新、および参照されない基底テーブルが変更された場合は、マテリアライズドビュー全体をスキャンします。`PROPERTIES ("session.enable_ivm_mv_partition_pruning" = "false")` で個別のマテリアライズドビューに、`SET GLOBAL` ですべてのマテリアライズドビューに設定できます。
* **デフォルト**: true
* **導入バージョン**: v26.2
...
* **説明**: JDBC Catalog が、データソース自身のシステムテーブルから読み取った列統計情報（列の異なる値の数、NULL の割合、平均列幅）をオプティマイザに渡すかどうかを指定します。有効な値は `true`（デフォルト）と `false` です。影響を受けるのはデータソースが実際に記述している列のみです。データソースが記述しなかった列は不明のままとなり、列統計情報をまったく報告しない方言（MySQL と ClickHouse は行数のみを報告します）は、従来どおり型に基づく比率推定を使い続けます。`false` に設定すると、すべての列がその推定に戻り、本機能の導入前の JDBC Catalog と同じ状態になります。この変数はテーブルの行数には影響しません。
* **デフォルト**: true
* **データ型**: Boolean

### enable_jdbc_runtime_filter_push_down

* **説明**: Join が構築したランタイムフィルタを、Build 側が生成した値の集合そのままの `IN` として、パラメータとしてバインドするのではなく SQL リテラルとして文に埋め込む形で JDBC Catalog スキャンのリモート SQL にプッシュダウンし、リモートのデータベースが一致しうる行だけを返すようにするかどうかを指定します。有効な値は `true` と `false` です。スキャンに届くのはローカル RF だけであるため、Join は JDBC テーブルを Probe 側に置いた Broadcast Hash Join である必要があります。Shuffle Join が構築するのはグローバル RF であり、プッシュダウンされません。Build 側の行数が `max_pushdown_conditions_per_column`（`be.conf` のデフォルトは `1024`）を超える場合は、ランタイムフィルタ自体が構築されません。すべての方言でサポートされる Join キーの型は `TINYINT`、`SMALLINT`、`INT`、`BIGINT`、およびリモートの元の型がテキスト型であると確認できる文字列カラムです（PostgreSQL では VARCHAR カラムにその確認は不要です）。さらに PostgreSQL に限り、`FLOAT` と `DATE` のカラム、およびリモートの元の型がそれぞれ `double precision`（`float8`）と `timestamp without time zone` であると確認できる `DOUBLE` と `DATETIME` のカラムもサポートされます。`timestamp with time zone` のカラムは決してプッシュダウンされません。StarRocks はそれをクエリのタイムゾーンにおける壁時計時刻として読み取るため、夏時間の巻き戻しで重複する 1 時間内の異なる 2 つの時刻が同じ壁時計時刻に潰れ、リモートの `IN` が返す行が同じにならずに少なくなってしまうからです。同じ理由で、集約・射影・Join のプッシュダウンが生成する派生テーブルではリモートの元の型が分からないため、`DOUBLE` と `DATETIME` のフィルタはローカルに残ります（`FLOAT` と `DATE` はそこでもプッシュダウンされます）。それ以外のキーではフィルタはローカルで実行されます。`TIME`、`CHAR`、`DECIMAL`、`JSON`、および精度を指定せずに宣言された PostgreSQL の `numeric` を StarRocks が DECIMAL(38,18) にマッピングしたカラムも、これに含まれます。基本多言語面 (BMP) の外側の文字を値に含む文字列カラムでも、フィルタはローカルに残ります。完成した文を JDBC bridge へ渡す部分は modified UTF-8 としてデコードされ、そのような文字はそこでは 6 バイトのサロゲートペアであるため、4 バイトの UTF-8 形式は途中で切り詰められて届き、一致する行が変わるだけでなく、リテラルの途中で文が終わってしまうこともあるからです。これは絵文字に限らず、人名の稀用字を含む CJK 拡張 B も該当します。値にバックスラッシュを含む文字列カラムでも、フィルタはローカルに残ります。値自身のシングルクォートは重ねることでエスケープしますが、それが正しいのはリモートがバックスラッシュを通常の文字として読む間だけであり、PostgreSQL の `standard_conforming_strings` と MySQL の `NO_BACKSLASH_ESCAPES` は BE からは見えないセッション設定だからです。スキャン自体が行数制限を持つ場合は、リモートでのフィルタリングは行われません。リモートの文は行数制限より先に WHERE を適用する一方、ローカルの計画は行数制限の後にフィルタを適用するため、プッシュダウンすると返る行数が減るのではなく、返る行そのものが変わってしまいます。これはパフォーマンス上の選択ではなく、正しさのための要件です。クエリの実際の動作を確認するには、FE がそのスキャンを許可したかどうかを示す `EXPLAIN VERBOSE` 出力の `RUNTIME FILTER PUSH DOWN` 行と、BE がリモートの文に実際に書き込んだ内容およびフィルタが破棄された理由を示す Query Profile の `PushdownRuntimeFilters` および `PushdownRuntimeFilterSkipped` を併せて確認してください。
* **デフォルト**: false
* **データ型**: Boolean
...
* **説明**: Tablet 内並列スキャンを使用する場合に、ベクタークエリの分割点が Segment 境界に揃うかどうかを指定します。ベクターインデックスは Segment ごとに検索されるため、Segment の内部で分割すると、生成された各サブタスクがそれぞれの行範囲に対して同じインデックス全体を再度検索することになります。この変数は分割点の位置を制約するものであり、どの分割方式を使うかを変えるものではありません。また、ベクターインデックスを使用しないクエリには影響しません。`false` に設定すると行数による分割に戻り、インデックス検索がサブタスクの数だけ繰り返されます。
* **デフォルト**: `true`
* **データ型**: Boolean
* **スコープ**: Session
...
* **説明**: 共有データモードでのみ有効です。ロードが 1 つの Tablet の書き込みを複数のコンピュートノードに分散できるかどうかを制御します。分散しない場合、すべての行はその Tablet が割り当てられた単一のノードへ送られます。3 つの値があります:
  * `off` — 決して分散しません。本機能が導入される前の動作です。
  * `auto` (デフォルト) — **range bucket** 分布のテーブルで、そのパーティションの Tablet 数が、このロードに見合う書き込みノード数 (`lake_multi_node_write_bytes_per_node` を参照) より少ない場合に分散します。pre-split はステートメントの計画前に実行され、その完了を待ってから進むため、ここで読まれる Tablet 数は pre-split の**結果**です: 十分に分割できていれば `auto` は介入せず、スキップされた場合や待機がタイムアウトして現在見えているレイアウトのままロードが続行された場合は `auto` が有効になります。**すべての key タイプに適用されます**: 同じ key を持つ行は key 列のハッシュでルーティングされるため 1 つのノードに到達し、結果はどのノードがどの行を書いたかに依存しません。ハッシュバケットのテーブルは、バケット数を増やせばクラスタを埋められるため対象外です。
  * `force` — 残りの前提条件が満たされる限り分散します。ハッシュバケット分布も含みます。`auto` が引き受けないテーブルでも分散させたい場合に使用します。
* モードにかかわらず、次の条件も必要です: `file_bundling` が有効なクラウドネイティブテーブル、combined txn log を使用するトランザクション、およびパーティションの Tablet 数が、このロードが実際に分散されるノード数より少ないこと。その幅は次の 3 つのうち最小のものです: ロードのサイズから求めたノード数 (`lake_multi_node_write_bytes_per_node` を参照)、`lake_multi_node_write_max_nodes`、稼働中のコンピュートノード数。すでにその数の Tablet を持つパーティションは `force` を指定しても単一ノードの書き込みのままです。6 ノードの Warehouse で Tablet が 5 つある場合、ロードのサイズが 3 ノード分の価値しかなければ分散しません。Stream Load と Routine Load は対象外です。どちらもプラン作成時にサイズが分からず、そこで未知のサイズを大きなサイズとして扱うと、最も小さく最も頻繁なバッチが最も広く分散されてしまうためです。部分更新 (行モードと列モード)、条件付き更新、自動インクリメント列を省略したロード、スキーマ変更中のロードはいずれもサポートされます。
* ロードの実行中に自動パーティション機能が作成したパーティションも分散されます。このパーティションはプランに含まれないため、その Tablet は後から create-partition RPC でロードへ渡されます。分散する幅はそのロードが計画時に決定したノード数をそのまま用い、ノード自体はパーティションが作成された時点で稼働しているコンピュートノードから選ばれます。長時間のロードは、計画時のノードリストより長く生き残ることがあるためです。新しいパーティションは分割の根拠となるデータ境界がまだ現れておらず、通常 Tablet を 1 つしか持ちません。つまり、まさにこの設定が対象とする形です。
* **デフォルト値**: auto
* **型**: String
* **スコープ**: Session
* **導入バージョン**: v4.2
...
* **説明**: **1 つの Tablet** を並列に書き込めるコンピュートノード数の上限です。Tablet のノードリストにある各ノードはそれぞれ自身の delta writer を開き、自身の segment を生成します。そのため非常に広い Warehouse では、通常規模のロードがその数だけ小さな segment に分割されてしまいます。この上限を超えたノードも自身の sink インスタンスは実行し、その行はネットワーク経由でリスト内のノードへ送られます。これは本機能が導入される前の動作であり、上限は局所性を犠牲にするだけで正しさには影響しません。これは上限であって並列度そのものではありません: 実際に使用される値は、この変数、ロードの推定サイズから算出されたノード数 (`lake_multi_node_write_bytes_per_node`)、稼働中コンピュートノード数の最小値です。`0` 以下に設定すると上限なしになります。
* **デフォルト値**: 6
* **型**: Int
* **スコープ**: Session
* **導入バージョン**: v4.2
...
* **説明**: 共有データモードでのみ有効で、`lake_multi_node_tablet_write_mode` が有効な場合にのみ作用します。Tablet の書き込み集合に次のノードを追加する前に、1 つのコンピュートノードへ割り当てるロードのバイト数です。ノード数はロードの推定サイズをこの値で割ったもので (整数除算のため、1 ノード分がまるごと確保できて初めてノードが追加されます)、最終的に使用される並列度は、その値、セッション変数 `lake_multi_node_write_max_nodes`、および稼働中コンピュートノード数の最小値になります。例えば 10 GB のロードはデフォルトの 2 GB で 5 ノードとなり、3 ノードの Warehouse ではさらに 3 に制限されます。書き込みの分散は無償ではありません: Tablet のノードリストにある各ノードはそれぞれ自身の segment を書き出し、自身の部分トランザクションログを生成します。さらに open/close は、そのノードが最終的に行を保持するかどうかにかかわらずリスト内のすべてのノードに到達します。したがって小さなロードを広く分散させると、ノードごとに segment とログが余分に増えるだけで得るものはわずかです。サイズは、`INSERT` ではオプティマイザの推定値から、Broker Load では解決済みのファイルリストから取得します。この 2 つの経路でどちらも利用できない場合、ノード数は `lake_multi_node_write_max_nodes` のみに委ねられます。Stream Load と Routine Load は例外で、そもそも分散しません。これらの経路でサイズが不明であることは、推定できなかった大きなロードではなく、マイクロバッチであることを意味するからです。サイズが不明であることは、サイズが小さいことを意味しないためです。0 に設定すると、サイズに基づく算出を無効にします。
* **デフォルト値**: 2147483648 (2 GB)
* **単位**: バイト
* **型**: Long
* **スコープ**: Session
* **導入バージョン**: v4.2
