...
* **デフォルト**: false
* **データ型**: boolean
* **説明**: `true` の場合、パーセンタイルのマテリアライズドビュー書き換えでは、クエリが要求する圧縮パラメータより低い値で作成されたダイジェストを使用しません。同じビューまたは別のビューの適切なダイジェストを使用するか、ベーステーブルにフォールバックします。`false` の場合、低い圧縮パラメータの候補も使用できますが、要求を満たす候補を優先します。拒否された等価式は MV trace に記録されます。
