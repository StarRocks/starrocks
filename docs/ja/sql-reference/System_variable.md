...
* **デフォルト**: false
* **型**: Boolean
* **説明**: グローバル遅延マテリアライゼーションを有効にし、一部のカラムの読み取りをクエリ実行の後の段階まで遅延させます。`SET enable_global_late_materialization = true` で明示的に有効にできます。アップグレード中に異なる Iceberg 行ロケーターを使用するバージョンが混在する場合は、無効のままにしてください。このデフォルト値は、既存のセッション設定や永続化されたグローバル設定を上書きしません。
