...
* **説明**: Paimon データテーブルのクエリで、実験的な paimon-cpp ネイティブ Reader を使用するかどうかを制御します。この Reader は条件を満たす Paimon の `DataSplit` スキャン範囲と、`WITH_PAIMON_CPP` を有効にしてビルドされた BE でのみ使用されます。Paimon システムテーブル、external path を使用するファイル、未対応の Split タイプやスキーマ（`TIME`、`MULTISET`、および精度が 0、3、6、9 以外のタイムスタンプを含む）、および paimon-cpp を含まない BE は引き続き JNI Reader を使用します。`paimon_force_jni_reader` も有効な場合は JNI Reader が優先されます。
* **デフォルト**: false
* **データ型**: Boolean
* **導入バージョン**: v4.2
