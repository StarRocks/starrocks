...
* **説明**: 現在のクエリに外部識別子を紐付けるために使用されます。クエリ実行前に `SET SESSION custom_query_id = 'my-query-id';` のように設定できます。クエリ終了後に値はリセットされます。この値は `KILL QUERY 'my-query-id'` に渡すことができます。監査ログでは `customQueryId` フィールドとして確認できます。
* **デフォルト**: ""
* **データタイプ**: String
* **導入バージョン**: v3.4.0
...
* **スコープ**: セッション
* **説明**: ColocatedBackendSelector がバケットシーケンスをバックエンドに割り当てる際に使用するバケットシーケンス走査アルゴリズムを制御します。値が <= 0 の場合、ColocatedBackendSelector は常に NormalBucketSequenceIterator を使用します。そうでない場合、FE は次を計算します:
  - numTotalBuckets（各バケットの第1レプリカのレプリカ数の合計）
  - backends.size()（異なるバックエンドの数）
  - hasMultiReplications（任意のバケットが >1 レプリカを持つかどうか）
  hasMultiReplications が true かつ numTotalBuckets <= maxBucketsPerBeToUseBalancerAssignment * backends.size() のとき、ColocatedBackendSelector は BalancerBucketSequenceIterator に切り替え、バックエンド間でより均等な割り当てを生成します。BalancerIterator はスキューを軽減しますが CPU コストが高く、useBackend() が O(numBucketsPerBe) となるため、バックエンドあたりのバケット数が十分小さい場合にのみこのアルゴリズムが選択されます。詳細は ColocatedBackendSelector.createBucketIterator および BalancerBucketSequenceIterator を参照してください。
* **デフォルト**: 6
* **データタイプ**: int
* **導入バージョン**: 3.2.0
