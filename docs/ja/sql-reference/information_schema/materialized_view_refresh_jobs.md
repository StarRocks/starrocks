...
| REFRESH_TRIGGER                    | リフレッシュがトリガーされる方法。マテリアライズドビューのリフレッシュスキームから導出されます。有効な値: `MANUAL`、`SCHEDULED`、`ON_BASE_TABLE_CHANGE`、`NONE`。マテリアライズドビューが削除されている場合は `UNKNOWN`。 |
| REFRESH_MODE                       | マテリアライズドビューに設定されたリフレッシュモード。有効な値: `AUTO`、`PCT`、`INCREMENTAL`。マテリアライズドビューが削除されている場合は `NULL`。 |
| IMV_SOURCE_VERSION_RANGE           | インクリメンタルリフレッシュが消費したソースバージョン範囲の JSON。該当しない場合は `{}` を返します。 |
| IMV_SOURCE_TIMESTAMP_RANGE         | インクリメンタルリフレッシュが消費したソースタイムスタンプ範囲の JSON。該当しない場合は `{}` を返します。 |
| IMV_SOURCE_PINNED_SNAPSHOT_ID_MAP  | インクリメンタルリフレッシュで固定されたソーススナップショット ID の JSON。該当しない場合は `{}` を返します。 |
| FAILED_TASK_RUN_ID                 | ジョブ内で失敗した task run の ID。失敗した task run がない場合は `NULL`。 |
