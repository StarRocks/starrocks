...
:::warning
呼び出しは入力データをクラスター外に送信し、プロバイダー料金が発生する場合があります。承認済みの関数ファミリーと Provider のみをクエリユーザーに付与してください。Provider 管理には引き続き SYSTEM OPERATE が必要で、既存のテーブル、列、ビューの権限チェックも適用されます。
:::
...
各物理実行プランは、プロバイダーが最初に必要になった時点で設定を取得し、同じプラン内のそのプロバイダーへのすべての呼び出しで再利用します。認可は最適化前に解決済みのすべての AI 呼び出しを検査します。実行設定を取得するのは物理プランに残る AI 呼び出しだけであり、設定をプランに取り込む前に実際のプロバイダーの UUID で再度認可します。後の ALTER や DROP は取得済みの設定を変更せず、実行中のクエリもキャンセルしません。再試行時の再計画や次のプリペアド EXECUTE を含む、新しく構築されたプランは現在のメタデータを取得します。
...
既存のタイプごとのデフォルトプロバイダー機構は変更しません。デフォルトを設定しても SYSTEM `ai_complete`、[テキスト補助関数](ai_text_functions.md)、[ai_embed](ai_embed.md) のルーティングは変わりません。既存のオーバーロード、FE 設定、BE ローカル認証情報は維持され、SYSTEM の明示的な `model` 引数は引き続きリモートモデル名を表します。
