...
各物理実行プランは、プロバイダーが最初に必要になった時点で設定を取得し、同じプラン内のそのプロバイダーへのすべての呼び出しで再利用します。オプティマイザーによって除去された呼び出しでは、プロバイダーメタデータを解決しません。後の ALTER や DROP は取得済みの設定を変更せず、実行中のクエリもキャンセルしません。再試行時の再計画や次のプリペアド EXECUTE を含む、新しく構築されたプランは現在のメタデータを取得します。
