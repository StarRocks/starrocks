...
Hive Bitmap UDF は、Hive で直接使用できる UDF を提供します。これらは Bitmap データを生成し、Bitmap 関連の計算を行うために使用できます。

Hive Bitmap UDF で定義された Bitmap フォーマットは StarRocks のフォーマットと一致しており、StarRocks に Bitmap データをロードしたり、StarRocks から Hive に Bitmap データをアンロードしたりする際に直接使用できます。

適用シナリオ:

- 生データの量が多く、これらのデータを直接 StarRocks にロードして計算すると、StarRocks クラスターに大きな負担がかかります。望ましい解決策は、Hive で Bitmap データを生成し、それを StarRocks にロードすることです。
- StarRocks で生成された Bitmap データを他のシステムで使用するために Hive にエクスポートします。

サポートされているソースおよびターゲットデータ型:

- v3.1 以降では、String、Base64、Binary のデータ型のロードおよびアンロードをサポートします。
- v2.5 および v3.0 では、String および Base64 データのロードおよびアンロードのみをサポートします。
...
### Hive で Bitmap を生成し、StarRocks に Binary フォーマットでロードする
...
4. Bitmap データを StarRocks に異なる方法でロードします。
...
3. 異なる方法でデータをエクスポートします。
   ...
   - INSERT INTO FILES を使用してデータをエクスポートします (Binary フォーマット)。
   ...
   - [Hive Catalog](../../data_source/catalog/hive_catalog.md) を使用してデータをエクスポートします (Binary フォーマット)。
