...
指定された期間内のユーザーリテンション率を計算します。この関数は1から31の条件を受け入れ、各条件が真であるかどうかを評価します。条件が真と評価されると、1が返されます。それ以外の場合は0が返されます。最終的に0と1の配列を返します。このデータに基づいてユーザーリテンション率を計算できます。
