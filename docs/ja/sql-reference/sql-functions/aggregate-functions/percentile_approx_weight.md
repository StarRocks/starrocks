...
description: "指定されたパーセンタイルの重み付き近似値を計算します。"
---
...
`compression` には、評価結果が整数となる定数式を指定します。`5000`、`5000.0`、`CAST(5000 AS DOUBLE)`、`2500 * 2` を使用できます。`5000.5` などの小数部分を持つ値や定数でない式は、丸めずにエラーとします。引数を省略した場合、`NULL` を指定した場合、または整数が [2048, 10000] の範囲外の場合は、デフォルト値 `10000` を使用します。
