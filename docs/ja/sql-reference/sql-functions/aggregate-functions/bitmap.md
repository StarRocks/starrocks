...
ここでは、Bitmap におけるいくつかの集計関数の使用例を示します。関数の詳細な定義やその他の Bitmap 関数については、bitmap-functions を参照してください。
...
`BITMAP_UNION_COUNT (expr)`: 入力された Bitmap の和集合を計算し、その基数を返します。これは BITMAP_COUNT (BITMAP_UNION (expr)) と同等です。BITMAP_UNION_COUNT 関数を最初に使用することをお勧めします。これは BITMAP_COUNT (BITMAP_UNION (expr)) よりも性能が良いためです。
