...
- `datetime`: 切り捨てる時間で、DATETIME または DATE 型である必要があります。日付と時間が存在しない場合、NULL が返されます。例えば、`2021-02-29 11:12:13` は存在しない日付であるため、NULL が返されます。

- `fmt`: 日付部分で、`datetime` がどの精度まで切り捨てられるかを示します。値は VARCHAR 定数でなければなりません。

  `fmt` は以下の表に記載されている値に設定する必要があります。値が正しくない場合、エラーが返されます。

  `datetime` が DATE 値の場合、`fmt` は `year`、`quarter`、`month`、`week`、または `day` のみ設定可能です。他の日付単位、例えば `hour` に設定するとエラーが報告されます。例8を参照してください。
