...
指定された曜日 (DOW) が入力日 (DATE または DATETIME) の後に初めて発生する日付を返します。例えば、`next_day('2023-04-06', 'Monday')` は '2023-04-06' の後に発生する次の月曜日の日付を返します。

この関数は v3.1 からサポートされています。[previous_day](./previous_day.md) の逆です。
...
```Plain
-- 2023-04-06 の後に発生する次の月曜日の日付を返します。2023-04-06 は木曜日で、次の月曜日の日付は 2023-04-10 です。
