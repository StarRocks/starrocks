...
- `from_tz`: 元のタイムゾーン。VARCHAR がサポートされています。タイムゾーンは 2 つの形式で表現できます。1 つはタイムゾーンデータベース（例: Asia/Shanghai）、もう 1 つは UTC オフセット（例: +08:00）です。
...
タイムゾーンデータベースについては、 [List of tz database time zones](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) （Wikipedia）を参照してください。
