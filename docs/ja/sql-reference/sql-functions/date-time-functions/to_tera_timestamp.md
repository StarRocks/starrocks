...
- `str`: 変換する時刻の表現。VARCHAR 型である必要があります。

- `format`: `str` の時刻フォーマット指定子。入力文字列を解析し変換するために使用されます。`format` は `string` と一致する必要があります。一致しない場合、NULL が返されます。`format` が無効な場合、エラーが返されます。
