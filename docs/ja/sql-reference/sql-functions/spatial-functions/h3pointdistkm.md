...
description: "2つの地理座標点間のHaversine（大圏）距離をキロメートル単位で返します。"
---

# h3PointDistKm

緯度と経度（度単位）で指定した2つの地理座標点間のHaversine（大圏）距離をキロメートル単位で返します。
...
2点間の大圏距離（キロメートル）をDOUBLEで返します。いずれかの引数がNULLの場合はNULLを返します。
