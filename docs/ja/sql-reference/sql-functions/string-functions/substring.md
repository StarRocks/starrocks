...
指定された位置から文字を抽出し、指定された長さのサブ文字列を返します。
...
-- 一致する文字が 5 つあります。長さ 9 は一致する文字の長さを超えており、一致するすべての文字が返されます。
