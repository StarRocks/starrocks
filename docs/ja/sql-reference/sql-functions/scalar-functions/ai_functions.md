...
テキスト埋め込みは [ai_embed](ai_embed.md)、登録済み AI プロバイダーによるチャットと埋め込みは [AI プロバイダー関数](ai_custom_functions.md)を参照してください。デフォルト AI プロバイダーを設定しても補助関数の SYSTEM ルーティングは変わらず、明示的な `model` 引数はリモートモデル名を選択します。
