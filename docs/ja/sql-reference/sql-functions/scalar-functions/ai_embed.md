...
`dimensions` を含む指定オプションはプロバイダーとモデルがサポートする必要があります。SYSTEM 設定ではなく登録済み AI プロバイダーを選択する場合は、[ai_custom_embedding](ai_custom_functions.md) を使用してください。デフォルト AI プロバイダーを設定しても `ai_embed` のルーティングは変わらず、明示的な `model` 引数はリモートモデル名を選択します。
