...
| パラメータ | デフォルト | 要件 |
|------------|------------|------|
| [`ai_default_chat_endpoint`](../../../administration/management/FE_parameters/user_query_loading.md#ai_default_chat_endpoint) | 空文字列 | 必須。チャット補完エンドポイントの完全な HTTPS POST URL。 |
| [`ai_default_chat_model`](../../../administration/management/FE_parameters/user_query_loading.md#ai_default_chat_model) | 空文字列 | prompt のみのオーバーロードでは必須。すべての呼び出しでモデルを明示する場合は省略可能。 |
| [`ai_default_chat_provider`](../../../administration/management/FE_parameters/user_query_loading.md#ai_default_chat_provider) | 空文字列 | 必須。有効な値は `openai_compatible` のみ。 |
...
- 独立した task timeout は task の作成後に固定され、retry ごとに再開されません。クエリのキャンセルと
  ...
  [BE AI 関数パラメータ](../../../administration/management/BE_parameters/query_loading.md#ai_function_request_timeout_ms)
  を参照してください。
