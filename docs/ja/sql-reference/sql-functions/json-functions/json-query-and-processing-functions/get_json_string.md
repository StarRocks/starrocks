...
- `json_path`: JSON パス。サポートされているデータ型は VARCHAR です。`json_path` は `$` で始まり、`.` をパスセパレータとして使用します。`[ ]` は配列の添字として使用され、0 から始まります。例えば、`$."my.key"[1]` は要素 `my.key` の2番目の値を取得することを示します。
