...
MAP 値の要素数を返します。MAP はキーと値のペアの無順序コレクションです。例えば、`{"a":1, "b":2}` のように表されます。1つのキーと値のペアが1つの要素を構成し、例えば `{"a":1, "b":2}` には2つの要素が含まれています。
...
クラスタ内に [Hive catalog](../../../data_source/catalog/hive_catalog.md#create-a-hive-catalog) が作成された後、この catalog と map_size() 関数を使用して、`col_map` 列の各行の要素数を取得できます。
