...
MAP 値の要素数を返します。MAP はキーと値のペアの無順序コレクションで、例えば `{"a":1, "b":2}` のようなものです。1 つのキーと値のペアが 1 つの要素を構成し、例えば `{"a":1, "b":2}` は 2 つの要素を含みます。

この関数にはエイリアス [cardinality()](cardinality.md) があります。v2.5 からサポートされています。
