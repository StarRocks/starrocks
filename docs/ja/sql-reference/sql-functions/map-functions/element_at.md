...
マップのキーと値のペアから指定されたキーに対応する値を返します。入力パラメータがNULLの場合、またはキーがマップに存在しない場合、結果はNULLです。
...
- `any_map`: 値を取得するためのMAP式。
- `any_key`: マップ内のキー。

## 戻り値

`any_key` が `any_map` に存在する場合、そのキーに対応する値が返されます。それ以外の場合はNULLが返されます。
