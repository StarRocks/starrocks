...
- `map_filter(lambda_func, any_map)`
  ...
  `lambda_func` を `any_map` 内のキーと値のペアに一つずつ適用し、結果が `true` であるキーと値のペアを返します。
...
マップ値のキーまたは値が NULL の場合、NULL は通常の値として処理されます。
...
次の例では、[map_from_arrays()](map_from_arrays.md) を使用してマップ値 `{1:"ab",3:"cdd",2:null,null:"abc"}` を生成します。その後、各キーと値のペアが `array<boolean>` に対して評価され、結果が `true` であるペアが返されます。
...
次の例では、map_from_arrays() を使用してマップ値 `{1:"ab",3:"cdd",2:null,null:"abc"}` を生成します。その後、各キーと値のペアが Lambda 式に対して評価され、値が null でないキーと値のペアが返されます。
