...
もし `x` がカラムの場合、この関数はまず `x` の値を昇順にソートし、パーセンタイル `y` に対応する値を返します。
...
- `y`: パーセンタイル。サポートされるデータ型は DOUBLE です。値の範囲: [0.0,1.0]。
...
`aggregate_tbl` テーブルを作成し、`percent` カラムを percentile_approx_raw() の入力とします。
