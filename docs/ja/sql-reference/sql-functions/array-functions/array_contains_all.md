...
`arr1` が `arr2` のすべての要素を含んでいるか、つまり `arr2` が `arr1` の部分集合であるかどうかを確認します。もしそうであれば、1 が返されます。そうでなければ、0 が返されます。
...
`arr2` が `arr1` の部分集合である場合、1 が返されます。そうでない場合、0 が返されます。
