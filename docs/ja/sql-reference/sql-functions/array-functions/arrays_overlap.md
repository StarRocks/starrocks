...
2つの配列の交差が空であるかどうかを確認します。指定する配列は同じデータ型でなければなりません。交差が空の場合、この関数は `0` を返します。それ以外の場合、この関数は `1` を返します。
...
- `input1`: 比較したい2つの配列のもう1つ。このパラメータの値は、`input0` パラメータの値と同じデータ型でなければなりません。
