...
- `input`: スライスを切り取りたい配列。この関数は、次のタイプの配列要素をサポートします: BOOLEAN, TINYINT, SMALLINT, INT, BIGINT, LARGEINT, FLOAT, DOUBLE, VARCHAR, DECIMALV2, DATETIME, DATE, および JSON。**JSON はバージョン 2.5 からサポートされています。**
...
Example 1: 3 番目の要素から 2 つの要素を切り取ります。
...
Example 2: 最初の要素から 2 つの要素を切り取ります。
...
Example 4: 3 番目の要素から 5 つの要素を切り取ります。
