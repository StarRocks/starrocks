...
列内の値（`NULL` を含む）を配列に集約し（複数行を1行に）、特定の列で要素を並べ替えることができます。バージョン3.0から、array_agg() は ORDER BY を使用して要素をソートすることをサポートしています。
...
- `col0`: `col` の順序を決定する列。ORDER BY 列は複数指定可能です。

- `[desc | asc]`: `col0` の昇順（デフォルト）または降順で要素をソートするかを指定します。

- `[nulls first | nulls last]`: NULL 値を最初または最後に配置するかを指定します。

## 戻り値

`col0` によってオプションでソートされた ARRAY 型の値を返します。

## 使用上の注意

- 配列内の要素の順序はランダムであり、ORDER BY 列が指定されていない場合、またはソートされていない場合、列内の値の順序と異なる可能性があります。
...
例3: WHERE 句を使用して列 `pv` の値を集約します。`pv` 内のデータがフィルタ条件を満たさない場合、`NULL` 値が返されます。
