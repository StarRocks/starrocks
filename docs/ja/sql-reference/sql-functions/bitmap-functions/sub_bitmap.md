...
BITMAP 値 `src` から、`offset` で指定された位置から始まる `len` 要素を抽出します。出力される要素は `src` の部分集合です。

この関数は主にページネーションされたクエリのようなシナリオで使用されます。v2.5 からサポートされています。
...
- `offset`: 開始位置。BIGINT 値である必要があります。`offset` を使用する際の注意点は以下の通りです:
  - オフセットは 0 から始まります。
  - 負のオフセットは右から左に数えます。例 3 と 4 を参照してください。
  - `offset` で指定された開始位置が BITMAP 値の実際の長さを超える場合、NULL が返されます。例 6 を参照してください。
- `len`: 取得する要素の数。1 以上の BIGINT 値である必要があります。マッチする要素の数が `len` の値より少ない場合、すべてのマッチする要素が返されます。例 2、3、7 を参照してください。
...
例 2: オフセットを 0 に設定して、BITMAP 値から 100 の要素を取得します。100 は BITMAP 値の長さを超えているため、すべてのマッチする要素が返されます。
...
例 3: オフセットを -3 に設定して、BITMAP 値から 100 の要素を取得します。100 は BITMAP 値の長さを超えているため、すべてのマッチする要素が返されます。
