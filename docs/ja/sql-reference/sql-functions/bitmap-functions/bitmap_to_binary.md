...
Parquet のようなバイナリファイルに直接データをエクスポートする場合、この関数を推奨します。
...
1. `AGGREGATE KEY` が (`page_id`, `visit_date`) である集計テーブル `page_uv` を作成します。このテーブルには、値が集計される BITMAP 列 `visit_users` が含まれています。
