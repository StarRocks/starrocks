...
この関数は、正規表現パターンに一致するターゲット値の最初の一致する部分文字列を返します。パターンに一致する項目を pos で抽出します。パターンは str の一部に完全に一致する必要があり、関数はパターン内で一致する必要がある部分を返します。一致するものが見つからない場合、空の文字列を返します。
...
:::tip
同じ結果は、最初の数字のセットをマッチンググループ `()` で囲まず、最初のグループを返すことで得られます:
