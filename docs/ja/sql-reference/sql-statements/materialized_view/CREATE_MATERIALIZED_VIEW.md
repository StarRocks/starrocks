...
- `excluded_refresh_tables`： このプロパティにリストされているベーステーブルまたはビューは、データが変更されてもマテリアライズドビューに更新されません。フォーマット `db_name.]table_name`。デフォルト値は空文字列です。値が空文字列の場合、ベーステーブルのデータが変更されると、対応するマテリアライズドビューが更新されます。

  :::note
  `excluded_refresh_tables` にリストされているベーステーブルまたはビューは、`CREATE OR REPLACE VIEW` や `DROP` + `CREATE` などで再構築された場合も、依存するマテリアライズドビューの強制リフレッシュを**トリガーしません**。マテリアライズドビューは非アクティブ状態になりますが、可視バージョンマップ (visible version map) は保持されるため、次回のリフレッシュで全パーティションが書き換えられることはありません。`excluded_refresh_tables` の設定を無視してマテリアライズドビューを強制的にリフレッシュするには、`REFRESH MATERIALIZED VIEW <mv_name> FORCE` を使用します。
  :::
