...
非同期マテリアライズドビューの分散方式。StarRocks はハッシュバケット法とランダムバケット法（v3.1 以降）をサポートしています。共有データモードで `enable_range_distribution` と `enable_mv_range_distribution` の両方が有効な場合、このパラメータを省略するとレンジ分散が選択されます。それ以外の場合、増分で維持されないマテリアライズドビューはランダムバケット法を使用し、StarRocks がバケット数を自動的に設定します。増分で維持されるマテリアライズドビュー（`refresh_mode` が `INCREMENTAL`、または定義が増分リフレッシュをサポートする `AUTO`）には、異なる分散ルールが適用されます。詳細は、[インクリメンタルマテリアライズドビュー](#インクリメンタルマテリアライズドビュー)を参照してください。
...
- `partition_refresh_strategy`：単一のリフレッシュ操作中のマテリアライズドビューのリフレッシュ戦略。このプロパティが `adaptive` に設定されている場合、ベーステーブルのパーティション内のデータ量に基づいてリフレッシュするパーティション数が自動的に決定され、リフレッシュ効率が大幅に向上します。このプロパティが指定されていない場合、デフォルト戦略は `strict` であり、単一の操作でリフレッシュされるパーティション数は `partition_refresh_number` によって厳密に制御されます。`force` に設定すると毎回すべてのパーティションがリフレッシュされます。`EFFECTIVE_REFRESH_MODE` が `INCREMENTAL` または `AUTO` のマテリアライズドビューには適用されません。これらは定義上ベーステーブルの変更から増分で維持されるためです。`refresh_mode` `AUTO` で作成されたが定義を増分で維持できず `PCT` に確定したビューには、`force` が引き続き適用されます。
...
- `refresh_mode`: マテリアライズドビューの更新方法を制御します。StarRocks v4.1 で導入され、Iceberg の追記（append-only）テーブルでのみサポートされています。有効な値：
  ...
  - `AUTO`: 可能な限り増分リフレッシュを行います。増分で維持できないベーステーブルの変更が発生した場合、そのリフレッシュは `PCT` リフレッシュにフォールバックし、次回のリフレッシュで増分リフレッシュに自動的に復帰します。定義上まったく増分リフレッシュができない場合、そのマテリアライズドビューは `PCT` マテリアライズドビューとして作成されます。
...
- `refresh_mode` を調整する際には次の制限があります：
  - マテリアライズドビューのリフレッシュモードは、どの方向にも変更できません。変更するには、マテリアライズドビューを再構築する必要があります。すでに設定されているモードを指定することは許容され、何も起こりません。
- `refresh_mode` が `INCREMENTAL` または `AUTO` のマテリアライズドビューでは、パーティションを指定したリフレッシュも `FORCE` リフレッシュもサポートされません。いずれかを試みると例外が発生します。この制限は設定値を読むため、定義上増分リフレッシュができず `PCT` として作成された `AUTO` のビューにも適用されます。

#### 分散方式

増分で維持されるマテリアライズドビュー（`refresh_mode` が `INCREMENTAL`、または定義が増分リフレッシュをサポートする `AUTO`）には、次の分散ルールが適用されます。

- `distribution_desc` を省略した場合、共有データモードで `enable_range_distribution` と `enable_mv_range_distribution` の両方が有効なときに限り、StarRocks はレンジ分散を使用します。それ以外の場合は、ターゲットのすべてのキー列によるハッシュ分散にフォールバックします。
...
- レンジ分散のパーティションは最初は 1 つのタブレットしか持たないため、初回リフレッシュは 1 ノードだけで書き込まれることになります。そこで StarRocks は、そのリフレッシュが書き込むタブレットを事前分割します。境界はデータをサンプリングせず、内部の Row ID 列から導出されます。これが適用されるのは、ビューの Row ID がストレージエンジンによって生成され、かつそのビューに対して Row ID がまだ 1 つも払い出されていない場合（新規作成したビューの初回リフレッシュがこれに該当します）で、さらにそのリフレッシュが単一パーティションのみを書き込む場合です。これはベストエフォートの最適化です。それ以外の場合はリフレッシュは事前分割なしで実行され、その理由は `starrocks_fe_tablet_pre_split_eligibility_skipped` に記録されます。無効にするには `enable_tablet_pre_split_for_mv_refresh` を `false` に設定します。

#### ソートキー

増分で維持されるマテリアライズドビュー（`refresh_mode` が `INCREMENTAL`、または定義が増分リフレッシュをサポートする `AUTO`）は、内部の Row ID 列を主キーとする主キーテーブルです。そのため `ORDER BY` は主キーの一部になるのではなく、独立したソートキーを定義します。
...
インクリメンタルリフレッシュは、ベーステーブルへの追記（append-only）操作のみをサポートします。`INCREMENTAL` では、`UPDATE`、`MERGE`、`OVERWRITE` などのサポートされていない操作が実行されるとリフレッシュが失敗します。`AUTO` では、そのうち増分プランを構築できないものはそのリフレッシュが `PCT` にフォールバックします。フォールバックが及ばない範囲は上記の `refresh_mode` の説明を参照してください。
...
インクリメンタルリフレッシュは各ロードが生成した変更を読み取るため、前回のリフレッシュ成功以降にベーステーブルが生成したすべてのバージョンは、いずれかのリフレッシュがそれを消費するまでベーステーブル上に保持されます。消費されたバージョンは解放されます。

保持されたバージョンはストレージを消費するため、共有データクラスタ内の StarRocks ベーステーブルでこのように保持されるバージョンには保持上限があります：[`bookmark_reference_max_ttl_ms`](../../../administration/configuration/FE_parameters/shared_lake_other.md#bookmark_reference_max_ttl_ms)（デフォルトは 3 日）。その上限内にリフレッシュが成功しなかったマテリアライズドビューは、次のリフレッシュで読み取るはずだったバージョンを失い、インクリメンタルリフレッシュを継続できなくなります：

- `INCREMENTAL` では、リフレッシュが失敗し、マテリアライズドビューは inactive になります。復旧するにはドロップして再作成してください。
- `AUTO` では、リフレッシュが `PCT` の完全リフレッシュにフォールバックし、それ以降のリフレッシュは再びインクリメンタルになります。

マテリアライズドビューがリフレッシュに成功できない期間は、いずれも保持上限より短く保ってください：

- スケジュールされたリフレッシュ間隔を上限より短く設定する。
- 手動リフレッシュの間隔を上限より短くする。
- 手動で inactive にしている期間を上限より短くする。
- リフレッシュの失敗を上限内に解消する。
