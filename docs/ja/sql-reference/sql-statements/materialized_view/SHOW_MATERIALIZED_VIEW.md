...
| last_refresh_mode_reason_table | モードの決定を引き起こしたベーステーブルです（`catalog.database.table` 形式）。単一のベーステーブルに起因しない場合は空です。BE が変更の読み取り中に報告する理由はテーブルではなく tablet を指すため、FE が自力でその tablet を単一のベーステーブルまで絞り込める場合にのみ設定されます。`CHANGE_CAPTURE_DISABLED` は、このリフレッシュが変更を読む必要があった主キーのベーステーブルからしか発生せず、該当するベーステーブルがちょうど 1 つのときにその名前が入ります。 |
