...
| name                       | マテリアライズドビューの名前。                           |
| refresh_type               | マテリアライズドビューのリフレッシュタイプ。ROLLUP、MANUAL、ASYNC、INCREMENTAL を含む。 |
| is_active                  | マテリアライズドビューの状態がアクティブかどうか。 有効な値: `true` および `false`。 |
...
| last_refresh_state         | 最後のリフレッシュのステータス。PENDING、RUNNING、FAILED、SUCCESS を含む。 |
| last_refresh_force_refresh | 最後のリフレッシュが FORCE リフレッシュかどうか。         |
