...
バージョン3.0以降、このステートメントの名前は SHOW MATERIALIZED VIEW から SHOW MATERIALIZED VIEWS に変更されました。
...
例2: 名前をマッチングしてマテリアライズドビューを表示します。
