...
| pause_on_fatal_parse_error          | NO | 回復不能なデータ解析エラーが発生した場合にジョブを自動的に一時停止するかどうかを指定します。有効な値は `true` と `false` です。デフォルト値は `true` です。このパラメータは v3.4.2 以降でサポートされています。<br/>このような解析エラーは、通常、不正なデータ形式によって引き起こされます。例えば：<br /><ul><li>JSON 配列をインポートする際に `strip_outer_array` を設定していない場合。</li><li>JSON データをインポートする際に、Kafka メッセージが不正な JSON（例：`abcd`）である場合。</li></ul> 注 以前のバージョンでは、このようなエラーが発生するとリトライが繰り返されていましたが、v3.4.2 以降にアップグレードすると、リトライから自動的にジョブを一時停止する動作に変更され、動作に変化が生じます。|
