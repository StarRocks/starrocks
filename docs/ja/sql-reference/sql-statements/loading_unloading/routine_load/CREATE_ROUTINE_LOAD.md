...
**必須**: いいえ\
**説明**: JSON が不正な Kafka メッセージに遭遇した際、ロードタスクを失敗させる代わりに BE がそのメッセージをスキップするかどうかを指定します。有効な値: `true` と `false`。デフォルト値: `false`。<br />無効の場合、そのメッセージを読むすべてのタスクが `parse error` で失敗し、オフセットは進みません: `pause_on_fatal_parse_error` が `false` ならジョブは `RUNNING` のままコンシューマーラグが無限に増え、`true` なら一時停止します。有効にすると BE はそのメッセージ 1 件だけを破棄し、途切れた文書の壊れる前までのフィールドも含めて何も取り込みません（非圧縮 JSON のみ。唯一の例外は 1 つの chunk より大きい数千行のメッセージで、欠陥が見つかる前に次の段階へ渡された先頭の行は取り戻せません）: フィルターされた 1 行として数えられ、topic・partition・offset とともにエラーログに書かれ、BE の `WARNING` として記録され、BE メトリクス `starrocks_be_routine_load{type="skipped_malformed_messages"}` に加算されます。同じバッチの他の行は通常どおりロードされ、オフセットは不正メッセージを越えて進みます。<br />注意:<ul><li>不正な JSON の 2 つの形はどちらもスキップされ、それぞれ正確に 1 行のフィルター行として数えられます: まったく JSON でないメッセージ（例: `abcd`。行を読む前に拒否されます）と、行を読んでいる途中で初めて文書が途切れていると分かるメッセージ（例: `{"a": 1, "b": }`。失敗した行とその元のメッセージは 2 回ではなく 1 回数えられます）。</li><li>検出されるのは実際に読み取られる JSON の欠陥だけです: 文書そのもの、`json_root` で選ばれたオブジェクト、`strip_outer_array` 設定時の各要素。それ以外の場所の構造的な欠陥（例: `json_root` オブジェクトの隣にある兄弟フィールド）はパーサーには見えず、読み取られる部分の中でもパーサーは読み取り中に実際に遭遇した問題しか報告しません（例: ネストした `json_root` オブジェクト内の欠けた値は、形の異なるオブジェクトとして通ることがあります）。そのメッセージはこのオプションがない場合とまったく同じように取り込まれます。</li><li>スキップされたメッセージはエラー閾値に加算されますが、閾値ごとに挙動が異なります。`max_error_number` はジョブ全体の累計で、超過した時点のタスクはコミットされたうえでジョブが一時停止します。`max_filter_ratio` はタスクごとに判定され、超過するとそのタスクは失敗し、何もコミットされず、同じオフセットから再試行されます。そのためこのオプションを使うジョブでは `max_filter_ratio` を高く（例: `1.0`）保ち、`max_error_number` をサーキットブレーカーとして使ってください。</li><li>`pause_on_fatal_parse_error` と併用できます。BE が先にスキップするため、後者は解析エラーがなお FE に到達した場合（このオプションを持たない BE など）にのみ作用します。</li><li>Stream Load、Broker Load、圧縮ストリームには影響せず、不正な JSON では従来どおり失敗します。</li></ul>
