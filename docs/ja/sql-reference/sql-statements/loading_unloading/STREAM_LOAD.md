...
| fill_default_on_absent_key | いいえ | 行のJSONオブジェクトにその列のキーが存在しない場合に、その列が何を読み込むかを指定します。有効な値：`true` および `false`。デフォルト値：`false`。<br />デフォルトでは、そのような列は `NULL` として読み込まれ、その列の `DEFAULT` を上書きします。このパラメータを `true` に設定すると、代わりにその列の `DEFAULT` が読み込まれます。キーが存在し、その値が `null` である場合は、明示的に指定された値とみなされ、このパラメータの設定に関わらず `NULL` として読み込まれます。<br />**注意**<br />このパラメータは `format` が `json` の場合にのみ適用されます。また、`partial_update` と同時に使用することはできません。部分更新では、読み込みが指定しなかった列は既存の値のまま保持されるためです。<br />定数の `DEFAULT` は補完されます。`ARRAY<INT> DEFAULT [1, 2]` のような複雑な定数や、`current_timestamp()`、`now()`、`uuid()`、`uuid_numeric()` の各デフォルト関数も同様です。次の場合は補完されず、既存の `NULL` の動作が維持されます：`DEFAULT` を持たない列、`DEFAULT` がそれ以外の式である列、`AUTO_INCREMENT` 列、`columns` 内のマッピング式で計算される列、この読み込みがテキストとして読み取る `JSON`、`ARRAY`、`MAP`、`STRUCT` 列（`columns` 内のマッピング式から参照される場合、生成列の式から読み取られる場合、またはその列のスキーマ変更が進行中の場合に発生します。これらの既定値には忠実なテキスト表現が存在しないためです）、主キーテーブル、ユニークキーテーブル、集計テーブルのキー列（キー列を補完すると、そのキーが欠落しているすべての行が同じキーを持つことになり、行同士がマージされてしまうためです）、および集計テーブルで `REPLACE_IF_NOT_NULL` と宣言された列（キーが存在しないことでその列が既存の値を保持するためです）。重複キーテーブルのキーはソートキーにすぎないため、他の列と同じように補完されます。<br />`jsonpaths` と同時に使用することはできません。パスの解決では、キーが存在しない場合と、キーが存在して値が `null` である場合を区別できないため、その経路では補完を行えません。そのため、黙って何もしないのではなく、読み込み自体がエラーになります。<br />このパラメータは、この読み込みが指定するすべての列（上記の補完されない列を除く）に適用されます。`columns` を指定しない場合は、テーブルスキーマ全体が対象になります。<br />FEとBEの両方がこのパラメータをサポートしている必要があります。サポートしていないバージョンはエラーを報告せずに無視するため、実際に有効になったかどうかは、読み込み結果に `"FillDefaultOnAbsentKey": true` が含まれているかどうかで確認できます。この確認フィールドは、通常の Stream Load とトランザクション Stream Load でのみ返されます。Merge Commit の読み込みと、channel を使用するトランザクション Stream Load では返されません。これらの経路では、確認情報の取得元となる plan の結果を受け取らないためです。いずれの場合もパラメータ自体は有効で、単に返されないだけです。|
