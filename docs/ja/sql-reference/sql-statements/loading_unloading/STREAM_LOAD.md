...
- `Expect`ヘッダーフィールドを追加し、その値を`100-continue`（例：`"Expect:100-continue"`）に指定できます。これにより、ジョブリクエストが拒否された場合の不要なデータ転送を防ぎ、リソースのオーバーヘッドを削減できます。このヘッダーフィールドはオプションです。L7プロキシ（例：nginx）がこのフィールドを削除する場合がありますが、ヘッダーがなくても受け入れられます。
