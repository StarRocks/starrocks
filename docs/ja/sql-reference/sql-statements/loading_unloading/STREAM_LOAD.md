...
-H "partial_update_mode: row | column | flexible_row"
...
| partial_update_mode | いいえ | 部分更新のモードを指定します。有効な値: `row`、`column`、および `flexible_row`。<ul><li>値 `row`（デフォルト）は行モードでの部分更新を意味し、多くの列と小さなバッチによるリアルタイム更新に適しています。</li><li>値 `column` は列モードでの部分更新を意味し、少ない列と多くの行によるバッチ更新に適しています。このようなシナリオでは、列モードを有効にすることでより高速な更新速度が得られます。たとえば、100列のテーブルで、全行に対して10列（全体の10%）のみを更新する場合、列モードの更新速度は10倍速くなります。</li><li>値 `flexible_row`（実験的機能）はフレキシブル部分更新を意味します。JSON 形式のロードの各行は、その行に含まれる列だけを更新し、行に含まれない列は現在の値を保持します。新しい主キーを挿入する行では、含まれない列に列のデフォルト値が使用されます。行に値 `null` で含まれる列は `NULL` に更新されます。ロードは行モードで実行され、行全体が書き換えられます。`partial_update` を `true` に設定し、各行に含まれる可能性のあるすべての列を `columns` に指定してください。</li></ul>**注意**<br/>値は大文字と小文字が区別されます。<br/>`flexible_row` は、共有データクラスタの主キーテーブルへの Stream Load でのみサポートされ、FE 設定項目 `enable_flexible_partial_update` と同名の BE 設定項目の両方が `true` に設定されている場合にのみ使用できます。それ以外の場合、ロードは失敗します。クラスタのすべての BE と CN がフレキシブル部分更新をサポートするバージョンにアップグレードされた後に有効にしてください。それより前のバージョンのノードは、このようなロードを通常の部分更新として実行し、含まれない列を `NULL` に更新します。<br/>`flexible_row` は、`merge_condition`、`jsonpaths`、または式を含む `columns` マッピングと併用できず、範囲分散テーブル、ソートキー（ORDER BY）に主キー以外の列を含むテーブル、AUTO_INCREMENT 列または生成列を持つテーブル、`__cset__` という名前の列を持つテーブル、トランザクション Stream Load、およびマージコミットではサポートされません。値 `flexible`（列モードに対応する値）は予約済みで、まだサポートされていません。<br/>1 回のロードに主キーが同じ行が複数ある場合、そのキーの各列には、それらの行のうちその列を含む最後の行の値が使用されます。削除行（`__op` = 1）はそれより前の同じキーの行を破棄し、削除行がそのキーの最後の行である場合は、そのキーが削除されます。 |
