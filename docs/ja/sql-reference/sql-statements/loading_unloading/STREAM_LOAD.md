...
| partial_update_mode | いいえ | 部分更新のモードを指定します。有効な値: `row`、`column`、`auto`、`flexible`、`flexible_row`。<ul><li>値 `row`（デフォルト）は行モードでの部分更新を意味し、多くの列と小さなバッチによるリアルタイム更新に適しています。</li><li>値 `column` は列モードでの部分更新を意味し、少ない列と多くの行によるバッチ更新に適しています。このようなシナリオでは、列モードを有効にすることでより高速な更新速度が得られます。たとえば、100列のテーブルで、全行に対して10列（全体の10%）のみを更新する場合、列モードの更新速度は10倍速くなります。</li><li>値 `auto`（実験的機能）は、更新する列の幅とワークロードの読み書き特性に基づいて、StarRocks がロードごとに行モードまたは列モードを自動的に選択することを意味します。共有データ（shared-data）主キーテーブルに対する JSON 形式の部分更新では、`auto` は行ごとに異なる列集合の更新（下記の `flexible` を参照）も有効にします。行ごとの列集合を適用できない場合（shared-nothing テーブル、AUTO_INCREMENT 列を持つテーブル、`merge_condition` の指定、または式を含む `columns` マッピング）、`auto` は従来どおり 1 回の同質な部分更新として動作します。</li><li>値 `flexible`（実験的機能。共有データ/クラウドネイティブの主キーテーブルのみ）は、JSON 形式の部分更新において各行が異なる列の部分集合を更新すること（異種部分更新）を許可します。行に含まれない列は既存の値を保持します。このモードは列モード（スパース）の書き込みパスで適用されます。</li><li>値 `flexible_row`（実験的機能。共有データのみ）は `flexible` と同じ行ごとの異種更新セマンティクスを提供しますが、行モード（行全体の書き換え）で適用されます。</li></ul>**注意**<br/>`auto`、`flexible`、`flexible_row` は実験的機能です。<br/>`flexible` と `flexible_row` は共有データ（クラウドネイティブ）主キーテーブルでのみサポートされます。`merge_condition` や `columns` パラメータ内の式と同時に使用することはできず、AUTO_INCREMENT 列を持つテーブルではサポートされません。<br/>部分更新が GIN（転置）インデックスを持つ列に及ぶ場合、StarRocks はインデックスの整合性を保つため、その更新を自動的に行モードで適用します。<br/>`column`/`auto`/`flexible` の基盤となるスパース書き込みパスはデフォルトで無効であり、BE 設定項目 `enable_sparse_dcg` で制御されます。 |
