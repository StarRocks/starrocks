...
| **フィールド** | **説明**                                                   |
| -------------- | ----------------------------------------------------------- |
| UserIdentity   | ユーザーの権限を照会する際に表示されるユーザーの識別情報。   |
| RoleName       | ロールの権限を照会する際に表示されるロール名。               |
| Catalog        | カタログ名。<br />GRANT 操作が StarRocks 内部カタログで行われた場合は `default` が返されます。<br />GRANT 操作が external catalog で行われた場合は、その external catalog の名前が返されます。<br />`Grants` 列に表示される操作がロールの割り当てである場合は `NULL` が返されます。 |
| Grants         | 特定の GRANT 操作。                                         |
