...
バージョン v2.5.4 以降では、SHOW CREATE VIEW を使用して、**マテリアライズドビュー** を作成するために使用された文を照会できます。
...
| ------------- | ------------ | ------------------------------------------------------------ |
| db_name       | No           | データベース名。このパラメータが指定されていない場合、現在のデータベース内の指定されたビューの CREATE VIEW 文がデフォルトで返されます。 |
| view_name     | Yes          | ビュー名。                                                   |
