...
| PROPERTIES      | データスナップショットのプロパティ。有効なキー:<ul><li>`type`: バックアップタイプ。現在、完全バックアップ `FULL` のみがサポートされています。デフォルト: `FULL`。</li><li>`timeout`: タスクのタイムアウト。単位: 秒。デフォルト: `86400`。</li><li>`ttl`: スナップショットを保持する期間。`<数値> <単位>` の形式で指定し、単位は `SECOND`、`MINUTE`、`HOUR`、`DAY`、`WEEK`、`MONTH`、`YEAR` のいずれかです（例: `7 DAY`）。このキーを指定しない場合、スナップショットは永久に保持されます。v4.2.0 以降でサポートされます。</li></ul> |
...
`ttl` プロパティは、BACKUP ジョブが作成するスナップショットを保持する期間を指定します。バックアップの完了時に絶対的な有効期限へ変換され、作成元クラスタの ID とともにスナップショット自身へ書き込まれます。いずれも [SHOW SNAPSHOT](./SHOW_SNAPSHOT.md) で確認できます。

スナップショットは有効期限を過ぎると自動的に削除されます。有効期限を持つのは `ttl` を指定したスナップショットだけなので、`ttl` を指定せずに取得したバックアップが削除されることはありません。自動クリーンアップを完全に停止するには、FE 設定項目 `enable_backup_snapshot_auto_clean` を `false` に設定します。自動クリーンアップが削除するのは、それを実行しているクラスタ自身が作成したスナップショットのみであるため、リポジトリを共有する複数のクラスタが互いのスナップショットを削除することはありません。v4.2.0 より前に作成されたスナップショットにはクラスタ ID が記録されていないため、自動的に削除されることはありません。

作成済みスナップショットの保持期間は変更できません。有効期限より前に削除するには [DROP SNAPSHOT](./DROP_SNAPSHOT.md) を使用してください。

保持ポリシーは v4.2.0 以降でサポートされます。
