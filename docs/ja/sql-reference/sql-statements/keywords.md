...
SQL ステートメント内のキーワード、例えば `CREATE` や `DROP` は、StarRocks によって解析される際に特別な意味を持ちます。キーワードは、予約されていないキーワードと予約キーワードに分類されます。

- **予約されていないキーワード** は、特別な処理をせずに識別子として直接使用できます。例えば、テーブル名やカラム名です。例えば、`DB` は予約されていないキーワードです。`DB` という名前のデータベースを作成できます。
