...
| LastStartTime         | BE ノードが最後に起動した時間。                              |
...
| UsedPct               | BE ノード内で使用されているストレージ容量の割合。            |
| MaxDiskUsedPct        | BE ノード内で使用されているストレージ容量の最大割合。        |
...
| UnhealthyTabletNum    | データ再配置中に未完了（不健康）なタブレットの数。           |
| InconsistentTabletNum | データベース内の不一致なタブレットの数。                     |
| CloningTabletNum      | データベース内でクローンされているタブレットの数。           |
| ErrorStateTabletNum   | Primary Key タイプのテーブルで、エラー状態のタブレットの数。 |
| ErrorStateTablets     | Primary Key タイプのテーブルで、エラー状態のタブレットの ID。|
...
| Port           | Broker ノードの Thrift サーバーポート。このポートはリクエストを受け取るために使用されます。 |
| Alive          | Broker ノードが生存しているかどうか。                        |
| LastStartTime  | Broker ノードが最後に起動した時間。                          |
...
| **Return** | **Description**                                  |
| ---------- | ------------------------------------------------ |
| Item       | `cluster_balance` のサブコマンド項目。 <ul><li>cluster_load_stat: クラスターの現在のロード状況。</li><li>working_slots: 現在利用可能な作業スロットの数。</li><li>sched_stat: スケジューラの現在の状態。</li><li>priority_repair: 優先されるタブレット修復タスクの数。</li><li>pending_tablets: 処理待ちのタブレットの数。</li><li>running_tablets: 現在修復中のタブレットの数。</li><li>history_tablets: 過去に修復されたタブレットの総数。</li></ul>         |
...
Example 14: クラスター内のデータレプリケーションタスクの情報を表示します。
