...
| IP                    | BE ノードの IP アドレス。                                   |
| HeartbeatPort         | BE ノードのハートビートサービスポート。                       |
...
| SystemDecommissioned  | BE ノードが退役しているかどうか。                            |
| ClusterDecommissioned | BE ノードがクラスター内で退役しているかどうか。         |
...
| UnhealthyTabletNum    | データ再配布中に未完了 (不健康) なタブレットの数。 |
| InconsistentTabletNum | データベース内の不一致タブレットの数。              |
...
| LastStartTime  | Broker ノードが最後に起動された時間。              |
