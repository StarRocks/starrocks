...
| HttpPort              | BE ノードの HTTP サーバーポート。ウェブページ経由で BE ノードにアクセスするために使用されます。 |
...
| SystemDecommissioned  | BE ノードが退役しているかどうか。退役する前に、BE ノードはデータを他の BE ノードに移行します。移行中もデータロードとクエリは影響を受けません。<ul><li>`true`: クラスタは BE ノードを退役としてマークしました。データは正常に移行されたか、移行中です。</li><li>`false`: BE ノードは稼働中です。</li></ul> |
...
| Status                | BE ノードがタブレット数を FE ノードに報告した最後の時刻。JSON 形式で表示されます。 |
| DataTotalCapacity     | `DataUsedCapacity` + `AvailCapacity` に相当します。データファイルが占有するストレージ容量と BE ノードの利用可能なストレージ容量の合計を示します。 |
