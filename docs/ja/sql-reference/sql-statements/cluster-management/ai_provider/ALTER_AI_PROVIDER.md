...
| `SET (...)`     | マージする `"key" = "value"` のペア。許可されるキーはプロバイダーの `TYPE` と同じです ([`CREATE AI PROVIDER`](./CREATE_AI_PROVIDER.md) を参照)。`protocol` は変更できますが、新しい値はプロバイダーの `TYPE` で許可されているものでなければなりません。 |
