...
- `embedding` — OpenAI 互換の `/v1/embeddings` エンドポイント。セマンティックコンテキストモジュールが
  `CONTEXT UPSERT` のコンテンツや `query_text` 検索を埋め込む際と、[ai_custom_embedding](../../../sql-functions/scalar-functions/ai_custom_functions.md) で使用されます。
...
[ai_custom_query](../../../sql-functions/scalar-functions/ai_custom_functions.md) と [ai_custom_embedding](../../../sql-functions/scalar-functions/ai_custom_functions.md) は現在 `openai` プロトコルを必要とします。これらの関数に他の登録済みプロトコルを指定すると、クエリの計画時にエラーになります。
