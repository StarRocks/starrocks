...
返される実行プランの詳細レベルは次のとおりです: EXPLAIN LOGICAL < EXPLAIN < EXPLAIN VERBOSE < EXPLAIN COSTS。

ほとんどの場合、EXPLAIN を使用するだけで十分です。EXPLAIN VERBOSE と EXPLAIN COSTS は、多くの内部情報を出力し、主に実行プランのデバッグに使用されます。
...
| COSTS         | カラム統計を含む詳細な論理実行プランを表示します。 |
