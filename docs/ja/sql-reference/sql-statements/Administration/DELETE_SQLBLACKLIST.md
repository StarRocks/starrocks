...
`sql_index_number`: ブラックリスト内の SQL 正規表現のインデックス番号です。複数のインデックス番号はカンマ (,) とスペースで区切ります。インデックス番号は [SHOW SQLBLACKLIST](../Administration/SHOW_SQLBLACKLIST.md) を使用して取得できます。
