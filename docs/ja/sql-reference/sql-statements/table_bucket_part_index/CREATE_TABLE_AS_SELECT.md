...
| column_name       | いいえ       | 新しいテーブルの列名です。列のデータ型を指定する必要はありません。StarRocks が自動的に適切なデータ型を指定します。StarRocks は FLOAT と DOUBLE データを DECIMAL(38,9) データに変換します。また、CHAR、VARCHAR、STRING データを VARCHAR データに変換します。CHAR および VARCHAR 列は宣言された長さを保持し、STRING は最大長の VARCHAR に変換されます。 |
