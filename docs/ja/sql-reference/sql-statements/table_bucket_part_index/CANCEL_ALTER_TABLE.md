...
- `{COLUMN ｜ OPTIMIZE | ROLLUP}`
  ...
  - `ROLLUP` が指定された場合、このステートメントはrollup index の追加または削除操作をキャンセルします。
...
4. 現在のデータベースにある `example_table` の特定のロールアップ変更を、ジョブIDを使ってキャンセルします。

   :::tip

   ロールアップのジョブIDは、[`SHOW ALTER MATERIALIZED VIEW`](../materialized_view/SHOW_ALTER_MATERIALIZED_VIEW.md) で確認できます。
