...
Engine タイプが `olap` のテーブルでは、大きなテキスト列や JSON 列を選んで ZSTD で圧縮し、それ以外の列は元の圧縮方式のままにできます。行同士の重複が非常に多い列(会話履歴、定型的なペイロードなど)を対象としており、こうした列では ZSTD の効果がテーブル全体を切り替える場合よりはるかに大きくなります。

次の制限があります:

- 指定できるのは CHAR、VARCHAR、STRING、JSON 型の列のみです。
- 指定できるのは値列のみで、キー列を指定するとエラーになります。
- 同じ列をリスト内に重複して指定することはできません。
- 指定された列は ZSTD で圧縮され、テーブルレベルの `compression` 設定を上書きします。それ以外の列は一切影響を受けません。
...
列名の後ろに、その列専用のデータページサイズを指定することもできます:
...
ページは解凍の単位であるため、ページを大きくして得られる効果は、その列の 1 行の
サイズがページに対してどれだけ大きいかで決まります。行がページより大きい列では、
既定の 64 KB では行をまたぐ冗長性がまったく窓に入らず、大きなページにすると数倍
小さくなることがあります。一方、1 ページに数百行が入る列では、その冗長性はすでに
ページ内で捉えられているため効果はほとんどなく、ポイントルックアップのたびに
大きなページ全体を解凍する分だけ損をします。これがサイズを列ごとに指定する理由です。
サイズを書かなかった列は BE の既定値 (`data_page_size`) のままです。
指定できる範囲は 4 KB から 1 MB です。上限をここに置いたのは、1 MB までならポイント
ルックアップのコストが抑えられる（1 行の読み取りが 0.1 ミリ秒程度）ためで、これを超えると
圧縮率の伸びよりコストの伸びが上回ります。

内部ディクショナリが実際にその列に対して構築されるかどうかは、このプロパティではなく
データが決めます:

- その列が最終的に plain エンコードになる必要があります。StarRocks はデータからエンコードを
  選ぶため、異なる値が少ない列はディクショナリエンコードになります。その方がここでのどの手段
  よりもよく圧縮されます。そうした列にこのプロパティを設定しても効果はありません。
- 行数が 256 以下の CHAR/VARCHAR/STRING 列は無条件にディクショナリエンコードになるため、
  非常に小さなテーブルでも内部ディクショナリは作られません。JSON 列にこの行数の下限はありません。
  もともと plain エンコードだからです。
- 1 つの Segment 内でその列が 9 ページを超えるデータページを埋める必要があります。最初の
  1 ページはディクショナリの採取元で、続く 8 ページで効果を実測します。この 9 ページは
  いずれもディクショナリなしで書かれ、10 ページ目から実際に使われます。そこまで届かない列は
  ディクショナリを使いません。それが正しい答えです。ディクショナリ自身が 1 ページを占めるため、
  数ページに対しては元が取れません。
- その 8 ページが、ディクショナリなしより明確に小さくなる必要があります。既定では 1 割以上で、
  `zstd_compression_dict_min_gain` で決まります。そうでなければディクショナリは破棄され、
