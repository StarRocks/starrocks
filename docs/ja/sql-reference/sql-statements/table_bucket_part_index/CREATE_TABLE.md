...
内部ディクショナリが実際にその列に対して構築されるかどうかは 2 つの条件で決まり、どちらも
このプロパティでは制御できません:
...
`zstd_compression_dict_pages_written` と `zstd_compression_dict_bytes` を見れば、
ディクショナリが実際に生成されているかどうかが分かります。

このプロパティはテーブル作成後に `ALTER TABLE ... SET ("zstd_compression_columns" = "...")` で変更することもできます。新しい設定はそれ以降に書き込まれるデータに適用されます。既存の Segment は書き換えられず、書き込まれた時点のエンコードのまま残ります。各 Segment は自身がどう書かれたかを記録しているため、どちらのエンコードも問題なく読み取れます。既存データは Compaction など別の理由で書き換えられたときに初めて新しいエンコードになります。
