...
Engine タイプが `olap` のテーブルでは、大きなテキスト列や JSON 列を共有 ZSTD ディクショナリで圧縮するように指定できます。StarRocks はデータファイル (Segment) ごと、列ごとに小さなディクショナリを 1 つ構築し、その列のすべてのページをそれに対して圧縮します。これにより、繰り返し現れる短い断片 (JSON のキー名、フィールド名、定型的な文言など) が 64 KB のページごとではなく、Segment 全体で 1 回だけエンコードされます。
...
- 指定された列は ZSTD で圧縮され、テーブルレベルの `compression` 設定を上書きします。
- BE 設定項目 `enable_compression_dict` が `true` である必要があります。ローリングアップグレードの安全のため既定値は `false` で、`false` の間は指定された列はテーブルに設定された圧縮方式をそのまま保持し、ディクショナリは書き込まれません。アップグレード済みのクラスタで有効にする前に、BE 設定リファレンスを参照してください。
...
このプロパティはテーブル作成後に `ALTER TABLE ... SET ("compression_dict_columns" = "...")` で変更することもできます。新しい設定はそれ以降に書き込まれるデータに適用されます。既存の Segment は書き換えられず、書き込まれた時点のエンコードのまま残ります。Segment がディクショナリを持つかどうかは Segment 自身に記録されているため、どちらのエンコードも問題なく読み取れます。既存データは Compaction など別の理由で書き換えられたときに初めて新しいエンコードになります。
