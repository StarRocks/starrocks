...
- 方法 4: [Routine Load](../../../loading/loading_introduction/loading_introduction.mdx) を使用して、Kafka から JSON 形式のデータを継続的に消費し、StarRocks にインポートします。
...
- ORDER BY および JOIN 句は JSON 型カラムを参照することをサポートしていません。参照する必要がある場合は、事前に CAST 関数を使用して JSON 型カラムを他の SQL 型に変換することができます。具体的な変換方法については、[JSON Type Conversion](../../sql-functions/json-functions/json-query-and-processing-functions/cast.md) を参照してください。
- GROUP BY はトップレベルの JSON カラムをサポートしています（例：`GROUP BY json_col`）。グループ化は JSON の値に基づいて行われます。オブジェクトのキー順序はグループ化に影響しないため、`{"a":1,"b":2}` と `{"b":2,"a":1}` は同じグループに属します。ただし、数値として等しい値（`1` と `1.0` など）は異なるグループとして扱われます。ARRAY、MAP、または STRUCT 内にネストされた JSON を GROUP BY に使用することはできません。
- JSON 型カラムは、重複キーテーブル、主キーテーブル、およびユニークキーテーブルに存在できますが、集計テーブルには存在できません。
- JSON 型カラムは、重複キーテーブル、主キーテーブル、およびユニークキーテーブルのパーティションキー、バケッティングキー、またはディメンションカラムとしてサポートされておらず、JOIN または ORDER BY 句で使用することはできません。
