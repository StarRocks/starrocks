...
### Stream Load または Routine Load を使用して CSV 形式の配列をロードする
  ...
  CSV ファイル内の配列はデフォルトでカンマで区切られています。[Stream Load または Routine Load](../../../loading/loading_introduction/Loading_intro.md) を使用して、CSV テキストファイルまたは Kafka 内の CSV データをロードできます。
