...
| 1053 | Semi-Join/Anti-Join クエリに不正なデータ列が指定されました。 |
...
| 1111 | `WHERE` 句で集約関数を不正に使用しました。 |
| 1113 | 新しいテーブルの列セットが空であってはなりません。 |
...
| 5023 | テーブルまたはデータを変更するために使用された文でアクションが指定されていません。 |
| 5024 | ジョブの実行がタイムアウトしました。 |
| 5025 | データ挿入に失敗しました。 |
| 5026 | `SELECT` 文を使用してテーブルを作成する際にサポートされていないデータ型が使用されました。 |
| 5027 | 指定されたパラメータが設定されていません。 |
| 5028 | 指定されたクラスターが見つかりませんでした。 |
| 5030 | ユーザーはクラスターにアクセスする権限がありません。 |
...
| 5063 | Colocate テーブルは同じデータ型のパーティション列を持つべきです。 |
