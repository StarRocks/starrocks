...
```SQL
-- 必要なフィールドをログに固定フィールドとして保存し、ビジネスで頻繁に変更される他のフィールドを JSON としてパッケージ化します。
SELECT
    time,
    event,
    user,
...
JSON 型の特性により、クエリでのパフォーマンスは標準型（INT、STRING など）ほど良くありません。その理由は以下の通りです：
- ストレージのオーバーヘッド：JSON は半構造化型であり、各行の構造情報を保存する必要があるため、ストレージ使用量が多く、圧縮効率が低くなります。
- クエリの複雑さ：クエリは実行時データに基づいてデータ構造を検出する必要があり、ベクトル化された実行最適化を達成するのが難しいです。
- 冗長データ：クエリは JSON データ全体を読み取る必要があり、多くの冗長フィールドを含みます。
...
Flat JSON の核心原理は、ロード時に JSON データを検出し、JSON データから共通フィールドを抽出して標準型データとして保存することです。JSON をクエリする際、これらの共通フィールドが JSON のクエリ速度を最適化します。例として以下のデータがあります：
...
上記の JSON データをロードする際、フィールド `a` と `b` はほとんどの JSON データに存在し、データ型が類似しています（どちらも INT）。したがって、フィールド `a` と `b` のデータは JSON から抽出され、2 つの INT カラムとして別々に保存されます。これらの 2 つのカラムがクエリで使用される際、追加の JSON フィールドを処理することなくデータを直接読み取ることができ、JSON 構造を扱う計算オーバーヘッドを削減します。

## Flat JSON が有効かどうかを確認する

データをロードした後、対応するカラムの抽出されたサブカラムをクエリできます：
...
実行されたクエリが Flat JSON 最適化の恩恵を受けているかどうかは、[Query Profile](../best_practices/query_tuning/query_profile_overview.md) を通じて以下のメトリクスを観察することで確認できます：
...
- `JsonFlattern`: Flat JSON がヒットしなかった場合に、現場でサブカラムを抽出するのにかかった時間。
...
- 再インポートを必要とせず、履歴データと互換性があります。履歴データは Flat JSON によってフラット化されたデータと共存します。
...
- Flat JSON は JSON オブジェクト内の共通キーのマテリアライズのみをサポートし、JSON 配列内のキーはサポートしません。
- Flat JSON はデータのソート方法を変更しないため、クエリパフォーマンスとデータ圧縮率はデータのソートによって影響を受けます。最適なパフォーマンスを達成するには、データソートのさらなる調整が必要な場合があります。
...
- データのロード時に、共通フィールドを抽出して JSON 型として別々に保存することをサポートし、型推論は行いません。
...
- データのロード時に、共通フィールドは自動的に BIGINT/LARGEINT/DOUBLE/STRING として型推論されます。認識されない型は JSON 型として推論され、予約フィールドカラムは JSON 型として保存されます。

## Flat JSON 機能の有効化（共有なしクラスタのみサポート）

1. テーブル作成時に、`flat_json.enable` プロパティをテーブルパラメータに設定できます。参考として [Table Creation](../sql-reference/sql-statements/table_bucket_part_index/CREATE_TABLE.md) を参照してください。
   ...
   ```
2. FE プルーニング機能を有効にする：`SET GLOBAL cbo_prune_json_subfield = true;`

## Flat JSON 機能の有効化

1. BE 設定を変更する：`enable_json_flat`、デフォルトは `false`。変更方法については、
[Configure BE parameters](../administration/management/BE_configuration.md#configure-be-parameters) を参照してください。
2. FE プルーニング機能を有効にする：`SET GLOBAL cbo_prune_json_subfield = true;`
