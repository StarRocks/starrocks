...
1. すべてのテーブルのバケットシーケンスと BE ノードの間のマッピングは、親テーブルのマッピングと同じです。
...
* **GroupId**: グループのクラスター全体で一意の識別子で、前半が db id で後半がグループ id です。
...
Colocation テーブルのレプリカ分布は、グループスキーマで指定された分布ルールに従う必要があるため、通常のシャーディングとは異なり、レプリカの修復とバランスが異なります。
