...
Sorted streaming aggregate は、GROUP BY キーをキーの順序に従って比較することでデータをグループ化し、ハッシュテーブルを作成する必要がありません。これにより、集計に消費されるメモリリソースを効果的に削減します。高い集計カーディナリティを持つクエリにおいて、sorted streaming aggregate は集計パフォーマンスを向上させ、メモリ使用量を削減します。
...
- GROUP BY のキーは、テーブルを作成する際に指定されたバケットキーと同じ分布でなければなりません。例えば、テーブルに `k1, k2, k3` の3つの列がある場合、バケットキーは `k1` または `k1, k2` です。
...
- Sorted streaming aggregate は、最初の段階の集計（つまり、AGG ノードの下に Scan ノードが1つだけある場合）でのみ機能します。
