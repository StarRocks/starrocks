...
description: "Troubleshooting guide for asynchronous materialized views in StarRocks: diagnose and fix refresh failures, query rewrite issues, and data consistency problems."
sidebar_position: 30
---
...
ロードによってトリガーされるマテリアライズドビュー（interval を指定しない `REFRESH ASYNC`）が、自動では回復しない理由で非アクティブな場合、StarRocks はベーステーブルへの書き込みのたびに自動リフレッシュを再投入しなくなります。これにより、失敗した TaskRun が繰り返し生成されることを防ぎます。自動では回復しない理由は次のとおりです。

- `ALTER MATERIALIZED VIEW ... INACTIVE` による手動の非アクティブ化
- バックアップ中
- ベーステーブルが Optimize された
- 連続したリフレッシュ失敗
- ベーステーブルの非 append-only 変更により増分リフレッシュが壊れた

手動の `REFRESH MATERIALIZED VIEW` および interval ベースのリフレッシュは影響を受けません。その他の一時的な理由で非アクティブなビューは、書き込み後もリトライします。
...
- **マテリアライズドビューがデータの一貫性要件を満たしているか確認する。**
  ...
  - `PROPERTIES('mv_rewrite_staleness_second'='5')`を追加して、一定のデータ不整合を許容します。マテリアライズドビューの最近の「完全な」リフレッシュが確認された時刻がこの時間間隔内であれば、ベーステーブルのデータが変更されているかどうかに関係なく、クエリは書き換えられます。一部のパーティションのみを対象とするリフレッシュは、この鮮度の基準を更新しません。
