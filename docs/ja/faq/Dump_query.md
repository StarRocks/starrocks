...
このトピックでは、query_dump インターフェースを使用して SQL クエリの詳細と関連情報を取得する方法について説明します。

StarRocks で SQL クエリを実行する際に次のような問題が発生した場合、query_dump を使用して SQL の詳細を取得し、その情報を StarRocks の技術サポートに送信してトラブルシューティングを行うことができます。

* SQL クエリまたは EXPLAIN を実行した際に `Unknown Error` が返される。
* SQL クエリを実行した際にエラーメッセージや例外が返される。
* SQL クエリの実行が期待通りに効率的でない、または実行プランを最適化できる（例えば、パーティションをプルーニングしたり、Join 順序を調整したりできる）。

## 機能概要

query_dump インターフェースは、SQL を実行する際に FE が依存する情報を返します。これには以下が含まれます。
