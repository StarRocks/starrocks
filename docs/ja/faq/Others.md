...
1. 例えば、**original** という名前のファイルがあり、そのテキストが文字化けしています。このファイルの文字セットは ISO-8859-1 です。次のコードを実行してファイルの文字セットを取得します。
...
3. 変換後もこのファイルのテキストが文字化けする場合、ファイルの文字セットを GBK に再設定し、再度 UTF-8 に変換します。
...
VARCHAR(n) の場合、StarRocks は "n" をバイトで定義し、MySQL は "n" を文字で定義します。UTF-8 によれば、1 つの漢字は 3 バイトに相当します。StarRocks と MySQL が "n" を同じ数値で定義した場合、MySQL は StarRocks の 3 倍の文字を保存します。

## テーブルのパーティション化されたフィールドのデータ型を FLOAT、DOUBLE、または DECIMAL にすることはできますか？
...
クォータ増加をリクエストするには、次のコードを実行します。

```plaintext
ALTER DATABASE example_db SET DATA QUOTA 10T;
```

## UPSERT ステートメントを実行してテーブル内の特定のフィールドを更新することは StarRocks でサポートされていますか？

StarRocks 2.2 以降では、主キーテーブルを使用してテーブル内の特定のフィールドを更新することがサポートされています。StarRocks 1.9 以降では、主キーテーブルを使用してテーブル内のすべてのフィールドを更新することがサポートされています。詳細については、StarRocks 2.2 の [Primary Key table](../table_design/table_types/primary_key_table.md) を参照してください。

## 2 つのテーブルまたは 2 つのパーティション間でデータを入れ替えるにはどうすればよいですか？

SWAP WITH ステートメントを実行して、2 つのテーブルまたは 2 つのパーティション間でデータを入れ替えます。SWAP WITH ステートメントは、INSERT OVERWRITE ステートメントよりも安全です。データを入れ替える前に、まずデータを確認し、入れ替え後のデータが入れ替え前のデータと一致しているかどうかを確認してください。

- 2 つのテーブルを入れ替える: 例えば、table 1 という名前のテーブルがあります。table 1 を別のテーブルに置き換えたい場合、次の手順を実行します。
...
- 2 つのパーティションを入れ替える: 例えば、table 1 という名前のテーブルがあります。table 1 のパーティションデータを置き換えたい場合、次の手順を実行します。
    ...
    3. table 1 のパーティションを一時パーティションに置き換えます。
...
Apache Hive のメタデータは FEs にキャッシュされています。しかし、StarRocks がメタデータを更新するのに 2 時間の遅延があります。StarRocks が更新を完了する前に、Apache Hive テーブルに新しいデータを挿入したり更新したりすると、BEs がスキャンする HDFS のデータと FEs が取得するデータが異なるため、このエラーが発生します。
...
- Apache Hive テーブルを手動で更新します。詳細については、[Metadata caching strategy](../data_source/External_table.md) を参照してください。
...
DROP TABLE ステートメントを実行してテーブルを削除した場合、StarRocks は割り当てられたディスクスペースを解放するのに時間がかかります。割り当てられたディスクスペースを即座に解放するには、DROP TABLE FORCE ステートメントを実行してテーブルを削除します。DROP TABLE FORCE ステートメントを実行すると、StarRocks は未完了のイベントがあるかどうかを確認せずにテーブルを直接削除します。テーブルが削除されると復元できないため、DROP TABLE FORCE ステートメントは慎重に実行することをお勧めします。
...
メタデータは FE が使用するメモリに保存されます。以下の表に示すように、tablet の数に応じて FE のメモリサイズを設定できます。例えば、tablet の数が 100 万未満の場合、FE に最低 16 GB のメモリを割り当てる必要があります。**fe.conf** ファイルの **JAVA_OPTS** 設定項目で `-Xms` と `-Xmx` のパラメータ値を設定し、`-Xms` と `-Xmx` のパラメータ値は一致している必要があります。すべての FEs で設定が同じである必要があります。なぜなら、どの FE も Leader に選出される可能性があるからです。
...
実際のビジネスシナリオやシミュレートされたビジネスシナリオに基づいて同時実行の制限をテストできます。一部のユーザーからのフィードバックによれば、最大 20,000 QPS または 30,000 QPS を達成できることがあります。

## StarRocks の SSB テストの初回パフォーマンスが 2 回目よりも遅いのはなぜですか？

最初のクエリのディスク読み取り速度はディスクのパフォーマンスに関連しています。最初のクエリの後、ページキャッシュが生成されるため、後続のクエリは以前よりも高速になります。

## クラスターに最低限必要な BEs の数はどれくらいですか？

StarRocks は単一ノードのデプロイメントをサポートしているため、最低 1 つの BE を設定する必要があります。BEs は AVX2 で動作する必要があるため、8 コア 16GB 以上の構成のマシンに BEs をデプロイすることをお勧めします。
...
## ロードタスクとパーティション作成タスクの同時実行による式に基づくパーティション化の競合を防ぐにはどうすればよいですか？

現在、式に基づくパーティション化戦略を持つテーブルでは、ロードタスク中に作成されたパーティションが ALTER TABLE タスク中に作成されたパーティションと競合します。ロードタスクが優先されるため、競合する ALTER タスクは失敗します。この問題を防ぐために、次の回避策を検討してください：

- 粗い時間ベースのパーティション（例えば、日や月でパーティション化）を使用する場合、ALTER 操作が時間境界を越えないようにすることで、パーティション作成の失敗のリスクを減らすことができます。
- 細かい時間ベースのパーティション（例えば、時間でパーティション化）を使用する場合、ロードタスクによって作成される新しいパーティションにより ALTER 操作が中断されないように、将来の時間範囲のパーティションを手動で作成することができます。トランザクションをコミットせずに INSERT ステートメントを実行してパーティション作成をトリガーするために [EXPLAIN ANALYZE](../sql-reference/sql-statements/cluster-management/plan_profile/EXPLAIN_ANALYZE.md) 機能を使用できます。これにより、実際のデータに影響を与えることなく必要なパーティションを作成できます。次の例は、次の 8 時間のパーティションを作成する方法を示しています：
