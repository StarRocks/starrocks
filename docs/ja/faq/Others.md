...
現在、式に基づくパーティション化を使用するテーブルでは、ロードタスクで作成されたパーティションと ALTER TABLE タスクで作成されたパーティションが競合する場合があります。ロードタスクが優先されるため、競合する ALTER タスクはキャンセルされるか失敗します。これは、データまたはテーブルレベルのメタデータを書き換える ALTER 操作（完全な schema change、rollup、`OPTIMIZE` など）にのみ適用されます。メタデータのみの ALTER 操作は競合しなくなりました。FE 設定項目 `enable_concurrent_add_partition_during_alter` が `true`（デフォルト）の場合、パーティション作成は、shared-data モードの ADD/DROP INDEX ファストパスジョブおよび fast schema evolution の一時的なメタデータ更新状態と並行して実行されます。引き続き影響を受ける ALTER タイプについては、以下の回避策を検討してください：
