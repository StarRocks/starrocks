...
Flink で localtimestap 関数によって生成された時間は正常ですが、StarRocks に送信されると8時間遅れます。Flink サーバーと StarRocks サーバーは同じタイムゾーン、つまり Asia/Shanghai UTC/GMT+08:00 にあります。Flink のバージョンは1.12です。ドライバーは flink-connector-jdbc_2.11 です。この問題を解決する方法を教えていただけますか？

**解決策:**

Flink のシンクテーブルで 'server-time-zone' = 'Asia/Shanghai' という時間パラメータを設定してみてください。また、jdbc URL に &serverTimezone=Asia/Shanghai を追加することもできます。以下に例を示します:
...
Kafka の通信にはホスト名が必要です。ユーザーは StarRocks クラスターのノードで /etc/hosts にホスト名解決を設定する必要があります。
...
`/etc/maven/settings.xml` のミラー部分がすべて Aliyun ミラーアドレスを通じて取得するように設定されていることを確認してください。
...
このパラメータが15秒に設定され、チェックポイント間隔が5分に等しい場合、この値はまだ有効ですか？

**解決策:**

3つのしきい値のうち、どれが最初に達成されるかによって、そのしきい値が最初に有効になります。これは、exactly once にのみ機能するチェックポイント間隔の値には影響されません。Interval-ms は at_least_once によって使用されます。