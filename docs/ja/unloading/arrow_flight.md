...
##### プロキシのバックプレッシャータイムアウト

動的に変更可能な FE 設定 `arrow_flight_proxy_backpressure_timeout_ms` は、プロキシ結果ストリームが下流クライアントの準備完了を連続して待機できる時間を制限します。デフォルトは `300000` ミリ秒（5 分）で、FE から BE および FE から FE のプロキシストリームに適用されます。待機ごとに新しいタイムアウトが開始されます。クエリや転送の総時間、および上流からの読み取り時間は含みません。クライアント RPC の期限が先に到来する場合は、その期限が適用されます。

タイムアウトすると、プロキシは上流ストリームをキャンセルし、結果転送ワーカーを解放して、クライアントに `DEADLINE_EXCEEDED` を返します。値は `1` から `2147483647` ミリ秒までの正の整数である必要があります。ゼロまたは負の値はタイムアウトを無効にせず、新しいプロキシ結果ストリームを `INTERNAL` で拒否します。設定変更に FE の再起動は不要で、新しく開かれるストリームに適用されます。既存のストリームは開始時の値を保持します。
