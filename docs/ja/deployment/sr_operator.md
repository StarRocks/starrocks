...
CN コンテナのエントリポイントは、コアダンプを監視、圧縮、アップロードできます。CN コンテナに次の環境変数を設定します。

| 変数 | 設定可能な値とデフォルト | 説明 |
| ---- | ------------------------ | ---- |
| `COREDUMP_ENABLED` | 有効にするには `true` を設定します。未設定を含むその他の値では無効になります。 | コアダンプアップローダーを起動します。CN が `SIGABRT`（終了ステータス `134`）または `SIGSEGV`（終了ステータス `139`）で終了すると、アップローダーを動作させ続けるためにエントリポイントが CN を再起動します。 |
| `CN_RESTART_WAIT_SECONDS` | 正の整数。デフォルト: `5`。 | 対象のクラッシュ後、CN を再起動するまで待機する秒数です。`COREDUMP_ENABLED=true` の場合にのみ使用されます。 |

コアダンプの収集には、コンテナイメージに `inotifywait`、`pigz`、`rclone` が必要です。標準の非 minimal Ubuntu BE/CN イメージにはこれらのコマンドが含まれます。minimal Ubuntu イメージと UBI イメージには含まれません。収集が有効で必要なコマンドがない場合、CN は起動せず、不足しているコマンドをコンテナログに出力します。
