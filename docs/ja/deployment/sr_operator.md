...
### BE と CN のクラッシュ後の再起動動作を設定する

BE および CN コンテナのエントリポイントは、クラッシュ後にコンポーネントを再起動して Pod を存続させ、コンテナ内に残ったコアダンプを取得できるようにします。また、デバッグのために終了のたびにコンポーネントを再起動することもできます。BE または CN コンテナに次の環境変数を設定します。

| 変数 | 設定可能な値とデフォルト | 説明 |
| ---- | ------------------------ | ---- |
| `COREDUMP_ENABLED` | 有効にするには `true` を設定します。未設定を含むその他の値ではクラッシュ時の再起動が無効になります。 | コンポーネントが `SIGABRT`（終了ステータス `134`）または `SIGSEGV`（終了ステータス `139`）で終了すると、エントリポイントはコンテナを終了させずにコンポーネントを再起動します。 |
| `DEBUG_MODE` | 有効にするには `true` を設定します。未設定を含むその他の値ではデバッグ再起動動作が無効になります。 | コンポーネントが終了するたびに再起動します。この動作はデバッグ専用です。 |
| `BE_RESTART_WAIT_SECONDS` | 正の整数。デフォルト: `5`。 | BE を再起動するまで待機する秒数です。`COREDUMP_ENABLED=true` または `DEBUG_MODE=true` の場合に使用されます。 |
| `CN_RESTART_WAIT_SECONDS` | 正の整数。デフォルト: `5`。 | CN を再起動するまで待機する秒数です。`COREDUMP_ENABLED=true` または `DEBUG_MODE=true` の場合に使用されます。 |

エントリポイントはコアダンプをアップロードしません。アップロードには `rclone` が必要ですが、BE および CN イメージには含まれていないため、アップローダーは無効になっています。コンテナ内のコアダンプディレクトリから手動で取得してください。ディレクトリのデフォルトは `${STARROCKS_HOME}/storage/coredumps` で、`COREDUMP_PATH` で変更できます。
