...
上記のポートが使用されている場合は、代替ポートを見つけ、FE ノードをデプロイする際に指定する必要があります。詳細な手順については、[Deploy StarRocks - Start the Leader FE node](../deploy_manually.md#ステップ-1-leader-fe-ノードを起動する) を参照してください。
...
上記のポートが使用されている場合は、代替ポートを見つけ、BE ノードをデプロイする際に指定する必要があります。詳細な手順については、[Deploy StarRocks - Start the BE service](../deploy_manually.md#ステップ-2-共有なしの場合-be-サービスを起動する) を参照してください。
...
上記のポートが使用されている場合は、代替ポートを見つけ、CN ノードをデプロイする際に指定する必要があります。詳細な手順については、[Deploy StarRocks - Start the CN service](../deploy_manually.md#ステップ-2-共有データの場合-cn-サービスを起動する) を参照してください。
...
StarRocks の Java コンポーネントは JDK 上で動作します。Java コンポーネントは次のとおりです。

- FE
- BE と CN 上の Java UDF、コネクター、Java Extensions 機能

デプロイ前に、お使いの StarRocks バージョンが要求する JDK バージョンをインストールし、その JDK を `JAVA_HOME` に指定してください。

### StarRocks バージョンごとの JDK バージョン

最小および推奨の JDK バージョンは、お使いの StarRocks バージョンによって異なります。

| StarRocks バージョン | 最小 JDK | 推奨 JDK |
| --- | --- | --- |
| v2.5、v3.0、v3.1、v3.2 | JDK 8 | JDK 11 |
| v3.3、v3.4 | JDK 11 | JDK 11 |
| v3.5、v4.0、v4.1 | JDK 17 | JDK 17 |
| v4.2 以降 | JDK 17 | JDK 21 |

補足事項:

- v2.5、v3.0、v3.1、v3.2 では、Ubuntu 22.04 へのデプロイに JDK 11 が必要です。
