...
:::tip
手動デプロイの準備は [Deployment prerequisites](./deployment_prerequisites.md) と [Check environment configurations](./environment_configurations.md) ドキュメントに記載されています。プロダクションデプロイを計画している場合は、まずそこから始めてください。StarRocks を始めるにあたり、クイックスタートの一つを試したい場合は [Quick Starts](../quick_start/quick_start.mdx) を参照してください。
:::
...
2. 以前に準備した [StarRocks FE deployment files](../deployment/prepare_deployment_files.md) を保存しているディレクトリに移動し、FE 設定ファイル **fe/conf/fe.conf** を変更します。
   ...
   b. [Environment Configuration Checklist](../deployment/environment_configurations.md#fe-ports) に記載されている FE ポートが占有されている場合は、FE 設定ファイルで有効な代替ポートを割り当てる必要があります。
      ...
      > **注意**
      >
      > クラスタに複数の FE ノードをデプロイする場合、各 FE ノードに同じ `http_port` を割り当てる必要があります。

   c. クラスタに対して IP アドレスアクセスを有効にしたい場合、設定ファイルに設定項目 `priority_networks` を追加し、FE ノードに専用の IP アドレス（CIDR 形式）を割り当てる必要があります。クラスタに対して [FQDN アクセス](../administration/management/enable_fqdn.md) を有効にしたい場合、この設定項目は無視できます。
...
3. FE ノードを起動します。
   ...
   - クラスタに対して IP アドレスアクセスを有効にするには、次のコマンドを実行して FE ノードを起動します。

     ```Bash
     ./fe/bin/start_fe.sh --daemon
     ```

   - クラスタに対して FQDN アクセスを有効にするには、次のコマンドを実行して FE ノードを起動します。
     ...
     > **注意**
     >
     > FQDN アクセスを有効にして FE ノードを起動する前に、**/etc/hosts** にすべてのインスタンスのホスト名を割り当てたことを確認してください。詳細については、[Environment Configuration Checklist - Hostnames](../deployment/environment_configurations.md#hostnames) を参照してください。
...
## ステップ 2: (共有なしの場合) BE サービスを開始する

:::note

共有なしクラスタにのみ BE ノードを追加できます。共有データクラスタに BE ノードを追加することは推奨されず、未知の動作を引き起こす可能性があります。
...
2. 以前に準備した [StarRocks BE deployment files](../deployment/prepare_deployment_files.md) を保存しているディレクトリに移動し、BE 設定ファイル **be/conf/be.conf** を変更します。
   ...
   c. クラスタに対して IP アドレスアクセスを有効にしたい場合、設定ファイルに設定項目 `priority_networks` を追加し、BE ノードに専用の IP アドレス（CIDR 形式）を割り当てる必要があります。クラスタに対して FQDN アクセスを有効にしたい場合、この設定項目は無視できます。
...
3. BE ノードを起動します。
      ...
      > **注意**
      >
      > - FQDN アクセスを有効にして BE ノードを起動する前に、**/etc/hosts** にすべてのインスタンスのホスト名を割り当てたことを確認してください。詳細については、[Environment Configuration Checklist - Hostnames](../deployment/environment_configurations.md#hostnames) を参照してください。
...
> **注意**
>
> 少なくとも 3 つの BE ノードがデプロイされて StarRocks クラスタに追加されると、BE の高可用性クラスタが自動的に形成されます。

## ステップ 2: (共有データの場合) CN サービスを開始する

:::note

共有データクラスタにのみ CN ノードを追加できます。共有なしクラスタに CN ノードを追加することは推奨されず、未知の動作を引き起こす可能性があります。

:::

Compute Node (CN) は、データを自ら保持しないステートレスなコンピューティングサービスです。クエリに対して追加のコンピューティングリソースを提供するために、オプションで CN ノードをクラスタに追加できます。CN ノードは BE デプロイメントファイルでデプロイできます。Compute Nodes は v2.4 からサポートされています。

1. 以前に準備した [StarRocks BE deployment files](../deployment/prepare_deployment_files.md) を保存しているディレクトリに移動し、CN 設定ファイル **be/conf/cn.conf** を変更します。
   ...
   b. クラスタに対して IP アドレスアクセスを有効にしたい場合、設定ファイルに設定項目 `priority_networks` を追加し、CN ノードに専用の IP アドレス（CIDR 形式）を割り当てる必要があります。クラスタに対して FQDN アクセスを有効にしたい場合、この設定項目は無視できます。
...
2. CN ノードを起動します。
   ...
   > **注意**
   >
   > - FQDN アクセスを有効にして CN ノードを起動する前に、**/etc/hosts** にすべてのインスタンスのホスト名を割り当てたことを確認してください。詳細については、[Environment Configuration Checklist - Hostnames](../deployment/environment_configurations.md#hostnames) を参照してください。
...
2. 次の SQL を実行して、Leader FE ノードの状態を確認します。
...
4. 次の SQL を実行して、BE/CN ノードの状態を確認します。

   - (共有なしの場合) BE ノードの状態を確認します。
   ...
   *************************** 1. row ***************************
   ...
   CN が正常に起動し、クエリ中に CN を使用したい場合、システム変数 `SET prefer_compute_node = true;` と `SET use_compute_nodes = -1;` を設定します。詳細については、[System variables](../sql-reference/System_variable.md#descriptions-of-variables) を参照してください。
...
3. 新しい FE インスタンスでターミナルを起動し、メタデータストレージ用の専用ディレクトリを作成し、StarRocks FE デプロイメントファイルを保存しているディレクトリに移動し、FE 設定ファイル **fe/conf/fe.conf** を変更します。詳細な手順については、[Step 1: Start the Leader FE node](#step-1-start-the-leader-fe-node) を参照してください。基本的には、FE ノードを起動するためのコマンドを除いて、ステップ 1 の手順を繰り返すことができます。
   ...
   > **注意**
   >
   > クラスタに新しい Follower FE ノードを追加する際、Follower FE ノードにメタデータを同期するためのヘルパーノード（基本的には既存の Follower FE ノード）を割り当てる必要があります。
   ...
   - FQDN アクセスで新しい FE ノードを起動するには、次のコマンドを実行して FE ノードを起動します。
     ...
     ノードを初めて起動する際に、パラメータ `--helper` と `--host_type` を指定する必要があるのは一度だけです。
...
5. 上記の手順 2、3、4 を繰り返して、すべての新しい Follower FE ノードを正常に起動し、次に MySQL クライアントから次の SQL を実行して FE ノードの状態を確認します。
...
StarRocks クラスタをデプロイした後、初期管理手順についての指示を得るために [Post-deployment Setup](../deployment/post_deployment_setup.md) に進むことができます。
```