...
StarRocks は異なるサービスに特定のポートを使用します。これらのポートが他のサービスによって占有されていないか、各インスタンスで確認してください。

### FE ポート

FE のデプロイメントに使用するインスタンスでは、以下のポートを確認する必要があります。
...
FE インスタンスで以下のコマンドを実行し、これらのポートが占有されていないか確認してください。
...
上記のポートがいずれか占有されている場合は、代替ポートを見つけ、FE ノードをデプロイする際に指定する必要があります。詳細な手順については、 [Deploy StarRocks - Start the Leader FE node](../deployment/deploy_manually.md#step-1-start-the-leader-fe-node) を参照してください。

### BE ポート

BE のデプロイメントに使用するインスタンスでは、以下のポートを確認する必要があります。
...
BE インスタンスで以下のコマンドを実行し、これらのポートが占有されていないか確認してください。
...
上記のポートがいずれか占有されている場合は、代替ポートを見つけ、BE ノードをデプロイする際に指定する必要があります。詳細な手順については、 [Deploy StarRocks - Start the BE service](../deployment/deploy_manually.md#step-2-start-the-be-service) を参照してください。

### CN ポート

CN のデプロイメントに使用するインスタンスでは、以下のポートを確認する必要があります。
...
CN インスタンスで以下のコマンドを実行し、これらのポートが占有されていないか確認してください。
...
上記のポートがいずれか占有されている場合は、代替ポートを見つけ、CN ノードをデプロイする際に指定する必要があります。詳細な手順については、 [Deploy StarRocks - Start the CN service](../deployment/deploy_manually.md#step-3-optional-start-the-cn-service) を参照してください。
...
> **注意**
>
> **/etc/hosts** ファイル内のすべての IP アドレスは一意である必要があります。

## JDK 設定

StarRocks は、環境変数 `JAVA_HOME` を使用してインスタンス上の Java 依存関係を見つけます。
...
トランスペアレント・ヒュージ・ページはデフォルトで有効になっています。この機能はメモリアロケータに干渉し、パフォーマンスの低下を引き起こす可能性があるため、無効にすることをお勧めします。
...
2. **/etc/fstab** 設定ファイルからスワップスペース情報を削除します。
...
**最大ファイルディスクリプタ**と**最大ユーザープロセス**の値が異常に小さい場合、StarRocks に問題が発生する可能性があります。これらの値を大きくすることをお勧めします。
...
デーモンが処理できない新しい接続試行でシステムがオーバーフローしている場合に、新しい接続をリセットするようシステムに許可します。
