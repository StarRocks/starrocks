...
> **注意**
>
> 例えば、2.4->2.5->3.0->3.1->3.2 のように連続したマイナーバージョンのアップグレードを行う必要がある場合、またはアップグレードに失敗してクラスタをダウングレードした後に再度アップグレードを行いたい場合、例えば、2.5->3.0->2.5->3.0 のように。Follower FE の一部でメタデータのアップグレードが失敗するのを防ぐために、2つの連続したアップグレードの間、または2回目のアップグレードの試行前にダウングレード後に次の手順を実行してください:
>
...
StarRocks クラスタを後のマイナーまたはメジャーバージョンにアップグレードする場合は、互換性設定を行う必要があります。一般的な互換性設定に加えて、アップグレード元の StarRocks クラスタのバージョンに応じて詳細な設定が異なります。
