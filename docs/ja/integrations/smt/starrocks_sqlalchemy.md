...
- **ビューおよびマテリアライズドビューの定義比較（StarRocks < 4.0.6）：** 4.0.6 より古いクラスターでは、StarRocks はビュー定義を保存時に正規形式に書き換えるため、モデルに記述した SQL がクラスターから返される SQL と構文的に異なる場合があります。このダイアレクトは、モデルの SQL を一時ビューにラウンドトリップさせて DB の正規形式を取得してから比較することでこれを解決します。これには、モデル定義で参照されるすべてのテーブルとビューがデータベースにすでに存在している必要があります。存在しない場合（例：それらのオブジェクトも作成するフォワードマイグレーション中）、ダイアレクトは正規表現ベースの正規化器にフォールバックします。これは最も一般的な書き換えパターンをカバーしますが、すべてのエッジケースを処理できるとは限りません。
- **クラスターアップグレード後のビュー定義のドリフト：** StarRocks クラスターがビューの存在中にアップグレードされた場合、それらのビューは旧バージョンで正規化されており、アップグレード後のクラスターが生成する形式と一致しない場合があります。その期間中、自動生成が不要なビューのマイグレーションを出力する可能性があります。影響を受けるビューを再作成する（削除してマイグレーションを再適用する）ことでドリフトが解消されます。
