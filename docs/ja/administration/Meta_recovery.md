...
このトピックでは、FE ノードがさまざまな例外に遭遇したときに、StarRocks クラスターのメタデータをリカバリーする方法について説明します。

一般的に、以下のいずれかの問題が発生した場合にのみ、メタデータのリカバリーを行う必要があります。
...
以下の手順に従ってメタデータをリカバリーし、FE を起動できます。
...
- FE ノードの半数以上がこの問題に遭遇した場合、この解決策は適用できません。[最後の手段](#7-measure-of-the-last-resort) の指示に従って問題を修正する必要があります。
...
この問題を解決するには、JVM ヒープサイズを増やすか、G1 GC アルゴリズムを使用します。
...
この問題は、Follower ノードが完全なメタデータ同期を必要とする場合に発生します。以下のいずれかの状況が発生した場合に発生する可能性があります。

- Follower ノードのメタデータが Leader ノードのメタデータよりも遅れており、Leader ノードがメタデータのチェックポイントを行った後、Follower ノードがメタデータの増分更新を実行できないため、完全なメタデータ同期が必要です。
- 元の Leader ノードがメタデータを書き込み、チェックポイントを行ったが、ハングする前に Follower FE ノードに同期できなかった場合。再起動後、Follower ノードになります。不正なメタデータがチェックポイントされているため、Follower ノードはメタデータの増分削除を実行できず、完全なメタデータ同期が必要です。

この例外は、新しい Follower ノードがクラスターに追加されたときにスローされます。この場合、何も行う必要はありません。この例外が既存の Follower ノードまたは Leader ノードに対してスローされた場合、ノードを再起動するだけで問題を解決できます。
...
この問題は、元の Leader ノードがハングし、再びアクティブになるときに、存続している Follower ノードが新しい Leader ノードを選出しようとする際に発生します。Follower ノードは元の Leader ノードと新しい接続を確立しようとしますが、Leader ノードは古い接続がまだ存在するため、接続要求を拒否します。要求が拒否されると、Follower ノードは環境を無効として設定し、この例外をスローします。
...
この問題を解決するには、FE メタデータ用に専用のディスクを割り当てるか、高性能のディスクに交換します。
...
4. すべての FE ノードを起動し、クラスター内のテーブルをクエリしてデータが無事かどうかを確認します。
   ...
   次のエラーが返された場合、メタデータのリカバリーが完了するまで待つ必要があります。
...
Follower FE ノードが Leader 選出を実行できない場合、FE はサービスを提供しません。この問題が発生すると、次のログ記録が繰り返されることがあります。
...
### 1. Follower ノードの大多数が稼働していない

Follower ノードの大多数が稼働していない場合、FE グループはサービスを提供しません。ここでの「大多数」とは `1 + (Follower ノード数/2)` を示します。Leader FE ノード自体は Follower ですが、Observer ノードは Follower ではありません。
...
- Follower ノードの総数は BDBJE ログから確認できます。
...
ノードの `priority_networks` が設定されていない場合、FE ノードは再起動時に利用可能な IP アドレスをランダムに選択します。BDBJE メタデータに記録された IP アドレスがノードの起動に使用されるアドレスと異なる場合、FE はサービスを提供しません。
...
この問題を解決するには、FE 設定ファイル **fe.conf** のノードの `priority_networks` を **fe/meta/image/ROLE** に記録された IP アドレスに設定し、ノードを再起動します。
...
StarRocks を v3.0 以降にアップグレードした後、または BDBJE を v18 以降にアップグレードした後、`meta_dir` を保存するディスクの利用可能スペースが 5 GB 未満の場合、ノードが再起動に失敗することがあります。

BDBJE バージョンは **.jar** パッケージから確認できます。ディレクトリ **fe/lib** にあります。

この問題を解決するには、ディスクをスケールアップするか、FE メタデータ用により大容量の専用ディスクを割り当てます。
...
この問題を解決するには、FE 設定ファイル **fe.conf** のノードの `edit_log_port` を **fe/meta/image/ROLE** に記録された `edit_log_port` に設定し、ノードを再起動します。
...
- Follower ノードの大多数が再起動できない。
- Follower ノードが BDBJE のバグにより Leader 選出を実行できない。
- 前述のセクションで言及されていない例外。

メタデータをリカバリーするために、以下の手順に従います。
...
5. 前のステップで選択した FE ノードの役割に基づいて対応する操作を実行します。
  ...
  <Tabs groupId="recovery_role">
   ...
   4. データとメタデータが無事で、ノードの役割が Leader である場合、以前に追加した設定を削除し、ノードを再起動できます。
   ...
   5. データとメタデータが無事で、ノードの役割が Leader である場合、以前に追加した設定を削除します。ただし、**ノードを再起動しないでください**。
   ...
   10. **新しい Follower のみの fe.conf** に次の設定を追加します。
       ...
       - StarRocks v2.5、v3.0、v3.1.9 および以前のパッチバージョン、v3.2.4 および以前のパッチバージョンの場合:

         ```Properties
         metadata_failure_recovery = true
         ```

       - StarRocks v3.1.10 および以降のパッチバージョン、v3.2.5 および以降のパッチバージョン、v3.3 および以降の場合:
   ...
   13. データとメタデータが無事で、ノードの役割が Leader である場合、以前に追加した設定を削除し、ノードを再起動できます。
...
6. 新しい Leader FE ノードをヘルパーとして使用して新しい Follower ノードを起動します。
...
メタデータバックアップを使用して新しい FE ノードを起動する場合、以下の手順に従います。
...
5. データとメタデータが無事で、ノードの役割が Leader である場合、設定 `bdbje_reset_election_group` を削除し、ノードを再起動する必要があります。
6. メタデータバックアップを使用して新しい Leader FE ノードを正常に起動しました。新しい Leader FE ノードをヘルパーとして使用して新しい Follower ノードを追加できます。
...
メタデータリカバリーが完了したら、以下の設定を削除する必要があります。
