...
- [メタデータバックアップを使用して新しい FE ノードでメタデータをリカバリする](#recover-metadata-on-a-new-fe-node-with-metadata-backup)
...
StarRocks クラスターをダウングレードすると、ダウングレード前のメタデータと互換性がない場合、FEs が再起動に失敗することがあります。
...
4. すべての FE ノードを起動し、データとメタデータが完全であるかどうかを確認します。
...
- この解決策は、FE ノードの半数以上がこの問題に遭遇した場合には適用できません。この問題を修正するには、[最後の手段の対策](#7-measure-of-the-last-resort) に従ってください。
...
- Follower ノードのメタデータが Leader ノードのメタデータに遅れをとっており、Leader ノードが自身のメタデータのチェックポイントを完了している。Follower ノードはメタデータの増分更新を行うことができず、完全なメタデータ同期が必要です。
- 元の Leader ノードがメタデータを書き込み、チェックポイントを行いますが、ハングする前に Follower FE ノードに同期できません。再起動後、Follower ノードになります。不正なメタデータがチェックポイントされているため、Follower ノードはメタデータの増分削除を行うことができず、完全なメタデータ同期が必要です。

この例外は、新しい Follower ノードがクラスターに追加されるときにスローされます。この場合、何もアクションを取る必要はありません。この例外が既存の Follower ノードまたは Leader ノードに対してスローされた場合、ノードを再起動するだけで問題を解決できます。
...
この問題は、元の Leader ノードがハングし、再びアクティブになったときに発生します。この間に生存している Follower ノードが新しい Leader ノードを選出しようとします。Follower ノードは元の Leader ノードと新しい接続を確立しようとしますが、Leader ノードは古い接続がまだ存在するため、接続要求を拒否します。要求が拒否されると、Follower ノードは環境を無効として設定し、この例外をスローします。
...
この問題を解決するには、FE メタデータ用に専用のディスクを割り当てるか、高性能なディスクに交換します。
...
4. すべての FE ノードを起動し、クラスター内のテーブルをクエリしてデータが完全であるかどうかを確認します。

   テーブルをクエリしたときに次のエラーが返された場合、メタデータのリカバリが完了するまで待機する必要があります。

   ```Plain
   ERROR 1064 (HY000): capture_consistent_versions error: version already been compacted.
   ```

   メタデータのリカバリの進行状況を確認するには、Leader FE ノードから次のステートメントを実行できます。
...
Follower FE ノードが Leader 選出を行えない場合、FE はサービスを提供しません。この問題が発生すると、次のログ記録が繰り返されることがあります。

```Plain
wait globalStateMgr to be ready. FE type: INIT. is ready: false
```

さまざまな例外がこの問題を引き起こす可能性があります。問題を段階的にトラブルシューティングするために、以下のセクションに従うことを強くお勧めします。不適切な解決策を適用すると、問題が悪化し、データ損失が発生する可能性があります。
...
この問題を解決するには、クラスター内のすべての Follower ノードを起動する必要があります。再起動できない場合は、[最後の手段の対策](#7-measure-of-the-last-resort) を参照してください。
...
- ノードの起動に使用された IP アドレスを FE ログから確認できます。
...
BDBJE バージョンは **fe/lib** ディレクトリ内の **.jar** パッケージから確認できます。
...
BDBJE メタデータに記録された `edit_log_port` を **fe/meta/image/ROLE** ファイルから確認できます。
...
以下の解決策は、前述の解決策がいずれも機能しない場合にのみ、最後の手段として試すことができます。
...
5. 前のステップで選択した FE ノードの役割に基づいて対応する操作を実行します。
  ...
  <Tabs groupId="recovery_role">
   ...
   Follower ノードが最新のメタデータを持っている場合、次の操作を行います。
   ...
   2. ノードを再起動し、データとメタデータが完全であるかどうかを確認します。
   ...
   Observer ノードが最新のメタデータを持っている場合、次の操作を行います。
   ...
   3. ノードを再起動し、データとメタデータが完全であるかどうかを確認します。
   ...
   9. 新しい FE ノードが正常に起動したら、両方の FE ノードのステータスと役割を確認します。
   ...
   11. **新しい Follower のみ** の **fe.conf** に次の設定を追加します。
   ...
   12. 新しい Follower ノードを再起動し、データとメタデータが完全であるかどうかを確認します。
   13. 現在の FE ノードが Leader FE ノードであるかどうかを確認します。
...
6. 生存している Follower ノードは、事実上クラスターの Leader ノードです。現在のノードを除くすべての FE ノードを削除します。
...
7. クラスターに戻す FE ノードのメタデータディレクトリ `meta_dir` をクリアします。
...
すべてのノードがクラスターに戻された後、メタデータは正常にリカバリされます。
