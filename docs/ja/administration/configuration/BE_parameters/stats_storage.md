...
- 単位: -
- 変更可能: はい
- 説明: ディクショナリを読み込み済みの ZSTD 解凍コンテキストを、共有コンテキストプールから借りる代わりに小さなスレッドローカル集合に保持するかどうか。共有プールから借りたコンテキストは返却時にリセットされ、粘着的な `refDDict` が消えるため、ページごとにディクショナリを冷えたコンテキストへ読み込み直すことになります。全列スキャンの対比測定では、コンテキストを温存することでディクショナリ読み取りのオーバーヘッドが +20〜25% から +5.6〜8.6% へ下がり、およそ 55%〜60% が取り除かれました。この最適化をスイッチとして公開しているのは、バイナリをロールバックせずに本番で無効化できるようにするため、また 1 つのクラスタ内で A/B 比較ができるようにするためです。この種の列を読むスレッドは、終了するまで最大 4 つのコンテキストを保持します。本パラメータは Segment に圧縮ディクショナリを持つ列の読み取りのみに影響し、それ以外の列には一切影響しません。
- 導入バージョン: v4.2
