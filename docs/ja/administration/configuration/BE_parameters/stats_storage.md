...
- 単位: バイト
- 変更可能: はい
- 説明: データページをディクショナリのサンプルとして使用するために必要な、エンコード済み値の最小サイズ。これは、ほとんど空の小さな先頭ページから無意味なディクショナリを構築し、その列を恒久的にディクショナリ使用済みとして記録してしまうことを防ぎます。いずれかのページが到達するまで、各データページのエンコード済み値と順に比較されます。したがって最初のページが小さすぎる列は、そこで諦めるのではなく後続のページからサンプリングされます。`0` 以上の値を指定してください。このパラメータは `enable_zstd_compression_dict` が `true` の場合にのみ有効です。
- 導入バージョン: v4.2
...
- 単位: -
- 変更可能: Yes
- 説明: string/varchar 列が、高カーディナリティまたは 1 MiB を超える実際の値によって plain（非辞書）エンコーディングにフォールバックする際、ページ末尾のオフセット配列を絶対オフセットではなく値ごとの差分（文字列長）として格納するかどうか。絶対オフセットは単調増加するため LZ4 ではほとんど圧縮できませんが、差分は長さがほぼ一定の文字列に対してほぼ一定値となり、はるかによく圧縮されます（非圧縮時の末尾サイズは変わりません）。圧縮後の列サイズの削減量はオフセット末尾のサイズ（1 行あたり約 4 バイト）にほぼ等しく、高カーディナリティの文字列列ほど効果が大きくなります。有効化すると、そのような列は独立した列エンコーディング `PLAIN_ENCODING_DELTA_OFFSET` として segment メタデータに記録されるため、形式は列ごとに自己記述的です。本設定は書き込み側のみを制御し、デフォルトで有効です。このエンコーディングを認識しない BE バージョンは、segment を誤読するのではなく開く際にエラーになります。したがって、サポートのない BE がまだ稼働している間（そのようなバージョンからのローリングアップグレード中など）、およびそのようなバージョンへダウングレードする前には `false` に設定してください。このエンコーディングで書き込まれた segment は、そのようなバージョンでは読み取れなくなる点に注意してください。
- 導入バージョン: v4.2.0
...
- 単位: -
- 変更可能: Yes
- 説明: 新しく書き込まれる FLOAT/DOUBLE 列に BIT_SHUFFLE の代わりに ALP（Adaptive Lossless floating-Point）列エンコーディングを使用するかどうか。ALP は十進数に近い浮動小数点値（センサー値、メトリクス、価格など）をスケーリングされた整数に変換し frame-of-reference ビットパッキングで格納するため、bitshuffle よりはるかに高い圧縮率（例えば小数第 2 位までの double で約 3 分の 1）とより高速なデコードを実現します。ALP が有利にならないベクトルはそのままの値で格納されるため、圧縮できないデータが膨張することはありません。有効化すると、そのような列は独立した列エンコーディング `ALP_ENCODING` として segment メタデータに記録されるため、形式は列ごとに自己記述的です。本設定は書き込み側のみを制御します。このエンコーディングを認識しない BE バージョンは、segment を誤読するのではなく開く際にエラーになります。したがってクラスタ全体をアップグレードするまで有効化しないでください。また、このエンコーディングで書き込まれた segment は、サポートのないバージョンへダウングレードすると読み取れなくなる点に注意してください。
- 導入バージョン: v4.2.0
...
- 説明: 列レベル圧縮ディクショナリ (各 Segment ファイル内に列ごとに保存される ZSTD ディクショナリ) のクラスタ全体のマスタースイッチ。このスイッチは Segment 書き込みのゲートで参照されます。`false` の場合でも、テーブルプロパティ `zstd_compression_columns` で指定された列は**引き続き ZSTD で圧縮されます**——これはユーザーの明示的な要求であり、このスイッチには依存しません——ただし共有ディクショナリは構築されません。つまりこのスイッチは圧縮率にのみ影響し、圧縮方式を変えることはないため、運用上の安全弁として実行時に切り替えられます。
  ...
  既定値は `true` です。このスイッチを含むビルドは必ず読み取り側 (`ColumnMetaPB.zstd_compression_dict_page` フィールド、フィールド 35。書き込み側より先に出荷) も含んでおり、ディクショナリページはテーブルプロパティ `zstd_compression_columns` で指定された列に対してのみ書き込まれます。このプロパティを一度も設定していないクラスタは、この既定値にかかわらずディクショナリページを一切書き込みません。

  依然として注意が必要な唯一の期間は、読み取り側を含まない古いリリースからのローリングアップグレードです。圧縮ディクショナリのデータページは raw content ディクショナリ (`dictID=0`) に対して圧縮された ZSTD フレームであり、そのフレームヘッダにはディクショナリが必要であるという情報が含まれません。フィールド 35 を知らない古い BE は、そのページをディクショナリなしで解凍しようとして ZSTD の破損エラーになります。そのようなアップグレードの間は、すべての BE のアップグレードが完了するまで、どのテーブルにも `zstd_compression_columns` を追加しない (またはこのパラメータを `false` にする) でください。バージョン混在期間中のレプリカ間 clone やレプリケーションについても同じことが当てはまります。
...
- 単位: パーセント (0-100)
- 変更可能: はい
- 説明: 共有データモードで、Primary Key インデックスの再構築時に使用される並列プリフェッチ経路に対するメモリ圧迫ゲートです。update メモリトラッカーの使用量が上限のこの割合を超えている場合、再構築は一度に 1 つのデコード済みカラムのみを保持するシングルパスループにフォールバックし、コールドスタートのレイテンシ改善を諦めてピークメモリを抑制します。再構築中の del ファイル、segment ファイルなどの並列読み込みを制御します。値を大きくすると、より高いメモリ圧迫下でも最適化を許可します。`100` に設定するとメモリゲートが無効化され、常に並列実行されます。
- 導入バージョン: -
...
- 単位: -
- 変更可能: はい
- 説明: 共有データモードでの部分更新セグメントレベル並列実行用のスレッドプールの最大スレッド数。このスレッドプールは行モード（並列 load_segment + rewrite_segment）と列モード（並列 DCG 生成）の両方の部分更新で使用されます。0 は CPU コア数の半分に自動設定されることを意味します。
- 導入バージョン: v4.1
...
- 単位: -
- 変更可能: はい
- 説明: Tablet の分割境界を計算する際に、1 つの Tablet から取得するソートキーサンプルの最大数。セグメントの Short Key インデックスがすでにソートキー全体をエンコードしている場合はそのインデックスからサンプルを取得し、そうでない場合はセグメントのデータページを限られた回数だけ読み取って取得するため、この値はその読み取り増幅も制限します。デフォルト値は FE の設定 `tablet_reshard_max_split_count` と一致しています。これは、Tablet を K 個の範囲に分割するには K-1 個の内部境界点が必要なためです。この値を `0` に設定するとサンプリングが無効になり、分割境界は各セグメントの最小値と最大値のソートキーのみから導出されるようになります。精度は粗くなりますが、正しさは保たれます。
- 導入バージョン: -
