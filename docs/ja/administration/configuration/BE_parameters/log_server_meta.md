...
- 単位: -
- 変更可能: いいえ
- 説明: bRPC `TimerThread` が内部ヒープ内のスケジュール解除済みタスクを走査できるようになる最小タスク数です。有効範囲は `[1, 2147483647]` です。走査コストを均すため、bRPC はさらに、ヒープが現在の走査基準値のおよそ2倍まで増えるのを待ちます。この基準値は走査後にリセットされ、タスクの実行によってヒープが縮小すると、それに合わせて低下します。走査によりタスクスロットは再利用できるよう bRPC プールへ戻され、さらなるメモリ増加の抑制には役立ちますが、プロセスの RSS が減少する保証はありません。値を大きくすると走査頻度は下がりますが、一時的に保持されるメモリが増える可能性があります。
- 導入バージョン: -
...
- 単位: ミリ秒
- 変更可能: いいえ
- 説明: 内部ヒープに保留中のタイマーがある場合に、bRPC `TimerThread` がタイマーバケットを処理し、条件付き走査を評価する機会を得るまでの最大スリープ時間です。有効範囲は `[0, 2147483647]` です。`0` は周期的な起床を無効にし、最も近いタスクの実行時刻までスリープするデフォルト動作を維持します。正の値を設定すると、遠い将来の期限を持つタイマーに対する回収機会が増えますが、走査のしきい値とヒープ増加の条件は引き続き適用されるため、設定した間隔内にタスクスロットが回収される保証はありません。この設定はプロセス全体に適用されるため、小さい正の値はすべての bRPC `TimerThread` インスタンスの起床回数と CPU 使用量を増やす可能性があります。
- 導入バージョン: -
