...
- 単位: -
- 変更可能: はい
- 説明: 1 つの bRPC stub が同時に処理中にできる RPC の最大数。`0` 以下の値は制限を無効にし、これがデフォルトです。`brpc_connection_type` が `"pooled"` の場合、stub は処理中の RPC 1 件につきプールから接続を 1 本取り出すため、この値は stub がピアに対して開く接続数の上限にもなります。ピアごとの上限はこの値と `brpc_max_connections_per_server` の積です。`brpc_connection_type` が `"single"` の場合も制限は有効ですが、すべての処理中の RPC が 1 本の接続を多重化するため接続数は制限されません。制限を超えた RPC は待機せず即座に失敗するため、ピーク時の同時実行数より十分大きな値を設定し、通常のスロットリングではなく安全弁として扱ってください。`brpc_max_connection_pool_size` はこの値以上に設定してください。そうしないと、プール容量を超えた接続は繰り返し作成・切断されます。
- 導入バージョン: v4.2.0
