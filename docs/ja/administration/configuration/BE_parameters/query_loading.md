...
- 単位: -
- 変更可能: はい
- 説明: 事前フィルター付き top-k ベクターインデックス検索の返却行数が `min(k, 一致候補数)` より少ない場合に、正確なブルートフォース計算へフォールバックするかどうかを指定します。`true` の場合、StarRocks は一致候補を正確に再計算し、結果を可能な限り `k` 件まで補います。`false` の場合、この件数ベースのフォールバックを行わず、ベクターインデックスの結果をそのまま返します。このパラメータは事前フィルター付き top-k 検索にのみ適用されます。範囲検索では、指定した半径を満たす候補がそれ以上存在しないことで結果が少なくなる場合があるため、適用されません。
- 導入バージョン: -
