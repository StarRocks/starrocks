...
- 単位: 行
- 変更可能: はい
- 説明: 集計ハッシュマップ/ハッシュセットのプローブループにおけるソフトウェアプリフェッチ距離（行単位）。集計ハッシュテーブルの構築中、ループは現在処理中の行からこの値分先にあるバケットをプリフェッチし、大規模なテーブルにおけるメモリレイテンシを隠蔽します。`0` に設定するとソフトウェアプリフェッチが無効になります。この値はチャンクごとに 1 回読み込まれるため、変更は次のチャンクから有効になります。デフォルト値の 16 は、L3 キャッシュに常駐するテーブルに対する経験値に基づいています。DRAM に常駐するワークロードではこの値を大きくし、キャッシュに常駐するワークロードでは小さくしてください。プリフェッチは `agg_prefetch_l2_ratio` によっても制御されます。この距離に関係なく、ハッシュテーブルが L2 キャッシュに収まっている間はプリフェッチは発行されません。
- 導入バージョン: -
...
- 単位: -
- 変更可能: はい
- 説明: L2 レジデンシーに基づいて、集約ハッシュテーブルのソフトウェアプリフェッチを制御します。プリフェッチは、バケット配列が L2 から溢れ出した場合、つまり `bucket_count * slot_bytes >= L2_size * agg_prefetch_l2_ratio` となった場合にのみ有効になります。ここで、L2 サイズは実行時に検出されます（検出に失敗した場合は 1 MiB にフォールバックします）。この閾値未満の場合、テーブルは L2 に常駐しており、プリフェッチは実質的な損失となります。コアあたり多数のドライバを実行する競合の激しい環境では、テーブルあたりのL2占有率が名目上のコアあたりサイズよりも小さくなるため、この比率を下げてください。1.0 以上に設定すると、テーブルが L2 を大幅に超えるまでプリフェッチが遅延します。`agg_hash_map_prefetch_dist` も参照してください。
- 導入バージョン: -
...
- 単位: -
- 変更可能: はい
- 説明: ベクターインデックスの構築時に、構築したばかりのインデックスを `vector_query_cache_capacity` で制御されるベクターインデックスキャッシュにも挿入するかどうかを指定します。デフォルトでは無効です。このキャッシュはクエリのワーキングセットを想定したサイズに設定されているため、ロードや Compaction が新しく構築したインデックスを次々と押し込むと、実行中のクエリが依存しているエントリが追い出され、その代償として一度もクエリされないかもしれないインデックスがウォームアップされるだけになります。クエリパスは、インデックスを最初に読み込む際に引き続きオンデマンドでキャッシュを埋めます。インデックスの構築とクエリが同じノードで実行され、かつ新しく書き込まれたデータがすぐにクエリされる場合にのみ、インデックスファイルの初回読み戻しを省くために有効化してください。変更は、その後に開始されるインデックス構築から反映されます。
- 導入バージョン: v4.2.0
...
- 単位: 秒
- 変更可能: はい
- 説明: 使用されていないベクターインデックスキャッシュエントリが期限切れになるまでのアイドル時間です。最後のキャッシュハンドルが解放された時点から計測を開始し、キャッシュヒット後はそのハンドルが解放された時点で期限を更新します。実行中のクエリによって pin されているエントリは削除されません。IVF-PQ の list block は所有元のインデックスエントリとまとめて解放され、独立した TTL は持ちません。`0` 以下に設定すると期限切れによる削除を無効にします。実行時の変更は、その後に解放されるハンドルに適用され、既存エントリの期限は書き換えられません。
- 導入バージョン: v4.2.0
...
- 単位: -
- 変更可能: はい
- 説明: トップレベルのベクターインデックスキャッシュミスが発生したとき、現在のクエリをブルートフォース検索にフォールバックさせ、同時にバックグラウンドでインデックスをキャッシュへロードするかどうかを指定します。同じインデックスに対する同時ミスは 1 つのバックグラウンドロードを共有し、インデックスの準備が完了するまではブルートフォース検索を使用します。`false` の場合、キャッシュミス時にインデックスを同期的にロードします。予想されるコールドロード遅延がブルートフォース検索のコストより大きく、キャッシュにクエリワーキングセットを保持できる場合にのみ有効にしてください。実行時の変更は、その後に初期化されるベクターインデックス reader に適用されます。
- 導入バージョン: v4.2.0
...
- 単位: スレッド
- 変更可能: いいえ
- 説明: ベクターインデックスキャッシュのバックグラウンドロードスレッドプールで使用する worker の最大数です。worker はオンデマンドで作成され、60 秒間アイドル状態が続くと終了します。`0` 以下の値は `1` に調整されます。この項目は StorageEnv の初期化時に読み取られるため、変更を反映するには BE/CN の再起動が必要です。
- 導入バージョン: v4.2.0
...
- 単位: ミリ秒
- 変更可能: はい
- 説明: 各同期キャッシュ呼び出し元が、進行中のベクターインデックスロードの完了を待機する最大時間です。待機がタイムアウトすると、呼び出し元にはキャッシュミスが返され、クエリパスはブルートフォース検索にフォールバックできます。すでに開始されているローダーはキャンセルされず、バックグラウンドでロードを続行します。後続の各呼び出し元は、設定されたタイムアウトで個別に待機を開始します。`0` 以下に設定すると待機しません。実行時の変更は、その後に開始される待機に適用されます。
- 導入バージョン: v4.2.0
...
- 単位: -
- 変更可能: はい
- 説明: top-k ベクター検索で結果件数が不足することを防ぐために、正確なブルートフォース計算を使用するかどうかを指定します。`true` の場合、述語またはランタイムフィルターが Segment 単位の ANN 検索後に評価されるクエリを正確な計算へ切り替え、事前フィルター付き ANN 検索の返却行数が `min(k, 一致候補数)` より少ない場合は一致候補を正確に再計算します。`false` の場合、一部の述語またはランタイムフィルターを ANN 候補生成後に評価する必要があっても ANN の物理実行プランを維持し、件数ベースの正確な再計算も行わないため、最終結果が `k` 件未満になる可能性があります。件数ベースのフォールバックは top-k 検索にのみ適用されます。範囲検索では、指定した半径を満たす候補がそれ以上存在しないことで結果が少なくなる場合があるため、適用されません。
- 導入バージョン: -
