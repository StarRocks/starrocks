...
- 単位: バイト
- 変更可能: はい
- 説明: 1 つのコンパクションマージが全入力のプリフェッチ範囲用に確保する共有バッファの合計バイト数の上限。共有プールは入力を初期化し、この予算内で登録済みの範囲をプリフェッチします。範囲がすべてプリフェッチされた入力は、その後コンパクションタスクのスレッドでデコードされます。その他の入力はプール上で読み取りとデコードを行い、必要時の読み取りで未プリフェッチのスキャン範囲を共有バッファに読み込むことはありません。この上限には、下位のファイルバッファ、展開後のページと辞書、Chunk、および Segment メタデータは含まれません。0 に設定すると範囲のプリフェッチを無効にします。enable_compaction_parallel_merge_init が有効な場合、並列読み取りとデコードは引き続き実行できます。
- 導入バージョン: v4.2
