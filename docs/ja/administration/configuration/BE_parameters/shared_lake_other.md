...
- 単位: -
- 変更可能: はい
- 説明: 共有データクラスタの主キーテーブルの publish が、必要な基底セグメントを毎回開き直す代わりに、セグメントメタデータキャッシュ経由で読み取るかどうか。行モードの部分更新と条件更新の両方が対象で、どちらも同じセグメントから古い値を読み直します。セグメントを開くにはフッターを解析し、テーブルスキーマの列ごとにカラムリーダーを構築する必要があるため、列数の多いテーブルでは、高頻度の部分更新が publish のたびに同じ基底セグメントに対してこのコストを繰り返し支払うことになります。キャッシュ経由で読み取ると、後続の publish が既に開かれたセグメントを再利用できます。キャッシュは、1 回の publish が読み取る基底セグメントが `lake_metadata_cache_limit` に収まる場合にのみ使用されます。それを超える場合、キャッシュへの追加は追加と同じ速さで既存のエントリを追い出すことになるため、従来の動作に戻ります。この項目を `false` に設定すると、常にセグメントを開き直します。
- 導入バージョン: v4.2
