...
- 単位: -
- 変更可能: はい
- 説明: 共有データクラスタで、コンパクションタスクが入力 Segment オブジェクトを自身の Rowset インスタンス上でタスク終了まで保持するかどうか。保持により、垂直コンパクションのカラムグループごとのパスはメタデータキャッシュ経由で再ロードする代わりにそれらを再利用します。保持しない場合、メタデータキャッシュが全入力 Segment を保持できないとき（`lake_metadata_cache_limit` が小さい、またはノード上のタブレットが多い場合）、各パスが全入力 Segment を再ロード・再パースします。このコストは CPU バウンドでカラム数に比例し、ワイドテーブルのコンパクションを一桁遅くしたり、メモリ制限超過の再試行を繰り返して失敗させたりすることがあります。保持中、タスクは入力 Segment と delete vector を共有メタデータキャッシュに書き込みません。入力はコンパクション完了直後に削除されるため、キャッシュしても他のタブレットのエントリを追い出すだけだからです。メモリコストは実行中タスクごとに入力 Segment メタデータ一式で、コンパクションタスクのメモリトラッカーに計上され、タスクとともに解放されます。`false` に設定すると、共有メタデータキャッシュ経由で Segment を再利用し、そのキャッシュに入力を書き込む以前の動作に戻ります。
- 導入バージョン: v4.2
