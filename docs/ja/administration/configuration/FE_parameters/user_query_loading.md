...
- 変更可能：Yes
- 説明：AIProject で実行される非同期 AI 関数の統計的な推定入力トークン数を制限します。`0` はアドミッションチェックを無効にします。正の値の場合、推定値が `UNKNOWN` または上限を超えると実行を拒否し、上限と等しい場合は許可します。物理プランの構築完了時、スケジューリングの前に一度チェックします。変更に FE の再起動は不要で、新規構築または再構築されたプランに適用されます。チェック済みのプランは再構築されない限り再チェックされません。従来の `ai_query` 関数は対象外です。
- 導入時期：-

options を指定しない `ai_complete`、`ai_custom_query`、`ai_embed`、`ai_custom_embedding` のパススルーテキスト入力を推定できます。文字列リテラルには既知の UTF-8 バイト長を使用します。サポートされる ANALYZE 統計があるネイティブテーブルの `VARCHAR` 列の直接参照には、実際の UTF-8 エンコード幅が統計に記録されないため、平均文字数の 4 倍を使用します。この換算は 1 文字あたり最大 4 バイトを見込みますが、統計的な過小推定を防ぐ保証ではありません。換算後の各バイトを推定テキストトークン 1 個として数えます。プラン内の各チャット評価には、固定の system/user メッセージ用に 16 トークンの予約量を加算します。これは概算であり、プロバイダーで実測したトークン数ではありません。NULL と確定できる入力はゼロとして扱いますが、統計だけでは NULL 評価分をメッセージの予約量から除外しません。これらをプランの評価回数で乗算し、すべての AI 呼び出しについて合計します。

複雑な式、組み込みプロンプトテンプレートを使用する AI 関数、options、JOIN、CTE、未対応または欠落した統計、およびローカル TopN プランでは `UNKNOWN` になる場合があります。アドミッションチェックを有効にする前や上限を決める前に、代表的なクエリの `EXPLAIN VERBOSE` または `EXPLAIN COSTS` で `AI INPUT TOKENS` を確認してください。これはクエリごとの事前確認であり、ワークロード全体の観測モードではありません。`EXPLAIN ANALYZE` はステートメントを実行し、アドミッションチェックを適用します。`SCHEDULER` を含むその他の `EXPLAIN` レベルでは、説明対象のステートメントにアドミッションチェックを適用しません。

この設定は、1 回のプラン実行における推定入力処理量を制御します。正確なトークナイザー、課金クォータ、出力トークン数の上限ではなく、追加の HTTP 再試行も計上しません。
