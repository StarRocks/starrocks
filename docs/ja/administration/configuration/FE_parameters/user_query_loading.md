...
- 変更可能：Yes
- 説明：AIProject で実行される非同期 AI 関数の統計的な推定入力トークン数を制限します。`0` はアドミッションチェックを無効にします。正の値の場合、推定値が `UNKNOWN` または上限を超えると実行を拒否し、上限と等しい場合は許可します。予算は AI プランの構築時に一度取得されます。変更に FE の再起動は不要で、既存プランの予算は変わりません。従来の `ai_query` 関数は対象外です。
- 導入時期：-

options を指定しない `ai_complete`、`ai_custom_query`、`ai_embed`、`ai_custom_embedding` のパススルーテキスト入力を推定できます。評価ごとに、文字列リテラルには UTF-8 バイト長を使用します。サポートされる ANALYZE 統計があるネイティブテーブルの `VARCHAR` 列の直接参照には、平均文字数の 4 倍を使用します。NULL でないチャット評価には 16 の予約量を加算します。これらをプランの評価回数で乗算し、すべての AI 呼び出しについて合計します。

複雑な式、組み込みプロンプトテンプレートを使用する AI 関数、options、JOIN、CTE、未対応または欠落した統計、およびローカル TopN プランでは `UNKNOWN` になる場合があります。`EXPLAIN VERBOSE` または `EXPLAIN COSTS` の `AI INPUT TOKENS` で推定値を確認できます。通常の `EXPLAIN`、`EXPLAIN VERBOSE`、`EXPLAIN COSTS` はステートメントを実行せず、アドミッションチェックも行いません。
