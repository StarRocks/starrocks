...
- 単位：-
- 変更可能：Yes
- 説明：分割が完全な UNSHARE 再書き込みを伴う場合に、1 つの分割ジョブが処理できるソースタブレットの最大数。単一の UNSHARE コンパクションが書き直すデータ量を制限し、そのコンパクションがパーティション唯一のコンパクションスロットを占有して size-tiered コンパクションを妨げる時間を制限します。最大のタブレットが優先的に選択されます。`0` 以下の値はウェアハウスのコンピュートノード数を意味します。
- 導入時期：-
