...
- 単位：Bytes
- 変更可能：Yes
- 説明：タブレットのプリスプリットで生成されるタブレットの最小サイズ。プリスプリット時のコンピュートノード数へのアライメントを制限し、ノード数の多いクラスターで小さなロードが多数の極小タブレットに分割されないようにします。`tablet_reshard_target_size` 以下にする必要があります。また、v4.1.5 および v4.2.0 以降、マテリアライズドインデックスのタブレット数がウェアハウスのコンピュートノード数を下回っている間は、この値がそのフェーズのターゲットタブレットサイズとしても使用されるため、この値を大きくすると早期分割のタイミングも遅くなります。`tablet_reshard_enable_early_split` を参照してください。このフェーズで生成されるタブレットは、一時的にこの値より小さくなることがあります。
- 導入時期：v4.1.0
...
- 単位：-
- 変更可能：Yes
- 説明：タブレット数がウェアハウスのコンピュートノード数を下回っているマテリアライズドインデックスが、`tablet_reshard_target_size` に達するのを待たずに `tablet_reshard_min_split_size` を基準に分割できるかどうか。このルールではこの値がターゲットタブレットサイズとして機能するため、タブレットがその 1.5 倍に達した時点で分割されます（デフォルトの 2 GB では 3 GB）。作成直後のパーティションはタブレットが 1 つしかないため、分割できる大きさに成長するまでは 1 つのコンピュートノードでしか書き込みを受け付けられません。この設定により、そのようなパーティションがクラスター全体の書き込み並列度に大幅に早く到達できます。継続的なロードで特に効果があります。早期分割は、サイズに基づく規則ではまったく分割が発生しない場合にのみ適用され、インデックスのタブレット数がウェアハウスのコンピュートノード数を超えることはありません。スロットリングとして、他のタブレット SPLIT または MERGE ジョブが実行中の間、早期分割は通常見送られます。この設定を `false` にすると、分割は `tablet_reshard_target_size` のみに従うようになります。すでに開始されたジョブには影響しません。この設定は共有データモードでのみ有効です。
- 導入時期：v4.1.5, v4.2.0
...
マルチパーティション経路は、サンプリングベースのタブレット事前分割を「1 つのステートメントで複数のパーティションをターゲットにする取り込み」へと拡張するものです。運用上の留意事項は 2 点：
