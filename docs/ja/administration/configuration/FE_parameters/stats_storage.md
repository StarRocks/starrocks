...
- 単位：-
- 変更可能：Yes
- 説明：range 分散の PRIMARY KEY テーブルが主キーと異なる `ORDER BY` キーを宣言できるかどうか。無効の場合、そのようなテーブルは作成時に拒否され、既存のものは再シャーディングされません。この形状のテーブルの分割では境界は主キー空間のままで、ファイルはソートキー順に並んだままとなるため、UNSHARE コンパクションが各子タブレットのデータを private に書き換えるまで、読み取りは親タブレットから提供されます。初期実装では `file_bundling=true` が必要で、再シャーディング中の DCG/IDG サイドカーは拒否されます。
- 導入時期：v4.1.3
