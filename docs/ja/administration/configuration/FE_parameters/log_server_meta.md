...
- 単位：秒
- 変更可能：Yes
- 説明：Leader の降格時に、進行中の WAL 適用の完了、journal writer の封印と停止、および Leader セッションのワーカースレッド、スレッドプール、クリーンアップの完了を待つタイムアウトです。Follower の replay はこれらの完了後に開始されます。セッションの終了待ちは journal の封印後に残った時間を使用します。降格は協調的な停止要求を使用し、実行中の業務スレッドを interrupt しません。待機がタイムアウトした場合、またはクリーンアップに失敗した場合、FE プロセスはクリーンな再起動のために終了します。通常の進行中の処理により長い時間が必要な場合は、この値を増やしてください。
- 導入時期：v4.2.0
