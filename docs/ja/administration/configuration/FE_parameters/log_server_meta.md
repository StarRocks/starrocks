...
- 単位：バイト
- 変更可能：Yes
- 説明：各 FE における組み込み監査ローダーのバッファのバイト上限で、1 回のロードバッチの最大サイズでもあります。バッファが上限の半分に達した時点でフラッシュがトリガーされ、残りの半分が次のデーモン周期までの流入を吸収します。下流の処理速度が監査イベントの流入速度を下回り続けてバッファが埋まったままになると、超過した監査イベントは破棄されカウントされます（FE ログにレート制限付きの警告が出力されます）。本機能は運用およびセキュリティ分析を目的としており、極端な過負荷時の監査データ無損失は保証しません。FE 停止時に未フラッシュのバッファイベントも失われます（外部 auditloader プラグインと同じ挙動です）。取り込みセマンティクスは at-least-once です。BE がコミット済みで応答が失われた場合、そのバッチは再送され、テーブルに重複行が残ります。フラッシュ時には組み立てと送信のためにバッチのコピーを一時的に保持するため、バッファに加えてそのコピーも収まるだけの FE ヒープを確保してください。1 件の SQL ステートメントのテキストはテーブル内で最大 1 MB まで保持され、それを超える部分は切り詰められます。設定値が 1.2 MB（最悪ケースの 1 行）から 512 MB の範囲外の場合はその範囲にクランプされます（FE ログにレート制限付きの警告が出力されます）。`enable_audit_loader` が有効な場合にのみ有効です。
- 導入時期：v4.2
...
- 単位：秒
- 変更可能：Yes
- 説明：組み込み監査ローダーの 2 回のフラッシュ間の最大間隔。監査イベントは FE メモリ内でバッチ化され、この間隔で監査テーブルに書き込まれます。バッファが `audit_loader_batch_max_bytes` の半分に達した時点でも早期にフラッシュされるため、監査データの可視化遅延はおおよそこの間隔にロード時間を加えたものになります。値を小さくすると可視化遅延は下がりますが、ロードトランザクションの頻度が上がります。0 を指定するとデーモン周期ごとにフラッシュを試みます。負の値は 0 として扱われ、1 日を超える値は 1 日にクランプされます（FE ログにレート制限付きの警告が出力されます）。バッチの読み込みが失敗し続ける場合（例えば対象の BE/CN が利用できない場合）、FE は有限回数リトライした後にそのバッチを破棄し（ログに記録しカウントします）、以降のすべてのイベントが滞留し続けるのを防ぎます。デフォルト設定では、試行が即座に失敗する場合は約 30 秒、各試行がリクエストタイムアウトに達する場合は最長約 90 秒です。`enable_audit_loader` が有効な場合にのみ有効です。
- 導入時期：v4.2
...
- 単位：-
- 変更可能：Yes
- 説明：組み込み監査ローダーを有効にするかどうか。有効にすると、FE は監査イベント（クエリと接続）をテーブル `starrocks_audit_db__.starrocks_audit_tbl__` に永続化し、SQL で直接監査データを分析できます。監査データベースとテーブルは機能の有効化後に Leader FE が自動的に作成し、日単位でパーティション化して直近 30 個の日次パーティションを保持します。`ALTER TABLE ... SET ("partition_live_number"="N")` で調整でき、システムはこの変更を上書きしません。テーブルを誤って削除した場合は空のテーブルとして自動的に再作成されます。クラスターに外部の動的 AUDIT プラグイン（auditloader プラグインなど）がインストールされている場合、監査データの二重取り込みを避けるため本機能は無効状態を維持し、外部プラグインのアンインストール後に自動的に復帰します。監査テーブルに NOT NULL カラムを追加しないでください。FE が書き込まないカラムは取り込み時に DEFAULT 値ではなく NULL が設定されるため、すべてのバッチが拒否され、数回のリトライの後に破棄されます。当該カラムを削除すれば、FE を再起動せずに取り込みが復旧します。本機能は `fe.audit.log` から独立しており、共存できます。
- 導入時期：v4.2
...
- 単位：-
- 変更可能：No
- 説明：`thrift_server_max_worker_threads` のワーカー スレッドがすべて処理中のときに、ワーカー スレッドの空きを待って待機できる接続の数。この値が制限するのは待機キューが保持できる接続数であり、最終的に処理される接続数ではありません。キューが満杯の状態で到着した接続は、再試行されることなく直ちにクローズされます。また、キューに入ってからの待機時間が `thrift_server_queue_timeout_ms` を超えた接続は、ワーカー スレッドが取り出した時点で処理されずにクローズされます。この 2 つはそれぞれ `starrocks_fe_thrift_server_rejected_connections_total` と `starrocks_fe_thrift_server_expired_connections_total` でカウントされます。
- 導入時期：-
...
- 単位：ms
- 変更可能：Yes
- 説明：ワーカー スレッドに取り出されるまで、接続が Thrift サーバーの待機キューに滞留できる最大時間。これを超えて待機した接続は処理されずにクローズされます。呼び出し元がすでに諦めており、処理してもワーカーを占有して、まだ待っている処理を妨げるだけだという前提に基づきます。有効な場合、バースト後にキューが排出されるまでの時間も、キューがどれだけ深くなっていたかにかかわらず上限で抑えます。

  デフォルトの `0` はこのチェックを無効にし、滞留時間にかかわらずすべての接続が処理されます。デフォルトで無効なのは、上記の前提がすべての呼び出し元に当てはまるわけではなく、待機中の接続にはサーバーが読み取れる期限が付いていないためです。呼び出し元が実際に用いる期限は 4 桁の幅があります。FE 内部の通常の RPC は `thrift_rpc_timeout_ms`（10 秒）で諦めますが、Follower から Leader に転送された文はセッションの実行タイムアウトに `thrift_rpc_timeout_ms` を加えた時間まで待機し、`query_timeout` のデフォルトでは約 310 秒、`insert_timeout` のデフォルトでは約 4 時間になります。BE のストリームロードおよびトランザクション コミットの RPC は `stream_load_thrift_rpc_timeout_ms`（60 秒）と、そのロードの `timeout_second` の 4 分の 1 のうち大きい方まで待機します。`query_timeout`、`insert_timeout`、`timeout_second` はいずれもユーザーが設定でき上限もないため、すべてのクラスターにとって安全な固定値は存在しません。回復を早めるほど短いタイムアウトは、まさにそれが緩和しようとしている飽和のさなかに、クライアントがまだ待っている転送済みの文を切断しかねません。

  クラスターごとに、そのクラスターのクライアントが依存する最大の期限を上回る値で有効化してください。値はそのクラスターの `query_timeout` と `insert_timeout` から導出します。破棄された接続は `starrocks_fe_thrift_server_expired_connections_total` でカウントされ、対象ピアを示すレート制限付きの WARN ログが出力されます。`starrocks_fe_thrift_server_queue_wait_ms` はチェックの有効・無効にかかわらずキューの待機時間を報告するので、有効化前の値の見積もりに利用してください。
- 導入時期：v4.2.0
...
- 単位：バイト
- 変更可能：Yes
- 説明：前回のロール以降に書き込まれたメタデータログエントリの合計サイズがこの値を超えると、新しいログファイルが作成されます（書き込みバッチごとにチェック）。個々のログエントリが大きいワークロード向けに `edit_log_roll_num` を補完し、FE 再起動時に再生が必要なジャーナル量を件数だけでなくサイズでも制限します。`0`（デフォルト）はサイズベースのトリガーを無効にします。`edit_log_roll_num` は常に適用されます。
- 導入時期：-
...
- 単位：-
- 変更可能：No
- 説明：カタログ名、データベース名、テーブル名、ビュー名、マテリアライズドビュー名の大文字小文字を区別しない処理を有効にするかどうか。この機能はデフォルトで無効であり、これらの名前は大文字小文字を区別します。有効にすると、StarRocks はこれらの名前を小文字で格納し、**クエリと書き込み（DDL/DML）の処理段階で、すべてのカタログ名、データベース名、テーブル名、ビュー名、およびマテリアライズドビュー名を強制的に小文字に変換します**。この機能はクラスター作成時にのみ有効にできます。**明確かつ十分に理解された理由がない限り、この機能は無効のままにすることを強く推奨します**。理由は次のとおりです。
  ...
  - **クラスター作成後は変更できません。** クラスター起動後、この設定の値をいかなる方法でも変更することはできません。変更しようとするとエラーが発生し、FE は、この設定アイテムの値がクラスターが最初に起動されたときと一致しないことを検出すると、起動に失敗します。
  - すべてのオブジェクト名（アクセス予定のすべての外部データソース内の名前を含む）がすでに小文字である場合にのみ、新しいクラスターでこの機能を有効にしてください。
- 導入時期：v4.0
...
- 単位：-
- 変更可能：No
- 説明：Lock Manager がメタデータロックを分散させるハッシュバケット (ロックテーブル) の数です。各リソースはリソース ID によって 1 つのバケットにマッピングされ、各バケットはそれぞれ独自のミューテックスで保護されます。そのため、値を大きくすると、無関係なリソースを操作するロッカー間の競合が減りますが、メモリ使用量がわずかに増えます。この値は Lock Manager の作成時に一度だけ読み取られるため、変更は FE の再起動後にのみ有効になります。v4.1.0、v4.0.1、v3.5.8 以降、デフォルト値は `32` から `256` に変更されました。
- 導入時期：v3.3.0
