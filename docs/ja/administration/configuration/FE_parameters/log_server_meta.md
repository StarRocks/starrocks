...
- 単位：-
- 変更可能：No
- 説明：Lock Manager がメタデータロックを分散させるハッシュバケット (ロックテーブル) の数です。各リソースはリソース ID によって 1 つのバケットにマッピングされ、各バケットはそれぞれ独自のミューテックスで保護されます。そのため、値を大きくすると、無関係なリソースを操作するロッカー間の競合が減りますが、メモリ使用量がわずかに増えます。この値は Lock Manager の作成時に一度だけ読み取られるため、変更は FE の再起動後にのみ有効になります。v4.1.0 以降、デフォルト値は `32` から `256` に変更されました。
- 導入時期：v3.3.0
