...
- 単位：バイト
- 変更可能：Yes
- 説明：各 FE における組み込み監査ローダーのバッファのバイト上限で、1 回のロードバッチの最大サイズでもあります。バッファが上限に達すると即座にフラッシュがトリガーされます。下流の取り込み速度が監査イベントの流入速度を継続的に下回りバッファが満杯のままの場合、超過した監査イベントは FE メモリ保護のために破棄されカウントされます（FE ログにレート制限付きの警告が出力されます）。この機能は運用・セキュリティ分析を目的としており、極端な過負荷時に監査データの損失ゼロを保証するものではありません。フラッシュ時には一時的にバッチサイズの約 3 倍の追加メモリコピーを保持するため、この値を大きくする前に FE ヒープが十分であることを確認してください。1 つの SQL ステートメントのテキストはテーブルに最大 1 MB まで保存され、それを超える部分は切り詰められます。`enable_audit_loader` が有効な場合にのみ有効です。
- 導入時期：v4.2
...
- 単位：秒
- 変更可能：Yes
- 説明：組み込み監査ローダーの 2 回のフラッシュ間の最大間隔。監査イベントは FE メモリ内でバッチ化され、この間隔ごと（またはバッファが `audit_loader_batch_max_bytes` に達した時点）で監査テーブルに書き込まれるため、監査データの可視化遅延はおおよそこの間隔にロード時間を加えたものになります。小さくすると可視化遅延は下がりますが、ロードトランザクションの頻度が増えます。`enable_audit_loader` が有効な場合にのみ有効です。
- 導入時期：v4.2
...
- 単位：-
- 変更可能：Yes
- 説明：組み込み監査ローダーを有効にするかどうか。有効にすると、FE は監査イベント（クエリと接続）をテーブル `starrocks_audit_db__.starrocks_audit_tbl__` に永続化し、SQL で直接監査データを分析できます。監査データベースとテーブルは機能の有効化後に Leader FE が自動的に作成します（日単位パーティション、デフォルト保持期間 30 日。`ALTER TABLE ... SET ("partition_live_number"="N")` で調整可能で、システムはこの変更を上書きしません。テーブルを誤って削除した場合は空のテーブルとして自動的に再作成されます）。クラスターに外部の動的 AUDIT プラグイン（auditloader プラグインなど）がインストールされている場合、監査データの二重取り込みを避けるため本機能は自動的に無効状態を維持し、外部プラグインのアンインストール後に自動的に復帰します。本機能は `fe.audit.log` から独立しており、共存できます。注意：`ADMIN SET FRONTEND CONFIG` は現在接続している FE にのみ有効で、再起動後には失われます。マルチ FE クラスターでは各 FE で有効化し、永続化するには各 FE の `fe.conf` に記載してください。
- 導入時期：v4.2
