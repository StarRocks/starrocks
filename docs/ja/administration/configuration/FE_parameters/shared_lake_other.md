...
- 単位：-
- 変更可能：Yes（再起動不要）
- 説明：ネイティブ Iceberg GEOGRAPHY クエリの CN 間データ転送を許可します。この設定だけではネイティブ読み取りは有効になりません。
- 導入時期：-
...
- 単位：-
- 変更可能：Yes（再起動不要）
- 説明：CRS84 の球面セマンティクスを持つトップレベルの Iceberg GEOGRAPHY 列で、ネイティブ型の使用を許可します。転送と MySQL 出力の設定も有効にする必要があります。それ以外の場合、これらの列は未対応のままですが、通常の列はクエリできます。作成済みのネイティブクエリプランも、送信前に現在の設定を再確認します。Spill を有効にしたネイティブクエリや、MySQL 以外への出力は拒否されます。GEOMETRY、ネストしたネイティブ GEO、書き込み、コンストラクター、計算関数は有効になりません。
- 導入時期：-
