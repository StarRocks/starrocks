...
- 単位：-
- 変更可能：Yes
- 説明：`date_trunc(granularity, col)`（granularity が `day` 以上）でパーティション化されたレンジマテリアライズドビューについて、より粗いベーステーブルパーティションを固定粒度のセルに展開するのではなく、ベーステーブルの実際の（混合粒度の可能性がある）パーティションバウンダリをミラーするかどうか。有効にすると、月にマージされた履歴のベースパーティションは 1 か月の MV パーティションとして保持され、直近の日パーティションは日パーティションとして保持されるため、各ベースパーティションは 1 つの MV パーティションに対応します。幅がちょうど 1 粒度単位のベースパーティション（離散およびマルチ union の場合を含む）では、デフォルトの動作と同じです。非アラインの入力ではデフォルトのマッパーにフォールバックします。
- 導入時期：-
