...
- 単位：Seconds
- 変更可能：Yes
- 説明：共有データクラスタにおいて、1 つのパーティションに対する 1 回の AutoVacuum ラウンドにコンピュートノードが費やせる最大時間です。この時間が経過するとノードはラウンドを終了し、そこまでの進捗を報告します。次のラウンドはその続きから再開されます。`3600` (vacuum RPC のタイムアウトで、コンパイル時定数) より大きい値、または `1` より小さい値は `3600` として扱われます。バージョンチェーンが長く 1 ラウンドで走査しきれないパーティションでは、この値を下げると vacuum ワーカーをより早く解放できます。1 ラウンドを短くしてもラウンド数が増えるだけで、進捗が減ることはありません。
- 導入時期：v4.2.0
