...
- 単位：-
- 変更可能：Yes
- 説明：共有データクラスターにおけるインクリメンタル（分割・再開可能）AutoVacuum の 1 ラウンドあたりの作業量上限。1 回の AutoVacuum ラウンドが 1 つのパーティションに対してコミットおよび提案できるメタデータバージョンの最大数です。インクリメンタルプロトコルは常に有効で無効化できません。レガシーの一括走査パスはその後で安全に再開できない（分割ラウンドが残したチェーンの穴で停止し、その下のガベージを取りこぼす）ため、フォールバックは存在しません。この値は各ラウンドの作業量を制限するだけで、大量に蓄積された履歴バージョンが、1 回の RPC でタイムアウトを繰り返すことなく、複数のラウンドにわたって段階的に削除されるようにします。`100` 未満の値は `100` に切り上げられます（1 ラウンドあたりの予算が小さすぎるとバックログの削除に多くのラウンドが必要になるため）。tablet からノードへのルーティングは変更しません。
- 導入時期：v4.1.0
...
- 単位：-
- 変更可能：Yes
- 説明：共有データクラスタで増分 AutoVacuum がスタックした場合の復旧用エスケープハッチ：増分 vacuum 状態を強制リセットする物理パーティション ID のセミコロン区切りリスト（形式 `id1;id2`）。AutoVacuum デーモンが次にリスト内のパーティションを処理する際、進行中の proposal・再開カーソル・tablet 世代を破棄し（回収済みの retain floor は保持するため、新しいパスは削除済みバージョンを再読み込みしません）、そのラウンドをスキップして、次のラウンドでクリーンなパスを開始します。各 ID は一度だけ消費されます（デーモンがリセット後ただちにリストから削除するため）。したがってスタックしたパスは毎ラウンドリセットされるのではなく、一度だけ促されます。この項目を処理するのは現在の Leader FE だけです：`ADMIN SET FRONTEND CONFIG` は値をすべての FE にブロードキャストしますが、各 FE は Leader になった時点で引き継いだ値を破棄します。そのため failover 直前のリクエストは失われ、新しい Leader に対して再度発行する必要があります。通常運用では空のままにしてください。
- 導入時期：v26.2.0
...
- デフォルト：`max(2, CPU コア数 * 3 / 4)`
- タイプ：Int
- 単位：Count
- 変更可能：No
- 説明：`"iceberg.catalog.type" = "hive"` で作成された Iceberg カタログの Hive metastore (HMS) クライアントプールのデフォルトサイズ。プールサイズは、そのカタログが同時に発行できる HMS 呼び出しの数を制限します。Iceberg 自身のデフォルトは `2` であり、いくつのスレッドがメタデータをロードしていても、その metastore に対する外部メタデータロードの並行度がこの値に固定されてしまいます。そのため、この項目は FE の CPU コア数からデフォルト値を導出します。カタログプロパティ `clients` はこの項目より優先され、カタログが `clients` を自身で設定していない場合にのみ、この項目の値が使われます。Iceberg は HMS クライアントプールを metastore URI ごとに FE プロセス全体でキャッシュするため、次の 2 つの影響があります。1 つ目は、作成済みのプールは値が変わっても再作成されないため、この項目の変更は FE を再起動して初めて反映される点です。そのため、`SHOW CREATE CATALOG` が新しい値を表示していても、実際に有効な並行度は古い値のままということがあります。2 つ目は、同じ metastore URI を指すすべてのカタログが 1 つのプールを共有し、その容量は起動後にその URI を最初に初期化したカタログによって決まる点です。後から作成されたカタログが持つ値は暗黙的に無視されます。同じ metastore URI を指すすべてのカタログには同じ値を設定してください。
- 導入時期：v4.2.0
...
- 単位：-
- 変更可能：Yes
- 説明：FE がサービスポートをバインドする前に、起動時のマテリアライズドビューのアクティベーションタスクが完了するまで待機するかどうか。これらのタスクは共有プランキャッシュプールのすべてのスレッドを占有するため、この項目を `false` に設定すると、そのプールに積まれた他の処理（プランキャッシュの構築を含む）がまだアクティベーションの後ろで待たされている状態でポートが開かれます。`true` に設定すると、再起動中の FE は起動時のアクティベーションが完了するまで接続を受け付けないため、それらの構築はその後フルの並列度で実行できます。なお、この待機が対象とするのはアクティベーションのみです。プランキャッシュの構築は別途キューイングされ待機対象には含まれないため、ポートが開いた直後に発行されたクエリが、まだプランを構築していないマテリアライズドビューに遭遇する可能性は残ります。この項目は起動レディネスゲートのキルスイッチであり、ゲートを段階的に有効化するためのつまみではありません。この項目は動的に変更できますが、FE は起動パスで一度だけ読み取ります。実行時に変更しても現在のプロセスには影響せず、次回の再起動時にのみ有効になります。`enable_mv_post_image_reload_cache` が `false` に設定されている場合、この項目は効果がありません。マテリアライズドビューの reload が完全に同期実行となり、ゲートが待機する非同期タスクが送信されないためです。
- 導入時期：v4.2.0
...
- 単位：ミリ秒
- 変更可能：Yes
- 説明：次の gtid が現在のバッチ終端を超えるときに、FE が先回りして予約するミリ秒ウィンドウ。`1` 未満の値は `1` として扱います。ウィンドウは snowflake の残り 42 ビットタイムスタンプに収まっている必要があり、それを超えると予約境界をラップせず発番に失敗します。有効な範囲で値を大きくするとメタデータジャーナルの書き込みは減ります。副作用として、リーダーフェイルオーバー時に未使用の予約は破棄され、gtid のタイムスタンプはこのウィンドウ分まで前方に跳ねることがあります。
- 導入時期：-
...
- 単位：Milliseconds
- 変更可能：Yes
- 説明：bookmark reference の 1 回のリースが継続できる時間のクラスター全体の上限。reference 自身の TTL に関わらず、リースがこの上限を超えた reference はクリーンアップ処理によって回収されます。`<= 0` の場合は上限を無効にします。reference の実際のリースは、この上限と reference 自身の TTL のうち小さい方ですが、どちらか一方が `<= 0` の場合はその側を制限なしとして扱うため、自身の TTL を設定しない reference もこの上限で制限されます。起点は最後の更新（更新されたことがない場合は取得時刻）です。更新し続ける保持者は reference を無期限に維持できます。クリーンアップ処理は毎回この値を読み直すため、値を小さくすると**すでに付与済み**のリースも短くなります。最後の更新からの経過時間が新しい上限に達した時点で reference は回収され、更新間隔がその上限より長い保持者は次の更新を迎える前に bookmark を失います。`bookmark_renew` の戻り値は、その呼び出し時点でのみ有効な上限です。この上限は最後の更新から計測されるため、更新されなくなった bookmark が、置き換えられた（reshard 以前の）index generation をリサイクルビンに保持できる時間の上限にもなります。reference が回収されればその generation は消去可能になるため、bookmark 保持者のリークやマテリアライズドビューの停止によってシャードが無期限に占有されることはありません。
- 導入時期：
...
- 単位：-
- 変更可能：Yes
- 説明：bookmark メタデータ関数(`bookmark_create` / `bookmark_renew` / `bookmark_release`)を有効にするかどうか。これらはデバッグ/テスト専用の関数で、デフォルトでは無効になっています。実行には OPERATE 権限が必要で、FE leader ノードでのみ実行できます。`holder` 引数は呼び出し側が自称するラベルであり、認証された identity ではありません。これらの関数を呼べる利用者は、他の保持者が作成した参照を解放したり、より短い TTL で再取得したりできます。`HOLDER_ID` は `information_schema.table_bookmark_references` から読み取れます。
- 導入時期：
