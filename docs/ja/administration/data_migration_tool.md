...
- StarRocks クロスクラスタデータ移行ツールは、共有なしクラスタから別の共有なしクラスタまたは共有データクラスタへのデータ移行のみをサポートします。2 つの共有データクラスタ間の移行については、[共有データクラスタ向けクロスクラスタデータ移行ツール](./lake_to_lake_migration.md)を参照してください。
