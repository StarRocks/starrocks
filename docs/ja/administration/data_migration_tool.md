...
Range 分散テーブルを移行する前に、両方のクラスタービルドに FE の Range トポロジーおよび正確な境界による分割ブリッジと、次の 2 つの共有データサーバー修正が含まれていることを確認してください。

- 古いデータバージョンのメタデータファイルが存在しない場合、分割後の子 Tablet が現在のメタデータとデータバージョンを UUID ファイル重複排除の基準として使用できること。
- 共有オブジェクトとバンドルされた Segment が 1 回だけコピーされ、物理パスが正しく、論理オフセットとサイズが維持されること。

これらの依存関係をすべて含むビルドがデプロイされるまで、エンドツーエンド検証または本番移行を実行しないでください。
...
このワークフローは分割による収束のみをサポートします。ソースでのマージ、ターゲットのみに存在する境界、交差する Range、またはソースより細かいターゲットトポロジーなど、収束にターゲットでのマージが必要な場合は安全側に停止します。Range Colocate レイアウトもサポートされません。ターゲットクラスターで透過的データ暗号化が有効な場合、分割後の複数の Range Tablet が共有する物理ファイルを新たにコピーすることはサポートされません。ターゲットに既存のファイルは、その暗号化メタデータを使用して引き続き再利用できます。
