...
- `VARBINARY` 型のレンジ分散列はサポートされません。初回リリースの境界転送では任意のバイナリ値を損失なく保持できないため、ターゲットクラスターは分割を送信する前にこのようなテーブルを拒否します。
- レンジ分散移行では、ソースクラスターとターゲットクラスターの両方で透過的データ暗号化（TDE）を無効にする必要があります。ターゲットは、暗号化されたソーススナップショットファイルと、ターゲット側で暗号化が必要な共有ファイルをデータコピー前に拒否します。
...
- TDE で暗号化されたストレージファイルを含むソーススナップショットはサポートされません。分割後に共有されるファイルを参照するレンジ分散移行では、ターゲットクラスターでも TDE を無効にする必要があります。
- レンジ分散移行は分割のみをサポートし、各論理パーティションに 1 つの物理パーティションを持ち、レンジ分散列に `VARBINARY` を含まない非 Colocate テーブルに限定されます。`range-colocate`、`VARBINARY` レンジキー、ソースでの統合、ターゲットでの統合が必要なトポロジーはサポートされません。
