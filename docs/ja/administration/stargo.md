...
description: "StarGo は複数の StarRocks クラスタを管理するためのコマンドラインツールです。"
---
...
StarGo は複数の StarRocks クラスタを管理するためのコマンドラインツールです。StarGo を使うことで、複数のクラスタを簡単にデプロイ、確認、アップグレード、ダウングレード、起動、停止することができます。
...
- **repo.yaml**: StarRocks インストーラーのダウンロードパスを設定するファイルです。

> Note
`http://cdn-thirdparty.starrocks.com` にアクセスすることで、対応するインストールインデックスファイルとインストーラーを取得できます。
...
- デプロイ対象のクラスタには、少なくとも 1 つの中央制御ノードと 3 つのデプロイノードが必要です。すべてのノードは 1 台のマシン上にデプロイすることも可能です。
- StarGo は中央制御ノードにデプロイする必要があります。
...
### デプロイディレクトリの作成（任意） {#create-deployment-directory-optional}

デプロイ対象の StarRocks のパスが存在せず、そのパスを作成する権限がある場合、これらのパスを事前に作成する必要はなく、StarGo が設定ファイルに基づいて自動的に作成します。パスが既に存在する場合は、書き込み権限があることを確認してください。また、各ノードに必要なデプロイディレクトリを、以下のコマンドを実行して手動で作成することもできます。
...
デプロイが成功すると、クラスタは自動的に起動します。beStatus と feStatus が true になっていれば、クラスタの起動は成功しています。
...
また、MySQL クライアントでクラスタに接続してテストすることもできます。
...
### クラスタ内のすべてのノードを起動する {#start-all-nodes-in-a-cluster}
...
### 特定のロールのノードを起動する {#start-nodes-of-a-specific-role}
...
### 特定のノードを起動する {#start-a-specific-node}
...
## クラスタを停止する {#stop-cluster}

StarGo を使用して StarRocks クラスタを停止できます。

### クラスタ内のすべてのノードを停止する {#stop-all-nodes-in-a-cluster}
...
### 特定のロールのノードを停止する {#stop-nodes-of-a-specific-role}
...
### 特定のノードを停止する {#stop-a-specific-node}
...
## クラスタをスケールアウトする {#scale-cluster-out}

StarGo を使用してクラスタをスケールアウトできます。

### 設定ファイルを作成する {#create-configuration-file-1}
...
### SSH 相互認証を構築する {#build-ssh-mutual-authentication}

クラスタに新しいノードを追加する場合は、新しいノードと中央制御ノードの間で相互認証を構築する必要があります。詳細な手順については [Prerequisites](#prerequisites) を参照してください。

### デプロイディレクトリを作成する（任意） {#create-deployment-directory-optional-1}

デプロイ対象の新しいノードの下のパスが存在せず、そのパスを作成する権限がある場合、これらのパスを自分で作成する必要はなく、StarGo が設定ファイルに基づいて作成します。パスがすでに存在する場合は、それらへの書き込みアクセス権があることを確認してください。次のコマンドを実行して、各ノードに必要なデプロイディレクトリを作成することもできます。
...
### クラスタをスケールアウトする {#scale-the-cluster-out}
...
## クラスタをスケールインする {#scale-cluster-in}
