...
- デプロイするクラスターには、少なくとも 1 つの中央制御ノードと 3 つのデプロイメントノードが必要です。すべてのノードを 1 台のマシンにデプロイできます。
...
StarRocks をデプロイするパスが存在しない場合、またそのパスを作成する権限がある場合、これらのパスを作成する必要はありません。StarGo は構成ファイルに基づいてそれらを作成します。パスが既に存在する場合は、それらに書き込みアクセス権があることを確認してください。また、次のコマンドを実行して各ノードに必要なデプロイメントディレクトリを作成することもできます。
...
また、MySQL クライアントを使用してクラスターに接続することでテストすることもできます。
...
新しいノードをデプロイするパスが存在しない場合、またそのパスを作成する権限がある場合、これらのパスを作成する必要はありません。StarGo は構成ファイルに基づいてそれらを作成します。パスが既に存在する場合は、それらに書き込みアクセス権があることを確認してください。また、次のコマンドを実行して各ノードに必要なデプロイメントディレクトリを作成することもできます。
