...
StarRocks をデプロイするパスが存在しない場合、またそのパスを作成する権限がある場合は、これらのパスを作成する必要はありません。StarGo は設定ファイルに基づいてそれらを作成します。既にパスが存在する場合は、それらに書き込みアクセス権があることを確認してください。また、以下のコマンドを実行して各ノードに必要なデプロイメントディレクトリを作成することもできます。
...
以下のコマンドを実行して、特定のクラスターの情報を表示します。
...
特定のノードの ID を確認するには、[特定のクラスターの情報を表示](#view-the-information-of-a-specific-cluster) を参照してください。
...
以下のテンプレートに基づいてスケールアウトタスクのトポロジファイルを作成します。必要に応じて FE および/または BE ノードを追加するファイルを指定できます。詳細については [Configuration](../administration/management/FE_configuration.md) を参照してください。
...
新しいノードをデプロイするパスが存在しない場合、またそのパスを作成する権限がある場合は、これらのパスを作成する必要はありません。StarGo は設定ファイルに基づいてそれらを作成します。既にパスが存在する場合は、それらに書き込みアクセス権があることを確認してください。また、以下のコマンドを実行して各ノードに必要なデプロイメントディレクトリを作成することもできます。
