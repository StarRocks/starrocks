...
FE がメタデータチェックポイントを完了した後に新しいメタデータイメージを作成するたびに、自動的にスナップショットが作成されます。スナップショットの名前は、`automated_snapshot_{timestamp}` という形式でシステムによって生成されます。
...
1. **(オプション)** クラスタスナップショットを保存しているストレージの場所（ストレージボリューム）が変更された場合、元のストレージパスのすべてのファイルを新しいパスにコピーする必要があります。これを実現するには、すべての FE ノードのディレクトリ **fe/conf** に設定ファイル **cluster_snapshot.yaml** を配置する必要があります。**cluster_snapshot.yaml** のテンプレートについては、[Appendix](#appendix) を参照してください。
...
4. **ローカルキャッシュをクリーニングした後に** CN ノードを起動します。

   ```Bash
   ./be/bin/start_cn.sh --daemon
   ```

ステップ 1 で **cluster_snapshot.yaml** を追加した場合、ノードとストレージボリュームはファイルの情報に基づいて新しいクラスタで再設定されます。元のクラスタの情報は自動的に削除または置き換えられます。
...
cluster_snapshot:
    # スナップショットの URI。
    # 例: s3://defaultbucket/test/f7265e80-631c-44d3-a8ac-cf7cdc7adec811019/meta/image/automated_cluster_snapshot_1704038400000
...
# このセクションが指定されていない場合、復旧後も元のクラスタのノード情報が保持されます。
