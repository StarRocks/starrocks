...
LDAP 内でユーザーの DN を指定せずにユーザーを作成することも可能です。ユーザーがログインすると、StarRocks は LDAP システムにアクセスしてユーザー情報を取得します。一致するものが一つだけある場合、認証は成功します。
...
LDAP 認証では、クライアントが StarRocks にクリアテキストのパスワードを渡す必要があります。クリアテキストのパスワードを渡す方法は3つあります。
