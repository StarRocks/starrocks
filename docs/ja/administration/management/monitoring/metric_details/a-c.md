...
- 単位: カウント
- 説明: 圧縮ディクショナリの対象となる列が最終的にディクショナリを得られず、通常の ZSTD として書き込まれた累計回数。試験対象のページがディクショナリありでも十分に小さくならなかった場合 (`zstd_compression_dict_min_gain` を参照)、またはディクショナリの構築が失敗した場合に計上されます。列ごと Segment ごとに 1 回カウントされます。サンプルにするには小さすぎたページはここに計上されません。次のページで改めて試されます。`zstd_compression_dict_pages_written` に対してこの値が大きい場合、テーブルプロパティ `zstd_compression_columns` で指定した列に対して圧縮ディクショナリがほとんど効いていないことを意味します。
