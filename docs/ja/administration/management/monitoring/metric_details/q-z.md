...
- 単位: カウント
- タイプ: 累積値
- 説明: スキャン時に観測された flat JSON サブカラムアクセスのヒット回数の累計。単一スキャンの `flat_json_hits` と `merge_json_hits` 統計から集計。

## `starrocks_be_flat_json_access_miss_total`

- 単位: カウント
- タイプ: 累積値
- 説明: スキャン時に観測された flat JSON サブカラムアクセスのミス回数の累計。単一スキャンの `dynamic_json_hits` 統計から集計（flat サブカラムとして物化されていないパス）。
...
- 単位: カウント
- タイプ: 累積値
- 説明: コンパクション中に入力の flat JSON スキーマが前のものと異なり、`HyperJsonTransformer` が再初期化された回数。値が高い場合はコンパクション対象のセグメント間でスキーマが頻繁に変化していることを示す。

## `starrocks_be_flat_json_compaction_total`

- 単位: カウント
- タイプ: 累積値
- 説明: `FlatJsonColumnCompactor` が JSON カラムをフラット化したコンパクション呼び出しの総回数。

## `starrocks_be_flat_json_flatten_duration_ns_total`

- 単位: ナノ秒
- タイプ: 累積値
- 説明: スキャン時に JSON をフラット化するのに費やした累積時間。

## `starrocks_be_flat_json_merge_duration_ns_total`

- 単位: ナノ秒
- タイプ: 累積値
- 説明: スキャン時に flat JSON サブカラムを完全な JSON に結合し直すのに費やした累積時間。

## `starrocks_be_flat_json_paths_discovered_total`

- 単位: カウント
- タイプ: 累積値
- 説明: flat JSON セグメント書き込み中に `JsonPathDeriver` が検出した JSON パスの総数。

## `starrocks_be_flat_json_paths_extracted_total`

- 単位: カウント
- タイプ: 累積値
- 説明: `FlatJsonColumnWriter` がサブカラムとして物化した JSON パスの総数（合成の null / remain カラムを含む）。
...
- 単位: カウント
- タイプ: 累積
- ラベル: `reason` — `SkipReason` 列挙値（小文字化）。`not_range_distribution`、`table_not_normal`、`has_materialized_view_or_rollup`、`unsupported_sort_key`、`metadata_not_resolved`、`multiple_base_index_tablets`、`partition_not_empty`、`disabled_by_config`、`disabled_by_session` のいずれか。
- 説明: FE 側の eligibility ゲートがサンプラーを起動する前にサンプリングベースのタブレット事前分割を拒否した回数を、理由別に集計した累計値。運用者はこのカウンタを参照することで「事前分割が実行されていない」原因を、どの eligibility 分岐に起因するか一目で判別できます。
...
- 単位: カウント
- タイプ: 累積
- ラベル: `reason` — eligibility 通過後の失敗カテゴリ（小文字化された `SkipReason`）。`sample_failed`（サンプラー実行が例外）、`timeout_pre_submit`（サンプリング + プランニング + 構築フェーズが `tablet_pre_split_pre_submit_timeout_seconds` を超過）、`submit_failed`（`TabletReshardJobMgr` が受理を拒否）のいずれか。
- 説明: サンプラーが起動したものの、受理された reshard ジョブを生成できなかった回数を、理由別に集計した累計値。`tablet_pre_split_eligibility_skipped`（サンプラーが起動しなかった）や `tablet_pre_split_tier_used`（成功したティアを記録）とは別物です。Tier 1 → Tier 2 のフォールバック自体は失敗ではなく、`tablet_pre_split_tier_used{tier=data}` で追跡されます。

## `starrocks_fe_tablet_pre_split_tier_used`

- 単位: カウント
- タイプ: 累積
- ラベル: `tier` — `metadata`（Parquet/ORC の row-group 統計から境界を計算。行データは読まない）または `data`（FILES サブクエリで実際に取得した行サンプルから境界を計算。Tier 2 の直接起動と Tier 1 → Tier 2 のフォールバックの両方を含む）。
...
- 単位: ミリ秒
- タイプ: ヒストグラム
- 説明: 受理されたサンプリングベースのタブレット事前分割 reshard ジョブの `FINISHED` をコーディネーターが待機した壁時計時間。オプションの同期待機ラッパーが使用された場合にのみ更新されます（現状ではテスト経路と将来の同期統合のみ）。本番取り込み経路は fire-and-forget のため、本番ではこのメトリックは 0 のままです。

## `starrocks_fe_tablet_pre_split_post_submit_hard_cap`

- 単位: カウント
- タイプ: 累積
- 説明: サンプリングベースのタブレット事前分割で post-submit ハードキャップが発火した累計回数。受理された reshard ジョブが `tablet_pre_split_post_submit_wait_seconds` 以内に `FINISHED` に到達しなかったときにインクリメントされます。オプションの同期待機ラッパーが使われた場合にのみ発火します（現状ではテスト経路と将来の同期統合のみ）。本番取り込み経路は fire-and-forget のため、本番ではこのメトリックは 0 のままです。

## `starrocks_fe_tablet_pre_split_load_abort`

- 単位: カウント
- タイプ: 累積
- 説明: サンプリングベースのタブレット事前分割が、受理された reshard ジョブの `FINISHED` 到達を期限内に確認できなかったために中止された取り込みトランザクションの累計回数。`tablet_pre_split_post_submit_hard_cap` の兄弟カウンタ。オプションの同期待機ラッパーが使われた場合にのみ発火します。本番取り込みが事前分割を理由に中止されることはないため、本番ではこのメトリックは 0 のままです。
