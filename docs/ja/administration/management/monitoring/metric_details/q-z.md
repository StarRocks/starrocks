...
- 単位: ミリ秒
- タイプ: ヒストグラム
- 説明: 受理されたサンプリングベースのタブレット事前分割 reshard ジョブの `FINISHED` をコーディネーターが待機した壁時計時間。INSERT-from-FILES 本番経路で更新されます（hook が同期的に FINISHED を待つことで、当該 INSERT 自身が分割後のタブレットレイアウトに対してプランされる）。テスト用の `runPreSplit` 同期待機ラッパーでも更新されます。Broker Load 本番経路は fire-and-forget のため、このヒストグラムは更新されません。
