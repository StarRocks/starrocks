...
- 単位: カウント
- タイプ: 瞬間
- ラベル: `type` (`current` または `starmgr_current`)
- 説明: 現在 BDB JE に保持されている編集ログの数。保持されている最も古いジャーナルから、最後に書き込まれたジャーナルまでを数えます。チェックポイントデーモンは、登録済みのすべての FE が新しいイメージを受け取った後にのみ古いジャーナルを削除するため、到達不能な登録済み FE があるとこの値は増え続け、メタデータディスクが逼迫していることの早期警告となります。この値は収集のたびにジャーナルから読み取られるため、FE の再起動直後でも正確であり、ジャーナルが実際に削除されると同時に低下します。`current` は FE メタデータジャーナルを表します。`starmgr_current` は StarMgr ジャーナルを表し、ストレージ・コンピュート分離モードでは FE メタデータジャーナルと同じ BDB JE 環境に格納されますが、それぞれ独立したチェックポイントデーモンによって削除されるため、独立して増加し同じメタデータディスクを消費する可能性があります。ストレージ・コンピュート一体モードでは常に 0 を報告します。
