...
- 単位: カウント（`current`）/ バイト（`current_bytes`）
- タイプ: 累積
- ラベル: `type` (`current` または `current_bytes`)
- 説明: 最後に有効なクリーンアップが行われて以降の FE メタデータジャーナルの滞留量です。`current` は件数、`current_bytes` はバイト数を数えます。いずれもジャーナルの書き込みに応じて加算され、チェックポイントのラウンドが実際に journal database を削除したときにのみ基準が引き直されます（`current_bytes` は 0 に戻り、`current` は BDB JE が保持しているジャーナル件数に設定されます）。チェックポイントデーモンは登録済みのすべての FE が新しいイメージを受け取った後にのみ古いジャーナルを削除するため、到達不能な登録済み FE があると両方の系列が増え続け、メタデータディスクが逼迫していることの早期警告となります。

  注意点が 2 つあります。FE プロセスが再起動すると両方の系列は 0 から再開します。また、いずれもストレージ・コンピュート分離モードで同じ BDB JE 環境を共有する StarMgr ジャーナルは含まず、FE メタデータジャーナルのみを対象とします。counter として報告されるため、ディスク使用量の正確な値ではなく、増加傾向の観察に使用してください。
