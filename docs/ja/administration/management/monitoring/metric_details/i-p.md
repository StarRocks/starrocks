...
- 単位: バイト
- タイプ: Gauge
- 説明: この FE 上の低基数グローバル辞書キャッシュ（`CacheDictManager`）にキャッシュされている辞書データの合計バイト数。キャッシュによって正確に追跡され（サンプリングではありません）、キャッシュされた辞書が保持するヒープの推定値であり、辞書 Map におけるキーごとのオブジェクトオーバーヘッドも含みます。このキャッシュは `low_cardinality_dict_cache_max_bytes` 設定によってこのサイズで上限が設定されます。
