...
- 単位: 個
- タイプ: 累積
- 説明: ストレージ・コンピュート分離（lake）コンパクションタスクが発行したオブジェクトストレージへの PUT 呼び出し回数。S3 だけでなく、任意のオブジェクトストレージバックエンド（S3、OSS、GCS、Azure Blob、HDFS など）に対して適用されます。ファイルのクローズごとに 1 PUT としてカウントされます（出力セグメント、sst、`.lcrm`、トランザクションログ）。これは概算値です: 大きなファイルのマルチパートアップロードは複数の実際の PUT を発行し、また publish 時の tablet メタデータ書き込みはここには含まれません（コンパクションタスクの外で発生するため）。
...
- 説明: ストレージ・コンピュート分離（lake）コンパクションタスクが生成する出力セグメントファイルサイズの分布。パーセンタイルを使用することで、多数の小さなファイルを生成しオブジェクトストレージの API コストを増大させる非効率なコンパクションを特定できます。注意: このメトリクスは bvar `LatencyRecorder` に基づいて実装されているため、単位はバイトですが、パーセンタイル系列の名前には `_latency` というサフィックスが付きます。
