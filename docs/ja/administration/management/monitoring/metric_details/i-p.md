...
- タイプ: Gauge
- 説明: 直近にストレージ・コンピュート分離（lake）コンパクションジョブを起動したパーティションの Centiscore。値はそのパーティション内タブレットの *最大* スコア（`Quantiles.getMax()`）で、スケジューラがコンパクション対象パーティションを選ぶ判定基準と一致します。long 型 Gauge で小数点以下 2 桁の精度を保つために 100 倍されているため、ダッシュボードでは値を 100 で割って元のスコアを復元してください。トリガごとに各パーティションで 1 回更新され、Gauge は最新の更新値を保持します。この Gauge は減衰しません。コンパクションが実行されていないときは直近のトリガ値を保持し（0 にリセットされません）、その値は次のトリガが上書きするまで FE Leader の切り替えをまたいで保持されます。そのため、この指標を単独で参照するのではなく、`lake_compaction_running > 0` と組み合わせてアラートを設定してください。`is_leader` ラベルが付与されており、Follower FE は `is_leader="false"` を返し値は 0 になるため、ダッシュボードでは `is_leader="true"` でフィルタしてください。
