...
- 単位: 個
- 説明: 共有データの `ADD INDEX` ファストパスが、対象列がその segment に物理的に存在しないために構築できなかった (segment, index) ペアのカウンタ。`ALTER TABLE ... ADD COLUMN` で追加された列では、これは正常な結果です。この操作はメタデータのみを更新し履歴 segment を書き換えないため、該当行は列のデフォルト値を読み取ります。したがってクエリ結果は正しいままで、インデックスによる絞り込みだけが利用できません。後続の書き換え (compaction など) がそのインデックスを持つスキーマを解決した時点で、対象 segment はインデックスを獲得します。本メトリクスは BE の稼働期間中のスキップ発生回数の単調カウンタであり、現在のカバレッジを示す gauge ではありません。書き換えで解消しても減少せず、スキップ後に失敗した alter も加算されます。この状況が発生するか、どの程度の頻度かを把握する用途に使用してください。現時点でインデックスカバレッジが完全かどうかは判断できません。
