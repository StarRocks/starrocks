...
- 単位: 個
- タイプ: 瞬時値
- ラベル: `host`（lag の報告対象である、生存中の非 Leader フロントエンドの IP またはホスト名）
- 説明: あるフロントエンドが再生済みの journal id が Leader の最新 journal id からどれだけ遅れているか、すなわち `leader_max_journal_id - replayed_journal_id` を表します。このメトリクスは Leader FE のみが報告します。権威ある最新の journal id を保持し、ハートビートを通じて他の各フロントエンドの再生済み journal id を収集しているのは Leader だけだからです。生存中の非 Leader フロントエンドごとに 1 本の系列が出力され、`host` ラベルで区別されます。Leader 以外の Follower / Observer はこのメトリクスの系列をまったく出力しません。値が継続的に増加する場合、対応する Follower / Observer がメタデータの再生で遅れていることを示します。
