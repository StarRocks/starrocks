...
- 単位: 個
- タイプ: 累積
- 説明: ストレージ・コンピュート分離（lake）コンパクションタスクが発行したオブジェクトストレージへの PUT 呼び出し回数。ファイルのクローズごとに 1 PUT としてカウントされます（出力セグメント、sst、`.lcrm`、トランザクションログ）。これは概算値です: 大きなファイルのマルチパートアップロードは複数の実際の PUT を発行し、また publish 時の tablet メタデータ書き込みはここには含まれません（コンパクションタスクの外で発生するため）。
