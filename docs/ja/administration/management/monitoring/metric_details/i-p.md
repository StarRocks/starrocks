...
- 説明: Iceberg `UPDATE` タスクによって書き込まれた総バイト数を、ファイル種別ごとに集計します。`data` は更新後の行を含む新規データファイルのサイズを表し、`position_delete` は更新対象の旧行をマークする position delete ファイルのサイズを表します。

## `iceberg_update_duration_ms_total`

- 単位: ミリ秒
- タイプ: 累積
- 説明: Iceberg `UPDATE` タスクの総実行時間（ミリ秒）。各タスクの実行時間は、タスク終了後に加算されます。
...
- タイプ: 累積
- ラベル: `file_type` (`data` または `position_delete`)
- 説明: Iceberg `UPDATE` タスクによって書き込まれたファイルの総数を、ファイル種別ごとに集計します。`data` は新規データファイルの件数を、`position_delete` は position delete ファイルの件数を数えます。

## `iceberg_update_rows`

- 単位: 行
- タイプ: 累積
- 説明: Iceberg `UPDATE` タスクが影響を与えた行の総数。生成されるファイル数に関係なく、1 行は 1 回のみカウントされます。
...
- ラベル:
  - `status` (`success` または `failed`)
  - `reason` (`none`、`timeout`、`oom`、`access_denied`、`unknown`)
- 説明: Iceberg テーブルを対象とする `UPDATE` タスクの総数。このメトリクスは、各タスクが成功・失敗にかかわらず終了するたびに 1 ずつ加算されます。Iceberg UPDATE は V2 Merge-On-Read モデルを採用しており、単一のスナップショットでデータファイルと position delete ファイルを原子的に書き込みます。
...
- 単位: KiB
- タイプ: 分布（gauge 系列として公開: `_count`、`_latency_80/90/99`、`_max_latency`。分位数のサフィックスは BE の `bvar_latency_p1/p2/p3` フラグに従い、デフォルトは p1=80）
- 説明: ストレージ・コンピュート分離（lake）コンパクションタスクが生成する出力セグメントファイルサイズの分布。パーセンタイルを使用することで、多数の小さなファイルを生成しリモートストレージの API コストを増大させる非効率なコンパクションを特定できます。サイズはバイトではなく KiB で記録されます。バックエンドの bvar `LatencyRecorder` はパーセンタイルのサンプルを 32 ビットで保持するため、バイト単位で 4 GiB 以上のサンプルはラップアラウンドしてしまいますが、KiB を使うことでその上限が 4 TiB まで引き上がります。注意: このメトリクスは bvar `LatencyRecorder` に基づいて実装されているため、ファイルサイズを測定しているにもかかわらず、パーセンタイル系列の名前には `_latency` というサフィックスが付きます。

## `lake_compaction_remote_write_count`

- 単位: 個
- タイプ: 累積
- 説明: ストレージ・コンピュート分離（lake）コンパクションタスクが発行したリモートストレージへの書き込み回数。「リモート」はバックエンド中立で、オブジェクトストレージバックエンド（S3、OSS、GCS、Azure Blob）と HDFS の両方を対象とします。ファイルのクローズごとに 1 回の書き込みとしてカウントされます（出力セグメント、sst、`.lcrm`）。トランザクションログもカウントされますが、コンパクションタスク自身が書き込む場合のみです。トランザクションログの書き込みを呼び出し側/publish パスに委ねるコンパクション（例: 並行コンパクションはマージ済みトランザクションログを FE に返して永続化させる）では、ここではカウントされません。これは概算値です: 大きなファイルのマルチパートアップロードは内部で複数の実際の PUT を発行し、publish 時の tablet メタデータ書き込みも同様にここには含まれず（いずれもコンパクションタスクの外で発生するため）、さらにセグメントごとの独立したインデックス付属ファイル（該当するインデックスを持つテーブルに対して書き出されるベクトルインデックス `.vi` ファイルおよび転置/GIN インデックスファイル）もカウントされません（カウントされるのはセグメントデータファイルのみです）。そのため、ベクトルインデックスや転置インデックスを持つテーブルでは、実際のリモート書き込み回数はこれより多くなります。
