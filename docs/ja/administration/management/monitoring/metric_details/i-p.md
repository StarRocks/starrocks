...
- タイプ: Histogram
- 説明: ストレージ・コンピュート分離（lake）コンパクションジョブが作成され実行中マップに入った時点での、パーティションのコンパクションスコアの分布。スコアはパーティション内タブレット単位のスコアの平均（`Quantiles.getAvg()`）であり、long 型 Histogram で小数点以下 2 桁の精度を保つために 100 倍されています。ダッシュボードでは報告された分位値を 100 で割って元のスコアを復元してください。トリガごとに各パーティションで 1 回サンプリングされます。`is_leader` ラベルが付与されており、Follower FE は `is_leader="false"` で空のスナップショットを出力するため、ダッシュボードでは `is_leader="true"` でフィルタしてください。
