...
- 単位: -
- 説明: 直近 10 秒間の Linux メモリ PSI（Pressure Stall Information）の `full` パーセンテージ。すなわち、すべての非アイドルタスクがメモリ待ちでストールしていた時間の割合です。この値が高止まりする場合、マシン全体（または BE の cgroup）がメモリ回収でスタックしていることを示します。BE プロセスの cgroup v2 `memory.pressure` から優先的に読み取り、取得できない場合は `/proc/pressure/memory` にフォールバックします。PSI 非対応カーネルでは 0 が報告されます。

## `mem_psi_full_avg300`

- 単位: -
- 説明: 直近 300 秒間のメモリ PSI `full` パーセンテージ。`mem_psi_full_avg10` を参照してください。

## `mem_psi_full_avg60`

- 単位: -
- 説明: 直近 60 秒間のメモリ PSI `full` パーセンテージ。`mem_psi_full_avg10` を参照してください。

## `mem_psi_full_total_us`

- 単位: us
- 説明: すべての非アイドルタスクがメモリ待ちでストールしていた累計時間（マイクロ秒、単調増加）。`rate()` を使うとストール比率を正確に算出できます。

## `mem_psi_some_avg10`

- 単位: -
- 説明: 直近 10 秒間のメモリ PSI `some` パーセンテージ。すなわち、少なくとも 1 つのタスクがメモリ待ちでストールしていた時間の割合です。全体的なストールになる前のメモリ圧迫を示します。

## `mem_psi_some_avg300`

- 単位: -
- 説明: 直近 300 秒間のメモリ PSI `some` パーセンテージ。`mem_psi_some_avg10` を参照してください。

## `mem_psi_some_avg60`

- 単位: -
- 説明: 直近 60 秒間のメモリ PSI `some` パーセンテージ。`mem_psi_some_avg10` を参照してください。

## `mem_psi_some_total_us`

- 単位: us
- 説明: 少なくとも 1 つのタスクがメモリ待ちでストールしていた累計時間（マイクロ秒、単調増加）。
