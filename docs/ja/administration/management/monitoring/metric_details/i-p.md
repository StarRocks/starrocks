...
- 単位: 個
- タイプ: 累積
- 説明: ストレージ・コンピュート分離（lake）コンパクションタスクが発行したリモートストレージへの書き込み回数。「リモート」はバックエンド中立で、オブジェクトストレージバックエンド（S3、OSS、GCS、Azure Blob）と HDFS の両方を対象とします。ファイルのクローズごとに 1 回の書き込みとしてカウントされます（出力セグメント、sst、`.lcrm`、トランザクションログ）。これは概算値です: 大きなファイルのマルチパートアップロードは内部で複数の実際の PUT を発行し、また publish 時の tablet メタデータ書き込みはここには含まれません（コンパクションタスクの外で発生するため）。
