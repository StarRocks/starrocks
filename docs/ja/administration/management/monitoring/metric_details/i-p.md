...
- 説明: ストレージ・コンピュート分離（lake）コンパクションタスクが生成する出力セグメントファイルサイズの分布。パーセンタイルを使用することで、多数の小さなファイルを生成しリモートストレージの API コストを増大させる非効率なコンパクションを特定できます。注意: このメトリクスは bvar `LatencyRecorder` に基づいて実装されているため、単位はバイトですが、パーセンタイル系列の名前には `_latency` というサフィックスが付きます。
