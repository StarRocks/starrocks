...
- 単位: 件数
- 説明: FE リーダーの `runningCompactions` マップで現在追跡されているストレージ・コンピュート分離（lake）コンパクションジョブの数。値は `lake_compaction_running` と同じで、この名前でも公開されます。`is_leader` ラベルが付与されており、Follower FE は `is_leader="false"` で値 0 を出力するため、ダッシュボードでは `is_leader="true"` でフィルタしてください。

## `lake_compaction_score_at_trigger`

- 単位: Centiscore（生スコア × 100）
- タイプ: Gauge
- 説明: 直近にストレージ・コンピュート分離（lake）コンパクションジョブを起動したパーティションの Centiscore。値はそのパーティション内タブレットの *最大* スコア（`Quantiles.getMax()`）で、スケジューラがコンパクション対象パーティションを選ぶ判定基準と一致します。long 型 Gauge で小数点以下 2 桁の精度を保つために 100 倍されているため、ダッシュボードでは値を 100 で割って元のスコアを復元してください。トリガごとに各パーティションで 1 回更新され、Gauge は最新の更新値を保持します。`is_leader` ラベルが付与されており、Follower FE は `is_leader="false"` を返し値は 0 になるため、ダッシュボードでは `is_leader="true"` でフィルタしてください。
