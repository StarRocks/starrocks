...
以下のメトリクスは、トランザクションのさまざまなフェーズのレイテンシ分布を提供する `summary` タイプのメトリクスです。これらのメトリクスは、リーダーFEノードによってのみ報告されます。
...
- 単位: ms
- タイプ: Summary
- 説明: `prepare` 時間から `finish` 時間まで測定される、トランザクションが完了するまでの合計レイテンシを追跡します。これはトランザクションの完全なエンドツーエンドの期間を表します。

#### starrocks_fe_txn_write_latency_ms

- 単位: ms
- タイプ: Summary
- 説明: トランザクションの「書き込み」フェーズのレイテンシを測定します（`prepare` 時間から `commit` 時間まで）。このメトリックは、トランザクションが公開される前のデータ書き込みおよび準備段階のパフォーマンスを分離します。

#### starrocks_fe_txn_publish_latency_ms

- 単位: ms
- タイプ: Summary
- 説明: 「公開」フェーズの合計レイテンシを測定します（`commit` 時間から `finish` 時間まで）。これは、コミットされたトランザクションがクエリに表示されるまでにかかる時間です。これは、`schedule`、`execute`、および `ack` の3つのサブフェーズの合計です。

#### starrocks_fe_txn_publish_schedule_latency_ms

- 単位: ms
- タイプ: Summary
- 説明: トランザクションがコミットされた後、公開タスクが取得されるまで待機する時間を追跡します（`commit` 時間から）。このレイテンシは、公開パイプラインでのスケジューリングの遅延またはキューイング時間を反映しています。

#### starrocks_fe_txn_publish_execute_latency_ms

- 単位: ms
- タイプ: Summary
- 説明: 公開タスクが取得されてから終了するまでのアクティブな実行時間を測定します。これは、トランザクションの変更を可視化するために行われる実際の作業を表します。

#### starrocks_fe_txn_publish_ack_latency_ms

- 単位: ms
- タイプ: Summary
- 説明: 公開タスクが終了してから、トランザクションが `VISIBLE` としてマークされる最終的な `finish` 時間までの最終確認レイテンシを測定します。これには、必要な最終ステップや確認が含まれます。
