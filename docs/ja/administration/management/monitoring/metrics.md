...
- 単位: -
- 説明: 各Routine LoadジョブにおけるKafkaパーティションの最大オフセットラグ。これは、FEの設定項目 `enable_routine_load_lag_metrics` が `true` に設定され、かつオフセットラグがFEの設定項目 `min_routine_load_lag_for_metrics` 以上である場合にのみ収集されます。デフォルトでは、`enable_routine_load_lag_metrics` は `false` に設定されており、`min_routine_load_lag_for_metrics` は `10000` に設定されています。
