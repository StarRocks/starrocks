...
StarRocks クラスタの監視サービスの構築方法について詳しくは、 [Monitor and Alert](./Monitor_and_Alert.md) を参照してください。
...
- 単位: -
- タイプ: Instantaneous
- 説明: セーフモードが有効かどうかを示します。有効な値は `0`（無効）と `1`（有効）です。セーフモードが有効な場合、クラスタはロードリクエストを受け付けません。
...
- 単位: ms
- タイプ: Instantaneous
- 説明: 特定のウェアハウスで最後に完了したクエリまたはロードの終了時間を示します。共有なしクラスタの場合、この項目はデフォルトのウェアハウスのみを監視します。
...
- 単位: Bytes
- 説明: 主キーテーブルの DELETE ベクターのキャッシュに使用されるメモリの総量。
...
- 単位: Bytes
- 説明: アロケータによってマッピングされた物理的に存在するデータページの最大バイト数で、アロケータメタデータに専用のすべてのページ、アクティブな割り当てをサポートするページ、および未使用のダーティページを含みます。
...
- 単位: Count
- 説明: スリフト接続の総数（完了した接続を含む）。

### thrift_opened_clients

- 単位: Count
- 説明: 現在開いているスリフトクライアントの数。

### thrift_used_clients

- 単位: Count
- 説明: 現在使用中のスリフトクライアントの数。
...
- 単位: Count
- 説明: ベースコンパクションおよび累積コンパクションからマージされた行セットの総数。
...
- 単位: Bytes
- 説明: munmap(2) などの操作によってオペレーティングシステムに返されるのではなく保持された仮想メモリマッピングの総バイト数。

### unused_rowsets_count

- 単位: Count
- 説明: 未使用の行セットの総数。これらの行セットは後で回収されることに注意してください。
...
- 単位: Bytes
- 説明: メタデータに専用のバイト数の合計で、ブートストラップに敏感なアロケータメタデータ構造と内部割り当てに使用されるベース割り当てを含みます。透明な巨大ページの使用はこの項目には含まれません。
...
- 単位: Bytes
- 説明: 主キーテーブルの APPLY タスクおよび主キーインデックスによって使用されるメモリ。
...
- 単位: Count
- 説明: 主キーテーブルの行セット COMMIT リクエストの失敗総数。
...
- 単位: -
- 説明: 各リソースグループの CPU コア制限の比率とすべてのリソースグループの総 CPU コア制限の比率。これは瞬時値です。

### resource_group_inuse_cpu_cores

- 単位: Count
- 説明: 各リソースグループで現在使用中の CPU コアの推定数。これは2つのメトリック取得間の時間間隔の平均値です。

### resource_group_cpu_use_ratio (Deprecated)

- 単位: -
- 説明: 各リソースグループで使用されたパイプラインスレッドの時間スライスの割合とすべてのリソースグループで使用された総時間スライスの割合。これは2つのメトリック取得間の時間間隔の平均値です。

### resource_group_connector_scan_use_ratio (Deprecated)

- 単位: -
- 説明: 各リソースグループで使用された外部テーブルスキャンスレッドの時間スライスの割合とすべてのリソースグループで使用された総時間スライスの割合。これは2つのメトリック取得間の時間間隔の平均値です。

### resource_group_scan_use_ratio (Deprecated)

- 単位: -
- 説明: 各リソースグループで使用された内部テーブルスキャンスレッドの時間スライスの割合とすべてのリソースグループで使用された総時間スライスの割合。これは2つのメトリック取得間の時間間隔の平均値です。
...
- 単位: Count
- 説明: BE でスケジューリングを待っている準備完了ドライバの現在の数。
