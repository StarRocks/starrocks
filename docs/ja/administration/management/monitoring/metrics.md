...
このトピックでは、StarRocks の重要な一般メトリクスについて紹介します。
...
StarRocks クラスタの監視サービスの構築方法についての詳細は、[Monitor and Alert](./Monitor_and_Alert.md) を参照してください。
...
- 単位: Count
- タイプ: Average
- 説明: BE のベースコンパクション速度（rowsets）。

### be_cumulative_compaction_rowsets_per_second

- 単位: Count
- タイプ: Average
- 説明: BE の累積コンパクション速度（rowsets）。
...
- 単位: Count
- タイプ: Instantaneous
- 説明: チェックポイントのない Edit Logs の数。`100000` 以内の値が妥当とされます。
...
- 単位: -
- タイプ: Average
- 説明: リソースグループが使用した CPU 時間の割合と、すべてのリソースグループの CPU 時間の割合。
...
- 単位: -
- タイプ: Instantaneous
- 説明: セーフモードが有効かどうかを示します。有効な値は `0`（無効）と `1`（有効）です。セーフモードが有効になると、クラスタはロードリクエストを受け付けなくなります。
...
- 単位: ms
- タイプ: Instantaneous
- 説明: 特定のウェアハウスで最後に終了したクエリまたはロードの終了時間を示します。共有なしクラスタの場合、この項目はデフォルトのウェアハウスのみを監視します。
...
- 単位: Count
- タイプ: Instantaneous
- 説明: 各 BE ノードでの最高のコンパクションスコアを示します。
...
- 単位: Count
- 説明: デルタカラムグループキャッシュヒットの総数（主キーテーブルのみ）。

### data_stream_receiver_count

- 単位: Count
- 説明: BE で Exchange 受信機として機能するインスタンスの累積数。
...
- 単位: Count
- 説明: 主キーテーブルの DELETE ベクターキャッシュアイテムの数。
...
- 単位: Bytes
- 説明: レプリカクローンのために使用されるメモリ。

### fragment_requests_total

- 単位: Count
- 説明: BE で実行されるフラグメントインスタンスの総数（非パイプラインエンジン用）。
...
### thrift_current_connections (廃止予定)
...
- 単位: Count
- 説明: ベースコンパクションおよび累積コンパクションからマージされた rowsets の総数。
...
- 単位: Bytes
- 説明: munmap(2) などの操作を通じてオペレーティングシステムに返されるのではなく、保持される仮想メモリマッピングの総バイト数。

### unused_rowsets_count

- 単位: Count
- 説明: 未使用の rowsets の総数。これらの rowsets は後で回収されます。

### update_rowset_commit_apply_total

- 単位: Count
- 説明: 主キーテーブルの COMMIT および APPLY の総数。
...
- 単位: Count
- 説明: セグメントフラッシュスレッドプール内のキュータスクの数。

### jemalloc_metadata_bytes

- 単位: Bytes
- 説明: メタデータに専用のバイト数の合計で、ブートストラップに敏感なアロケータメタデータ構造および内部割り当てに使用されるベース割り当てを含みます。透明な巨大ページの使用はこの項目に含まれません。
...
- 単位: Bytes
- 説明: 主キーテーブル APPLY タスクおよび主キーインデックスによって使用されるメモリ。

### blocks_created_total (廃止予定)
...
- 単位: Count
- 説明: Routine Load およびテーブル関数を介したロードを処理するために使用される RPC スレッドプールの現在のサイズ。デフォルト値は 10 で、最大値は 1000 です。この値はスレッドプールの使用状況に基づいて動的に調整されます。

### publish_version_queue_count

- 単位: Count
- 説明: Publish Version スレッドプール内のキュータスクの数。
...
- 単位: Count
- 説明: BE で Exchange 送信機として機能するインスタンスの累積数。

### disk_sync_total (廃止予定)
...
- 単位: Count
- 説明: カラムの部分更新の APPLY タスクの総数（カラムモード）。
...
- 単位: Count
- 説明: ストレージページキャッシュのヒット数の総数。

### bytes_read_total (廃止予定)

### update_rowset_commit_request_total

- 単位: Count
- 説明: 主キーテーブルでの rowset COMMIT リクエストの総数。
...
### http_requests_total (廃止予定)

### http_request_send_bytes (廃止予定)
...
- 単位: Count
- 説明: セグメントレプリケートスレッドプール内のキュータスクの数。
...
### readable_blocks_total (廃止予定)

### delta_column_group_get_non_pk_hit_cache

- 単位: Count
- 説明: 非主キーテーブルのデルタカラムグループキャッシュヒットの総数。
...
- 単位: Bytes
- 説明: ベースコンパクションおよび累積コンパクションからマージされた総バイト数。
...
- 単位: Count
- 説明: 主キーテーブルのデルタカラムグループを取得する回数の総数。
...
- 単位: Count
- 説明: メモリテーブルフラッシュスレッドプール内のキュータスクの数。
...
- 単位: Count
- 説明: メタデータの読み書きリクエストの総数。

### meta_request_duration

- 単位: us
- 説明: メタデータの読み書きに費やされた総時間。
...
### local_column_pool_bytes (廃止予定)

### total_column_pool_bytes (廃止予定)

### central_column_pool_bytes (廃止予定)
...
- 単位: Count
- 説明: 主キーインデックスコンパクションスレッドプール内のキュータスクの数。
...
### update_del_vector_deletes_total (廃止予定)

### update_del_vector_dels_num (廃止予定)
...
- 単位: Count
- 説明: BE と FE 間のすべての種類のリクエストの総数（CREATE TABLE、Publish Version、タブレットクローンを含む）。
...
- 単位: -
- 説明: BEGIN、COMMIT、ROLLBACK、および EXEC のトランザクションリクエスト。
...
- 単位: Count
- 説明: 主キーテーブルトランザクション APPLY スレッドプール内のキュータスクの数。

### async_delta_writer_queue_count

- 単位: Count
- 説明: タブレットデルタライタースレッドプール内のキュータスクの数。
...
### blocks_deleted_total (廃止予定)

### jemalloc_mapped_bytes

- 単位: Bytes
- 説明: アロケータによってマッピングされたアクティブなエクステントのバイト数の合計。
...
- 単位: Count
- 説明: メタデータに使用される透明な巨大ページの数。

### update_rowset_commit_request_failed

- 単位: Count
- 説明: 主キーテーブルでの失敗した rowset COMMIT リクエストの総数。
...
- 単位: -
- 説明: 各リソースグループの CPU コア制限の割合と、すべてのリソースグループの合計 CPU コア制限の割合。これは瞬時値です。

### resource_group_inuse_cpu_cores

- 単位: Count
- 説明: 各リソースグループで現在使用されている CPU コアの推定数。これは、2 つのメトリック取得間の時間間隔の平均値です。

### resource_group_cpu_use_ratio (廃止予定)

- 単位: -
- 説明: 各リソースグループで使用されたパイプラインスレッドの時間スライスの割合と、すべてのリソースグループで使用された合計の割合。これは、2 つのメトリック取得間の時間間隔の平均値です。

### resource_group_connector_scan_use_ratio (廃止予定)

- 単位: -
- 説明: 各リソースグループで使用された外部テーブルスキャンスレッドの時間スライスの割合と、すべてのリソースグループで使用された合計の割合。これは、2 つのメトリック取得間の時間間隔の平均値です。

### resource_group_scan_use_ratio (廃止予定)

- 単位: -
- 説明: 各リソースグループで使用された内部テーブルスキャンスレッドの時間スライスの割合と、すべてのリソースグループで使用された合計の割合。これは、2 つのメトリック取得間の時間間隔の平均値です。

### pipe_poller_block_queue_len

- 単位: Count
- 説明: パイプラインエンジン内の PipelineDriverPoller のブロックキューの現在の長さ。
...
- 単位: Count
- 説明: BE 内のパイプラインエグゼキュータのドライバースケジューリング回数の累積数。
...
- 単位: Count
- 説明: BE でスケジューリングを待っている準備完了ドライバーの現在の数。
...
- 単位: Count
- 説明: パイプライン PREPARE スレッドプール内のキュータスクの数。これは瞬時値です。
