...
- タイプ: Counter
- 単位: Requests
- 説明: 共有データモードにおける Primary Key インデックスの SST ファイル読み取り失敗の総数。SST ファイルのオープンまたは MultiGet 操作が失敗した際にインクリメントされます。

### pk_index_sst_write_error_total

- タイプ: Counter
- 単位: Requests
- 説明: 共有データモードにおける Primary Key インデックスの SST ファイル書き込み失敗の総数。SST ファイルの構築（フラッシュ）が失敗した際にインクリメントされます。
