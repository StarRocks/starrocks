...
sidebar_position: 20
description: "StarRocks の FE および BE プロセスの CPU とメモリのフレームグラフを収集・可視化し、パフォーマンスのボトルネックを診断します。"
---
...
| `proc_profile_cleanup_interval_s` | `300` | プロファイルファイルのクリーンアップ実行間隔 (秒)。 |
...
CPU プロファイルとメモリプロファイルは互いに独立して収集されるため、一方が失敗しても他方は停止しません。収集の試行が失敗
した場合、FE は 1 秒ごとに再試行するのではなく、初期 2 分・上限 15 分の指数バックオフで再試行し、バックオフ開始時と復旧時に
それぞれ 1 回だけログを記録します。停止処理の失敗によりプロファイラが動作したままになった場合、次の試行がそのセッションを
先に停止するため、FE を再起動せずにプロファイリングが復旧します。保持ポリシーは独立した `proc-profile-cleaner` デーモンが
適用するため、収集が失敗している間もプロファイルファイルは削除されます。
