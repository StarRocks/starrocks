...
| warehouses                 | このリソースグループが有効になる Warehouse。v4.1 以降でサポートされます。 | Warehouse 名をカンマ（`,`）で区切ったリスト | すべての Warehouse |
...
`warehouses` を指定した場合、CPU パーセンテージパラメータの検証における `min_be_cpu_cores` は、指定された Warehouse 内の BE の最小 CPU コア数を指します。`warehouses` を指定しない場合、`min_be_cpu_cores` はすべての BE における CPU コアの最小数を指します。

> **注意**
>
> `warehouses` を指定した場合、リソースグループの CPU クォータは `cpu_weight_percent` または `exclusive_cpu_percent` を使用して設定する必要があり、`cpu_weight` または `exclusive_cpu_cores` は使用できません。
