...
| mem_used_pct_limit         | このリソースグループのメモリ使用率の上限。`enable_group_level_query_queue` を有効にする必要があります。 | [0, 1] (0 より大きい場合に有効) | 0 |
...
このパラメータはリソースグループ粒度のクエリキューにのみ有効で、さらにグローバルセッション変数 `enable_group_level_query_queue` を `true` に設定する必要があります。
