...
| mem_used_pct_limit         | このリソースグループのメモリ使用率の上限。`enable_group_level_query_queue` を有効にする必要があります。`enable_query_queue_v2` が `false` の場合にのみ使用できます。 | [0, 1] (0 より大きい場合に有効) | 0 |
...
BE ノード上でこのリソースグループのメモリ使用率がこのしきい値を超えた場合、リソースグループが過負荷とみなされます。値の範囲は [0, 1] です。0 より大きい場合にのみ有効で、デフォルト値は 0 です。
このリソースグループが共有の `mem_pool`（`default_mem_pool` 以外）を使用している場合、このしきい値はその BE ノード上のプールレベルのメモリ使用量と上限に基づいて評価されます。
このパラメータはリソースグループ粒度のクエリキューにのみ有効で、さらにグローバルセッション変数 `enable_group_level_query_queue` を `true` に設定する必要があります。このパラメータは FE 設定項目 `enable_query_queue_v2` が `false` の場合にのみ使用できます。
