...
このトピックには 2 つのセクションが含まれます - [共有なし](#共有なし) と [共有データ](#共有データ)。
...
StarRocks は v4.1 以降、共有データクラスタにおいて、クラウドネイティブテーブルのデータレプリカ修復をサポートします。

### 概要

共有データアーキテクチャでは、ストレージコスト削減のため、データは通常、オブジェクトストレージや HDFS などのリモートストレージに**単一レプリカ**で保存されます。従来の共有なしアーキテクチャとは異なり、ファイル消失時に複数レプリカによる自動復旧を行うことはできません。

そのため、FE が保持する有効なメタデータバージョンが、リモートストレージ上に存在しないメタデータファイルまたはデータファイルを参照している場合、データロードやクエリは “File not found” エラーで失敗し、最悪の場合サービスが利用不能になります。

ファイル消失は、主に以下の要因で発生します。

- **誤削除**：運用ミスによりオブジェクトストレージ上のファイルが削除された場合。
- **整合性問題**：極端なケースでは、ストレージシステムで遅延整合やメタデータ消失が発生する可能性があります。
- **ソフトウェア不具合**：システムのバグにより、ファイルが早期にクリーンアップされる場合。

従来のスナップショットベースのリストア方式は、時間とコストがかかるため、本番環境での迅速な復旧には適していません。

この問題に対応するため、StarRocks は**低コストかつ秒単位の復旧機構**を提供します。システムは過去のメタデータバージョンをスキャンし、すべての関連ファイルが存在する直近の健全なバージョンを特定し、Tablet メタデータをそのバージョンにロールバックします。これにより、少量の最新データを犠牲にする代わりに、テーブルの可用性を迅速に回復できます。

#### 仕組み

本機能は、共有データクラスターにおけるクラウドネイティブテーブル向け `ADMIN REPAIR TABLE` 文の拡張機能です。主な仕組みは以下のとおりです。

1. **自動検出**

   FE が Compute Node（CN）を制御し、Tablet の過去メタデータバージョンを新しい順に、バッチ単位で検証します。

2. **決定的パス導出**

   メタデータファイルのパスは決定的に導出可能なため、コストの高いオブジェクトストレージの List Objects 操作を実行する必要がありません。

3. **複数の復旧戦略**

   業務要件に応じて、**厳密な整合性（Strict Consistency）** と **最大可用性（Maximum Availability）** の 2 種類の復旧戦略を選択できます。

4. **メタデータリセット機能**

   すべての履歴バージョンでメタデータが失われた場合でも、空の Tablet を作成する（Empty Tablet Recovery）ことで、少数の破損した Tablet がテーブル全体の可用性を阻害するのを防ぎます。

#### 主なメリット

- **超高速復旧**

  KB〜MB レベルのメタデータのみを更新し、データ移動は不要です。PB 規模のテーブルでも秒単位で復旧できます。

- **低運用コスト**

  追加レプリカや高コストなオブジェクトストレージ API 呼び出しは不要です。

### Tablet ステータスの確認

修復を実行する前に、[ADMIN SHOW TABLET STATUS](../../../sql-reference/sql-statements/cluster-management/tablet_replica/ADMIN_SHOW_TABLET_STATUS.md) 文を使用して、クラウドネイティブテーブルまたはクラウドネイティブマテリアライズドビューの各 Tablet のファイル完全性を確認し、メタデータやデータファイルの欠落の有無を調べることができます。

**この文は、共有データクラスターのクラウドネイティブテーブルおよびクラウドネイティブマテリアライズドビューのみサポートしています。**
...
| ステータス       | 説明                     |
| ------------ | ------------------------ |
| NORMAL       | メタデータとデータファイルはすべて正常。 |
| MISSING_META | メタデータファイルが欠落。           |
| MISSING_DATA | データファイルが欠落。             |
...
| Status               | Tablet ステータス。上記のステータスの値を参照してください。                                          |
| MissingDataFileCount | 欠落しているデータファイルの数。`Status` が `MISSING_DATA` の場合のみ値が設定されます。              |
| MissingDataFiles     | 欠落しているデータファイルパスのリスト。`max_missing_data_files_to_show` で制限されます（デフォルト: 5）。`Status` が `MISSING_DATA` の場合のみ値が設定されます。 |

#### 例：パーティション内の異常な Tablet を確認する
...
`PROPERTIES` を指定した `ADMIN REPAIR TABLE` 文を使用して復旧動作を制御します。
...
| プロパティ                       | 型       | デフォルト | 説明                                                                                                                                                                                                                     |
| --------------------------- | ------- | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| enforce_consistent_version  | Boolean | True  | パーティション内の全タブレットを一貫性のあるバージョンにロールバックさせるかどうか。この項目が `true` に設定されている場合、システムはロールバックを実行するために、パーティション全体のデータバージョン整合性を確保するため、すべてのタブレットで有効な履歴内の整合性のあるバージョンを検索します。`false` に設定されている場合、パーティション内の各タブレットは、利用可能な最新の有効なバージョンにロールバックすることが許可されます。異なるタブレットのバージョンは不整合になる可能性がありますが、これによりデータ保存が最大化されます。 |
| allow_empty_tablet_recovery | Boolean | False |空のタブレットを作成してリカバリを許可するかどうか。この項目は `enforce_consistent_version` が `false` の場合にのみ有効です。この項目が `true` に設定されている場合、一部のタブレットの全バージョンでメタデータが欠落しているが、少なくとも1つのタブレットに有効なメタデータが存在する場合、システムは欠落しているバージョンを埋めるために空のタブレットを作成しようと試みます。すべてのタブレットの全バージョンのメタデータが失われた場合、回復は不可能です。 |
| dry_run                     | Boolean | False | 実際に修復を実行せず、修復計画のみを返すかどうか。`true` に設定すると、システムは各パーティションの復旧可能性を評価して修復計画を返しますが、実際のメタデータのロールバックは実行しません。実行前に修復効果をプレビューするのに役立ちます。[修復計画のプレビュー（ドライラン）](#修復計画のプレビュードライラン)を参照してください。 |

### 例

#### 例 1：厳密な整合性復旧（強整合性が必要な場合に推奨）

パーティション `p20250101` 内のすべての Tablet を、最新かつ完全に整合したバージョンに復旧します。
...
各 Tablet をそれぞれの最新有効バージョンに復旧し、バージョンの不一致を許容します。
...
メタデータが完全に失われた Tablet を空 Tablet として復旧します。
...
修復を実行する前に、`dry_run` モードを使用して修復計画をプレビューできます。実際のメタデータを変更せずに、各パーティションの復旧可能性と各 Tablet がロールバックされるバージョンを確認できます。

**`dry_run` モードは、共有データクラスターのクラウドネイティブテーブルのみサポートしています。**
...
| RepairStatus     | 修復ステータス：`NORMAL`（すべてのタブレットが正常、修復不要）、`RECOVERABLE`（ファイルが欠落しているが復旧可能）、`UNRECOVERABLE`（ファイルが欠落しており復旧不可能）、`UNKNOWN`（検査中に例外が発生）。 |
...
修復計画を確認後、`dry_run` を削除（または `false` に設定）して実際の修復を実行してください。

### 制限事項と推奨事項

- 共有データクラスターにおけるレプリカ修復は v3.5 以降でサポートされます。
- 現在、`PROPERTIES` の指定は共有データクラスタ内のクラウドネイティブテーブルのみに対応しています。
- マテリアライズドビューの復旧時は以下に注意してください。
  - 非同期マテリアライズドビューは、復旧後に手動でリフレッシュする必要があります。
  - 同期マテリアライズドビューでは、ベーステーブルと Rollup データの整合性を保つため、**厳密な整合性復旧**を使用してください。
- 復旧はベストエフォートで行われます。利用可能な履歴バージョンおよび選択した復旧戦略に応じて、少量のデータ損失が発生する可能性があります。
