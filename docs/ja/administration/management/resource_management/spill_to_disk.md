...
## [実験的] スピルデータの適応列エンコーディング

このバージョン以降、BE はスピルデータに対する実験的な適応列コーデックセレクターをサポートします。
有効にすると、スピルされる各列は型に適したエンコーディング（RLE、デルタ、frame-of-reference、
patched FOR、辞書、フロントコーディング、ブロック圧縮、decimal 相当の double に対する ALP など）を
サンプリングし、固定の streamvbyte/LZ4 エンコーディングの代わりに、I/O と CPU のコストモデルを
最小化するものを選択します。
...
| 設定項目 | デフォルト | 説明 |
| --- | --- | --- |
| `spill_enable_codec_selector` | `false` | 適応スピル列コーデックセレクター（v2 スピルシリアライズ形式）を有効にします。`false` の場合、従来の `spill_encode_level` 駆動のエンコーディングがそのまま使用されます。 |
| `spill_codec_disk_bandwidth_mbps` | `100` | スピル先の実効書き込みスループット（MB/s）。エンコード・デコードの CPU コストと節約されるディスクバイト数を比較するために使用されます。これは実測値ではなくポリシー入力であり、「1 ナノ秒のコーデック CPU が何バイトの IO に相当するか」を表します。必要なのは桁の正しさだけで、低めに誤る方が安全です。過剰に圧縮しますが、余分な CPU は実際の I/O 削減を買っています。スピルボリュームが容量上限に近いときは `20` 程度に設定してください。ページキャッシュ領域とデバイス領域の両方における実測感度は、BE 設定リファレンスの `spill_codec_disk_bandwidth_mbps` を参照してください。 |
