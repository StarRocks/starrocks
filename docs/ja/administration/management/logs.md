...
sidebar_position: 10
description: "StarRocks の FE および BE/CN のログファイルの種類、典型的な内容、設定方法、ログのローテーションと保持戦略の概要。"
---
...
AI 実行統計が利用可能な場合、次のフィールドは観測された完了済み AI タスクを表します。実行中のタスクは含まれません。クエリのレポート動作に従うため、失敗、キャンセル、非同期レポート、および異なるバージョンが混在する実行では、観測が不完全になる場合があります。重複のない正確な課金記録を保証するものではありません。

| フィールド | 説明 |
| --- | --- |
| `AITaskCount` | 失敗またはキャンセルを含む完了済み AI タスク数。SQL NULL 入力と、タスク作成前に拒否された入力は含まれません。 |
| `AIRequestCount` | 完了済みタスクの受理された HTTP 送信回数。受理された再試行を含み、拒否された送信は含みません。 |
| `AIRetryCount` | 完了済みタスクの受理された再試行送信回数。 |
| `AITimeoutCount` | AI レスポンス処理パスで観測された HTTP タイムアウトとタスク期限切れの回数。すべてのキャンセル済みタスクの数ではありません。 |
| `AIErrorCount` | 最終的に行レベルの失敗となったタスク数。ライフサイクルのキャンセルと、再試行で成功した失敗は含みません。 |
| `AIHttpTimeNs` | 受理された HTTP 送信からトランスポートコールバックまでの累積経過時間（ナノ秒）。再試行のバックオフ、レスポンス解析、完了キューの時間は含みません。 |
| `AIPromptTokens` | プロバイダーが報告した有効な prompt トークン使用量の合計。 |
| `AICompletionTokens` | プロバイダーが報告した有効な completion トークン使用量の合計。 |
| `AITotalTokens` | プロバイダーが報告した有効な total トークン使用量の合計。prompt と completion トークンから計算しません。 |
| `AIPromptUsageCount` | 有効な prompt トークン使用量を含むレスポンス数。明示的なゼロの報告も含みます。 |
| `AICompletionUsageCount` | 有効な completion トークン使用量を含むレスポンス数。明示的なゼロの報告も含みます。 |
| `AITotalUsageCount` | 有効な total トークン使用量を含むレスポンス数。明示的なゼロの報告も含みます。 |

各トークンフィールドには独立したカバレッジカウントがあります。欠落または無効な使用量はゼロではなく不明です。有効な使用量が観測されなかったトークンフィールドは省略され、明示的なゼロは `0` として記録されます。カバレッジは不完全な場合があるため、トークン合計は対応する usage count と合わせて確認してください。AI 統計がない場合、AI フィールドは省略されます。数値カウンターのみを記録し、プロンプト、レスポンス本文、モデル認証情報、プロバイダーのエラー文字列は含みません。
