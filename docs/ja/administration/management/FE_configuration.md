...
- 単位: -
- 変更可能: はい
- 説明: ディスク使用率に基づく自動ログクリーンアップを有効にします。有効にすると、ログディレクトリのディスク使用率がしきい値を超えた場合、ログファイルが自動的にクリーンアップされます。ログクリーンアップは FE Leader ノードでバックグラウンドデーモンとして実行され、ログファイルの蓄積によるディスク容量の枯渇を防ぐのに役立ちます。
- 導入バージョン: -
