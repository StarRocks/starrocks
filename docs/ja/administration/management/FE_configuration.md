...
- 単位: -
- 変更可能: はい
- 説明: 非同期HTTPリクエスト処理を有効化するかどうか。有効の場合、Nettyワーカースレッドで受信したHTTPリクエストは、ビジネスロジック処理のために別のスレッドプールに割り当てられ、HTTPサーバーのブロックを防止します。無効の場合、ビジネスロジックもNettyワーカースレッド内で処理されます。
- 導入バージョン: 4.0.0
