...
- 単位：-
- 変更可能：Yes
- 説明：一定期間内に保持できるロードジョブの最大数。この数を超えると、履歴ジョブの情報は削除されます。注意：この数量ベースの削除は**経過時間を無視します**。データベースの完了トランザクション数が `label_keep_max_num` を超えると、`label_keep_max_second` よりはるかに新しいトランザクションであっても、古いものから削除されます。savepoint から復帰して当該トランザクションを再コミットするコネクター（例：Flink）は、そのトランザクションが存在しないと見なします。このような再コミットのために終了状態を保持するには、`transaction_terminal_state_cache_num` を参照してください。
- 導入時期：-
...
- 単位：Seconds
- 変更可能：Yes
- 説明：完了し、FINISHED または CANCELLED 状態にあるロードジョブのラベルを保持する最大期間 (秒単位)。デフォルト値は 3 日間です。この期間が経過すると、ラベルは削除されます。このパラメーターはすべてのタイプのロードジョブに適用されます。値が大きすぎると大量のメモリを消費します。注意：この経過時間ベースの削除は `label_keep_max_num` を上書きしません。数量ベースの削除は、この期間より前にトランザクションを削除する可能性があります。
- 導入時期：-
...
- 単位：-
- 変更可能：Yes
- 説明：`label_keep_max_num` の数量ベースの削除によって完全なトランザクション状態が削除された後、データベースごとに軽量キャッシュに保持される終了状態（VISIBLE/ABORTED）のトランザクション結果の最大数。数量ベースの削除は経過時間を無視するため、トランザクションは `label_keep_max_second` よりはるかに前に削除される可能性があります。コネクター（例：Flink）が savepoint から復帰して当該トランザクションを再コミットすると、FE は本来 "transaction not found" を返し、コネクターはこれを「一度もコミットされていない」と区別できず、abort/failover ループを引き起こします。このキャッシュにより、FE は再コミットに対して実際の結果（VISIBLE/COMMITTED には冪等な成功、ABORTED にはコミット失敗）を返せます。各エントリはトランザクション id、label、状態、理由、完了時刻のみを保持します。`label_keep_max_second` より古いエントリは存在しないものとして扱われます。`0` に設定するとキャッシュは無効になります。
- 導入時期：-
