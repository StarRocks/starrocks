...
- 単位：-
- 変更可能：Yes
- 説明：クエリ式で組み込み `search()` 関数を有効にするかどうか。アップグレード時に既存の未修飾 `search` UDF 呼び出しのバインドを維持するため、デフォルトでは無効です。有効にする前に、同名の UDF と永続化された定義を確認してください。有効にすると、未修飾の `search()` 呼び出しは組み込み関数用に予約され、無効にすると通常の関数解決に渡されます。`db.search()` などデータベース修飾された呼び出しは常に通常の関数解決を使用します。
- 導入時期：-
