...
- 単位：-
- 変更可能：Yes
- 説明：データベース内の各テーブルで実行できるロードトランザクションの最大数。`0`（デフォルト）はこのテーブル単位の制限を無効にし、その場合は `max_running_txn_num_per_db` のみが適用され、このパラメーター導入前と同じ動作になります。`0` より大きい値を設定すると、`max_running_txn_num_per_db` に加えて適用され、`max_running_txn_num_per_db` 以下に設定する必要があります。これは停滞したテーブルを分離します。あるテーブルのロードトランザクションが完了しなくなった場合（例：publish の停滞）でも、データベース全体の実行中トランザクションの割当を消費せず、同じデータベース内の他のテーブルへのロードをブロックしなくなります。Routine Load およびクラウドネイティブの Compaction トランザクションは、`max_running_txn_num_per_db` と同様にこの制限の対象外です。この制限はロードトランザクションの開始時にチェックされます。明示的な複数ステートメントのトランザクション（`BEGIN ... INSERT ... COMMIT`）は開始時に対象テーブルを宣言しませんが、最初の対象テーブルはその最初のステートメントの実行時にチェックされます。後続のステートメントで追加されるテーブルは受け入れ時にはチェックされません。クラウドネイティブの Compaction トランザクションはこのテーブル単位のチェックの対象外ですが、`max_running_txn_num_per_db` の割当は引き続き消費します。
- 導入時期：-
