...
- 単位：秒
- 変更可能：Yes
- 説明：Leader FE が収集したパーティションごとの `LAST_ACCESS_TIME` を、再起動やフェイルオーバーをまたいで永続化するために内部テーブル `_statistics_.partition_access_time` にフラッシュする間隔です。`enable_collect_partition_access_time` が `true` の場合のみ有効です。
- 導入時期：v4.2.0
