...
- 単位：Seconds
- 変更可能：Yes
- 説明：`remove_orphan_files` プロシージャの `older_than` 引数に許容される最小の保持期間。このプロシージャは読み込んだ時点のテーブル状態から到達可能なファイルを収集し、その後にストレージを列挙するため、その間に書き込まれたファイル（まだコミットされていない INSERT のデータファイルを含む）は孤立ファイルと区別できません。`older_than` から導かれる更新時刻のしきい値がそうしたファイルを保護する仕組みであるため、現在時刻からこの期間より近い `older_than` は拒否されます。この設定は明示的に指定された `older_than` 引数のみを制限し、引数を省略した場合はそのデフォルトの 7 日が使われます。このデフォルト値は、正当な書き込みが未コミットのファイルを保持しうる最長の期間（`insert_timeout` によって既定で 4 時間に制限されます）に対して十分な余裕を残しています。より長い書き込みを許可する環境では値を大きくし、プロシージャと並行して書き込みが実行されないことが確実な場合にのみ小さくしてください。
- 導入時期：v4.2.0
