...
- 単位：-
- 変更可能：Yes
- 説明：マテリアライズドビューのバックグラウンド操作を認可する際に使用するユーザー ID を制御します。これには、更新タスクとクエリリライトに使用されるプランキャッシュの構築の両方が含まれます。`true`（デフォルト、作成者ベースの認可）に設定すると、これらの操作はマテリアライズドビューの作成者として認可され、作成者のすべてのロールが有効化されます。`false`（root ベースの認可）に設定すると、`'root'@'%'` として認可されます。つまり、マテリアライズドビューのプランは ROOT 権限で構築されます。LDAP 認証クラスターとの互換性のために `false` に設定します。
- 導入時期：v3.5.5
