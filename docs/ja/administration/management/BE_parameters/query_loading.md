...
- 単位: MB/s
- 変更可能: はい
- 説明: 実験的。スピル先の実効書き込みスループット。列コーデックセレクター（`spill_enable_codec_selector`）が、エンコード・デコードの CPU コストと節約されるディスクバイト数を比較するために使用します。これは実測値ではなくポリシー入力です。すなわち「1 ナノ秒のコーデック CPU が何バイトの I/O に相当するか」を表します。実際のスループットより低い値を設定するとディスク容量をより重視します。たとえばスピルボリュームが容量上限に近いときに `20` を設定すると、CPU を使ってフットプリントを小さくします。高い値は軽量なエンコーディングに偏り、CPU を節約します。

  必要なのは桁の正しさだけで、カーブは非対称です。低めに誤ると過剰に圧縮しますが、余分な CPU は実際の I/O 削減を買っています。一方、高めに誤ると圧縮しなくなるため、I/O 削減を失った上にサンプリングのコストだけを払います。最適値は、そのスピルがデバイスに到達するほど大きいか、ページキャッシュに収まるほど小さいかによって決まります。スピルベンチマーク（20 データセット、バッファ書き込み）での実測を「圧縮しない場合に対して削減できたスピル時間の割合」で示すと、1 GB のスピル（ページキャッシュが吸収）では `50`/`100`/`200` がそれぞれ 32.5%/34.0%/34.6% を削減し、8 GB のスピル（デバイスに到達）ではそれぞれ 45.6%/44.2%/40.6%、`800` は 34.6% を削減します。デフォルトの `100` はどちらの領域でも最良値との差が約 1.5 ポイント以内に収まります。低めに誤る代価は高めに誤る代価より小さく、過剰圧縮でも実際の I/O 削減は得られますが、圧縮不足ではそれを失った上でサンプリングのコストだけを払います。
- 導入バージョン: -
...
- 単位: -
- 変更可能: はい
- 説明: 実験的。プラガブルなスピル列コーデックセレクター（v2 スピルシリアライズ形式）を有効にし、スピル時に列ごとに適応的にエンコーディングを選択します。`false` の場合、スピルは従来の `spill_encode_level` 駆動のエンコーディングをそのまま使用します。
- 導入バージョン: -
