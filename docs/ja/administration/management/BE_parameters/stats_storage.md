...
- 単位: -
- 変更可能: はい
- 説明: 列ごとの圧縮ディクショナリを構築する方式。有効な値は `sample` と `train` です。`sample` は最初の対象データページのバイト列をそのまま raw content ディクショナリとして使用します。バッファリングも学習コストも不要で、最初のデータページ自体もディクショナリで圧縮されます。`train` は最初の `zstd_compression_dict_train_pages` 個のデータページをバッファし、それらを断片に切り分けてディクショナリを学習します。`train` は多数の行から採取した頻出部分文字列だけを保持するため通常は圧縮率が高くなりますが、バッファしたページの圧縮が遅延し、学習のために追加の CPU を消費します。`train` 以外の値はすべて `sample` として扱われます。このパラメータは `enable_zstd_compression_dict` が `true` の場合にのみ有効です。
- 導入バージョン: v4.2
...
- 単位: バイト
- 変更可能: はい
- 説明: データページをディクショナリのサンプルとして使用するために必要な、エンコード済み値の最小サイズ。これは、ほとんど空の小さな先頭ページから無意味なディクショナリを構築し、その列を恒久的にディクショナリ使用済みとして記録してしまうことを防ぎます。`sample` モードでは最初のデータページのエンコード済み値と比較され、`train` モードではバッファされた学習サンプルの合計サイズと比較されます (負の値は `0` に丸められます)。`0` 以上の値を指定してください。このパラメータは `enable_zstd_compression_dict` が `true` の場合にのみ有効です。
- 導入バージョン: v4.2
...
- 単位: バイト
- 変更可能: はい
- 説明: 学習サンプルを作るために、バッファされたページを切り分ける断片のサイズ。少数の大きなサンプルよりも、多数の小さなサンプルの方が良いディクショナリを学習できます。`256` 未満の値は `256` として扱われます。このパラメータは `enable_zstd_compression_dict` が `true` かつ `zstd_compression_dict_build_mode` が `train` の場合にのみ有効です。
- 導入バージョン: v4.2
...
- 単位: ページ
- 変更可能: はい
- 説明: ディクショナリを学習する前にバッファするデータページ数。書き込み時の追加メモリを列あたり `zstd_compression_dict_train_pages * data_page_size` に制限します (既定値では 32 * 64 KB = 2 MB)。学習されるディクショナリの品質を実際に左右するのは `zstd_compression_dict_max_size` ではなくこのパラメータです。zstd はディクショナリサイズの約 100 倍のサンプル量を推奨しており、計測でも 32 ページは 8 ページに対して繰り返しの多いデータで約 10% 優れていました。一方、8 ページの予算のまま `zstd_compression_dict_max_size` を上げても、学習器が要求サイズを埋められないため効果はありませんでした。`1` 未満の値は `1` として扱われます。このパラメータは `enable_zstd_compression_dict` が `true` かつ `zstd_compression_dict_build_mode` が `train` の場合にのみ有効です。
- 導入バージョン: v4.2
...
- 単位: -
- 変更可能: はい
- 説明: 列レベル圧縮ディクショナリ (各 Segment ファイル内に列ごとに保存される ZSTD ディクショナリ) のクラスタ全体のマスタースイッチ。このスイッチは Segment 書き込みのゲートで参照されます。`false` の場合でも、テーブルプロパティ `zstd_compression_columns` で指定された列は**引き続き ZSTD で圧縮されます**——これはユーザーの明示的な要求であり、このスイッチには依存しません——ただし共有ディクショナリは構築されません。つまりこのスイッチは圧縮率にのみ影響し、圧縮方式を変えることはないため、運用上の安全弁として実行時に切り替えられます。

  既定値が `false` なのはローリングアップグレードの安全のためです。圧縮ディクショナリのデータページは raw content ディクショナリ (`dictID=0`) に対して圧縮された ZSTD フレームであり、そのフレームヘッダにはディクショナリが必要であるという情報が含まれません。`ColumnMetaPB.zstd_compression_dict_page` フィールド (フィールド 35) を知らない古い BE は、そのページをディクショナリなしで解凍しようとして ZSTD の破損エラーになります。したがって、クラスタ内のすべての BE がフィールド 35 を理解するまで、圧縮ディクショナリページを書き込んではいけません。このパラメータを `true` にするのは、クラスタ全体のアップグレードが完了した後にしてください。バージョン混在期間中のレプリカ間 clone やレプリケーションについても同じことが当てはまります。
