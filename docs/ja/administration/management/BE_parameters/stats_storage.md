...
- 単位: バイト
- 変更可能: はい
- 説明: 1 行のエンコード後のソートキーの最大サイズ。これを超えるソートキーを持つ行を書き込もうとするロード（Spark Load を含む）やスキーマ変更は、リトライ不可のエラーで失敗します。これにより、全ソートキーインデックスページのサイズと、ロード後に占有するメモリが制限されます。Compaction およびトランザクションコミット後の Segment 書き換えではこの検査を行いません。コミット後の失敗は呼び出し元に返せないためです。したがって、この制限が有効になる前に書き込まれた行は影響を受けません。ソートキーがエンコード可能であれば `enable_full_sort_key_index` の設定に関わらず検査が行われ、行の受け入れと Segment 書き込みの判断が食い違わないようにしています。0 または負の値を設定すると検査を無効化できます。
- 導入バージョン: -
