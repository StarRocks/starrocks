...
- 単位: -
- 変更可能: はい
- 説明: 列レベルの圧縮ディクショナリを保持するために、試験対象のページがどれだけ小さくならなければならないかを割合で指定します。書き込み側はサンプルページ以降の数ページをディクショナリあり・なしの両方で圧縮して比較し、この閾値を下回る場合は列全体でディクショナリを破棄し、通常の ZSTD 圧縮で書き込みます。ディクショナリは無料ではありません。列ごと Segment ごとに 1 ページを占め、読み取りのたびに Segment ごとに 1 回ロードされ、データページが参照した後はこの判断を取り消せません。そのため既定値は「測定できる差」ではなく「明確な差」を要求します。13 のデータセット × 3 種類のページサイズでの計測では、`0.10` はディクショナリを 10% 以上の効果がある場合にのみ保持し、その代わりに 5〜9% の効果 (プレーンなページだけで既に繰り返しの大部分を捉えられる 256 KB ページに集中) を見送ります。それらも取り込みたい場合は `0.05` 程度まで下げてください。`0` 未満の値は `0` として扱われます。このパラメータは `enable_zstd_compression_dict` が `true` の場合にのみ有効で、変更後に書き込まれたデータにのみ適用されます。
- 導入バージョン: v4.2
...
- 単位: バイト
- 変更可能: はい
- 説明: データページをディクショナリのサンプルとして使用するために必要な、エンコード済み値の最小サイズ。これは、ほとんど空の小さな先頭ページから無意味なディクショナリを構築し、その列を恒久的にディクショナリ使用済みとして記録してしまうことを防ぎます。最初のデータページのエンコード済み値と比較されます。`0` 以上の値を指定してください。このパラメータは `enable_zstd_compression_dict` が `true` の場合にのみ有効です。
- 導入バージョン: v4.2
