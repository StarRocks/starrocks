...
- 単位: -
- 変更可能: はい
- 説明: 列ごとの圧縮ディクショナリを構築する方式。有効な値は `sample` と `train` です。`sample` は最初の対象データページのバイト列をそのまま raw content ディクショナリとして使用します。バッファリングも学習コストも不要で、最初のデータページ自体もディクショナリで圧縮されます。`train` は最初の `compression_dict_train_pages` 個のデータページをバッファし、それらを断片に切り分けてディクショナリを学習します。`train` は多数の行から採取した頻出部分文字列だけを保持するため通常は圧縮率が高くなりますが、バッファしたページの圧縮が遅延し、学習のために追加の CPU を消費します。`train` 以外の値はすべて `sample` として扱われます。このパラメータは `enable_compression_dict` が `true` の場合にのみ有効です。
- 導入バージョン: v4.2
...
- 単位: バイト
- 変更可能: はい
- 説明: `train` モードで生成されるディクショナリの最大サイズ。既定値が zstd CLI の慣例である 110 KB ではなく 64 KB なのは、実際のエージェントログのデータセット (大きな列 3 つ、生データ 372 MB) での計測において、64 KB の指定がすべての列で 112 KB と 256 KB の両方を上回ったためです。ディクショナリは頻出部分文字列のコードブックであり効果はすぐに逓減する一方、そのバイト自体は列ごと Segment ごとに 1 回保存されます (41 MB の列では 256 KB のディクショナリが全体の 0.6% を占め、順位が逆転するのに十分です)。より大きな値を試す価値があるのは、単一列で非常に大きな Segment の場合だけです。`0` 以下の値を指定するとディクショナリの学習は無効になり、その列は通常の ZSTD 圧縮になります。なお、学習されるディクショナリの品質を実際に左右するのはこのパラメータではなく `compression_dict_train_pages` です。このパラメータは `train` モードでのみ使用されます。`sample` モードでのディクショナリサイズは `compression_dict_sample_bytes` によって制限されます。
- 導入バージョン: v4.2
...
- 単位: バイト
- 変更可能: はい
- 説明: データページをディクショナリのサンプルとして使用するために必要な、エンコード済み値の最小サイズ。これは、ほとんど空の小さな先頭ページから無意味なディクショナリを構築し、その列を恒久的にディクショナリ使用済みとして記録してしまうことを防ぎます。`sample` モードでは最初のデータページのエンコード済み値と比較され、`train` モードではバッファされた学習サンプルの合計サイズと比較されます (負の値は `0` に丸められます)。`0` 以上の値を指定してください。このパラメータは `enable_compression_dict` が `true` の場合にのみ有効です。
- 導入バージョン: v4.2
...
- 単位: バイト
- 変更可能: はい
- 説明: `sample` モード (先頭ページサンプリング) で圧縮ディクショナリを構築する際に、最初の対象データページからサンプリングするバイト数。既定値は 64 KB のデータページ約 1 つ分に相当します。実際のサンプルは `min(ページのエンコード済み値のサイズ, compression_dict_sample_bytes)` となるため、このパラメータは `sample` モードにおけるディクショナリサイズの上限にもなります。この値は列ライターの作成時に読み取られるため、変更はそれ以降に書き込まれる Segment に対して有効になります。このパラメータは `enable_compression_dict` が `true` の場合にのみ有効です。
- 導入バージョン: v4.2
...
- 単位: バイト
- 変更可能: はい
- 説明: 学習サンプルを作るために、バッファされたページを切り分ける断片のサイズ。少数の大きなサンプルよりも、多数の小さなサンプルの方が良いディクショナリを学習できます。`256` 未満の値は `256` として扱われます。このパラメータは `enable_compression_dict` が `true` かつ `compression_dict_build_mode` が `train` の場合にのみ有効です。
- 導入バージョン: v4.2
...
- 単位: ページ
- 変更可能: はい
- 説明: ディクショナリを学習する前にバッファするデータページ数。書き込み時の追加メモリを列あたり `compression_dict_train_pages * data_page_size` に制限します (既定値では 32 * 64 KB = 2 MB)。学習されるディクショナリの品質を実際に左右するのは `compression_dict_max_size` ではなくこのパラメータです。zstd はディクショナリサイズの約 100 倍のサンプル量を推奨しており、計測でも 32 ページは 8 ページに対して繰り返しの多いデータで約 10% 優れていました。一方、8 ページの予算のまま `compression_dict_max_size` を上げても、学習器が要求サイズを埋められないため効果はありませんでした。`1` 未満の値は `1` として扱われます。このパラメータは `enable_compression_dict` が `true` かつ `compression_dict_build_mode` が `train` の場合にのみ有効です。
- 導入バージョン: v4.2
...
- 単位: -
- 変更可能: はい
- 説明: 列レベル圧縮ディクショナリ (各 Segment ファイル内に列ごとに保存される ZSTD ディクショナリ) のクラスタ全体のマスタースイッチ。このスイッチは Segment 書き込みのゲートで参照されます。`false` の場合、テーブルプロパティ `compression_dict_columns` で指定された列はテーブルに設定された圧縮方式 (テーブルで `compression` を指定していなければ LZ4) をそのまま保持し、圧縮ディクショナリは書き込まれません。列単位で ZSTD に上書きされるのはこのスイッチが `true` のときだけなので、無効化しても列が暗黙のうちに ZSTD で再エンコードされることはありません。このスイッチは列単位のプロパティとは独立しているため、運用上の安全弁として実行時に切り替えられます。

  既定値が `false` なのはローリングアップグレードの安全のためです。圧縮ディクショナリのデータページは raw content ディクショナリ (`dictID=0`) に対して圧縮された ZSTD フレームであり、そのフレームヘッダにはディクショナリが必要であるという情報が含まれません。`ColumnMetaPB.compression_dict_page` フィールド (フィールド 35) を知らない古い BE は、そのページをディクショナリなしで解凍しようとして ZSTD の破損エラーになります。したがって、クラスタ内のすべての BE がフィールド 35 を理解するまで、圧縮ディクショナリページを書き込んではいけません。このパラメータを `true` にするのは、クラスタ全体のアップグレードが完了した後にしてください。バージョン混在期間中のレプリカ間 clone やレプリケーションについても同じことが当てはまります。

  警告: いったんクラスタが圧縮ディクショナリを含む Segment を書き込むと、この機能が導入されたバージョンより前へのダウングレードはできなくなります。古い BE はそれらの Segment を読むことも Compaction することもできないためです。
- 導入バージョン: v4.2
...
- 単位: -
- 変更可能: はい
- 説明: ディクショナリを読み込んだ ZSTD 解凍コンテキストを、共有コンテキストプールから借りるのではなく、小さなスレッドローカル集合に保持したままにするかどうか。共有プールから借りたコンテキストは reset 時に固定化された `refDDict` が消えるため、ページごとにディクショナリを冷たいコンテキストへ再読み込みする必要が生じます。同条件で対比した全列スキャンでは、コンテキストを保持することでディクショナリ読み取りのオーバーヘッドが +20〜25% から +5.6〜8.6% に下がり、およそ 55〜60% が解消されました。この最適化はスイッチとして公開されているため、ロールバックせずに本番環境で無効化でき、単一クラスタ上で A/B 検証することもできます。このパラメータは、Segment に圧縮ディクショナリを持つ列の読み取りにのみ影響します。
- 導入バージョン: v4.2
...
- 単位: -
- 変更可能: Yes
- 説明: 全ソートキーインデックスの書き込み側スイッチ。有効にすると、Segment Writer は常に書き込まれる従来の切り詰めショートキーインデックスページに**加えて**、完全な（未切り詰めの）全ソートキー列を含む順序保存の全ソートキーインデックスページを書き込み、`segment_sort_key_sample_row_interval` で制御されるメタデータのソートキーサンプルを書き込まなくなります。この設定に関わらず従来の切り詰めショートキーインデックスページは常に書き込まれるため、この機能より前の BE/CN バージョンでも Segment を従来どおり読み取れます（ダウングレードへの影響なし）。新しく書き込まれる Segment にのみ影響し、既にディスク上にある Segment には影響しません。
- 導入バージョン: -
...
- 単位: -
- 変更可能: Yes
- 説明: 全ソートキーインデックスの読み取り側スイッチ。有効にすると、クエリの読み取り経路（Segment Seek と論理スキャン分割）は、Segment に全ソートキーインデックスページがある場合それを使用します。無効にすると、全ソートキーインデックスページを持つものを含むすべての Segment で従来の切り詰めショートキーインデックスページにフォールバックします。両方の設定がデフォルトで有効かつ従来のページが常に存在するため、このスイッチを無効にすることは、新しく開始されるクエリが全ソートキーインデックスの使用を即座に停止するための、データ再書き込み不要の手段（ロールバックバルブ）です。タブレットの Split と Range-Split 並列コンパクションはこのスイッチの影響を受けません。
- 導入バージョン: -
