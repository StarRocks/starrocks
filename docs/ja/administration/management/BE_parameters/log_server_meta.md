...
- 単位: ミリ秒
- 変更可能: いいえ
- 説明: 内部ヒープに保留中のタイマーがある場合に、bRPC `TimerThread` がタイマーバケットを処理し、条件付き走査を評価する機会を得るまでの最大スリープ時間です。有効範囲は `[0, 2147483647]` です。`0` は周期的な起床を無効にし、最も近いタスクの実行時刻までスリープするという bRPC 本来の動作に戻します。この場合、遠い将来の期限よりはるか前にスケジュール解除されたタスクは、その期限までスロットを占有し続けます。正の値を設定すると、遠い将来の期限を持つタイマーに対する回収機会が増えますが、走査のしきい値とヒープ増加の条件は引き続き適用されるため、設定した間隔内にタスクスロットが回収される保証はありません。この設定はプロセス全体に適用されるため、小さい正の値はすべての bRPC `TimerThread` インスタンスの起床回数と CPU 使用量を増やす可能性があります。
- 導入バージョン: -
