...
- 単位: -
- 変更可能: はい
- 説明: コンパクションタスクの merge iterator が入力を並列にプリフィルするかどうか。マージは行を出力する前に、すべての入力からチャンクを 1 つずつ読み取る必要があります。デフォルトではこれらの最初の読み取りは順番に実行されるため、入力数の多いタスクは出力を開始する前に入力ごとに 1 回の読み取りラウンドトリップを支払うことになります。有効にすると、最初の読み取りは並列に発行され、元の順序でコミットされるため、マージ結果はシリアル実行時と同一に保たれます。コンパクションの読み取りがオブジェクトストレージに到達する共有データクラスタでの使用を推奨します。
- 導入バージョン: v4.2
...
- 単位: -
- 変更可能: いいえ
- 説明: 単一のコンパクションマージが同時に発行できるプリフィル読み取りの最大数。読み取りは共有のプリフィルスレッドプール（compaction_parallel_merge_init_pool_threads を参照）で実行されます。enable_compaction_parallel_merge_init が有効な場合にのみ効果があります。
- 導入バージョン: v4.2
...
- 単位: -
- 変更可能: いいえ
- 説明: ノード上で同時実行されるすべてのコンパクションタスクの並列マージプリフィル読み取りを実行する共有プールのスレッド数。タスクごとの同時発行上限より大きく設定されており、同時実行するタスクが互いの性能を薄め合わないようになっています。アイドルスレッドは 10 秒後に回収されるため、アイドル状態のノードにこの余裕分のコストは発生しません。
- 導入バージョン: v4.2
...
- 単位: -
- 変更可能: はい
- 説明: コンパクション中にマージ入力ごとに保持するチャンクスロットの数。スロットが 1 つの場合、使い切った入力の補充はマージを読み取りラウンドトリップ 1 回分ブロックします。スロットを増やすと、バックグラウンドのリーダーが空きスロットを埋め続けるため、ラウンドトリップはマージを停滞させるのではなくマージと重なって実行されます。追加スロット 1 つにつき、入力ごとにチャンク 1 つ分のメモリを余分に消費します。enable_compaction_parallel_merge_init が有効な場合にのみ効果があります。1 は元の動作を維持します。
- 導入バージョン: v4.2
...
- 単位: バイト
- 変更可能: はい
- 説明: 1 つのコンパクションマージが全入力を合わせてプリフェッチ読み取りバッファに保持できる合計バイト数。この予算内では、プリフィルは共有プール上で純粋な IO のみを実行し、デコードはコンパクションタスク自身のスレッドに留まるため、タスクの CPU 使用は自身のワーカーに限定されます。スキャンが残り予算を超える入力は、プール上でのフル読み取りにフォールバックします。0 に設定すると IO とデコードの分離は完全に無効になります。
- 導入バージョン: v4.2
...
- 単位: -
- 変更可能: はい
- 説明: 共有データクラスタの垂直コンパクションにおいて、Data Cache がワーキングセットを保持できないことが実証された場合に、オブジェクトストレージからの直接読み取りへの切り替えを許可するかどうか。判定はタスクの 2 番目の column group パスで計測されます。最初のパスがキャッシュをウォームアップするため、2 番目のパスでリモート読み取りが発生することは、キャッシュがワーキングセットを保持できなかったことを意味します。ウォームなキャッシュや十分なサイズのキャッシュではそこでのリモート読み取りバイトはほぼゼロになり、切り替えは発生しません。直接読み取りでは、各 Segment のカラム領域を少数の大きなリクエストにマージすることも行われます。enable_compaction_parallel_merge_init が必要です。
- 導入バージョン: v4.2
...
- 単位: MB
- 変更可能: はい
- 説明: Data Cache バイパスが作動するために、2 番目の column group パスが読み取っている必要のあるリモートバイト数の下限。小さなテーブルや測定ノイズが切り替えをトリガーしないようにするための絶対的な下限です。
- 導入バージョン: v4.2
...
- 単位: -
- 変更可能: はい
- 説明: Data Cache バイパスが作動するために、2 番目の column group パスで全読み取りバイトに占めるリモートの割合（remote / (remote + local)）が満たす必要のある下限。これにより、絶対リモートバイト数が lake_compaction_data_cache_bypass_threshold_mb を超えていても、読み取りの大部分をまだ提供できているキャッシュ上でコンパクションを継続します。
- 導入バージョン: v4.2
...
- 単位: -
- 変更可能: はい
- 説明: 共有データクラスタのコンパクションタスクが、入力 rowset のロード済み Segment オブジェクト（主キーテーブルの場合は delete vector も）を、共有メタデータキャッシュによる保持に頼るのではなく、タスク全体の期間にわたって自ら保持するかどうか。垂直コンパクションは同じ入力を column group ごとに 1 回ずつ読み取ります。メタデータキャッシュがそれらを保持できない場合、パスのたびにすべての入力 Segment の再ロードと再パースが発生し、この処理は CPU バウンドでテーブルのカラム数に比例します。保持することで、キャッシュが使用したはずのものと同量のメモリを、タスクの生存期間に限定してピン留めします。
- 導入バージョン: v4.2
