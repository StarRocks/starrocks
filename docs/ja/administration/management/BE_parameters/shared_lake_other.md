...
- 単位: Bytes
- 変更可能: はい
- 説明: `dump_tablet_metadata` 診断 API が、指定バージョンのメタデータキャッシュにミスした後、オブジェクトストレージから読み取ることのできる物理メタデータオブジェクトの最大サイズです。論理メタデータキャッシュにヒットした場合は直接返され、この物理オブジェクトの上限は適用されません。`0` 以下に設定すると、キャッシュミス時の読み取りをすべて拒否します。この制限は通常の Tablet メタデータ読み取りには影響しません。より大きな、または複雑なオフライン診断では、専用のストレージツールでオブジェクトを取得し、`meta_tool` で調査してください。
- 導入バージョン: v4.2
