...
TPC-H は、トランザクション処理性能評議会 (TPC) によって開発された意思決定支援ベンチマークです。これは、ビジネス指向のアドホッククエリと同時データ変更のスイートで構成されています。TPC-H は、実際の生産環境に基づいてモデルを構築し、販売システムのデータウェアハウスをシミュレートするために使用できます。このテストでは、データサイズが 1 GB から 3 TB の 8 つのテーブルを使用します。合計 22 のクエリがテストされ、主な性能指標は各クエリの応答時間であり、クエリが送信されてから結果が返されるまでの時間です。

## 1. テストの結論

TPC-H 100G スケールデータセットで合計 22 のクエリを用いた比較テストが実施されました。結果は以下の通りです。
...
StarRocks がネイティブストレージからデータをクエリする遅延は 21 秒、StarRocks が Hive 外部テーブルをクエリする遅延は 92 秒、Trino が Hive 外部テーブルをクエリする遅延は 187 秒でした。
...
設定ファイル `conf/starrocks.conf` を修正し、クラスタのアドレス (ホストとポート) を指定して、テーブル作成操作を行います。
...
### 4.2 StarRocks を使用した Hive 外部テーブルのクエリ
...
ステップ 4.1.1 で生成された TPC-H CSV オリジナルデータを HDFS の指定パス (この記事では `/user/tmp/csv/` パスを使用) にアップロードし、その後 Hive に外部テーブルを作成します。詳細なテーブル作成文は [5.4](#54-hive-external-table-creation-csv-storage-format) を参照してください。この Hive 外部テーブルのストレージ形式は CSV で、ストレージパスは `/user/tmp/csv/` であり、CSV オリジナルデータがアップロードされるパスです。
...
StarRocks は Catalog 機能を使用して Hive テーブルデータをクエリします。詳細な操作については [Hive catalog](../data_source/catalog/hive_catalog.md) を参照してください。
