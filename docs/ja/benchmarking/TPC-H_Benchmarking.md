...
1. tpch-poc ツールキットをダウンロードしてコンパイルします。
    ...
    関連ツールは `bin` ディレクトリに保存されており、ホームディレクトリで操作を開始できます。
...
1. 設定ファイル conf/starrocks.conf を修正し、クラスタのアドレスを指定します。
...
2. 次のスクリプトを実行してテーブルを作成します。
    ...
    テーブルが作成され、デフォルトのバケット数が指定されています。以前のテーブルを削除し、クラスタサイズとノード構成に基づいてバケット数を再計画することができます。これにより、より良いテスト結果が得られます。
...
1. Hive で Hive 外部テーブルを作成します。テーブル形式は CSV です。CSV テストデータを HDFS 上のデータストレージディレクトリにアップロードします。この例では、Hive 外部テーブルの HDFS データストレージパスは /user/tmp/csv/ です。
...
2. 各 Hive 外部テーブルの CSV データを ORC 形式の Hive テーブルに転送します。これにより、StarRocks と Trino で ORC 形式の Hive 外部テーブルを作成し、後続のテストを行うことができます。
