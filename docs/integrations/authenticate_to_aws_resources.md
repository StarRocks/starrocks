...
For Integration with external catalogs and with the internal catalog `default_catalog` that allows file external tables, you can bind multiple catalogs each to the credentials (Access Key and Secret Key) of a specific IAM user, so that different users within the same cluster can access different data sources.
