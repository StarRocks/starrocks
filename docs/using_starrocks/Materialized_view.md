...
- **About external catalog materialized views**
  ...
  - From v2.5.5 onwards, StarRocks can periodically refresh the metadata cache of the frequently accessed Hive catalogs to perceive data changes. You can configure the Hive metadata refresh through the following FE parameters:
    ...
    | background_refresh_metadata_time_secs_since_last_access_secs | `86400` (24 hours)                   | The expiration time of a Hive metadata cache refresh task. For the Hive catalog that has been accessed, if it has not been accessed for more than the specified time, StarRocks stops refreshing its metadata cache. For the Hive catalog that has not been accessed, StarRocks will not refresh its metadata cache. Unit: second. This item is an FE dynamic parameter. You can modify it using the [ADMIN SET FRONTEND CONFIG](../sql-reference/sql-statements/Administration/ADMIN%20SET%20CONFIG.md) command. |
