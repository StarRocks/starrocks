...
- 在备份或恢复作业中，StarRocks 自动备份或恢复表的 [单表物化视图](../using_starrocks/Materialized_view-single_table.md)。目前 StarRocks 暂不支持备份恢复视图和[异步物化视图](../using_starrocks/Materialized_view.md)。您只能手动备份异步物化视图所对应的实体表，且该实体表无法作为物化视图加速或改写查询。
