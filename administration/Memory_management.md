...
| load_mem_limit| 0| 单个导入任务的内存限制，单位是 Byte。如果此设置项被设置为 0，系统会采用 exec_mem_limit 作为内存上限。 |
| query_mem_limit | 0 | 单个查询的内存限制，单位是 Byte。如果此设置项被设置为 0，系统会采用 exec_mem_limit 作为内存上限。建议设置为 17179869184（16GB）以上。 |
