...
public final class KerberosTicketUtils {
    ...
    public static void dumpTicketTime(Subject subject) {
        {
            Set<KerberosTicket> tickets = subject.getPrivateCredentials(KerberosTicket.class);
            for (KerberosTicket ticket : tickets) {
                System.out.printf("[PRIVATE] ticket client = %s, server = %s, start = %s, end = %s\n",
                        ticket.getClient(), ticket.getServer(), ticket.getStartTime().toString(), ticket.getEndTime());
            }
        }
        {
            Set<KerberosTicket> tickets = subject.getPublicCredentials(KerberosTicket.class);
            for (KerberosTicket ticket : tickets) {
                System.out.printf("[PUBLIC] ticket client = %s, server = %s, start = %s, end = %s\n",
                        ticket.getClient(), ticket.getServer(), ticket.getStartTime().toString(), ticket.getEndTime());
            }
        }
    }
