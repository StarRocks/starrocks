...
public class LanceSplitScannerFactory implements ScannerFactory {
    ...
    static {
        ...
        File dir = new File(basePath + "/lib/lance-reader-lib");
        preloadFiles.addAll(Arrays.asList(Objects.requireNonNull(dir.listFiles())));
        dir = new File(basePath + "/lib/common-runtime-lib");
        preloadFiles.addAll(Arrays.asList(Objects.requireNonNull(dir.listFiles())));
        classLoader = ScannerHelper.createChildFirstClassLoader(preloadFiles, "lance scanner");
    }

    /**
     * Lance scanner uses its own independent classloader to find all classes
    ...
    public Class getScannerClass() throws ClassNotFoundException {
        try {
            return classLoader.loadClass("com.starrocks.lance.reader.LanceSplitScanner");
