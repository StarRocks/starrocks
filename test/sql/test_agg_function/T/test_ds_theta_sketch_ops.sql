...
-- ds_theta_intersect_cond_agg: anchor=A (1-50000), window=B (25001-75000)
-- overlap is 25001-50000 = 25000 elements; estimate should be between 22500 and 27500
with grp_sk as (select grp, ds_theta_accumulate(id) as s from t1 group by grp)
select cast(
  ds_theta_intersect_cond_agg(s, cast(grp = 'A' as int)) between 22500 and 27500
  as int)
from grp_sk;

-- ds_theta_intersect_cond_agg with disjoint sets: intersection estimate ~ 0
with a as (select ds_theta_accumulate(c) as s, 1 as is_anchor
           from (select generate_series as c from table(generate_series(1, 1000))) t),
     b as (select ds_theta_accumulate(c) as s, 0 as is_anchor
           from (select generate_series as c from table(generate_series(2001, 3000))) t),
     combined as (select s, is_anchor from a union all select s, is_anchor from b)
select cast(ds_theta_intersect_cond_agg(s, is_anchor) < 50 as int) as disjoint_intersect_near_zero
from combined;

-- ds_theta_intersect_cond_agg: no anchor rows → returns 0.0
with window_only as (select ds_theta_accumulate(c) as s, 0 as is_anchor
                     from (select generate_series as c from table(generate_series(1, 1000))) t)
select ds_theta_intersect_cond_agg(s, is_anchor) from window_only;
