...
You can deploy a StarRocks cluster with one BE node using [**docker-compose.yaml**](./docker-compose.yaml), or  simulate a distributed StarRocks cluster with multiple BEs on a single VM using [docker-compose-multiBE.yaml](./docker-compose-multiBE.yaml).

Note that deploying with docker compose is only recommended in a testing environment, as high availability cannot be guaranteed with a single VM instance deployment.
