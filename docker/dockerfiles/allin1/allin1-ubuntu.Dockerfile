...
RUN apt-get update -y && apt-get install -y --no-install-recommends \
        binutils-dev openjdk-17-jdk python2 mysql-client curl vim tree net-tools less tzdata linux-tools-common linux-tools-generic supervisor nginx netcat locales libssl-dev && \
