...
Usage: $0 [options] 
    ...
    After this command execute successfully, will build image named starrocks/fe-ubuntu.

Optional options:
    -b, --branch                build for branch, default is main
    -h, --help                  print usage
    -i, --image                 image name, default is starrocks/fe-ubuntu
...
build_docker() {
    DOCKER_CONTEXT=${CURDIR}/../.build-fe-ubuntu
    ...
    docker build ${DOCKER_CONTEXT} -f ${CURDIR}/../dockerfiles/Dockerfile-fe-ubuntu -t $IMAGENAME:${BRANCH#*-} --no-cache --build-arg BRANCH=$BRANCH
