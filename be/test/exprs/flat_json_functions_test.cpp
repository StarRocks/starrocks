...
TEST_P(FlatJsonQueryTestFixture2, flat_json_query) {
    ...
    JsonFlater jf(param_flat_path, param_flat_type);
...
public:
    StatusOr<Columns> setup() {
        ...
        JsonFlater jf(flat_path, flat_type);
...
INSTANTIATE_TEST_SUITE_P(GetJsonXXXTest, FlatGetJsonXXXTestFixture2,
    ::testing::Values(
        ...
        std::make_tuple(R"( {"k1": true} )", "$.k1", std::vector<std::string> { "k1"}, std::vector<LogicalType> {TYPE_JSON}, 1, 1, "1", 1.0),
        std::make_tuple(R"( {"k1": false} )", "$.k1", std::vector<std::string> { "k1"}, std::vector<LogicalType> {TYPE_JSON}, 0, 0, "0", 0.0),
...
INSTANTIATE_TEST_SUITE_P(FlatJsonPathDeriver, FlatJsonDeriverPaths,
    ::testing::Values(
        ...
        std::make_tuple(R"({ "k1": "v1" })",  R"({ "k1": 1.123 })", std::vector<std::string> {"k1"}, std::vector<LogicalType> {TYPE_VARCHAR}),
