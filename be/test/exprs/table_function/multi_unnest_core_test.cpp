...
//       --gtest_filter='MultiUnnestSlowTest.SLOW_offsets_across_int32_boundary'
//
...
GROUP_SLOW_TEST_F(MultiUnnestSlowTest, offsets_across_int32_boundary) {
    ...
    const auto big_values =
            ColumnHelper::get_data_column_by_type<TYPE_TINYINT>(result_columns[0].get())->immutable_data();
    const auto small_values =
            ColumnHelper::get_data_column_by_type<TYPE_TINYINT>(result_columns[1].get())->immutable_data();
    for (uint32_t i = 0; i < kRowCount; ++i) {
