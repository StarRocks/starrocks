...
TEST_F(JsonFunctionsTest, const_array_to_json_respects_allow_throw_exception) {
    auto map_keys = NullableColumn::create(Int32Column::create(), NullColumn::create());
    auto map_values = NullableColumn::create(Int32Column::create(), NullColumn::create());
    auto map_elements = MapColumn::create(std::move(map_keys), std::move(map_values), UInt32Column::create());
    auto arrays = ArrayColumn::create(std::move(map_elements), UInt32Column::create());

    DatumMap invalid_map;
    invalid_map[std::monostate{}] = int32_t(1);
    arrays->append_datum(DatumArray{Datum(invalid_map)});
    Columns input_columns{ConstColumn::create(std::move(arrays), 2)};

    {
        std::unique_ptr<FunctionContext> ctx(FunctionContext::create_test_context());
        auto result = JsonFunctions::to_json(ctx.get(), input_columns);
        ASSERT_TRUE(result.ok()) << result.status();
        ASSERT_TRUE(result.value()->is_constant());
        ASSERT_EQ(2, result.value()->size());
        ASSERT_EQ(2, ColumnHelper::count_nulls(result.value()));
    }

    {
        ...
        auto result = JsonFunctions::to_json(ctx.get(), input_columns);
        ASSERT_FALSE(result.ok());
        ASSERT_EQ("key of Map should not be null", result.status().message());
    }
}
