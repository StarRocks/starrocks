...
#include "exprs/agg/combinator/agg_state_merge_function.h"
#include "exprs/agg/combinator/agg_state_union_function.h"
...
void test_agg_state_union_function_basic(FunctionContext* ctx, const AggregateFunction* func,
    ...
    // Create AggStateUnionFunction
    std::vector<bool> arg_nullables = {false, false};
    AggStateUnionFunction union_func(agg_state_desc, immediate_type, arg_nullables);
...
void test_agg_state_union_function_nullable(FunctionContext* ctx, const AggregateFunction* func,
    ...
    // Create AggStateUnionFunction with nullable arguments
...
template <typename T>
void test_agg_state_union_function_error_cases(FunctionContext* ctx, const AggregateFunction* func,
                                               const std::string& func_name,
                                               const std::vector<TypeDescriptor>& arg_types,
                                               const TypeDescriptor& ret_type, const TypeDescriptor& immediate_type,
                                               bool is_result_nullable = false) {
    VLOG_ROW << "test_agg_state_union_function_error_cases: " << func_name
             << " is_result_nullable: " << is_result_nullable;
    ...
    ColumnPtr agg_state_column =
            create_agg_state_column<T>(func, ctx, input_column, is_result_nullable, immediate_type);

    // Test with only one column (should fail)
    Columns columns = {agg_state_column};
    auto result = union_func.execute(ctx, columns);
    ASSERT_FALSE(result.ok());
    ASSERT_TRUE(result.status().message().find("columns is not 2") != std::string::npos);

    // Test with three columns (should fail)
    columns = {agg_state_column, agg_state_column, agg_state_column};
    result = union_func.execute(ctx, columns);
    ASSERT_FALSE(result.ok());
    ASSERT_TRUE(result.status().message().find("columns is not 2") != std::string::npos);

    // Test with mismatched arg_nullables size
    std::vector<bool> wrong_arg_nullables = {false};
    AggStateUnionFunction wrong_union_func(agg_state_desc, immediate_type, wrong_arg_nullables);

    columns = {agg_state_column, agg_state_column};
    result = wrong_union_func.execute(ctx, columns);
    ASSERT_FALSE(result.ok());
    ASSERT_TRUE(result.status().message().find("not match with arg_nullables size") != std::string::npos);
}

template <typename T>
void test_agg_state_union_function_nullable_mismatch(FunctionContext* ctx, const AggregateFunction* func,
                                                     const std::string& func_name,
                                                     const std::vector<TypeDescriptor>& arg_types,
                                                     const TypeDescriptor& ret_type,
                                                     const TypeDescriptor& immediate_type,
                                                     bool is_result_nullable = true) {
    VLOG_ROW << "test_agg_state_union_function_nullable_mismatch: " << func_name
             << " is_result_nullable: " << is_result_nullable;

    // Create AggStateDesc
    AggStateDesc agg_state_desc(func_name, ret_type, arg_types, false, 1);

    // Create AggStateUnionFunction with non-nullable arguments
    std::vector<bool> arg_nullables = {false, false};
    AggStateUnionFunction union_func(agg_state_desc, immediate_type, arg_nullables);

    // Create nullable input columns
    ColumnPtr input_column = gen_input_column1<T>();
    auto nullable_column = ColumnHelper::cast_to_nullable_column(input_column);
    ColumnPtr agg_state_column =
            create_agg_state_column<T>(func, ctx, nullable_column, is_result_nullable, immediate_type);

    // Test with nullable column when function expects non-nullable (should fail)
    Columns columns = {agg_state_column, agg_state_column};
    auto result = union_func.execute(ctx, columns);
    ASSERT_FALSE(result.ok());
}

// Test cases for AggStateUnionFunction
TEST_F(AggStateFunctionsTest, test_agg_state_union_function_sum) {
    ...
    }
    {
        ...
        test_agg_state_union_function_nullable_mismatch<int32_t>(ctx, func, "sum", arg_type_descs, return_type_desc,
                                                                 return_type_desc, true);
        test_agg_state_union_function_error_cases<int32_t>(ctx, func, "sum", arg_type_descs, return_type_desc,
                                                           return_type_desc, true);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_union_function_max) {
    ...
    }
    {
        ...
        test_agg_state_union_function_nullable<int32_t>(ctx, func, "max", arg_type_descs, return_type_desc,
                                                        return_type_desc, true);
        test_agg_state_union_function_error_cases<int32_t>(ctx, func, "max", arg_type_descs, return_type_desc,
                                                           return_type_desc, true);
        test_agg_state_union_function_nullable_mismatch<int32_t>(ctx, func, "max", arg_type_descs, return_type_desc,
                                                                 return_type_desc, true);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_union_function_min) {
    ...
    }
    {
        ...
        test_agg_state_union_function_nullable<int32_t>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                        return_type_desc, true);
        test_agg_state_union_function_error_cases<int32_t>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                           return_type_desc, true);
        test_agg_state_union_function_nullable_mismatch<int32_t>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                                 return_type_desc, true);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_union_function_avg) {
    ...
    }
    {
        ...
        test_agg_state_union_function_error_cases<int32_t>(ctx, func, "avg", arg_type_descs, return_type_desc,
                                                           immediate_type_desc, true);
        test_agg_state_union_function_nullable_mismatch<int32_t>(ctx, func, "avg", arg_type_descs, return_type_desc,
                                                                 immediate_type_desc, true);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_union_function_string) {
    ...
    }
    {
        ...
        test_agg_state_union_function_nullable<Slice>(ctx, func, "any_value", arg_type_descs, return_type_desc,
                                                      return_type_desc, true);
        test_agg_state_union_function_error_cases<Slice>(ctx, func, "any_value", arg_type_descs, return_type_desc,
                                                         return_type_desc, true);
        test_agg_state_union_function_nullable_mismatch<Slice>(ctx, func, "any_value", arg_type_descs, return_type_desc,
                                                               return_type_desc, true);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_union_function_decimal) {
    ...
    }
    {
        ...
        test_agg_state_union_function_nullable<DecimalV2Value>(ctx, func, "sum", arg_type_descs, return_type_desc,
                                                               return_type_desc, true);
        test_agg_state_union_function_error_cases<DecimalV2Value>(ctx, func, "sum", arg_type_descs, return_type_desc,
                                                                  return_type_desc, true);
        test_agg_state_union_function_nullable_mismatch<DecimalV2Value>(ctx, func, "sum", arg_type_descs,
                                                                        return_type_desc, return_type_desc, true);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_union_function_timestamp) {
    ...
    }
    {
        ...
        test_agg_state_union_function_nullable<TimestampValue>(ctx, func, "max", arg_type_descs, return_type_desc,
                                                               return_type_desc, true);
        test_agg_state_union_function_error_cases<TimestampValue>(ctx, func, "max", arg_type_descs, return_type_desc,
                                                                  return_type_desc, true);
        test_agg_state_union_function_nullable_mismatch<TimestampValue>(ctx, func, "max", arg_type_descs,
                                                                        return_type_desc, return_type_desc, true);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_union_function_date) {
    ...
    }
    {
        ...
        test_agg_state_union_function_nullable<DateValue>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                          return_type_desc, true);
        test_agg_state_union_function_error_cases<DateValue>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                             return_type_desc, true);
        test_agg_state_union_function_nullable_mismatch<DateValue>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                                   return_type_desc, true);
    }
}
...
// Test helper functions for AggStateMergeFunction
template <typename T>
void test_agg_state_merge_function_basic(FunctionContext* ctx, const AggregateFunction* func,
    ...
    // Create AggStateMergeFunction
    std::vector<bool> arg_nullables = {is_nullable};
    AggStateMergeFunction merge_func(agg_state_desc, immediate_type, arg_nullables);
...
void test_agg_state_merge_function_nullable(FunctionContext* ctx, const AggregateFunction* func,
    ...
    // Create AggStateMergeFunction with nullable arguments
    std::vector<bool> arg_nullables = {is_nullable};
    AggStateMergeFunction merge_func(agg_state_desc, immediate_type, arg_nullables);
...
template <typename T>
void test_agg_state_merge_function_error_cases(FunctionContext* ctx, const AggregateFunction* func,
                                               const std::string& func_name,
                                               const std::vector<TypeDescriptor>& arg_types,
                                               const TypeDescriptor& ret_type, const TypeDescriptor& immediate_type,
                                               bool is_nullable = true) {
    VLOG_ROW << "test_agg_state_merge_function_error_cases: " << func_name << " is_nullable: " << is_nullable;
    ...
    // Test with no columns (should fail)
    Columns columns = {};
    auto result = merge_func.execute(ctx, columns);
    ASSERT_FALSE(result.ok());
    ASSERT_TRUE(result.status().message().find("columns is not 1") != std::string::npos);

    // Test with two columns (should fail)
    columns = {agg_state_column, agg_state_column};
    result = merge_func.execute(ctx, columns);
    ASSERT_FALSE(result.ok());
    ASSERT_TRUE(result.status().message().find("columns is not 1") != std::string::npos);

    // Test with mismatched arg_nullables size
    std::vector<bool> wrong_arg_nullables = {false, false};
    AggStateMergeFunction wrong_merge_func(agg_state_desc, immediate_type, wrong_arg_nullables);

    columns = {agg_state_column};
    result = wrong_merge_func.execute(ctx, columns);
    ASSERT_FALSE(result.ok());
}

template <typename T>
void test_agg_state_merge_function_nullable_mismatch(FunctionContext* ctx, const AggregateFunction* func,
                                                     const std::string& func_name,
                                                     const std::vector<TypeDescriptor>& arg_types,
                                                     const TypeDescriptor& ret_type,
                                                     const TypeDescriptor& immediate_type, bool is_nullable = true) {
    VLOG_ROW << "test_agg_state_merge_function_nullable_mismatch: " << func_name << " is_nullable: " << is_nullable;

    // Create AggStateDesc
    AggStateDesc agg_state_desc(func_name, ret_type, arg_types, is_nullable, 1);

    // Create AggStateMergeFunction with non-nullable arguments
    std::vector<bool> arg_nullables = {is_nullable};
    AggStateMergeFunction merge_func(agg_state_desc, immediate_type, arg_nullables);
...
// Test cases for AggStateMergeFunction
TEST_F(AggStateFunctionsTest, test_agg_state_merge_function_sum) {
    ...
    }
    {
        ...
        test_agg_state_merge_function_nullable<int32_t>(ctx, func, "sum", arg_type_descs, return_type_desc,
                                                        return_type_desc, true);
        test_agg_state_merge_function_error_cases<int32_t>(ctx, func, "sum", arg_type_descs, return_type_desc,
                                                           return_type_desc, true);
        test_agg_state_merge_function_nullable_mismatch<int32_t>(ctx, func, "sum", arg_type_descs, return_type_desc,
                                                                 return_type_desc, true);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_merge_function_max) {
    ...
    }
    {
        ...
        test_agg_state_merge_function_nullable<int32_t>(ctx, func, "max", arg_type_descs, return_type_desc,
                                                        return_type_desc);
        test_agg_state_merge_function_error_cases<int32_t>(ctx, func, "max", arg_type_descs, return_type_desc,
                                                           return_type_desc);
        test_agg_state_merge_function_nullable_mismatch<int32_t>(ctx, func, "max", arg_type_descs, return_type_desc,
                                                                 return_type_desc);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_merge_function_min) {
    ...
    }
    {
        ...
        test_agg_state_merge_function_nullable<int32_t>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                        return_type_desc);
        test_agg_state_merge_function_error_cases<int32_t>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                           return_type_desc);
        test_agg_state_merge_function_nullable_mismatch<int32_t>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                                 return_type_desc);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_merge_function_avg) {
    ...
    }
    {
        ...
        test_agg_state_merge_function_nullable<int32_t>(ctx, func, "avg", arg_type_descs, return_type_desc,
                                                        immediate_type_desc);
        test_agg_state_merge_function_error_cases<int32_t>(ctx, func, "avg", arg_type_descs, return_type_desc,
                                                           immediate_type_desc);
        test_agg_state_merge_function_nullable_mismatch<int32_t>(ctx, func, "avg", arg_type_descs, return_type_desc,
                                                                 immediate_type_desc);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_merge_function_string) {
    ...
    }
    {
        ...
        test_agg_state_merge_function_nullable<Slice>(ctx, func, "any_value", arg_type_descs, return_type_desc,
                                                      return_type_desc);
        test_agg_state_merge_function_error_cases<Slice>(ctx, func, "any_value", arg_type_descs, return_type_desc,
                                                         return_type_desc);
        test_agg_state_merge_function_nullable_mismatch<Slice>(ctx, func, "any_value", arg_type_descs, return_type_desc,
                                                               return_type_desc);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_merge_function_decimal) {
    ...
    }
    {
        ...
        test_agg_state_merge_function_nullable<DecimalV2Value>(ctx, func, "sum", arg_type_descs, return_type_desc,
                                                               return_type_desc);
        test_agg_state_merge_function_error_cases<DecimalV2Value>(ctx, func, "sum", arg_type_descs, return_type_desc,
                                                                  return_type_desc);
        test_agg_state_merge_function_nullable_mismatch<DecimalV2Value>(ctx, func, "sum", arg_type_descs,
...
TEST_F(AggStateFunctionsTest, test_agg_state_merge_function_timestamp) {
    ...
    }
    {
        ...
        test_agg_state_merge_function_nullable<TimestampValue>(ctx, func, "max", arg_type_descs, return_type_desc,
                                                               return_type_desc);
        test_agg_state_merge_function_error_cases<TimestampValue>(ctx, func, "max", arg_type_descs, return_type_desc,
                                                                  return_type_desc);
        test_agg_state_merge_function_nullable_mismatch<TimestampValue>(ctx, func, "max", arg_type_descs,
                                                                        return_type_desc, return_type_desc);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_merge_function_date) {
    ...
    }
    {
        ...
        test_agg_state_merge_function_nullable<DateValue>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                          return_type_desc);
        test_agg_state_merge_function_error_cases<DateValue>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                             return_type_desc);
        test_agg_state_merge_function_nullable_mismatch<DateValue>(ctx, func, "min", arg_type_descs, return_type_desc,
                                                                   return_type_desc);
    }
}
...
TEST_F(AggStateFunctionsTest, test_agg_state_merge_function_null_handling) {
    ...
    // Create AggStateMergeFunction with nullable support
    TypeDescriptor immediate_type = return_type_desc;
