...
TEST_F(TimeFunctionsTest, yesterday) {
    ColumnPtr return_date = TimeFunctions::yesterday(_utils->get_fn_ctx(), Columns());
    ASSERT_TRUE(return_date->is_constant());
    ASSERT_FALSE(return_date->is_date());

    ColumnPtr now = TimeFunctions::now(_utils->get_fn_ctx(), Columns());
    TimestampValue ts = ColumnHelper::as_column<ConstColumn>(now)->get(0).get_timestamp();
    auto v = ColumnHelper::as_column<ConstColumn>(return_date);
    ASSERT_EQ(((DateValue)ts).add<DAY>(-1), v->get(0).get_date());
}
