...
std::string request_body(const PreparedAIProviderOptions& options) {
...
TEST(ProviderUtilsTest, EmptyMapProducesAnEmptyPreparedObject) {
    auto options = make_map(BinaryColumn::create(), BinaryColumn::create(), 0);
    auto result = prepare_ai_provider_options(
...
TEST(ProviderUtilsTest, UntypedEmptyMapProducesAnEmptyPreparedObject) {
    auto options = make_map(BinaryColumn::create(), BinaryColumn::create(), 0);
    auto result = prepare_ai_provider_options(
...
TEST(ProviderUtilsTest, WireNormalizedUntypedEmptyMapProducesAnEmptyPreparedObject) {
...
TEST(ProviderUtilsTest, NonEmptyBooleanKeyMapIsNotTreatedAsUntyped) {
...
TEST(ProviderUtilsTest, EmptyMapWithANonStringTypedKeyIsRejected) {
...
TEST(ProviderUtilsTest, ConstAndNullableColumnsAreUnwrappedAtTheSelectedRow) {
...
TEST(ProviderUtilsTest, NullTopLevelMapMeansNoOptionsWithoutExposingValues) {
...
TEST(ProviderUtilsTest, SqlNullMapValueBecomesJsonNull) {
...
TEST(ProviderUtilsTest, SqlScalarCategoriesRemainJsonCategories) {
    {
        auto values = BooleanColumn::create();
        values->append(1);
...
TEST(ProviderUtilsTest, DecimalRemainsAJsonNumberInsteadOfBecomingAString) {
...
TEST(ProviderUtilsTest, Decimal128RetainsAllThirtyEightDigitsAsANumberToken) {
...
TEST(ProviderUtilsTest, LargeIntBeyondInt64RemainsAnExactJsonNumber) {
...
TEST(ProviderUtilsTest, JsonValuesPreserveNestedCategoriesAndJsonNull) {
...
TEST(ProviderUtilsTest, NestedArrayMapStructAndSqlNullAreSerializedRecursively) {
...
TEST(ProviderUtilsTest, NestedEmptyArrayAndMapRemainEmptyJsonContainers) {
...
TEST(ProviderUtilsTest, NestedUntypedEmptyMapRemainsAnEmptyJsonObject) {
    auto empty_map = make_map(BinaryColumn::create(), BinaryColumn::create(), 0);
...
TEST(ProviderUtilsTest, NestedWireNormalizedUntypedEmptyMapRemainsAnEmptyJsonObject) {
...
TEST(ProviderUtilsTest, DuplicateKeysAreRejectedAtEveryMapDepthWithoutEchoingValues) {
    {
        auto values = BinaryColumn::create();
...
TEST(ProviderUtilsTest, InvalidUtf8InKeysOrStringValuesIsRejectedWithoutEchoingBytes) {
...
TEST(ProviderUtilsTest, NullAndEmptyKeysAreRejectedAtEveryMapDepth) {
    {
...
TEST(ProviderUtilsTest, OnlyExactCaseSensitiveTopLevelReservedKeysAreRejected) {
...
TEST(ProviderUtilsTest, ReservedNamesRemainValidInsideNestedMaps) {
    auto nested_values = BinaryColumn::create();
...
TEST(ProviderUtilsTest, NonFiniteFloatingPointValuesAreRejectedWithoutEchoingInput) {
...
TEST(ProviderUtilsTest, UnsupportedOpaqueColumnsAreRejectedInsteadOfBecomingJsonNull) {
...
TEST(ProviderUtilsTest, PreparedOptionsOwnTheirStateAndSupportConcurrentReadOnlyRequestBuilding) {
    auto prepared_result = [&] {
        ...
        return prepare_ai_provider_options(
                *options, TypeDescriptor::create_map_type(TypeDescriptor(TYPE_VARCHAR), TypeDescriptor(TYPE_JSON)), 0);
    }();
