...
PreparedAIProviderOptions make_json_options(std::initializer_list<std::pair<std::string, JsonValue>> members) {
    ...
    offsets->append(0);
    offsets->append(static_cast<uint32_t>(members.size()));
    ...
    auto result = prepare_ai_provider_options(
            *map, TypeDescriptor::create_map_type(TypeDescriptor(TYPE_VARCHAR), TypeDescriptor(TYPE_JSON)), 0);
...
TEST(OpenAICompatibleProviderTest, MergesPreparedOptionsOnlyAtTheTopLevel) {
    OpenAICompatibleProvider provider;
    auto options = make_json_options({{"temperature", JsonValue::from_double(0.25)},
                                      {"response_format", JsonValue::parse(R"({"type":"json_object"})").value()}});
...
TEST(OpenAICompatibleProviderTest, RejectsEmptyModelWithASecretFreeFixedError) {
    OpenAICompatibleProvider provider;
    auto options = make_json_options({{"user_option", JsonValue::from_string("option-secret")}});
