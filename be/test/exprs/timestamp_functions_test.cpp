...
//   https://github.com/apache/incubator-doris/blob/master/be/test/exprs/timestamp_functions_test.cpp
...
TEST_F(TimestampFunctionsTest, day_of_week_test) {
    starrocks_udf::FunctionContext* context = new starrocks_udf::FunctionContext();
...
TEST_F(TimestampFunctionsTest, convert_tz_test) {
    ...
    dt1.to_datetime_val(&tv1);
    DateTimeVal t =
            TimestampFunctions::convert_tz(context, tv1, StringVal("Asia/Shanghai"), StringVal("America/Los_Angeles"));
...
TEST_F(TimestampFunctionsTest, timestampdiff_test) {
    ...
    starrocks_udf::DateTimeVal tv_year;
    ...
    starrocks_udf::DateTimeVal tv_month;
    ...
    ASSERT_EQ(6, TimestampFunctions::date_diff(context, tv2, tv1).val);
    DateTimeValue dt4(20120830000001);
    starrocks_udf::DateTimeVal tv4;
    dt4.to_datetime_val(&tv4);
    ASSERT_EQ(6, TimestampFunctions::days_diff(context, tv4, tv1).val);
    ASSERT_EQ(6, TimestampFunctions::date_diff(context, tv4, tv1).val);
    DateTimeValue dt5(20120901000001);
    starrocks_udf::DateTimeVal tv5;
    dt5.to_datetime_val(&tv5);
    ASSERT_EQ(8, TimestampFunctions::days_diff(context, tv5, tv1).val);
    ASSERT_EQ(8, TimestampFunctions::date_diff(context, tv5, tv1).val);

    DateTimeValue dt_day(20120823000005);
    starrocks_udf::DateTimeVal tv_day;
    dt_day.to_datetime_val(&tv_day);
    ASSERT_EQ(0, TimestampFunctions::days_diff(context, tv_day, tv1).val);
    ASSERT_EQ(-1, TimestampFunctions::date_diff(context, tv_day, tv1).val);

    DateTimeValue dt7(20200620134825), dt8(20200620134830);
    starrocks_udf::DateTimeVal tv7, tv8;
    dt7.to_datetime_val(&tv7);
    dt8.to_datetime_val(&tv8);
    ASSERT_EQ(-1, TimestampFunctions::days_diff(context, tv7, tv8).val);
    ASSERT_EQ(0, TimestampFunctions::date_diff(context, tv7, tv8).val);
    ASSERT_EQ(0, TimestampFunctions::days_diff(context, tv8, tv7).val);
    ASSERT_EQ(0, TimestampFunctions::date_diff(context, tv8, tv7).val);
