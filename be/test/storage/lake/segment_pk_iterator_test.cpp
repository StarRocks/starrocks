...
// Each emit appends `chunk_size` default rows to the chunk and (for the rowid form) writes
// the contiguous range [first_rowid, first_rowid+chunk_size).
...
protected:
    Status do_get_next(Chunk* chunk) override {
        ...
        return do_get_next(chunk, &unused);
    }
...
TEST_F(SegmentPKIteratorTest, NonSharedSegment_PhysicalBaseIsZero) {
    ...
    EXPECT_EQ(0u, iter.physical_rowid_base());
...
TEST_F(SegmentPKIteratorTest, SharedSegmentRangeFilter_BaseEqualsRangeStart) {
    ...
    // The first chunk's physical_rowid_offset (= 100) is the segment-wide range start,
    // which physical_rowid_base() also reports.
    ...
    EXPECT_EQ(100u, iter.physical_rowid_base());
    EXPECT_EQ(4u, ref.chunk->num_rows());
}
...
TEST_F(SegmentPKIteratorTest, MultipleInnerGetNextCalls_BaseSetOnce) {
    ...
    // base is captured from the first emit only and is not touched again by the
    // subsequent (plain) get_next calls.
    auto chunk_iter = make_iter({
            {/*chunk_size*/ 10, /*first_rowid*/ 50}, // physical 50..59
            {/*chunk_size*/ 10, /*first_rowid*/ 60}, // physical 60..69
    ...
    EXPECT_EQ(50u, iter.physical_rowid_base());
    EXPECT_EQ(30u, ref.chunk->num_rows());
}

TEST_F(SegmentPKIteratorTest, EmptyInnerEmit_PreservesBaseState) {
    // Iterator emits an empty buffer between non-empty ones. The empty emit
    // must not reset the segment-wide base captured from the first emit.
...
TEST_F(SegmentPKIteratorTest, LazyLoadAcrossNextCalls_BaseUnchangedAndChunkBaseAdvances) {
    ...
    // Each successive current().physical_rowid_offset advances by the chunk size,
    // while the segment-wide base (physical_rowid_base()) stays constant.
    auto chunk_iter = make_iter({
    ...
    while (!iter.done()) {
        ...
        // The segment-wide base must NOT change across _load() calls; 200 = range_start.
        EXPECT_EQ(200u, iter.physical_rowid_base());
