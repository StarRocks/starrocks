...
// Test that PK table with lake replication log (has tablet_metadata) uses non-pk finish logic
TEST(TxnLogApplierBatchTest, PrimaryKeyLakeReplicationApply) {
    Tablet tablet(ExecEnv::GetInstance()->lake_tablet_manager(), 30001);
    auto meta = build_pk_metadata(30001);
    auto applier = new_txn_log_applier(tablet, meta, 2, false, true);
