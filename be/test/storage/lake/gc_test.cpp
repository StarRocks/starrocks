...
TEST_F(GCTest, test_delvec_gc) {
    ...
    for (int i = 1; i < 10; i++) {
        ...
        ASSERT_OK(wf->close());
        if (i < 5) {
            (*metadata->mutable_delvec_meta()->mutable_version_to_file())[i].set_name(delvec_file);
        }
    }
    ASSERT_OK(tablet_manager_1->put_tablet_metadata(metadata));
    ...
    for (int i = 1; i < 5; i++) {
        auto itr = metadata->delvec_meta().version_to_file().find(i);
        EXPECT_TRUE(itr != metadata->delvec_meta().version_to_file().end());
        auto delvec_file = itr->second;
        auto location = location_provider_1->delvec_location(tablet_id_1, delvec_file.name());
        ASSERT_OK(fs->path_exists(location));
    }
}
...
TEST_F(GCTest, test_delete_compaction_inputs) {
    ...
    // Compact rowset [0, 1] into a new rowset [3], the new rowset list will become [3, 2]
    {
        ...
        ASSERT_FALSE(metadata->has_prev_garbage_version());
    }
    // Compact rowset [3] into a new rowset [4]
    {
        ...
        ASSERT_TRUE(metadata->has_prev_garbage_version());
        ASSERT_EQ(3, metadata->prev_garbage_version());
    }
    // New tablet metadata generated by data loading
    {
        ...
        ASSERT_TRUE(metadata->has_prev_garbage_version());
        ASSERT_EQ(4, metadata->prev_garbage_version());
    }
