...
// Test that already_finished flag is properly set and checked
TEST_F(LakeAsyncDeltaWriterTest, test_already_finished_flag) {
    // Prepare data for writing
    static const int kChunkSize = 128;
    auto chunk0 = generate_data(kChunkSize);
    auto indexes = std::vector<uint32_t>(kChunkSize);
    ...
    // Initially, already_finished should be false
    ASSERT_FALSE(delta_writer->already_finished());
    ...
    // Still not finished after write
    ASSERT_FALSE(delta_writer->already_finished());
    ...
    // After finish, already_finished should be true
    ASSERT_TRUE(delta_writer->already_finished());

    // Verify that write is rejected when already_finished is true
    CountDownLatch write_after_finish_latch(1);
    delta_writer->write(&chunk0, indexes.data(), indexes.size(), [&](const Status& st) {
        ASSERT_ERROR(st);
        ASSERT_TRUE(st.is_internal_error());
    ...
    // Verify that flush succeeds when already_finished is true (as no-op)
    CountDownLatch flush_after_finish_latch(1);
    delta_writer->flush([&](const Status& st) {
        ASSERT_OK(st);
        flush_after_finish_latch.count_down();
