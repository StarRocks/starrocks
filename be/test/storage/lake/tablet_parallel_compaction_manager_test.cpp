...
TEST_F(TabletParallelCompactionManagerTest, test_create_deshard_groups_cover_all_segments) {
    ...
    for (uint32_t rowset_id = 0; rowset_id < 2; ++rowset_id) {
        ...
        for (int segment = 0; segment < 6; ++segment) {
            auto* segment_meta = rowset->add_segment_metas();
            segment_meta->set_filename(fmt::format("deshard_{}_{}", rowset_id, segment));
    ...
    auto groups = _manager->_create_deshard_subtask_groups(metadata->id(), rowsets, 4, 200);
    ASSERT_EQ(4, groups.size());
    EXPECT_TRUE(_manager->_validate_deshard_group_coverage(rowsets, groups).ok());
    ...
    auto invalid = _manager->_validate_deshard_group_coverage(rowsets, groups);
    ...
    auto state_or =
            _manager->create_and_register_tablet_state(metadata->id(), txn_id, metadata->version(), 4, 200,
                                                       COMPACTION_MODE_DESHARD, nullptr, [](bool /*success*/) {});
    ASSERT_TRUE(state_or.ok());
    state_or.value()->expected_deshard_subtask_count = 2;
    auto merged_log = _manager->get_merged_txn_log(metadata->id(), txn_id);
    EXPECT_FALSE(merged_log.ok());
    EXPECT_TRUE(merged_log.status().message().find("Incomplete parallel DESHARD") != std::string::npos);
