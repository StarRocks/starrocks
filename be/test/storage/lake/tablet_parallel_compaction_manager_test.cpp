...
TEST_F(TabletParallelCompactionManagerTest, test_submit_subtasks_token_acquisition_failure) {
    ...
    int acquire_count = 0;
    // Fail on second acquire_token() call
    auto acquire_token = [&acquire_count]() {
        acquire_count++;
        return acquire_count <= 1; // Only first acquire succeeds
    };
    ...
    // Verify that acquired token was released
    EXPECT_EQ(1, release_count) << "The acquired token should be released on failure";
