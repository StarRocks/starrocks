...
TEST_F(TabletParallelCompactionManagerTest, test_abort_pending_states_completes_dropped_subtasks) {
    ...
    config.set_max_bytes_per_subtask(5 * 1024 * 1024);
    std::unique_ptr<ThreadPool> pool;
    ASSERT_OK(ThreadPoolBuilder("dropped_subtasks_pool").set_min_threads(0).set_max_threads(1).build(&pool));

    // Use real submission to initialize both running gauges, but replace the work so neither subtask
    // can complete itself. One reports below; the other models a queued task dropped during shutdown.
    ...
    auto created = _manager->create_parallel_tasks(
            tablet_id, txn_id, version, config, callback, false, pool.get(), []() { return true; }, [](bool) {});
    ASSERT_TRUE(created.ok()) << created.status();
    ASSERT_EQ(2, created.value());
    pool->wait();
    EXPECT_EQ(1, _manager->running_tasks());
    EXPECT_EQ(2, _manager->running_subtasks());
...
TEST_F(TabletParallelCompactionManagerTest, test_finalization_exception_completes_tablet_as_failed) {
    ...
    EXPECT_EQ(0, _manager->task_success_total());
    EXPECT_EQ(1, _manager->task_failure_total());
    EXPECT_EQ(TabletParallelCompactionState::ParentTaskMetricState::FINISHED, state->parent_task_metric_state);
    EXPECT_TRUE(state->parent_task_outcome_recorded);
...
TEST_F(TabletParallelCompactionManagerTest, test_acceptance_failure_completes_tablet_once_and_returns_parked_token) {
    ...
    EXPECT_EQ(1, released);
    EXPECT_EQ(1, released_mem_limit_exceeded);
    // The failed first acceptance must not leave a successful parent outcome behind.
    EXPECT_EQ(0, _manager->task_success_total());
    EXPECT_EQ(1, _manager->task_failure_total());
    EXPECT_EQ(TabletParallelCompactionState::ParentTaskMetricState::FINISHED, state->parent_task_metric_state);
    EXPECT_TRUE(state->parent_task_outcome_recorded);
