...
TEST_F(StarletGroupAssignerTest, starlet_group_test) {
    staros::starlet::ShardInfo shardInfo = {12345, {"s3://starlet_test/shard1"}};
    (void)g_worker->add_shard(shardInfo);
    shardInfo.id = 23456;
    shardInfo.obj_store_info.uri = "s3://starlet_test/shard2";
    (void)g_worker->add_shard(shardInfo);

    ASSIGN_OR_ABORT(auto group_path, _assigner->get_group(12345));
    EXPECT_EQ(group_path, "staros_s3://starlet_test/shard1/1001");

    ASSIGN_OR_ABORT(group_path, _assigner->get_group(23456));
    EXPECT_EQ(group_path, "staros_s3://starlet_test/shard2/1001");
    ...
    auto iter = std::find(groups.begin(), groups.end(), "staros_s3://starlet_test/shard1/1001");
    EXPECT_TRUE(iter != groups.end());
    iter = std::find(groups.begin(), groups.end(), "staros_s3://starlet_test/shard2/1001");
    EXPECT_TRUE(iter != groups.end());

    (void)g_worker->remove_shard(12345);
    res = _assigner->get_group(12345);
    EXPECT_TRUE(res.status().is_not_found());
