...
class SharedDataReplicationTxnManagerTest : public LakeReplicationTxnManagerTest {
public:
    SharedDataReplicationTxnManagerTest() = default;
    ~SharedDataReplicationTxnManagerTest() override = default;

protected:
    void SetUp() override {
        LakeReplicationTxnManagerTest::SetUp();

        ASSERT_TRUE(GetParam() != TKeysType::type::PRIMARY_KEYS);
        ASSERT_TRUE(
                _tablet_manager->create_tablet(get_create_tablet_req(_virtual_tablet_id, _version, _schema_hash)).ok());
    }

    void TearDown() override {
        auto status = StorageEngine::instance()->tablet_manager()->drop_tablet(_virtual_tablet_id, kDeleteFiles);
        EXPECT_TRUE(status.ok()) << status;
        status = StorageEngine::instance()->tablet_manager()->delete_shutdown_tablet(_virtual_tablet_id);
        EXPECT_TRUE(status.ok()) << status;

        LakeReplicationTxnManagerTest::TearDown();
    }

protected:
    int64_t _virtual_tablet_id = 40001;
    int64_t _src_db_id = 40002;
    int64_t _src_tablet_id = 40003;
...
TEST_P(SharedDataReplicationTxnManagerTest, test_replicate_no_missing_versions) {
    ...
    Status status = _replication_txn_manager->replicate_snapshot(request);
    EXPECT_FALSE(status.ok());
}
...
TEST_P(SharedDataReplicationTxnManagerTest, test_replicate_normal) {
    ...
    status = _replication_txn_manager->replicate_snapshot(request);
    EXPECT_TRUE(status.ok()) << status;
