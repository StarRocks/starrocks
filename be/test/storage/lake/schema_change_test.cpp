...
TEST_P(SchemaChangeAddColumnTest, test_add_column) {
    ...
    for (int i = 0; i < GetParam().writes_before; i++) {
        ...
        delta_writer->close();
        ASSERT_OK(_tablet_manager->publish_version(base_tablet_id, version, version + 1, txn_id, 0, 10).status());
        version++;
        txn_id++;
    }
    ...
    if (concurrency == 1) {
        ASSERT_OK(_tablet_manager->publish_version(new_tablet_id, 1, version + 1, alter_txn_id, 0, 10).status());
    } else {
        std::vector<std::thread> threads;
        for (int i = 0; i < concurrency; i++) {
            threads.emplace_back([&]() {
                (void)_tablet_manager->publish_version(new_tablet_id, 1, version + 1, alter_txn_id, 0, 10);
            });
        }
        for (auto& t : threads) {
            t.join();
