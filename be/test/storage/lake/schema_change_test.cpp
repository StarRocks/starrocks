...
protected:
    static ChunkPtr read(const VersionedTablet& tablet, bool sorted = false) {
        ...
        auto chunk = ChunkBuilder::new_chunk(*(schema->schema()), 10);

        while (true) {
            auto tmp = ChunkBuilder::new_chunk(*(schema->schema()), 10);
...
TEST_P(SchemaChangeModifyColumnOrderTest, test_alter_key_order) {
    ...
    auto chunk = ChunkBuilder::new_chunk(*_new_schema, 1024);
...
TEST_F(SchemaChangeBaseTabletReadSchemaTest, test_materialized_view) {
    ...
    auto chunk = ChunkBuilder::new_chunk(*new_schema_for_read, 1024);
    int row_count = 0;
    while (true) {
        auto st = reader->get_next(chunk.get());
