...
TEST_P(PartialUpdateTest, test_partial_update_publish_retry) {
...
TEST_P(PartialUpdateTest, test_partial_update_retry_check_file_exist) {
    if (GetParam().enable_persistent_index) return;
    auto chunk0 = generate_data(kChunkSize, 0, false, 3);
    auto chunk1 = generate_data(kChunkSize, 0, true, 5);
    auto indexes = std::vector<uint32_t>(kChunkSize);
...
TEST_F(LakeIncompleteSortKeyPartialUpdateTest, test_incomplete_sort_key) {
    ...
    auto txn_id = next_id();
    auto delta_writer = DeltaWriter::create(_tablet_mgr.get(), tablet_id, txn_id, _partition_id, &_slot_pointers,
                                            _mem_tracker.get());
    ASSERT_OK(delta_writer->open());
