...
#include "util/uid_util.h"
    ...
    TEST_F(MetaFileTest, test_sstable_delvec_integration) {
        ...
        LakeIOOptions lake_io_opts;
        EXPECT_TRUE(
                get_del_vec(_tablet_manager.get(), *metadata1, delvec_page, true, lake_io_opts, &read_delvec1).ok());
        ...
        EXPECT_TRUE(get_del_vec(_tablet_manager.get(), *metadata2, saved_sstable.delvec(), true, lake_io_opts,
                                &read_delvec2)
                            .ok());
        ...
        EXPECT_TRUE(get_del_vec(_tablet_manager.get(), *metadata4, saved_sstable.delvec(), true, lake_io_opts,
                                &read_delvec3)
                            .ok());
