...
TEST_F(LakeRowsetTest, test_read_by_column_hash_modulus_predicate) {
    create_rowsets_for_testing();

    auto rowset = std::make_shared<lake::Rowset>(_tablet_mgr.get(), _tablet_metadata, 0, 0 /* compaction_segment_limit */);
    ...
    auto column_hash_modulus_predicate_pb = mutable_rowset_meta_ptr->mutable_column_hash_modulus_predicate();
    ...
    column_hash_modulus_predicate_pb->add_hash_column_names(col_name);
