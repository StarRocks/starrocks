...
=======
    ...
    TEST_F(LakeRowsetTest, test_tablet_range_pruning_only_for_shared_segments) {
        ...
        auto rowset2 = std::make_shared<lake::Rowset>(_tablet_mgr.get(), _tablet_metadata, 0,
                                                      0 /* compaction_segment_limit */);
    ...
    TEST_F(LakeRowsetTest, test_tablet_range_multi_column_range_pruning) {
        ...
        auto rowset_obj = std::make_shared<lake::Rowset>(_tablet_mgr.get(), _tablet_metadata, 0,
                                                         0 /* compaction_segment_limit */);
        RowsetReadOptions rs_opts;
        OlapReaderStatistics stats;
        rs_opts.stats = &stats;
    ...
    TEST_F(LakeRowsetTest, test_get_each_segment_iterator_respects_tablet_range) {
        ...
        ASSIGN_OR_ABORT(auto seg_iters,
                        rowset->get_each_segment_iterator(input_schema, false /*file_data_cache*/, &stats));
    ...
    TEST_F(LakeRowsetTest, test_segment_range_mode_vs_normal_mode) {
        ...
        auto normal_rowset = std::make_shared<lake::Rowset>(_tablet_mgr.get(), _tablet_metadata, 0,
                                                            0 /* compaction_segment_limit */);
        EXPECT_FALSE(normal_rowset->is_segment_range_mode());
        EXPECT_EQ(3, normal_rowset->num_segments());

        // Segment range mode: 2 segments [0, 2)
        auto range_rowset = std::make_shared<lake::Rowset>(_tablet_mgr.get(), _tablet_metadata, 0,
                                                           0 /* segment_start */, 2 /* segment_end */);
        ...
        auto limit_rowset = std::make_shared<lake::Rowset>(_tablet_mgr.get(), _tablet_metadata, 0,
                                                           1 /* compaction_segment_limit */);
