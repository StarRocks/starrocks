...
TEST(PersistentIndexMemtableTest, test_basic_api) {
    ...
    ++version;
    ASSERT_TRUE(
            memtable->erase(num, erase_key_slices.data(), erase_old_values.data(), &erase_not_found, &erase_num_found, version)
                    .ok());
    ...
    ASSERT_TRUE(memtable->upsert(N, upsert_key_slices.data(), upsert_values.data(), upsert_old_values.data(),
                                 &upsert_not_found, &upsert_num_found, version)
                        .ok());
...
TEST(PersistentIndexMemtableTest, test_replace) {
    ...
    auto memtable = std::make_unique<PersistentIndexMemtable>();
    int64_t version = 1;
    ASSERT_OK(memtable->insert(N, key_slices.data(), values.data(), version));

    //replace
    ++version;
    Status st = memtable->replace(key_slices.data(), replace_values.data(), replace_idxes, version);
    ASSERT_TRUE(st.ok());
