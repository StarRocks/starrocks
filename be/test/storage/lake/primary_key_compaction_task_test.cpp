...
// The rowset a partial-update rewrite leaves behind on a cross-published SPLIT child carries no
// shared segment -- SegmentRewriter wrote a brand-new file and MetaFileBuilder::apply_opwrite cleared
// the flag, because the file really is private to this tablet -- yet it holds every row of its
// source, the siblings' included. All that is left saying so is the range
// convert_txn_log_for_splitting stamped on the rowset, and on this shape that range buys nothing at
// read time: Rowset::set_segment_tablet_range withholds it because it describes no rowid interval in
// sort-key space. So the UNSHARE rewrite is the only thing that can drop those rows, and it has to
// claim the rowset on the range alone.
//
// Without that, the child keeps its siblings' rows for good and any unpruned read returns their keys
// twice -- once from the child that owns them, once from this one. Nothing heals it either: ordinary
// compaction does not filter by range, and the next UNSHARE has no reason to look at the rowset.
TEST_P(LakePrimaryKeyCompactionTest, test_unshare_compaction_claims_a_rewritten_rowset_by_its_range) {
    // c0 is the key, c1 the value; ORDER BY c1 makes the sort key separate from the primary key.
    auto metadata = generate_simple_tablet_metadata(PRIMARY_KEYS);
    metadata->set_enable_persistent_index(GetParam().enable_persistent_index);
    metadata->set_persistent_index_type(GetParam().persistent_index_type);
    ...
    // Two chunks of kChunkSize keys each cover [0, 2*kChunkSize); the range straddles them so both
    // rowsets hold rows on either side of it.
    constexpr int kRangeLow = 5;
    constexpr int kRangeHigh = 17;
    auto to_tuple = [](int value) {
        VariantTuple tuple;
    ...
    // Stage the two shapes a split child ends up with side by side: rowset 0 as the split left it
    // (segments shared), rowset 1 as a row-mode partial update rewrote it (segments private, only the
    // cross-publish range left). Only the second one is at stake here.
    ASSIGN_OR_ABORT(auto before, _tablet_mgr->get_tablet_metadata(tablet_id, version));
    auto staged = std::make_shared<TabletMetadataPB>(*before);
    ASSERT_EQ(2, staged->rowsets_size());
    for (auto& segment : *staged->mutable_rowsets(0)->mutable_segment_metas()) {
        segment.set_shared(true);
    }
    staged->mutable_rowsets(1)->mutable_range()->CopyFrom(staged->range());
    for (const auto& segment : staged->rowsets(1).segment_metas()) {
        ASSERT_FALSE(segment.shared()) << "the rewrite output is private -- that is the whole point";
    }
    CHECK_OK(_tablet_mgr->put_tablet_metadata(*staged));

    ASSIGN_OR_ABORT(auto unshare_policy,
    ...
    ASSIGN_OR_ABORT(auto compaction_log, _tablet_mgr->get_txn_log(tablet_id, txn_id));
    ASSERT_TRUE(compaction_log->has_op_compaction());
    EXPECT_EQ(2, compaction_log->op_compaction().input_rowsets_size());
    EXPECT_EQ(kRangeHigh - kRangeLow, compaction_log->op_compaction().output_rowset().num_rows())
            << "the rewrite must keep exactly the keys in [" << kRangeLow << ", " << kRangeHigh << ")";
    EXPECT_FALSE(compaction_log->op_compaction().output_rowset().has_range())
            << "the output is private to this tablet and holds only its own rows";
}
