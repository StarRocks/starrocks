...
TEST_P(LakePrimaryKeyCompactionTest, test1) {
    ...
    EXPECT_FALSE(_update_mgr->TEST_check_compaction_cache_noexist(tablet_id, txn_id));
    ASSERT_OK(publish_single_version(tablet_id, version + 1, txn_id).status());
    EXPECT_TRUE(_update_mgr->TEST_check_compaction_cache_noexist(tablet_id, txn_id));
    version++;
    ASSERT_EQ(kChunkSize, read(version));
