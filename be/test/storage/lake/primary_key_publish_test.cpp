...
TEST_P(LakePrimaryKeyPublishTest, test_publish_clips_deletes_to_tablet_range) {
    ...
    // Needs a persistent index. The in-memory one cannot hold a range-distribution tablet at all:
    // PrimaryIndex::_set_schema hard-codes PK_ENCODING_TYPE_V1 for both the hash-index key type and the
    // fixed key size ("only use for share nothing mode now"), so with the V2 encoding that a tablet range
    // requires it builds a fixed-width index and is then fed encoded Slice keys, and keys collide. That
    // is orthogonal to this fix and pre-existing; the failure mode fixed here is in any case specific to
    // the sstables a SPLIT child inherits, which only a persistent index has.
