...
TEST_P(LakePrimaryKeyPublishTest, test_write_read_success) {
    ...
    ASSERT_OK(TEST_simple_publish_version(_tablet_mgr.get(), _tablet_metadata->id(), 2, txn_id).status());
...
TEST_P(LakePrimaryKeyPublishTest, test_publish_multi_times) {
    ...
    EXPECT_EQ(new_tablet_metadata->rowsets_size(), 3);
    // duplicate publish
    ASSERT_OK(simple_publish_version(tablet_id, version, txns.back()).status());
    // publish using old version
    ASSERT_OK(simple_publish_version(tablet_id, version - 1, txns.back()).status());
    // advince publish should fail, because version + 1 don't exist
    ASSERT_ERROR(simple_publish_version(tablet_id, version + 2, txns.back()).status());
    ASSERT_EQ(kChunkSize, read_rows(tablet_id, version));
    if (GetParam().enable_persistent_index) {
        check_local_persistent_index_meta(tablet_id, version);
    }
...
TEST_P(LakePrimaryKeyPublishTest, test_publish_concurrent) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int j = 0; j < 5; j++) {
            workers.emplace_back([&]() { (void)simple_publish_version(tablet_id, version + 1, txn_id); });
        }
        for (auto& t : workers) {
            t.join();
        }
