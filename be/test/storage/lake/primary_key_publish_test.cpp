...
TEST_P(LakePrimaryKeyPublishTest, test_full_replication_clears_sstable_meta) {
    ...
    // Note: In real scenario, SST files is generated by incremental replication.
    ...
    // Record current rowsets' segment ids for validation
    std::set<uint32_t> old_segment_ids;
    for (const auto& rowset : metadata_before_replication->rowsets()) {
        for (int i = 0; i < rowset.segments_size(); i++) {
            old_segment_ids.insert(rowset.id() + i);
        }
    }
