...
TEST_F(SegmentIteratorTest, testBasicColumnHashModulusPredicateFilter) {
    ...
    std::string file_name = kSegmentDir + "/basic_column_hash_modulus_predicate_filter";
    ASSIGN_OR_ABORT(auto wfile, _fs->new_writable_file(file_name));
    TabletSchemaBuilder builder;
...

    {
        ...
        column_hash_modulus_predicate_pb.add_hash_column_names("c0");
        seg_options.column_hash_modulus_predicate =
            std::move(ColumnHashModulusPredicate::create(column_hash_modulus_predicate_pb).value());
