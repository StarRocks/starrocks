...
=======
    ...
    ASSERT_OK(segment_data_builder.append(1, slice_provider));
    ASSERT_OK(segment_data_builder.finalize_footer());
>>>>>>> 2d20bdafb5 ([BugFix] Fix BE crash when enable filter_unused_columns and dictionary inconsistent (#30424))
...
<<<<<<< HEAD
    Schema vec_schema;
    vec_schema.append(std::make_shared<Field>(0, "c1", TYPE_INT, -1, -1, false));
    vec_schema.append(std::make_shared<Field>(1, "c2", TYPE_VARCHAR, -1, -1, false));
=======
    VecSchemaBuilder schema_builder;
    schema_builder.add(0, "c0", TYPE_INT).add(1, "c1", TYPE_VARCHAR);
    auto vec_schema = schema_builder.build();
