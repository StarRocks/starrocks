...
// and the fallback to the legacy short key page for a malformed/geometry-mismatched/arity-mismatched/key-order-corrupt page.
...
// DUP table ORDER BY (INT k1, FLOAT f2) -> sort-key arity 2, but a FLOAT sort-key column has no key
// coder, so is_full_sort_key_encodable() is false and the writer never emits a full page for it. Used
// to exercise the read-side encodability guard against an injected full page. The leading INT is the
// only key column, so the legacy short key indexes 1 column.
static std::shared_ptr<TabletSchema> make_int_float_sortkey_schema() {
    TabletSchemaPB pb;
    pb.set_keys_type(DUP_KEYS);
    pb.set_num_short_key_columns(1);
    pb.set_next_column_unique_id(4);
    auto add_col = [&](int id, const char* name, const char* type, bool is_key) {
    ...
    add_col(1, "k1", "INT", true);
    add_col(2, "f2", "FLOAT", false);
    add_col(3, "v3", "INT", false);
...
protected:
    ...
    void TearDown() override { config::enable_full_sort_key_index = _saved_write; }
    ...
    std::string write_segment(const std::string& name, bool write_full) {
        ...
        auto chunk = ChunkHelper::new_chunk(s, kNumRows);
    ...
    // Write a segment for make_int_float_sortkey_schema() (columns INT, FLOAT, INT). The float sort
    // key makes the writer skip the full page even with the write config on.
    std::string write_int_float_segment(const std::string& name) {
        const bool old = config::enable_full_sort_key_index;
        ...
        for (int64_t i = 0; i < kNumRows; ++i) {
            cols[0]->as_mutable_ptr()->append_datum(Datum(static_cast<int32_t>(i / kRowsPerBlock)));
            cols[1]->as_mutable_ptr()->append_datum(Datum(static_cast<float>(i)));
            cols[2]->as_mutable_ptr()->append_datum(Datum(static_cast<int32_t>(0)));
        }
        CHECK_OK(writer.append_chunk(*chunk));
        uint64_t file_size = 0;
    ...
    // Rewrite `orig` -> `dst` replacing field 11 with a well-formed page built from the given
    // geometry/keys. When |full_sort_key| is true it is a SORT_KEY_PAGE carrying |arity| sort-key
    // columns (the normal full page); when false it is a legacy SHORT_KEY_PAGE, used to verify the
    // reader rejects a wrong-page-type page in field 11.
    std::string rewrite_with_full_page(const std::string& name, const std::string& orig, uint32_t rows_per_block,
                                       uint32_t seg_rows, bool full_sort_key, uint32_t arity,
        ...
        PageFooterPB pf;
        if (full_sort_key) {
            CHECK_OK(builder.finalize_full_sort_key(seg_rows, &body, &pf, arity));
        } else {
            CHECK_OK(builder.finalize(seg_rows, &body, &pf));
        }
        PagePointer pp;
...
// request; ensure_full_sort_key_index_usable() lazily loads + validates it; num_sort_key_columns()
...
TEST_F(SegmentFullSortKeyIndexTest, dual_page_presence_then_lazy_usable) {
    ...
    EXPECT_TRUE(segment->ensure_full_sort_key_index_usable());
...
// (b) Legacy-only segment: presence false; the full page is never usable.
TEST_F(SegmentFullSortKeyIndexTest, legacy_only_not_present_not_usable) {
    ...
    EXPECT_FALSE(segment->ensure_full_sort_key_index_usable());
    EXPECT_EQ(nullptr, segment->full_sort_key_index_decoder());
}

// (c) A malformed field-11 page is permanently unusable: the segment falls back to the legacy
// short key page with no crash.
TEST_F(SegmentFullSortKeyIndexTest, malformed_full_page_falls_back_to_legacy) {
    ...
    // The legacy short key page is untouched and still readable.
    ASSERT_OK(segment->load_index());
...
TEST_F(SegmentFullSortKeyIndexTest, geometry_or_arity_mismatch_unusable) {
    ...
    // Geometry mismatch: wrong num_rows_per_block.
    {
        auto p = rewrite_with_full_page("d_geo", base, /*rows_per_block=*/rpb + 1, seg_rows,
                                        /*full_sort_key=*/true, /*arity=*/2, sorted_keys(items));
    ...
    {
        auto p = rewrite_with_full_page("d_arity", base, rpb, seg_rows, /*full_sort_key=*/true,
    ...
    // Also: a legacy SHORT_KEY_PAGE placed in field 11 (wrong page type) is unusable.
    {
        auto p = rewrite_with_full_page("d_enc", base, rpb, seg_rows, /*full_sort_key=*/false,
...
TEST_F(SegmentFullSortKeyIndexTest, key_order_corrupt_unusable) {
    ...
    auto p = rewrite_with_full_page("f_corrupt", base, legacy.num_rows_per_block(), legacy.num_segment_rows(),
                                    /*full_sort_key=*/true, /*arity=*/2, keys);
    auto segment = open_segment(p);
    EXPECT_TRUE(segment->has_full_sort_key_index_page());
    EXPECT_FALSE(segment->ensure_full_sort_key_index_usable());
    EXPECT_EQ(nullptr, segment->full_sort_key_index_decoder());
...
TEST_F(SegmentFullSortKeyIndexTest, concurrent_first_use_publishes_once) {
    ...
    auto worker = [&] {
        if (segment->ensure_full_sort_key_index_usable()) {
...
// (g) A geometry-, arity-, and order-valid full page is still rejected when the segment's resolved
// sort-key schema is no longer order-preserving-encodable (here a FLOAT sort-key column, which has no
// key coder). This mirrors the writer gate: without it, the seek/split path would encode search keys
// with a null KeyCoder (crash) or compare incompatible byte layouts (wrong pruning). Permanent legacy
// fallback.
TEST_F(SegmentFullSortKeyIndexTest, nonencodable_sortkey_schema_unusable) {
    _schema = make_int_float_sortkey_schema();
    ASSERT_EQ(2U, _schema->sort_key_idxes().size());
    ASSERT_FALSE(is_full_sort_key_encodable(*_schema->schema(), _schema->sort_key_idxes()));

    // The writer must not emit a full page for a non-encodable sort key.
    auto base = write_int_float_segment("g_base");
    SegmentFooterPB footer = read_seg_footer(base);
    ASSERT_FALSE(footer.has_full_sort_key_index_page());

    // Inject an otherwise-valid full page (matched geometry, arity 2, sorted); the read-side
    // encodability guard must still reject it and fall back to the legacy short key page.
    ShortKeyFooterPB legacy = read_page_footer(base, footer.short_key_index_page());
    auto p = rewrite_with_full_page("g_nonenc", base, legacy.num_rows_per_block(), legacy.num_segment_rows(),
                                    /*full_sort_key=*/true, /*arity=*/2, sorted_keys(legacy.num_items()));
    auto segment = open_segment(p);
    EXPECT_TRUE(segment->has_full_sort_key_index_page());
    EXPECT_FALSE(segment->ensure_full_sort_key_index_usable());
    EXPECT_EQ(nullptr, segment->full_sort_key_index_decoder());
