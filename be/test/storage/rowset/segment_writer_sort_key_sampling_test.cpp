...
protected:
    ...
    // Read the SortKeyFooterPB of a full sort key index page (SORT_KEY_PAGE) given its page pointer.
    SortKeyFooterPB read_sort_key_page_footer(const std::string& filename, const PagePointerPB& page) {
        ASSIGN_OR_ABORT(auto read_file, _fs->new_random_access_file(filename));
        PageReadOptions opts;
        opts.read_file = read_file.get();
        opts.page_pointer = PagePointer(page);
        opts.codec = nullptr; // sort key index page is not compressed
        OlapReaderStatistics stats;
        opts.stats = &stats;
        ...
        PageFooterPB page_footer;
        CHECK(PageIO::read_and_decompress_page(opts, &handle, &body, &page_footer).ok());
        CHECK_EQ(SORT_KEY_PAGE, page_footer.type());
        return page_footer.sort_key_page_footer();
    }
...
// field 9, a SHORT_KEY_PAGE) AND an additional full sort key index page (footer field 11, a
// SORT_KEY_PAGE, num_sort_key_columns==sort key arity -- ALL sort columns, not
// num_short_key_columns()). The two pages share block geometry (equal num_items,
...
TEST_F(SegmentWriterSortKeySamplingTest, full_sort_key_index_enabled_writes_dual_page_and_no_samples) {
    ...
    SortKeyFooterPB full_footer = read_sort_key_page_footer(w->segment_path(), seg_footer.full_sort_key_index_page());
    // The legacy page (field 9) is a plain SHORT_KEY_PAGE; the full page (field 11) is a SORT_KEY_PAGE
    // carrying all sort-key columns.
...
// key page (footer field 9, a SHORT_KEY_PAGE) is written -- NO full sort key index page (footer
// field 11), and metadata sort-key samples are still produced.
// ---------------------------------------------------------------------------
...
// even when config::enable_full_sort_key_index is on: the writer must fall back to writing only the
// legacy SHORT_KEY_PAGE and keep writing metadata sort-key samples. Writing must not crash (this is
// the regression the codec-support gate protects against).
// ---------------------------------------------------------------------------
