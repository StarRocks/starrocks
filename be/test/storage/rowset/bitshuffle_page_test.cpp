...
public:
    ...
    void test_encode_decode_page_vectorized() {
        using CppType = StorageCppType<Type>;
        auto src = ChunkBuilder::column_from_field_type(Type, false);
        CppType value = 0;
        ...
        std::unique_ptr<std::vector<uint8_t>> page = nullptr;
        {
            ...
            auto dst = ChunkBuilder::column_from_field_type(Type, false);
            dst->reserve(count);
            size_t size = count;
...
TEST_F(BitShufflePageTest, TestReadByRowids) {
    ...
    auto column = ChunkBuilder::column_from_field_type(TYPE_INT, false);
