...
protected:
    ...
    void CreateSrcTablet(TTabletId tablet_id, TKeysType::type type = TKeysType::DUP_KEYS) {
        ...
        VectorizedSchema base_schema = ChunkHelper::convert_schema_to_format_v2(tablet->tablet_schema());
    ...
    void test_convert_to_varchar(LogicalType type, int type_size, T val, const std::string& expect_val) {
        ...
        VectorizedField f = ChunkHelper::convert_field_to_format_v2(0, src_tablet_schema->column(0));
        VectorizedField f2 = ChunkHelper::convert_field_to_format_v2(0, dst_tablet_schema->column(0));

        Datum src_datum;
        src_datum.set<T>(val);
        Datum dst_datum;
        auto converter = get_type_converter(type, LOGICAL_TYPE_VARCHAR);
    ...
    void test_convert_from_varchar(LogicalType type, int type_size, std::string val, T expect_val) {
        ...
        Datum dst_datum;
        auto converter = get_type_converter(LOGICAL_TYPE_VARCHAR, type);
        std::unique_ptr<MemPool> mem_pool(new MemPool());
        Status st = converter->convert_datum(f.type().get(), src_datum, f2.type().get(), &dst_datum, mem_pool.get());
        ASSERT_TRUE(st.ok());
...
TEST_F(SchemaChangeTest, convert_float_to_double) {
    ...
    Datum dst_datum;
    auto converter = get_type_converter(LOGICAL_TYPE_FLOAT, LOGICAL_TYPE_DOUBLE);
    std::unique_ptr<MemPool> mem_pool(new MemPool());
    Status st = converter->convert_datum(f.type().get(), src_datum, f2.type().get(), &dst_datum, mem_pool.get());
    ASSERT_TRUE(st.ok());
...
TEST_F(SchemaChangeTest, convert_datetime_to_date) {
    ...
    Datum dst_datum;
    auto converter = get_type_converter(LOGICAL_TYPE_DATETIME, LOGICAL_TYPE_DATE);
...
TEST_F(SchemaChangeTest, convert_date_to_datetime) {
    ...
    Datum dst_datum;
    auto converter = get_type_converter(LOGICAL_TYPE_DATE, LOGICAL_TYPE_DATETIME);
...
TEST_F(SchemaChangeTest, convert_int_to_date_v2) {
    ...
    Datum dst_datum;
    auto converter = get_type_converter(LOGICAL_TYPE_INT, LOGICAL_TYPE_DATE_V2);
...
TEST_F(SchemaChangeTest, convert_int_to_date) {
    ...
    src_datum.set_int32(20210928);
    Datum dst_datum;
    auto converter = get_type_converter(LOGICAL_TYPE_INT, LOGICAL_TYPE_DATE);
...
TEST_F(SchemaChangeTest, convert_int_to_bitmap) {
    ...
    auto converter = get_materialized_converter(LOGICAL_TYPE_INT, OLAP_MATERIALIZE_TYPE_BITMAP);
...
TEST_F(SchemaChangeTest, convert_varchar_to_hll) {
    ...
    auto converter = get_materialized_converter(LOGICAL_TYPE_VARCHAR, OLAP_MATERIALIZE_TYPE_HLL);
    Status st = converter->convert_materialized(src_col, dst_col, f.type().get());
    ASSERT_TRUE(st.ok());
...
TEST_F(SchemaChangeTest, convert_int_to_count) {
    ...
    auto converter = get_materialized_converter(LOGICAL_TYPE_INT, OLAP_MATERIALIZE_TYPE_COUNT);
    Status st = converter->convert_materialized(src_col, dst_col, f.type().get());
    ASSERT_TRUE(st.ok());
...
TEST_F(SchemaChangeTest, convert_varchar_to_json) {
    ...
    for (const auto& json_str : test_cases) {
        ...
        auto converter = get_type_converter(LOGICAL_TYPE_VARCHAR, LOGICAL_TYPE_JSON);
...
TEST_F(SchemaChangeTest, convert_json_to_varchar) {
    ...
    auto converter = get_type_converter(LOGICAL_TYPE_JSON, LOGICAL_TYPE_VARCHAR);
    auto mem_pool = std::make_unique<MemPool>();
