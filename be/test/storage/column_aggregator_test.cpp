...
TEST(ColumnAggregator, testArrayReplace) {
    auto array_type_info = get_array_type_info(get_type_info(FieldType::OLAP_FIELD_TYPE_VARCHAR));
...
TEST(ColumnAggregator, testNullArrayReplaceIfNotNull2) {
    ...
    ASSERT_EQ(agg->size(), 3);
    ASSERT_TRUE(agg->get(0).is_null());
    ASSERT_EQ(agg->get(1).get_int32(), 3);
    ASSERT_EQ(agg->get(2).get_int32(), 4);
    ...
    aggregator->aggregate_values(0, 5, loops.data(), false);
    aggregator->finalize();

    ASSERT_EQ(agg->size(), 5);
    ASSERT_EQ(agg->get(0).get_int32(), 8);
    ASSERT_EQ(agg->get(1).get_int32(), 11);
    ASSERT_EQ(agg->get(2).get_int32(), 13);
    ASSERT_EQ(agg->get(3).get_int32(), 14);
    ASSERT_EQ(agg->get(4).get_int32(), 15);
}
