...
//   https://github.com/apache/incubator-doris/blob/master/be/test/olap/comparison_predicate_test.cpp
...
TEST_F(TestEqualPredicate, STRING_COLUMN) {
    ...
    SetTabletSchema(std::string("STRING_COLUMN"), "CHAR", "REPLACE", 5, false, true, &char_tablet_schema);
...
#define TEST_LESS_PREDICATE(TYPE, TYPE_NAME, FIELD_TYPE)                                                         \
    TEST_F(TestLessPredicate, TYPE_NAME##_COLUMN) {                                                              \
        TabletSchema tablet_schema;                                                                              \
        SetTabletSchema(std::string("TYPE_NAME_COLUMN"), FIELD_TYPE, "REPLACE", 1, false, true, &tablet_schema); \
        int size = 10;                                                                                           \
        std::vector<uint32_t> return_columns;                                                                    \
        for (int i = 0; i < tablet_schema.num_columns(); ++i) {                                                  \
            return_columns.push_back(i);                                                                         \
