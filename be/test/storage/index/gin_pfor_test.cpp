...
// A corrupt block whose exception high does not fit in the (32 - bit_width) high bits must be
// rejected. Otherwise the patch step (high << bit_width) would silently drop the overflowed bits and
// decode() would "succeed" with a wrong posting value instead of reporting malformed input. These
// bytes are read from persisted segments on shared storage. Hand-crafted block: bit_width=31,
// num_exceptions=1, exception {pos=0, high=2} (2 << 31 overflows uint32 -> 0), then 4 low-stream
// bytes for n=1 value.
TEST_F(GinPforTest, decode_rejects_overflowing_exception_high) {
    const uint8_t corrupt[] = {0x1F, 0x01, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00};
    std::vector<uint32_t> out(1, 0);
    EXPECT_EQ(gin_pfor::decode(corrupt, sizeof(corrupt), 1, out.data()), 0u);
}

// A corrupt block whose exception-high varint is non-canonical and overflows 32 bits (its 5th byte
// sets bits beyond bit 31, e.g. FF FF FF FF 7F = 0x7FFFFFFFF) must be rejected, not silently
// truncated to UINT32_MAX. With bit_width=0 the (32 - b) high-bit check cannot catch it (mask is
// 0xFFFFFFFF), so get_varint itself must reject the oversized encoding. Hand-crafted block:
// bit_width=0, num_exceptions=1, exception {pos=0, high=FF FF FF FF 7F}, no low stream for n=1.
TEST_F(GinPforTest, decode_rejects_overflowing_varint_high) {
    const uint8_t corrupt[] = {0x00, 0x01, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x7F};
    std::vector<uint32_t> out(1, 0);
    EXPECT_EQ(gin_pfor::decode(corrupt, sizeof(corrupt), 1, out.data()), 0u);
}
