...
PARALLEL_TEST(PersistentIndexTest, test_fixlen_mutable_index) {
    ...
    vector<IndexValue> erase_old_values(erase_keys.size(), NullIndexValue);
    ...
    vector<IndexValue> upsert_old_values(upsert_keys.size(), NullIndexValue);
...
PARALLEL_TEST(PersistentIndexTest, test_fixlen_mutable_index_wal) {
...

    {
        ...
        std::vector<IndexValue> old_values(N, NullIndexValue);
...

    {
        ...
        vector<IndexValue> old_values(invalid_keys.size(), NullIndexValue);
...
PARALLEL_TEST(PersistentIndexTest, test_small_varlen_mutable_index_snapshot_wal) {
...

    {
        ...
        std::vector<IndexValue> old_values(NUM_SNAPSHOT, NullIndexValue);
        ...
        std::vector<IndexValue> wal_old_values(NUM_WAL, NullIndexValue);
...
PARALLEL_TEST(PersistentIndexTest, test_small_varlen_mutable_index_wal) {
...

    {
        ...
        std::vector<IndexValue> old_values(keys.size(), NullIndexValue);
...
PARALLEL_TEST(PersistentIndexTest, test_multi_l2_not_tmp_l1) {
...

    {
        ...
        for (int i = 0; i < K; i++) {
            incre_key(i);
            std::vector<IndexValue> old_values(M, NullIndexValue);
            ASSERT_OK(index.load(index_meta));
