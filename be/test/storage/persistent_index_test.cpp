...
PARALLEL_TEST(PersistentIndexTest, test_mutable_flush_to_immutable) {
    ...
    size_t num_found = 0;
    auto st_get = idx_loaded->get(N, keys.data(), keys_info, get_values.data(), &num_found, sizeof(Key));
    if (!st_get.ok()) {
        LOG(WARNING) << st_get;
    }
    ...
    ASSERT_TRUE(idx_loaded->check_not_exist(N, keys.data(), sizeof(Key)).is_already_exist());

    vector<Key> check_not_exist_keys(10);
    for (int i = 0; i < 10; i++) {
        check_not_exist_keys[i] = N + i;
    }
    ASSERT_TRUE(idx_loaded->check_not_exist(10, check_not_exist_keys.data(), sizeof(Key)).ok());
}
