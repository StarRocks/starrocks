...
TEST_F(PublishVersionTaskTest, test_publish_version) {
    ...
    std::map<TabletInfo, RowsetSharedPtr> tablet_related_rs2;
    StorageEngine::instance()->txn_manager()->get_txn_related_tablets_include_finished(2222, 10, &tablet_related_rs);
    ...
    StorageEngine::instance()->txn_manager()->get_txn_related_tablets(2222, 10, &tablet_related_rs2);
    ASSERT_EQ(0, tablet_related_rs2.size());
    tablet_related_rs2.clear();
    tablet_related_rs.clear();
    size_t already_finished_cnt = StorageEngine::instance()->txn_manager()->get_txn_related_tablets_include_finished(
            2222, 10, &tablet_related_rs);
    ASSERT_EQ(1, tablet_related_rs.size());
    ASSERT_EQ(1, already_finished_cnt);
    for (const auto& info : tablet_related_rs) {
        ASSERT_EQ(false, info.second.second);
    }
