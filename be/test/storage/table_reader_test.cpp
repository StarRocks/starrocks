...
public:
    ...
    static void create_rowset(const TabletSharedPtr& tablet, int version, const vector<DatumTuple>& data, int start_pos,
        ...
        auto chunk = ChunkBuilder::new_chunk(schema, data.size());
...
void collect_chunk_iterator_result(StatusOr<ChunkIteratorPtr>& status_or, Chunk& result) {
    ...
    ChunkPtr chunk = ChunkBuilder::new_chunk(iterator->schema(), 2);
...
TEST_F(TableReaderTest, test_basic_read) {
    ...
    // 3. verify multi_get
    ChunkPtr key_chunk = ChunkBuilder::new_chunk(_key_schema, 10);
    std::vector<bool> expected_found;
    ChunkPtr expected_value_chunk = ChunkBuilder::new_chunk(_value_schema, 10);
    ...
    const ChunkPtr& value_chunk = ChunkBuilder::new_chunk(_value_schema, 10);
    ...
    const ChunkPtr& expect_scan_result = ChunkBuilder::new_chunk(scan_value_schema, 0);
    const ChunkPtr& scan_result = ChunkBuilder::new_chunk(scan_value_schema, 0);
    std::vector<const ColumnPredicate*> predicates;
