...
TEST_F(SegmentMetaCollecterTest, test_collect_drop_rename_reorder_column_by_uid) {
    ...
    DeferOp defer_op([&] { fs::delete_file(segment_name); });
    ...
    SegmentMetaCollecterParams params;
    params.fields = {META_MIN, META_MAX, META_COUNT_COL};
    params.field_type = {TYPE_INT, TYPE_INT, TYPE_BIGINT};
    params.cids = {0, 0, 0};
    params.read_page = {false, false, true};
    ...
    std::vector<Column*> columns = {min_col.get(), max_col.get(), count_col.get()};
...
TEST_F(SegmentMetaCollecterTest, test_collect_added_column_default_values) {
    ...
    SegmentMetaCollecter collecter(segment);
    SegmentMetaCollecterParams params;
    params.fields = {META_MIN, META_COUNT_COL, META_MIN, META_COUNT_COL};
    params.field_type = {TYPE_INT, TYPE_BIGINT, TYPE_INT, TYPE_BIGINT};
    params.cids = {1, 1, 2, 2};
    params.read_page = {false, true, false, true};
    params.tablet_schema = current_schema;
    ...
    std::vector<Column*> columns = {nullable_min.get(), nullable_count.get(), default_min.get(), default_count.get()};
