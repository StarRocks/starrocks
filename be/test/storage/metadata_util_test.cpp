...
TEST(MetadataUtilTest, testConvertThriftSchemaToPbSchema) {
    ...
    schema.__isset.distribution_key_column_ids = true;
    schema.distribution_key_column_ids.emplace_back(0);
    ...
    ASSERT_EQ(schemaPb.distribution_key_column_ids().size(), 1);
    ASSERT_EQ(schemaPb.distribution_key_column_ids(0), 0);
}
