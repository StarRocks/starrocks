...
TEST(TabletSchemaTest, test_schema_with_distribution_key) {
    ...
    schema_pb.add_distribution_key_column_ids(0);
    TabletSchema tablet_schema(schema_pb);

    ASSERT_TRUE(tablet_schema.distribution_key_column_ids().size() == 1);
    ASSERT_TRUE(tablet_schema.distribution_key_column_ids()[0] == 0);
}
