...
TEST(ChunkAggregatorTest, testNoneAggregator) {
    FieldPtr key = std::make_shared<Field>(1, "key", LogicalType::OLAP_FIELD_TYPE_INT, false);
    key->set_aggregate_method(FieldAggregationMethod::OLAP_FIELD_AGGREGATION_NONE);
    key->set_is_key(true);

    FieldPtr value = std::make_shared<Field>(1, "value", LogicalType::OLAP_FIELD_TYPE_INT, false);
