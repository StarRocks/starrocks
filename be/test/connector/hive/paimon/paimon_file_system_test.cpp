...
TEST_F(PaimonFileSystemTest, CreateRenameAndDelete) {
    PaimonFileSystem file_system(_local_fs, DataCacheOptions{});
    const std::string new_file = _root + "/new-file";
    const std::string renamed_file = _root + "/renamed-file";
    const std::string new_directory = _root + "/new-directory";

    auto create_result = file_system.Create(new_file, false);
    ASSERT_TRUE(create_result.ok()) << create_result.status().ToString();
    auto output = std::move(create_result).value();
    auto write_result = output->Write("xyz", 3);
    ASSERT_TRUE(write_result.ok()) << write_result.status().ToString();
    EXPECT_EQ(3, write_result.value());
    EXPECT_TRUE(output->Flush().ok());
    EXPECT_TRUE(output->Close().ok());

    EXPECT_TRUE(file_system.Mkdirs(new_directory).ok());
    EXPECT_TRUE(file_system.Rename(new_file, renamed_file).ok());
    EXPECT_TRUE(file_system.Delete(renamed_file, false).ok());
    EXPECT_TRUE(file_system.Delete(new_directory, false).ok());

    auto exists_result = file_system.Exists(renamed_file);
    ASSERT_TRUE(exists_result.ok()) << exists_result.status().ToString();
    EXPECT_FALSE(exists_result.value());
}
