...
protected:
    ...
    std::shared_ptr<RuntimeState> create_runtime_state() {
        ...
        std::shared_ptr<RuntimeState> runtime_state = std::make_shared<RuntimeState>(
                fragment_id, query_options, query_globals, &_query_execution_services, nullptr);
        TUniqueId id;
...
// A default whose declared type is complex cannot be handed through a column this load reads as
// text, so the scan opens normally and the column keeps its NULL. Before this the missing cast
// failed the whole load at open, including the rows that did supply the column.
TEST_F(JsonScannerTest, test_no_default_fill_when_column_is_read_as_text) {
    std::vector<TypeDescriptor> types;
    types.emplace_back(TypeDescriptor::create_varchar_type(20));
    types.emplace_back(TypeDescriptor::create_varchar_type(20));
    ...
    // The slot reads as text while the default is declared ARRAY, which is exactly the shape an
    // older FE would send.
    TExpr array_default;
    TExprNode node;
    ...
    array_default.nodes.emplace_back(node);

    std::map<TSlotId, TExpr> defaults;
    defaults[1] = array_default;
...
TEST_F(JsonScannerTest, test_gzip_compressed_json_with_compression_type) {
    ...
    range.__set_path("./be/test/exec/test_data/json_scanner/books.json.gz");
    ranges.emplace_back(range);
