...
// Tests for check_rejected_record_format_support(), shared by file connector
// scan and legacy FileScanNode paths.
...
// ===========================================================================
// check_rejected_record_format_support:
//   When logging is enabled, supported formats (CSV/JSON/PARQUET/ORC/ARROW)
//   return OK.
// ===========================================================================
...
TEST(CheckRejectedRecordFormatSupportTest, SupportedFormatsReturnOkWhenLoggingEnabled) {
    ...
    EXPECT_TRUE(check_rejected_record_format_support(true, TFileFormatType::FORMAT_ARROW).ok());
}
...
TEST(CheckRejectedRecordFormatSupportTest, EachWhitelistedFormatIndividuallyOk) {
    const std::vector<TFileFormatType::type> ok_formats = {
            TFileFormatType::FORMAT_CSV_PLAIN, TFileFormatType::FORMAT_JSON,  TFileFormatType::FORMAT_PARQUET,
            TFileFormatType::FORMAT_ORC,       TFileFormatType::FORMAT_ARROW,
    };
...
TEST(CheckRejectedRecordFormatSupportTest, ErrorMessageMatchesCanonicalString) {
    auto st = check_rejected_record_format_support(true, TFileFormatType::FORMAT_AVRO);
    EXPECT_FALSE(st.ok());
    // FileScanNode and file connector scan paths use the same helper/message.
