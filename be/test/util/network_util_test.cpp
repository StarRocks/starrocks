...
//=============================================================================
// is_link_local_ip() Tests
//=============================================================================

PARALLEL_TEST(NetworkUtilTest, IsLinkLocalIP_IPv4_LinkLocal) {
...
PARALLEL_TEST(NetworkUtilTest, IsLinkLocalIP_IPv6_LinkLocal) {
...
PARALLEL_TEST(NetworkUtilTest, IsLinkLocalIP_NotLinkLocal) {
    // Loopback is NOT link-local
    EXPECT_FALSE(is_link_local_ip("127.0.0.1"));
    EXPECT_FALSE(is_link_local_ip("::1"));

    // Private IPs are NOT link-local
    EXPECT_FALSE(is_link_local_ip("10.0.0.1"));
    EXPECT_FALSE(is_link_local_ip("172.16.0.1"));
    EXPECT_FALSE(is_link_local_ip("192.168.1.1"));

    // Public IPs are NOT link-local
    EXPECT_FALSE(is_link_local_ip("8.8.8.8"));
    EXPECT_FALSE(is_link_local_ip("1.1.1.1"));
    EXPECT_FALSE(is_link_local_ip("2001:4860:4860::8888"));
}

PARALLEL_TEST(NetworkUtilTest, IsLinkLocalIP_IPv4Mapped) {
    // IPv4-mapped IPv6 link-local addresses (::ffff:169.254.x.x)
    EXPECT_TRUE(is_link_local_ip("::ffff:169.254.1.1"));
    EXPECT_TRUE(is_link_local_ip("::ffff:169.254.169.254"));
    // IPv4-mapped non-link-local should return false
    EXPECT_FALSE(is_link_local_ip("::ffff:192.168.1.1"));
    EXPECT_FALSE(is_link_local_ip("::ffff:10.0.0.1"));
}

PARALLEL_TEST(NetworkUtilTest, IsLinkLocalIP_EmptyAndInvalid) {
    EXPECT_FALSE(is_link_local_ip(""));
    EXPECT_FALSE(is_link_local_ip("not-an-ip"));
    EXPECT_FALSE(is_link_local_ip("256.256.256.256"));
}
