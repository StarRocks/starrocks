...
>>>>>>> 546210a3be ([BugFix] Do not swallow task exceptions in ThreadPool by default (#76863)):be/test/common/thread/threadpool_test.cpp
...
public:
    ...
    static shared_ptr<Runnable> new_slow_task(CountDownLatch* latch) { return std::make_shared<SlowTask>(latch); }
...
TEST_F(ThreadPoolTest, TestThreadPoolWithNoMaxThreads) {
    ...
    ASSERT_TRUE(rebuild_pool_with_builder(
                        ThreadPoolBuilder(kDefaultPoolName).set_max_threads(std::numeric_limits<int>::max()))
                        .ok());
    CountDownLatch latch(1);
...
TEST_F(ThreadPoolTest, TestZeroQueueSize) {
    ...
    ASSERT_TRUE(rebuild_pool_with_builder(
                        ThreadPoolBuilder(kDefaultPoolName).set_max_queue_size(0).set_max_threads(kMaxThreads))
                        .ok());
...
INSTANTIATE_TEST_CASE_P(Tokens, ThreadPoolTestTokenTypes,
                        ::testing::Values(ThreadPool::ExecutionMode::SERIAL, ThreadPool::ExecutionMode::CONCURRENT));
...
TEST_P(ThreadPoolTestTokenTypes, TestTokenSubmitsProcessedConcurrently) {
    ...
    ASSERT_TRUE(rebuild_pool_with_builder(ThreadPoolBuilder(kDefaultPoolName).set_max_threads(kNumTokens)).ok());
...
TEST_F(ThreadPoolTest, TestTokenSubmitsNonSequential) {
    ...
    ASSERT_TRUE(rebuild_pool_with_builder(ThreadPoolBuilder(kDefaultPoolName).set_max_threads(kNumSubmissions)).ok());
...
TEST_P(ThreadPoolTestTokenTypes, TestTokenShutdown) {
    ASSERT_TRUE(rebuild_pool_with_builder(ThreadPoolBuilder(kDefaultPoolName).set_max_threads(4)).ok());
    ...
    for (int i = 0; i < kNumOperations; i++) {
        ...
        if (op < 40) {
            ...
            ThreadPool::ExecutionMode mode =
                    r.Next() % 2 ? ThreadPool::ExecutionMode::SERIAL : ThreadPool::ExecutionMode::CONCURRENT;
...
TEST_F(ThreadPoolTest, TestTokenConcurrency) {
    ...
    for (int i = 0; i < kNumTokens; i++) {
        ...
        {
            std::lock_guard<SpinLock> l(lock);
            mode = rng.Next() % 2 ? ThreadPool::ExecutionMode::SERIAL : ThreadPool::ExecutionMode::CONCURRENT;
        }
            ...
            while (latch.count()) {
                {
                    ...
                    int idx = rng.Uniform(kNumTokens);
                    ThreadPool::ExecutionMode mode =
                            rng.Next() % 2 ? ThreadPool::ExecutionMode::SERIAL : ThreadPool::ExecutionMode::CONCURRENT;
