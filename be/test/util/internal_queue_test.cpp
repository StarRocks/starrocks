...
//   https://github.com/apache/incubator-doris/blob/master/be/test/util/internal_queue_test.cpp
...
void ProducerThread(InternalQueue<IntNode>* queue, int num_inserts, std::vector<IntNode>* nodes,
    ...
    for (int i = 0; i < num_inserts && !*failed; ++i) {
        ...
        std::atomic<int32_t> value(counter->fetch_add(1));
...
void ConsumerThread(InternalQueue<IntNode>* queue, int num_consumes, int delta, std::vector<int>* results,
                    bool* failed) {
...
TEST(InternalQueue, TestMultiProducerMultiConsumer) {
    ...
    for (int num_producers = 1; num_producers < 5; num_producers += 3) {
        ...
        for (int i = 0; i < num_producers; ++i) {
            producers.add_thread(new thread(ProducerThread, &queue, num_per_producer, &nodes, &counter, &failed));
        }

        for (int i = 0; i < NUM_CONSUMERS; ++i) {
            consumers.add_thread(
                    new thread(ConsumerThread, &queue, num_per_consumer, expected_delta, &results[i], &failed));
        }
