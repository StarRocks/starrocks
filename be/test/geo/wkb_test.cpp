...
TEST(WkbCodecTest, RejectsCoordinatesOutsideCrs84Bounds) {
    for (const auto& input : {"POINT (180.0001 0)", "POINT (-180.0001 0)", "POINT (0 90.0001)", "POINT (0 -90.0001)"}) {
        WkbGeometry geometry;
        EXPECT_FALSE(WkbCodec::parse_wkt(input, &geometry).ok()) << input;
    }

    WkbGeometry geometry;
    const std::string longitude = from_hex("01010000000000000000a066400000000000000000");
    EXPECT_FALSE(WkbCodec::parse_wkb(Slice(longitude), &geometry).ok());
}
