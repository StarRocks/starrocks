...
static void verify_join_does_not_close_foreign_fds(int num_workers) {
    auto handler = std::make_unique<WorkerEchoHandler>();
    auto server = std::make_unique<EvHttpServer>(0, num_workers);
    ASSERT_TRUE(server->register_handler(GET, "/echo", handler.get()));
    ASSERT_OK(server->start());

    // Make sure the workers have entered dispatch before stop(), see port_auto_assignment_distinct.
    {
        HttpClient client;
...
TEST_F(EvHttpServerTest, join_does_not_close_foreign_fds_single_worker) {
    verify_join_does_not_close_foreign_fds(1);
}

TEST_F(EvHttpServerTest, join_does_not_close_foreign_fds_multi_worker) {
