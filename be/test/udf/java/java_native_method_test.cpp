...
TEST_F(JavaNativeMethodTest, get_addrs_date_datetime) {
    auto env = getJNIEnv();
    {
        ...
        EXPECT_EQ(results[1], (jlong)date_col->get_data().data());
        env->DeleteLocalRef(arr);
    }
    {
        ...
        EXPECT_EQ(results[0], (jlong)nullable->null_column_data().data());
        EXPECT_EQ(results[1], (jlong)ts_col->get_data().data());
        env->DeleteLocalRef(arr);
    }
}
...
TEST_F(JavaNativeMethodTest, get_struct_field_addrs) {
    ...
    auto* struct_col = down_cast<StructColumn*>(nullable->data_column_mutable_ptr().get());
    for (int i = 0; i < len; ++i) {
        EXPECT_EQ(reinterpret_cast<jlong>(struct_col->fields_column()[i]->as_mutable_ptr().get()), buf[i])
