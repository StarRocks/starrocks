...
TEST_F(LakeServiceTest, test_publish_version_for_write) {
    ...
    ASSERT_FALSE(fs::path_exist(tablet.txn_log_location(1000)));
    ASSERT_TRUE(tablet.get_txn_log(1000).status().is_not_found()) << "Maybe you forgot to clear the in-memory cache";
    ASSERT_FALSE(fs::path_exist(tablet.txn_log_location(1001)));
    ASSERT_TRUE(tablet.get_txn_log(1001).status().is_not_found()) << "Maybe you forgot to clear the in-memory cache";
...
TEST_F(LakeServiceTest, test_abort) {
    ...
    // TxnLog with 2 segments
    {
        write_random_content(_tablet_mgr->segment_location(_tablet_id, "1.dat"));
        write_random_content(_tablet_mgr->segment_location(_tablet_id, "2.dat"));
    ...
    {
        write_random_content(_tablet_mgr->segment_location(_tablet_id, "3.dat"));
        write_random_content(_tablet_mgr->segment_location(_tablet_id, "4.dat"));
    ...
    ASSERT_TRUE(!fs::path_exist(tablet.txn_vlog_location(1000)));
    ASSERT_TRUE(!fs::path_exist(tablet.txn_vlog_location(1001)));
    ASSERT_TRUE(!fs::path_exist(tablet.txn_vlog_location(1002)));

    ASSERT_TRUE(tablet.get_txn_log(1000).status().is_not_found()) << "Maybe you forgot to clear the in-memory cache";
    ASSERT_TRUE(tablet.get_txn_log(1001).status().is_not_found()) << "Maybe you forgot to clear the in-memory cache";
    ASSERT_TRUE(tablet.get_txn_log(1002).status().is_not_found()) << "Maybe you forgot to clear the in-memory cache";
...
// NOLINTNEXTLINE
TEST_F(LakeServiceTest, test_abort_multi_tablets) {
    std::vector<std::string> segment_names;
    std::vector<int64_t> tablet_ids{next_id(), next_id(), next_id()};
    int64_t txn_id = next_id();

    // Generate txn log and segment files
    for (auto tablet_id : tablet_ids) {
        segment_names.emplace_back(lake::gen_segment_filename(txn_id));

        write_random_content(_tablet_mgr->segment_location(tablet_id, segment_names.back()));

        lake::TxnLog txnlog;
        txnlog.set_tablet_id(tablet_id);
        txnlog.set_txn_id(txn_id);
        txnlog.mutable_op_write()->mutable_rowset()->set_overlapped(false);
        txnlog.mutable_op_write()->mutable_rowset()->set_num_rows(12);
        txnlog.mutable_op_write()->mutable_rowset()->set_data_size(4096);
        txnlog.mutable_op_write()->mutable_rowset()->add_segments(segment_names.back());
        ASSERT_OK(_tablet_mgr->put_txn_log(txnlog));
    }
    ...
    // Send AbortTxn request
    {
        lake::AbortTxnRequest request;
        lake::AbortTxnResponse response;
        request.mutable_tablet_ids()->Add(tablet_ids.begin(), tablet_ids.end());
        request.add_txn_ids(txn_id);
        _lake_service.abort_txn(nullptr, &request, &response, nullptr);
    }

    // Check whether files have been deleted.
    for (auto tablet_id : tablet_ids) {
        auto log_path = _tablet_mgr->txn_log_location(tablet_id, txn_id);
        ASSERT_FALSE(fs::path_exist(log_path));
        ASSERT_TRUE(_tablet_mgr->get_txn_log(tablet_id, txn_id).status().is_not_found())
                << "Maybe you forgot to clear the in-memory cache";
    }

    for (auto&& segment : segment_names) {
        auto seg_path = _tablet_mgr->segment_location(tablet_ids[0], segment);
        ASSERT_FALSE(fs::path_exist(seg_path));
    }
}
...
TEST_F(LakeServiceTest, test_publish_log_version) {
    ...
    }
    {
        ...
        ASSERT_TRUE(!fs::path_exist(_tablet_mgr->txn_log_location(_tablet_id, 1001)));
...
TEST_F(LakeServiceTest, test_publish_version_issue28244) {
    ...
    ExecEnv::GetInstance()->vacuum_thread_pool()->wait();
    ASSERT_TRUE(!fs::path_exist(_tablet_mgr->txn_log_location(_tablet_id, 102301)));
