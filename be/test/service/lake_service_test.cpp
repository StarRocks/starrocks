...
TEST_F(LakeServiceTest, test_replication_abort_installs_local_fence) {
    auto txn_id = next_id();
    AbortTxnRequest request;
    request.set_skip_cleanup(true);
    auto* txn_info = request.add_txn_infos();
    ...
    TReplicateSnapshotRequest replicate_request;
    replicate_request.__set_transaction_id(txn_id);
    replicate_request.__set_tablet_id(_tablet_id);
    replicate_request.__set_visible_version(1);
    replicate_request.__set_data_version(1);
