...
TEST_F(LakeServiceTest, test_publish_version_missing_tablet_ids) {
    ...
    ASSERT_EQ("neither tablet_ids nor dynamic_tablets_info is set, one of them must be set", cntl.ErrorText());
}
...
TEST_F(LakeServiceTest, test_publish_splitting_tablet) {
    ...
    DynamicTabletsInfoPB dynamic_tablets_info;
    auto* splitting_tablet_info = dynamic_tablets_info.add_splitting_tablet_infos();
    splitting_tablet_info->set_old_tablet_id(_tablet_id);
    splitting_tablet_info->add_new_tablet_ids(next_id());
    splitting_tablet_info->add_new_tablet_ids(next_id());

    // Test dynamic tablet transaction
    {
        PublishVersionRequest publish_request;
        ...
        txn_info->set_txn_type(TXN_DYNAMIC_TABLET);
        txn_info->set_combined_txn_log(false);
        txn_info->set_commit_time(12345);
        txn_info->set_force_publish(false);

        publish_request.add_distribution_columns("test_col");

        publish_request.mutable_dynamic_tablets_info()->CopyFrom(dynamic_tablets_info);
...
TEST_F(LakeServiceTest, test_publish_merging_tablet) {
    ...
    DynamicTabletsInfoPB dynamic_tablets_info;
    auto* merging_tablet_info = dynamic_tablets_info.add_merging_tablet_infos();
...
TEST_F(LakeServiceTest, test_publish_identical_tablet) {
    ...
    DynamicTabletsInfoPB dynamic_tablets_info;
    auto* identical_tablet_info = dynamic_tablets_info.add_identical_tablet_infos();
