...
// FE's aggregatePublishWithCarryForward fills one request from two batches -- the touched tablets and
// the untouched-but-visible indexes it must carry forward -- and the batch added last ends up first in
// publish_reqs. The carry-forward batch carries synthetic TXN_EMPTY infos with no_op_publish=true,
// which have neither the real transaction's commit time nor its gtid. merge_tablet stamps both into
// the parent it builds, so taking them from publish_reqs(0) unconditionally wrote a placeholder
// commit_time/gtid into a metadata page queries then read.
TEST_F(AggregateParentViewTest, test_parent_is_stamped_from_the_batch_that_carries_the_transaction) {
    brpc::Server server;
    MockParentViewLakeService mock_service;
    int port = 0;
    init_server_with_mock(&mock_service, &server, &port);
    ...
    const int64_t kChildTabletId1 = next_id();
    const int64_t kChildTabletId2 = next_id();
    constexpr int64_t kVersion = 83;
    constexpr int64_t kCommitTime = 1750000000;
    constexpr int64_t kGtid = 900001;

    // Two batches, in the order FE produces them: carry-forward first, the real transaction second.
    // Both need a compute node, because the dispatch loop pairs compute_nodes(i) with publish_reqs(i).
    auto request = build_default_agg_request(port);
    auto* carry_forward = request.mutable_publish_reqs(0);
    carry_forward->set_new_version(kVersion);
    auto* empty_txn = carry_forward->add_txn_infos();
    empty_txn->set_txn_id(-1);
    empty_txn->set_no_op_publish(true);

    auto* second_node = request.add_compute_nodes();
    second_node->set_host("127.0.0.1");
    second_node->set_brpc_port(port);
    auto* real_req = request.add_publish_reqs();
    real_req->set_timeout_ms(5000);
    real_req->set_new_version(kVersion);
    real_req->add_tablet_ids(kChildTabletId1);
    real_req->add_tablet_ids(kChildTabletId2);
    auto* real_txn = real_req->add_txn_infos();
    real_txn->set_txn_id(12345);
    real_txn->set_commit_time(kCommitTime);
    real_txn->set_gtid(kGtid);

    // Two children, so the parent goes through merge_tablet -- the single-child path is a plain copy
    // and would carry the child's own commit_time whatever this picks.
    auto* parent_info = request.add_parent_tablet_publish_infos();
    parent_info->set_parent_tablet_id(kParentTabletId);
    parent_info->add_child_tablet_ids(kChildTabletId1);
    parent_info->add_child_tablet_ids(kChildTabletId2);
    ...
    EXPECT_CALL(mock_service, publish_version(_, _, _, _))
            .WillRepeatedly(Invoke([&](::google::protobuf::RpcController*, const PublishVersionRequest* req,
                                       PublishVersionResponse* resp, ::google::protobuf::Closure* done) {
                resp->mutable_status()->set_status_code(0);
                // The carry-forward batch names no tablets and returns no metadata, as on a real publish.
    ...
    ASSERT_EQ(0, response.status().status_code());
    ASSIGN_OR_ABORT(auto parent, _tablet_mgr->get_single_tablet_metadata(kParentTabletId, kVersion));
    EXPECT_EQ(kCommitTime, parent->commit_time()) << "the parent must be stamped from the real transaction";
    EXPECT_EQ(kGtid, parent->gtid()) << "the parent must be stamped from the real transaction";
