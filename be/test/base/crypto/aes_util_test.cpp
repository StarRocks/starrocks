...
static int decode_base64(const std::string& input, char* output) {
    int len = EVP_DecodeBlock(reinterpret_cast<unsigned char*>(output),
...
void do_aes_test(const std::string& source, const std::string& key) {
    ...
    int ret_code = AesUtil::encrypt_ex(AES_128_ECB, (unsigned char*)source.c_str(), source.length(),
    ...
    ret_code = AesUtil::decrypt_ex(AES_128_ECB, dest.get(), encrypted_length, (unsigned char*)key.c_str(), key.length(),
                                   nullptr, 0, true, (unsigned char*)decrypted.get());
    ASSERT_TRUE(ret_code > 0);
...
TEST_F(AesUtilTest, aes_test_by_case) {
    ...
    std::unique_ptr<char[]> encrypt_1(new char[case_1.length() + 3]);
    int length_1 = decode_base64(case_1, encrypt_1.get());
    std::unique_ptr<char[]> decrypted_1(new char[case_1.length()]);
    int ret_code = AesUtil::decrypt_ex(AES_128_ECB, (unsigned char*)encrypt_1.get(), length_1,
                                       (unsigned char*)_aes_key.c_str(), _aes_key.length(), nullptr, 0, true,
    ...
    std::unique_ptr<char[]> encrypt_2(new char[case_2.length() + 3]);
    int length_2 = decode_base64(case_2, encrypt_2.get());
    std::unique_ptr<char[]> decrypted_2(new char[case_2.length()]);
    ret_code = AesUtil::decrypt_ex(AES_128_ECB, (unsigned char*)encrypt_2.get(), length_2,
                                   (unsigned char*)_aes_key.c_str(), _aes_key.length(), nullptr, 0, true,
