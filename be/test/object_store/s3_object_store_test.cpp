...
TEST_F(S3ObjectStoreTest, bucket_operation) {
    ...
    EXPECT_OK(client->delete_bucket(kBucketName));

    // create bucket
    EXPECT_OK(client->create_bucket(kBucketName));
...
TEST_F(S3ObjectStoreTest, object_operation) {
    ...
    EXPECT_OK(os->write(object_value.data(), object_value.size()));
    EXPECT_OK(os->close());

    // test object existence
    EXPECT_OK(client->exist_object(uri));
    ...
    EXPECT_OK(client->download_object(uri, object_path));
    ...
    EXPECT_OK(os2->write(object_value.data(), object_value.size()));
    EXPECT_OK(os2->close());
    std::vector<std::string> vector;
    EXPECT_OK(client->list_objects(kBucketName, "/" /* object_prefix */, &vector));
    ASSERT_EQ(vector.size(), 2);
    ASSERT_EQ(vector[0], "hello");
    ASSERT_EQ(vector[1], "test/hello");

    // delete object
    EXPECT_OK(client->delete_object(uri));
    EXPECT_OK(client->delete_object(uri2));
