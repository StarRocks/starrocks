...
protected:
    void SetUp() override {
        _test_dir = "./test_shared_file_" + std::to_string(time(nullptr));
...
TEST_F(SharedFileTest, test_write_read) {
    // test data
    ...
    std::string shared_file_path = _test_dir + "/test_shared_file.dat";
    ...
    // 2. test write
    {
        ...
        // first writer
        auto shared_writer1 = std::make_unique<SharedWritableFile>(shared_context.get(), opts.encryption_info);

        // second writer
        auto shared_writer2 = std::make_unique<SharedWritableFile>(shared_context.get(), opts.encryption_info);

        // third writer
        auto shared_writer3 = std::make_unique<SharedWritableFile>(shared_context.get(), opts.encryption_info);
    ...
    // 3. test whole read
    {
        ...
        ASSIGN_OR_ABORT(auto reader, fs::new_random_access_file(shared_file_path));
    ...
    // 4. test partial read
    {
        ASSIGN_OR_ABORT(auto fs, FileSystem::CreateSharedFromString(shared_file_path));
        RandomAccessFileOptions opts;

        // read first segment
        FileInfo file_info1{.path = shared_file_path, .size = test_data1.size(), .shared_file_offset = 0};
        ASSIGN_OR_ABORT(auto reader1, fs->new_random_access_file_with_sharing(opts, file_info1));
        std::string buffer1;
        ...
        // read second segment
        FileInfo file_info2{
                .path = shared_file_path, .size = test_data2.size(), .shared_file_offset = test_data1.size()};
        ASSIGN_OR_ABORT(auto reader2, fs->new_random_access_file_with_sharing(opts, file_info2));
        std::string buffer2;
        ...
        // read third segment
        FileInfo file_info3{.path = shared_file_path,
                            .size = test_data3.size(),
                            .shared_file_offset = test_data1.size() + test_data2.size()};
        ASSIGN_OR_ABORT(auto reader3, fs->new_random_access_file_with_sharing(opts, file_info3));
        std::string buffer3;
...
TEST_F(SharedFileTest, test_empty_write) {
    ...
    WritableFileOptions opts;
    auto shared_writer = std::make_unique<SharedWritableFile>(shared_context.get(), opts.encryption_info);
    shared_context->increase_active_writers();
    // write empty data
...
TEST_F(SharedFileTest, test_concurrent_write) {
    ...
    {
        std::vector<std::unique_ptr<SharedWritableFile>> writers;
        WritableFileOptions opts;
    ...
    // verify the content of the shared file
    {
        ...
        RandomAccessFileOptions opts;
        for (int i = 0; i < num_writers; ++i) {
            FileInfo file_info{.path = shared_file_path,
                               .size = base_data.size() + std::to_string(i).size() + 5,
                               .shared_file_offset = (base_data.size() + std::to_string(i).size() + 5) * i};
            ASSIGN_OR_ABORT(auto reader, fs->new_random_access_file_with_sharing(opts, file_info));
