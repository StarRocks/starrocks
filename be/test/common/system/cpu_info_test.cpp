...
TEST_F(CpuInfoTest, ArmCrc32Support) {
    const base::CPU* cpu = base::CPU::instance();
    ASSERT_NE(nullptr, cpu);
#if defined(__aarch64__)
    // On ARM64, base::CPU::has_crc32() should be consistent with CpuInfo's ARM_CRC32 detection.
    EXPECT_EQ(CpuInfo::is_supported(CpuInfo::ARM_CRC32), cpu->has_crc32());
#else
    // On non-ARM64 platforms, ARM CRC32 is not supported.
    EXPECT_FALSE(cpu->has_crc32());
#endif
    // Verify that debug_string() contains crc32 if has_crc32() is true.
    if (cpu->has_crc32()) {
        EXPECT_NE(std::string::npos, cpu->debug_string().find("crc32"));
    }
}
