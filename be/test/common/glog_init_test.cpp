...
TEST_F(UpdateLoggingTest, test_unknown_severity_is_rejected) {
    ...
    // Config validation rejects this before it can reach update_logging, but if the enum in config.h
    // and the severity table here ever drift apart, the caller has to hear about it rather than run
    // on with a level nobody asked for.
