...
TEST_F(SIMDTest, count_zeros_int32) {
    EXPECT_EQ(0u, SIMD::count_zero_int32(std::vector<uint32_t>{}));
    EXPECT_EQ(3u, SIMD::count_zero_int32(std::vector<uint32_t>{0, 0, 0}));
    EXPECT_EQ(1u, SIMD::count_zero_int32(std::vector<uint32_t>{0, 1, 2}));
    EXPECT_EQ(1u, SIMD::count_zero_int32(std::vector<uint32_t>{11, 0, 1}));

    // size greater than 64 will use SSE2 instructions.
    std::vector<uint32_t> nums(100, 0);
    EXPECT_EQ(100u, SIMD::count_zero_int32(nums));
    nums.emplace_back(1);
    EXPECT_EQ(100u, SIMD::count_zero_int32(nums));
    nums.emplace_back(0);
    EXPECT_EQ(101u, SIMD::count_zero_int32(nums));
...
TEST_F(SIMDTest, count_nonzero_int32) {
    // size less than 64, count by loop.
    EXPECT_EQ(0u, SIMD::count_nonzero_int32(std::vector<uint32_t>{}));
    EXPECT_EQ(0u, SIMD::count_nonzero_int32(std::vector<uint32_t>{0, 0, 0}));
    EXPECT_EQ(3u, SIMD::count_nonzero_int32(std::vector<uint32_t>{1, 1, 1}));
    EXPECT_EQ(3u, SIMD::count_nonzero_int32(std::vector<uint32_t>{11, 1, 2}));
    EXPECT_EQ(2u, SIMD::count_nonzero_int32(std::vector<uint32_t>{0, 1, 2}));

    // size greater than 64 will use SSE2 instructions.
    std::vector<uint32_t> numbers(100, 0);
    EXPECT_EQ(0u, SIMD::count_nonzero_int32(numbers));

    for (int i = 1; i <= 10; i++) {
        numbers.emplace_back(1);
    }
    EXPECT_EQ(10u, SIMD::count_nonzero_int32(numbers));

    for (int i = 1; i <= 10; i++) {
        numbers.emplace_back(i);
    }
    EXPECT_EQ(20u, SIMD::count_nonzero_int32(numbers));

    for (int i = 1; i <= 10; i++) {
        numbers.emplace_back(i + 100);
    }
    EXPECT_EQ(30u, SIMD::count_nonzero_int32(numbers));
}
