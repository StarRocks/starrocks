...
TEST_F(AgentTaskTest, get_thread_pool_returns_registered_pools) {
    ...
    auto* remote_snapshot_pool = agent_server->get_thread_pool(TTaskType::REMOTE_SNAPSHOT);
    ...
    EXPECT_NE(nullptr, remote_snapshot_pool);
    EXPECT_NE(nullptr, agent_server->get_thread_pool(TTaskType::EXTERNAL_CLUSTER_SNAPSHOT));
    EXPECT_EQ(remote_snapshot_pool, agent_server->get_thread_pool(TTaskType::TABLET_RESTORE));
}
