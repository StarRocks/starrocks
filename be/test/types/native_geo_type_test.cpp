...
TEST(NativeGeoTypeTest, PrimitiveAndMetadataRoundTrip) {
    for (auto primitive : {TYPE_GEOGRAPHY, TYPE_GEOMETRY}) {
        ...
        EXPECT_EQ(type.geo_type->to_thrift(), thrift.types[0].scalar_type.geo);
        EXPECT_EQ(type, TypeDescriptor::from_thrift(thrift));
        auto protobuf = type.to_protobuf();
        EXPECT_EQ(type.geo_type->to_protobuf().SerializeAsString(),
                  protobuf.types(0).scalar_type().geo().SerializeAsString());
        PTypeDesc decoded;
...
TEST(NativeGeoTypeTest, ColumnRepresentationIsSeparateFromType) {
    ...
    auto thrift = type.to_thrift();
    thrift.types[0].scalar_type.__set_geo(column.type.to_thrift());
    EXPECT_EQ(type, TypeDescriptor::from_thrift(thrift));
    auto proto = type.to_protobuf();
    *proto.mutable_types(0)->mutable_scalar_type()->mutable_geo() = column.type.to_protobuf();
    EXPECT_EQ(type, TypeDescriptor::from_protobuf(proto));

    thrift.types[0].scalar_type.__isset.geo = false;
...
TEST(NativeGeoTypeTest, FrontendWireFixture) {
    // FE serializes semantic metadata directly in PScalarType.geo.
    const std::string_view wire(
            "\x0a\x1c\x08\x00\x12\x18\x08\x1e\x2a\x14\x08\x01\x10\x01\x18\x01\x22\x09\x4f\x47\x43\x3a\x43\x52"
            "\x53\x38\x34\x28\xe6\x21",
            30);
    PTypeDesc proto;
...
TEST(NativeGeoTypeTest, ReservedLogicalTypeSlots) {
    ...
    EXPECT_EQ(TYPE_GEOGRAPHY, thrift_to_type(TPrimitiveType::GEOGRAPHY));
    EXPECT_EQ(TYPE_GEOMETRY, thrift_to_type(TPrimitiveType::GEOMETRY));
}
