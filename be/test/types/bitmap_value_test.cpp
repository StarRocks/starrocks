...
TEST(BitmapValueTest, bitmap_union) {
    ...
    BitmapValue empty4;
    empty4.fast_union({&empty});
    ASSERT_EQ(0, empty4.cardinality());
    empty4.fast_union({&single});
    ASSERT_EQ(1, empty4.cardinality());
    BitmapValue empty5;
    empty5.fast_union({&bitmap});
    ASSERT_EQ(3, empty3.cardinality());
    BitmapValue empty6;
    empty6.fast_union({&empty, &single, &bitmap});
    ASSERT_EQ(3, empty3.cardinality());
    ...
    single4.fast_union({&empty});
    ASSERT_EQ(1, single4.cardinality());
    single4.fast_union({&single});
    ASSERT_EQ(2, single4.cardinality());
    BitmapValue single5(1027);
    single5.fast_union({&bitmap});
    ASSERT_EQ(4, single5.cardinality());
    BitmapValue single6(1027);
    single6.fast_union({&empty, &single, &bitmap});
    ASSERT_EQ(4, single6.cardinality());
    ...
    bitmap3.add(2048);
    bitmap3.add(4096);
    bitmap3.fast_union({&empty});
    ASSERT_EQ(3, bitmap3.cardinality());
    bitmap3.fast_union({&single});
    ASSERT_EQ(3, bitmap3.cardinality());
    bitmap3.fast_union({&bitmap});
    ASSERT_EQ(5, bitmap3.cardinality());
}
