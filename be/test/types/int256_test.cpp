...
TEST_F(Int256Test, conversion_operator_consistency) {
    ...
    std::vector<int256_t> test_values = {
        int256_t(0), int256_t(1), int256_t(-1),
        int256_t(1000), int256_t(-1000),
        int256_t(INT32_MAX), int256_t(INT32_MIN),
        int256_t(1) << 32, int256_t(1) << 40,
    };
    ...
    for (const auto& value : test_values) {
        ...
        if (value >= int256_t(INT32_MIN) && value <= int256_t(INT32_MAX)) {
            ASSERT_EQ(value, back_converted)
                << "Round-trip failed for " << value.to_string();
        }
    }
}
...
TEST_F(Int256Test, constructor_from_double_round_trip_large_values) {
    ...
    std::vector<double> large_test_values = {
        std::pow(2.0, 100),
        std::pow(2.0, 128),
        std::pow(2.0, 150),
        std::pow(2.0, 200),
    ...
    for (double original : large_test_values) {
        ...
        if (!std::isinf(back_to_double)) {
            ...
            ASSERT_TRUE(relative_error < 1e-10)
                << "Too much error in round-trip for " << original
                << " (got " << back_to_double << ", error=" << relative_error << ")";
        }
    }
}
