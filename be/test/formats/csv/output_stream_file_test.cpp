...
TEST_F(OutputStreamFileTest, TestSnappyCompression) {
    ...
    // Decompress and verify
    std::string decompressed = starrocks::test::decompress_data(compressed_content, CompressionTypePB::SNAPPY);
    EXPECT_EQ(decompressed, test_data);
}
...
TEST_F(OutputStreamFileTest, TestLz4Compression) {
    ...
    // Decompress and verify
    std::string decompressed = starrocks::test::decompress_data(compressed_content, CompressionTypePB::LZ4);
    EXPECT_EQ(decompressed, test_data);
}
...
// Test all compression types with large data
TEST_F(OutputStreamFileTest, TestAllCompressionTypesLargeData) {
    std::vector<std::pair<CompressionTypePB, std::string>> compression_types = {
            {CompressionTypePB::GZIP, ".gz"}, {CompressionTypePB::SNAPPY, ".snappy"}, {CompressionTypePB::ZSTD, ".zst"},
            {CompressionTypePB::LZ4, ".lz4"}, {CompressionTypePB::LZ4_FRAME, ".lz4"},
    };
