...
TEST(MapConverterTest, test_read_write_map_int_string) {
    ...
    // {NULL:NULL}
    EXPECT_EQ(1, col->get(2).get_map().size());
    EXPECT_EQ("{NULL:NULL}", col->debug_item(2));
    // {1:NULL,2:NULL}
    EXPECT_EQ(2, col->get(3).get_map().size());
    EXPECT_EQ("{1:NULL},{2:NULL}", col->debug_item(3));
    ...
    ASSERT_EQ("{}{1:'abc'},{2:NULL},{3:''}{NULL:NULL}{1:NULL},{2:NULL}", buff.as_string());
}
...
TEST(MapConverterTest, test_read_write_nest_map) {
    ...
    // {NULL:{NULL:NULL}}
    EXPECT_EQ(1, col->get(3).get_map().size());
    EXPECT_EQ("{NULL:{NULL:NULL}}", col->debug_item(3));
    // {NULL:{NULL:"NULL"},-20308764:{33:""}}
    EXPECT_EQ(2, col->get(4).get_map().size());
    ...
    EXPECT_EQ("{11:{{1:'abc'},{2:NULL},{1:NULL}},22:{{2:'unique'}}", col->debug_item(5));
...
// NOLINTNEXTLINE
TEST(MapConverterTest, test_hive_read_string01) {
    // ARRAY<TINYINT>
...
// NOLINTNEXTLINE
TEST(MapConverterTest, test_hive_read_string02) {
    // ARRAY<STRING>
    TypeDescriptor t(TYPE_ARRAY);
    t.children.emplace_back(TYPE_VARCHAR);
...
// NOLINTNEXTLINE
TEST(MapConverterTest, test_hive_read_string03) {
    // ARRAY<ARRAY<STRING>>
    TypeDescriptor t(TYPE_ARRAY);
...
// NOLINTNEXTLINE
TEST(MapConverterTest, test_hive_read_string04) {
...
// NOLINTNEXTLINE
TEST(MapConverterTest, test_hive_read_string05) {
...
// NOLINTNEXTLINE
TEST(MapConverterTest, test_get_collection_delimiter) {
    // Rule refer to:
...
// NOLINTNEXTLINE
TEST(MapConverterTest, test_write_string) {
    // ARRAY<DATE>
    TypeDescriptor t(TYPE_ARRAY);
