...
TEST_F(AvroReaderTest, test_read_primitive_types) {
    ...
    int32_t num_of_columns_from_file = 8;
    ...
    // read data
    ASSERT_OK(reader->open(filename, _state.get(), _counter, &slot_descs, num_of_columns_from_file, strict_mode,
                           invalid_as_null));
...
TEST_F(AvroReaderTest, test_read_complex_types) {
    std::string filename = "complex.avro";
    std::vector<SlotDescriptor*> slot_descs;
    int32_t num_of_columns_from_file = 6;
    bool strict_mode = true;
    bool invalid_as_null = false;
    int rows_to_read = 2;
...
TEST_F(AvroReaderTest, test_read_complex_nest_types) {
    std::string filename = "complex_nest.avro";
    std::vector<SlotDescriptor*> slot_descs;
    int32_t num_of_columns_from_file = 13;
    bool strict_mode = true;
    bool invalid_as_null = false;
    int rows_to_read = 2;
    ...
    materialize_src_chunk_adaptive_nullable_column(chunk);
    ASSERT_EQ(1, chunk->num_rows());
    ASSERT_EQ("[{inner:{val:123}}, {list:[1,2,3]}, {dict:{'a':'x','b':'y'}}, [{value:'one'},{value:'two'}], "
              "[[1,2],[3,4]], [{'a':1,'b':2},{'c':3}], {'r1':{id:10},'r2':{id:20}}, {'nums':[5,6,7]}, "
              "{'outer':{'inner':'val'}}, {entries:[{flag:1},{flag:0}]}, {entries:[{'x':'1'},{'y':'2'}]}, "
              "[{'one':{ok:1}},{'two':{ok:0}}], {'group':[{score:99.9},{score:88.8}]}]", chunk->debug_row(0));
...
TEST_F(AvroReaderTest, test_read_logical_types) {
    std::string filename = "logical.avro";
    std::vector<SlotDescriptor*> slot_descs;
    int32_t num_of_columns_from_file = 10;
    bool strict_mode = true;
    bool invalid_as_null = false;
    int rows_to_read = 2;
    ...
    // add last duration type ut later
    ...
    materialize_src_chunk_adaptive_nullable_column(chunk);
    ASSERT_EQ(1, chunk->num_rows());
    ASSERT_EQ("[1234.56, 1234.56, '61ed1775-2ce2-4f88-8352-1da6847512d6', 2025-04-11, 55543806, 55543806481, "
              "2025-04-11 15:25:43.806000, 2025-04-11 15:25:43.806481, 1744356343806, 1744356343806481]",
              chunk->debug_row(0));
