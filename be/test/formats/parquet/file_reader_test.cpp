...
private:
    ...
    static void _append_column_for_chunk(PrimitiveType column_type, ChunkPtr* chunk);
...
HdfsScannerContext* FileReaderTest::_create_context_for_partition() {
    ...
    make_column_info_vector(ctx->tuple_desc, &ctx->materialized_columns);
    ctx->scan_ranges.emplace_back(_create_scan_range(_file1_path, 1024));
    auto column = ColumnHelper::create_const_column<PrimitiveType::TYPE_INT>(1, 1);
...
void FileReaderTest::_append_column_for_chunk(PrimitiveType column_type, ChunkPtr* chunk) {
