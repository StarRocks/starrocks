...
TEST_F(FileReaderTest, test_read_variant_shredding_events_typed_array) {
    const std::string shred_file_path = "./be/test/formats/parquet/test_data/variant_shredding.parquet";
    auto file_reader = _create_file_reader(shred_file_path);

    // Read only data.typed_value.events.typed_value and verify typed array layout.
    TypeDescriptor event_type_struct =
            TypeDescriptor::create_struct_type({"value", "typed_value"}, {TYPE_VARBINARY_DESC, TYPE_VARCHAR_DESC});
    TypeDescriptor event_count_struct =
            TypeDescriptor::create_struct_type({"value", "typed_value"}, {TYPE_VARBINARY_DESC, TYPE_INT_DESC});
    TypeDescriptor event_object_typed_struct =
            TypeDescriptor::create_struct_type({"type", "count"}, {event_type_struct, event_count_struct});
    TypeDescriptor event_element_struct =
            TypeDescriptor::create_struct_type({"typed_value"}, {event_object_typed_struct});
    TypeDescriptor events_array_typed = TypeDescriptor::create_array_type(event_element_struct);
    TypeDescriptor events_struct = TypeDescriptor::create_struct_type({"typed_value"}, {events_array_typed});
    TypeDescriptor typed_value_struct = TypeDescriptor::create_struct_type({"events"}, {events_struct});
    TypeDescriptor data_struct = TypeDescriptor::create_struct_type({"typed_value"}, {typed_value_struct});
    ...
    int64_t end_offset = 0;
    file_reader->_row_group_readers[0]->collect_io_ranges(&ranges, &end_offset);
    // events element fields can read both fallback and typed leaves.
    EXPECT_GE(ranges.size(), 2);

    auto chunk = std::make_shared<Chunk>();
    chunk->append_column(ColumnHelper::create_column(data_struct, true), chunk->num_columns());
    status = file_reader->get_next(&chunk);
    ASSERT_TRUE(status.ok()) << "Failed to read shredded variant events data: " << status.message();
    ASSERT_EQ(chunk->num_rows(), 5);
    ...
    const Column* typed_value_col =
            ColumnHelper::get_data_column(data_struct_col->field_column_raw_ptr("typed_value").value());
    const auto* typed_value_struct_col = down_cast<const StructColumn*>(typed_value_col);
    const Column* events_col =
            ColumnHelper::get_data_column(typed_value_struct_col->field_column_raw_ptr("events").value());
    const auto* events_struct_col = down_cast<const StructColumn*>(events_col);
    const Column* events_typed_col = events_struct_col->field_column_raw_ptr("typed_value").value();

    const Column* events_typed_data = ColumnHelper::get_data_column(events_typed_col);
    ASSERT_TRUE(events_typed_data->is_array());
    const auto* events_array_col = down_cast<const ArrayColumn*>(events_typed_data);
    const auto& offsets = events_array_col->offsets().get_data();
    ASSERT_EQ(offsets.size(), chunk->num_rows() + 1);
    for (size_t i = 0; i < chunk->num_rows(); ++i) {
        EXPECT_EQ(offsets[i + 1] - offsets[i], 2) << "row=" << i;
    }

    const Column* element_col = ColumnHelper::get_data_column(events_array_col->elements_column().get());
    ASSERT_TRUE(element_col->is_struct());
    const auto* element_struct_col = down_cast<const StructColumn*>(element_col);
    const Column* element_typed_col =
            ColumnHelper::get_data_column(element_struct_col->field_column_raw_ptr("typed_value").value());
    const auto* element_typed_struct_col = down_cast<const StructColumn*>(element_typed_col);

    const Column* type_col =
            ColumnHelper::get_data_column(element_typed_struct_col->field_column_raw_ptr("type").value());
    const auto* type_struct_col = down_cast<const StructColumn*>(type_col);
    const Column* type_value_col = type_struct_col->field_column_raw_ptr("value").value();
    const Column* type_typed_col = type_struct_col->field_column_raw_ptr("typed_value").value();
    ASSERT_TRUE(type_value_col->is_nullable());
    ASSERT_TRUE(type_typed_col->is_nullable());
    const auto* type_value_nullable_col = down_cast<const NullableColumn*>(type_value_col);
    const auto* type_nullable_col = down_cast<const NullableColumn*>(type_typed_col);
    const auto* type_value_data_col = down_cast<const BinaryColumn*>(type_value_nullable_col->data_column().get());
    const auto* type_data_col = down_cast<const BinaryColumn*>(type_nullable_col->data_column().get());

    const Column* count_col =
            ColumnHelper::get_data_column(element_typed_struct_col->field_column_raw_ptr("count").value());
    const auto* count_struct_col = down_cast<const StructColumn*>(count_col);
    const Column* count_value_col = count_struct_col->field_column_raw_ptr("value").value();
    const Column* count_typed_col = count_struct_col->field_column_raw_ptr("typed_value").value();
    ASSERT_TRUE(count_value_col->is_nullable());
    ASSERT_TRUE(count_typed_col->is_nullable());
    const auto* count_value_nullable_col = down_cast<const NullableColumn*>(count_value_col);
    const auto* count_nullable_col = down_cast<const NullableColumn*>(count_typed_col);
    const auto* count_value_data_col = down_cast<const BinaryColumn*>(count_value_nullable_col->data_column().get());
    const auto* count_data_col = down_cast<const FixedLengthColumn<int32_t>*>(count_nullable_col->data_column().get());

    ASSERT_EQ(type_nullable_col->size(), 10);
    ASSERT_EQ(count_nullable_col->size(), 10);
    ASSERT_EQ(type_value_nullable_col->size(), 10);
    ASSERT_EQ(count_value_nullable_col->size(), 10);

    size_t type_typed_non_null = 0;
    size_t type_value_non_null = 0;
    size_t count_typed_non_null = 0;
    size_t count_value_non_null = 0;
    for (size_t i = 0; i < 10; ++i) {
        if (!type_nullable_col->is_null(i)) {
            type_typed_non_null++;
        }
        if (!type_value_nullable_col->is_null(i)) {
            type_value_non_null++;
        }
        if (!count_nullable_col->is_null(i)) {
            count_typed_non_null++;
        }
        if (!count_value_nullable_col->is_null(i)) {
            count_value_non_null++;
        }
    }
    EXPECT_GT(type_typed_non_null + type_value_non_null, 0);
    EXPECT_GT(count_typed_non_null + count_value_non_null, 0);
    EXPECT_EQ(type_typed_non_null + type_value_non_null, 10);
    EXPECT_EQ(count_typed_non_null + count_value_non_null, 10);

    if (type_typed_non_null > 1) {
        EXPECT_EQ(type_data_col->get_slice(0).to_string(), "view");
        EXPECT_EQ(type_data_col->get_slice(1).to_string(), "click");
    } else if (type_value_non_null > 1) {
        EXPECT_FALSE(type_value_data_col->get_slice(0).empty());
        EXPECT_FALSE(type_value_data_col->get_slice(1).empty());
    }

    if (count_typed_non_null > 1) {
        EXPECT_EQ(count_data_col->get_data()[0], 1);
        EXPECT_EQ(count_data_col->get_data()[1], 2);
    } else if (count_value_non_null > 1) {
        EXPECT_FALSE(count_value_data_col->get_slice(0).empty());
        EXPECT_FALSE(count_value_data_col->get_slice(1).empty());
    }
}

TEST_F(FileReaderTest, test_read_variant_shredding_variant_typed_schema) {
    const std::string shred_file_path = "./be/test/formats/parquet/test_data/variant_shredding.parquet";
    auto file_reader = _create_file_reader(shred_file_path);
    ...
    const Column* variant_data_col = ColumnHelper::get_data_column(chunk->get_column_by_index(0).get());
    ASSERT_TRUE(variant_data_col->is_variant());
    const auto* variant_col = down_cast<const VariantColumn*>(variant_data_col);

    // Typed columns are optional for parquet reader. When not materialized, we still
    // consider this read path valid and rely on metadata/remain_value fallback.
    int age_idx = variant_col->find_shredded_path("age");
    if (age_idx < 0) {
        SUCCEED();
        return;
    }
    ASSERT_LT(static_cast<size_t>(age_idx), variant_col->typed_columns().size());
    ASSERT_LT(static_cast<size_t>(age_idx), variant_col->shredded_types().size());
    EXPECT_EQ(variant_col->shredded_types()[age_idx].type, TYPE_INT);

    const Column* age_typed_col = variant_col->typed_columns()[age_idx].get();
    ASSERT_TRUE(age_typed_col->is_nullable());
    const Column* age_typed_data_col = ColumnHelper::get_data_column(age_typed_col);
    ASSERT_TRUE(age_typed_data_col->is_numeric());

    int events_idx = variant_col->find_shredded_path("events");
    if (events_idx < 0) {
        // In mixed fallback batches (for example, events.type comes from fallback value),
        // top-level array typed path may be intentionally not materialized to preserve
        // query correctness over fallback semantics.
        SUCCEED();
        return;
    }
    ASSERT_LT(static_cast<size_t>(events_idx), variant_col->typed_columns().size());
    ASSERT_LT(static_cast<size_t>(events_idx), variant_col->shredded_types().size());
    EXPECT_EQ(variant_col->shredded_types()[events_idx].type, TYPE_VARIANT);

    const Column* events_typed_col = variant_col->typed_columns()[events_idx].get();
    const Column* events_data_col = ColumnHelper::get_data_column(events_typed_col);
    ASSERT_TRUE(events_data_col->is_variant());
    const auto* events_variant_col = down_cast<const VariantColumn*>(events_data_col);
    // Nested variant may have typed_columns for read-time acceleration, but does not
    // participate in top-level merge/append schema alignment semantics.
    EXPECT_GT(events_variant_col->size(), 0);
}

TEST_F(FileReaderTest, test_read_variant_shredding_typed_schema_not_flattened) {
    const std::string shred_file_path = "./be/test/formats/parquet/test_data/variant_shredding.parquet";
    auto file_reader = _create_file_reader(shred_file_path);
    ...
    const auto& top_paths = variant_col->shredded_paths();
    if (top_paths.empty()) {
        // Typed paths are optional in parquet reader minimal mode.
        SUCCEED();
        return;
    }
    EXPECT_LT(variant_col->find_shredded_path("events.type"), 0);
    EXPECT_LT(variant_col->find_shredded_path("events.count"), 0);
    for (const auto& path : top_paths) {
        EXPECT_FALSE(path.starts_with("events.")) << "top-level path should not flatten array element field: " << path;
    }

    int events_idx = variant_col->find_shredded_path("events");
    if (events_idx < 0) {
        // Mixed fallback in array elements: top-level array typed path can be suppressed.
        SUCCEED();
        return;
    }
    ASSERT_LT(static_cast<size_t>(events_idx), variant_col->typed_columns().size());
    ...
    const TypeDescriptor& events_type = variant_col->shredded_types()[events_idx];
    ASSERT_EQ(events_type.type, TYPE_VARIANT);

    const Column* events_typed_col = variant_col->typed_columns()[events_idx].get();
    const Column* events_data_col = ColumnHelper::get_data_column(events_typed_col);
    ASSERT_TRUE(events_data_col->is_variant());
    const auto* events_variant_col = down_cast<const VariantColumn*>(events_data_col);
    // Nested variant typed schema is implementation-defined in this mode.
    EXPECT_GT(events_variant_col->size(), 0);
}
