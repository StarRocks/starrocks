...
TEST_F(FileReaderTest, test_read_variant_shredding_deep_items_children_weight) {
    const std::string shred_file_path = "./be/test/formats/parquet/test_data/variant_shredding.parquet";
    auto file_reader = _create_file_reader(shred_file_path);

    // Read only data.typed_value.deep_items.typed_value.children.typed_value.weight.typed_value (INT32).
    TypeDescriptor weight_struct = TypeDescriptor::create_struct_type({"typed_value"}, {TYPE_INT_DESC});
    TypeDescriptor child_typed_struct = TypeDescriptor::create_struct_type({"weight"}, {weight_struct});
    TypeDescriptor child_struct = TypeDescriptor::create_struct_type({"typed_value"}, {child_typed_struct});
    TypeDescriptor children_array = TypeDescriptor::create_array_type(child_struct);
    TypeDescriptor children_struct = TypeDescriptor::create_struct_type({"typed_value"}, {children_array});
    TypeDescriptor deep_item_typed_struct = TypeDescriptor::create_struct_type({"children"}, {children_struct});
    TypeDescriptor deep_item_struct = TypeDescriptor::create_struct_type({"typed_value"}, {deep_item_typed_struct});
    TypeDescriptor deep_items_array = TypeDescriptor::create_array_type(deep_item_struct);
    TypeDescriptor deep_items_struct = TypeDescriptor::create_struct_type({"typed_value"}, {deep_items_array});
    TypeDescriptor typed_value_struct = TypeDescriptor::create_struct_type({"deep_items"}, {deep_items_struct});
    TypeDescriptor data_struct = TypeDescriptor::create_struct_type({"typed_value"}, {typed_value_struct});
    ...
    int64_t end_offset = 0;
    file_reader->_row_group_readers[0]->collect_io_ranges(&ranges, &end_offset);
    // Only the deep code leaf column should be read.
    EXPECT_EQ(ranges.size(), 1);
    ...
    ColumnPtr typed_value_col = ColumnHelper::get_data_column(data_struct_col->field_column("typed_value"));
    const auto* typed_value_struct_col = down_cast<const StructColumn*>(typed_value_col.get());
    ColumnPtr deep_items_col = ColumnHelper::get_data_column(typed_value_struct_col->field_column("deep_items"));
    const auto* deep_items_struct_col = down_cast<const StructColumn*>(deep_items_col.get());
    ColumnPtr deep_items_array_col_data =
            ColumnHelper::get_data_column(deep_items_struct_col->field_column("typed_value"));
    const auto* deep_items_array_col = down_cast<const ArrayColumn*>(deep_items_array_col_data.get());
    ColumnPtr deep_items_elements = ColumnHelper::get_data_column(deep_items_array_col->elements_column());
    const auto* deep_item_struct_col = down_cast<const StructColumn*>(deep_items_elements.get());
    ColumnPtr deep_item_typed_col = ColumnHelper::get_data_column(deep_item_struct_col->field_column("typed_value"));
    const auto* deep_item_typed_struct_col = down_cast<const StructColumn*>(deep_item_typed_col.get());
    ColumnPtr children_col = ColumnHelper::get_data_column(deep_item_typed_struct_col->field_column("children"));
    const auto* children_struct_col = down_cast<const StructColumn*>(children_col.get());
    ColumnPtr children_array_col_data = ColumnHelper::get_data_column(children_struct_col->field_column("typed_value"));
    const auto* children_array_col = down_cast<const ArrayColumn*>(children_array_col_data.get());
    ColumnPtr children_elements = ColumnHelper::get_data_column(children_array_col->elements_column());
    const auto* child_struct_col = down_cast<const StructColumn*>(children_elements.get());
    ColumnPtr child_typed_col = ColumnHelper::get_data_column(child_struct_col->field_column("typed_value"));
    const auto* child_typed_struct_col = down_cast<const StructColumn*>(child_typed_col.get());
    ColumnPtr weight_col = ColumnHelper::get_data_column(child_typed_struct_col->field_column("weight"));
    const auto* weight_struct_col = down_cast<const StructColumn*>(weight_col.get());
    ColumnPtr weight_typed_col = ColumnHelper::get_data_column(weight_struct_col->field_column("typed_value"));

    const auto* weight_values = down_cast<const FixedLengthColumn<int32_t>*>(weight_typed_col.get());
    ASSERT_GT(weight_values->size(), 0);

    const auto* deep_items_offsets =
            down_cast<const ArrayColumn::OffsetColumn*>(deep_items_array_col->offsets_column().get());
    const auto* children_offsets =
            down_cast<const ArrayColumn::OffsetColumn*>(children_array_col->offsets_column().get());
    ASSERT_EQ(deep_items_array_col->size(), chunk->num_rows());
    ASSERT_EQ(children_array_col->size(), deep_items_elements->size());

    const auto& deep_items_offset_data = deep_items_offsets->get_data();
    const auto& children_offset_data = children_offsets->get_data();
    for (size_t row = 0; row < chunk->num_rows(); ++row) {
        uint32_t deep_start = deep_items_offset_data[row];
        uint32_t deep_end = deep_items_offset_data[row + 1];
        ASSERT_EQ(deep_end - deep_start, 2);
        for (uint32_t di = deep_start; di < deep_end; ++di) {
            uint32_t child_start = children_offset_data[di];
            uint32_t child_end = children_offset_data[di + 1];
            ASSERT_EQ(child_end - child_start, 2);
            int j = static_cast<int>(di - deep_start);
            for (uint32_t ci = child_start; ci < child_end; ++ci) {
                int k = static_cast<int>(ci - child_start);
                int32_t expected = static_cast<int32_t>(row + j + k);
                ASSERT_EQ(weight_values->get_data()[ci], expected);
            }
        }
    }
}
