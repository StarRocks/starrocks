...
protected:
    ...
    void check(const std::string& filepath, const TypeDescriptor& col_type, const std::string& col_name, const std::string& expected_value,
               const size_t expected_rows, bool is_failed = false) {
    ...
    TEST_F(ColumnConverterTest, Int32Test) {
        ...
        }
        {
            ...
            {
                const TypeDescriptor col_type = TypeDescriptor::from_logical_type(LogicalType::TYPE_DECIMAL128, -1, 9,
                                                                                  1);
    ...
    TEST_F(ColumnConverterTest, FLBATest) {
        ...
        }
        {
            ...
            {
                const TypeDescriptor col_type = TypeDescriptor::from_logical_type(LogicalType::TYPE_DECIMALV2, -1, 5, 1);
                check(file_path, col_type, col_name, "[6809.6]", expected_rows);
            }
            {
                const TypeDescriptor col_type = TypeDescriptor::from_logical_type(LogicalType::TYPE_DECIMAL32, -1, 5, 1);
                check(file_path, col_type, col_name, "[6809.6]", expected_rows);
            }
            {
                const TypeDescriptor col_type = TypeDescriptor::from_logical_type(LogicalType::TYPE_DECIMAL64, -1, 5, 1);
                check(file_path, col_type, col_name, "[6809.6]", expected_rows);
            }
            {
