...
// rle-encode for boolean column  test
TEST_F(ParquetEncodingTest, Boolean) {
    std::vector<uint8_t> values;
    for (int i = 0; i < 32; i++) {
        values.push_back(i % 3 == 0);
    ...
    const EncodingInfo* rle_encoding = nullptr;
    EncodingInfo::get(tparquet::Type::BOOLEAN, tparquet::Encoding::RLE, &rle_encoding);
    ASSERT_TRUE(rle_encoding != nullptr);
    // rle
    {
        std::unique_ptr<Decoder> decoder;
        auto st = rle_encoding->create_decoder(&decoder);
        ASSERT_TRUE(st.ok());

        std::unique_ptr<Encoder> encoder;
        st = rle_encoding->create_encoder(&encoder);
        ASSERT_TRUE(st.ok());
