...
//   https://github.com/apache/incubator-doris/blob/master/be/test/runtime/disk_io_mgr_test.cpp
...
public:
    void write_validate_callback(int num_writes, DiskIoMgr::WriteRange** written_range, DiskIoMgr* io_mgr,
                                 DiskIoMgr::RequestContext* reader, int32_t* data, Status expected_status,
                                 const Status& status) {
        ...
        if (status.ok()) {
            ...
            scan_range->reset(NULL, (*written_range)->file(), (*written_range)->len(), (*written_range)->offset(), 0,
                              false, false, DiskIoMgr::ScanRange::NEVER_CACHE);
            validate_sync_read(io_mgr, reader, scan_range, reinterpret_cast<const char*>(data), sizeof(int32_t));
        }

        {
            std::lock_guard<std::mutex> l(_written_mutex);
            ++_num_ranges_written;
...
protected:
    ...
    static void validate_sync_read(DiskIoMgr* io_mgr, DiskIoMgr::RequestContext* reader, DiskIoMgr::ScanRange* range,
                                   const char* expected, int expected_len = -1) {
    ...
    static void validate_scan_range(DiskIoMgr::ScanRange* range, const char* expected, int expected_len,
        ...
        while (true) {
            ...
            memcpy(result + range->offset() + buffer->scan_range_offset(), buffer->buffer(), buffer->len());
            buffer->return_buffer();
        }
    ...
    static void scan_range_thread(DiskIoMgr* io_mgr, DiskIoMgr::RequestContext* reader, const char* expected_result,
                                  int expected_len, const Status& expected_status, int max_ranges,
    ...
    DiskIoMgr::ScanRange* init_range(int num_buffers, const char* file_path, int offset, int len, int disk_id,
                                     int64_t mtime, void* meta_data = NULL, bool is_cached = false) {
    ...
    std::mutex _written_mutex;
    std::condition_variable _writes_done;
...
TEST_F(DiskIoMgrTest, SingleWriter) {
    ...
    for (int num_threads_per_disk = 1; num_threads_per_disk <= 5; ++num_threads_per_disk) {
        for (int num_disks = 1; num_disks <= 5; num_disks += 2) {
            ...
            for (int i = 0; i < num_ranges; ++i) {
                ...
                *data = rand();
                ...
                DiskIoMgr::WriteRange::WriteDoneCallback callback =
                        std::bind(std::mem_fn(&DiskIoMgrTest::write_validate_callback), this, num_ranges, new_range,
                                  read_io_mgr.get(), reader, data, Status::OK(), std::placeholders::_1);
                *new_range =
                        _pool->add(new DiskIoMgr::WriteRange(tmp_file, cur_offset, num_ranges % num_disks, callback));
            ...
            {
                std::unique_lock<std::mutex> lock(_written_mutex);
...
TEST_F(DiskIoMgrTest, InvalidWrite) {
    ...
    *data = rand();
    ...
    DiskIoMgr::WriteRange** new_range = _pool->add(new DiskIoMgr::WriteRange*);
    DiskIoMgr::WriteRange::WriteDoneCallback callback = std::bind(
            std::mem_fn(&DiskIoMgrTest::write_validate_callback), this, 2, new_range, (DiskIoMgr*)NULL,
            (DiskIoMgr::RequestContext*)NULL, data, Status::InternalError("Test Failure"), std::placeholders::_1);
    *new_range = _pool->add(new DiskIoMgr::WriteRange(tmp_file, rand(), 0, callback));
    ...
    callback = std::bind(std::mem_fn(&DiskIoMgrTest::write_validate_callback), this, 2, new_range, (DiskIoMgr*)NULL,
                         (DiskIoMgr::RequestContext*)NULL, data, Status::InternalError("Test Failure"),
                         std::placeholders::_1);
        ...
        for (int num_disks = 1; num_disks <= 5; num_disks += 2) {
            ...
            for (int i = 0; i < num_ranges; ++i) {
                ...
                *data = rand();
                DiskIoMgr::WriteRange** new_range = _pool->add(new DiskIoMgr::WriteRange*);
                DiskIoMgr::WriteRange::WriteDoneCallback callback = std::bind(
                        std::mem_fn(&DiskIoMgrTest::write_validate_callback), this, num_ranges_before_cancel, new_range,
                        read_io_mgr.get(), reader, data, Status::Cancelled(""), std::placeholders::_1);
                *new_range =
                        _pool->add(new DiskIoMgr::WriteRange(tmp_file, cur_offset, num_ranges % num_disks, callback));
                (*new_range)->set_data(reinterpret_cast<uint8_t*>(data), sizeof(int32_t));
                cur_offset += sizeof(int32_t);
            ...
            for (int num_buffers = 1; num_buffers <= 5; ++num_buffers) {
                for (int num_read_threads = 1; num_read_threads <= 5; ++num_read_threads) {
                    ...
                    for (int i = 0; i < len; ++i) {
                        ...
                        ranges.push_back(init_range(num_buffers, tmp_file, 0, len, disk_id, stat_val.st_mtime));
                    }
                    status = io_mgr.add_scan_ranges(reader, ranges);
                    ASSERT_TRUE(status.ok());

                    std::atomic<int> num_ranges_processed{0};
                    thread_group threads;
                    for (int i = 0; i < num_read_threads; ++i) {
                        threads.add_thread(new thread(scan_range_thread, &io_mgr, reader, data, len, Status::OK(), 0,
                                                      &num_ranges_processed));
                    }
                    threads.join_all();
    ...
    for (int num_threads_per_disk = 1; num_threads_per_disk <= 5; ++num_threads_per_disk) {
        for (int num_disks = 1; num_disks <= 5; num_disks += 2) {
            for (int num_buffers = 1; num_buffers <= 5; ++num_buffers) {
                ...
                for (int i = 0; i < len; ++i) {
                    ...
                    if (i > len / 2) {
                        ranges_second_half.push_back(
                                init_range(num_buffers, tmp_file, i, 1, disk_id, stat_val.st_mtime));
                    } else {
                ...
                scan_range_thread(&io_mgr, reader, data, strlen(data), Status::OK(), 2, &num_ranges_processed);
                ...
                thread_group threads;
                for (int i = 0; i < 3; ++i) {
                    threads.add_thread(new thread(scan_range_thread, &io_mgr, reader, data, strlen(data),
    ...
    for (int num_threads_per_disk = 1; num_threads_per_disk <= 5; ++num_threads_per_disk) {
        for (int num_disks = 1; num_disks <= 5; num_disks += 2) {
            for (int num_buffers = 1; num_buffers <= 5; ++num_buffers) {
                ...
                DiskIoMgr::ScanRange* complete_range = init_range(1, tmp_file, 0, strlen(data), 0, stat_val.st_mtime);
                ...
                thread_group threads;
                for (int i = 0; i < 5; ++i) {
                    threads.add_thread(new thread(scan_range_thread, &io_mgr, reader, data, strlen(data), Status::OK(),
                                                  0, &num_ranges_processed));
                }
        ...
        for (int num_disks = 1; num_disks <= 5; num_disks += 2) {
            for (int num_buffers = 1; num_buffers <= 5; ++num_buffers) {
                ...
                for (int i = 0; i < num_succesful_ranges; ++i) {
                    scan_range_thread(&io_mgr, reader, data, strlen(data), Status::OK(), 1, &num_ranges_processed);
                }
...
TEST_F(DiskIoMgrTest, MemTrackers) {
    ...
    int64_t iters = 0;
    {
        ...
        std::atomic<int> num_ranges_processed{0};
        scan_range_thread(&io_mgr, reader, data, strlen(data), Status::MemoryLimitExceeded("Mem"), 1,
                          &num_ranges_processed);
                ...
                while (read_offset < file_size) {
                    for (int context_index = 0; context_index < num_contexts; ++context_index) {
                        ...
                        for (int i = 0; i < num_scan_ranges; ++i) {
                            ranges.push_back(
                                    init_range(1, file_name.c_str(), read_offset, 1, i % num_disks, stat_val.st_mtime));
                            threads.add_thread(
                                    new thread(scan_range_thread, &io_mgr, contexts[context_index],
                        ...
                        for (int i = 0; i < num_write_ranges; ++i) {
                            DiskIoMgr::WriteRange::WriteDoneCallback callback =
                                    std::bind(std::mem_fn(&DiskIoMgrTest::write_complete_callback), this,
                                              num_write_ranges, std::placeholders::_1);
                            DiskIoMgr::WriteRange* new_range = _pool->add(
                                    new DiskIoMgr::WriteRange(file_name, write_offset, i % num_disks, callback));
                            new_range->set_data(reinterpret_cast<const uint8_t*>(data + (write_offset % strlen(data))),
                                                1);
                ...
                for (int num_buffers = 1; num_buffers <= 5; ++num_buffers) {
                    ...
                    for (int i = 0; i < NUM_READERS; ++i) {
                        ...
                        for (int j = 0; j < DATA_LEN; ++j) {
                            ...
                            ranges.push_back(init_range(num_buffers, file_names[i].c_str(), j, 1, disk_id, mtimes[i]));
                        }
                    ...
                    for (int i = 0; i < NUM_READERS; ++i) {
                        for (int j = 0; j < NUM_THREADS_PER_READER; ++j) {
                            threads.add_thread(new thread(scan_range_thread, &io_mgr, readers[i], data[i].c_str(),
                                                          data[i].size(), Status::OK(), 0, &num_ranges_processed));
                        }
                    }
                    threads.join_all();
