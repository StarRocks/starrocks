...
TEST(AIProjectProcessorTest, TypedResponseValidationRespectsOnErrorOnTheSourceDriver) {
    for (const std::string on_error : {"ignore", "fail"}) {
        ...
        ASSERT_NE(nullptr, processor);
        put_and_finish(buffer, make_input_chunk(0, 2));
        QueryRuntimeState query_state;
        RuntimeState state;
        state.set_query_runtime_state(&query_state);
        ASSERT_OK(processor->try_process(&state, 0));
        ...
        EXPECT_EQ(0, processor->statistics(0).error_count);
        EXPECT_TRUE(query_state.ai_statistics().empty());
        auto result = processor->pull_chunk(&state, 0);
        if (on_error == "ignore") {
            ASSERT_TRUE(result.ok()) << result.status();
            const auto& nullable = ai_column(result.value());
        ...
        EXPECT_EQ(1, processor->statistics(0).error_count);
        EXPECT_EQ(2, processor->statistics(0).task_count);
        EXPECT_EQ(2, processor->statistics(0).request_count);
        EXPECT_EQ(1, query_state.ai_statistics().error_count);
        auto delta = query_state.consume_delta_ai_statistics();
        EXPECT_EQ(1, delta.error_count);
        delta.error_count = 0;
        EXPECT_TRUE(delta.empty()) << "Materialization must publish only additional row errors";
...
TEST(AIProjectProcessorTest, TypedFailureStatisticsSurviveLaterMaterializationFailure) {
    auto buffer = make_input_buffer();
    auto projection = std::make_shared<RecordingProjection>();
    projection->result_type = TypeDescriptor(TYPE_JSON);
    projection->result_kind = AIFunctionResultKind::JSON;
    ...
    state.set_query_runtime_state(&query_state);
    ASSERT_OK(processor->try_process(&state, 0));
    submitter->succeed_value("prompt-0", std::string("not json"));
    std::string oversized_json = "{\"payload\":\"";
    oversized_json.append(kJSONLengthLimit, 'x');
    oversized_json.append("\"}");
    submitter->succeed_value("prompt-1", std::move(oversized_json));
    ...
    EXPECT_EQ(1, processor->statistics(0).error_count);
    EXPECT_EQ(1, query_state.ai_statistics().error_count);
    EXPECT_EQ(1, query_state.consume_delta_ai_statistics().error_count);
    EXPECT_FALSE(processor->pull_chunk(&state, 0).ok());
    EXPECT_TRUE(query_state.consume_delta_ai_statistics().empty());
    ASSERT_OK(processor->set_source_finished(0));
}

TEST(AIProjectProcessorTest, TypedFailureStatisticsStayLaneLocalAndAggregateAtQueryLevel) {
    auto buffer = make_input_buffer();
    auto projection = std::make_shared<RecordingProjection>();
    projection->result_type = TypeDescriptor(TYPE_BOOLEAN);
    projection->result_kind = AIFunctionResultKind::BOOLEAN;
    ...
    ASSERT_NE(nullptr, processor);
    put_and_finish(buffer, make_input_chunk(0, 2), 0);
    ...
    for (const auto* prompt : {"prompt-0", "prompt-1", "prompt-100"}) {
        submitter->succeed_value(prompt, std::string("not a boolean"));
    }
    auto first = processor->pull_chunk(&state, 0);
    ASSERT_TRUE(first.ok()) << first.status();
    EXPECT_EQ(2, processor->statistics(0).error_count);
    EXPECT_EQ(0, processor->statistics(1).error_count);
    EXPECT_EQ(2, query_state.consume_delta_ai_statistics().error_count);
    auto second = processor->pull_chunk(&state, 1);
    ASSERT_TRUE(second.ok()) << second.status();
    EXPECT_EQ(2, processor->statistics(0).error_count);
    EXPECT_EQ(1, processor->statistics(1).error_count);
    EXPECT_EQ(1, query_state.consume_delta_ai_statistics().error_count);
    EXPECT_EQ(3, query_state.ai_statistics().error_count);
}

TEST(AIProjectProcessorTest, TypedFailureAccountingDoesNotRecountDispatcherFailuresOrNullInputs) {
    auto buffer = make_input_buffer();
    auto projection = std::make_shared<RecordingProjection>();
    projection->result_type = TypeDescriptor(TYPE_BOOLEAN);
    projection->result_kind = AIFunctionResultKind::BOOLEAN;
    projection->row_actions = {AIFunctionRowAction::DISPATCH, AIFunctionRowAction::DISPATCH,
                               AIFunctionRowAction::SQL_NULL};
    auto submitter = std::make_shared<ManualTaskSubmitter>();
    auto processor = make_processor(buffer, projection, submitter);
    ASSERT_NE(nullptr, processor);
    ...
    EXPECT_EQ(3, ai_column(output.value()).null_count());
    EXPECT_EQ(2, processor->statistics(0).error_count);
    EXPECT_EQ(2, processor->statistics(0).task_count);
    EXPECT_EQ(2, processor->statistics(0).request_count);
    EXPECT_EQ(1, query_state.ai_statistics().error_count);
...
TEST(AIProjectProcessorTest, SourceFinishDoesNotValidateDiscardedTypedResponses) {
    auto buffer = make_input_buffer();
    auto projection = std::make_shared<RecordingProjection>();
    projection->result_type = TypeDescriptor(TYPE_BOOLEAN);
    projection->result_kind = AIFunctionResultKind::BOOLEAN;
    ...
    ASSERT_OK(processor->try_process(&state, 0));
    ASSERT_OK(processor->set_source_finished(0));
    submitter->succeed_value("prompt-0", std::string("not a boolean"));
    auto output = processor->pull_chunk(&state, 0);
    ASSERT_TRUE(output.ok()) << output.status();
    EXPECT_EQ(nullptr, output.value());
