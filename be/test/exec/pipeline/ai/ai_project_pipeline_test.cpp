...
protected:
    ...
    StatusOr<std::shared_ptr<AIProjectExpressionProjection>> create_expression_projection(
        ...
        return AIProjectExpressionProjection::create(
                {
                        AIProjectExpressionOutput{
                ...
                {kCommonSlotId}, {common}, std::move(default_model));
    }
...
TEST_F(AIProjectPipelineTest, SplitsPipelinesAndPreservesDopSkewBucketAndLimitOrder) {
    ...
    auto* ai_sink = dynamic_cast<AIBufferSinkOperatorFactory*>(upstream_pipeline->_op_factories.back().get());
...
TEST_F(AIProjectPipelineTest, SourcePrepareUsesFinalCreateDopAndIsIdempotentAcrossDrivers) {
