...
protected:
    ...
    TPlanNode make_mixed_route_node() const {
        ...
        provider_chat.__set_provider_id("00000000-0000-0000-0000-000000000101");
        ...
        embedding.__set_provider_id("00000000-0000-0000-0000-000000000102");
...
TEST_F(AIProjectNodeProviderRoutesTest, RejectsInvalidModelRouteWireContracts) {
    ...
    const std::vector<Case> cases = {
            ...
            {"reserved resource source",
             [](auto& project) {
                 project.ai_model_configs.at("provider:chat").__set_source(static_cast<TAIModelSource::type>(1));
             }},
            {"unknown source",
             [](auto& project) {
            ...
            {"provider id missing",
             [](auto& project) { project.ai_model_configs.at("provider:chat").__isset.provider_id = false; }},
            {"empty provider id",
             [](auto& project) { project.ai_model_configs.at("provider:chat").__set_provider_id(""); }},
            {"malformed provider id",
             [](auto& project) { project.ai_model_configs.at("provider:chat").__set_provider_id("not-a-uuid"); }},
            {"duplicate provider id",
             [](auto& project) {
                 project.ai_model_configs.at("provider:embedding")
                         .__set_provider_id("00000000-0000-0000-0000-000000000101");
             }},
            {"system provider id",
             [](auto& project) { project.ai_model_configs.at("__system_chat__").__set_provider_id(""); }},
            ...
            {"nonpositive embedding dimensions",
             [](auto& project) {
                 project.ai_model_configs.at("provider:embedding").text_embedding.__set_dimensions(0);
             }},
            {"chat dimensions",
...
TEST_F(AIProjectPipelineTest, ProviderCredentialsComeFromSnapshotAndNeverFallBackToSystem) {
    ...
    AIProjectModelConfigs configs{{"provider:embedding", AIProjectModelConfig{
                                                                 ...
                                                                 .provider_id = "00000000-0000-0000-0000-000000000102",
                                                                 .timeout_ms = 75,
                                                                 .dimensions = 3,
                                                         }}};
    ...
    configs.at("provider:embedding").api_key.clear();
    for (const std::string id : {"", "102", "00000000-0000-0000-0000-00000000010z"}) {
        configs.at("provider:embedding").provider_id = id;
        EXPECT_FALSE(
                AIProjectDispatcherSubmitter::create(_runtime_state.get(), configs, _config_source->snapshot()).ok());
    }
    configs.at("provider:embedding").provider_id = "00000000-0000-0000-0000-000000000102";
    for (const auto source : {TAIModelSource::SYSTEM, static_cast<TAIModelSource::type>(1),
                              static_cast<TAIModelSource::type>(2), static_cast<TAIModelSource::type>(99)}) {
    ...
    EXPECT_FALSE(
            AIProjectDispatcherSubmitter::create(_runtime_state.get(), duplicate, _config_source->snapshot()).ok());
...
TEST_F(AIProjectPipelineTest, SystemEmbeddingUsesItsOwnEndpointAndCredential) {
    ...
    configs.at("__system_text_embedding__").provider_id = "00000000-0000-0000-0000-000000000102";
    EXPECT_FALSE(AIProjectDispatcherSubmitter::create(_runtime_state.get(), configs, _config_source->snapshot()).ok());
    configs.at("__system_text_embedding__").provider_id.clear();
    configs.at("__system_text_embedding__").api_key = "embed-provider-secret";
    EXPECT_FALSE(AIProjectDispatcherSubmitter::create(_runtime_state.get(), configs, _config_source->snapshot()).ok());
}
...
TEST_F(AIProjectPipelineTest, ProjectionRejectsModelSourceMismatch) {
    ...
    configs.at(std::string(kSystemChatConfigId)).provider_id = "00000000-0000-0000-0000-000000000101";
    AIProjectProjectionSpec spec(_runtime_state.get(),
                                 {AIProjectOutputSpec{.slot_id = kAnswerSlotId,
                                                      .expr_ctx = ai.value(),
                                                      .nullable = true,
