...
public:
    void SetUp() override {
        ...
        _fragment_ctx = _query_ctx->fragment_mgr()->get_or_register(_fragment_id);
...
protected:
    TPlanNode make_analytic_plan_node(bool force_merge_sort, bool is_skewed) const {
        ...
        tnode.analytic_node.__set_is_skewed(is_skewed);
        tnode.analytic_node.__set_force_merge_sort(force_merge_sort);
        return tnode;
    }

    pipeline::Pipelines decompose_analytic(bool force_merge_sort, bool is_skewed,
        ...
        TPlanNode analytic_tnode = make_analytic_plan_node(force_merge_sort, is_skewed);
        ...
        pipeline::PipelineBuilderContext context(_fragment_ctx, kPipelineDop, kPipelineDop);
...
TEST_F(AnalyticNodePipelineTest, ForceMergeSortUsesOrderedPartitionExchange) {
    pipeline::OpFactories analytic_source_ops;
    auto pipelines = decompose_analytic(true, false, &analytic_source_ops);
...
TEST_F(AnalyticNodePipelineTest, NonSkewedAnalyticDoesNotUseOrderedPartitionExchangeWithoutForce) {
    pipeline::OpFactories analytic_source_ops;
    auto pipelines = decompose_analytic(false, false, &analytic_source_ops);
...
TEST_F(AnalyticNodePipelineTest, SkewedAnalyticStillUsesOrderedPartitionExchangeWithoutForce) {
    pipeline::OpFactories analytic_source_ops;
    auto pipelines = decompose_analytic(false, true, &analytic_source_ops);
