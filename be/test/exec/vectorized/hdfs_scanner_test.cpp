...
protected:
    ...
    THdfsScanRange* _create_scan_range(const std::string& file, uint64_t offset, uint64_t length,
                                       bool trim_space = false, int64_t skip_header = 0);
...
THdfsScanRange* HdfsScannerTest::_create_scan_range(const std::string& file, uint64_t offset, uint64_t length,
                                                    bool trim_space, int64_t skip_header) {
    ...
    if (skip_header != 0) {
        scan_range->__set_skip_header(skip_header);
    }
...
TEST_F(HdfsScannerTest, TestCSVSkipHeader) {
    ...
    Status status;
    auto* range = _create_scan_range(skip_file, 0, 0, false, 3);
    auto* tuple_desc = _create_tuple_desc(csv_descs);
    auto* param = _create_param(skip_file, range, tuple_desc);
