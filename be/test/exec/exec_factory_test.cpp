...
TEST_F(ExecFactoryTest, test_stream_scan_not_supported) {
    ...
    ASSERT_NE(st.message().find("legacy stream scan nodes are not supported"), std::string::npos);
}

TEST_F(ExecFactoryTest, test_stream_agg_not_supported) {
    ...
    ASSERT_ERROR(st);
    ASSERT_TRUE(st.is_not_supported());
    ASSERT_NE(st.message().find("legacy stream aggregate nodes are not supported"), std::string::npos);
}
