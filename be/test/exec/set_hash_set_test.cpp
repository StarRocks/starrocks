...
public:
    void SetUp() override {
        ...
        _runtime_state = std::make_shared<RuntimeState>(fragment_id, query_options, query_globals, exec_env);
        ...
        ASSERT_OK(Expr::prepare(_exprs, _runtime_state.get()));
        ASSERT_OK(Expr::open(_exprs, _runtime_state.get()));
    }

    void TearDown() override { Expr::close(_exprs, _runtime_state.get()); }
