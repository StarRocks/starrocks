...
// Three ranges on a single INT column fully covering (-inf, +inf):
//   R0: (-inf, 10]
//   R1: (10, 20]
//   R2: (20, +inf)
// NOLINTNEXTLINE
TEST_F(RangeRouterTest, IntThreeRangesFullCoverage) {
    ...
    ranges.emplace_back(make_single_long_range(20, false, std::nullopt, false)); // (20, +inf)
    ...
    std::vector<int64_t> expected = {100, 100, 100, 100, 200, 200, 200, 300, 300};
...
TEST_F(RangeRouterTest, ManyRangesBinarySearchFullCoverage) {
    ...
    // R0: (-inf, 0]
    ranges.emplace_back(make_single_long_range(std::nullopt, false, 0, true));
    tablet_ids.emplace_back(100);
    // R1..R7: (k*10, (k+1)*10] for k = 0..6 -> values (0,10], ..., (60,70]
    for (int k = 1; k <= 7; ++k) {
        ranges.emplace_back(make_single_long_range((k - 1) * 10, false, k * 10, true));
        tablet_ids.emplace_back(100 + k);
    }
    // R8: (70, +inf)
    ranges.emplace_back(make_single_long_range(70, false, std::nullopt, false));
...
TEST_F(RangeRouterTest, MultiColumnTwoRangesFullCoverage) {
    ...
    std::vector<int64_t> expected = {100, 100, 100, 200, 200};
    ASSERT_EQ(expected.size(), routed_ids.size());
    for (int i = 0; i < static_cast<int>(expected.size()); ++i) {
...
// VARCHAR ranges with three ranges fully covering (-inf, +inf):
//   R0: (-inf, "banana"]
//   R1: ("banana", "date"]
//   R2: ("date", +inf)
// NOLINTNEXTLINE
TEST_F(RangeRouterTest, VarcharThreeRangesFullCoverage) {
    std::vector<TTabletRange> ranges;
    ranges.emplace_back(make_single_string_range(std::nullopt, false, std::string("banana"), true)); // (-inf, "banana"]
    ranges.emplace_back(
            make_single_string_range(std::string("banana"), false, std::string("date"), true));     // ("banana","date"]
    ranges.emplace_back(make_single_string_range(std::string("date"), false, std::nullopt, false)); // ("date", +inf)
    ...
    // "cherry","date"    -> R1 (20)
    // "fig"              -> R2 (30)
    std::vector<int64_t> expected = {10, 10, 20, 20, 30};
    ASSERT_EQ(expected.size(), routed_ids.size());
    for (int i = 0; i < static_cast<int>(expected.size()); ++i) {
        ASSERT_EQ(expected[i], routed_ids[i]) << "row " << i << " value " << values[i] << " mismatch";
    }
...
TEST_F(RangeRouterTest, ValidateRangeAdjacentInclusiveExclusiveOverlap) {
    // Case 1: both-inclusive at boundary 10: (-inf,10] and [10,+inf)
    {
        ...
        auto st = router.init(ranges, 1);
        ASSERT_FALSE(st.ok());
        assert_status_message_contains(st, "adjacent ranges are overlapping for the inclusive/exclusive bound");
    }
...
TEST_F(RangeRouterTest, ValidateRangeEndpointsNotEqual) {
    ...
    // R0: (-inf,10]
    TTabletRange r0 = make_single_long_range(std::nullopt, false, 10, true);
    // R1: (20,+inf) -> lower bound 20, which does not equal the previous upper bound 10.
    TTabletRange r1 = make_single_long_range(20, false, std::nullopt, false);
