...
>>>>>>> b0f8291e71 ([BugFix] Fix connector sink mem tracker release on poller threads (#70121))
    ...
    TEST_F(ConnectorSinkOperatorTest, need_input_releases_flush_memory_under_instance_tracker) {
        ...
        auto sink_mem_mgr =
                std::make_shared<connector::SinkMemoryManager>(_query_pool_tracker.get(), _query_tracker.get());
    ...
    TEST_F(ConnectorSinkOperatorTest, is_finished_releases_polled_stream_under_instance_tracker) {
        ...
        auto stream = [&]() {
            ...
            return std::make_shared<Stream>(
                    std::make_unique<ReleaseOnDestructWritableFile>("stream.out", kTrackedBytes), nullptr,
                    _runtime_state);
        }();
        ASSERT_EQ(_query_pool_tracker->consumption(), kTrackedBytes);
        ASSERT_EQ(_query_tracker->consumption(), kTrackedBytes);
