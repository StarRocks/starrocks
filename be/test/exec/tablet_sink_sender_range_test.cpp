...
class TestTabletSinkSender : public TabletSinkSender {
public:
    TestTabletSinkSender(PUniqueId load_id, int64_t txn_id, IndexIdToTabletBEMap index_id_to_tablet_be_map,
            ...
            : TabletSinkSender(std::move(load_id), txn_id, index_id_to_tablet_be_map, partition_params,
...
protected:
    TTabletRange _make_range(int64_t lower, bool lower_included, int64_t upper, bool upper_included) {
        ...
        lower_var.__set_long_value(lower);
        ...
        upper_var.__set_long_value(upper);
    ...
    OlapTablePartitionParam* _create_partition_param(const std::vector<std::vector<TTabletRange>>& index_ranges) {
        ...
        tpartition_param.__set_range_distribution_columns({"c1"});
...
TEST_F(TabletSinkSenderRangeTest, NormalRouting) {
    ...
    TestTabletSinkSender sender(load_id, 1, index_id_to_tablet_be_map, partition_param, channels, node_channels,
    ...
    Status st = sender.send_chunk_by_range(_schema_param, partitions, validate_select_idx, index_id_partition_id,
                                           chunk.get());
    ASSERT_OK(st);
...
TEST_F(TabletSinkSenderRangeTest, MultiIndexRouting) {
    ...
    // Index 1 (100): [0, 10) -> 10000, [10, 20) -> 10001
    // Index 2 (101): [0, 5) -> 10100, [5, 20) -> 10101
    std::vector<TTabletRange> ranges1;
    ranges1.push_back(_make_range(0, true, 10, false));
    ranges1.push_back(_make_range(10, true, 20, false));

    std::vector<TTabletRange> ranges2;
    ranges2.push_back(_make_range(0, true, 5, false));
    ranges2.push_back(_make_range(5, true, 20, false));
...
TEST_F(TabletSinkSenderRangeTest, GapRangeWithPartialMatchAndError) {
    ...
    // Tablet 1: [0, 5)  -> 10000
    // Tablet 2: [10, 20) -> 10001
    // Gap: [5, 10) has no tablet range, rows falling into this interval should trigger no-match error.
    std::vector<TTabletRange> ranges;
    ...
    // 2 and 12 have matching ranges; 7 falls into the gap [5,10) and should cause a routing error.
    ...
    Status st = sender.send_chunk_by_range(_schema_param, partitions, validate_select_idx, index_id_partition_id,
                                           chunk.get());
    ASSERT_FALSE(st.ok());
    ASSERT_TRUE(st.message().find("Failed to match range") != std::string::npos);
}
...
TEST_F(TabletSinkSenderRangeTest, MultiColumnRangeRouting) {
    ...
    // Tablet 10000: range [(2,10), (3,60)] (inclusive)
    // Tablet 10001: range [(4,10), (4,50)] (inclusive)
    TOlapTablePartitionParam tpartition_param;
    tpartition_param.db_id = _db_id;
    tpartition_param.table_id = _table_id;
    tpartition_param.version = 0;
    tpartition_param.__set_range_distribution_columns({"c1", "c2"});
    ...
    auto make_two_bigint_tuple = [](int64_t v1, int64_t v2) {
        ...
        var1.__set_long_value(v1);
        TVariant var2;
        var2.__set_type(TYPE_BIGINT_DESC.to_thrift());
        var2.__set_long_value(v2);
        TTuple t;
        t.__set_values(std::vector<TVariant>{var1, var2});
        return t;
    };

    // Tablet 10000: [(2,10), (3,60)]
    {
        int64_t tablet_id = 10000;
        index_tablets.tablet_ids.push_back(tablet_id);

        TTabletRange range;
        range.__set_lower_bound(make_two_bigint_tuple(2, 10));
        range.__set_upper_bound(make_two_bigint_tuple(3, 60));
    ...
    // Tablet 10001: [(4,10), (4,50)]
    {
        int64_t tablet_id = 10001;
        index_tablets.tablet_ids.push_back(tablet_id);

        TTabletRange range;
        range.__set_lower_bound(make_two_bigint_tuple(4, 10));
        range.__set_upper_bound(make_two_bigint_tuple(4, 50));
        range.__set_lower_bound_included(true);
        range.__set_upper_bound_included(true);
...
TEST_F(TabletSinkSenderRangeTest, MultiPartitionRoutingSingleIndex) {
    ...
    // Build partition param with two partitions:
    //   Partition 1000: [0, 10)  -> tablet 10000
    //   Partition 1001: [10, 20) -> tablet 10001
    TOlapTablePartitionParam tpartition_param;
    tpartition_param.db_id = _db_id;
    tpartition_param.table_id = _table_id;
    tpartition_param.version = 0;
    ...
    // Partition 1000
    {
        ...
        TOlapTableTablet tablet;
        tablet.id = tablet_id;
        tablet.__set_range(_make_range(0, true, 10, false));
        index_tablets.tablets.push_back(tablet);
    ...
    // Partition 1001
    {
        ...
        TOlapTableTablet tablet;
        tablet.id = tablet_id;
        tablet.__set_range(_make_range(10, true, 20, false));
        index_tablets.tablets.push_back(tablet);
...
TEST_F(TabletSinkSenderRangeTest, OverlappingRangesRoutingOrder) {
    ...
    // Realistic overlap scenario with point range at boundary:
    // Tablet 1: (0, 10)   -> 10000
    // Tablet 2: [10, 10]  -> 10001 (point range for boundary value)
    // Tablet 3: (10, 20]  -> 10002
    std::vector<TTabletRange> ranges;
    ranges.push_back(_make_range(0, false, 10, false));  // (0, 10)
    ranges.push_back(_make_range(10, true, 10, true));   // [10, 10]
    ranges.push_back(_make_range(10, false, 20, true));  // (10, 20]
...
TEST_F(TabletSinkSenderRangeTest, MultiBENonReplicatedRouting) {
    ...
    // Tablet 1: [0, 10)  -> 10000, replicas on BE 101, 102
    // Tablet 2: [10, 20) -> 10001, replica on BE 102
    std::vector<TTabletRange> ranges;
    ranges.push_back(_make_range(0, true, 10, false));
    ranges.push_back(_make_range(10, true, 20, false));
...
TEST_F(TabletSinkSenderRangeTest, MultiBEReplicatedPrimaryOnlyRouting) {
    ...
    // Tablet 1: [0, 10)  -> 10000, replicas on BE 201(primary), 202
    // Tablet 2: [10, 20) -> 10001, replicas on BE 202(primary), 203
    std::vector<TTabletRange> ranges;
    ranges.push_back(_make_range(0, true, 10, false));
    ranges.push_back(_make_range(10, true, 20, false));
...
TEST_F(TabletSinkSenderRangeTest, SparseDataRouting) {
    ...
    // Range [0, 100) -> 10000
    std::vector<TTabletRange> ranges;
    ranges.push_back(_make_range(0, true, 100, false));
...
TEST_F(TabletSinkSenderRangeTest, EmptyInput) {
    ...
    Status st =
            sender.send_chunk_by_range(_schema_param, partitions, validate_select_idx, index_id_partition_id, &chunk);
    ASSERT_OK(st);
