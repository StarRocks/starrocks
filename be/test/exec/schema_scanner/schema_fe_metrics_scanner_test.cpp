...
public:
    void start_mock(const std::string& json, bool include_json = true) {
        ...
        // the ExecEnv installed by setup() (done at BE startup in a real process). Point the
        // process-wide ExecEnv's frontend client cache at a local cache so the RPC works in the
        // unit test, then restore it in TearDown() so other tests aren't affected. The test
        // build is compiled with -fno-access-control, so the private member is reachable here.
