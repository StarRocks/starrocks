...
static void clear_sort_exprs(std::vector<ExprContext*>& exprs, RuntimeState* state = nullptr) {
    // Explicitly close each context before deletion to ensure proper cleanup order.
    // The close() call is idempotent, so it's safe to call even if already closed.
    if (state != nullptr) {
...
TEST_F(ChunksSorterTest, full_sort_incremental) {
    ...
    std::vector<int32_t> permutation{70, 69, 71, 2, 4, 6, 12, 16, 24, 41, 49, 52, 54, 55, 56, 58};
...
TEST_F(ChunksSorterTest, full_sort_by_2_columns_null_last) {
    ...
    ASSERT_EQ(16, page_1->num_rows());
    const size_t Size = 16;
    std::vector<int32_t> permutation{58, 56, 55, 54, 52, 49, 41, 24, 16, 12, 6, 4, 2, 71, 69, 70};
    std::vector<int32_t> result;
    for (size_t i = 0; i < Size; ++i) {
        result.push_back(page_1->get(i).get(0).get_int32());
    }
...
TEST_F(ChunksSorterTest, full_sort_by_3_columns) {
    ...
    ASSERT_EQ(16, page_1->num_rows());
    const size_t Size = 16;
    std::vector<int32_t> permutation{70, 69, 71, 54, 4, 56, 55, 49, 41, 16, 52, 58, 24, 12, 2, 6};
    std::vector<int32_t> result;
    for (size_t i = 0; i < Size; ++i) {
        result.push_back(page_1->get(i).get(0).get_int32());
    }
...
TEST_F(ChunksSorterTest, full_sort_by_4_columns) {
    ...
    ASSERT_EQ(16, page_1->num_rows());
    const size_t Size = 16;
    std::vector<int32_t> permutation{24, 55, 4, 58, 12, 52, 41, 56, 49, 16, 6, 2, 54, 70, 69, 71};
    std::vector<int32_t> result;
    for (size_t i = 0; i < Size; ++i) {
        result.push_back(page_1->get(i).get(0).get_int32());
    }
