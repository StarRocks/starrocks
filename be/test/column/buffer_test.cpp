...
// Test allocated_bytes on empty sentinel
...
// Test pointer-range copy for non-trivial relocatable type
...
TEST_F(BufferTest, BufferFrontBack) {
    Buffer<int, 16> buf(&_allocator);
    buf.push_back(10);
    buf.push_back(20);
    buf.push_back(30);
    ...
    // const accessors
    const auto& cbuf = buf;
    ASSERT_EQ(100, cbuf.front());
    ASSERT_EQ(300, cbuf.back());
}

// Test data() accessor and empty sentinel behavior
TEST_F(BufferTest, BufferDataAndEmptySentinel) {
    Buffer<int, 16> buf(&_allocator);

    // Empty buffer should have non-null data() (points to empty sentinel)
    ...
    // const data()
    const auto& cbuf = buf;
    const int* cptr = cbuf.data();
    ASSERT_EQ(42, *cptr);
}

// Test used_bytes()
TEST_F(BufferTest, BufferUsedBytes) {
    Buffer<int, 16> buf(&_allocator);
    ASSERT_EQ(0, buf.used_bytes());

    buf.push_back(1);
    ASSERT_EQ(sizeof(int), buf.used_bytes());

    buf.push_back(2);
    buf.push_back(3);
    ASSERT_EQ(3 * sizeof(int), buf.used_bytes());

    buf.pop_back();
    ASSERT_EQ(2 * sizeof(int), buf.used_bytes());

    buf.clear();
    ASSERT_EQ(0, buf.used_bytes());
}

// Test Buffer(allocator, count) and Buffer(allocator, count, value) constructors
TEST_F(BufferTest, BufferCountConstructor) {
...
TEST_F(BufferTest, BufferResizeValueTrivial) {
    Buffer<int, 16> buf(&_allocator);
    ...
    // Grow further - existing elements preserved, new ones filled
    buf.resize(8, 77);
...
// Test iterator usage with range-for
TEST_F(BufferTest, BufferIterators) {
    Buffer<int, 16> buf(&_allocator);
    for (int i = 0; i < 5; ++i) {
        buf.push_back(i * 10);
    }
    ...
    expected = 0;
    const auto& cbuf = buf;
    for (const auto& val : cbuf) {
        ASSERT_EQ(expected, val);
        expected += 10;
    }

    // begin/end pointer arithmetic
    ASSERT_EQ(5, buf.end() - buf.begin());
    ASSERT_EQ(buf.data(), buf.begin());
}

// Test large buffer to exercise realloc path (>4096 bytes for relocatable types)
TEST_F(BufferTest, BufferLargeRealloc) {
    Buffer<int, 16> buf(&_allocator);
    // Fill enough to exceed 4096 bytes (4096 / 4 = 1024 ints)
    constexpr int count = 2000;
    for (int i = 0; i < count; ++i) {
        buf.push_back(i);
    }
    ASSERT_EQ(count, buf.size());
    ASSERT_GT(buf.allocated_bytes(), 4096);
    ...
    // Reserve even larger to trigger realloc on already-large buffer
    buf.reserve(count * 4);
    ASSERT_GE(buf.capacity(), count * 4);
    for (int i = 0; i < count; ++i) {
        ASSERT_EQ(i, buf[i]);
    }
}

// Test non-relocatable non-trivial type reallocation path
TEST_F(BufferTest, BufferNonRelocatableRealloc) {
    CountingType::reset();
    Buffer<CountingType, 16> buf(&_allocator);

    // Fill enough elements to trigger multiple reallocations
    for (int i = 0; i < 20; ++i) {
        buf.emplace_back(i);
    }
    ASSERT_EQ(20, buf.size());
    for (int i = 0; i < 20; ++i) {
        ASSERT_EQ(i, buf[i].value);
    }

    // Reserve to trigger non-relocatable relocate path
    size_t old_cap = buf.capacity();
    buf.reserve(old_cap * 4);
    ASSERT_GE(buf.capacity(), old_cap * 4);

    // All values must be intact after relocation
    for (int i = 0; i < 20; ++i) {
        ASSERT_EQ(i, buf[i].value);
    }
}

// Test append with non-pointer forward iterators (std::vector::iterator)
TEST_F(BufferTest, BufferAppendNonPointerIterator) {
    Buffer<int, 16> buf(&_allocator);
    buf.push_back(0);
...
// Test resize shrinking destroys non-trivial elements
TEST_F(BufferTest, BufferResizeShrinkDestroysElements) {
    CountingType::reset();
    Buffer<CountingType, 16> buf(&_allocator);
    ...
    int dtors_before = CountingType::dtor_count;
    buf.resize(3);
    ASSERT_EQ(3, buf.size());
    // 7 elements should have been destroyed
    ASSERT_EQ(dtors_before + 7, CountingType::dtor_count);
...
// Test assign overwrites existing non-trivial elements
TEST_F(BufferTest, BufferAssignOverwriteNonTrivial) {
    CountingType::reset();
    Buffer<CountingType, 16> buf(&_allocator);

    // First assign
    buf.assign(3, CountingType(5));
    ASSERT_EQ(3, buf.size());
    for (size_t i = 0; i < 3; ++i) {
        ASSERT_EQ(5, buf[i].value);
    }

    // Overwrite with larger assign
    int dtors_before = CountingType::dtor_count;
    buf.assign(5, CountingType(9));
    ASSERT_EQ(5, buf.size());
    // Old 3 elements should have been destroyed
    ASSERT_GE(CountingType::dtor_count, dtors_before + 3);
    for (size_t i = 0; i < 5; ++i) {
        ASSERT_EQ(9, buf[i].value);
    }
}

// Test clear on non-trivial types calls destructors
TEST_F(BufferTest, BufferClearNonTrivial) {
    CountingType::reset();
    Buffer<CountingType, 16> buf(&_allocator);
    ...
    ASSERT_EQ(0, buf.size());
    ASSERT_EQ(cap, buf.capacity());
    ASSERT_EQ(dtors_before + 5, CountingType::dtor_count);
}

// Test append exception safety for throwing types
TEST_F(BufferTest, BufferAppendExceptionSafety) {
...

    {
        ...
        // append should throw during copy of 2nd element
        ASSERT_THROW(buf.append(src, src + 3), std::runtime_error);

        // Buffer state: the append partially copied before throwing.
        // The buffer may have grown but original element should be preserved.
        // At minimum, the buffer should be usable afterwards.
        ThrowOnCopyType::throw_on_copy_at = -1;
        buf.emplace_back(99);
        ASSERT_EQ(99, buf.back().value);
    }
    ASSERT_EQ(0, ThrowOnCopyType::live_count);
}

// Test move constructor from buffer with allocator
TEST_F(BufferTest, BufferMoveConstructorPreservesAllocator) {
    Buffer<int, 16> buf1(&_allocator);
    buf1.push_back(1);
    buf1.push_back(2);
    ...
    Buffer<int, 16> buf2(std::move(buf1));
    ASSERT_EQ(&_allocator, buf2.allocator());
    ASSERT_EQ(nullptr, buf1.allocator());
    ASSERT_EQ(2, buf2.size());
    ASSERT_EQ(1, buf2[0]);
    ASSERT_EQ(2, buf2[1]);
    ASSERT_EQ(0, buf1.size());
}

// Test assign exception leaves buffer in reusable state
