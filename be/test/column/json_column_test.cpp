...
JsonColumn::Ptr make_plain_json(const std::vector<std::string>& jsons) {
...
std::vector<std::string> plain_dump(const JsonColumn::Ptr& column) {
    // Already a JsonColumn here, and unflatten() is const -- main needed the cast only because
    // its helper took a generic MutableColumnPtr.
    auto plain = column->unflatten();
