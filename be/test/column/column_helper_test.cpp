...
TEST_F(ColumnHelperTest, update_nested_has_null_struct) {
    ...
    auto* sc = down_cast<StructColumn*>(outer->data_column_mutable_ptr().get());
    auto* a_col = down_cast<NullableColumn*>(sc->fields_column()[0]->as_mutable_ptr().get());
    auto* b_col = down_cast<NullableColumn*>(sc->fields_column()[1]->as_mutable_ptr().get());
