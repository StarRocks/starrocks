...
void test_nullable_fixed_size_binary(const TestCaseArray<std::string>& test_cases) {
    ...
    for (auto& tc : test_cases) {
        ...
        auto value = std::get<1>(tc);
        auto fail = std::get<2>(tc);
        add_fixed_size_binary_array_to_nullable_binary_column<bytes_width, AT, LT>(col->as_mutable_raw_ptr(),
...
PARALLEL_TEST(ArrowConverterTest, test_convert_paimon_variant_non_nullable) {
    ...
    const auto* null_placeholder = column->get_row_value(2, &row);
    ASSERT_NE(nullptr, null_placeholder);
    EXPECT_TRUE(null_placeholder->get_value().is_null());
...
PARALLEL_TEST(ArrowConverterTest, test_convert_paimon_variant_nullable) {
    ...
    VariantRowValue row;
    const auto* null_placeholder = variants->get_row_value(1, &row);
    ASSERT_NE(nullptr, null_placeholder);
    EXPECT_TRUE(null_placeholder->get_value().is_null());
    const auto* last = variants->get_row_value(2, &row);
    ASSERT_NE(nullptr, last);
    EXPECT_EQ(three, last->get_value().raw());
}
...
PARALLEL_TEST(ArrowConverterTest, test_reject_malformed_paimon_variant_layout) {
    ...
    auto null_child = create_variant_array({{.value = std::nullopt, .metadata = metadata}});
    status = direct_converter.func(null_child.get(), 0, 1, column.get(), 0, nullptr, &filter, nullptr, nullptr);
    EXPECT_FALSE(status.ok());
