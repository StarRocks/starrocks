...
TEST_F(IcebergChunkSinkTest, test_utils) {
    int128_t val = 123;
    std::string str128 = HiveUtils::to_string_int128(val);
    EXPECT_EQ("123", str128);
    str128 = HiveUtils::to_string_int128(-val);
    EXPECT_EQ("-123", str128);
    ...
    EXPECT_ERROR(HiveUtils::format_decimal_value<int32_t>(123, -1));
    {
        ...
        auto ret = HiveUtils::iceberg_make_partition_name({"k1", "k2"}, {"day", "hour"}, chunk.get(), field_is_null);

        EXPECT_EQ("k1=1970-01-24/k2=1970-01-02-16/", ret.value());
        ret = HiveUtils::iceberg_make_partition_name({"k1", "k2"}, {"year", "month"}, chunk.get(), field_is_null);
...

    {
        ...
        chunk->set_extra_data(chunk_extra_data);
        std::vector<bool> field_is_null;
        auto ret = HiveUtils::iceberg_make_partition_name({"k1", "k2"}, {"identity", "truncate"}, chunk.get(),
                                                          field_is_null);
...

    {
        ...
        auto ret = HiveUtils::iceberg_make_partition_name(
                ...
                {"bucket", "identity", "truncate", "truncate", "truncate", "truncate", "truncate"}, chunk.get(),
                field_is_null);
...

    {
        ...
        chunk->set_extra_data(chunk_extra_data);
        std::vector<bool> field_is_null;
        auto ret = HiveUtils::iceberg_make_partition_name({"k1", "k2", "k3"}, {"identity", "truncate", "bucket"},
                                                          chunk.get(), field_is_null);
