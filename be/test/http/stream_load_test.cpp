...
public:
    ...
    void SetUp() override {
        ...
        ASSERT_OK(_batch_write_mgr->init(&_metrics));
...
TEST_F(StreamLoadActionTest, no_auth) {
    StreamLoadAction action(&_env, &_stream_load_orchestrator, _stream_load_executor.get(), _batch_write_mgr.get(),
                            _limiter.get());
...
TEST_F(StreamLoadActionTest, no_content_length) {
    StreamLoadAction action(&_env, &_stream_load_orchestrator, _stream_load_executor.get(), _batch_write_mgr.get(), _limiter.get());
