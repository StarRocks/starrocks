...
constexpr std::array<const char*, 6> kExpectedRedactedFields = {
...
TEST_F(DumpTabletMetadataActionTest, ReturnsDiagnosticUnavailableWhenNoTabletManagerExists) {
    ...
    EXPECT_EQ(HttpStatus::OK, g_response_status);
    expect_status_response("Service unavailable", "lake tablet manager is unavailable on this compute node");
}
