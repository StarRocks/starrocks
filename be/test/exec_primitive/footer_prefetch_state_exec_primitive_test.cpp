...
// Window is (frontier, frontier + lead_distance]. Concurrency is bounded by each caller's free
// io-task slots (data + warm <= cap), not by the state, so try_take_next keeps handing out files
// across the whole window with no in-flight cap.
TEST(FooterPrefetchStateTest, TakesUpToLeadDistance) {
    FooterPrefetchState st(make_items(100), /*lead_distance=*/4, /*metacache_on=*/true,
...
TEST(FooterPrefetchStateTest, FrontierAdvancesWindow) {
    FooterPrefetchState st(make_items(100), /*lead_distance=*/2, true, false);
    FooterPrefetchItem out;
    EXPECT_TRUE(st.try_take_next(&out));
    EXPECT_EQ("f0", out.key);
    EXPECT_TRUE(st.try_take_next(&out));
...
TEST(FooterPrefetchStateTest, SkipsStartedAndNeverFallsBehindFrontier) {
    FooterPrefetchState st(make_items(100), /*lead_distance=*/10, true, false);
    for (int i = 0; i < 5; ++i) {
...
TEST(FooterPrefetchStateTest, CancelStopsTaking) {
    FooterPrefetchState st(make_items(10), 4, true, false);
    st.cancel();
...
TEST(FooterPrefetchStateTest, AppendExtendsList) {
    FooterPrefetchState st(make_items(2), /*lead_distance=*/100, true, false);
    FooterPrefetchItem out;
    EXPECT_TRUE(st.try_take_next(&out));
    EXPECT_EQ("f0", out.key);
    EXPECT_TRUE(st.try_take_next(&out));
...
TEST(FooterPrefetchStateTest, DedupsDuplicateKeys) {
    ...
    FooterPrefetchState st(std::move(items), /*lead_distance=*/100, true, false);
...
TEST(FooterPrefetchStateTest, WarmableFlags) {
    FooterPrefetchState neither(make_items(1), 4, /*metacache_on=*/false, /*datacache_populate_on=*/false);
    EXPECT_FALSE(neither.warmable());

    FooterPrefetchState meta(make_items(1), 4, /*metacache_on=*/true, /*datacache_populate_on=*/false);
