...
TEST_F(S3Test, bucket_operation) {
    ...
    S3Client client(config, false /* use_transfer_manager */);
...
TEST_F(S3Test, object_operation) {
    ...
    // 3. test object exist
    bool exist = false;
    ASSERT_TRUE(client.exist_object(bucket_name, object_key, &exist).ok());
    ...
    ASSERT_TRUE(
            client.get_object_range(bucket_name, object_key, &object_value_range, 1 /* offset */, 2 /* length */).ok());
    ...
    // 7. test object not exist
    ASSERT_TRUE(client.exist_object(bucket_name, object_key, &exist).ok());
    ASSERT_EQ(exist, false);
    ASSERT_TRUE(client.exist_object(bucket_name, object_key2, &exist).ok());
    ASSERT_EQ(exist, false);
