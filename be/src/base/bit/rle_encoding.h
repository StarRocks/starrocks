...
//   https://github.com/apache/incubator-doris/blob/master/be/src/util/rle_encoding.h
...
public:
    ...
    bool Skip(size_t to_skip, size_t* non_zero_count = nullptr);
    ...
    bool GetBatch(T* vals, size_t batch_num);
...
public:
    ...
    explicit RleEncoder(faststring* buffer, int bit_width) : bit_width_(bit_width), bit_writer_(buffer) {
        DCHECK_GE(bit_width_, 0);
        DCHECK_LE(bit_width_, 64);
        Clear();
    }
...
inline bool RleDecoder<T>::Get(T* val) {
    ...
    if (PREDICT_TRUE(repeat_count_ > 0)) {
        *val = current_value_;
        ...
        if (PREDICT_FALSE(!bit_reader_.GetValue(bit_width_, val))) {
            return false;
        }
        --literal_count_;
...
template <typename T>
inline bool RleDecoder<T>::Skip(size_t to_skip, size_t* non_zero_count) {
    DCHECK(bit_reader_.is_initialized());
    ...
    while (to_skip > 0) {
        ...
        if (PREDICT_TRUE(repeat_count_ > 0)) {
            ...
            repeat_count_ -= nskip;
            to_skip -= nskip;
            if (current_value_ != 0 && non_zero_count != nullptr) {
                *non_zero_count += nskip;
            }
        } else {
            DCHECK(literal_count_ > 0);
            ...
            for (; nskip > 0; nskip--) {
                T value = 0;
                if (PREDICT_FALSE(!bit_reader_.GetValue(bit_width_, &value))) {
                    return false;
                }
                if (value != 0 && non_zero_count != nullptr) {
                    (*non_zero_count)++;
                }
            }
        }
...
template <typename T>
inline bool RleDecoder<T>::GetBatch(T* vals, size_t batch_num) {
    size_t read_num = 0;
...
inline void RleBatchDecoder<T>::NextCounts() {
    ...
    if (is_literal) {
        ...
        bool result = bit_reader_.get_bytes<T>(BitUtil::Ceil(bit_width_, 8), &repeated_value_);
...
inline int32_t RleBatchDecoder<T>::GetBatch(T* values, int32_t batch_num) {
    ...
    while (num_consumed < batch_num) {
        ...
        if (num_repeats > 0) {
            ...
            std::fill_n(values + num_consumed, num_repeats_to_set, repeated_value);
...
static inline bool IndicesInRange(const T* values, int32_t length, int32_t dictionary_length) {
    // For sizeof(T)==4 simd_minmax reads through a signed int32 view: any T value
    // > INT32_MAX would emerge as a negative min, which IndexInRange rejects —
    // preserving the original semantics.
    T min_index, max_index;
    simd_minmax(values, length, min_index, max_index);
...
inline int RleBatchDecoder<T>::GetBatchWithDict(const TV* dictionary, int32_t dictionary_length, TV* values,
    ...
    while (num_consumed < batch_num) {
        ...
        if (num_repeats > 0) {
            ...
            std::fill_n(values + num_consumed, num_repeats_to_set, value);
            num_consumed += num_repeats_to_set;
            continue;
        }
        ...
        simd_dict_gather(values + num_consumed, dictionary, indices, num_literals_to_set);
        num_consumed += num_literals_to_set;
    }
    return num_consumed;
}
