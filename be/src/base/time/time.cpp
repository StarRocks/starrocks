...
void starrocks::SleepForMs(const int64_t duration_ms) {
...
std::string starrocks::ToStringFromUnix(int64_t s, TimePrecision p) {
    chrono::system_clock::time_point t = TimepointFromUnix(s);
    return ToString(t, p, false);
}

std::string starrocks::ToUtcStringFromUnix(int64_t s, TimePrecision p) {
    chrono::system_clock::time_point t = TimepointFromUnix(s);
    return ToString(t, p, true);
}

std::string starrocks::ToStringFromUnixMillis(int64_t ms, TimePrecision p) {
    chrono::system_clock::time_point t = TimepointFromUnixMillis(ms);
    return ToString(t, p, false);
}

std::string starrocks::ToUtcStringFromUnixMillis(int64_t ms, TimePrecision p) {
    chrono::system_clock::time_point t = TimepointFromUnixMillis(ms);
    return ToString(t, p, true);
}

std::string starrocks::ToStringFromUnixMicros(int64_t us, TimePrecision p) {
    chrono::system_clock::time_point t = TimepointFromUnixMicros(us);
    return ToString(t, p, false);
}

std::string starrocks::ToUtcStringFromUnixMicros(int64_t us, TimePrecision p) {
    chrono::system_clock::time_point t = TimepointFromUnixMicros(us);
    return ToString(t, p, true);
}
