...
public:
    ...
    struct BenchContext {
        ...
        BenchContext() {
            ...
            write_throught = std::make_unique<bvar::PerSecond<bvar::Adder<size_t>>>("star_bench", "write_throught", write_bytes.get());
            read_bytes = std::make_unique<bvar::Adder<size_t>>("star_bench", "read_bytes");
            read_throught = std::make_unique<bvar::PerSecond<bvar::Adder<size_t>>>("star_bench", "read_throught", read_bytes.get());
    ...
    BlockCacheBenchSuite(const CacheOptions& options, const BenchParams& params) {
        ...
        if (params.cache_engine == CacheEngine::CACHELIB) {
    ...
    static bool check_buffer(const char* data, size_t length, char ch) {
        for (size_t i = 0; i < length; ++i) {
            if (data[i] != ch) {
                LOG(ERROR) << "check buffer failed, " << "real: " << data[i] << ", expect: " << ch << ", data: " << (uint64_t)data
                           << ", i: " << i;
                return false;
            }
        }
