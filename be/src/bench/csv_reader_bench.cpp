...
public:
    BenchCSVReader(std::shared_ptr<SequentialFile> file, const std::string& record_delimiter,
                   const std::string& field_delimiter, const size_t bufferSize)
            : CSVReader(record_delimiter, field_delimiter, bufferSize) {
        _file = std::move(file);
    }
...
public:
    ...
    Status open() {
        ...
        _csv_reader = std::make_unique<BenchCSVReader>(file, _record_delimiter, _field_delimiter, _bufferSize);
        return st;
    }
    ...
    Status get_all(int64_t& read_line_cnt) {
...
int main(int argc, char** argv) {
    ...
    st = scanner->get_all(read_line_cnt);
    if (!st.ok() && !st.is_end_of_file()) {
