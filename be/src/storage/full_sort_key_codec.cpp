...
// This branch has no types/storage_type_traits.h; storage/type_traits.h provides the same on-disk
// C++ representation under CppTypeTraits. Alias it so the decoders below stay type-for-type
// identical to the upstream implementation.
template <LogicalType LT>
using StorageCppType = typename CppTypeTraits<LT>::CppType;
template <LogicalType LT>
inline constexpr int32_t StorageCppTypeSize = sizeof(StorageCppType<LT>);

// Reverses encoding_utils::encode_slice (be/src/storage/primary_key_encoder.cpp): un-escape
