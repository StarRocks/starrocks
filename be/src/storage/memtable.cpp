...
MemTable::MemTable(int64_t tablet_id, const TabletSchema* tablet_schema, const std::vector<SlotDescriptor*>* slot_descs,
                   ChunkStreamSink* sink, MemTracker* mem_tracker)
...
MemTable::MemTable(int64_t tablet_id, const Schema& schema, ChunkStreamSink* sink, int64_t max_buffer_size,
...
Status MemTable::flush() {
    ...
    int64_t duration_ns = 0;
    {
        SCOPED_RAW_TIMER(&duration_ns);
        if (_deletes) {
            RETURN_IF_ERROR(_sink->add_chunk_with_deletes(*_result_chunk, *_deletes));
        } else {
            RETURN_IF_ERROR(_sink->add_chunk(*_result_chunk));
        }
        RETURN_IF_ERROR(_sink->flush());
    }
