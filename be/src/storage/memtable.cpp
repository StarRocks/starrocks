...
Status MemTable::prepare(std::optional<PrimaryKeyEncodingType> pk_encoding_type) {
    ...
    if (_keys_type == KeysType::PRIMARY_KEYS && pk_encoding_type.has_value()) {
        _pk_encoding_type = pk_encoding_type.value();
    }
    return Status::OK();
}
...
Status MemTable::_split_upserts_deletes(ChunkPtr& src, ChunkPtr* upserts, MutableColumnPtr* deletes) {
    ...
    *upserts = src->clone_empty_with_schema(nupsert);
    ...
    if (!(*deletes)) {
        auto st = PrimaryKeyEncoder::create_column(*_vectorized_schema, deletes, _pk_encoding_type.value());
        if (!st.ok()) {
