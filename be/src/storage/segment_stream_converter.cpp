...
std::atomic<size_t> s_segment_footer_buffer_size = 4 * 1024 * 1024; // 4MB
...
Status SegmentStreamConverter::append(const void* data, size_t size) {
    ...
    if (_segment_footer_buffer.capacity() >= _input_file_size) {
...
Status SegmentStreamConverter::close() {
    ...
    CHECK(_output_file->size() + _segment_footer_buffer.size() == _input_file_size)
            << ", output_file_size: " << _output_file->size()
            << ", segment_footer_buffer_size: " << _segment_footer_buffer.size()
            << ", input_file_size: " << _input_file_size;
