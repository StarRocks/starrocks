...
class EngineReplicationTask : public EngineTask {
public:
    EngineReplicationTask(MemTracker* mem_tracker, const TReplicationRequest& replication_req, int64_t signature,
                          vector<string>* error_msgs, vector<TTabletInfo>* tablet_infos, AgentStatus* res_status);

    ~EngineReplicationTask() override = default;
...
private:
    ...
    Status _make_snapshot(const std::string& ip, int port, TTableId tablet_id, TSchemaHash schema_hash,
                          TVersion version, int timeout_s, const std::vector<Version>* missed_versions,
...
private:
    ...
    const TReplicationRequest& _replication_req;
