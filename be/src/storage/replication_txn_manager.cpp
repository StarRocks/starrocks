...
Status ReplicationTxnManager::replicate_remote_snapshot(const TReplicateSnapshotRequest& request,
    ...
    std::shared_ptr<const TabletSchema> source_schema;
    if (tablet->updates() == nullptr) { // None-pk table
        ...
        source_schema = tablet_meta.tablet_schema_ptr();
    } else { // Pk table
    ...
    ReplicationUtils::calc_column_unique_id_map(*source_schema, tablet->tablet_schema(), &column_unique_id_map);

    auto file_converter = [&](const std::string& file_name,
    ...
    RETURN_IF_ERROR(ReplicationUtils::download_remote_snapshot(
            src_snapshot_info.backend.host, src_snapshot_info.backend.http_port, request.src_token,
            src_snapshot_info.snapshot_path, request.src_tablet_id, request.src_schema_hash, file_converter,
            tablet->data_dir()));
    ...
    if (tablet->updates() == nullptr) {
        ...
        RETURN_IF_ERROR(
                convert_snapshot_for_primary(tablet_snapshot_dir_path, source_schema, column_unique_id_map, request));
    }
...
Status ReplicationTxnManager::convert_snapshot_for_primary(
        const std::string& tablet_snapshot_path, const std::shared_ptr<const TabletSchema>& source_schema,
        const std::unordered_map<uint32_t, uint32_t>& column_unique_id_map, const TReplicateSnapshotRequest& request) {
    ...
    if (!tablet_meta_pb.has_schema()) {
        source_schema->to_schema_pb(tablet_meta_pb.mutable_schema());
    }
    for (auto& rowset_meta : *tablet_meta_pb.mutable_rs_metas()) {
        rowset_meta.set_partition_id(request.partition_id);
        rowset_meta.set_tablet_id(request.tablet_id);
...
Status ReplicationTxnManager::publish_full_meta(Tablet* tablet, TabletMeta* cloned_tablet_meta,
    ...
    for (auto& rs_meta_ptr : rs_metas_found_in_src) {
        ...
        if (auto s = RowsetFactory::create_rowset(cloned_tablet_meta->tablet_schema_ptr().get(), tablet->schema_hash_path(),
...
std::string ReplicationTxnManager::get_txn_dir_path(DataDir* data_dir, TTransactionId transaction_id) {
    return fmt::format("{}/{}/", data_dir->get_replication_path(), transaction_id);
}

std::string ReplicationTxnManager::get_tablet_txn_dir_path(DataDir* data_dir, TTransactionId transaction_id,
                                                           TPartitionId partition_id, TTabletId tablet_id) {
    return fmt::format("{}/{}/{}/{}/", data_dir->get_replication_path(), transaction_id, partition_id, tablet_id);
}

std::string ReplicationTxnManager::get_tablet_snapshot_dir_path(DataDir* data_dir, TTransactionId transaction_id,
                                                                TPartitionId partition_id, TTabletId tablet_id) {
    return fmt::format("{}/{}/{}/{}/snapshot/", data_dir->get_replication_path(), transaction_id, partition_id,
                       tablet_id);
}

std::string ReplicationTxnManager::get_tablet_snapshot_dir_path(const std::string& tablet_txn_dir_path) {
    return tablet_txn_dir_path + "snapshot/";
}

std::string ReplicationTxnManager::get_tablet_txn_meta_file_path(DataDir* data_dir, TTransactionId transaction_id,
                                                                 TPartitionId partition_id, TTabletId tablet_id) {
    return fmt::format("{}/{}/{}/{}/txn_meta", data_dir->get_replication_path(), transaction_id, partition_id,
                       tablet_id);
}

std::string ReplicationTxnManager::get_tablet_txn_meta_file_path(const std::string& tablet_txn_dir_path) {
