...
StatusOr<std::shared_ptr<CLuceneFileWriter>> CLuceneFileManager::get_or_create_clucene_file_writer(
    ...
    if (!_path_to_file_writers.contains(path)) {
        ...
        ASSIGN_OR_RETURN(auto writer, fs->new_writable_file(*opts, path));

        auto index_file_writer =
                std::make_shared<CLuceneFileWriter>(fs, path, std::move(writer), config::inverted_index_ram_dir_enable);
