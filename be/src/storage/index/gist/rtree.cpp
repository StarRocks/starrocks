...
static uint64_t write_leaf_node(std::string& buf, const std::vector<RTreeLeafEntry>& entries, size_t from, size_t to) {
...
static uint64_t write_internal_node(std::string& buf, const std::vector<InternalEntry>& entries, size_t from,
                                    size_t to) {
    uint64_t offset = buf.size();
    write_u8(buf, 0u);
...
static std::vector<InternalEntry> build_leaf_level(std::string& buf, std::vector<RTreeLeafEntry>& entries, int M) {
    ...
    for (int s = 0; s < S; ++s) {
        ...
        std::sort(entries.begin() + static_cast<ptrdiff_t>(s_from), entries.begin() + static_cast<ptrdiff_t>(s_to),
...
static std::vector<InternalEntry> build_internal_level(std::string& buf, std::vector<InternalEntry>& entries, int M) {
    const size_t n = entries.size();
    const int S = static_cast<int>(std::ceil(std::sqrt(static_cast<double>(n) / M)));
    const size_t slice_size = static_cast<size_t>(std::ceil(static_cast<double>(n) / S));
...
void rtree_search(const char* data, size_t size, std::function<bool(const MBR&)> node_predicate,
    ...
    while (!stk.empty()) {
        uint64_t node_off = stk.top();
        stk.pop();
        if (node_off + 5 > size) continue;

        const char* p = data + node_off;
        uint8_t is_leaf = read_u8(p);
        p += 1;
        uint32_t num_entries = read_u32(p);
        p += 4;
        ...
        if (is_leaf) {
            ...
            for (uint32_t i = 0; i < num_entries; ++i) {
                ...
                uint64_t child_offset = read_u64(p);
                p += 8;
                MBR mbr = read_mbr(p);
                p += 32;
...
void rtree_search_intersects(const char* data, size_t size, const MBR& q, std::vector<uint32_t>* result_row_ids) {
    auto pred = [&](const MBR& m) { return mbr_intersects(m, q); };
    rtree_search(data, size, pred, pred, result_row_ids);
}

void rtree_search_within(const char* data, size_t size, const MBR& q, std::vector<uint32_t>* result_row_ids) {
    // row is "within" query → row MBR is contained by query MBR
    // node pruning: node MBR must intersect query
    rtree_search(
            data, size, [&](const MBR& m) { return mbr_intersects(m, q); },
            [&](const MBR& m) { return mbr_contains(q, m); }, result_row_ids);
}

void rtree_search_contains(const char* data, size_t size, const MBR& q, std::vector<uint32_t>* result_row_ids) {
    // row "contains" query → row MBR must contain query MBR
    // node pruning: node MBR must contain or overlap query
    rtree_search(
            data, size, [&](const MBR& m) { return mbr_intersects(m, q); },
            [&](const MBR& m) { return mbr_contains(m, q); }, result_row_ids);
}
