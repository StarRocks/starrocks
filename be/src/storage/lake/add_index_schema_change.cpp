    ...
    for (const auto& rowset : base_metadata->rowsets()) {
        for (int seg_idx = 0; seg_idx < rowset.segment_metas_size(); ++seg_idx) {
            ...
            Status submit_st = runner.submit([this, rowset_copy = std::move(rowset_copy), seg_idx, rssid, op_add_index,
                ...
                // A segment also gets no `.idx` when every base index was skipped
                // because its column is physically absent (see
                // classify_index_for_segment). No .idx file was written then, so
                // publishing an entry would point readers at payloads that do not
                // exist. The DCG rewrite below is independent of that outcome.
                const bool has_idg_entry = entry.keys_size() > 0;
                ...
                if (!has_idg_entry && dcg_indexes.empty()) {
                    return Status::OK();
                }
