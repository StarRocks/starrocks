...
void HorizontalGeneralTabletWriter::close() {
    if (!_finished && !_files.empty()) {
        ...
        delete_list.reserve(_files.size());
        for (const auto& name : _files) {
            delete_list.emplace_back(_tablet.segment_location(name));
        }
...
void VerticalGeneralTabletWriter::close() {
    if (!_finished && !_files.empty()) {
        ...
        files_to_delete.reserve(_files.size());
        for (const auto& name : _files) {
            files_to_delete.emplace_back(_tablet.segment_location(name));
        }
        (void)delete_files(files_to_delete);
    }
    _files.clear();
}
