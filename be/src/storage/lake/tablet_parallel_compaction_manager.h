...
struct TabletParallelCompactionState {
    ...
    // DESHARD is all-or-nothing across every physical subtask. Recorded before
    // submission so a partial thread-pool submission cannot be published.
    int32_t expected_deshard_subtask_count = 0;
...
private:
    ...
    StatusOr<std::shared_ptr<TabletParallelCompactionState>> create_and_register_tablet_state(
            ...
            CompactionModePB mode, std::shared_ptr<CompactionTaskCallback> callback,
            const ReleaseTokenFunc& release_token);
    ...
    // DESHARD must rewrite every selected shared rowset atomically. This planner
    ...
    std::vector<SubtaskGroup> _create_deshard_subtask_groups(int64_t tablet_id, const std::vector<RowsetPtr>& rowsets,
                                                             int32_t max_parallel, int64_t max_bytes_per_subtask);

    static Status _validate_deshard_group_coverage(const std::vector<RowsetPtr>& rowsets,
    ...
    FRIEND_TEST(TabletParallelCompactionManagerTest, test_create_deshard_groups_cover_all_segments);
