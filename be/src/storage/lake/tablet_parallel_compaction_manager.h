...
struct TabletParallelCompactionState {
    ...
    // That context is destroyed at the hand-off, so without carrying this the merged context would only
...
public:
    ...
    // |handoff_in_queue_time_sec| carries the queue wait already accumulated by the caller's context,
    // which is destroyed once the subtasks take over; it is added once to the merged context so the
    // reported queue time stays complete.
