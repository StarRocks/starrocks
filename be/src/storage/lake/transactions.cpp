...
std::ostream& operator<<(std::ostream& out, const lake::PublishTabletInfo& tablet_info) {
...
StatusOr<TabletMetadataPtr> publish_version(TabletManager* tablet_mgr, const PublishTabletInfo& tablet_info,
    ...
    if (txns.size() == 1 && txns[0].txn_id() == EMPTY_TXNLOG_TXNID) {
        LOG(INFO) << "publish version tablet_info: " << tablet_info << ", txn: " << EMPTY_TXNLOG_TXNID
    ...
    for (size_t i = txn_offset, sz = txns.size(); i < sz; i++) {
        ...
        auto txn_log = convert_txn_log_for_dynamic_tablet(txn_log_st.value(), base_metadata,
                                                          tablet_info.get_publish_tablet_type());
    ...
    if (alter_version != -1 && alter_version + 1 < new_version) {
        ...
        for (int64_t v = alter_version + 1; v < new_version; ++v) {
            ...
            auto txn_vlog = convert_txn_log_for_dynamic_tablet(txn_vlog_or.value(), base_metadata,
                                                               tablet_info.get_publish_tablet_type());
