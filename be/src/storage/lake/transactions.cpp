...
int64_t cal_new_base_version(int64_t tablet_id, TabletManager* tablet_mgr, int64_t base_version, int64_t new_version,
    ...
    if (metadata != nullptr && metadata->version() <= new_version) {
        ...
        LOG(INFO) << "PCU_DIAG_PUB: cal_new_base_version tablet=" << tablet_id << " base=" << base_version
                  << " cached_ver=" << metadata->version() << " new_version=" << new_version
...
StatusOr<TabletMetadataPtr> publish_version(TabletManager* tablet_mgr, int64_t tablet_id, int64_t base_version,
    ...
    if (new_base_version > base_version) {
        LOG(INFO) << "PCU_DIAG_PUB: base version adjusted tablet=" << tablet_id << " ori_base=" << base_version
                  << " new_base=" << new_base_version << " new_version=" << new_version << " txns_size=" << txns.size()
    ...
    if (txns.size() > 1) {
        ...
        LOG(INFO) << "PCU_DIAG_PUB: batch_publish tablet=" << tablet_id << " ori_base=" << ori_base_version
                  << " adjusted_base=" << base_version << " new_version=" << new_version << " txns_size=" << txns.size()
                  << " txn_offset=" << (base_version - ori_base_version) << " cached_meta_ver=" << cached_ver
