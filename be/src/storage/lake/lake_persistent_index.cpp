...
Status LakePersistentIndex::load_dels(const RowsetPtr& rowset, const Schema& pkey_schema, int64_t rowset_version) {
    ...
    // A decoded del file ready to replay: key Slices (into the owned pk column) + per-key skip filter.
    struct DecodedDel {
        MutableColumnPtr pkc;     // owns the key bytes; keys/filter borrow from it
        std::vector<Slice> keys;  // Slices into pkc (binary: stored Slices; fixed: _key_size stride)
        std::vector<bool> filter; // true = skip this delete (too old)
    };

    // Extract key Slices from a decoded pk column. The Slices borrow from pkc, so pkc must outlive them.
    auto extract_keys = [&](const MutableColumnPtr& pkc) -> std::vector<Slice> {
        std::vector<Slice> keys;
        if (pkc->is_binary()) {
            // When PK table have multi pk columns or one pk column with varchar type, we treat it as a
            // binary column, which already stores Slices contiguously.
            const Slice* s = reinterpret_cast<const Slice*>(pkc->raw_data());
            keys.assign(s, s + pkc->size());
        } else {
