...
public:
    ...
    // on_accepted must not throw; it runs once the context is owned, before the RPC can finish.
    void finish_task(std::unique_ptr<CompactionTaskContext>&& context, const std::function<void()>& on_accepted = {});
