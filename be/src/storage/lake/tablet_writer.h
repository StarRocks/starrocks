...
class TabletWriter {
public:
    explicit TabletWriter(Tablet tablet, std::shared_ptr<const TabletSchema> tschema)
            : _tablet(tablet), _schema(std::move(tschema)) {}

    virtual ~TabletWriter() = default;

    virtual int64_t tablet_id() const { return _tablet.id(); }
    ...
    virtual std::vector<std::string> files() const { return _files; }

    // The sum of all segment file sizes, in bytes.
    virtual int64_t data_size() const { return _data_size; }

    // The total number of rows have been written.
    virtual int64_t num_rows() const { return _num_rows; }
