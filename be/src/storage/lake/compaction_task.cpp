...
StatusOr<bool> CompactionTask::try_execute_dcg_overlay_merge() {
    ...
    if (max_sparse_chain_depth_for_rowset(rowset_meta, metadata->dcg_meta()) <
        sdcg_effective_trigger_depth_for_rowset(rowset_meta, metadata->dcg_meta())) {
        return false;
    }
    ...
    for (int pos = 0; pos < rowset_meta.segment_metas_size(); ++pos) {
        ...
        // SAFE-PREFIX FOLD: fold the SPARSE_PERCOL layers with version <= compact_version; LEAVE any layer
        // that raced the snapshot (v > compact_version) untouched on top. Racing layers are strictly newer
        // than every folded version, so the merged file (inserted by the apply at max(folded) <=
        // compact_version) sits BELOW them in version order -> last-write-wins preserved. This is the key to
        // converging under sustained ingest: previously ANY racing layer abandoned the whole segment (and a
        // racing layer is the common case under load), so the cheap fast-path almost never fired.
        //
        // A DENSE_COLS layer with v <= compact_version would interleave the folded version range with a
        // row-complete rewrite the sparse merge cannot represent; in that case defer the whole segment to
        // full compaction (sparse-only fold across a dense layer would break LWW). Racing layers all have
        // v > compact_version > every foldable version, so they never interleave the folded range.
        bool has_dense_below = false;
        int sparse_layers = 0;
        for (int i = 0; i < ver.column_files_size(); ++i) {
            const int64_t v = (i < ver.versions_size()) ? ver.versions(i) : 0;
            if (v > compact_version) {
                continue; // racing layer: leave on top, do NOT abandon the segment
            }
            const DeltaColumnFileKindPB kind = (i < ver.file_kinds_size()) ? ver.file_kinds(i) : DENSE_COLS;
            if (kind != SPARSE_PERCOL) {
                has_dense_below = true; // a dense rewrite inside the foldable version range
                break;
            }
            ++sparse_layers; // PACKED (heterogeneous) sparse files are foldable too (read step gates by roaring)
        }
        if (has_dense_below) {
            continue; // defer to full compaction: a sparse-only fold cannot represent an interleaved dense layer
        }
        // Need at least 2 foldable layers to collapse; a single sparse layer is already minimal.
        if (sparse_layers < 2) {
            continue;
        }
        ...
        for (int i = 0; i < ver.column_files_size(); ++i) {
            ...
            if (layer_version > compact_version) {
                continue; // racing layer (newer than snapshot): left on top, not folded this round
            }
            const DeltaColumnFileKindPB kind = (i < ver.file_kinds_size()) ? ver.file_kinds(i) : DENSE_COLS;
            if (kind != SPARSE_PERCOL) {
                continue; // no dense below compact_version here (guarded above); defensive
            }
            ...
            const bool packed =
                    (i < ver.column_presence_lists_size()) && ver.column_presence_lists(i).entries_size() > 0;
            const size_t layers_before = layers.size();
            if (!packed) {
                // Homogeneous layer: every column covers all K source_rowids (no per-column placeholders).
                ...
                // PACKED (heterogeneous) layer: each column covers ONLY the base rowids in its roaring; the
                // other K rows are append_default placeholders that must NOT be merged as winners. Decompose
                // into one single-column layer per column, keeping only covered (rowid, value) cells. Mirrors
                // the read-path gate (LayeredOverlayColumnIterator::column_covers). All single-column layers
                // share `layer_version`; merge_overlay_layers does per-(column,rowid) LWW so same-version
                // different-column layers never conflict, and the file's version is dropped once below.
                const auto& presence_list = ver.column_presence_lists(i);
                for (size_t c = 0; c < column_uids.size(); ++c) {
                    const int32_t uid = column_uids[c];
                    const std::string* roaring_bytes = nullptr;
                    for (int e = 0; e < presence_list.entries_size(); ++e) {
                        if (static_cast<int32_t>(presence_list.entries(e).column_uid()) == uid) {
                            roaring_bytes = &presence_list.entries(e).roaring();
                            break;
                        }
                    }
                    std::vector<uint32_t> covered_rowids;
                    std::vector<uint32_t> covered_idx;
                    if (roaring_bytes == nullptr || roaring_bytes->empty()) {
                        // Defensive: a column with no per-column roaring covers every row (homogeneous column).
                        covered_rowids = source_rowids;
                        covered_idx.resize(source_rowids.size());
                        for (uint32_t r = 0; r < source_rowids.size(); ++r) covered_idx[r] = r;
                    } else {
                        roaring::Roaring rb;
                        RETURN_IF_EXCEPTION(
                                rb = roaring::Roaring::readSafe(roaring_bytes->data(), roaring_bytes->size()));
                        for (uint32_t r = 0; r < source_rowids.size(); ++r) {
                            if (rb.contains(source_rowids[r])) {
                                covered_rowids.push_back(source_rowids[r]);
                                covered_idx.push_back(r);
                            }
                        }
                    }
                    if (covered_rowids.empty()) {
                        continue; // this column covers nothing in this layer
                    }
                    auto gated = values[c]->clone_empty();
                    gated->append_selective(*values[c], covered_idx);
                    OverlaySparseLayer layer;
                    layer.version = layer_version;
                    layer.column_uids = {uid};
                    layer.source_rowids = std::move(covered_rowids);
                    layer.values.emplace_back(std::move(gated));
                    layers.push_back(std::move(layer));
                }
            }
            // Drop this file's version only if it actually contributed >=1 layer to the merge (a packed file
            // covering nothing -- shouldn't happen -- must not have its version collapsed without its data).
            if (layers.size() > layers_before) {
        ...
        StorageMetrics::instance()->sdcg_overlay_merge_total.increment(1);
        StorageMetrics::instance()->sdcg_overlay_merge_layers_folded_total.increment(
