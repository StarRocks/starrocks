...
private:
    ...
    // Rebuild _sstable_vec from _sstable_map. Used by init() only; append() grows
    // _sstable_vec incrementally in O(1) to avoid O(N^2) over N appends.
    void _rebuild_sstable_vec();

    // Flattened view of _sstable_map for cache-friendly binary search in multi_get.
    // Sorted by start_key. Slices point into _sstable_map's string keys, which is
    // safe only because _sstable_map is append-only: std::map guarantees that
    // existing nodes' storage is not invalidated by insertions, and we never erase
    // or mutate keys. If erase is ever added, this scheme must be revisited.
    struct SstableVecEntry {
        Slice start_key;
        Slice end_key;
        PersistentIndexSstable* sstable;
    };
    std::vector<SstableVecEntry> _sstable_vec;

    // From <start_key, end_key> to sstable_ptr. Append-only: nodes are never erased
    // or re-keyed. This invariant is what lets _sstable_vec hold Slices into the
    // map's string keys. The class is not thread-safe; callers must serialize
    // init/append against multi_get.
