...
Status merge_delvec_files(TabletManager* tablet_mgr, const std::vector<DelvecFileInfo>& old_delvec_files,
                          ...
                          uint64_t* extra_data_offset = nullptr, bool force_encrypt_output = false);
...
// is written at offset 0; the resulting FileMetaPB is shared=false, encryption
// is per-call when |buffer| is non-empty.
Status write_delvec_file_from_buffer(TabletManager* tablet_mgr, int64_t new_tablet_id, int64_t txn_id,
                                     const Slice& buffer, FileMetaPB* new_delvec_file, bool encrypt_output = false);
