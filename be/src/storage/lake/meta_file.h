...
public:
    ...
    // skip_input_deletion: if true, skip deleting input rowsets (for large rowset split non-first subtasks)
    Status apply_opcompaction(const TxnLogPB_OpCompaction& op_compaction, uint32_t max_compact_input_rowset_id,
                              int64_t output_rowset_schema_id, bool skip_input_deletion = false);
