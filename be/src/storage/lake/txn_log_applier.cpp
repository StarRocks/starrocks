...
public:
    ...
    Status apply(const TxnLogVector& tnx_logs) override {
        if (tnx_logs.empty()) {
            return Status::OK();
        }
        ...
        for (const auto& log : tnx_logs) {
            _max_txn_id = std::max(_max_txn_id, log->txn_id());

            // Check for any non-write operations
            if (log->has_op_schema_change() || log->has_op_alter_metadata() || log->has_op_compaction() ||
                log->has_op_replication()) {
                return Status::NotSupported("Only op_write operations are supported in batch transaction log apply");
            }
...
public:
    ...
    Status apply(const TxnLogVector& tnx_logs) override {
        ...
        VLOG(2) << "Applying " << tnx_logs.size() << " transaction logs in batch for tablet " << _tablet.id();
