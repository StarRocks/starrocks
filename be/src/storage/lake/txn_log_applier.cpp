...
private:
    ...
    Status apply_replication_log(const TxnLogPB_OpReplication& op_replication, int64_t txn_id) {
        ...
        if (op_replication.has_schema()) {
            _metadata->mutable_schema()->CopyFrom(op_replication.schema());
            _metadata->mutable_schema()->set_id(TabletSchema::invalid_id());
        }
...
private:
    ...
    Status apply_replication_log(const TxnLogPB_OpReplication& op_replication) {
        ...
        if (op_replication.has_schema() && TabletSchema(op_replication.schema()) != TabletSchema(_metadata->schema())) {
            _metadata->mutable_schema()->CopyFrom(op_replication.schema());
            _metadata->mutable_schema()->set_id(TabletSchema::invalid_id());
        }
