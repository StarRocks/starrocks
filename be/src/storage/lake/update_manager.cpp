...
Status UpdateManager::publish_primary_compaction(const TxnLogPB_OpCompaction& op_compaction, int64_t txn_id,
    ...
    auto compaction_entry = _compaction_cache.get_or_create(strings::Substitute("$0_$1", tablet->id(), txn_id));
...
bool UpdateManager::TEST_check_update_state_cache_noexist(uint32_t tablet_id, int64_t txn_id) {
    ...
    if (state_entry == nullptr) {
        ...
        DeferOp release_state_entry([&] { _update_state_cache.release(state_entry); });
        return false;
    }
}

bool UpdateManager::TEST_check_compaction_cache_noexist(uint32_t tablet_id, int64_t txn_id) {
    auto compaction_entry = _compaction_cache.get(strings::Substitute("$0_$1", tablet_id, txn_id));
    if (compaction_entry == nullptr) {
        return true;
    } else {
        DeferOp release_compaction_entry([&] { _compaction_cache.release(compaction_entry); });
        return false;
    }
}
...
void UpdateManager::preload_compaction_state(const TxnLog& txnlog, Tablet* tablet,
                                             const TabletSchema& tablet_schema) {
    ...
    auto compaction_entry = _compaction_cache.get_or_create(strings::Substitute("$0_$1", tablet->id(), txnlog.txn_id()));
    compaction_entry->update_expire_time(MonotonicMillis() + get_cache_expire_ms());
    auto& compaction_state = compaction_entry->value();
