...
Status UpdateManager::publish_primary_compaction(const TxnLogPB_OpCompaction& op_compaction, int64_t txn_id,
    ...
    const RowsetMetadata* input_rowset_ptr = nullptr;
    auto rowset_iter = std::find_if(metadata.rowsets().begin(), metadata.rowsets().end(),
                                    [&](const RowsetMetadata& r) { return r.id() == max_rowset_id; });
    if (rowset_iter != metadata.rowsets().end()) {
        input_rowset_ptr = &(*rowset_iter);
    } else if (is_non_first_split_subtask) {
        // For non-first split subtasks, input rowsets may have been moved to compaction_inputs()
        // by the first subtask's apply_opcompaction
        auto compaction_input_iter =
                std::find_if(metadata.compaction_inputs().begin(), metadata.compaction_inputs().end(),
                             [&](const RowsetMetadata& r) { return r.id() == max_rowset_id; });
        if (compaction_input_iter != metadata.compaction_inputs().end()) {
            input_rowset_ptr = &(*compaction_input_iter);
        }
    }
    if (input_rowset_ptr == nullptr) {
    ...
    uint32_t max_src_rssid = max_rowset_id + input_rowset_ptr->segments_size() - 1;
    std::map<uint32_t, size_t> segment_id_to_add_dels;
