...
Status ColumnModePartialUpdateHandler::_update_source_chunk_by_upt_flexible(
        ...
        const std::vector<std::vector<ColumnUID>>& distinct_column_sets, StreamChunkContainer container) {
    ...
    for (const auto& each : upt_id_to_rowid_pairs) {
        ...
        // declare keep the base/current value already in the container's chunk (read through the DCG
        // overlay). Sort the covered source rowids ascending to match the homogeneous path's update_rows
        // contract. The source segment is streamed, so a pair outside [start_rowid, end_rowid) belongs to
        // another round and is skipped here; the ones that remain are rebased onto the container, exactly
        // as split_rowid_pairs() does for the homogeneous path.
        for (size_t c = 0; c < num_value_cols; ++c) {
            std::vector<std::pair<uint32_t, uint32_t>> covered; // (source_rowid, upt_rowid)
            covered.reserve(pairs.size());
            for (const auto& p : pairs) {
                if (!set_covers[p.set_id][c]) continue;
                if (!container.contains(p.source_rowid)) continue;
                covered.emplace_back(p.source_rowid - container.start_rowid, p.upt_rowid);
            }
