...
static StatusOr<std::map<std::string, DirEntry>> list_data_files(FileSystem* fs,
    ...
    RETURN_IF_ERROR_WITH_WARN(ignore_not_found(fs->iterate_dir2(
                                      ...
                                      [&](DirEntry entry) {
                                          ...
                                          // should consider segment files, sst, del file, delvector, lcrm
                                          // NOTE: .lcrm files are the Lake Compaction Rows Mapper files
                                          // produced by (parallel and serial) PK compaction. They are
                                          // referenced only from the transaction log (OpCompaction.lcrm_file
                                          // / OpParallelCompaction subtask/orphan lcrm), never from any live
                                          // TabletMetadataPB field -- on a successful publish they are
                                          // consumed and deleted by RowsMapperIterator, and superseded ones
                                          // enter orphan_files. So an .lcrm left behind by an
                                          // aborted/failed/crashed compaction is referenced by nothing
                                          // durable and, before this filter included it, could never be
                                          // reclaimed by any GC path. An in-flight .lcrm is protected here
                                          // identically to the output segments the same compaction wrote:
                                          // the production full-vacuum path keeps any file whose txn-id
                                          // filename prefix is >= min_active_txn_id (see
                                          ...
                                          if (!is_segment(entry.name) && !is_sst(entry.name) &&
                                              !is_delvec(entry.name) && !is_del(entry.name) && !is_lcrm(entry.name)) {
                                              return true;
                                          }
