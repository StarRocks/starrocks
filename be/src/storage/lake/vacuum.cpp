...
static Status commit_metadata_range(TabletManager* tablet_mgr, std::string_view root_dir,
    ...
    if (!shared_file_deleter.is_empty()) {
        RETURN_IF_ERROR(collect_alive_shared_files(tablet_mgr, tablet_infos, pass_start_version, root_dir,
                                                   &shared_file_deleter));
        RETURN_IF_ERROR(shared_file_deleter.finish());
        (*vacuumed_files) += shared_file_deleter.delete_count();
    }
...
Status vacuum_impl(TabletManager* tablet_mgr, const VacuumRequest& request, VacuumResponse* response,
    ...
    if (request.has_max_versions_per_round()) {
        ...
        if (req_to_delete_low < req_to_delete_high) {
            if (auto st =
                        commit_metadata_range(tablet_mgr, root_loc, tablet_infos, req_to_delete_low, req_to_delete_high,
                                              pass_start_version, enable_file_bundling, enable_shared_file_cleanup,
        ...
        // An old FE never sets it: fall back to max_versions_per_round so the walk behaves exactly as before.
