...
#ifndef BE_TEST
    auto tp = StorageEngine::instance()->storage_cleanup_executor()->thread_pool();
...
void delete_files_async(std::vector<std::string> files_to_delete) {
    ...
    auto st = StorageEngine::instance()->storage_cleanup_executor()->submit(std::move(task));
...
std::future<Status> delete_files_callable(std::vector<std::string> files_to_delete) {
    ...
    return StorageEngine::instance()->storage_cleanup_executor()->submit_callable(
