...
bool TabletRetainInfo::filter_by_file_name(const std::string& file_name) const {
    return _files.find(file_name) != _files.end();
}

bool TabletRetainInfo::filter_by_version(int64_t version) const {
    return _versions.find(version) != _versions.end();
}

bool TabletRetainInfo::filter_by_rowset_id(uint32_t rowset_id) const {
