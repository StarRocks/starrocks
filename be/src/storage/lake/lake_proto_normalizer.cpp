...
// segment_metas[i] and present in the legacy array (structured wins; never overwrites a real value).
// The NAME fields (segment filename / del name / rewrite name) additionally treat a present-but-EMPTY
// value as "needs filling": an empty name is never valid and resolves to ".../data/" (unreadable), so
// recover the real name from the legacy array rather than keeping/persisting "".
//
...
void backfill_segment_metas_from_legacy(RowsetMetadataPB* rowset_metadata, bool allow_extend) {
    ...
    for (int i = 0; i < segment_count; ++i) {
        ...
        if ((!segment_metadata->has_filename() || segment_metadata->filename().empty()) &&
            i < rowset_metadata->deprecated_segments_size()) {
...
void backfill_op_write_metas_from_legacy(TxnLogPB::OpWrite* op_write, bool allow_extend) {
    ...
    for (int i = 0; i < del_file_count; ++i) {
        ...
        if ((!del_file_metadata->has_name() || del_file_metadata->name().empty()) &&
            i < op_write->deprecated_dels_size()) {
    ...
    for (int i = 0; i < rewrite_segment_count; ++i) {
        ...
        if ((!rewrite_segment_metadata->has_name() || rewrite_segment_metadata->name().empty()) &&
            i < op_write->deprecated_rewrite_segments_size()) {
