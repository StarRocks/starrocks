...
Status Rowset::load_segments(std::vector<SegmentPtr>* segments, SegmentReadOptions& seg_options,
    ...
    if (use_index_mapping) {
        segments->resize(base_idx + (seg_end - seg_start));
    }
    ...
    for (index = seg_start; index < seg_end; index++) {
        ...
        int current_idx = base_idx + (index - seg_start);
    ...
    for (auto& [target_idx, future] : segment_futures) {
        ...
        if (auto status = check_status_at_index(segment_or, result_pair.second, target_idx - base_idx + seg_start,
                                                target_idx);
            !status.ok()) {
            return status;
        }
