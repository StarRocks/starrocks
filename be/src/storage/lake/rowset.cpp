...
<<<<<<< HEAD
    ...
    seg_options.stats = stats;
    seg_options.is_primary_keys = true;
    seg_options.delvec_loader = std::make_shared<LakeDelvecLoader>(_tablet->update_mgr(), builder, true /*fill cache*/);
...
Status Rowset::load_segments(std::vector<SegmentPtr>* segments, bool fill_data_cache, bool fill_metadata_cache) {
    ...
    auto segment_size_size = _rowset_metadata->segment_size_size();
