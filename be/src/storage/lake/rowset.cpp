...
Status Rowset::load_segments(std::vector<SegmentPtr>* segments, bool fill_cache) {
    ...
    uint32_t seg_id = 0;
    bool ignore_missing_segment = config::experimental_lake_ignore_missing_segment;
    ...
    for (const auto& seg_name : _rowset_metadata->segments()) {
        ...
        } else if (segment_or.status().is_not_found() && ignore_missing_segment) {
            LOG(WARNING) << "Ignored missing segment " << seg_name;
            continue;
        } else {
