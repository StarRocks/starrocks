...
#include "util/trace.h"
    ...
    Rowset::Rowset(TabletManager * tablet_mgr, TabletMetadataPtr tablet_metadata, int rowset_index,
        ...
        if (_tablet_metadata->rowset_to_schema().empty() ||
            ...
            _tablet_schema = GlobalTabletSchemaMap::Instance()
                                     ->emplace(_tablet_metadata->historical_schemas().at(schema_id))
                                     .first;
        }
    ...
    Status Rowset::add_partial_compaction_segments_info(TxnLogPB_OpCompaction * op_compaction, TabletWriter * writer,
        ...
        op_compaction->mutable_output_rowset()->set_next_compaction_offset(
    ...
    StatusOr<std::vector<ChunkIteratorPtr>> Rowset::read(const Schema& schema, const RowsetReadOptions& options) {
        ...
        if (_metadata->has_record_predicate()) {
            ASSIGN_OR_RETURN(seg_options.record_predicate,
                             RecordPredicateHelper::create(_metadata->record_predicate()));
        }
        ...
        if (_metadata->has_record_predicate()) {
            RETURN_IF_ERROR(RecordPredicateHelper::get_column_ids(*seg_options.record_predicate,
    ...
    StatusOr<std::vector<ChunkIteratorPtr>> Rowset::get_each_segment_iterator(
            const Schema& schema, bool file_data_cache, OlapReaderStatistics* stats) {
        std::vector<SegmentPtr> segments;
            ...
            if (segment_encryption_metas_size > 0) {
                if (index >= segment_encryption_metas_size) {
                    string msg = fmt::format("tablet:{} rowset:{} index:{} >= segment_encryption_metas size:{}",
                                             _tablet_id, metadata().id(), index, segment_encryption_metas_size);
                    LOG(ERROR) << msg;
...
=======
#ifdef BE_TEST Status injected_st;
