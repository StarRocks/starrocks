...
Status SnapshotFileSyncer::upload(const TabletSnapshotInfo snapshot_info, UploadSnapshotFilesResponsePB* response) {
    ...
    for (auto& data_file : snapshot_info.tablet_snapshot.new_data_files()) {
        ...
        ASSIGN_OR_RETURN(auto src_fs, FileSystem::CreateSharedFromString(src_data_file));
        ASSIGN_OR_RETURN(auto input_file, src_fs->new_sequential_file(src_data_file));
        ...
        ASSIGN_OR_RETURN(auto dst_fs, FileSystem::CreateSharedFromString(dst_data_file));
        ASSIGN_OR_RETURN(auto output_file, dst_fs->new_writable_file(dst_data_file));
