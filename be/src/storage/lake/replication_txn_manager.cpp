...
StatusOr<TxnLogPtr> ReplicationTxnManager::replicate_remote_snapshot_for_primary(
    ...
    // Note that snapshot_version in snapshot_meta may be 0 in incremental_snapshot, so we do not use it
    CHECK(((src_snapshot_info.incremental_snapshot &&
          ...
          (snapshot_meta.rowset_metas().back().end_version() == request.src_visible_version))
            << ", incremental_snapshot: " << src_snapshot_info.incremental_snapshot
            << ", snapshot_type: " << SnapshotTypePB_Name(snapshot_meta.snapshot_type())
            << ", snapshot_version: " << snapshot_meta.rowset_metas().back().end_version()
    ...
    if (!snapshot_meta.delete_vectors().empty()) {
        std::string delvec_file_name = gen_delvec_filename(request.transaction_id);
        std::string delvec_file_path = _tablet_manager->delvec_location(request.tablet_id, delvec_file_name);
        std::string delvec_file_content;
        for (const auto& pair : snapshot_meta.delete_vectors()) {
            std::string delvec_content = pair.second.save();

            auto* delvec_meta = txn_log->mutable_op_replication()->mutable_delvec_meta();
            auto& file_meta = (*delvec_meta->mutable_version_to_file())[request.src_visible_version];
            file_meta.set_name(delvec_file_name);
            file_meta.set_size(delvec_content.size());
            auto& delvec_page = (*delvec_meta->mutable_delvecs())[pair.first];
            delvec_page.set_version(request.src_visible_version);
            delvec_page.set_offset(delvec_file_content.size());
            delvec_page.set_size(delvec_content.size());

            delvec_file_content.append(delvec_content);
        }
        ...
        RETURN_IF_ERROR(writer_file->append(Slice(delvec_file_content.data(), delvec_file_content.size())));
