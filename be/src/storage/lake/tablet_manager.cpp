...
static Status save_lake_protobuf(const std::string& path, const ::google::protobuf::Message& message,
    ...
    if (config::lake_enable_protobuf_file_checksum) {
        ...
        return file.save(message, true, must_create);
    }
...
StatusOr<TabletMetadataPtr> TabletManager::get_tablet_metadata(const string& path, const CacheOptions& cache_opts,
    ...
    const std::string bundle_path = join_path(prefix_name(path), tablet_metadata_filename(0, version));
    auto load_single_from_path = [&]() {
        return get_single_tablet_metadata_from_paths(tablet_id, version, cache_opts, expected_gtid, fs, path,
                                                     bundle_path, bundle_path);
    };
    auto cache_key = _location_provider->real_location(tablet_metadata_root_location(tablet_id));
    if (cache_key.ok() && _metacache->lookup_aggregation_partition(*cache_key)) {
        metadata_or = load_single_from_path();
        ...
        if (metadata_or.status().is_not_found()) {
            metadata_or = load_single_from_path();
            // An explicit filesystem can belong to a remote cluster. Do not let a source tablet-id
            // collision teach the target metacache that its local partition uses bundled metadata.
            if (metadata_or.ok() && cache_key.ok() && fs == nullptr) {
...
StatusOr<TabletMetadataPtr> TabletManager::get_single_tablet_metadata_from_paths(
    ...
    if (expected_gtid > 0 && metadata->gtid() > 0 && expected_gtid != metadata->gtid()) {
        auto drop_status = drop_local_cache(bundle_path, file_system);
        if (!drop_status.ok()) {
            LOG(WARNING) << "clear dirty cache for " << bundle_path << " failed, "
                         << "error: " << drop_status;
            return drop_status;
        }
        LOG(INFO) << "clear dirty cache for " << bundle_path;
        return Status::NotFound("Not found expected tablet metadata");
    }
...
Status TabletManager::put_txn_slog_if_absent(const TxnLogPtr& log) {
    if (UNLIKELY(!log->has_tablet_id())) {
        return Status::InvalidArgument("txn log does not have tablet id");
    }
    if (UNLIKELY(!log->has_txn_id())) {
    ...
    auto serializable_log = std::make_shared<TxnLog>(*log);
    RETURN_IF_ERROR(normalize_txn_log_before_save(serializable_log.get()));
    auto path = txn_slog_location(log->tablet_id(), log->txn_id());
