...
StatusOr<TabletMetadataPtr> TabletManager::get_tablet_metadata_from_bundle_file(
        const std::string& bundle_file_path, int64_t tablet_id, const std::shared_ptr<FileSystem>& fs) {
    RandomAccessFileOptions opts{.skip_fill_local_cache = true};
    ...
    if (meta_it == bundle_metadata->tablet_meta_pages().end()) {
        return Status::NotFound(strings::Substitute("can not find tablet $0 from bundle metadata $1", tablet_id,
                                                    bundle_file_path));
    }
    ...
    if (file_size < offset + size) {
        return Status::Corruption(strings::Substitute(
                "deserialized bundle metadata($0) failed, file_size($1) too small($2/$3)", bundle_file_path, file_size,
                offset, size));
    }
    ...
    if (crc_it != bundle_metadata->tablet_meta_page_checksum().end() &&
        ...
        return Status::Corruption(
                strings::Substitute("mismatched checksum of tablet $0 metadata in bundle metadata $1", tablet_id,
                                    bundle_file_path));
    }

    if (!metadata->ParseFromArray(metadata_str.data(), size)) {
        return Status::Corruption(strings::Substitute("deserialized tablet $0 metadata failed from bundle $1", tablet_id,
                                                      bundle_file_path));
    }
