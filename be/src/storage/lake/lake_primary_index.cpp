    ...
    if (tablet->get_enable_persistent_index(base_version) && (fix_size <= 128)) {
        ...
        if (persistent_index_type.ok()) {
            ...
            case PersistentIndexTypePB::LOCAL: {
                ...
                // it may not take effect if is as compute node.
                if (StorageEngine::instance()->is_as_cn()) {
                    LOG(WARNING) << "lake_persistent_index_type of LOCAL will not take effect when as cn";
                    return Status::InternalError("lake_persistent_index_type of LOCAL will not take effect when as cn");
                }
