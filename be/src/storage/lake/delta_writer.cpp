...
StatusOr<TxnLogPtr> DeltaWriterImpl::finish_with_txnlog(DeltaWriterFinishMode mode) {
    ...
    if (mode == kWriteTxnLog) {
        Status res;
        FAIL_POINT_TRIGGER_ASSIGN_STATUS_OR_DEFAULT(load::failpoint::COMMIT_TXN, res,
                                                    COMMIT_TXN_FP_ACTION(_txn_id, _tablet_id),
    ...
    if (_tablet_schema->keys_type() == KeysType::PRIMARY_KEYS && !skip_pk_preload) {
        ...
        FAIL_POINT_TRIGGER_EXECUTE_OR_DEFAULT(load::failpoint::PK_PRELOAD,
                                              (void)PK_PRELOAD_FP_ACTION(_txn_id, _tablet_id),
