...
Status HorizontalPkTabletWriter::write_single_flush_with_op(const Chunk& chunk_with_op, SegmentPB* segment, bool eos) {
    ...
    // (the raw-data pointer is invalidated by remove_column_by_index). __op is a non-nullable TINYINT (Int8)
    // column, so read its raw buffer directly (branch-4.1 has no RawDataVisitor).
    const auto* ops = data_chunk->get_column_by_index(op_col_id)->raw_data();
