...
#include "vacuum.h"
    ...
    Status LakeReplicationTxnManager::replicate_lake_remote_storage(const TReplicateSnapshotRequest& request) {
        ...
        if (has_full_path) {
            ...
            if (src_partition_full_path.find("s3://") != 0) {
                return Status::InvalidArgument(fmt::format("Full path must be S3 type (start with 's3://'), got: {}",
                                                           src_partition_full_path));
            }
            std::string src_partition_starlet_uri =
                    convert_s3_path_to_starlet_uri(src_partition_full_path, src_tablet_id);
            ...
            ASSIGN_OR_RETURN(src_tablet_meta, try_build_source_tablet_meta_with_fallback(
                                                      src_tablet_id, src_visible_version, src_db_id, txn_id,
                                                      src_meta_dir, src_data_dir, shared_src_fs));
        } else {
        ...
        for (const auto& pair : filename_map) {
            ...
            tasks.emplace_back([&, src_file_name, src_file_location, target_file_location, target_file_name,
                                src_file_size, is_seg, encryption_info]() -> Status {
                ...
                TEST_SYNC_POINT_CALLBACK("LakeReplicationTxnManager::replicate_lake_remote_storage::before_copy",
                                         nullptr);
        ...
        if (use_parallel) {
            ...
            for (size_t i = 0; i < tasks.size(); i++) {
                auto st = token->submit_func(
                        [&task_results, i, task = std::move(tasks[i])]() { task_results[i] = task(); });
                if (!st.ok()) {
            ...
            for (const auto& r : task_results) {
                if (!r.ok()) {
                    LOG(WARNING) << "Parallel file copy failed, txn_id: " << txn_id
                                 << ", tablet_id: " << target_tablet_id
    ...
    StatusOr<TabletMetadataPtr> LakeReplicationTxnManager::try_build_source_tablet_meta_with_fallback(
            int64_t src_tablet_id, int64_t version, int64_t src_db_id, TTransactionId txn_id,
            std::string & src_meta_dir, std::string & src_data_dir, const std::shared_ptr<FileSystem>& shared_src_fs) {
    ...
    StatusOr<std::shared_ptr<TabletMetadataPB>> LakeReplicationTxnManager::convert_and_build_new_tablet_meta(
        ...
        for (const auto& src_rowset_meta : src_tablet_meta->rowsets()) {
            ...
            for (int i = 0; i < src_rowset_meta.del_files_size(); ++i) {
                ...
                ASSIGN_OR_RETURN(
                        auto is_existed,
                        determine_final_filename(src_del_filename, txn_id, existed_filename_uuids, final_del_filename,
        ...
        if (src_tablet_meta->has_sstable_meta()) {
            ...
            for (int i = 0; i < dest_meta->sstables_size(); ++i) {
                ...
                ASSIGN_OR_RETURN(
                        auto is_existed,
                        determine_final_filename(src_sst_filename, txn_id, existed_filename_uuids, final_sst_filename,
                                                 target_tablet_id, src_data_dir, file_locations, filename_map));
        ...
        if (src_tablet_meta->has_delvec_meta()) {
            ...
            for (const auto& [version, file_meta_pb] : dest_meta->version_to_file()) {
                ...
                ASSIGN_OR_RETURN(auto is_existed,
                                 determine_final_filename(src_delvec_filename, txn_id, existed_filename_uuids,
        ...
        if (src_tablet_meta->has_dcg_meta()) {
            ...
            for (auto& [segment_id, dcg_ver_pb] : *dest_meta->mutable_dcgs()) {
                for (int i = 0; i < dcg_ver_pb.column_files_size(); ++i) {
                    ...
                    ASSIGN_OR_RETURN(auto is_existed,
                                     determine_final_filename(src_dcg_filename, txn_id, existed_filename_uuids,
                                                              final_dcg_filename, target_tablet_id, src_data_dir,
                                                              file_locations, filename_map));
