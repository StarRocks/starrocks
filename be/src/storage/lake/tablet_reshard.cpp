...
>>>>>>> e96ab2a5e0 ([Refactor] Refactor tablet reshard structure with helper/splitter/merger files (#69323))
    ...
    CONTINUE_HANDLE_SPLITTING_TABLET:
        ...
        if (!old_tablet_old_metadata_or.ok()) {
            LOG(WARNING) << "Failed to get tablet: " << splitting_tablet.old_tablet_id()
                         << ", version: " << base_version << ", txn_id: " << txn_info.txn_id()
        ...
        ASSIGN_OR_RETURN(auto split_new_metadatas, split_tablet(tablet_manager, old_tablet_old_metadata,
                                                                splitting_tablet, new_version, txn_info));
    ...
    CONTINUE_HANDLE_MERGING_TABLET:
        ...
        for (auto old_tablet_id : merging_tablet.old_tablet_ids()) {
            ...
            if (old_tablet_old_metadata_or.status().is_not_found()) {
                ...
                auto new_tablet_new_metadata_or = tablet_manager->get_tablet_metadata(merging_tablet.new_tablet_id(),
                                                                                      new_version, txn_info.gtid());
                if (!new_tablet_new_metadata_or.ok()) {
                    return old_tablet_old_metadata_or.status();
                }
                auto& new_tablet_new_metadata = new_tablet_new_metadata_or.value();
    ...
    CONTINUE_HANDLE_IDENTICAL_TABLET:
        ...
        if (!old_tablet_old_metadata_or.ok()) {
            LOG(WARNING) << "Failed to get tablet: " << identical_tablet.old_tablet_id()
                         << ", version: " << base_version << ", txn_id: " << txn_info.txn_id()
                         << ", status: " << old_tablet_old_metadata_or.status();
            return old_tablet_old_metadata_or.status();
        }
    ...
    StatusOr<TxnLogPtr> convert_txn_log(const TxnLogPtr& txn_log, const TabletMetadataPtr& base_tablet_metadata,
        ...
        if (publish_tablet_info.get_publish_tablet_type() == PublishTabletInfo::SPLITTING_TABLET) {
            ...
            RETURN_IF_ERROR(
                    tablet_reshard_helper::update_rowset_ranges(new_txn_log.get(), base_tablet_metadata->range()));
        }
    ...
    Status publish_resharding_tablet(TabletManager * tablet_manager, const ReshardingTabletInfoPB& resharding_tablet,
        ...
        if (resharding_tablet.has_splitting_tablet_info()) {
            RETURN_IF_ERROR(handle_splitting_tablet(tablet_manager, resharding_tablet.splitting_tablet_info(),
                                                    base_version, new_version, txn_info, tablet_metadatas,
                                                    tablet_ranges));
            ...
            RETURN_IF_ERROR(handle_identical_tablet(tablet_manager, resharding_tablet.identical_tablet_info(),
                                                    base_version, new_version, txn_info, tablet_metadatas));
        }
