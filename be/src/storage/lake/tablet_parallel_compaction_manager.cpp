...
StatusOr<int> TabletParallelCompactionManager::create_parallel_tasks(
    ...
    // For PK tables, use the new grouping algorithm that supports large rowset splitting
    if (is_pk_table) {
        ...
        if (subtask_groups.empty()) {
            VLOG(1) << "Parallel compaction: tablet=" << tablet_id << " txn=" << txn_id
                    << " fallback to normal compaction (PK), total_rowsets=" << total_rowsets_count;
            return 0;
        }

        VLOG(1) << "Parallel compaction: tablet=" << tablet_id << " created " << subtask_groups.size()
                << " subtask groups from " << total_rowsets_count << " rowsets (PK table)";

        // Step 3: Create and register tablet state
        ASSIGN_OR_RETURN(auto state_ptr,
                         create_and_register_tablet_state(tablet_id, txn_id, version, max_parallel, max_bytes,
                                                          std::move(callback), release_token));

        // Step 4: Submit subtasks using the new API
    ...
    // For non-PK tables, use the existing grouping algorithm
...
StatusOr<TxnLogPB> TabletParallelCompactionManager::get_merged_txn_log(int64_t tablet_id, int64_t txn_id) {
...

    {
        ...
        // If any subtask in a group failed, the entire group is considered failed.
        ...
        // Fill subtask_compactions: each successful subtask's OpCompaction is copied
        // For LARGE_ROWSET_PART subtasks, segment_range_start > 0 indicates non-first subtask
        for (const auto& ctx : state->completed_subtasks) {
            ...
            // For LARGE_ROWSET_PART subtasks: keep input_rowsets for all subtasks
            // The publish phase uses segment_range_start > 0 to identify non-first subtasks
            // and skip input rowset deletion accordingly (first subtask handles deletion)
        }
...
bool TabletParallelCompactionManager::_is_large_rowset_for_split(const RowsetPtr& rowset,
    ...
    return data_size >= min_size && data_size > max_bytes_per_subtask && rowset->metadata().segments_size() >= 4 &&
    ...
    for (size_t group_idx = 0; group_idx < groups.size(); group_idx++) {
        ...
        if (!acquire_token()) {
            ...
            if (subtasks_created == 0) {
                cleanup_tablet(tablet_id, txn_id);
                return Status::ResourceBusy(strings::Substitute(
                        "Failed to acquire limiter token: tablet_id=$0, txn_id=$1", tablet_id, txn_id));
            }
            break;
        }
...
void TabletParallelCompactionManager::execute_subtask_segment_range(int64_t tablet_id, int64_t txn_id,
                                                                    int32_t subtask_id, RowsetPtr input_rowset,
