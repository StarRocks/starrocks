...
// Fold |src| -- the txn log one multi-node write node produced for a tablet -- into |dst|, the log being
// assembled for the same tablet from every node that wrote it in this transaction.
//
// Multi-node write (TOlapTableSink.enable_multi_node_write) has N compute nodes each write a disjoint part of
