...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/row_cursor.cpp
...
OLAPStatus RowCursor::_init_with_schema() {
    _variable_len = 0;
...
OLAPStatus RowCursor::init(std::unique_ptr<Schema> schema) {
...
OLAPStatus RowCursor::init(const TabletSchema& schema, size_t column_count) {
    if (column_count > schema.num_columns()) {
        LOG(WARNING) << "Input param are invalid. Column count is bigger than num_columns of schema. "
                     << "column_count=" << column_count << ", schema.num_columns=" << schema.num_columns();
...
OLAPStatus RowCursor::init_scan_key(const TabletSchema& schema, const std::vector<std::string>& scan_keys) {
    size_t scan_key_size = scan_keys.size();
    if (scan_key_size > schema.num_columns()) {
        LOG(WARNING) << "Input param are invalid. Column count is bigger than num_columns of schema. "
                     << "column_count=" << scan_key_size << ", schema.num_columns=" << schema.num_columns();
        return OLAP_ERR_INPUT_PARAMETER_ERROR;
    }
...
OLAPStatus RowCursor::from_tuple(const OlapTuple& tuple) {
    ...
    for (size_t i = 0; i < tuple.size(); ++i) {
        ...
        if (res != OLAP_SUCCESS) {
            LOG(WARNING) << "fail to convert field from string. string=" << tuple.get_value(i) << ", res=" << res;
            return res;
        }
    }
