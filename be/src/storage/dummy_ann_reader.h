...
namespace starrocks {
class DummyAnnReader : public VectorIndexReader {
public:
    ~DummyAnnReader() {
        delete searcher;
    }

	Status _init_hnsw_searcher(const tenann::IndexMeta& meta) {
		searcher = new DummyAnnSearcher(meta);
        return Status::OK();
	}

	Status search(tenann::PrimitiveSeqView query_vector, int k, int64_t* result_ids, uint8_t* result_distances,
                tenann::IFilter* id_filter = nullptr) {
        searcher.dummyAnnSearch(result_ids, result_distances);
        return Status::OK();
	};
...
private:
	DummyAnnSearcher* searcher = nullptr;
};
