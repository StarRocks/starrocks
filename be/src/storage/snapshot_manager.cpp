...
StatusOr<std::string> SnapshotManager::snapshot_full(const TabletSharedPtr& tablet, int64_t snapshot_version,
    ...
    if (tablet->updates() == nullptr) {
        ...
        /*
            should hold tablet->updates()->_lock during get_applied_rowsets
            and make_snapshot_on_tablet_meta. If hold the lock successfully,
            there are three possible situations that can happen:

            (suppose _edit_version_infos.back() is (12.5), we want to snapshot major = 12)
            1. there is no compaction or compaction has been finished (including apply),
               erverything is fine.
            2. the lock block the _commit_compaction, (12.6) can not be added into _edit_version_infos
               we still get (12.5) in get_applied_rowsets and delvector for version 12 which is generated
               by (12.5)
            3. the lock does not block the _commit_compaction but block _apply_compaction_commit. In this case,
               (12.6) has been added into _edit_version_infos. But _apply_version_idx and delvector has not been
               updated yet because of this lock. we still get (12.5) in get_applied_rowsets and delvector for
               version 12 which is generated by (12.5)
        */
        tablet->updates()->get_lock(&ul);
        RETURN_IF_ERROR(tablet->updates()->get_applied_rowsets_unlocked(snapshot_version, &snapshot_rowsets));
    }
    tablet->generate_tablet_meta_copy_unlocked(snapshot_tablet_meta);
    rdlock.unlock();
...
StatusOr<std::string> SnapshotManager::snapshot_primary(const TabletSharedPtr& tablet,
    ...
    if (st.ok() && (snapshot_rowsets.empty() || need_full_snapshot)) {
        ...
        */
        tablet->updates()->get_lock(&ul);
        st = tablet->updates()->get_applied_rowsets_unlocked(full_snapshot_version, &snapshot_rowsets);
