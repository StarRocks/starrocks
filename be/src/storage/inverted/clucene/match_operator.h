...
public:
    ...
    Status match(roaring::Roaring* result, std::vector<float_t>* scores);
...
public:
    MatchPhraseOperator(lucene::search::IndexSearcher* searcher, lucene::store::Directory* dir, std::wstring field_name,
                        const std::vector<std::string>& terms, int slop)
            : MatchOperator(searcher, dir, std::move(field_name)), _terms(terms), _slop(slop) {}
