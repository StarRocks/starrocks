...
public:
    ...
    Status try_query(OlapReaderStatistics* stats, const std::string& column_name, const void* query_value,
...
class StringCLuceneInvertedReader : public CLuceneInvertedReader {
public:
    explicit StringCLuceneInvertedReader(std::string path, const uint32_t index_id, InvertedIndexParserType parser_type)
            : CLuceneInvertedReader(std::move(path), index_id), _parser_type(parser_type) {}

    // query_value to output result to bitmap
    Status query(OlapReaderStatistics* stats, const std::string& column_name, const void* query_value,
                 InvertedIndexQueryType query_type, roaring::Roaring* bit_map) override;
    ...
    InvertedIndexReaderType get_inverted_index_reader_type() override { return InvertedIndexReaderType::STRING; }

private:
    InvertedIndexParserType _parser_type;
};

class BKDIndexReader : public CLuceneInvertedReader {
public:
    explicit BKDIndexReader(std::string path, const uint32_t index_id, LogicalType field_type)
            : CLuceneInvertedReader(std::move(path), index_id), _field_type(field_type) {}
    Status query(OlapReaderStatistics* stats, const std::string& column_name, const void* query_value,
                 InvertedIndexQueryType query_type, roaring::Roaring* bit_map) override;
    ...
    InvertedIndexReaderType get_inverted_index_reader_type() override { return InvertedIndexReaderType::NUMERIC; }

private:
    LogicalType _field_type;
};
