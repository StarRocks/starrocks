...
public:
    ...
    Status init_bkd_index() {
        ...
        _bkd_writer = std::make_shared<lucene::util::bkd::bkd_writer>(max_doc, DIMS, DIMS, value_length, MAX_LEAF_COUNT,
                                                                      MAXMBSortInHeap, total_point_count, true,
                                                                      MAX_DEPTH_IN_BKD_TREE);
        return Status::OK();
    }
    ...
    void add_array_values(size_t field_size, const Collection* values, size_t count) override {
        if constexpr (is_string_type(field_type)) {
            if (_field == nullptr || _index_writer == nullptr) {
                LOG(ERROR) << "field or index writer is null in inverted index writer.";
            }
            for (int i = 0; i < count; ++i) {
                auto* item_data_ptr = const_cast<Collection*>(values)->data;
    ...
    Status finish() override {
        ...
        try {
            ...
            if constexpr (is_numeric_type(field_type)) {
                ...
                data_out = dir->createOutput(IndexDescriptor::get_temporary_bkd_index_data_file_name().c_str());
                meta_out = dir->createOutput(IndexDescriptor::get_temporary_bkd_index_meta_file_name().c_str());
                index_out = dir->createOutput(IndexDescriptor::get_temporary_bkd_index_file_name().c_str());
                if (data_out != nullptr && meta_out != nullptr && index_out != nullptr) {
                    _bkd_writer->meta_finish(meta_out, _bkd_writer->finish(data_out, index_out), int(field_type));
                }
