...
TabletMeta::TabletMeta(int64_t table_id, int64_t partition_id, int64_t tablet_id, int32_t schema_hash,
    ...
    tablet_meta_pb.set_use_persistent_index(use_persistent_index);
...
void TabletMeta::init_from_pb(TabletMetaPB* ptablet_meta_pb) {
    ...
    // _use_persistent_index decide use persistent index in primary index or not
    // it is assigned when create tablet, and it can not be changed so far
    if (tablet_meta_pb.has_use_persistent_index()) {
        _use_persistent_index = tablet_meta_pb.use_persistent_index();
    } else {
        _use_persistent_index = false;
    }
