...
Status RowsetColumnUpdateState::_load_upserts(Rowset* rowset, MemTracker* update_mem_tracker, uint32_t start_idx,
    ...
    if (!PrimaryKeyEncoder::create_column(pkey_schema, &pk_column, PrimaryKeyEncodingType::V1).ok()) {
        ...
        if (itr != nullptr) {
            ...
            while (true) {
                ...
                if (st.is_end_of_file()) {
                    ...
                    TRY_CATCH_BAD_ALLOC(
                            PrimaryKeyEncoder::encode(pkey_schema, *chunk, 0, chunk->num_rows(), col.get(), PrimaryKeyEncodingType::V1));
                }
            }
        }
...
Status RowsetColumnUpdateState::_update_primary_index(const TabletSchemaCSPtr& tablet_schema, Tablet* tablet,
    ...
    for (const auto& each_chunk : segid_to_chunk) {
        ...
        PrimaryKeyEncoder::encode(pkey_schema, *each_chunk.second, 0, each_chunk.second->num_rows(), pk_column.get(), PrimaryKeyEncodingType::V1);
