...
public:
    ...
    Status flush_to_immutable_index(const std::string& dir, const EditVersion& version) const override {
        auto [nshard, npage_hint] = estimate_nshard_and_npage(KeySize + kIndexValueSize, size(), default_usage_percent);
...
Status PersistentIndex::_flush_l0() {
    auto [nshard, npage_hint] = estimate_nshard_and_npage(kv_pair_size(), _size, default_usage_percent);
