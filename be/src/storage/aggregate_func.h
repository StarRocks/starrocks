...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/aggregate_func.h
...
public:
    ...
    inline void finalize(RowCursorCell* src, MemPool* mem_pool) const { _finalize_fn(src, mem_pool); }
...
struct AggregateFuncTraits<OLAP_FIELD_AGGREGATION_NONE, OLAP_FIELD_TYPE_DATETIME>
        : public BaseAggregateFuncs<OLAP_FIELD_TYPE_DATETIME> {
    static void init(RowCursorCell* dst, const char* src, bool src_null, MemPool* mem_pool, ObjectPool* agg_pool) {
        dst->set_is_null(src_null);
        if (src_null) {
            return;
...
template <>
struct AggregateFuncTraits<OLAP_FIELD_AGGREGATION_NONE, OLAP_FIELD_TYPE_TIMESTAMP>
        : public BaseAggregateFuncs<OLAP_FIELD_TYPE_TIMESTAMP> {
    static void init(RowCursorCell* dst, const char* src, bool src_null, MemPool* mem_pool, ObjectPool* agg_pool) {
        dst->set_is_null(src_null);
        if (src_null) {
            return;
        ...
        auto* datetime_value = reinterpret_cast<const DateTimeValue*>(src);
        auto* storage_date_value = reinterpret_cast<vectorized::TimestampValue*>(dst->mutable_cell_ptr());
        storage_date_value->from_timestamp(datetime_value->year(), datetime_value->month(), datetime_value->day(),
...
struct AggregateFuncTraits<OLAP_FIELD_AGGREGATION_NONE, OLAP_FIELD_TYPE_DATE>
        : public BaseAggregateFuncs<OLAP_FIELD_TYPE_DATE> {
    static void init(RowCursorCell* dst, const char* src, bool src_null, MemPool* mem_pool, ObjectPool* agg_pool) {
        dst->set_is_null(src_null);
        if (src_null) {
            return;
...
template <>
struct AggregateFuncTraits<OLAP_FIELD_AGGREGATION_NONE, OLAP_FIELD_TYPE_DATE_V2>
        : public BaseAggregateFuncs<OLAP_FIELD_TYPE_DATE_V2> {
    // TODO(zc): If we support vectorized load later, we should add another iterface,
    // maybe init_by_column_data
    static void init(RowCursorCell* dst, const char* src, bool src_null, MemPool* mem_pool, ObjectPool* agg_pool) {
        dst->set_is_null(src_null);
        if (src_null) {
            return;
        ...
        auto* date_value = reinterpret_cast<const DateTimeValue*>(src);
        auto* storage_date_value = reinterpret_cast<vectorized::DateValue*>(dst->mutable_cell_ptr());
        storage_date_value->from_date(date_value->year(), date_value->month(), date_value->day());
    }
};
...
template <>
struct AggregateFuncTraits<OLAP_FIELD_AGGREGATION_MIN, OLAP_FIELD_TYPE_DECIMAL_V2>
        : public AggregateFuncTraits<OLAP_FIELD_AGGREGATION_MIN, OLAP_FIELD_TYPE_LARGEINT> {};
...
template <>
struct AggregateFuncTraits<OLAP_FIELD_AGGREGATION_MAX, OLAP_FIELD_TYPE_DECIMAL_V2>
        : public AggregateFuncTraits<OLAP_FIELD_AGGREGATION_MAX, OLAP_FIELD_TYPE_LARGEINT> {};
...
template <>
struct AggregateFuncTraits<OLAP_FIELD_AGGREGATION_SUM, OLAP_FIELD_TYPE_DECIMAL_V2>
        : public AggregateFuncTraits<OLAP_FIELD_AGGREGATION_SUM, OLAP_FIELD_TYPE_LARGEINT> {};
...
struct AggregateFuncTraits<OLAP_FIELD_AGGREGATION_BITMAP_UNION, OLAP_FIELD_TYPE_OBJECT> {
    static void init(RowCursorCell* dst, const char* src, bool src_null, MemPool* mem_pool, ObjectPool* agg_pool) {
        ...
        // we use zero size represent this slice is a agg object
        dst_slice->size = 0;
        BitmapValue* bitmap = nullptr;
        if (!src_null) {
            bitmap = new BitmapValue(src_slice->data);
        } else {
            bitmap = new BitmapValue(nullptr);
        }
...
template <>
struct AggregateFuncTraits<OLAP_FIELD_AGGREGATION_PERCENTILE_UNION, OLAP_FIELD_TYPE_PERCENTILE> {
    static void init(RowCursorCell* dst, const char* src, bool src_null, MemPool* mem_pool, ObjectPool* agg_pool) {
        dst->set_not_null();
        auto* src_slice = reinterpret_cast<const Slice*>(src);
        auto* dst_slice = reinterpret_cast<Slice*>(dst->mutable_cell_ptr());
        ...
        if (src_null) {
            percentile = new PercentileValue();
        } else {
            percentile = new PercentileValue(src_slice->data);
        }
    ...
    static void update(RowCursorCell* dst, const RowCursorCell& src, MemPool* mem_pool) {
        ...
        auto dst_slice = reinterpret_cast<Slice*>(dst->mutable_cell_ptr());
        DCHECK_EQ(dst_slice->size, 0);
        auto dst_percentile = reinterpret_cast<PercentileValue*>(dst_slice->data);
        auto src_slice = reinterpret_cast<const Slice*>(src.cell_ptr());

        if (mem_pool == nullptr) { // for query
            PercentileValue src_digest(src_slice->data);
            dst_percentile->merge(&src_digest);
        } else { // for stream load
            DCHECK_EQ(src_slice->size, 0);
            PercentileValue* src_percentile = reinterpret_cast<PercentileValue*>(src_slice->data);
            dst_percentile->merge(src_percentile);
        }
    }

    // The TDigest object memory will be released by ObjectPool
    static void finalize(RowCursorCell* src, MemPool* mem_pool) {
        auto slice = reinterpret_cast<Slice*>(src->mutable_cell_ptr());
        DCHECK_EQ(slice->size, 0);
        auto percentile = reinterpret_cast<PercentileValue*>(slice->data);

        slice->size = percentile->serialize_size();
        slice->data = (char*)mem_pool->allocate(slice->size);
        percentile->serialize((uint8_t*)slice->data);
    }
};
...
const AggregateInfo* get_aggregate_info(const FieldAggregationMethod agg_method, const FieldType field_type);
} // namespace starrocks
