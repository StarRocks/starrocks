...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/reader.h
...
#ifndef STARROCKS_BE_SRC_OLAP_READER_H
#define STARROCKS_BE_SRC_OLAP_READER_H
...
struct ReaderParams {
    ...
    // The RowsetReader will be set when using Merger, eg Cumulative, BE.
    ...
    std::string to_string() {
        std::stringstream ss;
        ss << "tablet=" << tablet->full_name() << " reader_type=" << reader_type << " aggregation=" << aggregation
           << " version=" << version << " range=" << range << " end_range=" << end_range;
...
private:
    ...
    std::vector<const ColumnPredicate*> _col_predicates;
    DeleteHandler _delete_handler;

    OLAPStatus (Reader::*_next_row_func)(RowCursor* row_cursor, MemPool* mem_pool, ObjectPool* agg_pool,
                                         bool* eof) = nullptr;
...
#endif // STARROCKS_BE_SRC_OLAP_READER_H
