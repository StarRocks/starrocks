...
std::vector<TabletSharedPtr> TabletManager::find_best_tablets_to_compaction(CompactionType compaction_type,
                                                                            DataDir* data_dir) {
    using TabletScorePair = std::pair<TabletSharedPtr, int32_t>;

    struct CmpScore {
        bool operator()(const TabletScorePair& left, const TabletScorePair& right) {
    ...
    std::priority_queue<TabletScorePair, std::vector<TabletScorePair>, CmpScore> best_tablets;
    int i = 0;
    for (const auto& tablets_shard : _tablets_shards) {
        ...
        for (auto [tablet_id, tablet_ptr] : tablets_shard.tablet_map) {
            ...
            if (i < config::cumulative_compaction_batch) {
    ...
    std::vector<TabletSharedPtr> tablets(best_tablets.size());
    std::string tablet_str;
    std::string score_str;
    size_t idx = best_tablets.size() - 1;
    while (!best_tablets.empty()) {
        tablets[idx] = best_tablets.top().first;
        highest_score = best_tablets.top().second;
        tablet_str.append(std::to_string(tablets[idx]->tablet_id()));
        tablet_str.append(",");
        score_str.append(std::to_string(highest_score));
        score_str.append(",");
        idx--;
        best_tablets.pop();
    }
    ...
    if (!tablets.empty()) {
        LOG(INFO) << "Found the best tablet to compact. "
                  << "compaction_type=" << compaction_type_str << " tablet_id=[" << tablet_str
                  << "] highest_score=[" << score_str << "]";
...
Status TabletManager::_create_tablet_meta_unlocked(const TCreateTabletReq& request, DataDir* store,
    ...
    return TabletMeta::create(norm_request, TabletUid::gen_uid(), shard_id, next_unique_id, col_idx_to_unique_id,
