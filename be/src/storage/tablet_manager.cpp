...
Status TabletManager::drop_tablet(TTabletId tablet_id, TabletDropFlag flag, bool pk_wait_for_apply_done) {
    TabletSharedPtr dropped_tablet = nullptr;
    {
        ...
        if (!pk_wait_for_apply_done && dropped_tablet->keys_type() == KeysType::PRIMARY_KEYS &&
            !dropped_tablet->updates()->stop_and_check_apply_not_running()) {
            std::stringstream ss;
            ss << "apply task of primary key tablet: " << tablet_id << " is still running, retry drop tablet later";
            LOG(WARNING) << ss.str();
