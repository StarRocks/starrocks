...
>>>>>>> 68d879914c ([BugFix] Fix PK tablet rowset meta loss caused by GC race during disk re-migration (A→B→A) (#70727))
...
#include "util/starrocks_metrics.h"
        ...
        if (old_tablet == nullptr) {
            ...
            if (old_tablet->schema_hash_path() == new_tablet->schema_hash_path()) {
                ...
                return Status::InternalError(
                        fmt::format("tablet already exists, tablet_id: {}", old_tablet->tablet_id()));
            }
    ...
    TabletSharedPtr TabletManager::_internal_create_tablet_unlocked(
            AlterTabletType alter_type, const TCreateTabletReq& request, bool is_schema_change,
    ...
    TabletSharedPtr TabletManager::_create_tablet_meta_and_dir_unlocked(
            const TCreateTabletReq& request, bool is_schema_change, const Tablet* base_tablet,
    ...
    TabletSharedPtr TabletManager::find_best_tablet_to_compaction(CompactionType compaction_type, DataDir * data_dir,
        ...
        const std::string& compaction_type_str =
            ...
            for (auto [tablet_id, tablet_ptr] : _tablets_shards[i].tablet_map) {
                ...
                if (compaction_type == CompactionType::CUMULATIVE_COMPACTION) {
                    if (tablet_ptr->last_cumu_compaction_failure_status() == TStatusCode::NOT_FOUND) {
                        if (now_ms - last_failure_ts <=
                            5 * config::cumulative_compaction_check_interval_seconds * 1000) {
                    ...
                    if (now_ms - last_failure_ts <= config::min_compaction_failure_interval_sec * 1000) {
                        VLOG(2) << "Too often to schedule failure compaction, skip it."
                                << "compaction_type=" << compaction_type_str << ", min_compaction_failure_interval_sec="
    ...
    TabletSharedPtr TabletManager::find_best_tablet_to_do_update_compaction(DataDir * data_dir) {
        ...
        for (const auto& tablets_shard : _tablets_shards) {
            std::vector<TabletSharedPtr> all_tablets_by_shard =
                    _get_all_tablets_from_shard(tablets_shard, PRIMARY_KEYS);
    ...
    Status TabletManager::load_tablet_from_meta(DataDir * data_dir, TTabletId tablet_id, TSchemaHash schema_hash,
                                                std::string_view meta_binary, bool update_meta, bool force,
                                                bool restore, bool check_path) {
        std::unique_lock wlock(_get_tablets_shard_lock(tablet_id));
        ...
        if (tablet->tablet_state() == TABLET_SHUTDOWN) {
            if (init_st.ok()) {
                ...
                LOG(WARNING) << "Loaded shutdown tablet " << tablet_id
                             << " with ignored failure: " << init_st.to_string();
            }
        ...
        if (old_tablet_ptr != nullptr) {
            if (old_tablet_ptr->tablet_state() == TabletState::TABLET_SHUTDOWN) {
                ...
                do {
                    ...
                    if (st.ok() || st.is_not_found()) {
                        LOG(INFO) << "before adding new cloned tablet, delete stale TABLET_SHUTDOWN tablet:"
                                  << tablet_id << " successfully, retried "
                                  << RETRY_TIMES_ON_SHUTDOWN_TABLET_OCCUPIED - retry << " times";
                        break;
    ...
    Status TabletManager::create_tablet_from_meta_snapshot(
            DataDir * store, TTabletId tablet_id, SchemaHash schema_hash, const string& schema_hash_path, bool restore,
            bool need_rebuild_pk_index, int32_t rebuild_pk_index_wait_seconds) {
        ...
        RETURN_IF_ERROR(
                SnapshotManager::instance()->assign_new_rowset_id(snapshot_meta, schema_hash_path, tablet_schema));
    ...
    std::unordered_map<TTabletId, vector<pair<uint32_t, uint32_t>>>
    TabletManager::get_tablets_need_repair_compaction() {
    ...
    void TabletManager::_add_shutdown_tablet_unlocked(int64_t tablet_id, DroppedTabletInfo && drop_info) {
        ...
        if (iter != _shutdown_tablets.end()) {
            if ((iter->second).tablet != nullptr) {
                ...
                if (st.ok() && (tablet_meta.tablet_uid() != tablet->tablet_uid() ||
                                tablet_meta.tablet_state() != TABLET_SHUTDOWN)) {
