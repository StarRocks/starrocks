...
// Last, and deliberately: butil defines a PACKED macro that collides with a template parameter of
// that name in this branch's fmt, so anything that formats must be parsed before these.
...
// load-bearing: get_key_coder resolves the delegate internally, but CppTypeTraits<LT>::CppType and the
...
Status decode_fixed_key_column(Slice* s, MemPool* pool, Datum* out) {
    ...
    CppType raw{};
    RETURN_IF_ERROR(
            get_key_coder(LT)->decode_ascending(s, TypeTraits<LT>::size, reinterpret_cast<uint8_t*>(&raw), pool));
    out->set<CppType>(raw);
    return Status::OK();
}
...
StatusOr<SortKeySamples> sample_sort_key_from_short_key_index(Segment& segment, const Schema& schema,
    ...
    // Metadata (SegmentMetadataPB num_rows) and the segment footer (segment.num_rows()) must agree,
    // and the short key footer's own (num_rows_per_block, num_items) must satisfy the identity
    // below. This branch's ShortKeyFooterPB carries no segment-wide row count of its own, so the
    // index footer cannot be cross-checked against the other two directly; the entry-0-vs-min_key
    // comparison further down is what catches an index that does not describe this segment. Path B
    // makes the same metadata-vs-footer check.
    //
    ...
    //     num_rows == segment.num_rows() clause would re-open a release-silent mis-weighting that
    //     the consumer cannot detect (its own DCHECKs compile out);
    ...
    if (block_rows <= 0 || num_rows <= 0 || num_rows != static_cast<int64_t>(segment.num_rows()) ||
        num_items != (num_rows + block_rows - 1) / block_rows) {
        g_sort_key_sampling_short_key_index_fallback_total << 1;
        return out;
    }
    if (num_items < 2) {
...
StatusOr<SortKeySamples> sample_sort_key_from_segment_data(Segment& segment, const TabletSchemaCSPtr& segment_schema,
    ...
    auto chunk = ChunkHelper::new_chunk(sort_key_schema, static_cast<size_t>(num_ordinals));
    ...
    for (int64_t k = 0; k < num_ordinals; ++k) {
        VariantTuple sample = chunk->get(static_cast<size_t>(k), positions);
