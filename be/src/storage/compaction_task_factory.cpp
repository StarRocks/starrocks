...
std::shared_ptr<CompactionTask> CompactionTaskFactory::create_compaction_task() {
    ...
    VLOG(2) << "choose algorithm:" << algorithm_to_string(algorithm) << ", for tablet:" << _tablet->tablet_id()
            << ", segment_iterator_num:" << segment_iterator_num << ", max_columns_per_group:" << max_columns_per_group
