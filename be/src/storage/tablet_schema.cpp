...
void TabletSchema::_init_from_pb(const TabletSchemaPB& schema) {
    ...
    // The following cases which has empty distribution key column ids:
    // 1. The tablet is created in the older version and upgrade.
    // 2. The table does not organized by hash distribution.
    if (!schema.distribution_key_column_ids().empty()) {
        for (const auto& cid : schema.distribution_key_column_ids()) {
            _distribution_key_column_ids.emplace_back(cid);
        }
    }
}
...
Status TabletSchema::_build_current_tablet_schema(int64_t schema_id, int32_t version,
    ...
    _distribution_key_column_ids = ori_tablet_schema.distribution_key_column_ids();
    return Status::OK();
}
...
void TabletSchema::to_schema_pb(TabletSchemaPB* tablet_schema_pb) const {
    ...
    for (const auto& cid : _distribution_key_column_ids) {
        tablet_schema_pb->add_distribution_key_column_ids(cid);
    }
}
