...
struct ColumnPtrBuilder {
    ...
    MutableColumnPtr operator()(const Field& field, int precision, int scale) {
        ...
        if constexpr (ftype == TYPE_ARRAY) {
            auto elements = NullableColumn::wrap_if_necessary(field.sub_field(0).create_column())->assume_mutable();
            ...
            auto keys = NullableColumn::wrap_if_necessary(field.sub_field(0).create_column())->assume_mutable();
            auto values = NullableColumn::wrap_if_necessary(field.sub_field(1).create_column())->assume_mutable();
