...
VectorizedField ChunkHelper::convert_field(ColumnId id, const TabletColumn& c) {
    ...
    VectorizedField f(id, std::string(c.name()), type_info, c.is_nullable());
...
VectorizedSchema ChunkHelper::convert_schema(const TabletSchema& schema) {
    VectorizedFields fields;
    for (ColumnId cid = 0; cid < schema.num_columns(); ++cid) {
        auto f = convert_field(cid, schema.column(cid));
        fields.emplace_back(std::make_shared<VectorizedField>(std::move(f)));
    }
    return VectorizedSchema(std::move(fields), schema.keys_type(), schema.sort_key_idxes());
}

VectorizedField ChunkHelper::convert_field_to_format_v2(ColumnId id, const TabletColumn& c) {
...
VectorizedSchema ChunkHelper::convert_schema_to_format_v2(const TabletSchema& schema) {
    return VectorizedSchema(schema.schema());
}

VectorizedSchema ChunkHelper::convert_schema_to_format_v2(const TabletSchema& schema,
                                                          const std::vector<ColumnId>& cids) {
    return VectorizedSchema(schema.schema(), cids);
}

VectorizedSchema ChunkHelper::get_short_key_schema_with_format_v2(const TabletSchema& schema) {
    ...
    return VectorizedSchema(schema.schema(), short_key_cids);
}

ColumnId ChunkHelper::max_column_id(const VectorizedSchema& schema) {
    ColumnId id = 0;
