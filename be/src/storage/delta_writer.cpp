...
Status DeltaWriter::commit() {
    ...
    if (_tablet->keys_type() == KeysType::PRIMARY_KEYS && !config::skip_pk_preload &&
        ...
        FAIL_POINT_TRIGGER_ASSIGN_STATUS_OR_DEFAULT(
                load::failpoint::PK_PRELOAD, st, PK_PRELOAD_FP_ACTION(_opt.txn_id, _opt.tablet_id),
    ...
    Status res;
    FAIL_POINT_TRIGGER_ASSIGN_STATUS_OR_DEFAULT(
            load::failpoint::COMMIT_TXN, res, COMMIT_TXN_FP_ACTION(_opt.txn_id, _opt.tablet_id),
