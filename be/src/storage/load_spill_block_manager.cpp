...
Status LoadSpillBlockMergeExecutor::init() {
    RETURN_IF_ERROR(ThreadPoolBuilder("load_spill_block_merge")
                            ...
                            .build(&_merge_pool));
    RETURN_IF_ERROR(ThreadPoolBuilder("in_tablet_parallel_merge")
                            .set_min_threads(1)
                            .set_max_threads(calc_max_merge_blocks_thread())
                            .set_max_queue_size(40960 /*a random chosen number that should big enough*/)
                            .set_idle_timeout(MonoDelta::FromMilliseconds(/*5 minutes=*/5 * 60 * 1000))
                            .build(&_in_tablet_parallel_merge_pool));
    return Status::OK();
}
...
Status LoadSpillBlockMergeExecutor::refresh_max_thread_num() {
    ...
    if (_in_tablet_parallel_merge_pool != nullptr) {
        RETURN_IF_ERROR(_in_tablet_parallel_merge_pool->update_max_threads(calc_max_merge_blocks_thread()));
    }
    return Status::OK();
}
...
std::unique_ptr<ThreadPoolToken> LoadSpillBlockMergeExecutor::create_in_tablet_parallel_merge_token() {
    return _in_tablet_parallel_merge_pool->new_token(ThreadPool::ExecutionMode::CONCURRENT);
}
