...
#ifndef STARROCKS_BE_SRC_OLAP_SKIPLIST_H
#define STARROCKS_BE_SRC_OLAP_SKIPLIST_H
...
typename SkipList<Key, Comparator>::Node* SkipList<Key, Comparator>::NewNode(const Key& key, int height) {
    char* mem = (char*)_mem_pool->allocate(sizeof(Node) + sizeof(std::atomic<Node*>) * (height - 1));
...
#endif // STARROCKS_BE_SRC_OLAP_SKIPLIST_H
