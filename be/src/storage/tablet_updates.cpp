...
Status TabletUpdates::compaction(MemTracker* mem_tracker) {
    ...
    for (auto& e : candidates) {
        ...
        if (info->inputs.size() > 0 && (new_rows > config::update_compaction_result_rows * 2 ||
                                        new_bytes > config::update_compaction_result_bytes * 2)) {
            break;
        }
        ...
        if (total_bytes_after_compaction > config::update_compaction_result_bytes ||
            total_rows_after_compaction > config::update_compaction_result_rows ||
