...

    {
        ...
        if (rowset_meta_pb.has_txn_meta()) {
            ...
            st = TabletMetaManager::apply_rowset_commit(_tablet.data_dir(), tablet_id, _next_log_id, version,
                                                        new_del_vecs, &(rowset->rowset_meta()->get_meta_pb()));
        } else {
            st = TabletMetaManager::apply_rowset_commit(_tablet.data_dir(), tablet_id, _next_log_id, version,
                                                        new_del_vecs, nullptr);
        }
...
>>>>>>> 2f2dfb844 ([BugFix] keep at least one version whose create_time is before expire time in remove_expire_versions (#15654))
            break;
        }
        keep_index_min--;
    }
