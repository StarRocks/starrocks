...
Status PrimaryIndex::_upsert_into_persistent_index(uint32_t rssid, uint32_t rowid_start, const Column& pks,
    ...
    Status st;
    uint32_t n = idx_end - idx_begin;
    cb->params.back()->values.reserve(n);
    cb->params.back()->old_values.resize(n, NullIndexValue);
    const Slice* vkeys = build_persistent_keys(pks, _key_size, idx_begin, idx_end, &cb->params.back()->keys);
    RETURN_IF_ERROR(_build_persistent_values(rssid, rowid_start, idx_begin, idx_end, &cb->params.back()->values));
    RETURN_IF_ERROR(_persistent_index->upsert(n, vkeys, reinterpret_cast<IndexValue*>(cb->params.back()->values.data()),
                                              reinterpret_cast<IndexValue*>(cb->params.back()->old_values.data()), stat,
                                              cb));
    return st;
}
...
Status PrimaryIndex::_upsert_into_persistent_index(uint32_t rssid, uint32_t rowid_start, const Column& pks,
    ...
    RETURN_IF_ERROR(_upsert_into_persistent_index(rssid, rowid_start, pks, idx_begin, idx_end, stat, &cb));
    for (unsigned long old : cb.params.back()->old_values) {
...
Status PrimaryIndex::upsert(uint32_t rssid, uint32_t rowid_start, const Column& pks, IOStat* stat,
    ...
    DCHECK(_status.ok() && (_pkey_to_rssid_rowid || _persistent_index));
    if (_persistent_index != nullptr) {
        return _upsert_into_persistent_index(rssid, rowid_start, pks, 0, pks.size(), stat, cb);
    } else {
