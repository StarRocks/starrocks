...
Status DictionaryCacheManager::refresh(const PProcessDictionaryCacheRequest* request) {
    ...
    dictionary_schema = ChunkBuilder::get_non_nullable_schema(dictionary_schema, &keys);
    ...
    ChunkPtr key_chunk = ChunkBuilder::new_chunk(*key_schema.get(), chunk->num_rows());
    ChunkPtr value_chunk = ChunkBuilder::new_chunk(*value_schema.get(), chunk->num_rows());
