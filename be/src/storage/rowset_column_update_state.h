...
struct ColumnPartialUpdateState {
    ...
    // rssid -> source rowid -> update file's LOGICAL rowid (chunk-local index in the
    // iterator's emit sequence). Consumed by _update_source_chunk_by_upt, which reads
    // the update file through the range-pruned iterator and indexes the emitted chunk
    // via append_selective — so this must stay logical.
    std::map<uint32_t, std::vector<RowidPairs>> rss_rowid_to_update_rowid;
    // PHYSICAL rowids in the update segment file of rows that need to be inserted
    // (their PKs do not exist in the source PK index), stored as
    // upt_segment_physical_rowid_offset + logical_offset. Consumers read the full
    // update segment and address rows by physical position (the lake path via
    // fetch_values_by_rowid; the non-lake path via append_selective on a full-file
    // read). For non-shared segments the base is 0, so physical == logical.
    std::vector<uint32> insert_rowids;
    // First physical rowid emitted by the source iterator for this update segment
    // (the iterator's range_start; 0 for non-shared segments and for the non-lake
    // path). Added to each insert row's logical offset to produce its physical
    // position in the segment file.
    uint32_t upt_segment_physical_rowid_offset = 0;

    // build `rss_rowid_to_update_rowid` and `insert_rowids` from `src_rss_rowids`.
    // The two outputs use different coordinate systems on purpose (see field docs):
    // rss_rowid_to_update_rowid keeps logical offsets, insert_rowids are translated
    // to physical positions via upt_segment_physical_rowid_offset.
    void build_rss_rowid_to_update_rowid() {
        ...
        for (uint32_t upt_row_id = 0; upt_row_id < src_rss_rowids.size(); upt_row_id++) {
            ...
            if (each_rss_rowid < UINT64_MAX) {
                ...
                insert_rowids.push_back(upt_segment_physical_rowid_offset + upt_row_id);
            }
        }
