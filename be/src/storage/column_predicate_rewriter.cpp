...
Status ColumnPredicateRewriter::rewrite_predicate(PushDownPredicates& _predicates, ObjectPool* pool) {
    ...
    for (size_t i = 0; i < _column_size; i++) {
        ...
        if (_need_rewrite[cid]) {
            RETURN_IF_ERROR(_rewrite_predicate(_predicates, pool, field));
        }
    }
    return Status::OK();
}

StatusOr<bool> ColumnPredicateRewriter::_rewrite_predicate(PushDownPredicates& _predicates, ObjectPool* pool, const FieldPtr& field) {
    auto cid = field->id();
