...
template <>
struct BatchT<TYPE_BOOLEAN> {
    using value = ScalarColumnVectorBatch<typename CppTypeTraits<TYPE_TINYINT>::CppType>;
};
...
ArrayColumnVectorBatch::ArrayColumnVectorBatch(const TypeInfoPtr& type_info, bool is_nullable, size_t init_capacity,
    ...
    _item_offsets[0] = 0;
    ColumnVectorBatch::create(init_capacity, field->get_sub_field(0)->is_nullable(),
                              get_item_type_info(type_info.get()), field->get_sub_field(0), &_elements);
}
