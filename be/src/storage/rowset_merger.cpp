...
private:
    Status _do_merge_horizontally(Tablet& tablet, const starrocks::TabletSchemaCSPtr& tablet_schema, int64_t version,
        ...
        if (schema.sort_key_idxes().size() > 1) {
            if (!PrimaryKeyEncoder::create_column(schema, &sort_column, schema.sort_key_idxes(), PrimaryKeyEncodingType::V1).ok()) {
...
Status compaction_merge_rowsets(Tablet& tablet, int64_t version, const vector<RowsetSharedPtr>& rowsets,
    ...
    auto key_type = PrimaryKeyEncoder::encoded_primary_key_type(schema, schema.sort_key_idxes(), PrimaryKeyEncodingType::V1);
    switch (key_type) {
    case TYPE_BOOLEAN:
