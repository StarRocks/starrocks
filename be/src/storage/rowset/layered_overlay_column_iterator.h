...
//                Each layer is a `.spcols` FILE, ordered by version.
//
...
public:
    // One overlay layer over the base segment: a `.spcols` Segment v2 file (source_rowid column +
    // value columns), loaded on first use (zero or one file IO per layer). Positional apply via
    // binary search, presence pre-filter, version-ascending last-write-wins; materialization is
    // hidden behind _ensure_layer_loaded.
    struct SparseLayer {
        // === FILE layer state ===
        ...
        // === Shared layer state ===
        // The value column to read for this overlay (its real uid/type/nullability): selects the
        // `.spcols` value column.
        TabletColumn value_column;
        // The DCG entry version of this layer (used for version-ascending ordering / diagnostics).
        int64_t version = 0;
        // Number of rows (K): footer.num_rows of the `.spcols` file.
        int64_t row_count = 0;
        ...
        // `covered_known == false` => legacy / homogeneous single-class layer: the whole
    ...
    // ---- Pruning hooks ----
    // The base column's zone map / bloom filter describe the *base* values, which are stale for any row
    // covered by an overlay layer. ZONE MAP pruning is supported soundly (v1) by delegating to the base
    // and re-including overlay layers' presence ranges (see get_row_ranges_by_zone_map below). BLOOM /
    // ngram / dict / predicate-pushdown remain conservative no-prune (return index-absent / false), so a
    // matching overlay value is never wrongly pruned by a base-value bloom -- deferred to a later version.

    // SDCG zone-map pruning (v1). has_zone_map mirrors the base column (dense `.cols` or original base
    // segment column) -- pruning is only useful when the base can prune. get_row_ranges_by_zone_map
    // returns base_ranges (base zone-map) UNION each overlay layer's presence range when that layer's
    // `.spcols` value zone-map COULD match the predicate. Sound: a row whose overlay value matches is
    // inside its layer's presence range and thus never pruned; base zone-map's stale values for covered
    // rows only widen the kept set.
    bool has_zone_map() const override { return _base->has_zone_map(); }
    Status get_row_ranges_by_zone_map(const std::vector<const ColumnPredicate*>& predicates,
                                      const ColumnPredicate* del_predicate, SparseRange<>* row_ranges,
                                      CompoundNodeType pred_relation, const Range<>* src_range) override;
...
private:
    ...
    // reads the layer's `.spcols` (one IO pass).
    Status _ensure_layer_loaded(SparseLayer& l);

    // Zone-map pruning helper: does layer `l`'s `.spcols` value column POSSIBLY contain a value matching
    // `predicates` (under `pred_relation`)? Opens a lightweight value-column iterator on the layer segment
    // and queries its zone-map ONLY (no full value load). Returns true (conservative "keep") when the layer
    // has no value zone-map, so a matching overlay value can never be wrongly pruned.
    StatusOr<bool> _layer_value_could_match(SparseLayer& l, const std::vector<const ColumnPredicate*>& predicates,
    ...
    // Assemble the resolved winning values in ONE pass and write them with a SINGLE update_rows.
    // `winner_layer[rel]` is the index (into _layers) of the NEWEST layer covering dst row
    // `dst_offset + rel` (-1 if no layer covers it); `winner_local[rel]` is that layer's local value
    // ordinal. Replaces the previous per-layer update_rows, which for a variable-length (BinaryColumn)
    // dst rebuilt the WHOLE column per layer -- O(N*L) byte copies. See the .cpp comment.
    Status _finalize_winners(const std::vector<int32_t>& winner_layer, const std::vector<uint32_t>& winner_local,
                             size_t n, size_t dst_offset, Column* dst);
