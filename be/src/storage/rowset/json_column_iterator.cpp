...
StatusOr<std::unique_ptr<ColumnIterator>> create_json_flat_iterator(
    ...
    VLOG(2) << fmt::format("create_json_flat_iterator: source_paths={} target_paths={} need_remain={}",
                           fmt::join(source_paths, ","), fmt::join(target_paths, ","), need_remain);
...
StatusOr<std::unique_ptr<ColumnIterator>> create_json_dynamic_flat_iterator(
    ...
    VLOG(2) << fmt::format("create_json_flat_iterator: target_paths={} need_remain={}", fmt::join(target_paths, ","),
                           need_remain);
...
StatusOr<std::unique_ptr<ColumnIterator>> create_json_merge_iterator(
    ...
    VLOG(2) << fmt::format("create_json_flat_iterator: merge_paths={}", fmt::join(merge_paths, ","));
