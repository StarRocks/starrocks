...
Status BetaRowsetWriter::init() {
    ...
    _rowset_meta_pb->set_rowset_id_v2(_context.rowset_id.to_string());
    ...
    _rowset_meta_pb->set_rowset_type(_context.rowset_type);
...
StatusOr<RowsetSharedPtr> BetaRowsetWriter::build() {
    ...
    if (_context.tablet_schema->keys_type() == KeysType::PRIMARY_KEYS) {
        _rowset_meta_pb->set_num_delete_files(!_segment_has_deletes.empty() && _segment_has_deletes[0]);
