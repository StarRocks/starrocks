...
private:
    ...
    StatusOr<uint16_t> _filter_by_column_hash_modulus_predicate(Chunk* chunk, vector<rowid_t>* rowid);
...
Status SegmentIterator::_do_get_next(Chunk* result, vector<rowid_t>* rowid) {
    ...
    ASSIGN_OR_RETURN(chunk_size, _filter_by_column_hash_modulus_predicate(chunk, rowid));
...
Status SegmentIterator::_build_context(ScanContext* ctx) {
    ...
    for (size_t i = 0; i < early_materialize_fields; i++) {
        ...
        if (delete_pred_columns.count(f->id()) || output_columns.count(f->id()) ||
            column_hash_modulus_cols.count(f->id()) /* need decode to compute the hash predicate */) {
