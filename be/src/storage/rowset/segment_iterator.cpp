...
private:
    ...
    struct VectorIndexContext {
        ...
        VectorMetricType metric_type = VectorMetricType::kL2Distance;
    ...
    // Walk _dcgs newest -> oldest collecting files that contain |ucid|. Stop the FILE walk after the
    // first DENSE file (it is row-complete and becomes the base). Returns file hits in newest ->
    // oldest order; |has_sparse| is set true iff any collected SPARSE file is present.
...
SegmentIterator::SegmentIterator(std::shared_ptr<Segment> segment, Schema schema, const SegmentReadOptions& options)
    ...
    if (_opts.use_vector_index) {
        ...
        // The explicit bit supports negative similarity thresholds. The fallback keeps manually
        // constructed/legacy options that use the old non-negative sentinel convention working.
        _vector_index_ctx->has_vector_range =
                _opts.vector_search_option->has_vector_range || _opts.vector_search_option->vector_range >= 0;
        ...
        if (_vector_index_ctx->has_vector_range && _vector_index_ctx->refine_distance) {
...
// range; descending (cosine/IP) keeps similarity >= range. Bounds are inclusive, matching the FE's
// `<=` / `>=` conjunct forms.
static inline bool within_vector_range(float d, float range, bool ascending) {
    return ascending ? d <= range : d >= range;
}

static StatusOr<VectorMetricType> get_vector_metric_type(const TabletIndex& index) {
    auto it = index.common_properties().find("metric_type");
    if (it == index.common_properties().end()) {
        return Status::InvalidArgument("metric_type is missing from vector index metadata");
    }
    if (boost::iequals(it->second, "l2_distance")) {
        return VectorMetricType::kL2Distance;
    }
    if (boost::iequals(it->second, "cosine_similarity")) {
        return VectorMetricType::kCosineSimilarity;
    }
    if (boost::iequals(it->second, "inner_product")) {
        return VectorMetricType::kInnerProduct;
    }
    return Status::InvalidArgument(fmt::format("unsupported vector metric_type: {}", it->second));
}
...
Status SegmentIterator::_setup_brute_force_fallback(const TabletIndex& index) {
    ...
    ASSIGN_OR_RETURN(_vector_index_ctx->metric_type, get_vector_metric_type(index));
...
Status SegmentIterator::_init_ann_reader() {
    ...
    if (_vector_index_ctx->use_vector_index) {
        // Exact rescans reached from the ANN route still need the logical metric from tablet metadata.
        ASSIGN_OR_RETURN(_vector_index_ctx->metric_type, get_vector_metric_type(*tablet_index_meta));
    }
...
Status SegmentIterator::_get_row_ranges_by_vector_index() {
    ...
    if (pre_narrowed && !_vector_index_ctx->has_vector_range && config::enable_vector_index_topk_underfill_fallback) {
...
StatusOr<std::vector<SegmentIterator::DcgLayerHit>> SegmentIterator::_collect_dcg_layers(uint32_t ucid,
    ...
    *has_sparse = false;
    // iterate dcg from new ver to old ver
    for (const auto& dcg : _dcgs) {
        // Collect ALL file slots in this dcg carrying |ucid| (a converged lake DCG can hold the same
        // uid across multiple .spcols files at different versions). Collecting only the first (the
        // old get_column_idx behavior) silently drops the rest -> partial-overlap rows revert to base.
        std::vector<std::pair<int32_t, int32_t>> matches; // (file_idx, col_idx)
        if (!dcg->get_all_column_file_indices(ucid, &matches)) {
            continue; // this dcg carries no file slot for ucid
        }
        bool dcg_has_dense = false;
        for (const auto& m : matches) {
            DcgLayerHit hit;
            hit.dcg = dcg;
            hit.file_idx = m.first;
            hit.col_idx = m.second;
            hit.kind = dcg->file_kind(m.first);
            if (hit.kind == DeltaColumnFileKind::SPARSE_PERCOL) {
                *has_sparse = true; // OR-in across EVERY collected file, not just a terminal hit
            } else {
                dcg_has_dense = true;
            }
            hits.push_back(std::move(hit));
        }
        // A DENSE file is row-complete: stop the across-dcg walk AFTER fully scanning this dcg (older
        // dcgs are superseded). Never abort mid-dcg -- higher-versioned sparse slots in the SAME dcg
        // must still be collected. Ordering vs the dense base is resolved by file_version in
        // _build_overlay_column_iterator (collection order no longer implies version order). For lake
        // the outer loop is a single iteration anyway.
        if (dcg_has_dense) {
            break;
        }
    }
...
StatusOr<std::unique_ptr<ColumnIterator>> SegmentIterator::_build_overlay_column_iterator(
    ...
    // `hits` may now carry MULTIPLE files from one dcg in collection (file_idx) order, which is NOT
    // version order. The DENSE base is the NEWEST dense file (by per-file version), and only overlays
    // STRICTLY NEWER than it survive. With no dense base every collected overlay applies.
    ...
    for (const auto& h : hits) {
        if (h.kind == DeltaColumnFileKind::DENSE_COLS) {
            ...
            if (v >= dense_base_version) { // newest dense wins as the row-complete base
    ...
    for (const auto& hit : hits) {
        if (hit.kind != DeltaColumnFileKind::SPARSE_PERCOL) {
            continue; // skip the dense base (consumed above) — only sparse become layers
        }
        // Collection order no longer guarantees sparse-precede-dense (multiple files per dcg in
        // file_idx order). Filter explicitly: only sparse files STRICTLY NEWER than the dense base
        // survive (older ones are superseded by the row-complete dense). Use the per-file version.
    ...
    // A stable sort keeps any same-version layers in collection order.
...
Status SegmentIterator::_init_inverted_index_iterators() {
    ...
    for (const auto& pair : _opts.pred_tree.get_all_column_predicate_map()) {
        ...
        // SDCG: a column with any SPARSE overlay layer must NOT be probed via its base-segment
        // inverted (GIN) index. The base `.idx` reflects stale pre-overlay values and a sparse
        // `.spcols` carries no index of its own -- probing the base index would MATCH/prune on the
        // old values and return wrong rows for an overlay-rewritten column. Leave the iterator null
        // (== "no index") so _apply_inverted_index falls through to a normal value evaluation over
        // the overlay-applied column. Mirrors the _apply_bitmap_index sparse guard above;
        // metadata-driven (NOT flag-driven), same rationale as _init_column_iterator_by_cid.
        if (!_dcgs.empty()) {
            bool has_sparse = false;
            ASSIGN_OR_RETURN(auto hits, _collect_dcg_layers(static_cast<uint32_t>(ucid), &has_sparse));
