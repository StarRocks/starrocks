...
SegmentIterator::SegmentIterator(std::shared_ptr<Segment> segment, Schema schema, SegmentReadOptions options)
        : ChunkIterator(std::move(schema), options.chunk_size),
          ...
          _predicate_columns(_opts.chunk_pred->num_columns()) {
...
Status SegmentIterator::_init_column_iterators(const Schema& schema) {
    DCHECK_EQ(_predicate_columns, _opts.chunk_pred->num_columns());
    ...
    bool has_predicate = !_opts.chunk_pred->empty();
    ...
    for (const FieldPtr& f : schema.fields()) {
        ...
        if (_column_iterators[cid] == nullptr) {
            ...
            if (_opts.global_dictmaps->count(cid)) {
                ...
                check_dict_enc = true;
            } else if (_opts.chunk_pred->contains_column(cid)) {
...
void SegmentIterator::_init_column_predicates() {
    DCHECK_EQ(_predicate_columns, _opts.chunk_pred->num_columns());
    // TODO(lzh): initlize _vectorized_preds, _branchless_preds, _expr_ctx_preds
...
Status SegmentIterator::_get_row_ranges_by_zone_map() {
    ...
    // -------------------------------------------------------------
    for (const auto& [cid, _] : _opts.chunk_pred->get_all_column_preds()) {
        columns.insert(cid);
    }
...
Status SegmentIterator::_do_get_next(Chunk* result, vector<rowid_t>* rowid) {
    ...
    const bool has_predicate = !_opts.chunk_pred->empty();
...
inline bool SegmentIterator::_can_using_dict_code(const FieldPtr& field) const {
    ...
    if (_opts.chunk_pred->contains_column(field->id())) {
        return _predicate_need_rewrite[field->id()];
    } else {
        return (_has_bitmap_index || !_opts.chunk_pred->empty()) &&
...
Status SegmentIterator::_check_low_cardinality_optimization() {
    ...
    const size_t n = _opts.chunk_pred->num_columns();
    for (size_t i = 0; i < n; i++) {
        const FieldPtr& field = _schema.field(i);
        ...
        ColumnId cid = field->id();
        auto iter = _opts.chunk_pred->get_all_column_preds().find(cid);
        DCHECK(iter != _opts.chunk_pred->get_all_column_preds().end());
...
Status SegmentIterator::_init_bitmap_index_iterators() {
    ...
    for (const auto& [cid, _] : _opts.chunk_pred->get_column_preds()) {
...
Status SegmentIterator::_apply_bitmap_index() {
    ...
    for (auto& [cid, pred_list] : _opts.chunk_pred->get_column_preds()) {
    ...
    for (const ColumnPredicate* pred : erased_preds) {
        PredicateList& pred_list = _opts.chunk_pred->get_column_preds()[pred->column_id()];
...
Status SegmentIterator::_get_row_ranges_by_bloom_filter() {
    RETURN_IF(_scan_range.empty(), Status::OK());
    RETURN_IF(_opts.chunk_pred->empty(), Status::OK());
    SCOPED_RAW_TIMER(&_opts.stats->bf_filter_ns);
    size_t prev_size = _scan_range.span_size();
    for (const auto& [cid, preds] : _opts.chunk_pred->get_column_preds()) {
...
inline Schema reorder_schema(const Schema& input, ChunkPredicate* predicates) {
    const std::vector<FieldPtr>& fields = input.fields();

    Schema output;
    output.reserve(fields.size());
    for (const auto& field : fields) {
        if (predicates->contains_column(field->id())) {
            output.append(field);
        }
    }
    for (const auto& field : fields) {
...
ChunkIteratorPtr new_segment_iterator(const std::shared_ptr<Segment>& segment, const Schema& schema,
    ...
    if (options.chunk_pred->empty() || options.chunk_pred->size() >= schema.num_fields()) {
        return std::make_shared<SegmentIterator>(segment, schema, options);
    } else {
        Schema ordered_schema = reorder_schema(schema, options.chunk_pred);
