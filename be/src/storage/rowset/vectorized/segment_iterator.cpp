...
ColumnIterator* SegmentIterator::get_column_iterator(std::size_t idx) {
    if (!_inited) {
        RETURN_CODE_IF_ERROR_WITH_WARN(_init(), nullptr, "_init failed");
        _inited = true;
    }
    return _column_iterators[idx];
}
