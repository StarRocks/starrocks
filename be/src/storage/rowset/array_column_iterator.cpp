...
Status ArrayColumnIterator::init(const ColumnIteratorOptions& opts) {
    ...
    const TypeInfoPtr& null_type = get_type_info(LogicalType::OLAP_FIELD_TYPE_TINYINT);
    RETURN_IF_ERROR(ColumnVectorBatch::create(opts.chunk_size, true, null_type, nullptr, &_null_batch));

    const TypeInfoPtr& array_size_type = get_type_info(LogicalType::OLAP_FIELD_TYPE_INT);
