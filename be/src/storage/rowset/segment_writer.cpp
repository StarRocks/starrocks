...
SegmentWriter::SegmentWriter(std::unique_ptr<WritableFile> wfile, uint32_t segment_id,
                             TabletSchemaCSPtr tablet_schema, SegmentWriterOptions opts)
        : _segment_id(segment_id), _tablet_schema(std::move(tablet_schema)), _opts(std::move(opts)), _wfile(std::move(wfile)) {
