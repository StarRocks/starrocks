...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/rowset/segment_v2/segment_iterator.cpp
...
Status SegmentIterator::_init() {
    ...
    RETURN_IF_ERROR(_opts.block_mgr->open_block(_segment->_fname, &_rblock));
...
Status SegmentIterator::_get_row_ranges_by_keys() {
    ...
    for (auto& key_range : _opts.key_ranges) {
        ...
        if (key_range.upper_key != nullptr) {
            ...
            RETURN_IF_ERROR(_lookup_ordinal(*key_range.upper_key, !key_range.include_upper, num_rows(), &upper_rowid));
        }
        if (upper_rowid > 0 && key_range.lower_key != nullptr) {
            RETURN_IF_ERROR(_lookup_ordinal(*key_range.lower_key, key_range.include_lower, upper_rowid, &lower_rowid));
        }
    ...
    StarRocksMetrics::instance()->segment_rows_by_short_key.increment(_row_bitmap.cardinality());
...
Status SegmentIterator::_get_row_ranges_from_conditions(RowRanges* condition_row_ranges) {
    ...
    for (auto& cid : cids) {
        ...
        RETURN_IF_ERROR(_column_iterators[cid]->get_row_ranges_by_bloom_filter(column_cond, &column_bf_row_ranges));
    ...
    for (auto& cid : cids) {
        ...
        RETURN_IF_ERROR(_column_iterators[cid]->get_row_ranges_by_zone_map(column_cond, nullptr, &column_row_ranges));
    ...
    for (auto& delete_condition : _opts.delete_conditions) {
        ...
        for (auto& delete_column_condition : delete_condition->columns()) {
            ...
            RETURN_IF_ERROR(_column_iterators[cid]->get_row_ranges_by_zone_map(
                    column_cond, delete_column_condition.second, &single_delete_condition_row_ranges));
    ...
    StarRocksMetrics::instance()->segment_rows_read_by_zone_map.increment(zone_map_row_ranges.count());
...
Status SegmentIterator::_apply_bitmap_index() {
    ...
    std::vector<const ColumnPredicate*> remaining_predicates;
...
Status SegmentIterator::_lookup_ordinal(const RowCursor& key, bool is_include, rowid_t upper_bound, rowid_t* rowid) {
...
Status SegmentIterator::_read_columns(const std::vector<ColumnId>& column_ids, RowBlockV2* block, size_t row_offset,
                                      size_t nrows) {
    for (auto cid : column_ids) {
