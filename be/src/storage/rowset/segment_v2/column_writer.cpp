...
public:
    ...
    OwnedSlice finish() {
        ...
        int64_t r =
                bitshuffle::compress_lz4(_null_map.data(), _encode_buf.data(), _null_map.size(), sizeof(uint8_t), 0);
        if (r < 0) {
...
Status ScalarColumnWriter::init() {
    ...
    if (is_nullable()) {
        ...
        _null_map_builder_v2 = std::make_unique<NullMapBitshuffleBuilder>();
    }
...
Status ScalarColumnWriter::finish_current_page() {
    ...
    starrocks::segment_v2::DataPageFooterPB* data_page_footer = page->footer.mutable_data_page_footer();
