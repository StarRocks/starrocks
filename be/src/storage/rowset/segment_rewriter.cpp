...
Status SegmentRewriter::rewrite_partial_update(const FileInfo& src, FileInfo* dest,
    ...
    auto chunk = ChunkBuilder::new_chunk(schema, columns[0]->size());
...
Status SegmentRewriter::rewrite_auto_increment(const std::string& src_path, const std::string& dest_path,
    ...
    auto chunk_shared_ptr = ChunkBuilder::new_chunk(src_schema, num_rows);
    ...
    auto chunk = ChunkBuilder::new_chunk(schema, full_column_ids.size());
