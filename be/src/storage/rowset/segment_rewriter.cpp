...
// FLEXIBLE-on-ROW masked full-row rewrite. See the header for the contract. Assembles a COMPLETE
// chunk = KEY columns (read from the |src| .upt segment) + the caller-merged value columns, and
// writes a fresh full segment through column writers over the whole schema.
Status SegmentRewriter::rewrite_full_row_lake(const FileInfo& src, FileInfo* dest, const TabletSchemaCSPtr& tschema,
                                              const std::vector<uint32_t>& value_column_ids,
                                              MutableColumns& value_columns, uint32_t segment_id,
                                              const starrocks::lake::Tablet* tablet,
                                              RewriteVectorIndexOptions vector_index_opts,
                                              std::vector<int64_t>* out_vector_index_ids) {
    if (value_column_ids.size() != value_columns.size()) {
        return Status::InternalError("rewrite_full_row_lake: value_column_ids/value_columns length mismatch");
    }

    ASSIGN_OR_RETURN(auto fs, FileSystemFactory::CreateSharedFromString(dest->path));

    // The non-key value columns are supplied by the caller (the masked merge). The complement -- read
    // from the .upt src segment -- is exactly the KEY columns. (Auto-increment + flexible is rejected
    // upstream, so there is no auto-increment column to special-case here.)
    std::set<ColumnId> value_column_id_set(value_column_ids.begin(), value_column_ids.end());
    std::vector<ColumnId> key_column_ids;
    for (ColumnId i = 0, sz = tschema->num_columns(); i < sz; ++i) {
        if (value_column_id_set.count(i) == 0) {
            key_column_ids.emplace_back(i);
        }
    }
    Schema key_schema = ChunkHelper::convert_schema(tschema, key_column_ids);
    ...
    LakeIOOptions lake_io_opts{.fill_data_cache = fill_cache, .buffer_size = -1};
    ASSIGN_OR_RETURN(auto segment,
                     tablet_mgr->load_segment(src, segment_id, &footer_size_hint, lake_io_opts, fill_cache, tschema));
    uint32_t num_rows = segment->num_rows();

    auto key_chunk_shared_ptr = ChunkFactory::new_chunk(key_schema, num_rows);
    auto read_chunk = key_chunk_shared_ptr.get();
    ...
    // Read the KEY columns from the (partial) .upt segment file generated by this import task.
    ASSIGN_OR_RETURN(auto itr, segment->new_iterator(key_schema, seg_options));
    RETURN_IF_ERROR(itr->get_next(read_chunk));
    if (UNLIKELY(read_chunk->num_rows() != num_rows)) {
        LOG(ERROR) << "rewrite_full_row_lake: unexpected row number. expected=" << num_rows
                   << " real=" << read_chunk->num_rows();
        return Status::InternalError("rewrite_full_row_lake: unexpected row count");
    }
    itr->close();

    // The value columns are in upsert/physical row order, the SAME order the .upt key columns were
    // read in. Assert equal length so a key/value misalignment can never silently corrupt rows.
    if (!value_columns.empty() && value_columns[0] != nullptr && UNLIKELY(value_columns[0]->size() != num_rows)) {
        LOG(ERROR) << "rewrite_full_row_lake: value column row count " << value_columns[0]->size()
                   << " != .upt row count " << num_rows;
        return Status::InternalError("rewrite_full_row_lake: value/key row count mismatch");
    }
    ...
    // Splice: key columns from the .upt read, value columns from the caller's masked merge.
    size_t key_column_index = 0;
    size_t value_column_index = 0;
    for (ColumnId i = 0, sz = tschema->num_columns(); i < sz; ++i) {
        if (value_column_id_set.count(i) > 0) {
            chunk->get_column_by_index(i).reset(std::move(value_columns[value_column_index]));
            ++value_column_index;
        } else {
            chunk->get_column_by_index(i).swap(read_chunk->get_column_by_index(key_column_index));
            ++key_column_index;
        }
    }

    // Write a complete segment file. All columns go through column writers here, so (like
    // rewrite_auto_increment_lake) the inline (sync) vector index build covers the whole schema.
    SegmentWriterOptions opts;
    opts.vector_index_file_paths = std::move(vector_index_opts.file_paths);
    opts.defer_vector_index_build = vector_index_opts.defer_build;
    opts.vector_index_build_threshold = vector_index_opts.build_threshold;
    ...
    RETURN_IF_ERROR(writer.append_chunk(*chunk));
    RETURN_IF_ERROR(writer.finalize_columns(&index_size));
    TEST_ERROR_POINT("SegmentRewriter::rewrite_full_row_lake");
    RETURN_IF_ERROR(writer.finalize_footer(&segment_file_size));
