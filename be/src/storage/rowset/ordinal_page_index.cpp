...
Status OrdinalIndexReader::_do_load(const IndexReadOptions& opts, const OrdinalIndexPB& meta, ordinal_t num_values) {
    ScopedTimerPrinter do_load_timer("OrdinalIndexReader::_do_load inner on file " + opts.read_file->filename());
    ...
    for (int i = 0; i < _num_pages; i++) {
        ...
        ordinal_t ordinal = 0;
        {
            ScopedTimerPrinter decode_ascending_timer("decode_ascending outer on file " + opts.read_file->filename());
