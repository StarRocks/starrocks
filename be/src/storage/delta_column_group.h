...
public:
    ...
    // Collect EVERY (file_idx, col_idx) slot in this DCG whose unique-id set contains `uid`. Unlike
    // get_column_idx (FIRST match only), this is required by the SDCG overlay reader: one converged
    // lake DCG legitimately carries the same uid across multiple .spcols files at different versions
    // (carry-forward in append_dcg + convergence rebuild). Collecting only the first silently drops
    // the others -> partial-overlap rows revert to base (data loss). col_idx is resolved per matched
    // file. Appends to *out (does not clear) so callers can accumulate across DCGs. Returns true if
    // any slot matched.
    bool get_all_column_file_indices(ColumnUID uid, std::vector<std::pair<int32_t, int32_t>>* out) const {
        bool found = false;
        for (int idx = 0; idx < _column_uids.size(); ++idx) {
            for (int cidx = 0; cidx < _column_uids[idx].size(); ++cidx) {
                if (_column_uids[idx][cidx] == uid) {
                    out->emplace_back(idx, cidx);
                    found = true;
                    break; // a uid appears at most once WITHIN a single file slot
                }
            }
        }
        return found;
    ...
    // Per-file write version, 1:1 with relative_column_files(). The in-memory projection of the lake
    // DeltaColumnGroupVerPB.versions[] field. Falls back to the DCG's own _version when the slot is
    // absent (local meta / legacy mutator) OR <= 0 (a merge-rebuilt carry-forward can stamp 0 for a
    // file whose source meta lacked a versions entry; treat as unknown). A 0 must NEVER sort below a
    // real version: the SDCG overlay reader uses this as the last-write-wins sort key across the
    // multiple sparse files a single converged DCG can carry for the same uid.
    int64_t file_version(size_t idx) const {
        if (idx < _file_versions.size() && _file_versions[idx] > 0) {
            return _file_versions[idx];
        }
        return _version;
    }
    const std::vector<int64_t>& file_versions() const { return _file_versions; }
...
private:
    ...
    // Per-file write version, 1:1 with _column_files; in-memory projection of the lake VerPB.versions[].
    // EMPTY for local-engine metas / legacy mutators (file_version() then falls back to _version).
    // NOT part of the serialized / merge / GC contract.
    std::vector<int64_t> _file_versions;
