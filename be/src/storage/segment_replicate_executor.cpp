...
// Read Chunk size for append file to iobuf, 256MB, test show that unlimit read chunk size will cause
// crash for the brpc framework.
constexpr int64_t kSegmentReplicateReadChunkSize = 256L * 1024 * 1024;

Status serialize_segment_http_request_to_iobuf(const PTabletWriterAddSegmentRequest& request, const butil::IOBuf& data,
                                               butil::IOBuf* iobuf) {
...
Status append_file_to_iobuf(FileSystem* fs, const std::string& path, int64_t file_size, const char* file_type,
    ...
    // Read large files into multiple IOBuf blocks to avoid a single huge allocation.
    // This does not change the wire format: the receiver still observes one brpc
    // attachment byte stream and consumes it with IOBuf::cutn/cut_into_writer, so
    // older BEs that support tablet_writer_add_segment can parse the attachment.
    int64_t offset = 0;
...
Status ReplicateChannel::_send_request(SegmentPB* segment, butil::IOBuf& data, bool eos) {
    ...
    const bool use_http = config::enable_segment_replication_http_fallback &&
                          (request.ByteSizeLong() + data.size() + sizeof(size_t)) > kSegmentReplicateRpcHttpMinSize;
    ...
    if (use_http) {
        ...
        RETURN_IF_ERROR(serialize_segment_http_request_to_iobuf(request, data, &http_attachment));
