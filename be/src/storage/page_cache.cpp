...
void StoragePageCache::insert(const CacheKey& key, const Slice& data, PageCacheHandle* handle, bool in_memory) {
    ...
    current_thread::mem_release(mem_size);
    MemTracker* prev_tracker = current_thread::set_mem_tracker(_mem_tracker);
    current_thread::mem_consume(mem_size);
    DeferOp op([&] { current_thread::set_mem_tracker(prev_tracker); });
#endif
