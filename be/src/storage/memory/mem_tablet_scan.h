...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/memory/mem_tablet_scan.h
...
class ScanSpec {
public:
    ScanSpec(vector<std::string>&& columns, uint64_t version = UINT64_MAX, uint64_t limit = UINT64_MAX)
...
private:
    ...
    MemTabletScan(std::shared_ptr<MemTablet>&& tablet, std::unique_ptr<ScanSpec>* spec, size_t num_rows,
