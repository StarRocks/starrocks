...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/memory/mem_sub_tablet.cpp
...
#include "storage/memory/column_reader.h"
#include "storage/memory/column_writer.h"
...
Status MemSubTablet::read_column(uint64_t version, uint32_t cid, std::unique_ptr<ColumnReader>* reader) {
...
Status MemSubTablet::commit_write(uint64_t version) {
    ...
    _writers.clear();
    _schema = nullptr;
    LOG(INFO) << StringPrintf("commit writetxn(insert=%zu update=%zu update_cell=%zu) %.3lfs", _num_insert, _num_update,
                              _num_update_cell, GetMonoTimeSecondsAsDouble() - _write_start);
    return Status::OK();
}
...
scoped_refptr<HashIndex> MemSubTablet::rebuild_hash_index(size_t new_capacity) {
    ...
    for (size_t i = 0; i < _row_size; i++) {
        ...
        if (!hi->add(hashcode, i)) {
            ...
            LOG(INFO) << StringPrintf("Rebuild hash index %zu failed time: %.3lfs, expand", new_capacity, t1 - t0);
