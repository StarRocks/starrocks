...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/memory/typed_column_writer.h
...
public:
    TypedColumnWriter(Column* column, uint64_t version, uint64_t real_version, vector<ColumnDelta*>&& deltas)
            : _column(column),
              _base(&_column->_base),
              _version(version),
...
private:
    ...
    Status expand_base() {
        ...
        size_t new_base_capacity = padding(_base->capacity() + added, Column::BASE_CAPACITY_MIN_STEP_SIZE);
        // check if version needs expanding too
        size_t new_version_capacity = 0;
        if (_column->_versions.size() == _column->_versions.capacity()) {
            new_version_capacity = padding(_column->_versions.capacity() + Column::VERSION_CAPACITY_STEP_SIZE,
        ...
        DLOG(INFO) << StringPrintf("%s memory=%.1lfM expand base base=%zu version=%zu",
                                   _column->schema().debug_string().c_str(), _column->memory() / 1000000.0,
                                   new_base_capacity, new_version_capacity);
        scoped_refptr<Column> cow(new Column(*_column.get(), new_base_capacity, new_version_capacity));
        cow.swap(_column);
        _base = &(_column->_base);
        return Status::OK();
    }
    ...
    Status add_block() {
        ...
        if (_column->schema().cid() == 1) {
            ...
            DLOG(INFO) << StringPrintf("Column(cid=%u) add ColumnBlock %zu/%zu", _column->schema().cid(), _base->size(),
                                       _base->capacity());
        }
        return Status::OK();
    }
    ...
    Status expand_versions() {
        size_t new_capacity = padding(_column->_versions.capacity() + Column::VERSION_CAPACITY_STEP_SIZE,
                                      Column::VERSION_CAPACITY_STEP_SIZE);
        DLOG(INFO) << StringPrintf("%s memory=%.1lfM expand delta base=%zu version=%zu",
                                   _column->schema().debug_string().c_str(), _column->memory() / 1000000.0,
                                   _base->capacity(), new_capacity);
    ...
    Status add_delta(scoped_refptr<ColumnDelta>&& delta, uint64_t version) {
        ...
        DLOG(INFO) << StringPrintf("%s add version %zu update: %zu", _column->debug_string().c_str(), version,
                                   delta->size());
