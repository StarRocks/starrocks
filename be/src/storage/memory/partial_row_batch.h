...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/memory/partial_row_batch.h
...
private:
    ...
    struct CellInfo {
        CellInfo(uint32_t cid, const void* data) : cid(cid), data(reinterpret_cast<const uint8_t*>(data)) {}
        uint32_t cid = 0;
...
public:
    ...
    Status start_batch(size_t row_capacity = DEFAULT_ROW_CAPACIT, size_t byte_capacity = DEFAULT_BYTE_CAPACITY);
