...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/memory/column.cpp
...
std::string Column::debug_string() const {
    return StringPrintf("Column(%s version=%zu)", _cs.debug_string().c_str(), _versions.back().version);
}

Status Column::capture_version(uint64_t version, std::vector<ColumnDelta*>* deltas, uint64_t* real_version) const {
    uint64_t base_version = _versions[_base_idx].version;
    *real_version = base_version;
    if (version < base_version) {
        uint64_t oldest = _versions[0].version;
        if (version < oldest) {
            return Status::NotFound(StringPrintf("version %zu(oldest=%zu) deleted", version, oldest));
        }
...
#define CREATE_READER(T)                                                                                  \
    if (nullable) {                                                                                       \
        (*reader).reset(new TypedColumnReader<T, true>(this, version, real_version, std::move(deltas)));  \
    } else {                                                                                              \
        (*reader).reset(new TypedColumnReader<T, false>(this, version, real_version, std::move(deltas))); \
    }
...
#define CREATE_WRITER(T)                                                                                       \
    if (nullable) {                                                                                            \
        (*writer).reset(new TypedColumnWriter<T, true>(this, real_version, real_version, std::move(deltas)));  \
    } else {                                                                                                   \
        (*writer).reset(new TypedColumnWriter<T, false>(this, real_version, real_version, std::move(deltas))); \
    }
