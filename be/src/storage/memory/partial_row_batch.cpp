...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/memory/partial_row_batch.cpp
...
PartialRowBatch::PartialRowBatch(scoped_refptr<Schema>* schema) : _schema(*schema), _bit_set_size(_schema->cid_size()) {
...
Status PartialRowBatch::cur_row_get_cell(size_t idx, const ColumnSchema** cs, const void** data) const {
    if (idx >= _cells.size()) {
