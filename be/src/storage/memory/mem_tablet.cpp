...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/memory/mem_tablet.cpp
...
Status MemTablet::scan(std::unique_ptr<ScanSpec>* spec, std::unique_ptr<MemTabletScan>* scan) {
    ...
    scan->reset(new MemTabletScan(std::static_pointer_cast<MemTablet>(shared_from_this()), spec, num_rows, &readers));
    return Status::OK();
}
