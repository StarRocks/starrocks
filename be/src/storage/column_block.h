...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/column_block.h
...
public:
    ...
    const TypeInfoPtr& type_info() const { return _batch->type_info(); }
    ...
    void set_null_bits(size_t offset, size_t num_rows, bool val) const { _batch->set_null_bits(offset, num_rows, val); }
...
public:
    ...
    const TypeInfoPtr& type_info() const { return _block->type_info(); }
    MemPool* pool() const { return _block->pool(); }
    void set_null_bits(size_t num_rows, bool val) { _block->set_null_bits(_row_offset, num_rows, val); }
