...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/row.h
...
template <typename DstRowType, typename SrcRowType>
void direct_copy_row(DstRowType* dst, const SrcRowType& src, MemPool* mem_pool = nullptr) {
    for (auto cid : dst->schema()->column_ids()) {
        auto dst_cell = dst->cell(cid);
        dst->schema()->column(cid)->direct_copy(&dst_cell, src.cell(cid), mem_pool);
    }
}
