...
#ifndef BE_TEST
    Status st = current_thread::mem_tracker()->check_mem_limit("LinkedSchemaChange");
    if (!st.ok()) {
...
#ifndef BE_TEST
        Status st = current_thread::mem_tracker()->check_mem_limit("DirectSchemaChange");
        if (!st.ok()) {
            LOG(WARNING) << "fail to execute schema change: " << st.message() << std::endl;
            return false;
        }
...
#ifndef BE_TEST
        Status st = current_thread::mem_tracker()->check_mem_limit("SortSchemaChange");
        if (!st.ok()) {
            LOG(WARNING) << "fail to execute schema change: " << st.message() << std::endl;
            return false;
        }
