...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/reader.cpp
...
OLAPStatus Reader::init(const ReaderParams& read_params) {
    ...
    if (res != OLAP_SUCCESS) {
        LOG(WARNING) << "fail to init reader when init params. res:" << res
                     ...
                     << ", reader type:" << read_params.reader_type << ", version:" << read_params.version;
        return res;
    }
    ...
    if (res != OLAP_SUCCESS) {
        LOG(WARNING) << "fail to init reader when _capture_rs_readers. res:" << res
                     << ", tablet_id:" << read_params.tablet->tablet_id()
                     << ", schema_hash:" << read_params.tablet->schema_hash()
                     << ", reader_type:" << read_params.reader_type << ", version:" << read_params.version;
        return res;
    }
...
OLAPStatus Reader::_dup_key_next_row(RowCursor* row_cursor, MemPool* mem_pool, ObjectPool* agg_pool, bool* eof) {
    ...
    direct_copy_row(row_cursor, *_next_key, mem_pool);
...
OLAPStatus Reader::_capture_rs_readers(const ReaderParams& read_params) {
    ...
    for (int i = 0; i < _keys_param.start_keys.size(); ++i) {
        ...
        if (_keys_param.range == "gt") {
            if (end_key != nullptr && compare_row_key(*start_key, *end_key) >= 0) {
                VLOG(3) << "return EOF when range=" << _keys_param.range << ", start_key=" << start_key->to_string()
    ...
    if (read_params.reader_type == READER_QUERY) {
        if (_tablet->tablet_schema().keys_type() == DUP_KEYS || _tablet->tablet_schema().keys_type() == PRIMARY_KEYS) {
            // duplicated keys are allowed, no need to merge sort keys in rowset
            // or primary key constraint will filter duplicate records
    ...
    _reader_context.chunk_size = read_params.chunk_size;
...
OLAPStatus Reader::_init_keys_param(const ReaderParams& read_params) {
    ...
    for (size_t i = 0; i < start_key_size; ++i) {
        ...
        OLAPStatus res =
                _keys_param.start_keys[i]->init_scan_key(_tablet->tablet_schema(), read_params.start_key[i].values());
        if (res != OLAP_SUCCESS) {
    ...
    for (size_t i = 0; i < end_key_size; ++i) {
        ...
        OLAPStatus res =
                _keys_param.end_keys[i]->init_scan_key(_tablet->tablet_schema(), read_params.end_key[i].values());
        if (res != OLAP_SUCCESS) {
            LOG(WARNING) << "fail to init row cursor. res=" << res;
            return res;
        }
...
#define COMPARISON_PREDICATE_CONDITION_VALUE(NAME, PREDICATE)                                                     \
    ColumnPredicate* Reader::_new_##NAME##_pred(const TabletColumn& column, int index, const std::string& cond) { \
        ...
        case OLAP_FIELD_TYPE_DECIMAL_V2: {                                                                        \
            ...
            value.parse_from_str(cond.c_str(), cond.size());                                                      \
            predicate = new PREDICATE<DecimalV2Value>(index, value);                                              \
            break;                                                                                                \
        }                                                                                                         \
        ...
        case OLAP_FIELD_TYPE_DATE_V2: {                                                                           \
            ...
            value.from_string(cond.data(), cond.size());                                                          \
            predicate = new PREDICATE<int32_t>(index, value.julian());                                            \
            break;                                                                                                \
        }                                                                                                         \
        ...
        case OLAP_FIELD_TYPE_TIMESTAMP: {                                                                         \
            ...
            value.from_string(cond.data(), cond.size());                                                          \
            predicate = new PREDICATE<int64_t>(index, value.timestamp());                                         \
            break;                                                                                                \
        }                                                                                                         \
...
ColumnPredicate* Reader::_parse_to_predicate(const TCondition& condition) {
    ...
    if (condition.condition_op == "*=" && condition.condition_values.size() == 1) {
        ...
        case OLAP_FIELD_TYPE_DECIMAL_V2: {
            std::set<DecimalV2Value> values;
            for (auto& cond_val : condition.condition_values) {
                DecimalV2Value value;
                value.parse_from_str(cond_val.c_str(), cond_val.size());
                values.insert(value);
            }
            predicate = new InListPredicate<DecimalV2Value>(index, std::move(values));
            break;
        }
        case OLAP_FIELD_TYPE_CHAR: {
        ...
        case OLAP_FIELD_TYPE_DATE_V2: {
            ...
            for (auto& cond_val : condition.condition_values) {
                vectorized::DateValue value;
                value.from_string(cond_val.data(), cond_val.size());
                values.insert(value.julian());
            }
            predicate = new InListPredicate<int32_t>(index, std::move(values));
            break;
        }
...
void Reader::_init_load_bf_columns(const ReaderParams& read_params) {
    ...
    for (const auto& cond_column : _conditions.columns()) {
        for (const Cond* cond : cond_column.second->conds()) {
            if (cond->op == OP_EQ || (cond->op == OP_IN && cond->operand_set.size() < MAX_OP_IN_FIELD_NUM)) {
