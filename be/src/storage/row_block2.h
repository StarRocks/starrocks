...
//   https://github.com/apache/incubator-doris/blob/master/be/src/olap/row_block2.h
...
private:
    friend class ::starrocks::vectorized::BlockConverter;
...
public:
    ...
    void set_is_null(size_t col_idx, bool is_null) { return column_block(col_idx).set_is_null(_row_index, is_null); }
    uint8_t* mutable_cell_ptr(size_t col_idx) const { return column_block(col_idx).mutable_cell_ptr(_row_index); }
    const uint8_t* cell_ptr(size_t col_idx) const { return column_block(col_idx).cell_ptr(_row_index); }
