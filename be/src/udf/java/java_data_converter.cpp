...
static jobject decimal_cell_to_bigdecimal(const TypeDescriptor& td, const Column* col, int row_num,
    ...
    case TYPE_DECIMAL32: {
        ...
        return helper.newBigDecimal(static_cast<int64_t>(spec->get_data()[row_num]), td.scale);
    }
    case TYPE_DECIMAL64: {
        auto* spec = down_cast<const RunTimeColumnType<TYPE_DECIMAL64>*>(col);
        return helper.newBigDecimal(spec->get_data()[row_num], td.scale);
    }
    case TYPE_DECIMAL128: {
        auto* spec = down_cast<const RunTimeColumnType<TYPE_DECIMAL128>*>(col);
        return helper.newBigDecimal(
                DecimalV3Cast::to_string<int128_t>(spec->get_data()[row_num], td.precision, td.scale));
    }
    case TYPE_DECIMAL256: {
        auto* spec = down_cast<const RunTimeColumnType<TYPE_DECIMAL256>*>(col);
        return helper.newBigDecimal(
                DecimalV3Cast::to_string<int256_t>(spec->get_data()[row_num], td.precision, td.scale));
    }
    default:
...
#define CREATE_BOX_TYPE(NAME, TYPE)                                     \
    ...
    case TYPE_DATE: {
        ...
        v.l = helper.newLocalDate(spec_col->get_data()[row_num]._julian);
        break;
    }
    case TYPE_DATETIME: {
        auto spec_col = down_cast<const RunTimeColumnType<TYPE_DATETIME>*>(col);
        v.l = helper.newLocalDateTime(spec_col->get_data()[row_num].timestamp());
        break;
    }
    case TYPE_DECIMAL32:
    case TYPE_DECIMAL64:
    ...
    case TYPE_STRUCT: {
        ...
        int num_fields = static_cast<int>(struct_col->fields_column().size());
        ...
        for (int f = 0; f < num_fields; ++f) {
            const Column* field_col = struct_col->fields_column()[f].get();
...
#define APPEND_BOX_TYPE(NAME, TYPE)          \
    case NAME: {                             \
        ...
        case TYPE_STRUCT: {
            ...
            int num_fields = static_cast<int>(struct_col->fields().size());
            if (static_cast<int>(type_desc.children.size()) != num_fields) {
            ...
            for (int f = 0; f < num_fields; ++f) {
                ...
                RETURN_IF_ERROR(append_jvalue(type_desc.children[f], true, struct_col->fields_column()[f].get(),
...
static std::unique_ptr<DirectByteBuffer> wrap_decimal_data(const Column* data_column) {
    ...
    const auto& container = spec->get_data();
...
static StatusOr<jobject> build_list_boxed_array(JVMFunctionHelper& helper, const TypeDescriptor& type_desc,
    ...
    auto offsets_buf = std::make_unique<DirectByteBuffer>((void*)array_col->offsets().get_data().data(),
                                                          array_col->offsets().get_data().size() * sizeof(uint32_t));
...
static StatusOr<jobject> build_map_boxed_array(JVMFunctionHelper& helper, const TypeDescriptor& type_desc,
    ...
    auto offsets_buf = std::make_unique<DirectByteBuffer>((void*)map_col->offsets().get_data().data(),
                                                          map_col->offsets().get_data().size() * sizeof(uint32_t));
...
Status JavaDataTypeConverter::convert_to_boxed_array(FunctionContext* ctx, const Column** columns, int num_cols,
    ...
    for (int i = 0; i < num_cols; ++i) {
        ...
        if (columns[i]->only_null() ||
            ...
            auto* data_column = down_cast<const ConstColumn*>(columns[i])->data_column().get();
