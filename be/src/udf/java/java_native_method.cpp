...
jlongArray JavaNativeMethods::getStructFieldAddrs(JNIEnv* env, jclass clazz, jlong columnAddr) {
    ...
    auto* data_col = down_cast<NullableColumn*>(column)->data_column_mutable_ptr().get();
    ...
    int n = static_cast<int>(struct_col->fields_column().size());
    jlongArray jarr = env->NewLongArray(n);
    std::vector<jlong> addrs(n);
    for (int i = 0; i < n; ++i) {
        addrs[i] = reinterpret_cast<jlong>(struct_col->fields_column()[i]->as_mutable_ptr().get());
    }
    if (n > 0) {
