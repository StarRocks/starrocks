...
//   https://github.com/apache/incubator-doris/blob/master/be/src/udf/uda_test_harness.h
...
public:
    ...
    UdaTestHarness(typename BaseClass::InitFn init_fn, UpdateFn update_fn, typename BaseClass::MergeFn merge_fn,
                   typename BaseClass::SerializeFn serialize_fn, typename BaseClass::FinalizeFn finalize_fn)
            : BaseClass(init_fn, merge_fn, serialize_fn, finalize_fn), _update_fn(update_fn) {}

    bool execute(const std::vector<INPUT>& values, const RESULT& expected) { return execute(values, expected, ALL); }
    // Runs the UDA in all the modes, validating the result is 'expected' each time.
...
public:
    ...
    ~UdaTestHarness2() {}
    UdaTestHarness2(typename BaseClass::InitFn init_fn, UpdateFn update_fn, typename BaseClass::MergeFn merge_fn,
                    typename BaseClass::SerializeFn serialize_fn, typename BaseClass::FinalizeFn finalize_fn)
            : BaseClass(init_fn, merge_fn, serialize_fn, finalize_fn), _update_fn(update_fn) {}

    // Runs the UDA in all the modes, validating the result is 'expected' each time.
    bool execute(const std::vector<INPUT1>& values1, const std::vector<INPUT2>& values2, const RESULT& expected,
...
public:
    typedef void (*UpdateFn)(FunctionContext* context, const INPUT1& input1, const INPUT2& input2, const INPUT3& input3,
                             INTERMEDIATE* result);

    typedef UdaTestHarnessBase<RESULT, INTERMEDIATE> BaseClass;

    ~UdaTestHarness3() {}
    UdaTestHarness3(typename BaseClass::InitFn init_fn, UpdateFn update_fn, typename BaseClass::MergeFn merge_fn,
                    typename BaseClass::SerializeFn serialize_fn, typename BaseClass::FinalizeFn finalize_fn)
            : BaseClass(init_fn, merge_fn, serialize_fn, finalize_fn), _update_fn(update_fn) {}
...
template <typename RESULT, typename INTERMEDIATE, typename INPUT1, typename INPUT2, typename INPUT3, typename INPUT4>
class UdaTestHarness4 : public UdaTestHarnessBase<RESULT, INTERMEDIATE> {
public:
    typedef void (*UpdateFn)(FunctionContext* context, const INPUT1& input1, const INPUT2& input2, const INPUT3& input3,
                             const INPUT4& input4, INTERMEDIATE* result);

    typedef UdaTestHarnessBase<RESULT, INTERMEDIATE> BaseClass;

    ~UdaTestHarness4() {}
    UdaTestHarness4(typename BaseClass::InitFn init_fn, UpdateFn update_fn, typename BaseClass::MergeFn merge_fn,
                    typename BaseClass::SerializeFn serialize_fn, typename BaseClass::FinalizeFn finalize_fn)
            : BaseClass(init_fn, merge_fn, serialize_fn, finalize_fn), _update_fn(update_fn) {}

    // Runs the UDA in all the modes, validating the result is 'expected' each time.
    bool execute(const std::vector<INPUT1>& values1, const std::vector<INPUT2>& values2,
                 const std::vector<INPUT3>& values3, const std::vector<INPUT4>& values4, const RESULT& expected) {
