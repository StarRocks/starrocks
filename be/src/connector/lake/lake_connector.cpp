...
Status LakeDataSource::open(RuntimeState* state) {
    ...
    if (_needs_reopen) {
        ...
        if (needs_late_runtime_filter_reinit(runtime_filter_snapshots)) {
            COUNTER_UPDATE(_lake_late_rf_reinit_counter, 1);
            return reinit_reader_with_late_runtime_filters(state, std::move(runtime_filter_snapshots));
        }
    ...
    if (thrift_lake_scan_node.__isset.vector_search_options) {
        ...
        if (_use_vector_index) {
            // use_ivfpq is the deprecated predecessor of refine_distance (both mean "run the refine
            // path"). Honor it too so an older FE that only sets use_ivfpq picks the right path under
            // a rolling upgrade.
...
Status LakeDataSource::init_reader_params(const std::vector<OlapScanRange*>& key_ranges) {
    ...
    _params.enable_prepared_physical_split_scan = _provider->enable_lake_prepared_physical_split_scan();
...
Status LakeDataSource::init_tablet_reader(RuntimeState* runtime_state, bool use_prepared_state) {
    ...
    if (_split_context != nullptr) {
        ...
        apply_child_split_context(*split_context, use_prepared_state);
    }
...
Status LakeDataSource::reopen_reader(RuntimeState* /*state*/) {
    ...
    apply_child_split_context(*split_context, true /* use_prepared_state */);
...
LakeDataSource::RuntimeFilterSnapshots LakeDataSource::capture_runtime_filter_snapshots() const {
    RuntimeFilterSnapshots snapshots;
    ...
    for (const auto& [filter_id, desc] : _runtime_filters->descriptors()) {
        if (desc == nullptr) {
            snapshots.emplace(filter_id, RuntimeFilterSnapshot{});
            continue;
        }
        const auto* rf = desc->runtime_filter(runtime_membership_filter_eval_context.driver_sequence);
        snapshots.emplace(filter_id, RuntimeFilterSnapshot{true, desc->is_stream_build_filter(), rf != nullptr,
                                                           rf != nullptr ? rf->rf_version() : 0});
    }
    return snapshots;
}

void LakeDataSource::remember_runtime_filter_snapshots(RuntimeFilterSnapshots snapshots) {
    _observed_runtime_filter_snapshots = std::move(snapshots);
}

bool LakeDataSource::needs_late_runtime_filter_reinit(const RuntimeFilterSnapshots& current_snapshots) const {
    if (current_snapshots.empty()) {
        return std::any_of(_observed_runtime_filter_snapshots.begin(), _observed_runtime_filter_snapshots.end(),
                           [](const auto& entry) { return !entry.second.stream_build; });
    }

    for (const auto& [filter_id, current] : current_snapshots) {
        if (!current.has_descriptor) {
            return true;
        }
        auto it = _observed_runtime_filter_snapshots.find(filter_id);
        if (it == _observed_runtime_filter_snapshots.end()) {
            if (current.stream_build) {
                continue;
            }
            return true;
        }
        if (it->second.has_descriptor != current.has_descriptor || it->second.stream_build != current.stream_build) {
            return true;
        }
        if (current.stream_build) {
            continue;
        }
        if (it->second.arrived != current.arrived || (current.arrived && current.version > it->second.version)) {
            return true;
        }
    }
    for (const auto& [filter_id, snapshot] : _observed_runtime_filter_snapshots) {
        if (!snapshot.stream_build && current_snapshots.find(filter_id) == current_snapshots.end()) {
            return true;
        }
    }
    return false;
...
Status LakeDataSource::rebuild_scan_conjuncts(RuntimeState* state) {
    const TLakeScanNode& thrift_lake_scan_node = _provider->_t_lake_scan_node;
    TupleDescriptor* tuple_desc = state->desc_tbl().get_tuple_descriptor(thrift_lake_scan_node.tuple_id);
    ...
    return build_scan_range(state);
}

void LakeDataSource::reset_reader_before_runtime_filter_reinit(RuntimeState* state) {
...
void LakeDataSource::init_counter(RuntimeState* state) {
    ...
    _data_page_prefetch_wait_timer = ADD_CHILD_TIMER(_runtime_profile, "DataPagePrefetchWait", segment_read_name);
    ...
    _data_page_prefetch_lookahead_segment_counter =
            ADD_CHILD_COUNTER(_runtime_profile, "DataPagePrefetchLookaheadSegments", TUnit::UNIT, segment_read_name);
...
void LakeDataSource::update_counter(RuntimeState* state) {
    ...
    COUNTER_UPDATE(_data_page_prefetch_wait_timer, _reader->stats().data_page_prefetch_wait_ns);
    ...
    COUNTER_UPDATE(_data_page_prefetch_lookahead_segment_counter,
                   _reader->stats().data_page_prefetch_lookahead_segments);
...
Status LakeDataSourceProvider::init(ObjectPool* pool, RuntimeState* state) {
    ...
    _runtime_state = state;
    _enable_lake_prepared_physical_split_scan = _t_lake_scan_node.__isset.use_prepared_physical_split_scan &&
                                                _t_lake_scan_node.use_prepared_physical_split_scan;
...
StatusOr<bool> LakeDataSourceProvider::_could_tablet_internal_parallel(
    ...
    *splitted_scan_rows = std::max(config::tablet_internal_parallel_min_splitted_scan_rows,
    ...
    // The ideal-share denominator is scan_ranges.size() -- THIS call's slice -- NOT the fragment-wide
    // num_total_scan_ranges: under group execution / colocate, convert_scan_range_to_morsel_queue_builder
    // runs once per driver sequence with only that driver's slice of ranges (see
    // ScanNode::convert_scan_range_to_morsel_queue_factory), while num_total_scan_ranges stays the global
    // total. num_table_rows/max_tablet_rows above are summed over the slice, so the denominator must share
    // that scope -- otherwise a uniform layout with one big tablet per driver (slice_rows / global_dop) would
    // look skewed and split when it should not. In the non-group path scan_ranges.size() equals
    // num_total_scan_ranges, so this is unchanged there.
    const int64_t effective_dop = std::max<int64_t>(1, std::min<int64_t>(scan_ranges.size(), pipeline_dop));
