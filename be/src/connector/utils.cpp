...
StatusOr<std::string> HiveUtils::iceberg_make_partition_name(
    ...
    if (chunk->has_extra_data()) {
        ...
        for (size_t i = 0; i < columns.size(); i++) {
            ...
            bool is_null;
            ASSIGN_OR_RETURN(std::string value, iceberg_column_value(meta.type, column, transform_exprs[i], is_null));
            field_is_null[i] = is_null;
            if (!support_null_partition && is_null) {
                return Status::NotSupported("Partition value can't be null.");
            }
        ...
        for (size_t i = 0; i < column_evaluators.size(); i++) {
            ...
            auto type = column_evaluators[i]->type();
            bool is_null;
            ASSIGN_OR_RETURN(std::string value, iceberg_column_value(type, column, transform_exprs[i], is_null));
            field_is_null[i] = is_null;
            if (!support_null_partition && is_null) {
                return Status::NotSupported("Partition value can't be null.");
            }
