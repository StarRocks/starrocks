...
StatusOr<std::string> HiveUtils::iceberg_column_value(const TypeDescriptor& type_desc, const ColumnPtr& column,
    ...
    if (column->size() <= i) {
        ...
        const auto years_from_epoch = ColumnViewer<TYPE_BIGINT>(column).value(i);
        value = std::to_string(years_from_epoch + 1970);
    } else if (transform_expr == "month") {
        const auto months_from_epoch = ColumnViewer<TYPE_BIGINT>(column).value(i);
        ...
        const auto days_from_epoch = ColumnViewer<TYPE_BIGINT>(column).value(i);
        ...
        const auto hours_from_epoch = ColumnViewer<TYPE_BIGINT>(column).value(i);
        ...
        ASSIGN_OR_RETURN(value, column_value(type_desc, column, i));
