...
StatusOr<std::unique_ptr<RandomAccessFile>> DeletionVector::open_random_access_file(
    ...
    const connector::ScanFileOpenOptions options{.fs = _ctx.fs,
                                                 .file_path = file_path,
                                                 .fs_stats = &fs_stats,
                                                 .app_stats = &app_stats,
                                                 .datacache_options = _ctx.datacache_options};
    ASSIGN_OR_RETURN(auto file, connector::open_scan_random_access_file(shared_buffered_input_stream,
                                                                        cache_input_stream, options));
