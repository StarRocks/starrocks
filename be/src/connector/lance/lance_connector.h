...
public:
    ~LanceConnector() override = default;

    DataSourceProviderPtr create_data_source_provider(ConnectorScanNode* scan_node,
                                                      const TPlanNode& plan_node) const override;

    ConnectorType connector_type() const override { return ConnectorType::LANCE; }
};

class LanceDataSource;
class LanceDataSourceProvider;

class LanceDataSourceProvider final : public DataSourceProvider {
public:
    ~LanceDataSourceProvider() override = default;
    friend class LanceDataSource;
    LanceDataSourceProvider(ConnectorScanNode* scan_node, const TPlanNode& plan_node);
