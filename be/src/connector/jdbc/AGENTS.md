...
- Allowed internal include prefixes: `connector/jdbc/`, `connector_primitive/`, `platform/`, `fs/`, `exprs/`, `runtime/`, `column/`, `types/`, `common/`, `base/`, `gutil/`, `gen_cpp/`
...
- Remediation: Keep ConnectorJDBC limited to JDBC read-side scan, driver management, and type checking; move registration into ModuleBootstrap and avoid pulling Connector, storage, service, or full Exec code into the connector library.
