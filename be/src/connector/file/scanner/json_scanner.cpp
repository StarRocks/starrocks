...
Status JsonReader::_construct_row_without_jsonpath(simdjson::ondemand::object* row, Chunk* chunk) {
    ...
    if (_flexible_partial_update && _cset_col_index >= 0) {
        ...
        // Out of set-id space (> kMaxColumnSets distinct per-row column sets in one load). Fail the load:
        // the previous "degrade to set-id 0" was WRONG -- set 0 is a specific interned set, not the union,
        // so applying it would overlay the wrong columns for this row (silent corruption). A load with this
        // many distinct column sets is pathological; fail fast instead.
