...
Status JsonScanner::_construct_default_exprs_for_absent_key() {
    ...
    for (size_t column_pos = 0; column_pos < slot_size; ++column_pos) {
        ...
        // The same refusal the FE applies, repeated here because an older FE does not apply it and
        // would still send a default for a column this load reads as text. Converting JSON to text
