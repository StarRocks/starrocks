...
Status CacheSelectScanner::do_init(RuntimeState* runtime_state, const HdfsScannerContext& scanner_ctx) {
    return Status::OK();
}

Status CacheSelectScanner::do_open(RuntimeState* runtime_state) {
    bool cache_select_supported_format = _scanner_ctx->scan_range->file_format == THdfsFileFormat::PARQUET ||
                                         _scanner_ctx->scan_range->file_format == THdfsFileFormat::ORC ||
                                         _scanner_ctx->scan_range->file_format == THdfsFileFormat::TEXT;
...
Status CacheSelectScanner::do_get_next(RuntimeState* runtime_state, ChunkPtr* chunk) {
    if (_scanner_ctx->scan_range->file_format == THdfsFileFormat::TEXT) {
        RETURN_IF_ERROR(_fetch_textfile());
    } else if (_scanner_ctx->scan_range->file_format == THdfsFileFormat::PARQUET) {
        RETURN_IF_ERROR(_fetch_parquet());
    } else if (_scanner_ctx->scan_range->file_format == THdfsFileFormat::ORC) {
        ...
        return Status::EndOfFile("Unsupported file format in cache select: " +
                                 to_string(_scanner_ctx->scan_range->file_format));
    }
...
Status CacheSelectScanner::_fetch_orc() {
    ...
    std::set<uint64_t> selected_column_ids;
    {
        ...
        orc_mapping_options.case_sensitive = _scanner_ctx->format_scan_context.options.case_sensitive;
...
Status CacheSelectScanner::_fetch_textfile() {
    ...
    const int64_t start_offset = _scanner_ctx->scan_range->offset;
    const int64_t end_offset = _scanner_ctx->scan_range->offset + _scanner_ctx->scan_range->length;
...
Status CacheSelectScanner::_write_entire_file(const std::string& file_path, size_t file_size) {
    ...
    options.datacache_options = _scanner_ctx->datacache_options;
