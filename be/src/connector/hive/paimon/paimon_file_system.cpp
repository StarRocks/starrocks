...
class PaimonOutputStream final : public paimon::OutputStream {
public:
    explicit PaimonOutputStream(std::unique_ptr<WritableFile> file) : _file(std::move(file)) {}
    ~PaimonOutputStream() override = default;

    paimon::Status Close() override {
        const Status status = _file->close();
        if (!status.ok()) {
            return paimon::Status::IOError(
                    fmt::format("Failed to close file {}, reason: {}", _file->filename(), status.detailed_message()));
        }
        return paimon::Status::OK();
    }
    ...
    paimon::Result<int64_t> Write(const char* buffer, int64_t size) override {
        if (size < 0) {
            return paimon::Status::Invalid(fmt::format("negative write size {} for {}", size, _file->filename()));
        }
        const Status status = _file->append(Slice(buffer, static_cast<size_t>(size)));
        if (!status.ok()) {
            return paimon::Status::IOError(
                    fmt::format("Failed to write file {}, reason: {}", _file->filename(), status.detailed_message()));
        }
        return size;
    }

    paimon::Status Flush() override {
        const Status status = _file->flush(WritableFile::FLUSH_SYNC);
        if (!status.ok()) {
            return paimon::Status::IOError(
                    fmt::format("Failed to flush file {}, reason: {}", _file->filename(), status.detailed_message()));
        }
        return paimon::Status::OK();
    }

    paimon::Result<int64_t> GetPos() const override { return static_cast<int64_t>(_file->size()); }
...
paimon::Result<std::unique_ptr<paimon::InputStream>> PaimonFileSystem::Open(const std::string& path,
    ...
    auto cache_input_stream = std::make_shared<CacheInputStream>(shared_buffered_input_stream, filename, file_size, 0,
                                                                 _remote_read_buffer_block_count);
...
paimon::Result<std::unique_ptr<paimon::OutputStream>> PaimonFileSystem::Create(const std::string& path,
    ...
    auto result = _file_system->new_writable_file(options, path);
    if (!result.ok()) {
        return paimon::Status::IOError(
                fmt::format("Failed to create file {}, reason: {}", path, result.status().detailed_message()));
    }
    return std::make_unique<PaimonOutputStream>(std::move(result).value());
}

paimon::Status PaimonFileSystem::Mkdirs(const std::string& path) const {
    const Status status = _file_system->create_dir_recursive(path);
    if (!status.ok()) {
        return paimon::Status::IOError(
                fmt::format("Failed to mkdirs for {}, reason: {}", path, status.detailed_message()));
    }
    return paimon::Status::OK();
}

paimon::Status PaimonFileSystem::Rename(const std::string& src, const std::string& dst) const {
    auto is_directory = _file_system->is_directory(src);
    if (!is_directory.ok()) {
        return paimon::Status::IOError(
                fmt::format("Failed to check whether source path {} is directory or not from rename, reason: {}", src,
                            is_directory.status().detailed_message()));
    }
    if (is_directory.value()) {
        return paimon::Status::IOError("Not support rename directory currently.");
    }
    const Status status = _file_system->rename_file(src, dst);
    if (!status.ok()) {
        return paimon::Status::IOError(
...
paimon::Status PaimonFileSystem::Delete(const std::string& path, bool recursive) const {
    auto is_directory = _file_system->is_directory(path);
    if (!is_directory.ok()) {
        return paimon::Status::IOError(
    ...
    Status status;
    if (!is_directory.value()) {
        status = _file_system->delete_file(path);
    } else if (recursive) {
        status = _file_system->delete_dir_recursive(path);
    } else {
        status = _file_system->delete_dir(path);
    }
    if (!status.ok()) {
        return paimon::Status::IOError(
