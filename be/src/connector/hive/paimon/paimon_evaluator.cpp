...
std::shared_ptr<::paimon::Predicate> PaimonEvaluator::evaluate(starrocks::Expr* conjunct, bool neg) {
    ...
    switch (node_type) {
    case TExprNodeType::type::IS_NULL_PRED:
    case TExprNodeType::type::FUNCTION_CALL:
    case TExprNodeType::type::BINARY_PRED:
    case TExprNodeType::type::IN_PRED:
        break;
    default:
        return nullptr;
    }
    ...
    for (size_t i = 0; i < slots.size(); ++i) {
        ...
        if (slot->id() == ref->slot_id()) {
            ...
            if (node_type == TExprNodeType::type::BINARY_PRED) {
                if (conjunct->get_num_children() != 2) {
                    return nullptr;
                }
            ...
            if (node_type == TExprNodeType::IN_PRED && conjunct->get_num_children() > 1) {
                for (int child_idx = 1; child_idx < conjunct->get_num_children(); ++child_idx) {
                    if (!_ok_to_paimon_literal(conjunct->get_child(child_idx))) {
                        return nullptr;
                    }
                }
