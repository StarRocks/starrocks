...
int256_t::int256_t(float value) {
    ...
    if (abs_value >= max_safe_float) {
        ...
        if (exponent <= 64) {
            ...
            if (exponent >= 23) {
                uint128_t full_value = static_cast<uint128_t>(significand) << (exponent - 23);
                low = static_cast<uint128_t>(full_value);
                high = static_cast<int128_t>(full_value >> 128);
            } else {
                low = static_cast<uint128_t>(significand) >> (23 - exponent);
                high = 0;
            }
...
int256_t::operator double() const {
    ...
    const int double_precision = 53;  // IEEE 754 double 精度位数
