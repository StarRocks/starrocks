...
public:
    ...
    explicit DataSketchesQuantile(uint16_t k, int64_t* memory_usage)
    ...
    void merge(const DataSketchesQuantile& other) {
        if (UNLIKELY(_sketch == nullptr)) {
            _sketch = std::make_unique<quantile_sketch_type>(other._sketch->get_k(), std::less<T>(), alloc_type(_memory_usage));
        }
...
public:
    using alloc_type = STLCountingAllocator<T>;
    using frequent_sketch_type = datasketches::frequent_items_sketch<T, uint64_t, std::hash<T>, std::equal_to<T>, alloc_type>;
    ...
    : _memory_usage(memory_usage), _lg_max_map_size(lg_max_map_size) , _lg_start_map_size(lg_start_map_size){
        _sketch = std::make_unique<frequent_sketch_type>(
                _lg_max_map_size, _lg_start_map_size, std::equal_to<T>(), alloc_type(_memory_usage));
    }
    ...
    : _memory_usage(std::move(other._memory_usage)), _lg_max_map_size(other._lg_max_map_size) ,
    _lg_start_map_size(other._lg_start_map_size), _sketch(std::move(other._sketch)) {}
    ...
    explicit DataSketchesFrequent(const Slice& src, uint8_t lg_max_map_size, uint8_t lg_start_map_size, int64_t* memory_usage)
    : _memory_usage(memory_usage), _lg_max_map_size(lg_max_map_size), _lg_start_map_size(lg_start_map_size) {
        if (!deserialize(src)) {
    ...
    void clear() {
        *_memory_usage = 0;
        this->_sketch = std::make_unique<frequent_sketch_type>(
                _lg_max_map_size, _lg_start_map_size, std::equal_to<T>(), alloc_type(_memory_usage));
    }
...
public:
    ...
    void merge(const DataSketchesTheta& other) {
        if (_sketch_union == nullptr) {
            _sketch_union = std::make_unique<theta_union_type>(theta_union_type::builder(alloc_type(_memory_usage)).build());
        }
    ...
    void serialize_if_needed() const {
        ...
        if (_is_changed) {
            ...
            _sketch_data = std::make_unique<sketch_data_type>(sketch_data_type(sketch_ser.begin(),sketch_ser.end(), sketch_ser.get_allocator()));
            _is_changed = false;
        }
    }
