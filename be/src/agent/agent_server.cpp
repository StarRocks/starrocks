...
#undef HANDLE_TYPE
    ...
    for (const auto& task_item : task_divider) {
        ...
        case TTaskType::EXTERNAL_CLUSTER_SNAPSHOT:
            submit_task_batch<ExternalClusterSnapshotTaskRequest>(TTaskType::EXTERNAL_CLUSTER_SNAPSHOT, all_tasks,
                                                                  get_thread_pool(TTaskType::EXTERNAL_CLUSTER_SNAPSHOT),
                                                                  &TAgentTaskRequest::external_cluster_snapshot_req,
                                                                  run_external_cluster_snapshot_task, &ret_st,
                                                                  _exec_env);
            break;
        case TTaskType::TABLET_RESTORE:
            submit_task_batch<RestoreTabletAgentTaskRequest>(
                    TTaskType::TABLET_RESTORE, all_tasks, get_thread_pool(TTaskType::TABLET_RESTORE),
                    &TAgentTaskRequest::restore_tablet_req, run_restore_tablet_task, &ret_st, _exec_env);
            break;
        case TTaskType::REALTIME_PUSH:
        case TTaskType::PUSH: {
            // should not run here
