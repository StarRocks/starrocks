...
void run_replicate_snapshot_task(const std::shared_ptr<ReplicateSnapshotAgentTaskRequest>& agent_task_req,
    ...
    if (replicate_snapshot_req.tablet_type == TTabletType::TABLET_TYPE_LAKE) {
        // On this branch the dedicated `replicate_file` pool is consumed inside
        // LakeReplicationTxnManager::get_replicate_file_thread_pool(), so the
        // explicit pool parameter on `run_replicate_snapshot_task` is forwarded
        // for API parity with the source branch but not threaded into the call.
        (void)replicate_file_pool;
