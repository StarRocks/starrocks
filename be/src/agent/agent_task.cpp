...
void run_create_tablet_task(const std::shared_ptr<CreateTabletAgentTaskRequest>& agent_task_req, ExecEnv* exec_env) {
    auto& create_tablet_req = const_cast<TCreateTabletReq&>(agent_task_req->task_req);
    TFinishTaskRequest finish_task_request;
    TStatusCode::type status_code = TStatusCode::OK;
    std::vector<std::string> error_msgs;
    ...
    Status preprocess_status = preprocess_default_expr_for_tcolumns(
        create_tablet_req.tablet_schema.columns);
    if (!preprocess_status.ok()) {
        LOG(WARNING) << "[DEFAULT_EXPR_PREPROCESS] Failed to preprocess default_expr in CREATE TABLE: "
