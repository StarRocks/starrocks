...
void TabletSinkSender::set_enable_multi_node_write(
        bool enable, std::unordered_map<int64_t, std::vector<SlotId>> key_slots_by_index) {
    _enable_multi_node_write = enable;
    _multi_node_write_key_slots = std::move(key_slots_by_index);
...
Status TabletSinkSender::_assign_multi_node_write_targets(
    ...
    auto key_slots_iter = _multi_node_write_key_slots.find(channel->index_id());
    const std::vector<SlotId>* key_slots =
            (key_slots_iter != _multi_node_write_key_slots.end() && !key_slots_iter->second.empty())
    ...
    if (key_slots != nullptr) {
        ...
        for (unsigned short selection : selection_idx) {
            ...
            // The hash is avalanched first -- see multi_node_write_node_slot for why the raw hash
            // would confine a tablet's rows to N / gcd(T, N) of its N writers.
            const int64_t target = be_ids[multi_node_write_node_slot(_key_hashes[selection], be_ids.size())];
            _row_target_node[selection] = target;
            ++(target == _local_node_id ? _multi_node_write_local_rows : _multi_node_write_remote_rows);
        }
        return Status::OK();
    }
    // Rows are handed out to nodes in runs of kMultiNodeWriteRowsPerNode, which matches the default
    ...
    // measurement.
    constexpr uint64_t kMultiNodeWriteRowsPerNode = 4096;
    ...
    for (unsigned short selection : selection_idx) {
        ...
        if (tablet_id != last_tablet_id) {
            ...
            last_counter = &_multi_node_write_counters[tablet_id];
        ...
        // lake_multi_node_write_max_nodes, so on a warehouse wider than that bound every sink instance
        ...
        if (!keep_local) {
            target = (*last_be_ids)[((*last_counter)++ / kMultiNodeWriteRowsPerNode) % last_be_ids->size()];
        }
        _row_target_node[selection] = target;
        ++(target == _local_node_id ? _multi_node_write_local_rows : _multi_node_write_remote_rows);
    }
...
Status TabletSinkSender::_send_chunk_by_node(Chunk* chunk, IndexChannel* channel,
    ...
    // Held across the multi-node write routing decision too: local-first probes the local node channel
    ...
    if (_enable_multi_node_write) {
        RETURN_IF_ERROR(_assign_multi_node_write_targets(chunk, channel, tablet_to_be, selection_idx));
    }
...
Status TabletSinkSender::close_wait(RuntimeState* state, Status close_status, TabletSinkProfile* ts_profile,
    ...
    if (_enable_multi_node_write && ts_profile != nullptr && ts_profile->runtime_profile != nullptr) {
        // How this instance's rows were split. A non-zero remote count is EXPECTED whenever the
        // warehouse is wider than lake_multi_node_write_max_nodes: this instance's node is then not in
        ...
        COUNTER_UPDATE(ADD_COUNTER(ts_profile->runtime_profile, "MultiNodeWriteLocalRows", TUnit::UNIT),
                       _multi_node_write_local_rows);
        COUNTER_UPDATE(ADD_COUNTER(ts_profile->runtime_profile, "MultiNodeWriteRemoteRows", TUnit::UNIT),
                       _multi_node_write_remote_rows);
    }
...
Status TabletSinkSender::_collect_txn_logs() {
    ...
    // Multi-node write: several nodes wrote the same tablet, so each of them handed back a PARTIAL txn log
        ...
        for (auto& [node_id, channel] : ordered_nodes) {
            for (auto& log : channel->txn_logs()) {
                ...
                if (it == tablet_logs.end()) {
                    ...
                    slot->Swap(&log);
                    // The structured arrays are the only ones merge_multi_node_write_txn_log maintains;
                ...
                RETURN_IF_ERROR(lake::merge_multi_node_write_txn_log(it->second, &log));
                ++merged_logs;
            }
        }
    }
    if (merged_logs > 0) {
        VLOG(2) << "multi-node write: folded " << merged_logs << " extra txn logs, txn_id=" << _txn_id;
    }
    return Status::OK();
}
