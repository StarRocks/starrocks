...
void LakeServiceImpl::abort_txn(::google::protobuf::RpcController* controller,
    ...
    // Replication copy tasks are not load-channel tasks. Publish a process-local, non-blocking fence
    // so in-flight copies stop and remove their late writes. FE keeps the existing asynchronous,
    // per-worker abort routing; an unreachable worker is ultimately covered by the transaction
    // watermark and full vacuum instead of making this fixed-timeout RPC drain the entire table.
