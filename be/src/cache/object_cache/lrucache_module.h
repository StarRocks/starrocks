...
public:
    LRUCacheModule(const ObjectCacheOptions& options) : _options(options), _own_lru_cache(true) {}
    LRUCacheModule(std::shared_ptr<Cache> lru_cache) : _cache(lru_cache) {}

    ~LRUCacheModule() = default;
