...
public:
    ...
    METRIC_DEFINE_INT_GAUGE(snapshot_mem_bytes, MetricUnit::BYTES);
...
void SystemMetrics::_install_memory_metrics(MetricRegistry* registry) {
    ...
    registry->register_metric("snapshot_mem_bytes", &_memory_metrics->snapshot_mem_bytes);
...
void SystemMetrics::_update_memory_metrics() {
    ...
    if (ExecEnv::GetInstance()->snapshot_mem_tracker() != nullptr) {
