...
//   https://github.com/apache/incubator-doris/blob/master/be/src/util/tuple_row_compare.h
...
#ifndef INF_STARROCKS_BE_SRC_UTIL_TUPLE_ROW_COMPARE_H
#define INF_STARROCKS_BE_SRC_UTIL_TUPLE_ROW_COMPARE_H
...
public:
    ...
    TupleRowComparator(const std::vector<ExprContext*>& key_expr_ctxs_lhs,
                       const std::vector<ExprContext*>& key_expr_ctxs_rhs, const std::vector<bool>& is_asc,
                       const std::vector<bool>& nulls_first)
            : _key_expr_ctxs_lhs(key_expr_ctxs_lhs), _key_expr_ctxs_rhs(key_expr_ctxs_rhs), _is_asc(is_asc) {
    ...
    TupleRowComparator(const std::vector<ExprContext*>& key_expr_ctxs_lhs,
                       const std::vector<ExprContext*>& key_expr_ctxs_rhs, bool is_asc, bool nulls_first)
    ...
    int compare(TupleRow* lhs, TupleRow* rhs) const {
        for (int i = 0; i < _key_expr_ctxs_lhs.size(); ++i) {
            ...
            int result = RawValue::compare(lhs_value, rhs_value, _key_expr_ctxs_lhs[i]->root()->type());
            if (!_is_asc[i]) {
                result = -result;
            }
            if (result != 0) {
