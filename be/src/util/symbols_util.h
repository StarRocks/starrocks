...
//   https://github.com/apache/incubator-doris/blob/master/be/src/util/symbols_util.h
...
#ifndef STARROCKS_BE_SRC_COMMON_UTIL_SYMBOLS_UTIL_H
#define STARROCKS_BE_SRC_COMMON_UTIL_SYMBOLS_UTIL_H
...
public:
    ...
    /// Returns the original input if it cannot be demangled.
    /// Example: "starrocks::foo(int arg1)" => "starrocks::foo"
    ...
    /// Returns the original input if it cannot be demangled.
    /// Example: "starrocks::foo(int arg1)" => "foo"
    ...
    static std::string mangle_user_function(const std::string& fn_name, const std::vector<TypeDescriptor>& arg_types,
                                            bool has_var_args, TypeDescriptor* ret_argument);

    /// Mangles fn_name assuming arguments
    /// (starrocks_udf::FunctionContext*, starrocks_udf::FunctionContext::FunctionStateScope).
