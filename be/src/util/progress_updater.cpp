...
//   https://github.com/apache/incubator-doris/blob/master/be/src/util/progress_updater.cpp
...
void ProgressUpdater::update(int64_t delta) {
    ...
    if (num_complete >= _total) {
        ...
        VLOG(_logging_level) << _label << " 100\% Complete (" << num_complete << " out of " << _total << ")";
        return;
    }
    ...
    if (new_percentage - old_percentage > _update_period) {
        ...
        VLOG(_logging_level) << _label << ": " << new_percentage << "\% Complete (" << num_complete << " out of "
                             << _total << ")";
    }
}
} // namespace starrocks
