...
//   https://github.com/apache/incubator-doris/blob/master/be/src/util/mysql_load_error_hub.cpp
...
Status MysqlLoadErrorHub::gen_sql(MYSQL* my_conn, const LoadErrorHub::ErrorMsg& error_msg,
    ...
    (*sql_stream) << "insert into " << _info.table << " (job_id, error_msg) values(" << error_msg.job_id << ", '"
                  << sql_start << "'); ";
    return Status::OK();
}
...
Status MysqlLoadErrorHub::open_mysql_conn(MYSQL** my_conn) {
    *my_conn = mysql_init(nullptr);
    ...
    if (!mysql_real_connect(*my_conn, _info.host.c_str(), _info.user.c_str(), _info.passwd.c_str(), _info.db.c_str(),
