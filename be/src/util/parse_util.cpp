...
int64_t ParseUtil::parse_mem_spec(const std::string& mem_spec_str, const int64_t memory_limit) {
    ...
    if (multiplier != -1) {
        ...
        auto limit_val =
                StringParser::string_to_float<double>(mem_spec_str.data(), static_cast<int>(number_str_len), &result);
        ...
        auto limit_val =
                StringParser::string_to_int<int64_t>(mem_spec_str.data(), static_cast<int>(number_str_len), &result);
