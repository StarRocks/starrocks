...
Status convert_to_starrocks_type(const arrow::DataType& type, TSlotDescriptorBuilder* builder) {
    switch (type.id()) {
...
Status convert_to_slot_desc(const arrow::Field& field, int column_pos, TSlotDescriptorBuilder* builder) {
    RETURN_IF_ERROR(convert_to_starrocks_type(*field.type(), builder));
...
public:
    FromRowBatchConverter(const RowBatch& batch, const std::shared_ptr<arrow::Schema>& schema, arrow::MemoryPool* pool)
...
private:
    template <typename T>
    typename std::enable_if<std::is_base_of<arrow::PrimitiveCType, T>::value, arrow::Status>::type _visit(
            const T& type) {
...
public:
    ...
    ToRowBatchConverter(const arrow::RecordBatch& batch, const RowDescriptor& row_desc, MemTracker* tracker)
...
private:
    template <typename T>
    typename std::enable_if<std::is_base_of<arrow::PrimitiveCType, typename T::TypeClass>::value, arrow::Status>::type
