...
//   https://github.com/apache/incubator-doris/blob/master/be/src/util/symbols_util.cpp
...
//   namespace::Function(starrocks_udf::FunctionContext*, const starrocks_udf::AnyVal&, etc)
//
...
std::string SymbolsUtil::demangle_name_only(const std::string& symbol) {
    ...
    // Chop off namespace and/or class name if present (e.g. "starrocks::foo" => "foo")
...
static void append_any_val_type(int namespace_id, const TypeDescriptor& type, std::stringstream* s) {
    (*s) << "N";
    // All the AnyVal types are in the starrocks_udf namespace, that token
    // already came with starrocks_udf::FunctionContext
    ...
    (*s) << "E"; // end starrocks_udf namespace
}

std::string SymbolsUtil::mangle_user_function(const std::string& fn_name, const std::vector<TypeDescriptor>& arg_types,
    ...
    // Sequence id for the starrocks_udf namespace token
    int starrocks_udf_seq_id = -1;
    ...
    for (auto& name_token : name_tokens) {
        append_mangled_token(name_token, &ss);
    }
    if (name_tokens.size() > 1) {
        ss << "E"; // End fn namespace
    }
    ss << "PN"; // First argument and start of FunctionContext namespace
    append_mangled_token("starrocks_udf", &ss);
    starrocks_udf_seq_id = seq_id++;
    ...
    for (int i = 0; i < arg_types.size(); ++i) {
        ...
        seq_id += 2; // For starrocks_udf::*Val, which is two tokens.
        append_any_val_type(starrocks_udf_seq_id, arg_types[i], &ss);
    ...
    if (ret_arg_type != nullptr) {
        ...
        if (repeated_symbol_idx != -1) {
            ...
            append_any_val_type(starrocks_udf_seq_id, *ret_arg_type, &ss);
        }
    }
