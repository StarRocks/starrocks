...
void BrpcStubCache::cleanup_expired(const butil::EndPoint& endpoint, EndpointCleanupTask<BrpcStubCache>* cleanup_task) {
    std::lock_guard<SpinLock> l(_lock);

    auto stub_pool = _stub_map.seek(endpoint);
    if (stub_pool == nullptr || (*stub_pool)->_cleanup_task.get() != cleanup_task) {
        return;
    }
...
StatusOr<std::shared_ptr<PInternalService_RecoverableStub>> HttpBrpcStubCache::get_http_stub(
    ...
    if (stub_pair_ptr == nullptr) {
        ...
        timespec tm = butil::microseconds_to_timespec((*stub_pair_ptr).second->deadline_locked());
        ...
        (*stub_pair_ptr).second->renew_deadline_locked(absolute_deadline_us(config::brpc_stub_expire_s));
    }
...
void HttpBrpcStubCache::cleanup_expired(const butil::EndPoint& endpoint,
                                        EndpointCleanupTask<HttpBrpcStubCache>* cleanup_task) {
    std::lock_guard<SpinLock> l(_lock);

    auto stub_pair_ptr = _stub_map.seek(endpoint);
    if (stub_pair_ptr == nullptr || (*stub_pair_ptr).second.get() != cleanup_task) {
        return;
    }
...
void LakeServiceBrpcStubCache::cleanup_expired(const butil::EndPoint& endpoint,
                                               EndpointCleanupTask<LakeServiceBrpcStubCache>* cleanup_task) {
    std::lock_guard<SpinLock> l(_lock);
