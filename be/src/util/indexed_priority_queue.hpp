...
//     execution/resourcegroups/IndexedPriorityQueue.java
...
// LOW_TO_HIGH places the lowest-priority element at the head; HIGH_TO_LOW
// places the highest-priority element at the head. Among equal priorities,
// earlier-inserted elements come first.
...
struct IndexedPriorityQueueEntry {
    ...
    IndexedPriorityQueueEntry(T val, int64_t prio, int64_t gen)
...
struct IndexedPriorityQueueComparator {
    bool operator()(const IndexedPriorityQueueEntry<T>& lhs, const IndexedPriorityQueueEntry<T>& rhs) const {
...
template <typename T,
          IndexedPriorityQueuePriorityOrdering priority_ordering = IndexedPriorityQueuePriorityOrdering::HIGH_TO_LOW>
...
public:
    ...
    // Returns true if the queue was modified (either a new entry inserted, or an
    // existing entry's priority was changed). Returns false when the entry exists
    // with the same priority already.
    bool add_or_update(T element, int64_t priority) {
        ...
        IndexedPriorityQueueEntry<T> entry{element, priority, _generation++};
        _queue.insert(entry);
        _index.insert_or_assign(entry.value, entry);
        return true;
    }
    ...
    public:
        ...
        Iterator() = default;
        explicit Iterator(typename std::set<IndexedPriorityQueueEntry<T>,
    ...
    private:
        typename std::set<IndexedPriorityQueueEntry<T>,
                          IndexedPriorityQueueComparator<T, priority_ordering>>::const_iterator _iter;
    };
...
private:
    ...
    std::set<IndexedPriorityQueueEntry<T>, IndexedPriorityQueueComparator<T, priority_ordering>> _queue;
