...
public:
    ...
    size_t Next(bool* value, size_t max_run) { return NextWithLimit(value, std::min(num_bits_, max_run + offset_)); }
...
public:
    Bitmap(int64_t num_bits) {
        ...
        buffer_.resize(BitUtil::round_up_numi64(num_bits));
        num_bits_ = num_bits;
    }
    ...
    static int64_t MemUsage(int64_t num_bits) {
        DCHECK_GE(num_bits, 0);
        return BitUtil::round_up_numi64(num_bits) * sizeof(int64_t);
    }
