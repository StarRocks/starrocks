...
template <class T>
struct IsTriviallyMoveAssignableReference<T&> : phmap::is_trivially_move_assignable<T>::type {};
...
template <class T,
          class IsNoexcept = std::integral_constant<bool, noexcept(swap(std::declval<T&>(), std::declval<T&>()))>>
...
using btree_is_key_compare_to =
        std::is_convertible<phmap::invoke_result_t<Compare, const T&, const T&>, phmap::weak_ordering>;
...
struct common_params {
    ...
    using node_count_type =
            phmap::conditional_t<(kNodeValueSpace / sizeof(value_type) > (std::numeric_limits<uint8_t>::max)()),
...
private:
    ...
    // value j in node x.
    void uninitialized_move_n(const size_type n, const size_type i, const size_type j, btree_node* x,
...
public:
    ...
    // binary size reasons.
    template <typename N, typename R, typename P,
              phmap::enable_if_t<std::is_same<btree_iterator<N, R, P>, iterator>::value &&
...
private:
    ...
    template <typename N, typename R, typename P,
              phmap::enable_if_t<std::is_same<btree_iterator<N, R, P>, const_iterator>::value &&
...
private:
    ...
    node_type* allocate(const size_type sz) {
        return reinterpret_cast<node_type*>(
                phmap::priv::Allocate<node_type::Alignment()>(mutable_allocator(), (size_t)sz));
    }
...
void btree_node<P>::rebalance_left_to_right(const int to_move, btree_node* right, allocator_type* alloc) {
    ...
    if (right->count() >= to_move) {
        ...
        params_type::move(alloc, slot(count() - (to_move - 1)), slot(count() - uninitialized_remaining),
                          right->slot(0));
    }
...
void btree_node<P>::swap(btree_node* x, allocator_type* alloc) {
    ...
    if (!leaf()) {
        ...
        std::swap_ranges(&smaller->mutable_child(0), &smaller->mutable_child(smaller->count() + 1),
...
constexpr bool btree<P>::static_assert_validation() {
    ...
    static_assert(std::is_same<compare_result_type, bool>::value ||
...
protected:
    ...
    template <class K>
    using key_arg =
            typename KeyArg<IsTransparent<typename Tree::key_compare>::value>::template type<K,
...
public:
    ...
    template <typename T, typename phmap::enable_if_t<
                                  phmap::conjunction<std::is_same<value_type, typename T::value_type>,
...
public:
    ...
    iterator insert(const_iterator hint, node_type&& node) {
        if (!node) return this->end();
        iterator res = this->tree_.insert_hint_multi(iterator(hint),
