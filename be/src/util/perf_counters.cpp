...
//   https://github.com/apache/incubator-doris/blob/master/be/src/util/perf_counters.cpp
...
void PerfCounters::pretty_print(ostream* s) const {
    ...
    for (int s = 0; s < _snapshots.size(); s++) {
        ...
        for (int i = 0; i < snapshot.size(); ++i) {
            stream << setw(PRETTY_PRINT_WIDTH) << PrettyPrinter::print(snapshot[i], _counters[i].type);
        }
