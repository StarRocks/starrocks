...
private:
    absl::StatusOr<std::shared_ptr<staros::starlet::fslib::FileSystem>> get_shard_filesystem(
            int64_t shard_id, staros::starlet::fslib::ReplicationOptions* replication_options = nullptr) {
...
std::shared_ptr<FileSystem> new_fs_starlet(int64_t shard_id, bool use_raw_path) {
    ...
    std::shared_ptr<staros::starlet::fslib::FileSystem> shard_fs = fs_st.value().file_system;
