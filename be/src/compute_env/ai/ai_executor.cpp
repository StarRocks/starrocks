...
Status AIExecutor::_publish_locked(const AIRuntimeConfig& previous, AIRuntimeConfig candidate, bool worker_changed) {
    RETURN_IF_ERROR(candidate.validate());

    if (worker_changed) {
        RETURN_IF_ERROR(_completion_executor->update_worker_threads(candidate.worker_thread_num));
    }

    Status publish_status = _config_source->update(std::move(candidate));
    if (!publish_status.ok()) {
        if (worker_changed) {
            Status rollback_status = _completion_executor->update_worker_threads(previous.worker_thread_num);
            CHECK(rollback_status.ok()) << "failed to restore AI completion worker count: " << rollback_status;
        }
    ...
    try {
        {
            ...
            update_status = _publish_locked(previous, std::move(candidate), worker_changed);
...
Status AIExecutor::update_on_error(std::string_view value) {
    ...
    try {
        ...
        return _publish_locked(previous, std::move(candidate), false);
    } catch (const std::bad_alloc&) {
        return Status::MemoryLimitExceeded("failed to allocate AI runtime config update");
    }
}
