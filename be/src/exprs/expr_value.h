...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exprs/expr_value.h
...
#ifndef STARROCKS_BE_SRC_QUERY_EXPRS_EXPR_VALUE_H
#define STARROCKS_BE_SRC_QUERY_EXPRS_EXPR_VALUE_H
...
struct ExprValue {
    ...
    int16_t smallint_val = 0;
    int32_t int_val = 0;
    int64_t bigint_val = 0;
    __int128 large_int_val = 0;
    float float_val = 0.0;
    ...
    ExprValue(const std::string& str)
            : string_data(str), string_val(const_cast<char*>(string_data.data()), string_data.size()) {}
    ...
    void* set_to_min(const TypeDescriptor& type) {
        ...
        case TYPE_TINYINT:
            tinyint_val = std::numeric_limits<int8_t>::lowest();
            return &tinyint_val;

        case TYPE_SMALLINT:
            smallint_val = std::numeric_limits<int16_t>::lowest();
            return &smallint_val;

        case TYPE_INT:
            int_val = std::numeric_limits<int32_t>::lowest();
            return &int_val;

        case TYPE_BIGINT:
            bigint_val = std::numeric_limits<int64_t>::lowest();
            return &bigint_val;

        case TYPE_LARGEINT:
            large_int_val = std::numeric_limits<int128_t>::lowest();
            return &large_int_val;
