...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exprs/agg_fn.cc
...
AggFn::AggFn(const TExprNode& tnode, const SlotDescriptor& intermediate_slot_desc,
    ...
    DCHECK_EQ(TypeDescriptor::from_thrift(tnode.type).type, TypeDescriptor::from_thrift(_fn.ret_type).type);
...
Status AggFn::Init(const RowDescriptor& row_desc, RuntimeState* state) {
    ...
    DCHECK_EQ(intermediate_slot_desc_.type().type, TypeDescriptor::from_thrift(aggregate_fn.intermediate_type).type);
    ...
    RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(
            _fn.id, aggregate_fn.init_fn_symbol, _fn.hdfs_location, _fn.checksum, &init_fn_, &_cache_entry));
    RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(
            _fn.id, aggregate_fn.update_fn_symbol, _fn.hdfs_location, _fn.checksum, &update_fn_, &_cache_entry));

    // Merge() is not defined for purely analytic function.
    if (!aggregate_fn.is_analytic_only_fn) {
        RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(
                _fn.id, aggregate_fn.merge_fn_symbol, _fn.hdfs_location, _fn.checksum, &merge_fn_, &_cache_entry));
    }
    // Serialize(), GetValue(), Remove() and Finalize() are optional
    if (!aggregate_fn.serialize_fn_symbol.empty()) {
        RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(_fn.id, aggregate_fn.serialize_fn_symbol,
                                                                        _fn.hdfs_location, _fn.checksum, &serialize_fn_,
                                                                        &_cache_entry));
    }
    if (!aggregate_fn.get_value_fn_symbol.empty()) {
        RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(_fn.id, aggregate_fn.get_value_fn_symbol,
                                                                        _fn.hdfs_location, _fn.checksum, &get_value_fn_,
                                                                        &_cache_entry));
    }
    if (!aggregate_fn.remove_fn_symbol.empty()) {
        RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(
                _fn.id, aggregate_fn.remove_fn_symbol, _fn.hdfs_location, _fn.checksum, &remove_fn_, &_cache_entry));
    }
    if (!aggregate_fn.finalize_fn_symbol.empty()) {
        RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(_fn.id, _fn.aggregate_fn.finalize_fn_symbol,
                                                                        _fn.hdfs_location, _fn.checksum, &finalize_fn_,
                                                                        &_cache_entry));
    }
    return Status::OK();
}

Status AggFn::Create(const TExpr& texpr, const RowDescriptor& row_desc, const SlotDescriptor& intermediate_slot_desc,
