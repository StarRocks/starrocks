...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exprs/new_in_predicate.cpp
...
template <typename T, typename SetType>
void InPredicate::set_lookup_prepare(FunctionContext* ctx, FunctionContext::FunctionStateScope scope) {
...
void InPredicate::set_lookup_close(FunctionContext* ctx, FunctionContext::FunctionStateScope scope) {
    if (scope != FunctionContext::FRAGMENT_LOCAL) {
        return;
    }
    SetLookupState<SetType>* state = reinterpret_cast<SetLookupState<SetType>*>(ctx->get_function_state(scope));
    delete state;
}

template <typename T, typename SetType, bool not_in, InPredicate::Strategy strategy>
BooleanVal InPredicate::templated_in(FunctionContext* ctx, const T& val, int num_args, const T* args) {
    if (val.is_null) {
    ...
    if (strategy == SET_LOOKUP) {
        SetLookupState<SetType>* state =
                reinterpret_cast<SetLookupState<SetType>*>(ctx->get_function_state(FunctionContext::FRAGMENT_LOCAL));
        DCHECK(state != NULL);
...
template <typename T>
BooleanVal InPredicate::iterate(const FunctionContext::TypeDesc* type, const T& val, int num_args, const T* args) {
...
#define IN_FUNCTIONS(AnyValType, SetType, type_name)                                                                  \
    BooleanVal InPredicate::in_set_lookup(FunctionContext* context, const AnyValType& val, int num_args,              \
                                          const AnyValType* args) {                                                   \
        return templated_in<AnyValType, SetType, false, SET_LOOKUP>(context, val, num_args, args);                    \
    }                                                                                                                 \
                                                                                                                      \
    BooleanVal InPredicate::not_in_set_lookup(FunctionContext* context, const AnyValType& val, int num_args,          \
                                              const AnyValType* args) {                                               \
        return templated_in<AnyValType, SetType, true, SET_LOOKUP>(context, val, num_args, args);                     \
    }                                                                                                                 \
                                                                                                                      \
    BooleanVal InPredicate::in_iterate(FunctionContext* context, const AnyValType& val, int num_args,                 \
                                       const AnyValType* args) {                                                      \
        return templated_in<AnyValType, SetType, false, ITERATE>(context, val, num_args, args);                       \
    }                                                                                                                 \
                                                                                                                      \
    BooleanVal InPredicate::not_in_iterate(FunctionContext* context, const AnyValType& val, int num_args,             \
                                           const AnyValType* args) {                                                  \
    ...
    void InPredicate::set_lookup_prepare_##type_name(FunctionContext* ctx,                                            \
        ...
        set_lookup_prepare<AnyValType, SetType>(ctx, scope);                                                          \
                                                                                                                      ...
                                                                                                                      \
    void InPredicate::set_lookup_close_##type_name(FunctionContext* ctx, FunctionContext::FunctionStateScope scope) { \
...
template BooleanVal InPredicate::iterate<IntVal>(const FunctionContext::TypeDesc*, const IntVal&, int, const IntVal*);
} // namespace starrocks
