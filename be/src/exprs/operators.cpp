...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exprs/operators.cpp
...
#define BINARY_OP_FN(NAME, TYPE_NAME, TYPE, OP)                                                            \
    TYPE Operators::NAME##_##TYPE_NAME##_##TYPE_NAME(FunctionContext* c, const TYPE& v1, const TYPE& v2) { \
...
#define BINARY_PREDICATE_NUMERIC_FN(NAME, TYPE_NAME, TYPE, OP)                                                   \
    BooleanVal Operators::NAME##_##TYPE_NAME##_##TYPE_NAME(FunctionContext* c, const TYPE& v1, const TYPE& v2) { \
        if (v1.is_null || v2.is_null) return BooleanVal::null();                                                 \
        return BooleanVal(v1.val OP v2.val);                                                                     \
    }

#define BINARY_PREDICATE_NONNUMERIC_FN(NAME, TYPE_NAME, FUNC_NAME, TYPE, STARROCKS_TYPE, OP)                     \
    BooleanVal Operators::NAME##_##TYPE_NAME##_##TYPE_NAME(FunctionContext* c, const TYPE& v1, const TYPE& v2) { \
        if (v1.is_null || v2.is_null) return BooleanVal::null();                                                 \
        STARROCKS_TYPE iv1 = STARROCKS_TYPE::from_##FUNC_NAME(v1);                                               \
        STARROCKS_TYPE iv2 = STARROCKS_TYPE::from_##FUNC_NAME(v2);                                               \
...
#define BINARY_PREDICATE_ALL_TYPES(NAME, OP)                                                  \
    ...
    BINARY_PREDICATE_NONNUMERIC_FN(NAME, datetime_val, datetime_val, DateTimeVal, DateTimeValue, OP);
