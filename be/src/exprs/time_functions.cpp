...
StatusOr<ColumnPtr> TimeFunctions::from_unix_to_datetime_with_format(FunctionContext* context,
    ...
    if (state->const_format) {
        ...
        if (func_version > 4) {
            return from_unix_with_format_const<TYPE_BIGINT>(format_content, context, columns);
        } else {
            return from_unix_with_format_const<TYPE_INT>(format_content, context, columns);
        }
    }

    if (func_version > 4) {
        return from_unix_with_format_general<TYPE_BIGINT>(context, columns);
    } else {
        return from_unix_with_format_general<TYPE_INT>(context, columns);
    }
}
