...
struct ArrayAggAnyTypeAggregateState {
    void update(const Column& column, size_t offset, size_t count) { data_column->append(column, offset, count); }
...
class ArrayAggAnyTypeAggregateFunction
        : public AggregateFunctionBatchHelper<ArrayAggAnyTypeAggregateState, ArrayAggAnyTypeAggregateFunction> {
public:
    void update(FunctionContext* ctx, const Column** columns, AggDataPtr __restrict state,
                size_t row_num) const override {
        if (this->data(state).data_column == nullptr) {
            this->data(state).data_column = ctx->create_column(*ctx->get_arg_type(0), false);
        }
        this->data(state).update(*columns[0], row_num, 1);
    }
    ...
    void merge(FunctionContext* ctx, const Column* column, AggDataPtr __restrict state, size_t row_num) const override {
        ...
        auto* element_data_column = ColumnHelper::get_data_column(&array_element);
