...
class LeadLagWindowFunction final : public ValueWindowFunction<LT, LeadLagState<LT, ignoreNulls>, T> {
    ...
    void reset(FunctionContext* ctx, const Columns& args, AggDataPtr __restrict state) const override {
        ...
        if (arg2->is_constant()) {
            ...
            } else if constexpr (lt_is_array<LT>) {
        ...
        if constexpr (ignoreNulls) {
            ...
            if (!found_target || columns[0]->is_null(value_index)) {
                if (this->data(state).default_value_is_constant) {
                    ...
                    // non-const default: safe-guard columns[2]
                    const Column* def_col = columns[2];
                    const bool def_col_ok = (def_col != nullptr) && (current_row >= 0) &&
                                            (current_row < static_cast<int64_t>(def_col->size()));
                    if (def_col_ok && !def_col->is_null(current_row)) {
                        this->data(state).is_null = false;
                        const Column* data_column = ColumnHelper::get_data_column(def_col);
                        const auto* column = down_cast<const InputColumnType*>(data_column);
                        AggDataTypeTraits<LT>::assign_value(this->data(state).value,
            ...
            if ((frame_start < peer_group_start) | (frame_end > peer_group_end)) {
                if (this->data(state).default_is_null) {
                    ...
                    if (this->data(state).default_value_is_constant) {
                        ...
                        // MySQL-compatible semantics: when the offset goes out of range
                        // and the third parameter is a column, use the current row's value
                        // of that column as the default. Derive current row index from frame.
                        // For lag: frame_end - 1 == current_row - offset => current_row = frame_end - 1 + offset
                        // For lead: frame_end - 1 == current_row + offset => current_row = frame_end - 1 - offset
                        int64_t current_row_index = isLag ? (frame_end - 1 + this->data(state).offset)
                                                         : (frame_end - 1 - this->data(state).offset);
                        const Column* def_col = columns[2];
                        if (def_col != nullptr && current_row_index >= 0 &&
                            ...
                            if (!this->data(state).is_null) {
                                ...
                                AggDataTypeTraits<LT>::assign_value(
                                        this->data(state).value,
                                        AggDataTypeTraits<LT>::get_row_ref(*column, static_cast<size_t>(current_row_index)));
                            }
                        } else {
                            this->data(state).is_null = true;
                        }
