    ...
    void operator()(AggregateFuncResolver* resolver) {
        if constexpr (lt_is_aggregate<kt>) {
            ...
            if constexpr (lt_is_largeint<kt>) {
                using MyHashMap = phmap::flat_hash_map<KeyCppType, Datum, Hash128WithSeed<PhmapSeed1>>;
                ...
                using MyHashMap = phmap::flat_hash_map<KeyCppType, Datum, StdHash<KeyCppType>>;
                auto func = std::make_shared<MapAggAggregateFunction<kt, MyHashMap>>();
                resolver->add_aggregate_mapping_notnull<kt, TYPE_MAP, AggregateFunctionPtr>("map_agg", false, func);
            } else if constexpr (lt_is_string<kt>) {
                using MyHashMap = phmap::flat_hash_map<SliceWithHash, Datum, HashOnSliceWithHash, EqualOnSliceWithHash>;
                auto func = std::make_shared<MapAggAggregateFunction<kt, MyHashMap>>();
                resolver->add_aggregate_mapping_notnull<kt, TYPE_MAP, AggregateFunctionPtr>("map_agg", false, func);
            } else {
