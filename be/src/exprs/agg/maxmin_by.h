...
#include "util/raw_container.h"
        ...
        void get_values(FunctionContext* ctx, ConstAggDataPtr __restrict state, Column* dst, size_t start,
            ...
            if constexpr (State::not_filter_nulls_flag) {
                if (this->data(state).null_result) {
                    ...
                    for (size_t i = start; i < end; ++i) {
                        ColumnHelper::get_data_column(dst)->deserialize_and_append(
                                this->data(state).buffer_result.data());
                    }
                }
            } else {
                if (this->data(state).buffer_result.empty()) {
    ...
    public:
        ...
        void convert_to_serialize_format(FunctionContext* ctx, const Columns& src, size_t chunk_size,
            ...
            const BinaryColumn* col_maxmin =
                    down_cast<const BinaryColumn*>(ColumnHelper::get_data_column(src[1].get()));
