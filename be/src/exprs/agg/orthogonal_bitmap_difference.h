...
class OrthogonalBitmapDifferenceAggregateFunction final
        : public AggregateFunctionBatchHelper<BitmapIntersectAggregateState<BitmapRuntimeCppType<LT>>,
                                              OrthogonalBitmapDifferenceAggregateFunction<LT, T>> {
public:
    ...
    void finalize_to_column(FunctionContext* ctx, ConstAggDataPtr __restrict state, Column* to) const override {
        ...
        auto head_arg_column = ctx->get_constant_column(2);
        auto head_arg_value = ColumnHelper::get_const_value<LT>(head_arg_column);

        BitmapValue head_bitmap;
        if constexpr (lt_is_string_or_binary<LT>) {
            std::string key(head_arg_value.data, head_arg_value.size);
            head_bitmap = intersect.get_bitmap(key);
        } else {
            head_bitmap = intersect.get_bitmap(head_arg_value);
        }

        BitmapValue result = head_bitmap;
        BitmapValue intersect_bitmap = intersect.intersect();
        result -= intersect_bitmap;

        col->append(std::move(result));
    }

    std::string get_name() const override { return "orthogonal bitmap difference"; }
};
