...
public:
    ...
    void merge(FunctionContext* ctx, const Column* column, AggDataPtr __restrict state, size_t row_num) const override {
        ...
        if constexpr (lt_is_string_or_binary<LT>) {
            ...
            const Column* element_data_column = ColumnHelper::get_data_column(&array_element);
            size_t end = offset_size.first + offset_size.second;
            for (size_t i = offset_size.first; i < end; ++i) {
                if (!array_element.is_null(i)) {
                    this->data(state).update_with_slice(mem_pool,
                                                        ColumnHelper::get_binary_slice(element_data_column, i));
                }
            }
        } else {
    ...
    void update_batch_single_state_with_frame(FunctionContext* ctx, AggDataPtr __restrict state, const Column** columns,
        ...
        if constexpr (lt_is_string_or_binary<LT>) {
            ...
            for (auto i = frame_start; i < frame_end; ++i) {
                this->data(state).update_with_slice(mem_pool, ColumnHelper::get_binary_slice(columns[0], i));
            }
