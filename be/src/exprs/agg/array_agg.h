...
struct ArrayAggAggregateState {
    ...
    void update(const ColumnType& column, size_t offset, size_t count) {
...
public:
    void create(FunctionContext* ctx, AggDataPtr __restrict ptr) const override {
        ...
        for (auto i = 0; i < num; ++i) {
            ...
            if (arg_type->type == PrimitiveType::TYPE_STRUCT || arg_type->type == PrimitiveType::TYPE_ARRAY ||
                arg_type->type == PrimitiveType::TYPE_MAP) {
        ...
        if (ctx->get_is_distinct()) {
            ...
            for (auto row_id = 0; row_id < elem_size; row_id++) {
                for (auto next_id = row_id + 1; next_id < elem_size; next_id++) {
                    if (res->compare_at(next_id, row_id, *res, 1) == 0) {
                        is_duplicated = true;
                        filter[row_id] = 0;
                        break;
                    }
