...
StatusOr<ColumnPtr> CastStringToArray::evaluate_checked(ExprContext* context, Chunk* input_chunk) {
    if (_constant_res != nullptr && _constant_res->is_constant()) {
        ...
        if (input->only_null()) {
            return ColumnHelper::create_const_null_column(context->allocator(), rows);
        } else {
            return ConstColumn::create(context->allocator(), std::move(*(input->data_column())).clone(), rows);
        }
    }
    ...
    UInt32Column::MutablePtr offsets = UInt32Column::create(context->allocator());
    ...
    if (element_type != TYPE_VARCHAR && element_type != TYPE_CHAR) {
        ...
        elements = ColumnHelper::cast_to_nullable_column(context->allocator(), std::move(cast_res));
    }
...
StatusOr<ColumnPtr> CastVariantToArray::evaluate_checked(ExprContext* context, Chunk* input_chunk) {
    ...
    MutableColumnPtr res =
            ArrayColumn::create(context->allocator(), std::move(elements)->as_mutable_ptr(), std::move(offsets));
