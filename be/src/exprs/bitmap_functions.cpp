...
StatusOr<ColumnPtr> BitmapFunctions::bitmap_to_binary(FunctionContext* context, const starrocks::Columns& columns) {
    ...
    for (int row = 0; row < size; ++row) {
        BitmapValue* bitmap = viewer.value(row);
        int serialize_size = bitmap->getSizeInBytes();
        if (serialize_size > cur_buf_size) {
            buf.reset(new char[serialize_size]);
            cur_buf_size = serialize_size;
        }

        bitmap->write((char*)buf.get());
        builder.append(Slice(buf.get(), serialize_size));
    }
    ColumnPtr col = builder.build(ColumnHelper::is_all_const(columns));
    if (col->capacity_limit_reached()) {
        return Status::InternalError("Size of binary column generated by bitmap_to_binary reaches limit (4G)");
    } else {
        return col;
    }
}
...
StatusOr<ColumnPtr> BitmapFunctions::bitmap_from_binary(FunctionContext* context, const Columns& columns) {
    ...
    for (int row = 0; row < size; ++row) {
        ...
        BitmapValue bitmap(src_value.data);
        builder.append(std::move(bitmap));
    }
    return builder.build(ColumnHelper::is_all_const(columns));
}
