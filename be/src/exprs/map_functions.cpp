    ...
    if (!keys_column->has_null() && !values_column->has_null()) {
        ...
        if (keys_null != nullptr) {
            if (values_null != nullptr) {
                ...
                ColumnHelper::or_two_filters(num_rows, null_column->get_data().data(), values_null->immutable_data().data());
            } else {
                null_column = NullColumn::static_pointer_cast(keys_null->clone());
            }
        } else {
...
void MapFunctions::_filter_map_items(const MapColumn* src_column, const ColumnPtr& raw_filter, MapColumn* dest_column,
    ...
    dest_column->keys_column_mutable_ptr()->append_selective(src_column->keys(), indexes);
    dest_column->values_column_mutable_ptr()->append_selective(src_column->values(), indexes);
}
...
static inline std::tuple<NullColumn::MutablePtr, Column*, Column*, const UInt32Column*> unpack_map_column(
    ...
    if (map->is_nullable()) {
        ...
        map_null = nullable->null_column_mutable_ptr();
    } else {
        map_null = NullColumn::create(input->size(), 0);
        map_col = down_cast<MapColumn*>(map.get());
    }

    return {std::move(map_null), map_col->keys_column_mutable_ptr().get(), map_col->values_column_mutable_ptr().get(), &map_col->offsets()};
}
