...
StatusOr<ColumnPtr> ArrayFunctions::zip(FunctionContext* ctx, const Columns& columns) {
    RETURN_IF_COLUMNS_ONLY_NULL(columns);

    if (columns.empty()) {
        return Status::InvalidArgument("zip function requires at least one argument");
    }
    ...
    // Build struct fields from all array elements
    std::vector<ColumnPtr> struct_fields;
    for (size_t i = 0; i < num_arrays; ++i) {
        struct_fields.push_back(array_columns[i]->elements().clone_empty());
    }
    ...
    auto result_elements = StructColumn::create(struct_fields, field_names);
    auto result_offsets = UInt32Column::create();
            ...
            for (size_t elem_idx = 0; elem_idx < max_length; ++elem_idx) {
                for (size_t array_idx = 0; array_idx < num_arrays; ++array_idx) {
                    ...
                    if (elem_idx < array_length) {
                        ...
                        struct_fields[array_idx]->append(array_columns[array_idx]->elements(),
                                                         offset_size.first + elem_idx, 1);
                    } else {
                        // Element doesn't exist, add NULL
                        struct_fields[array_idx]->append_nulls(1);
                    }
                }
            }
        }
    ...
    auto result_array = ArrayColumn::create(result_elements, result_offsets);
