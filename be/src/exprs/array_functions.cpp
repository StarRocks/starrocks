...
StatusOr<ColumnPtr> ArrayFunctions::array_length([[maybe_unused]] FunctionContext* context, const Columns& columns) {
    ...
    if (columns[0]->is_constant()) {
        ...
        auto const_column = ConstColumn::create(context->allocator(), std::move(col_result), num_rows);
        return const_column;
    } else {
        ...
        if (arg0->has_null()) {
            ...
            return NullableColumn::create(
                    context->allocator(), std::move(col_result),
                    NullColumn::static_pointer_cast(std::move(*(down_cast<const NullableColumn*>(arg0)->null_column()))
                                                            .mutate()));
        } else {
            return col_result;
        }
...
static StatusOr<ColumnPtr> do_array_append(memory::Allocator* allocator, const Column& elements,
                                           const UInt32Column& offsets, const Column& data) {
    ...
    auto result_array = ArrayColumn::create(allocator, elements.clone_empty(), UInt32Column::create(allocator));
...
StatusOr<ColumnPtr> ArrayFunctions::array_append([[maybe_unused]] FunctionContext* context, const Columns& columns) {
    ...
    if (arg1->only_null()) {
        result = do_array_append<true, true>(context->allocator(), *element, *offsets, *arg1);
    } else if (const_data) {
        result = do_array_append<false, true>(context->allocator(), *element, *offsets, *arg1);
    } else {
        result = do_array_append<false, false>(context->allocator(), *element, *offsets, *arg1);
    }
    RETURN_IF_ERROR(result);

    if (nullable_array != nullptr) {
        return NullableColumn::create(
                context->allocator(), Column::mutate(std::move(result.value())),
                NullColumn::static_pointer_cast(std::move(*nullable_array->null_column()).mutate()));
    }
    return result;
}
...
public:
    static StatusOr<ColumnPtr> evaluate(memory::Allocator* allocator, const ColumnPtr& array, const ColumnPtr& element) {
        return _array_remove_generic(allocator, array, element);
    }

private:
    template <bool NullableElement, bool NullableTarget, bool ConstTarget, typename ElementColumn,
              typename TargetColumn>
    static StatusOr<ColumnPtr> _process(memory::Allocator* allocator, const ElementColumn& elements,
        ...
        auto result_array = ArrayColumn::create(allocator,
                                                NullableColumn::create(allocator, elements.clone_empty(),
    ...
    static StatusOr<ColumnPtr> _array_remove(memory::Allocator* allocator, const Column& array_elements,
                                             const UInt32Column& array_offsets,
...
#define HANDLE_ELEMENT_TYPE(ElementType)                                                                          \
    do {                                                                                                          \
        if (typeid(*elements_ptr) == typeid(ElementType)) {                                                       \
            return _process<NullableElement, NullableTarget, ConstTarget>(allocator,                             \
...
#undef HANDLE_ELEMENT_TYPE
    ...
    static StatusOr<ColumnPtr> _array_remove_non_nullable(memory::Allocator* allocator, const ArrayColumn& array,
                                                          const Column& arg) {
        ...
        if (targets->only_null()) {
            ...
            targets_holder = NullableColumn::create(allocator, std::move(data), NullColumn::create(allocator));
        ...
        if (nullable_element && nullable_target) {
            return _array_remove<true, true, false>(allocator, *elements, offsets, *targets);
        } else if (nullable_element) {
            return const_target ? _array_remove<true, false, true>(allocator, *elements, offsets, *targets)
                                : _array_remove<true, false, false>(allocator, *elements, offsets, *targets);
        } else if (nullable_target) {
            return _array_remove<false, true, false>(allocator, *elements, offsets, *targets);
        } else {
            return const_target ? _array_remove<false, false, true>(allocator, *elements, offsets, *targets)
                                : _array_remove<false, false, false>(allocator, *elements, offsets, *targets);
        }
    }

    static StatusOr<ColumnPtr> _array_remove_generic(memory::Allocator* allocator, const ColumnPtr& array,
                                                     const ColumnPtr& target) {
        if (array->is_nullable()) {
            ...
            ASSIGN_OR_RETURN(auto result, _array_remove_non_nullable(allocator, *array_col, *target))
            DCHECK_EQ(nullable->size(), result->size());
            return NullableColumn::create(allocator, Column::mutate(std::move(result)),
                                          NullColumn::static_pointer_cast(std::move(*nullable->null_column()).mutate()));
        }

        return _array_remove_non_nullable(allocator, down_cast<const ArrayColumn&>(*array), *target);
    }
};
...
public:
    static StatusOr<ColumnPtr> evaluate(memory::Allocator* allocator, const ColumnPtr& col) {
        if (col->is_constant()) {
            ...
            return ConstColumn::create(allocator, std::move(arr_col_h), input->size());
...
#define DEFINE_ARRAY_CUMSUM_FN(NAME, TYPE)                                                              \
    StatusOr<ColumnPtr> ArrayFunctions::array_cum_sum_##NAME(FunctionContext* context,                  \
                                                             const Columns& columns) {                  \
        DCHECK_EQ(columns.size(), 1);                                                                   \
        RETURN_IF_COLUMNS_ONLY_NULL(columns);                                                           \
        const ColumnPtr& arg0 = columns[0];                                                             \
                                                                                                        \
        return ArrayCumSumImpl<TYPE>::evaluate(context->allocator(), arg0);                             \
    }
...
StatusOr<ColumnPtr> ArrayFunctions::array_remove([[maybe_unused]] FunctionContext* context, const Columns& columns) {
    ...
    return ArrayRemoveImpl::evaluate(context->allocator(), arg0, arg1);
}
...
public:
    static StatusOr<ColumnPtr> evaluate(memory::Allocator* allocator, const Column& array, const Column& element) {
        return _array_contains_generic(allocator, array, element);
    }
...
private:
    ...
    static StatusOr<ColumnPtr> _process(memory::Allocator* allocator, const ElementColumn& elements,
                                        ...
                                        const TargetColumn& targets, const NullColumn::ImmContainer& null_map_elements,
                                        const NullColumn::ImmContainer& null_map_targets) {
        auto result = ReturnType::create(allocator);
    ...
    static StatusOr<ColumnPtr> _array_contains(memory::Allocator* allocator, const Column& array_elements,
                                               const UInt32Column& array_offsets,
                                               const Column& argument) {
        const Column* elements_ptr = &array_elements;
        const Column* targets_ptr = &argument;
...
#define HANDLE_ELEMENT_TYPE(ElementType)                                                                          \
    do {                                                                                                          \
        if (typeid(*elements_ptr) == typeid(ElementType)) {                                                       \
            return _process<NullableElement, NullableTarget, ConstElement, ConstTarget>(allocator,               \
                    *down_cast<const ElementType*>(elements_ptr), array_offsets, *targets_ptr, null_map_elements, \
                    null_map_targets);                                                                            \
        }                                                                                                         \
    } while (0)
...
#undef HANDLE_ELEMENT_TYPE
    ...
    static StatusOr<ColumnPtr> _array_contains_non_nullable(memory::Allocator* allocator, const ArrayColumn& array,
                                                            const Column& arg) {
        bool nullable_element = false;
        bool nullable_target = false;
        bool const_target = false;
        ...
        if (nullable_element && nullable_target) {
            return _array_contains<true, true, ConstArray, false>(allocator, *elements, offsets, *targets);
        } else if (nullable_element) {
            return const_target ? _array_contains<true, false, ConstArray, true>(allocator, *elements, offsets, *targets)
                                : _array_contains<true, false, ConstArray, false>(allocator, *elements, offsets, *targets);
        } else if (nullable_target) {
            return _array_contains<false, true, ConstArray, false>(allocator, *elements, offsets, *targets);
        } else {
            return const_target ? _array_contains<false, false, ConstArray, true>(allocator, *elements, offsets, *targets)
                                : _array_contains<false, false, ConstArray, false>(allocator, *elements, offsets, *targets);
        }
    }

    static StatusOr<ColumnPtr> _array_contains_generic(memory::Allocator* allocator, const Column& array,
                                                       const Column& target) {
        ...
        ASSIGN_OR_RETURN(ColumnPtr result,
                         is_const_array ? _array_contains_non_nullable<true>(allocator, *array_column, target)
                                        : _array_contains_non_nullable<false>(allocator, *array_column, target));

        if (data_column->is_nullable()) {
            DCHECK_EQ(nullable_column->size(), result->size());
            if (nullable_column->has_null()) {
                result = NullableColumn::create(allocator, Column::mutate(std::move(result)),
                                                NullColumn::static_pointer_cast(
                                                        std::move(*nullable_column->null_column()).mutate()));
            }
        }

        bool return_const = is_const_array && is_const_target;
        return return_const ? ConstColumn::create(allocator, std::move(result), target.size()) : result;
    }
};
...
public:
    static StatusOr<ColumnPtr> evaluate(memory::Allocator* allocator, const Column& array, const Column& element) {
        return _array_has_generic(allocator, array, element);
    }
...
private:
    ...
    template <bool NullableElement, bool NullableTarget, bool ConstTarget, typename ElementColumn>
    static StatusOr<ColumnPtr> _process(memory::Allocator* allocator, const ElementColumn& elements,
                                        const UInt32Column& element_offsets,
        ...
        auto result = UInt8Column::create(allocator);
    ...
    template <bool NullableElement, bool NullableTarget, bool ConstTarget>
    static StatusOr<ColumnPtr> _array_has(memory::Allocator* allocator, const Column& array_elements,
                                          const UInt32Column& array_offsets,
...
#define HANDLE_HAS_TYPE(ElementType)                                                                                   \
    ...
    // array is non-nullable.
    static StatusOr<ColumnPtr> _array_has_non_nullable(memory::Allocator* allocator, const ArrayColumn& array,
                                                       const ArrayColumn& arg) {
        bool nullable_element = false;
        bool nullable_target = false;
        bool const_target = false;
        MutableColumnPtr targets_holder;
        ...
        if (nullable_element && nullable_target) {
            return _array_has<true, true, false>(allocator, *elements, array_offsets, *targets, target_offsets);
        } else if (nullable_element) {
            return const_target ? _array_has<true, false, true>(allocator, *elements, array_offsets, *targets, target_offsets)
                                : _array_has<true, false, false>(allocator, *elements, array_offsets, *targets, target_offsets);
        } else if (nullable_target) {
            return _array_has<false, true, false>(allocator, *elements, array_offsets, *targets, target_offsets);
        } else {
            return const_target ? _array_has<false, false, true>(allocator, *elements, array_offsets, *targets, target_offsets)
                                : _array_has<false, false, false>(allocator, *elements, array_offsets, *targets, target_offsets);
        }
    }
    ...
    static StatusOr<ColumnPtr> _array_has_generic(memory::Allocator* allocator, const Column& array,
                                                  const Column& target) {
        ...
        if (array_nullable == nullptr && target_nullable == nullptr) {
            return _array_has_non_nullable(allocator, *array_col, *target_col);
        }

        ASSIGN_OR_RETURN(auto result, _array_has_non_nullable(allocator, *array_col, *target_col))
        DCHECK_EQ(array_col->size(), result->size());
        return NullableColumn::create(allocator, Column::mutate(std::move(result)),
                                      NullColumn::static_pointer_cast(std::move(*merge_nullcolum(array_nullable, target_nullable))
                                                                              .mutate()));
    }
};
...
StatusOr<ColumnPtr> ArrayFunctions::array_contains_generic([[maybe_unused]] FunctionContext* context,
    ...
    return ArrayContainsImpl<false, UInt8Column>::evaluate(context->allocator(), *arg0, *arg1);
}
...
StatusOr<ColumnPtr> ArrayFunctions::array_position_generic([[maybe_unused]] FunctionContext* context,
    ...
    return ArrayContainsImpl<true, Int32Column>::evaluate(context->allocator(), *arg0, *arg1);
}
...
StatusOr<ColumnPtr> ArrayFunctions::array_contains_any([[maybe_unused]] FunctionContext* context,
    ...
    return ArrayHasImpl<true, false>::evaluate(context->allocator(), *arg0, *arg1);
}
...
StatusOr<ColumnPtr> ArrayFunctions::array_contains_all([[maybe_unused]] FunctionContext* context,
    ...
    return ArrayHasImpl<false, false>::evaluate(context->allocator(), *arg0, *arg1);
}
...
StatusOr<ColumnPtr> ArrayFunctions::array_contains_seq([[maybe_unused]] FunctionContext* context,
    ...
    return ArrayHasImpl<false, true>::evaluate(context->allocator(), *arg0, *arg1);
}
...
StatusOr<ColumnPtr> ArrayFunctions::array_slice(FunctionContext* ctx, const Columns& columns) {
    ...
    if (is_nullable) {
        if (columns[0]->is_nullable()) {
            ...
            return NullableColumn::create(ctx->allocator(), std::move(dest_column), std::move(null_result));
        }
    } else {
        return dest_column;
    }
...
static inline std::tuple<NullColumn::Ptr, const Column*, const UInt32Column*> unpack_array_column(
        memory::Allocator* allocator, const ColumnPtr& input) {
    ...
    if (array->is_nullable()) {
        ...
        array_null = NullColumn::create(allocator, input->size(), 0);
...
StatusOr<ColumnPtr> ArrayFunctions::array_distinct_any_type(FunctionContext* ctx, const Columns& columns) {
    ...
    auto [array_null, elements, offsets] = unpack_array_column(ctx->allocator(), columns[0]);
    auto* offsets_ptr = offsets->immutable_data().data();
    auto* row_nulls = array_null->get_data().data();

    auto result_elements = elements->clone_empty();
    auto result_offsets = UInt32Column::create(ctx->allocator());
    ...
    return NullableColumn::create(
            ctx->allocator(), ArrayColumn::create(ctx->allocator(), std::move(result_elements), std::move(result_offsets)),
            NullColumn::static_pointer_cast(std::move(*array_null).mutate()));
}
...
StatusOr<ColumnPtr> ArrayFunctions::array_intersect_any_type(FunctionContext* ctx, const Columns& columns) {
    ...
    NullColumn::MutablePtr nulls = NullColumn::create(ctx->allocator(), rows, 0);
    // find minimum column
    ...
    for (size_t col_idx = 0; col_idx < columns.size(); col_idx++) {
        ...
        auto [_2, cmp_elements, cmp_offsets] = unpack_array_column(ctx->allocator(), columns[col_idx]);
    ...
    return NullableColumn::create(ctx->allocator(),
                                  ArrayColumn::create(ctx->allocator(), base_elements->as_mutable_ptr(),
...
StatusOr<ColumnPtr> ArrayFunctions::repeat(FunctionContext* ctx, const Columns& columns) {
    ...
    auto dest_offsets = UInt32Column::create(ctx->allocator(), 1);
    ...
    if (dest_column_elements->is_nullable()) {
        dest_column = ArrayColumn::create(ctx->allocator(), std::move(dest_column_elements), std::move(dest_offsets));
    } else {
        auto dest_size = dest_column_elements->size();
        auto nullable_dest_column_elements = NullableColumn::create(
                ctx->allocator(), std::move(dest_column_elements),
                NullColumn::create(ctx->allocator(), dest_size, DATUM_NOT_NULL));
        dest_column = ArrayColumn::create(ctx->allocator(), std::move(nullable_dest_column_elements), std::move(dest_offsets));
    }
    ...
    if (null_result) {
        return NullableColumn::create(ctx->allocator(), Column::mutate(std::move(dest_column)),
...
StatusOr<ColumnPtr> ArrayFunctions::array_flatten(FunctionContext* ctx, const Columns& columns) {
    ...
    auto build_result_array_elements_and_offsets =
            [allocator = ctx->allocator()](const ColumnPtr& elements_column,
               size_t offsets_size) -> std::pair<MutableColumnPtr, UInt32Column::MutablePtr> {
        DCHECK(elements_column->is_array());
        auto [array_null, elements, offsets] = unpack_array_column(allocator, elements_column);
        auto result_elements = elements->clone_empty();
        auto result_offsets = UInt32Column::create(allocator);
    ...
    if (columns[0]->is_constant()) {
        ...
        return ConstColumn::create(
                ctx->allocator(),
                ArrayColumn::create(ctx->allocator(), std::move(result_elements), std::move(result_offsets)), chunk_size);
    }
    ...
    auto result = ArrayColumn::create(ctx->allocator(), std::move(result_elements), std::move(result_offsets));
    if (src_nullable_column != nullptr) {
        return NullableColumn::create(
                ctx->allocator(), std::move(result),
                NullColumn::static_pointer_cast(std::move(*src_nullable_column->null_column()).mutate()));
    }
    return result;
}
...
StatusOr<ColumnPtr> ArrayFunctions::arrays_zip(FunctionContext* ctx, const Columns& columns) {
    ...
    auto result_elements = StructColumn::create(ctx->allocator(), std::move(struct_fields), std::move(field_names));
    auto result_offsets = UInt32Column::create(ctx->allocator());
    ...
    auto result_array = ArrayColumn::create(ctx->allocator(), std::move(nullable_struct), std::move(result_offsets));
    ...
    if (result_nulls != nullptr) {
        return NullableColumn::create(ctx->allocator(), std::move(result_array), std::move(result_nulls));
    }
...
StatusOr<ColumnPtr> ArrayFunctions::null_or_empty(FunctionContext* context, const starrocks::Columns& columns) {
    ...
    if (columns[0]->only_null()) {
        return ColumnHelper::create_const_column<TYPE_BOOLEAN>(context->allocator(), 1, size);
    }
    if (columns[0]->is_constant()) {
        auto* array_column = down_cast<const ArrayColumn*>(ColumnHelper::get_data_column(columns[0].get()));
        return ColumnHelper::create_const_column<TYPE_BOOLEAN>(context->allocator(), array_column->get_element_size(0) == 0,
                                                               size);
    }
    auto* array_column = down_cast<const ArrayColumn*>(ColumnHelper::get_data_column(columns[0].get()));
