...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exprs/agg_fn_evaluator.cpp
...
Status AggFnEvaluator::create(ObjectPool* pool, const TExpr& desc, bool is_analytic_fn, AggFnEvaluator** result) {
    *result = pool->add(new AggFnEvaluator(desc.nodes[0], is_analytic_fn));
    ...
    for (int i = 0; i < desc.nodes[0].num_children; ++i) {
        ...
        RETURN_IF_ERROR(Expr::create_tree_from_thrift(pool, desc.nodes, NULL, &node_idx, &expr, &ctx));
...
AggFnEvaluator::AggFnEvaluator(const TExprNode& desc, bool is_analytic_fn)
    ...
    if (_fn.name.function_name == "count") {
        ...
        _agg_op = NDV;
    } else if (_fn.name.function_name == "count_distinct" || _fn.name.function_name == "count_distinct") {
        _agg_op = COUNT_DISTINCT;
    } else if (_fn.name.function_name == "sum_distinct" || _fn.name.function_name == "sum_distinct") {
...
Status AggFnEvaluator::prepare(RuntimeState* state, const RowDescriptor& desc, MemPool* pool,
    ...
    for (int i = 0; i < _input_exprs_ctxs.size(); ++i) {
        _staging_input_vals.push_back(create_any_val(obj_pool, input_expr_ctxs()[i]->root()->type()));
    }
    ...
    RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(_fn.id, _fn.aggregate_fn.init_fn_symbol,
                                                                    _fn.hdfs_location, _fn.checksum, &_init_fn, NULL));

    RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(
            _fn.id, _fn.aggregate_fn.update_fn_symbol, _fn.hdfs_location, _fn.checksum, &_update_fn, NULL));

    // Merge() is not loaded if evaluating the agg fn as an analytic function.
    if (!_is_analytic_fn) {
        RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(
                _fn.id, _fn.aggregate_fn.merge_fn_symbol, _fn.hdfs_location, _fn.checksum, &_merge_fn, NULL));
    }

    // Serialize and Finalize are optional
    if (!_fn.aggregate_fn.serialize_fn_symbol.empty()) {
        RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(
                _fn.id, _fn.aggregate_fn.serialize_fn_symbol, _fn.hdfs_location, _fn.checksum, &_serialize_fn, NULL));
    }
    if (!_fn.aggregate_fn.finalize_fn_symbol.empty()) {
        RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(
                _fn.id, _fn.aggregate_fn.finalize_fn_symbol, _fn.hdfs_location, _fn.checksum, &_finalize_fn, NULL));
    }

    if (!_fn.aggregate_fn.get_value_fn_symbol.empty()) {
        RETURN_IF_ERROR(UserFunctionCache::instance()->get_function_ptr(
                _fn.id, _fn.aggregate_fn.get_value_fn_symbol, _fn.hdfs_location, _fn.checksum, &_get_value_fn, NULL));
    }
    ...
    for (int j = 0; j < _input_exprs_ctxs.size(); ++j) {
        arg_types.push_back(AnyValUtil::column_type_to_type_desc(_input_exprs_ctxs[j]->root()->type()));
    }

    FunctionContext::TypeDesc intermediate_type = AnyValUtil::column_type_to_type_desc(_intermediate_type);
    FunctionContext::TypeDesc output_type = AnyValUtil::column_type_to_type_desc(_output_slot_desc->type());

    *agg_fn_ctx = FunctionContextImpl::create_context(state, pool, intermediate_type, output_type, arg_types, 0, false);
    return Status::OK();
}
...
inline void AggFnEvaluator::set_output_slot(const AnyVal* src, const SlotDescriptor* dst_slot_desc, Tuple* dst) {
    if (src->is_null) {
...
bool AggFnEvaluator::count_distinct_data_filter(TupleRow* row, Tuple* dst) {
    ...
    for (int i = 0; i < input_expr_ctxs().size(); ++i) {
        ...
        case TYPE_TINYINT: {
            memcpy(begin, &reinterpret_cast<TinyIntVal*>(_staging_input_vals[i])->val, TINYINT_SIZE);
            begin += TINYINT_SIZE;
            break;
        }

        case TYPE_SMALLINT: {
            memcpy(begin, &reinterpret_cast<SmallIntVal*>(_staging_input_vals[i])->val, SMALLINT_SIZE);
            begin += SMALLINT_SIZE;
            break;
        }
    ...
    DCHECK(begin == string_val.ptr + string_val.len) << "COUNT_DISTINCT: StringVal's len dosn't match";
...
void AggFnEvaluator::update_or_merge(FunctionContext* agg_fn_ctx, TupleRow* row, Tuple* dst, void* fn) {
    if (fn == NULL) {
        return;
    }
    ...
    if (_is_multi_distinct && _agg_op == AggregationOp::COUNT_DISTINCT) {
        reinterpret_cast<UpdateFn1>(fn)(agg_fn_ctx, *_staging_input_vals[0], _staging_intermediate_val);
    } else {
        ...
        case 2:
            reinterpret_cast<UpdateFn2>(fn)(agg_fn_ctx, *_staging_input_vals[0], *_staging_input_vals[1],
                                            _staging_intermediate_val);
            break;

        case 3:
            reinterpret_cast<UpdateFn3>(fn)(agg_fn_ctx, *_staging_input_vals[0], *_staging_input_vals[1],
                                            *_staging_input_vals[2], _staging_intermediate_val);
            break;

        case 4:
            reinterpret_cast<UpdateFn4>(fn)(agg_fn_ctx, *_staging_input_vals[0], *_staging_input_vals[1],
                                            *_staging_input_vals[2], *_staging_input_vals[3],
                                            _staging_intermediate_val);
            break;

        case 5:
            reinterpret_cast<UpdateFn5>(fn)(agg_fn_ctx, *_staging_input_vals[0], *_staging_input_vals[1],
                                            *_staging_input_vals[2], *_staging_input_vals[3], *_staging_input_vals[4],
                                            _staging_intermediate_val);
            break;

        case 6:
            reinterpret_cast<UpdateFn6>(fn)(agg_fn_ctx, *_staging_input_vals[0], *_staging_input_vals[1],
                                            *_staging_input_vals[2], *_staging_input_vals[3], *_staging_input_vals[4],
                                            *_staging_input_vals[5], _staging_intermediate_val);
            break;

        case 7:
            reinterpret_cast<UpdateFn7>(fn)(agg_fn_ctx, *_staging_input_vals[0], *_staging_input_vals[1],
                                            *_staging_input_vals[2], *_staging_input_vals[3], *_staging_input_vals[4],
        ...
        case 8:
            reinterpret_cast<UpdateFn8>(fn)(agg_fn_ctx, *_staging_input_vals[0], *_staging_input_vals[1],
                                            *_staging_input_vals[2], *_staging_input_vals[3], *_staging_input_vals[4],
                                            *_staging_input_vals[5], *_staging_input_vals[6], *_staging_input_vals[7],
                                            _staging_intermediate_val);
            break;
...
void AggFnEvaluator::update(FunctionContext* agg_fn_ctx, TupleRow* row, Tuple* dst, void* fn, MemPool* pool) {
...
void AggFnEvaluator::merge(FunctionContext* agg_fn_ctx, Tuple* src, Tuple* dst) {
    ...
    reinterpret_cast<UpdateFn1>(_merge_fn)(agg_fn_ctx, *_staging_merge_input_val, _staging_intermediate_val);

    set_output_slot(_staging_intermediate_val, _intermediate_slot_desc, dst);
}

void AggFnEvaluator::choose_update_or_merge(FunctionContext* agg_fn_ctx, TupleRow* row, Tuple* dst) {
    if (_is_merge) {
        return update_or_merge(agg_fn_ctx, row, dst, _merge_fn);
    } else {
...
void AggFnEvaluator::serialize_or_finalize(FunctionContext* agg_fn_ctx, Tuple* src, const SlotDescriptor* dst_slot_desc,
                                           Tuple* dst, void* fn) {
