...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exprs/scalar_fn_call.h
...
#ifndef STARROCKS_BE_SRC_QUERY_EXPRS_SCALAR_FN_CALL_H
#define STARROCKS_BE_SRC_QUERY_EXPRS_SCALAR_FN_CALL_H
...
public:
    ...
    virtual Expr* clone(ObjectPool* pool) const override { return pool->add(new ScalarFnCall(*this)); }
...
private:
    ...
    int num_fixed_args() const { return _vararg_start_idx >= 0 ? _vararg_start_idx : _children.size(); }
    ...
    /// interpreted path.
    void evaluate_children(ExprContext* context, TupleRow* row, std::vector<starrocks_udf::AnyVal*>* input_vals);
