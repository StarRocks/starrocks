...
static ColumnPtr cast_to_json_fn(ColumnPtr& column) {
    if constexpr (pt_is_map<FromType> || pt_is_struct<FromType>) {
...
public:
    ...
    ColumnPtr evaluate(ExprContext* context, Chunk* ptr) {
        ColumnPtr column = _children[0]->evaluate(context, ptr);
        if (ColumnHelper::count_nulls(column) == column->size() && column->size() != 0) {
            return ColumnHelper::create_const_null_column(column->size());
        }
...
Expr* VectorizedCastExprFactory::create_primitive_cast(ObjectPool* pool, const TExprNode& node, PrimitiveType from_type,
                                                       PrimitiveType to_type, bool allow_throw_exception) {
...
StatusOr<std::unique_ptr<Expr>> VectorizedCastExprFactory::create_cast_expr(ObjectPool* pool, const TExprNode& node,
    ...
    if (from_type.is_array_type() && to_type.is_array_type()) {
        ...
        auto child_cast_ptr = child_cast.get();
        pool->add(child_input.release());
        pool->add(child_cast.release());
        return std::make_unique<VectorizedCastArrayExpr>(child_cast_ptr, node);
    }
...
Expr* VectorizedCastExprFactory::from_thrift(ObjectPool* pool, const TExprNode& node, bool allow_throw_exception) {
    ...
    auto ret = create_cast_expr(pool, from_type, to_type, allow_throw_exception);
    if (!ret.ok()) {
