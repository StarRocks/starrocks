...
void ArrayFunctions::_array_distinct_item(const ArrayColumn& column, size_t index, HashSet* hash_set,
...
template <PrimitiveType PT, typename HashSet>
ColumnPtr ArrayFunctions::array_distinct(const Columns& columns) {
        ...
        if (src_nullable_column->has_null()) {
            for (size_t i = 0; i < chunk_size; i++) {
                if (!src_nullable_column->is_null(i)) {
                    _array_distinct_item<PT, HashSet>(*src_data_column, i, &hash_set, &dest_data_column);
                    hash_set.clear();
                } else {
...
ColumnPtr ArrayFunctions::array_distinct_boolean(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_BOOLEAN, phmap::flat_hash_set<int8_t, StdHash<int8_t>>>(
            columns);
}

ColumnPtr ArrayFunctions::array_distinct_tinyint(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_TINYINT, phmap::flat_hash_set<int8_t, StdHash<int8_t>>>(
            columns);
}

ColumnPtr ArrayFunctions::array_distinct_smallint(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_SMALLINT, phmap::flat_hash_set<int16_t, StdHash<int16_t>>>(
            columns);
}

ColumnPtr ArrayFunctions::array_distinct_int(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_INT, phmap::flat_hash_set<int32_t, StdHash<int32_t>>>(columns);
}

ColumnPtr ArrayFunctions::array_distinct_bigint(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_BIGINT, phmap::flat_hash_set<int64_t, StdHash<int32_t>>>(
            columns);
}

ColumnPtr ArrayFunctions::array_distinct_largeint(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_LARGEINT,
                                                   phmap::flat_hash_set<int128_t, Hash128WithSeed<PhmapSeed1>>>(
            columns);
}

ColumnPtr ArrayFunctions::array_distinct_float(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_FLOAT, phmap::flat_hash_set<float, StdHash<float>>>(columns);
}

ColumnPtr ArrayFunctions::array_distinct_double(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_DOUBLE, phmap::flat_hash_set<double, StdHash<double>>>(columns);
}

ColumnPtr ArrayFunctions::array_distinct_varchar(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_VARCHAR, phmap::flat_hash_set<Slice, SliceHash>>(columns);
}

ColumnPtr ArrayFunctions::array_distinct_char(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_CHAR, phmap::flat_hash_set<Slice, SliceHash>>(columns);
}

ColumnPtr ArrayFunctions::array_distinct_decimalv2(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_DECIMALV2, phmap::flat_hash_set<DecimalV2Value>>(columns);
}

ColumnPtr ArrayFunctions::array_distinct_datetime(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_DATETIME,
                                                   phmap::flat_hash_set<TimestampValue, StdHash<TimestampValue>>>(
            columns);
}

ColumnPtr ArrayFunctions::array_distinct_date(FunctionContext* context, const Columns& columns) {
    return ArrayFunctions::template array_distinct<TYPE_DATE, phmap::flat_hash_set<DateValue, StdHash<DateValue>>>(
            columns);
}
