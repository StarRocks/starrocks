...
NullColumn::MutablePtr FunctionHelper::union_nullable_column(const ColumnPtr& v1, const ColumnPtr& v2) {
    ...
    if (v1->is_nullable() && v2->is_nullable()) {
        ...
        result = std::move(*(ColumnHelper::as_raw_column<NullableColumn>(v1)->null_column())).mutate();
    } else if (v2->is_nullable()) {
        result = std::move(*(ColumnHelper::as_raw_column<NullableColumn>(v2)->null_column())).mutate();
    } else {
        return nullptr;
    }
...
ColumnPtr FunctionHelper::merge_column_and_null_column(ColumnPtr&& column, NullColumnPtr&& null_column) {
    if (column->only_null()) {
        ...
        return NullableColumn::create(std::move(*(nullable_column->data_column())).mutate(),
