...
constexpr std::string_view kEmbeddingConfigId = "__system_embedding__";
...
bool contains_only_java_whitespace(const Slice& value) {
    // Match FE String.isBlank(), which excludes nonbreaking spaces from Unicode whitespace.
    size_t index = 0;
    while (index < value.size) {
        UChar32 code_point;
        U8_NEXT(value.data, index, value.size, code_point);
        if (code_point < 0 || !u_isWhitespace(code_point)) {
            return false;
        }
    }
    return true;
...
StatusOr<std::string> read_constant_string_array(const Column& column) {
    ...
    for (size_t index = offset; index < offset + count; ++index) {
        ...
        if (contains_only_java_whitespace(value)) {
            return invalid_ai_expression();
        }
        if (index != offset) {
            result += ", ";
...
StatusOr<AIFunctionCallExpr*> AIFunctionCallExpr::create(ObjectPool* pool, const TExprNode& node) {
    ...
    if (spec->model_source == TAIModelSource::PROVIDER) {
...
StatusOr<AIFunctionInputBatch> AIFunctionCallExpr::build_input_batch(ExprContext* context, Chunk* chunk,
    ...
    if (spec.provider_argument >= 0) {
        const size_t index = spec.provider_argument;
    ...
    for (size_t row = 0; row < chunk->num_rows(); ++row) {
        ...
        for (size_t index = 0; index < strings.size(); ++index) {
            ...
            if (strings[index]->is_null(row)) {
                ...
                is_null |= spec.blank_as_null_arguments[index] &&
