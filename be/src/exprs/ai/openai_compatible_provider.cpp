...
#include "exprs/ai/provider_utils.h"
...
StatusOr<AIProviderHttpRequest> OpenAICompatibleProvider::build_request(const AIChatRequest& request) const {
    ...
    if (request.options != nullptr) {
        for (const auto& option : request.options->_serialized_members()) {
            if (!writer.Key(option.key.data(), option.key.size()) ||
                !writer.RawValue(option.value.data(), option.value.size(), option.value_type)) {
                return invalid_request();
            }
        }
    }
