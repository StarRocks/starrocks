...
StatusOr<std::pair<std::string, rapidjson::Type>> serialize_value(const Column& column, const TypeDescriptor& type,
                                                                  size_t row) {
    JsonBuffer buffer;
    ...
    return std::make_pair(std::string(buffer.GetString(), buffer.GetSize()), value_type);
}

} // namespace

const PreparedAIProviderOptions::SerializedMembers& PreparedAIProviderOptions::_serialized_members() const {
    static const SerializedMembers kEmpty;
...
StatusOr<PreparedAIProviderOptions> prepare_ai_provider_options(const Column& column, const TypeDescriptor& type,
    ...
    PreparedAIProviderOptions::SerializedMembers members;
    if (unwrapped->is_null) {
        return PreparedAIProviderOptions(
                std::make_shared<const PreparedAIProviderOptions::SerializedMembers>(std::move(members)));
    }
    ...
    for (size_t index = offset; index < offset + count; ++index) {
        ...
        members.emplace_back(PreparedAIProviderOptions::SerializedMember{
                .key = std::move(*key), .value = std::move(value->first), .value_type = value->second});
    }
