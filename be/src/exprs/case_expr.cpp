...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exprs/case_expr.cpp
...
Status CaseExpr::open(RuntimeState* state, ExprContext* ctx, FunctionContext::FunctionStateScope scope) {
    RETURN_IF_ERROR(Expr::open(state, ctx, scope));
    FunctionContext* fn_ctx = ctx->fn_context(_fn_context_index);
    CaseExprState* case_state = reinterpret_cast<CaseExprState*>(fn_ctx->allocate(sizeof(CaseExprState)));
...
void CaseExpr::close(RuntimeState* state, ExprContext* ctx, FunctionContext::FunctionStateScope scope) {
...
bool CaseExpr::any_val_eq(const TypeDescriptor& type, const AnyVal* v1, const AnyVal* v2) {
    ...
    case TYPE_INT:
        return AnyValUtil::equals(type, *reinterpret_cast<const IntVal*>(v1), *reinterpret_cast<const IntVal*>(v2));
    case TYPE_BIGINT:
        return AnyValUtil::equals(type, *reinterpret_cast<const BigIntVal*>(v1),
                                  *reinterpret_cast<const BigIntVal*>(v2));
    case TYPE_FLOAT:
        return AnyValUtil::equals(type, *reinterpret_cast<const FloatVal*>(v1), *reinterpret_cast<const FloatVal*>(v2));
    case TYPE_DOUBLE:
...
#define CASE_COMPUTE_FN(THEN_TYPE, TYPE_NAME)                                                                \
    THEN_TYPE CaseExpr::get_##TYPE_NAME(ExprContext* ctx, TupleRow* row) {                                   \
        FunctionContext* fn_ctx = ctx->fn_context(_fn_context_index);                                        \
        CaseExprState* state =                                                                               \
                reinterpret_cast<CaseExprState*>(fn_ctx->get_function_state(FunctionContext::THREAD_LOCAL)); \
