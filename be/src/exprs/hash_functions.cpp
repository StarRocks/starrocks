...
StatusOr<ColumnPtr> HashFunctions::xx_hash3_32(FunctionContext* context, const starrocks::Columns& columns) {
    std::vector<ColumnViewer<TYPE_VARCHAR>> column_viewers;
    ...
    const uint32_t default_xxhash_seed = HashUtil::XXHASH3_32_SEED;
    ...
    for (const auto& viewer : column_viewers) {
        for (size_t row = 0; row < row_size; ++row) {
            ...
            auto slice = viewer.value(row);
            uint32_t seed = seeds_vec[row];
            seeds_vec[row] = HashUtil::xx_hash3_32(slice.data, slice.size, seed);
        }
    }
