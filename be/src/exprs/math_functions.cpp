...
template <LogicalType TYPE, bool isNorm>
StatusOr<ColumnPtr> MathFunctions::cosine_similarity2(FunctionContext* context, const Columns& columns) {
    DCHECK_EQ(columns.size(), 2);
    ...
    if (!base->is_constant() && !target->is_constant()) {
        return Status::InvalidArgument(
                fmt::format("cosine_similarity need a constant array as one of the parameter!"));
    }

    using CppType = RunTimeCppType<TYPE>;
    using ColumnType = RunTimeColumnType<TYPE>;

    std::vector<CppType> base_elements;
    ...
    std::string token;
    while ((pos_end = pattern_str.find(delimiter, pos_start)) != std::string::npos) {
        token = pattern_str.substr(pos_start, pos_end - pos_start);
        CppType num = ::atof(token.c_str());
        base_elements.push_back(num);
        pos_start = pos_end + delim_len;
    }
    float last = ::atof(pattern_str.substr(pos_start).c_str());
    base_elements.push_back(last);
    size_t base_flat_size = base_elements.size();

    const Column* target_flat = down_cast<const ArrayColumn*>(target)->elements_column().get();
    const uint32_t* target_offset = down_cast<const ArrayColumn*>(target)->offsets().get_data().data();

    if (target_flat->has_null()) {
        return Status::InvalidArgument("cosine_similarity does not support null values");
    }
    if (target_flat->is_nullable()) {
        target_flat = down_cast<const NullableColumn*>(target_flat)->data_column().get();
    ...
    for (size_t i = 0; i < target_size; i++) {
        size_t t_dim_size = target_offset[i + 1] - target_offset[i];
        if (t_dim_size != base_flat_size) {
            return Status::InvalidArgument(
                    fmt::format("cosine_similarity requires equal length arrays in each row. base array dimension size "
                                "is {}, target array dimension size is {}.",
                                base_flat_size, t_dim_size));
        }
        if (t_dim_size == 0) {
            return Status::InvalidArgument("cosine_similarity requires non-empty arrays in each row");
        }
    ...
    const CppType* target_data = target_data_head;
    const CppType* base_data = base_elements.data();
    for (size_t i = 0; i < target_size; i++) {
        CppType sum = 0;
        CppType base_sum = 0;
        CppType target_sum = 0;
...
template StatusOr<ColumnPtr> MathFunctions::cosine_similarity<TYPE_FLOAT, false>(FunctionContext* context,
                                                                                 const Columns& columns);
template StatusOr<ColumnPtr> MathFunctions::cosine_similarity2<TYPE_FLOAT, true>(FunctionContext* context,
                                                                                const Columns& columns);
template StatusOr<ColumnPtr> MathFunctions::cosine_similarity2<TYPE_FLOAT, false>(FunctionContext* context,
                                                                                 const Columns& columns);
...
template <LogicalType TYPE>
StatusOr<ColumnPtr> MathFunctions::l2_distance2(FunctionContext* context, const Columns& columns) {
    DCHECK_EQ(columns.size(), 2);
    ...
    if (base->has_null() || target->has_null()) {
        return Status::InvalidArgument(
                fmt::format("l2_distance2 does not support null values. {} array has null value.",
                            base->has_null() ? "base" : "target"));
    }
    if (base->is_nullable()) {
        base = down_cast<const NullableColumn*>(base)->data_column().get();
    ...
    if (!base->is_constant() && !target->is_constant()) {
        return Status::InvalidArgument(
                fmt::format("l2_distance2 need a constant array as one of the parameter!"));
    }
    ...
    if (target_flat->has_null()) {
        return Status::InvalidArgument("l2_distance2 does not support null values");
    }
    if (target_flat->is_nullable()) {
        target_flat = down_cast<const NullableColumn*>(target_flat)->data_column().get();
    }
    ...
    for (size_t i = 0; i < target_size; i++) {
        ...
        if (t_dim_size != base_flat_size) {
            return Status::InvalidArgument(
                    fmt::format("l2_distance2 requires equal length arrays in each row. base array dimension size "
                                "is {}, target array dimension size is {}.",
                                base_flat_size, t_dim_size));
        }
        if (t_dim_size == 0) {
            return Status::InvalidArgument("l2_distance2 requires non-empty arrays in each row");
        }
    }
    ...
    for (size_t i = 0; i < target_size; i++) {
        CppType sum = 0;
        CppType base_magnitude = 0;
        CppType target_magnitude = 0;
        size_t dim_size = target_offset[i + 1] - target_offset[i];
        for (size_t j = 0; j < dim_size; j++) {
            base_magnitude += base_data[j] * base_data[j];
            target_magnitude += target_data[j] * target_data[j];
        }
        target_magnitude = std::sqrt(target_magnitude);
        base_magnitude = std::sqrt(base_magnitude);
        for (size_t j = 0; j < dim_size; j++) {
            CppType distance = (base_data[j] - target_data[j]) * (base_data[j] - target_data[j]);
            sum += distance;
        }
        result_data[i] = sum;
        target_data += dim_size;
...
template StatusOr<ColumnPtr> MathFunctions::l2_distance2<TYPE_FLOAT>(FunctionContext* context, const Columns& columns);
