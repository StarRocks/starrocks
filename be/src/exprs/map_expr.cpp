...
StatusOr<ColumnPtr> MapExpr::evaluate_checked(ExprContext* context, Chunk* chunk) {
    ...
    auto key_col = ColumnHelper::create_column(context->allocator(), _type.children[0], true);
    auto value_col = ColumnHelper::create_column(context->allocator(), _type.children[1], true);
    auto offsets = UInt32Column::create(context->allocator());
    ...
    auto res = MapColumn::create(context->allocator(), std::move(key_col), std::move(value_col), std::move(offsets));
