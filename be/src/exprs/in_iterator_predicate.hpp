...
public:
    VectorizedInIteratorPredicate(const TExprNode& node) : Predicate(node), _is_not_in(node.in_predicate.is_not_in) {}
    ...
    Expr* clone(ObjectPool* pool) const override { return pool->add(new VectorizedInIteratorPredicate(*this)); }
