...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exprs/agg_fn_evaluator.h
...
#ifndef STARROCKS_BE_SRC_QUERY_EXPRS_AGG_FN_EVALUATOR_H
#define STARROCKS_BE_SRC_QUERY_EXPRS_AGG_FN_EVALUATOR_H
...
public:
    ...
    Status prepare(RuntimeState* state, const RowDescriptor& desc, MemPool* pool,
                   ...
                   MemTracker* mem_tracker, FunctionContext** agg_fn_ctx);
    ...
    bool is_count_star() const { return _agg_op == AggregationOp::COUNT && _input_exprs_ctxs.empty(); }
    ...
    void add(starrocks_udf::FunctionContext* agg_fn_ctx, TupleRow* src, Tuple* dst);
    ...
    void remove(starrocks_udf::FunctionContext* agg_fn_ctx, TupleRow* src, Tuple* dst);
    ...
    // builtins.
    void get_value(starrocks_udf::FunctionContext* agg_fn_ctx, Tuple* src, Tuple* dst);
    ...
    static void add(const std::vector<AggFnEvaluator*>& evaluators,
                    const std::vector<starrocks_udf::FunctionContext*>& fn_ctxs, TupleRow* src, Tuple* dst);
    ...
    static void get_value(const std::vector<AggFnEvaluator*>& evaluators,
                          const std::vector<starrocks_udf::FunctionContext*>& fn_ctxs, Tuple* src, Tuple* dst);
    ...
    static void init(const std::vector<AggFnEvaluator*>& evaluators,
                     const std::vector<starrocks_udf::FunctionContext*>& fn_ctxs, Tuple* dst);
...
private:
    ...
    std::unique_ptr<HybirdMap> _hybird_map;
    ...
    // per thread.
    // std::unique_ptr<starrocks_udf::FunctionContext> _ctx;
    ...
    starrocks_udf::AnyVal* _staging_intermediate_val;
    starrocks_udf::AnyVal* _staging_merge_input_val;
    // starrocks_udf::AnyVal* _staging_output_val;
    ...
    std::string to_string(TAggregationOp::type index) {
        std::map<int, const char*>::const_iterator it = _TAggregationOp_VALUES_TO_NAMES.find(_agg_op);
    ...
    void serialize_or_finalize(FunctionContext* agg_fn_ctx, Tuple* src, const SlotDescriptor* dst_slot_desc, Tuple* dst,
                               void* fn);

    // Writes the result in src into dst pointed to by _output_slot_desc
    void set_output_slot(const starrocks_udf::AnyVal* src, const SlotDescriptor* dst_slot_desc, Tuple* dst);
    // Sets 'dst' to the value from 'slot'.
    void set_any_val(const void* slot, const TypeDescriptor& type, starrocks_udf::AnyVal* dst);
};

inline void AggFnEvaluator::add(starrocks_udf::FunctionContext* agg_fn_ctx, TupleRow* row, Tuple* dst) {
    agg_fn_ctx->impl()->increment_num_updates();
    update(agg_fn_ctx, row, dst, _is_merge ? _merge_fn : _update_fn, NULL);
}
inline void AggFnEvaluator::remove(starrocks_udf::FunctionContext* agg_fn_ctx, TupleRow* row, Tuple* dst) {
    agg_fn_ctx->impl()->increment_num_removes();
    update(agg_fn_ctx, row, dst, _remove_fn, NULL);
}

inline void AggFnEvaluator::finalize(starrocks_udf::FunctionContext* agg_fn_ctx, Tuple* src, Tuple* dst) {
    serialize_or_finalize(agg_fn_ctx, src, _output_slot_desc, dst, _finalize_fn);
}
inline void AggFnEvaluator::get_value(starrocks_udf::FunctionContext* agg_fn_ctx, Tuple* src, Tuple* dst) {
...
inline void AggFnEvaluator::add(const std::vector<AggFnEvaluator*>& evaluators,
                                const std::vector<starrocks_udf::FunctionContext*>& fn_ctxs, TupleRow* src,
                                Tuple* dst) {
    DCHECK_EQ(evaluators.size(), fn_ctxs.size());
...
inline void AggFnEvaluator::get_value(const std::vector<AggFnEvaluator*>& evaluators,
                                      const std::vector<starrocks_udf::FunctionContext*>& fn_ctxs, Tuple* src,
                                      Tuple* dst) {
    DCHECK_EQ(evaluators.size(), fn_ctxs.size());
