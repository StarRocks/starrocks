...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exprs/string_functions.cpp
...
StringVal StringFunctions::substring(FunctionContext* context, const StringVal& str, const IntVal& pos,
                                     const IntVal& len) {
    if (str.is_null || pos.is_null || len.is_null) {
        return StringVal::null();
    }
    if (pos.val == 0 || len.val <= 0 || str.len == 0 || pos.val > str.len) {
        return StringVal();
    }
...
// This behaves identically to the mysql implementation.
StringVal StringFunctions::right(FunctionContext* context, const StringVal& str, const IntVal& len) {
...
BooleanVal StringFunctions::starts_with(FunctionContext* context, const StringVal& str, const StringVal& prefix) {
...
BooleanVal StringFunctions::ends_with(FunctionContext* context, const StringVal& str, const StringVal& suffix) {
...
StringVal StringFunctions::repeat(FunctionContext* context, const StringVal& str, const IntVal& n) {
    if (str.is_null || n.is_null || ((n.val * str.len) > OLAP_STRING_MAX_LENGTH)) {
        return StringVal::null();
    }
...
StringVal StringFunctions::append_trailing_char_if_absent(starrocks_udf::FunctionContext* context,
...
IntVal StringFunctions::instr(FunctionContext* context, const StringVal& str, const StringVal& substr) {
...
IntVal StringFunctions::locate(FunctionContext* context, const StringVal& substr, const StringVal& str) {
    return instr(context, str, substr);
}

IntVal StringFunctions::locate_pos(FunctionContext* context, const StringVal& substr, const StringVal& str,
                                   const IntVal& start_pos) {
...
static re2::RE2* compile_regex(const StringVal& pattern, std::string* error_str, const StringVal& match_parameter) {
    ...
    if (!match_parameter.is_null && !StringFunctions::set_re2_options(match_parameter, error_str, &options)) {
        return NULL;
    }
...
void StringFunctions::regexp_prepare(FunctionContext* context, FunctionContext::FunctionStateScope scope) {
...
void StringFunctions::regexp_close(FunctionContext* context, FunctionContext::FunctionStateScope scope) {
    if (scope != FunctionContext::FRAGMENT_LOCAL) {
        return;
    }
...
StringVal StringFunctions::regexp_extract(FunctionContext* context, const StringVal& str, const StringVal& pattern,
                                          const BigIntVal& index) {
    ...
    re2::RE2* re = reinterpret_cast<re2::RE2*>(context->get_function_state(FunctionContext::FRAGMENT_LOCAL));
...
StringVal StringFunctions::regexp_replace(FunctionContext* context, const StringVal& str, const StringVal& pattern,
    ...
    re2::StringPiece replace_str = re2::StringPiece(reinterpret_cast<char*>(replace.ptr), replace.len);
...
StringVal StringFunctions::concat(FunctionContext* context, int num_children, const StringVal* strs) {
...
StringVal StringFunctions::concat_ws(FunctionContext* context, const StringVal& sep, int num_children,
...
IntVal StringFunctions::find_in_set(FunctionContext* context, const StringVal& str, const StringVal& str_set) {
...
void StringFunctions::parse_url_prepare(FunctionContext* ctx, FunctionContext::FunctionStateScope scope) {
    if (scope != FunctionContext::FRAGMENT_LOCAL) {
        return;
    }
...
StringVal StringFunctions::parse_url(FunctionContext* ctx, const StringVal& url, const StringVal& part) {
...
void StringFunctions::parse_url_close(FunctionContext* ctx, FunctionContext::FunctionStateScope scope) {
    if (scope != FunctionContext::FRAGMENT_LOCAL) {
        return;
    }
    UrlParser::UrlPart* url_part = reinterpret_cast<UrlParser::UrlPart*>(ctx->get_function_state(scope));
    delete url_part;
}

StringVal StringFunctions::parse_url_key(FunctionContext* ctx, const StringVal& url, const StringVal& part,
                                         const StringVal& key) {
    ...
    StringValue result;
    if (!UrlParser::parse_url_key(StringValue::from_string_val(url), url_part, StringValue::from_string_val(key),
                                  &result)) {
        // url is malformed, or url_part is invalid.
        if (url_part == UrlParser::INVALID) {
            std::stringstream ss;
...
static int index_of(const uint8_t* source, int source_offset, int source_count, const uint8_t* target,
                    int target_offset, int target_count, int from_index) {
...
StringVal StringFunctions::split_part(FunctionContext* context, const StringVal& content, const StringVal& delimiter,
                                      const IntVal& field) {
