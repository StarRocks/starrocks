...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exprs/new_agg_fn_evaluator.h
...
public:
    ...
    static Status Create(const AggFn& agg_fn, RuntimeState* state, ObjectPool* pool, MemPool* mem_pool,
                         NewAggFnEvaluator** eval, MemTracker* tracker,
    ...
    static Status Create(const std::vector<AggFn*>& agg_fns, RuntimeState* state, ObjectPool* pool, MemPool* mem_pool,
                         std::vector<NewAggFnEvaluator*>* evals, MemTracker* tracker,
                         const RowDescriptor& row_desc) WARN_UNUSED_RESULT;
    ...
    static Status Open(const std::vector<NewAggFnEvaluator*>& evals, RuntimeState* state) WARN_UNUSED_RESULT;
    ...
    static void ShallowClone(ObjectPool* pool, MemPool* mem_pool, const std::vector<NewAggFnEvaluator*>& evals,
    ...
    static void Remove(const std::vector<NewAggFnEvaluator*>& evals, const TupleRow* src, Tuple* dst);
...
private:
    ...
    std::vector<starrocks_udf::AnyVal*> staging_input_vals_;

    /// Staging intermediate and merged values used in the interpreted
    /// Update() / Merge() paths.
    starrocks_udf::AnyVal* staging_intermediate_val_ = nullptr;
    starrocks_udf::AnyVal* staging_merge_input_val_ = nullptr;
    ...
    inline void SetDstSlot(const starrocks_udf::AnyVal* src, const SlotDescriptor& dst_slot_desc, Tuple* dst);
...
inline void NewAggFnEvaluator::Add(const std::vector<NewAggFnEvaluator*>& evals, const TupleRow* src, Tuple* dst) {
    for (int i = 0; i < evals.size(); ++i) evals[i]->Add(src, dst);
}

inline void NewAggFnEvaluator::Remove(const std::vector<NewAggFnEvaluator*>& evals, const TupleRow* src, Tuple* dst) {
...
inline void NewAggFnEvaluator::GetValue(const std::vector<NewAggFnEvaluator*>& evals, Tuple* src, Tuple* dst) {
    for (int i = 0; i < evals.size(); ++i) evals[i]->GetValue(src, dst);
}

inline void NewAggFnEvaluator::Finalize(const std::vector<NewAggFnEvaluator*>& evals, Tuple* agg_val,
                                        Tuple* output_val) {
