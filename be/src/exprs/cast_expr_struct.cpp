    ...
    for (size_t i = 0; i < src.size(); i++) {
        ...
        if (json_value && json_value->get_type() == JsonType::JSON_ARRAY) {
            ...
            for (int j = 0; j < _type.field_names.size(); j++) {
                ...
                ASSIGN_OR_RETURN(auto json_path, JsonPath::parse(Slice(path_string)));
                vpack::Builder builder;
                vpack::Slice json_slice = JsonPath::extract(json_value, json_path, &builder);
                if (json_slice.isNone()) {
                    json_columns[j].append_null();
                    continue;
                }
                JsonValue element_value(json_slice);
                json_columns[j].append(std::move(element_value));
            }
            null_column->append(0);
        } else {
            APPEND_NULL(json_columns, null_column);
