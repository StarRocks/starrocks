...
StatusOr<ColumnPtr> CastJsonToStruct::evaluate_checked(ExprContext* context, Chunk* input_chunk) {
    ...
    for (size_t i = 0; i < field_size; i++) {
        ...
        if (_field_casts[i] != nullptr) {
            ...
            casted_fields.emplace_back(std::move(casted_field)->assume_mutable());
        } else {
            casted_fields.emplace_back(NullableColumn::wrap_if_necessary(elements->clone())->assume_mutable());
        }
