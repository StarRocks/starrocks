...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exprs/cast_functions.cpp
...
#define CAST_FROM_STRING(num_type, type_name, native_type, string_parser_fn)                                       \
    num_type CastFunctions::cast_to_##type_name(FunctionContext* ctx, const StringVal& val) {                      \
        ...
        num_type ret;                                                                                              \
        ret.val = StringParser::string_parser_fn<native_type>(reinterpret_cast<char*>(val.ptr), val.len, &result); \
...
int float_to_string<float>(float value, char* buf) {
    int len = f2s_buffered_n(value, buf);
    return len;
}

template <>
int float_to_string<double>(double value, char* buf) {
    int len = d2s_buffered_n(value, buf);
    return len;
}
