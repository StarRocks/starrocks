...
public:
    ...
    virtual void shallow_append(const Column& src, size_t offset, size_t count) { append(src, offset, count); }

    virtual void append(const Column& src) { append(src, 0, src.size()); }

    virtual void shallow_append(const Column& src) { shallow_append(src, 0, src.size()); }
    ...
    virtual void shallow_append_selective(const Column& src, const uint32_t* indexes, uint32_t from, uint32_t size) {
    ...
    void shallow_append_selective(const Column& src, const Buffer<uint32_t>& indexes) {
        return shallow_append_selective(src, indexes.data(), 0, indexes.size());
    }
