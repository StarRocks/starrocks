...
void Chunk::append_selective_shallow_copy(const Chunk& src, const uint32_t* indexes, uint32_t from, uint32_t size) {
    DCHECK_EQ(_columns.size(), src.columns().size());
    for (size_t i = 0; i < _columns.size(); ++i) {
        _columns[i]->append_selective_shallow_copy(*src.columns()[i].get(), indexes, from, size);
    }
}
