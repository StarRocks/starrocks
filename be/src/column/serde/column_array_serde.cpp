...
class BinaryColumnSerde {
    ...
    //    offset entry, so its legacy offset_bytes_size is at least 4. The
    //    format_version field after the escape header lets future extended
    //    BinaryColumn layouts evolve without consuming another escape marker.
    //
    //      u32 bytes_size (0)
    ...
    //
    // By default, once a BinaryColumn payload overflows the legacy u32 layout,
    // new BEs serialize it with the extended escape format. That format is
    // structurally disjoint from every valid legacy payload, so new BEs can
    // decode it unambiguously, but old BEs cannot read it.
    //
    // For rolling downgrade or mixed-version runtime exchange where old BEs may
    // still receive serde payloads, enable
    // enable_binary_column_serde_overflow_check to reject overflow BinaryColumn
    // payloads instead of emitting the extended format. Delete-file persistence
    // has a separate legacy guard in PrimaryKeyEncoder.
public:
    template <typename T>
    static int64_t max_serialized_size(const BinaryColumnBase<T>& column, const int encode_level) {
        if constexpr (std::is_same_v<T, uint32_t>) {
            if (!column.is_payload_size_representable().ok()) {
    ...
    static StatusOr<uint8_t*> serialize(const BinaryColumnBase<T>& column, uint8_t* buff, const int encode_level) {
        if constexpr (std::is_same_v<T, uint32_t>) {
            const auto payload_size_status = column.is_payload_size_representable();
            if (!payload_size_status.ok()) {
                if (config::enable_binary_column_serde_overflow_check) {
                    return payload_size_status;
                }
                buff = _write_size<uint32_t>(kExtendedU32Escape, buff);
                buff = _write_size<uint32_t>(kExtendedU32Escape, buff);
                buff = _write_size<uint32_t>(kExtendedFormatVersionV1, buff);
        ...
        if constexpr (std::is_same_v<T, uint32_t>) {
            ...
            if (bytes_size == kExtendedU32Escape) {
                ...
                if (offset_bytes_size == kExtendedU32Escape) {
                    ...
                    ASSIGN_OR_RETURN(buff, _read_size<uint32_t>(buff, end, &format_version));
                    if (UNLIKELY(format_version != kExtendedFormatVersionV1)) {
                        return Status::NotSupported(fmt::format(
                                "unsupported BinaryColumn extended serde format version {}, supported version {}",
                                format_version, static_cast<uint32_t>(kExtendedFormatVersionV1)));
                    }
                    return _deserialize_body<uint64_t>(buff, end, column, encode_level);
                }
                // no bytes at all, just resize 0 for bytes part
                column->get_bytes().resize(0);
...
private:
    ...
    enum ExtendedFormatVersion : uint32_t {
        kExtendedFormatVersionV1 = 1,
    };
    static constexpr int64_t kExtendedHeaderSize = sizeof(uint32_t) * 3;
    ...
    static StatusOr<const uint8_t*> _deserialize_offsets(const uint8_t* buff, const uint8_t* end,
        ...
        return _deserialize_offsets_with_size<SerializedOffset>(buff, end, column, offset_bytes_size, encode_level);
    }
