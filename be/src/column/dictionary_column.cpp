...
namespace starrocks {
    ...
    DictionaryColumn::DictionaryColumn(ColumnPtr data_column, IndexColumnPtr index_column)
            : _data_column(std::move(data_column)),
              _index_column(std::move(index_column)) {}
    ...
    size_t DictionaryColumn::serialize_batch_at_interval(uint8_t* dst, size_t byte_offset, size_t byte_interval, size_t start,
                                                       size_t count) {
        DCHECK(false);
        return 0;
    }
