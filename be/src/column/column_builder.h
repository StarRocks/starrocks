...
public:
    ...
    MutableColumnPtr build(bool is_const) {
        ...
        if (is_const) {
            return ConstColumn::create(_column->assume_mutable(), _column->size());
        } else if (_has_null) {
            return NullableColumn::create(_column->assume_mutable(), _null_column->assume_mutable());
        } else {
            return _column->assume_mutable();
        }
    }
