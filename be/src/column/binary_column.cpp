...
void BinaryColumnBase<T>::fnv_hash(uint32_t* hashes, uint32_t from, uint32_t to) const {
    ...
    for (uint32_t i = from; i < to; ++i) {
        hashes[i] =
                HashUtil::fnv_hash(base + _offsets[i], static_cast<uint32_t>(_offsets[i + 1] - _offsets[i]), hashes[i]);
    }
}
template <typename T>
...
void BinaryColumnBase<T>::fnv_hash_selective(uint32_t* hashes, uint16_t* sel, uint16_t sel_size) const {
    ...
    for (uint16_t i = 0; i < sel_size; i++) {
        uint16_t idx = sel[i];
        hashes[idx] = HashUtil::fnv_hash(base + _offsets[idx], static_cast<uint32_t>(_offsets[idx + 1] - _offsets[idx]),
                                         hashes[idx]);
    }
}
...
void BinaryColumnBase<T>::crc32_hash_selective(uint32_t* hashes, uint16_t* sel, uint16_t sel_size) const {
    ...
    for (uint16_t i = 0; i < sel_size; i++) {
        uint16_t idx = sel[i];
        hashes[idx] = HashUtil::zlib_crc_hash(base + _offsets[idx],
...
void BinaryColumnBase<T>::murmur_hash3_x86_32(uint32_t* hashes, uint32_t from, uint32_t to) const {
    ...
    for (uint32_t i = from; i < to; ++i) {
        hashes[i] =
                HashUtil::murmur_hash3_32(base + _offsets[i], static_cast<uint32_t>(_offsets[i + 1] - _offsets[i]), 0);
    }
}
