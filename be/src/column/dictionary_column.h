...
public:
    ...
    DictionaryColumn(DictionaryColumn&& rhs) noexcept
            : _data_column(std::move(rhs._data_column)), _index_column(std::move(rhs._index_column)) {}
    ...
    size_t size() const override { return _index_column->size(); }
    ...
    void reserve(size_t n) override { _index_column->reserve(n); }

    void resize(size_t n) override { _index_column->resize(n); }

    void resize_uninitialized(size_t n) override { _index_column->resize_uninitialized(n); }

    void assign(size_t n, size_t idx) override { _index_column->assign(n, idx); }
    ...
    [[nodiscard]] bool append_strings_overflow(const Buffer<Slice>& strs __attribute__((unused)),
                                               size_t max_length __attribute__((unused))) override {
        return false;
    }
    ...
    uint32_t max_one_element_serialize_size() const override { return _data_column->max_one_element_serialize_size(); }
    ...
    MutableColumnPtr clone_empty() const override { return _data_column->clone_empty(); }
    ...
    size_t memory_usage() const override { return _data_column->memory_usage() + _index_column->memory_usage(); }
