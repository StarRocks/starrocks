...
// offset arrays. It stores offsets in Small (uint32_t) while every active
// value fits, and switches to Large (uint64_t) when a value is known to exceed
// UINT32_MAX. Promotion preserves existing values; make_room() and whole-buffer
// setters are explicit discard/replace paths. The container does not
// automatically demote except through reset() or set_small_buffer().
//
...
public:
    ...
    // Mutable storage accessors bypass scalar overflow checks; callers must
    // select a sufficient active width before writing through them.
    Small& small_storage() {
        DCHECK(!_large);
        return _u32;
    }
    ...
    // Ensure the active storage can represent |v| while preserving existing
    // offsets. Builders that know the final offset upper bound can call this
    // once before visit_storage() to avoid generic per-element checks.
    ...
    void resize(size_t n, uint64_t fill) {
        if (LIKELY(!_large)) {
            if (LIKELY(fill <= std::numeric_limits<uint32_t>::max())) {
    ...
    // Force back to small mode and request release of large storage. Small-buffer
    ...
    // Append |count| copies of the current last offset. Requires non-empty storage when count > 0.
    ...
    // Replace the active storage with |n| uninitialized slots. Existing contents
    // are discarded and may be uninitialized; this path must not observe them.
    // Caller guarantees |max_value| covers every value that will be written
    // into the new active storage.
    void make_room(size_t n, uint64_t max_value) {
        if (UNLIKELY(!_large && max_value > std::numeric_limits<uint32_t>::max())) {
            _switch_to_large_discarding_values();
        }
    ...
    // range [old_size, n) must be fully overwritten before any read observes it.
    ...
    // Width-resolved uninitialized resize. Existing prefix is preserved; promotion,
    // if needed, happens before any uninitialized slots are exposed.
    ...
    // Mutable visitors bypass scalar overflow checks; callers that write through
    // them must have already selected a sufficient width.
    //
    // Usage:
...
private:
    // One-time promotion from small to large while preserving existing initialized
    // offsets. O(n) but happens at most once between explicit demotions.
    void _promote_to_large();
    // Switch width for discard-and-rebuild paths without observing existing small slots.
    void _switch_to_large_discarding_values();
