...
Status convert_column_to_arrow_array(const ColumnPtr& input_column, const TypeDescriptor& type_desc,
                                     const std::shared_ptr<arrow::DataType>& arrow_type, arrow::MemoryPool* pool,
                                     std::shared_ptr<arrow::Array>* result) {
    ColumnPtr column = input_column;
    if (column->is_constant()) {
        column = ColumnHelper::copy_and_unfold_const_column(type_desc, column->is_nullable(), column, column->size());
    }

    std::shared_ptr<arrow::Array> array;
    ColumnToArrowArrayConverter converter(column, pool, type_desc, arrow_type, array);
    auto arrow_st = arrow::VisitTypeInline(*arrow_type, &converter);
    if (!arrow_st.ok()) {
