...
class CACHELINE_ALIGNED ColumnPool {
    ...
    public:
        ...
        inline void return_object(T* ptr, size_t batch_size) {
            if (UNLIKELY(column_reserved_size(ptr) > batch_size)) {
                delete ptr;
                return;
            }
...
public:
    ...
    inline void return_column(T* ptr, size_t batch_size) {
        LocalPool* lp = _get_or_new_local_pool();
        if (LIKELY(lp != nullptr)) {
            reset_column(ptr);
            lp->return_object(ptr, batch_size);
        } else {
            delete ptr;
        }
    }
...
inline void return_column(T* ptr, size_t batch_size) {
    static_assert(InList<ColumnPool<T>, ColumnPoolList>::value, "Cannot use column pool");
    ColumnPool<T>::singleton()->return_column(ptr, batch_size);
}
