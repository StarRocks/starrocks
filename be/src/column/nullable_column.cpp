...
bool NullableColumn::serialize_column(io::ZeroCopyOutputStream* out) {
    if (!_null_column->serialize_column(out)) return false;
    if (!_data_column->serialize_column(out)) return false;
    return true;
}

bool NullableColumn::deserialize_column(io::ZeroCopyInputStream* in) {
    if (!_null_column->deserialize_column(in)) return false;
    if (!_data_column->deserialize_column(in)) return false;
    return true;
}
