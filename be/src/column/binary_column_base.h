...
template <typename T>
class BinaryColumnBase : public Column {
    friend class ColumnFactory<Column, BinaryColumnBase>;
...
public:
    ...
    // NOTE: do *NOT* copy |_slices|
    BinaryColumnBase<T>(BinaryColumnBase<T>&& rhs) noexcept : _bytes(std::move(rhs._bytes)), _offsets(std::move(rhs._offsets)) {}
    ...
    uint32_t serialize_size(size_t idx) const override { return sizeof(T) + _offsets[idx + 1] - _offsets[idx]; }
