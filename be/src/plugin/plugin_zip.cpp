...
//   https://github.com/apache/incubator-doris/blob/master/be/src/plugin/plugin_zip.cpp
...
Status PluginZip::download(const std::string& zip_path) {
    ...
    auto download_cb = [&status, &digest, &file](const void* data, size_t length) {
        ...
        if (!status.ok()) {
            LOG(WARNING) << "fail to download data, file: " << file->filename() << ", error: " << status.to_string();
            return false;
        }
    ...
    std::string expect;
    auto download_md5_cb = [&expect](const void* data, size_t length) {
