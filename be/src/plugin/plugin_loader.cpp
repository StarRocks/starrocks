...
//   https://github.com/apache/incubator-doris/blob/master/be/src/plugin/plugin_loader.cpp
...
Status DynamicPluginLoader::open_plugin() {
    ...
    if (STARROCKS_PLUGIN_VERSION > *(int*)symbol) {
...
BuiltinPluginLoader::BuiltinPluginLoader(const std::string& name, int type, const starrocks::Plugin* plugin)
