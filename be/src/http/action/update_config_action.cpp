...
Status UpdateConfigAction::update_config(const std::string& name, const std::string& value) {
    std::call_once(_once_flag, [&]() {
        ...
        _config_callback.emplace("storage_page_cache_limit", [&]() -> Status {
            ASSIGN_OR_RETURN(int64_t cache_limit, CacheEnv::GetInstance()->get_storage_page_cache_size());
            cache_limit = CacheEnv::GetInstance()->check_storage_page_cache_size(cache_limit);
        ...
        _config_callback.emplace("datacache_mem_size", [&]() -> Status {
            ...
            Status st = DataCacheUtils::parse_conf_datacache_mem_size(config::datacache_mem_size,
                                                                      GlobalEnv::GetInstance()->mem_limit(), &mem_size);
            if (!st.ok()) {
