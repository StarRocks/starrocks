...
Status UpdateConfigAction::update_config(const std::string& name, const std::string& value) {
    std::call_once(_once_flag, [&]() {
        ...
        _config_callback.emplace("pk_index_rebuild_thread_pool_num_max", [&]() -> Status {
            LOG(INFO) << "set pk_index_rebuild_thread_pool_num_max: " << config::pk_index_rebuild_thread_pool_num_max;
            return StorageEngine::instance()->update_manager()->get_pindex_rebuild_executor()->refresh_max_thread_num();
        });
