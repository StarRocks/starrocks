...
Status UpdateConfigAction::update_config(const std::string& name, const std::string& value) {
    std::call_once(_once_flag, [&]() {
        ...
        _config_callback.emplace("datacache_mem_size", [&]() -> Status {
            ...
            size_t mem_size = 0;
            Status st = parse_conf_datacache_mem_size(config::datacache_mem_size, mem_limit, &mem_size);
            if (!st.ok()) {
...
>>>>>>> 9bf55233a4 ([BugFix] The update_config interface still returns success after the dynamic parameter setup fails. (#50821))
        });
...
<<<<<<< HEAD
            (void)BlockCache::instance()->update_disk_spaces(spaces);
        });
