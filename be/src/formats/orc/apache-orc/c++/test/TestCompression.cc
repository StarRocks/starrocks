...
//   https://github.com/apache/orc/tree/main/c++/test/TestCompression.cc
...
void generateRandomData(char* data, size_t size, bool letter) {
    for (size_t i = 0; i < size; ++i) {
        if (letter) {
            ...
            data[i] =
                    capitalized ? static_cast<char>('A' + std::rand() % 26) : static_cast<char>('a' + std::rand() % 26);
        } else {
...
void decompressAndVerify(const MemoryOutputStream& memStream, CompressionKind kind, const char* data, size_t size,
                         MemoryPool& pool) {
...
void compressAndVerify(CompressionKind kind, OutputStream* outStream, CompressionStrategy strategy, uint64_t capacity,
                       uint64_t block, MemoryPool& pool, const char* data, size_t dataSize) {
    ...
    while (dataSize > 0 && compressStream->Next(reinterpret_cast<void**>(&compressBuffer), &compressBufferSize)) {
...
void compress_original_string(orc::CompressionKind kind) {
    ...
    compressAndVerify(kind, &memStream, CompressionStrategy_SPEED, capacity, block, *pool, testData, sizeof(testData));
...
void compress_two_blocks(orc::CompressionKind kind) {
    ...
    compressAndVerify(kind, &memStream, CompressionStrategy_SPEED, capacity, block, *pool, testData, 170);
...
void compress_random_letters(orc::CompressionKind kind) {
    ...
    compressAndVerify(kind, &memStream, CompressionStrategy_SPEED, capacity, block, *pool, testData, dataSize);
...
void protobuff_compression(orc::CompressionKind kind, proto::CompressionKind protoKind) {
    ...
    std::unique_ptr<BufferedOutputStream> compressStream =
            createCompressor(kind, &memStream, CompressionStrategy_SPEED, capacity, block, *pool);
...
void testSeekDecompressionStream(CompressionKind kind) {
    ...
    AppendOnlyBufferedStream outStream(
            createCompressor(kind, &memStream, strategy, DEFAULT_MEM_STREAM_SIZE, blockSize, *pool));
