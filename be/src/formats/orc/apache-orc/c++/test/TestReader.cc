...
void verifySelection(const std::unique_ptr<Reader>& reader, const RowReaderOptions::IdReadIntentMap& idReadIntentMap,
...
std::unique_ptr<Reader> createNestedListMemReader(MemoryOutputStream& memStream) {
    ...
    auto type =
            std::unique_ptr<Type>(Type::buildTypeFromString("struct<"
                                                            "int_array:array<int>,"
...
TEST(TestReadIntent, testMapConflictingIntent) {
    ...
    verifySelection(reader, {{5, ReadIntent_ALL}, {9, ReadIntent_OFFSETS}}, {0, 5, 6, 7, 8, 9, 10, 11});
...
TEST(TestReadIntent, testMapRowBatchContent) {
    ...
    RowReaderOptions::IdReadIntentMap idReadIntentMap = {{2, ReadIntent_ALL}, {5, ReadIntent_OFFSETS}};
    RowReaderOptions rowReaderOpts;
    rowReaderOpts.includeTypesWithIntents(idReadIntentMap);
    std::unique_ptr<RowReader> rowReader = reader->createRowReader(rowReaderOpts);
...
TEST(TestReadIntent, testUnionOffsets) {
    ...
    verifySelection(reader, {{2, ReadIntent_OFFSETS}, {8, ReadIntent_OFFSETS}}, {0, 2, 5, 6, 7, 8, 11});
}
...
TEST(TestReadIntent, testUnionConflictingIntent) {
    ...
    verifySelection(reader, {{5, ReadIntent_OFFSETS}, {8, ReadIntent_ALL}}, {0, 5, 6, 7, 8, 9, 10, 11});
    verifySelection(reader, {{5, ReadIntent_ALL}, {8, ReadIntent_OFFSETS}}, {0, 5, 6, 7, 8, 9, 10, 11});
