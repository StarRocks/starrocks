...
std::unique_ptr<SeekableInputStream> MockStripeStreams::getStream(uint64_t columnId, proto::Stream_Kind kind,
...
TEST_P(TestColumnReaderEncoded, testDictionaryWithNulls) {
    ...
    if (encoded) {
        EncodedStringVectorBatch* encodedStringBatch = new EncodedStringVectorBatch(1024, *getDefaultPool());
        StructVectorBatch batch(1024, *getDefaultPool());
        ...
        for (size_t i = 0; i < batch.numElements; ++i) {
            if (i & 4) {
                ...
                for (size_t letter = 0; letter < strlen(expected); ++letter) {
                    EXPECT_EQ(expected[letter], actualString[letter]) << "Wrong contents at " << i << ", " << letter;
                }
            }
        }
    } else {
...
TEST_P(TestColumnReaderEncoded, testVarcharDictionaryWithNulls) {
    ...
    EXPECT_CALL(streams, getEncoding(testing::Ge(2))).WillRepeatedly(testing::Return(dictionary0Encoding));
    ...
    EXPECT_CALL(streams, getStreamProxy(2, proto::Stream_Kind_PRESENT, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(buffer5, ARRAY_SIZE(buffer5))));
...
TEST(TestColumnReader, testBinaryDirect) {
    ...
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_PRESENT, true)).WillRepeatedly(testing::Return(nullptr));
...
TEST(TestColumnReader, testStringDirectNullAcrossWindow) {
    ...
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_LENGTH, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(lenData, ARRAY_SIZE(lenData))));
...
TEST(TestColumnReader, testStringDirectSkip) {
    ...
    const unsigned char buffer1[] = {0x7f, 0x01, 0x00, 0x7f, 0x01, 0x82, 0x01, 0x7f, 0x01, 0x84, 0x02, 0x7f, 0x01,
                                     0x86, 0x03, 0x7f, 0x01, 0x88, 0x04, 0x7f, 0x01, 0x8a, 0x05, 0x7f, 0x01, 0x8c,
                                     0x06, 0x7f, 0x01, 0x8e, 0x07, 0x7f, 0x01, 0x90, 0x08, 0x1b, 0x01, 0x92, 0x09};
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_LENGTH, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(buffer1, ARRAY_SIZE(buffer1))));
...
TEST(TestColumnReader, testStringDirectSkipWithNulls) {
    ...
    // range(1200)
    const unsigned char buffer2[] = {0x7f, 0x01, 0x00, 0x7f, 0x01, 0x82, 0x01, 0x7f, 0x01, 0x84, 0x02, 0x7f, 0x01,
                                     0x86, 0x03, 0x7f, 0x01, 0x88, 0x04, 0x7f, 0x01, 0x8a, 0x05, 0x7f, 0x01, 0x8c,
                                     0x06, 0x7f, 0x01, 0x8e, 0x07, 0x7f, 0x01, 0x90, 0x08, 0x1b, 0x01, 0x92, 0x09};
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_LENGTH, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(buffer2, ARRAY_SIZE(buffer2))));
...
TEST_P(TestColumnReaderEncoded, testList) {
    ...
    // range(1200)
    const unsigned char buffer2[] = {0x7f, 0x01, 0x00, 0x7f, 0x01, 0x84, 0x02, 0x7f, 0x01, 0x88, 0x04, 0x7f, 0x01,
                                     0x8c, 0x06, 0x7f, 0x01, 0x90, 0x08, 0x7f, 0x01, 0x94, 0x0a, 0x7f, 0x01, 0x98,
                                     0x0c, 0x7f, 0x01, 0x9c, 0x0e, 0x7f, 0x01, 0xa0, 0x10, 0x1b, 0x01, 0xa4, 0x12};
    EXPECT_CALL(streams, getStreamProxy(2, proto::Stream_Kind_DATA, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(buffer2, ARRAY_SIZE(buffer2))));
...
TEST(TestColumnReader, testListWithNulls) {
    ...
    EXPECT_CALL(streams, getStreamProxy(2, proto::Stream_Kind_PRESENT, true)).WillRepeatedly(testing::Return(nullptr));
    ...
    // [19]
    const unsigned char buffer2[] = {0x7f, 0x00, 0x01, 0x7f, 0x00, 0x01, 0x7f, 0x00, 0x04, 0x7f, 0x00, 0x04, 0x7f,
                                     0x00, 0x00, 0x7f, 0x00, 0x00, 0x7f, 0x00, 0x03, 0x6e, 0x00, 0x03, 0xff, 0x13};
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_LENGTH, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(buffer2, ARRAY_SIZE(buffer2))));

    // range(2048)
    const unsigned char buffer3[] = {0x7f, 0x01, 0x00, 0x7f, 0x01, 0x84, 0x02, 0x7f, 0x01, 0x88, 0x04, 0x7f, 0x01,
                                     0x8c, 0x06, 0x7f, 0x01, 0x90, 0x08, 0x7f, 0x01, 0x94, 0x0a, 0x7f, 0x01, 0x98,
...
TEST(TestColumnReader, testListSkipWithNullsNoData) {
    ...
    EXPECT_CALL(streams, getStreamProxy(2, proto::Stream_Kind_DATA, true)).WillRepeatedly(testing::Return(nullptr));
...
TEST_P(TestColumnReaderEncoded, testMap) {
    ...
    // range(8, 1208)
    const unsigned char buffer3[] = {0x7f, 0x01, 0x10, 0x7f, 0x01, 0x94, 0x02, 0x7f, 0x01, 0x98, 0x04, 0x7f, 0x01,
                                     0x9c, 0x06, 0x7f, 0x01, 0xa0, 0x08, 0x7f, 0x01, 0xa4, 0x0a, 0x7f, 0x01, 0xa8,
                                     0x0c, 0x7f, 0x01, 0xac, 0x0e, 0x7f, 0x01, 0xb0, 0x10, 0x1b, 0x01, 0xb4, 0x12};
    EXPECT_CALL(streams, getStreamProxy(3, proto::Stream_Kind_DATA, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(buffer3, ARRAY_SIZE(buffer3))));

    // create the row type
    std::unique_ptr<Type> rowType = createStructType();
    rowType->addStructField("col0", createMapType(createPrimitiveType(LONG), createPrimitiveType(LONG)));
...
TEST(TestColumnReader, testMapWithNulls) {
    ...
    // [3 for x in range(243)] +
    // [19]
    const unsigned char buffer3[] = {0x7f, 0x00, 0x01, 0x7f, 0x00, 0x01, 0x7f, 0x00, 0x04, 0x7f, 0x00, 0x04, 0x7f,
                                     0x00, 0x00, 0x7f, 0x00, 0x00, 0x7f, 0x00, 0x03, 0x6e, 0x00, 0x03, 0xff, 0x13};
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_LENGTH, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(buffer3, ARRAY_SIZE(buffer3))));

    // range(2048)
    const unsigned char buffer4[] = {0x7f, 0x01, 0x00, 0x7f, 0x01, 0x84, 0x02, 0x7f, 0x01, 0x88, 0x04, 0x7f, 0x01,
                                     0x8c, 0x06, 0x7f, 0x01, 0x90, 0x08, 0x7f, 0x01, 0x94, 0x0a, 0x7f, 0x01, 0x98,
                                     0x0c, 0x7f, 0x01, 0x9c, 0x0e, 0x7f, 0x01, 0xa0, 0x10, 0x7f, 0x01, 0xa4, 0x12,
                                     0x7f, 0x01, 0xa8, 0x14, 0x7f, 0x01, 0xac, 0x16, 0x7f, 0x01, 0xb0, 0x18, 0x7f,
                                     0x01, 0xb4, 0x1a, 0x7f, 0x01, 0xb8, 0x1c, 0x5f, 0x01, 0xbc, 0x1e};
    ...
    // range(8, 1032)
    const unsigned char buffer5[] = {0x7f, 0x01, 0x10, 0x7f, 0x01, 0x94, 0x02, 0x7f, 0x01, 0x98, 0x04,
                                     0x7f, 0x01, 0x9c, 0x06, 0x7f, 0x01, 0xa0, 0x08, 0x7f, 0x01, 0xa4,
                                     0x0a, 0x7f, 0x01, 0xa8, 0x0c, 0x6f, 0x01, 0xac, 0x0e};
    EXPECT_CALL(streams, getStreamProxy(3, proto::Stream_Kind_DATA, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(buffer5, ARRAY_SIZE(buffer5))));
...
TEST(TestColumnReader, testMapSkipWithNulls) {
    ...
    // range(8, 2056)
    const unsigned char buffer4[] = {0x7f, 0x01, 0x10, 0x7f, 0x01, 0x94, 0x02, 0x7f, 0x01, 0x98, 0x04, 0x7f, 0x01,
                                     0x9c, 0x06, 0x7f, 0x01, 0xa0, 0x08, 0x7f, 0x01, 0xa4, 0x0a, 0x7f, 0x01, 0xa8,
...
TEST(TestColumnReader, testFloatWithNulls) {
    ...
    const float test_vals[] = {1.0f,
                               ...
                               1.4e-45f,
                               -1.4e-45f};
    const unsigned char buffer2[] = {0x00, 0x00, 0x80, 0x3f, 0x00, 0x00, 0x20, 0x40, 0x00, 0x40, 0xc8, 0xc2, 0x00,
...
TEST(TestColumnReader, testDoubleWithNulls) {
    ...
    const unsigned char buffer2[] = {
            0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0xf0, 0x3f, 0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0x40, 0x0,  0x0,
            0x0,  0x0,  0x0,  0x0,  0x0,  0xc0, 0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0x59, 0x40, 0xe8, 0x38, 0x65, 0x99,
            0xf9, 0x58, 0x98, 0x46, 0xa1, 0x88, 0x41, 0x98, 0xc5, 0x90, 0x4f, 0xbc, 0x0,  0x0,  0x0,  0x0,  0x0,  0x0,
            0xf0, 0x7f, 0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0xf8, 0x7f, 0x0,  0x0,  0x0,  0x0,  0x0,  0x0,  0xf0, 0xff,
            0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xef, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xef, 0xff, 0x1,  0x0,
...
TEST(TestColumnReader, testDoubleSkipWithNulls) {
    ...
    // 1, 2, -2
    const unsigned char buffer2[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x3f, 0x00, 0x00, 0x00, 0x00,
...
TEST(TestColumnReader, testTimestamp) {
    ...
    const unsigned char buffer1[] = {0xf6, 0x9f, 0xf4, 0xc6, 0xbd, 0x03, 0xff, 0xec, 0xf3, 0xbc, 0x03, 0xff, 0xb1,
    ...
    const unsigned char buffer2[] = {0xf6, 0x00, 0xa8, 0xd1, 0xf9, 0xd6, 0x03, 0x00, 0x9e, 0x01, 0xec,
...
TEST(DecimalColumnReader, testDecimal64) {
    ...
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_DATA, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(numBuffer, ARRAY_SIZE(numBuffer), 3)));
...
TEST(DecimalColumnReader, testDecimal64Skip) {
    ...
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_PRESENT, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(presentBuffer, ARRAY_SIZE(presentBuffer))));

    // [493827160549382716, 4938271605493827, 49382716054938, 493827160549,
    //  4938271605, 49382716, 493827, 4938, 49]
    const unsigned char numBuffer[] = {0xf8, 0xe8, 0xe2, 0xcf, 0xf4, 0xcb, 0xb6, 0xda, 0x0d, 0x86, 0xc1, 0xcc,
    ...
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_DATA, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(numBuffer, ARRAY_SIZE(numBuffer))));
...
TEST(DecimalColumnReader, testDecimal128Skip) {
    ...
    const unsigned char numBuffer[] = {
            0xf8, 0xe8, 0xe2, 0xcf, 0xf4, 0xcb, 0xb6, 0xda, 0x0d, 0x86, 0xc1, 0xcc, 0xcd, 0x9e, 0xd5, 0xc5, 0x11,
            0xb4, 0xf6, 0xfc, 0xf3, 0xb9, 0xba, 0x16, 0xca, 0xe7, 0xa3, 0xa6, 0xdf, 0x1c, 0xea, 0xad, 0xc0, 0xe5,
            0x24, 0xf8, 0x94, 0x8c, 0x2f, 0x86, 0xa4, 0x3c, 0x94, 0x4d, 0x62, 0xaa, 0xcd, 0xb3, 0xf2, 0x9e, 0xf0,
            0x99, 0xd6, 0xbe, 0xf8, 0xb6, 0x9e, 0xe4, 0xb7, 0xfd, 0xce, 0x8f, 0x34, 0xa9, 0xcd, 0xb3, 0xf2, 0x9e,
            0xf0, 0x99, 0xd6, 0xbe, 0xf8, 0xb6, 0x9e, 0xe4, 0xb7, 0xfd, 0xce, 0x8f, 0x34, 0xfe, 0xff, 0xff, 0xff,
            0xff, 0x8f, 0x91, 0x8a, 0x93, 0xe8, 0xa3, 0xec, 0xd0, 0x96, 0xd4, 0xcc, 0xf6, 0xac, 0x02, 0xfd, 0xff,
            0xff, 0xff, 0xff, 0x8f, 0x91, 0x8a, 0x93, 0xe8, 0xa3, 0xec, 0xd0, 0x96, 0xd4, 0xcc, 0xf6, 0xac, 0x02,
    };
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_DATA, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(numBuffer, ARRAY_SIZE(numBuffer))));
    ...
    EXPECT_EQ("1.7320508075688772935274463415058723669", values[1].toDecimalString(decimals->scale));
...
TEST(DecimalColumnReader, testDecimalHive11) {
    ...
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_SECONDARY, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(scaleBuffer, ARRAY_SIZE(scaleBuffer))));
...
TEST(DecimalColumnReader, testDecimalHive11Skip) {
    ...
    EXPECT_EQ(5, decimals->numElements);
    EXPECT_EQ(49, values[0].toLong());
    EXPECT_EQ("17320508075688772935274463415058723.669", values[1].toDecimalString(decimals->scale));
    EXPECT_EQ("-17320508075688772935274463415058723.669", values[2].toDecimalString(decimals->scale));
    EXPECT_EQ("99999999999999999999999999999999999.999", values[3].toDecimalString(decimals->scale));
    EXPECT_EQ("-99999999999999999999999999999999999.999", values[4].toDecimalString(decimals->scale));
}
...
TEST(DecimalColumnReader, testDecimalHive11ScaleUp) {
    ...
    // [1] * 21
    const unsigned char numBuffer[] = {0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
...
TEST(DecimalColumnReader, testDecimalHive11ScaleDown) {
    ...
    const unsigned char numBuffer[] = {
            };
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_DATA, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(numBuffer, ARRAY_SIZE(numBuffer))));
...
TEST(DecimalColumnReader, testDecimalHive11OverflowNull) {
    ...
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_PRESENT, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(presentBuffer, sizeof(presentBuffer))));

    // [1000000000000000000000000000000000000000, 1,
    //  -10000000000000000000000000000000000000, 1]
    unsigned char numBuffer[] = {0x80, 0x80, 0x80, 0x80, 0x80, 0xc0, 0xb0, 0xf5, 0xf3, 0xae, 0xfd, 0xcb, 0x94, 0xd7,
                                 0xe1, 0xf1, 0xd3, 0x8c, 0xeb, 0x01, 0x02, 0xff, 0xff, 0xff, 0xff, 0xff, 0x8f, 0x91,
                                 0x8a, 0x93, 0xe8, 0xa3, 0xec, 0xd0, 0x96, 0xd4, 0xcc, 0xf6, 0xac, 0x02, 0x02};
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_DATA, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(numBuffer, ARRAY_SIZE(numBuffer))));
...
TEST(DecimalColumnReader, testDecimalHive11BigBatches) {
    ...
    for (size_t i = 0; i < decimals->numElements; ++i) {
        EXPECT_EQ((i % 64) * (i < 1024 ? 10 : 100), decimals->values[i].toLong()) << "Wrong value at " << i;
    }
}
...
TEST(TestColumnReader, testUnion) {
    ...
    const unsigned char buffer1[] = {0x7f, 0x00, 0x7f, 0x00, 0x7f, 0x00, 0x7f, 0x00, 0x7f, 0x00, 0x7f, 0x00, 0x7f, 0x00,
                                     0x57, 0x00, 0x7f, 0x01, 0x7f, 0x01, 0x7f, 0x01, 0x7f, 0x01, 0x7f, 0x01, 0x7f, 0x01,
                                     0x7f, 0x01, 0x57, 0x01, 0x7f, 0x00, 0x43, 0x00, 0x7f, 0x01, 0x43, 0x01};
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_DATA, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(buffer1, ARRAY_SIZE(buffer1))));
...
TEST(TestColumnReader, testUnionWithNulls) {
    ...
    // [0, 1] * 24
    const unsigned char buffer2[] = {0xd0, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01,
...
TEST(TestColumnReader, testUnionLongSkip) {
    ...
    // [1] * 1300 + [0] * 1300
    const unsigned char buffer1[] = {0x7f, 0x01, 0x7f, 0x01, 0x7f, 0x01, 0x7f, 0x01, 0x7f, 0x01, 0x7f, 0x01, 0x7f, 0x01,
                                     0x7f, 0x01, 0x7f, 0x01, 0x7f, 0x01, 0x7f, 0x00, 0x7f, 0x00, 0x7f, 0x00, 0x7f, 0x00,
...
TEST(TestColumnReader, testUnionWithManyVariants) {
    ...
    EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_DATA, true))
            .WillRepeatedly(testing::Return(new SeekableArrayInputStream(tagBuffer, ARRAY_SIZE(tagBuffer))));
    ...
    for (size_t i = 0; i < batch.numElements; ++i) {
        ...
        EXPECT_EQ(i & 0x3f,
                  dynamic_cast<LongVectorBatch*>(unions->children[unions->tags[i]])->data[unions->offsets[i]]);
    }
    reader->skip(30);
