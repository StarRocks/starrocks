...
//   https://github.com/apache/orc/tree/main/c++/src/RLE.cc
...
std::unique_ptr<RleEncoder> createRleEncoder(std::unique_ptr<BufferedOutputStream> output, bool isSigned,
                                             RleVersion version, MemoryPool&, bool alignedBitpacking) {
    ...
    case RleVersion_2:
        return std::unique_ptr<RleEncoder>(new RleEncoderV2(std::move(output), isSigned, alignedBitpacking));
    default:
