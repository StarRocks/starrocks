...
ExpressionTree::ExpressionTree(Operator op) : mOperator(op), mLeaf(UNUSED_LEAF), mConstant(TruthValue::YES_NO_NULL) {}
...
TreeNode ExpressionTree::getChild(size_t i) {
    return std::const_pointer_cast<ExpressionTree>(const_cast<const ExpressionTree*>(this)->getChild(i));
}
