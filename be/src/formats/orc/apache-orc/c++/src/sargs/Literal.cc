...
//   https://github.com/apache/orc/tree/main/c++/src/sargs/Literal.cc
...
Literal::Literal(const Literal& r) : mType(r.mType), mSize(r.mSize), mIsNull(r.mIsNull), mHashCode(r.mHashCode) {
...
size_t Literal::hashCode() const {
    ...
    case PredicateDataType::TIMESTAMP:
        return std::hash<int64_t>{}(mValue.TimeStampVal.second) * 17 + std::hash<int32_t>{}(mValue.TimeStampVal.nanos);
    case PredicateDataType::FLOAT:
...
bool Literal::operator==(const Literal& r) const {
    ...
    case PredicateDataType::FLOAT:
        return std::fabs(mValue.DoubleVal - r.mValue.DoubleVal) < std::numeric_limits<double>::epsilon();
    case PredicateDataType::BOOLEAN:
...
inline void validate(const bool& isNull, const PredicateDataType& type, const PredicateDataType& expected) {
    if (isNull) {
