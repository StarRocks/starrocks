...
bool BloomFilterImpl::operator==(const BloomFilterImpl& other) const {
    return mNumBits == other.mNumBits && mNumHashFunctions == other.mNumHashFunctions && *mBitSet == *other.mBitSet;
}
...
std::unique_ptr<BloomFilter> BloomFilterUTF8Utils::deserialize(const proto::Stream_Kind& streamKind,
