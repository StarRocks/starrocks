...
//   https://github.com/apache/orc/tree/main/c++/src/Statistics.hh
...
public:
    ...
    void reset() override { _stats.reset(); }
...
public:
    ...
    bool hasTotalLength() const override { return _stats.hasTotalLength(); }
    ...
    void update(size_t length) { _stats.setTotalLength(_stats.getTotalLength() + length); }

    void merge(const MutableColumnStatistics& other) override {
        const BinaryColumnStatisticsImpl& binStats = dynamic_cast<const BinaryColumnStatisticsImpl&>(other);
...
class BooleanColumnStatisticsImpl : public BooleanColumnStatistics, public MutableColumnStatistics {
private:
...
public:
    ...
    void merge(const MutableColumnStatistics& other) override {
        const BooleanColumnStatisticsImpl& boolStats = dynamic_cast<const BooleanColumnStatisticsImpl&>(other);
...
public:
    ...
    bool hasMaximum() const override { return _stats.hasMaximum(); }
    ...
    void update(int32_t value) { _stats.updateMinMax(value); }

    void merge(const MutableColumnStatistics& other) override {
        const DateColumnStatisticsImpl& dateStats = dynamic_cast<const DateColumnStatisticsImpl&>(other);
...
class DecimalColumnStatisticsImpl : public DecimalColumnStatistics, public MutableColumnStatistics {
private:
...
public:
    ...
    bool hasSum() const override { return _stats.hasSum(); }
    ...
    void merge(const MutableColumnStatistics& other) override {
        const DecimalColumnStatisticsImpl& decStats = dynamic_cast<const DecimalColumnStatisticsImpl&>(other);
...
public:
    ...
    void merge(const MutableColumnStatistics& other) override {
        const DoubleColumnStatisticsImpl& doubleStats = dynamic_cast<const DoubleColumnStatisticsImpl&>(other);
...
class IntegerColumnStatisticsImpl : public IntegerColumnStatistics, public MutableColumnStatistics {
private:
    ...
    void update(const char* value, size_t length) {
        if (value != nullptr) {
            if (!_stats.hasMinimum()) {
                ...
                // update min
                int minCmp =
                        strncmp(_stats.getMinimum().c_str(), value, std::min(_stats.getMinimum().length(), length));
                ...
                // update max
                int maxCmp =
                        strncmp(_stats.getMaximum().c_str(), value, std::min(_stats.getMaximum().length(), length));
...
class TimestampColumnStatisticsImpl : public TimestampColumnStatistics, public MutableColumnStatistics {
private:
    InternalIntegerStatistics _stats;
    bool _hasLowerBound;
    bool _hasUpperBound;
...
public:
    ...
    void update(int64_t value) { _stats.updateMinMax(value); }
    ...
    void merge(const MutableColumnStatistics& other) override {
        const TimestampColumnStatisticsImpl& tsStats = dynamic_cast<const TimestampColumnStatisticsImpl&>(other);
        ...
        if (tsStats.hasMinimum()) {
            if (!_stats.hasMinimum()) {
                ...
                if (tsStats.getMaximum() >= _stats.getMaximum()) {
                    if (tsStats.getMaximum() > _stats.getMaximum() || tsStats.getMaximumNanos() > _maximumNanos) {
                        _maximumNanos = tsStats.getMaximumNanos();
                    }
                    _stats.setMaximum(tsStats.getMaximum());
                }
                if (tsStats.getMinimum() <= _stats.getMinimum()) {
                    if (tsStats.getMinimum() < _stats.getMinimum() || tsStats.getMinimumNanos() < _minimumNanos) {
                        _minimumNanos = tsStats.getMinimumNanos();
                    }
                    _stats.setMinimum(tsStats.getMinimum());
                }
    ...
    bool hasLowerBound() const override { return _hasLowerBound; }

    bool hasUpperBound() const override { return _hasUpperBound; }
