...
//   https://github.com/apache/orc/tree/main/c++/src/RleEncoderV2.cc
...
RleEncoderV2::RleEncoderV2(std::unique_ptr<BufferedOutputStream> outStream, bool hasSigned, bool alignBitPacking)
...
void RleEncoderV2::determineEncoding(EncodingOption& option) {
    ...
    if (option.min == max) {
        if (!option.isFixedDelta) {
            throw InvalidArgument(to_string(option.min) + "==" + to_string(max) + ", isFixedDelta cannot be false");
        }

        if (currDelta != 0) {
            throw InvalidArgument(to_string(option.min) + "==" + to_string(max) + ", currDelta should be zero");
        }
        option.fixedDelta = 0;
        option.encoding = DELTA;
        return;
