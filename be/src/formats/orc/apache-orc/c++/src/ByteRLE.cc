...
//   https://github.com/apache/orc/tree/main/c++/src/ByteRLE.cc
...
ByteRleEncoderImpl::ByteRleEncoderImpl(std::unique_ptr<BufferedOutputStream> output) : outputStream(std::move(output)) {
    ...
    while (numValues > 0) {
        ...
        if (!repeating) {
            ...
            while (consumedBytes > 0) {
                ...
                size_t skipSize =
                        std::min(static_cast<size_t>(consumedBytes), static_cast<size_t>(bufferEnd - bufferStart));
        ...
        if (repeating) {
            ...
            if (notNull) {
                ...
                while (i < count) {
                    ...
                    uint64_t copyBytes =
                            std::min(static_cast<uint64_t>(count - i), static_cast<uint64_t>(bufferEnd - bufferStart));
...
BooleanRleDecoderImpl::BooleanRleDecoderImpl(std::unique_ptr<SeekableInputStream> input, ReaderMetrics* _metrics)
