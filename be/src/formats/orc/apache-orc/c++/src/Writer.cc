...
bool WriterOptions::isColumnUseBloomFilter(uint64_t column) const {
    return privateBits->columnsUseBloomFilter.find(column) != privateBits->columnsUseBloomFilter.end();
}
...
WriterImpl::WriterImpl(const Type& t, OutputStream* stream, const WriterOptions& opts)
    ...
    compressionStream = createCompressor(options.getCompression(), outStream, options.getCompressionStrategy(),
                                         1 * 1024 * 1024, // buffer capacity: 1M
                                         options.getCompressionBlockSize(), *options.getMemoryPool());
...
std::unique_ptr<Writer> createWriter(const Type& type, OutputStream* stream, const WriterOptions& options) {
