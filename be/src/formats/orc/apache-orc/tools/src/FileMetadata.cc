...
void printStripeInformation(std::ostream& out, uint64_t index, uint64_t columns,
    ...
    out << "      \"index\": " << stripe->getIndexLength() << ", \"data\": " << stripe->getDataLength()
    ...
    if (verbose) {
        ...
        for (uint64_t col = 0; col < columns; ++col) {
            ...
            out << "         { \"column\": " << col << ", \"encoding\": \"" << columnEncodingKindToString(encoding)
                << "\"";
        ...
        for (uint64_t str = 0; str < stripe->getNumberOfStreams(); ++str) {
            ...
            out << "        { \"id\": " << str << ", \"column\": " << stream->getColumnId() << ", \"kind\": \""
                << streamKindToString(stream->getKind()) << "\", \"offset\": " << stream->getOffset()
...
void printAttributes(std::ostream& out, const orc::Type& type, const std::string name, bool* hasAnyAttributes) {
