...
//     auto status = _writer->write(chunk.get());
//     return parquet::make_completed_future(Status::OK());
// }
//
// void OrcFileWriter::commitAsync(std::function<void(CommitResult)> callback) {
...
//         auto exception = Status::IOError(fmt::format("{}: {}", "close output stream error", e.what()));
//         LOG(WARNING) << exception;
//         callback({.io_status = exception});
//         return;
//     }
