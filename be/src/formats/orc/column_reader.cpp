...
StatusOr<std::unique_ptr<ORCColumnReader>> ORCColumnReader::create(const TypeDescriptor& type,
    ...
    case TYPE_TIME:
        return std::make_unique<TimeColumnReader<TYPE_TIME>>(type, orc_type, nullable, reader);
    case TYPE_STRUCT: {
...
template <LogicalType Type>
Status TimeColumnReader<Type>::get_next(orc::ColumnVectorBatch* cvb, ColumnPtr& column, size_t from, size_t size) {
    auto* data = down_cast<orc::LongVectorBatch*>(cvb);
    size_t column_start = column->size();
    column->resize_uninitialized(column_start + size);
    if (_nullable) {
