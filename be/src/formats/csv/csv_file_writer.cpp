...
#include "util/defer_op.h"
    ...
    int64_t CSVFileWriter::get_written_bytes() { return _output_stream->size(); }

    int64_t CSVFileWriter::get_allocated_bytes() { return _output_stream->buffer_size(); }

    int64_t CSVFileWriter::get_flush_batch_size() { return 0; }
            ...
            for (size_t c = 0; c < columns.size(); c++) {
                ...
                if (use_enclose) {
                    ...
                    if (columns[c]->is_null(r)) {
                        ...
                        RETURN_IF_ERROR(
                                converter->write_string(_output_stream.get(), *columns[c], r, *_converter_options));
                    } else {
    ...
    StatusOr<WriterAndStream> CSVFileWriterFactory::create(const std::string& path) const {
        ASSIGN_OR_RETURN(auto file, _fs->new_writable_file(WritableFileOptions{.direct_write = true,
                                                                               .content_type = http::ContentType::CSV},
                                                           path));
