...
class StructConverter final : public Converter {
public:
    explicit StructConverter(std::vector<std::unique_ptr<Converter>> child_converters)
            : _child_converters(std::move(child_converters)) {}
...
private:
    void _split_struct_fields(Slice s, std::vector<Slice>& fields, const Options& options) const;
    std::vector<std::unique_ptr<Converter>> _child_converters;
};
