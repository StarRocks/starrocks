...
Status GroupReader::get_next(ChunkPtr* chunk, size_t* row_count) {
    ...
    while (true) {
        ...
        // (*chunk)->num_rows() uses _columns[0]->size(), which may be a not-existed column
        // (e.g. a renamed-away Iceberg column) that is filled later by the file reader and
        // so still has size 0 here.  Read the count from the first read_cols slot that is
        // present in *chunk instead; fall back to active_chunk if read_cols is empty.
        auto filled_num_rows = [&]() -> size_t {
            for (const auto& col : _param.read_cols) {
                SlotId sid = col.slot_id();
                if ((*chunk)->is_slot_exist(sid)) {
                    return (*chunk)->get_column_by_slot_id(sid)->size();
                }
            }
            return active_chunk->num_rows();
        };
    ...
    for (const auto& column : _param.read_cols) {
        if (column.is_extended_variant_virtual) {
            ...
            if (physical_it != physical_variant_slots_by_name.end()) {
                ...
                DCHECK(source_schema_node != nullptr);
                DCHECK(source_schema_node->type == ColumnType::STRUCT);
