...
constexpr static const LogicalType kDictCodeLogicalType = LOGICAL_TYPE_INT;
    ...
    for (const auto& column : _dict_filter_columns) {
        ...
        if (dict_codes.size() == 1) {
            _dict_filter_preds[slot_id] =
                    new_column_eq_predicate(get_type_info(kDictCodeLogicalType), slot_id, std::to_string(dict_codes[0]));
        } else {
            ...
            _dict_filter_preds[slot_id] =
                    new_column_in_predicate(get_type_info(kDictCodeLogicalType), slot_id, str_codes);
        }
