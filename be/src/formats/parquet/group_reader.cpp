...
GroupReader::GroupReader(GroupReaderParam& param, int row_group_number, std::set<std::int64_t> need_skip_rowids)
        : _need_skip_rowids(std::move(need_skip_rowids)), _param(param) {
    _row_group_metadata =
            std::make_shared<tparquet::RowGroup>(param.file_metadata->t_metadata().row_groups[row_group_number]);
    for (int i = 0; i < row_group_number; ++i) {
        _row_group_first_row += param.file_metadata->t_metadata().row_groups[i].num_rows;
    }
}
...
Status GroupReader::get_next(vectorized::ChunkPtr* chunk, size_t* row_count) {
    ...
    if (!_need_skip_rowids.empty()) {
        ...
        auto end_iter =
                upper_bound(_need_skip_rowids.begin(), _need_skip_rowids.end(), current_chunk_base_row + chunk_size);
