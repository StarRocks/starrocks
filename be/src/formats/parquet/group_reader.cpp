...
static std::string debug_filter(vectorized::Filter* f) {
    std::stringstream ss;
    ss << "(size = " << f->size() << ", content = ";
...
Status GroupReader::get_next(vectorized::ChunkPtr* chunk, size_t* row_count) {
    ...
    VLOG_FILE << "expect to read chunk size = " << count;
    ...
    if (has_dict_filter) {
        ...
        VLOG_FILE << "[XXX] filter = " << debug_filter(&chunk_filter);
        has_filter = true;
    }
...
Status GroupReader::_read(const std::vector<int>& read_columns, size_t* row_count, vectorized::ChunkPtr* chunk) {
    ...
    for (int col_idx : read_columns) {
        ...
        VLOG_FILE << "column addr = " << &column << ", type = " << content_type;
        SlotId slot_id = column.slot_id;
        _column_reader_opts.context->next_row = 0;
        count = *row_count;
        VLOG_FILE << "col_idx = " << col_idx << ", column type = " << content_type
                  << ", DICT_CODE = " << ColumnContentType::DICT_CODE << ", VALUE = " << ColumnContentType::VALUE;
