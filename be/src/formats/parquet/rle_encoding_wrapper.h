//
// Created by beyond-up on 2022/5/11.
//
...
#ifndef STARROCKS_RLE_ENCODING_WAPPER_H
#define STARROCKS_RLE_ENCODING_WAPPER_H

#endif //STARROCKS_RLE_ENCODING_WAPPER_H

//RleDecoder is not deriving from Decoder,  create this wrapper class deriving from Decoder and encapsulate RleDecoder

namespace starrocks::parquet {

template <typename T>
class RleEncoderWrapper final : public Encoder {
public:
    RleEncoderWrapper() = default;
    ~RleEncoderWrapper() override = default;
...
template <>
class RleEncoderWrapper<Slice> final : public Encoder {
public:
    RleEncoderWrapper() = default;
    ~RleEncoderWrapper() override = default;
...
template <>
class RleEncoderWrapper<bool> final : public Encoder {
public:
    RleEncoderWrapper() : _bit_writer(&_buffer) {}
    ~RleEncoderWrapper() override = default;

    Status append(const uint8_t* vals, size_t count) override {
        // TODO(@DorianZheng) Rle boolean encoder is using by UT currently, optimize it in the future.
        for (int i = 0; i < count; i++) {
...
template <typename T>
class RleDecoderWrapper final : public Decoder {
public:
    RleDecoderWrapper() = default;
    ~RleDecoderWrapper() override = default;
...
template <>
class RleDecoderWrapper<Slice> final : public Decoder {
public:
    RleDecoderWrapper() = default;
    ~RleDecoderWrapper() override = default;
...
// Rle encoding for boolean type is stored as `Bit Packed`, `LSB` first format
// more details refer to: https://github.com/apache/parquet-format/blob/master/Encodings.md#Rle
template <>
class RleDecoderWrapper<bool> final : public Decoder {
public:
    RleDecoderWrapper() = default;
    ~RleDecoderWrapper() override = default;
