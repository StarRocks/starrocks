...
StatusOr<bool> ScalarColumnReader::row_group_zone_map_filter(const std::vector<const ColumnPredicate*>& predicates,
    ...
    if (is_all_null) {
        ...
        if (st.ok()) {
            st = StatisticsHelper::decode_value_into_column(min_column, min_values, null_pages, *_col_type,
                                                            get_column_parquet_field(), _opts.timezone);
            RETURN_IF(!st.ok(), true);
            st = StatisticsHelper::decode_value_into_column(max_column, max_values, null_pages, *_col_type,
                                                            get_column_parquet_field(), _opts.timezone);
            RETURN_IF(!st.ok(), true);
...
>>>>>>> 11e2295cdf ([BugFix]Don't return error when decoding min/max is not suppoted (#59342))
