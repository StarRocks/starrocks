...
Status PageReader::_get_header(size_t allowed_page_size, uint8_t** page_buf, bool* peek_mode, IOBuffer* buffer) {
    ...
    Status res;
    {
        ...
        res = _cache->read_buffer(_pagecache_key, buffer);   
        LOG(INFO) << "[Gavin] read page buffer, _pagecache_key: " << _pagecache_key << ", buf size: " << buffer->size()
                  << ", res: " << res.message();
    }
    if (res.ok()) {
...
Status PageReader::set_page_buffer(Slice* slice) {
    ...
    LOG(INFO) << "[Gavin] remote page buffer offset: " << _remote_page_buffer.offset << ", header size: " << _remote_page_buffer.header.size()
              << ", page size: " << slice->get_size();
    ...
    Status res;
    {
        ...
        res = _cache->write_buffer(_pagecache_key, buffer);   
        LOG(INFO) << "[Gavin] set page buffer, _pagecache_key: " << _pagecache_key << ", buf size: " << buffer.size()
                  << ", res: " << res.message() << ", header size: " << _remote_page_buffer.offset
                  << ", page size: " << slice->get_size();
    }
    if (res.ok()) {
