...
struct PredicateFilterEvaluator {
    enum Evaluator {
        ...
        PAGE_INDEX = 2,
        BLOOM_FILTER = 4,
        ALL = STATS_ZONEMAP | PAGE_INDEX | BLOOM_FILTER,
    };

    template <CompoundNodeType Type>
    StatusOr<std::optional<SparseRange<uint64_t>>> visit_for_zonemap(const PredicateCompoundNode<Type>& node) {
        ...
        for (const auto& child : node.compound_children()) {
            ASSIGN_OR_RETURN(auto cur_row_ranges_opt, child.visit(*this, Evaluator::STATS_ZONEMAP));
    ...
    StatusOr<std::optional<SparseRange<uint64_t>>> visit_for_page_index(const PredicateCompoundNode<Type>& node) {
        ...
        for (const auto& [cid, col_preds] : cid_to_col_preds) {
            ...
            if (column_reader == nullptr) {
                ...
                if (page_filtered) {
                    bool row_group_filtered = cur_row_ranges.span_size() == 0 ? true : false;
        ...
        for (const auto& child : node.compound_children()) {
            ASSIGN_OR_RETURN(auto cur_row_ranges_opt, child.visit(*this, Evaluator::PAGE_INDEX));
            if (cur_row_ranges_opt.has_value()) {
                PredicateFilterEvaluatorUtils::merge_row_ranges<Type>(row_ranges, cur_row_ranges_opt.value());
            }
        }
