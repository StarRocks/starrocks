...
Status read_encrypted_bloom_filter(RandomAccessFile* file, int64_t offset, const FileMetaData& file_metadata,
    ...
    // A serialized BloomFilterHeader is a handful of bytes; these are sanity bounds on
    // unauthenticated input, not tuning knobs.
    ...
    auto read_module = [&](int64_t module_offset, uint32_t max_size, int8_t module_type, std::string* plaintext,
        ...
        uint8_t len_buf[4];
        RETURN_IF_ERROR(file->read_at_fully(module_offset, len_buf, 4));
        ...
        RETURN_IF_ERROR(file->read_at_fully(module_offset, cipher.data(), total));
        RETURN_IF_ERROR(decrypt_metadata_module(file_metadata, encryption_info, module_type, row_group_ordinal,
                                                column_ordinal, cipher.data(), total, plaintext));
    ...
    RETURN_IF_ERROR(deserialize_thrift_msg(reinterpret_cast<const uint8_t*>(header_plain.data()), &header_plain_len,
    ...
    bloom_filter_data.push_back(has_null_byte);
...
// Defined outside the anonymous namespace above: this is the definition of the declaration in
// metadata.h, so the test can link it. Inside, it would be a second, internal-linkage overload and
// every call in this file becomes ambiguous. CompactThriftReader stays internal.
            ...
            if (fid == 1 && type == 12) { // encryption_algorithm union
                r.read_struct([&](int afid, uint8_t atype) {
                    if (atype == 12 && (afid == 1 || afid == 2)) {
                        out->algorithm = static_cast<int32_t>((afid == 1) ? ::parquet::ParquetCipher::AES_GCM_V1
...
Status FileMetaDataParser::_decrypt_and_deserialize_footer(const std::vector<char>& footer_buffer, uint32_t footer_len,
    ...
    try {
        ...
        file_decryptor = std::make_shared<::parquet::InternalFileDecryptor>(
                decryption_props, file_aad, static_cast<::parquet::ParquetCipher::type>(crypto_md.algorithm),
                key_metadata, ::arrow::default_memory_pool());
    ...
    *file_metadata_ptr = std::make_shared<FileMetaData>();
    ...
    enc_ctx.algorithm = crypto_md.algorithm;
