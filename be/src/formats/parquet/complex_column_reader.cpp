...
static Status _read_shredded_field_node(const Range<uint64_t>& range, const Filter* filter, ShreddedFieldNode* node) {
    ...
    if (node->value_reader != nullptr) {
        ...
        node->value_column = NullableColumn::create(allocator, BinaryColumn::create(allocator),
...
Status VariantColumnReader::read_range(const Range<uint64_t>& range, const Filter* filter, ColumnPtr& dst) {
    ...
    ColumnPtr metadata_col =
            NullableColumn::create(metadata_allocator, BinaryColumn::create(metadata_allocator),
    ...
    if (_top_level.root_typed_value_reader != nullptr) {
        ...
        _top_level.root_typed_value_column =
                ColumnHelper::create_column(_top_level.root_typed_value_reader->allocator(),
                                            *_top_level.root_typed_value_type, true);
        RETURN_IF_ERROR(
    ...
    for (const auto& binding : materialized_bindings) {
        ...
        typed_columns.emplace_back(ColumnHelper::create_column(_top_level.value_reader->allocator(), binding.type,
                                                               true));
    }
    variant_column->set_shredded_columns(
            std::move(typed_paths), std::move(typed_types), std::move(typed_columns),
