...
public:
    static int64_t max_serialized_size(const StructColumn& column) {
        int64_t size = 0;
    ...
    static uint8_t* serialize(const StructColumn& column, uint8_t* buff) {
        for (const auto& field : column.fields()) {
    ...
    static const uint8_t* deserialize(const uint8_t* buff, StructColumn* column) {
