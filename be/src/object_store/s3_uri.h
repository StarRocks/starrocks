...
class S3URI {
public:
    explicit S3URI(std::string uri) : _uri(std::move(uri)) {}
...
inline bool S3URI::parse() {
    auto pos0 = _uri.find(kSchemaDelim);
    if (pos0 == std::string::npos || pos0 == 0) {
        return false;
    }
    _schema = std::string_view(_uri.data(), pos0);

    // Move pos0 to the end of kSchemaDelim
    pos0 = pos0 + sizeof(kSchemaDelim) - 1;

    auto pos1 = _uri.find(kPathDelim, pos0);
    if (pos1 == std::string::npos || pos0 == pos1) {
        return false;
    }
    _bucket = std::string_view(_uri.data() + pos0, pos1 - pos0);

    pos1 = pos1 + sizeof(kPathDelim) - 1;
    _key = std::string_view(_uri.data() + pos1, _uri.size() - pos1);

    auto pos2 = _key.find(kQueryDelim);
    auto pos3 = _key.find(kFragmentDelim);
    if (pos2 != std::string::npos && pos3 != std::string::npos) {
        _key = std::string_view(_key.data(), std::min(pos2, pos3));
    } else if (pos2 != std::string::npos) {
        _key = std::string_view(_key.data(), pos2);
    } else if (pos3 != std::string::npos) {
        _key = std::string_view(_key.data(), pos3);
    }
    return true;
}
