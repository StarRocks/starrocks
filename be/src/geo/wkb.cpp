...
constexpr size_t kMaxInputBytes = 64 * 1024 * 1024;
...
public:
    ...
    Status parse(WkbGeometry* output) {
        if (_input.size() > kMaxInputBytes) {
            return invalid_wkt("input exceeds safety limit");
        }
...
private:
    ...
    Status consume_elements(size_t count) {
        if (count > kMaxElements - _elements) {
            return invalid_wkt("geometry exceeds element safety limit");
        }
        _elements += count;
        return Status::OK();
    }
    ...
    Status parse_number(double* output) {
        ...
        const char* begin = _input.data() + begin_position;
        const char* end = _input.data() + _position;
        if (*begin == '+') ++begin;
        double value = 0;
        auto [parsed_end, error] = std::from_chars(begin, end, value, std::chars_format::general);
        if (parsed_end != end || error != std::errc() || !std::isfinite(value)) {
            _position = begin_position;
            return invalid_wkt("invalid coordinate at byte " + std::to_string(_position));
        }
        *output = value;
...
public:
    ...
    Status parse(WkbGeometry* output) {
        if (_size > kMaxInputBytes) {
            return invalid_wkb("input exceeds safety limit");
        }
        RETURN_IF_ERROR(parse_geometry(0, output));
        if (_position != _size) {
...
private:
    ...
    Status parse_polygon(bool little_endian, WkbGeometry* output) {
        ...
        RETURN_IF_ERROR(consume_elements(ring_count));
    ...
    Status read_coordinates(bool little_endian, uint32_t count, std::vector<WkbCoordinate>* output) {
        RETURN_IF_ERROR(consume_elements(count));
