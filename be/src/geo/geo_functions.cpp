...
//   https://github.com/apache/incubator-doris/blob/master/be/src/geo/geo_functions.cpp
...
DoubleVal GeoFunctions::st_distance_sphere(FunctionContext* ctx, const DoubleVal& x_lng, const DoubleVal& x_lat,
...
starrocks_udf::StringVal GeoFunctions::st_point(starrocks_udf::FunctionContext* ctx, const starrocks_udf::DoubleVal& x,
...
DoubleVal GeoFunctions::st_x(starrocks_udf::FunctionContext* ctx, const starrocks_udf::StringVal& point_encoded) {
...
DoubleVal GeoFunctions::st_y(starrocks_udf::FunctionContext* ctx, const starrocks_udf::StringVal& point_encoded) {
    if (point_encoded.is_null) {
        return DoubleVal::null();
    }
    GeoPoint point;
...
StringVal GeoFunctions::st_as_wkt(starrocks_udf::FunctionContext* ctx, const starrocks_udf::StringVal& shape_encoded) {
...
void GeoFunctions::st_from_wkt_close(FunctionContext* ctx, FunctionContext::FunctionStateScope scope) {
...
void GeoFunctions::st_from_wkt_prepare_common(FunctionContext* ctx, FunctionContext::FunctionStateScope scope,
...
StringVal GeoFunctions::st_from_wkt_common(FunctionContext* ctx, const StringVal& wkt, GeoShapeType shape_type) {
    if (wkt.is_null) {
        return StringVal::null();
    }
...
void GeoFunctions::st_circle_prepare(starrocks_udf::FunctionContext* ctx,
...
starrocks_udf::StringVal GeoFunctions::st_circle(FunctionContext* ctx, const DoubleVal& lng, const DoubleVal& lat,
...
void GeoFunctions::st_contains_prepare(starrocks_udf::FunctionContext* ctx,
                                       starrocks_udf::FunctionContext::FunctionStateScope scope) {
    if (scope != FunctionContext::FRAGMENT_LOCAL) {
        return;
    }
...
void GeoFunctions::st_contains_close(starrocks_udf::FunctionContext* ctx,
                                     starrocks_udf::FunctionContext::FunctionStateScope scope) {
    if (scope != FunctionContext::FRAGMENT_LOCAL) {
        return;
    }
...
starrocks_udf::BooleanVal GeoFunctions::st_contains(starrocks_udf::FunctionContext* ctx,
    ...
    const StContainsState* state =
            reinterpret_cast<StContainsState*>(ctx->get_function_state(FunctionContext::FRAGMENT_LOCAL));
