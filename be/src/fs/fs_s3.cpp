...
// Get an AWSCredentialsProvider based on CloudCredential
    ...
    S3ClientFactory::S3ClientPtr S3ClientFactory::new_client(const TCloudConfiguration &t_cloud_configuration) {
        const AWSCloudConfiguration aws_cloud_configuration = CloudConfigurationFactory::create_aws(
                t_cloud_configuration);
        ...
        S3ClientPtr client = std::make_shared<Aws::S3::S3Client>(
                credential_provider, config, Aws::Client::AWSAuthV4Signer::PayloadSigningPolicy::Never,
                !path_style_access);
    ...
    S3ClientFactory::S3ClientPtr S3ClientFactory::new_client(const ClientConfiguration &config, const FSOptions &opts) {
        ...
        if (!access_key_id.empty() && !secret_access_key.empty()) {
            auto credentials = std::make_shared<Aws::Auth::SimpleAWSCredentialsProvider>(access_key_id,
                                                                                         secret_access_key);
...
// If you find yourself change this code, see also `bool operator==(const Aws::Client::ClientConfiguration&, const Aws::Client::ClientConfiguration&)`
    ...
    public:
        ...
        Status path_exists(const std::string &path) override {
            return Status::NotSupported("S3FileSystem::path_exists");
        }
    ...
    S3FileSystem::new_random_access_file(const RandomAccessFileOptions &opts,
        ...
        do {
            ...
            if (!outcome.IsSuccess()) {
                return Status::IOError(
                        fmt::format("S3: fail to list {}: {}", dirname, outcome.GetError().GetMessage()));
            }
            result = outcome.GetResultWithOwnership();
            directory_exist |= !result.GetContents().empty();
            ...
            if (!objects.empty()) {
                ...
                if (!delete_outcome.IsSuccess()) {
                    return Status::IOError(
                            fmt::format("fail to batch delete {}: {}", dirname,
