...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/bufferpool/buffer_pool.cc
...
BufferPool::BufferPool(int64_t min_buffer_len, int64_t buffer_bytes_limit, int64_t clean_page_bytes_limit)
        : allocator_(new BufferAllocator(this, min_buffer_len, buffer_bytes_limit, clean_page_bytes_limit)),
...
Status BufferPool::ExtractBuffer(ClientHandle* client, PageHandle* page_handle, BufferHandle* buffer_handle) {
...
Status BufferPool::TransferBuffer(ClientHandle* src_client, BufferHandle* src, ClientHandle* dst_client,
                                  BufferHandle* dst) {
    DCHECK(src->is_open());
    DCHECK(!dst->is_open());
...
BufferPool::Client::Client(BufferPool* pool, //TmpFileMgr::FileGroup* file_group,
    ...
    reservation_.InitChildTracker(child_profile, parent_reservation, mem_tracker, reservation_limit);
    counters_.alloc_time = ADD_TIMER(child_profile, "AllocTime");
    counters_.cumulative_allocations = ADD_COUNTER(child_profile, "CumulativeAllocations", TUnit::UNIT);
    counters_.cumulative_bytes_alloced = ADD_COUNTER(child_profile, "CumulativeAllocationBytes", TUnit::BYTES);
    counters_.peak_unpinned_bytes = child_profile->AddHighWaterMarkCounter("PeakUnpinnedBytes", TUnit::BYTES);
}
...
Status BufferPool::Client::DecreaseReservationTo(int64_t target_bytes) {
    ...
    int64_t amount_to_free = std::min(reservation_.GetUnusedReservation(), current_reservation - target_bytes);
...
string BufferPool::Client::DebugString() {
    std::lock_guard<std::mutex> lock(lock_);
    std::stringstream ss;
    ss << "<BufferPool::Client> " << this << " name: " << name_ << " write_status: " << write_status_.get_error_msg()
       << " buffers allocated " << buffers_allocated_bytes_ << " num_pages: " << num_pages_
       << " pinned_bytes: " << pinned_pages_.bytes() << " dirty_unpinned_bytes: " << dirty_unpinned_pages_.bytes()
...
string BufferPool::BufferHandle::DebugString() const {
    std::stringstream ss;
    if (is_open()) {
        ss << "<BufferPool::BufferHandle> " << this << " client: " << client_ << "/" << client_->impl_
           << " data: " << data_ << " len: " << len_;
    } else {
