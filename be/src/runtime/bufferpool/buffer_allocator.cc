...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/bufferpool/buffer_allocator.cc
...
*/
    ...
    std::pair<int64_t, int64_t> FreeSystemMemory(int64_t target_bytes_to_free, int64_t target_bytes_to_claim,
...
private:
    ...
    struct PerSizeLists {
        ...
        std::atomic<int64_t> num_free_buffers{0};
        ...
        std::atomic<int64_t> num_clean_pages{0};
    ...
    int NumBufferSizes() const { return parent_->log_max_buffer_len_ - parent_->log_min_buffer_len_ + 1; }
...
int64_t BufferPool::BufferAllocator::CalcMaxBufferLen(int64_t min_buffer_len, int64_t system_bytes_limit) {
    // Find largest power of 2 smaller than 'system_bytes_limit'.
    int64_t upper_bound = system_bytes_limit == 0 ? 1L : 1L << BitUtil::Log2Floor64(system_bytes_limit);
    upper_bound = std::min(MAX_BUFFER_BYTES, upper_bound);
    return std::max(min_buffer_len, upper_bound); // Can't be < min_buffer_len.
}

BufferPool::BufferAllocator::BufferAllocator(BufferPool* pool, int64_t min_buffer_len, int64_t system_bytes_limit,
...
Status BufferPool::BufferAllocator::Allocate(ClientHandle* client, int64_t len, BufferHandle* buffer) {
...
*/
        ...
        if (delta < len) {
            ...
            std::stringstream err_stream;
            err_stream << "Could not allocate : " << len << "bytes: was only able to free up " << delta
                       << " bytes after " << max_scavenge_attempts_ << " attempts:\n"
                       << pool_->DebugString();
            return Status::InternalError(err_stream.str());
        }
    }
...
int64_t DecreaseBytesRemaining(int64_t max_decrease, bool require_full_decrease,
    ...
    while (true) {
        ...
        if (bytes_remaining->compare_exchange_strong(old_value, new_value, std::memory_order_release)) {
            return decrease;
        }
    }
}

int64_t BufferPool::BufferAllocator::ScavengeBuffers(bool slow_but_sure, int current_core, int64_t target_bytes) {
    ...
    for (int i = 0; i < per_core_arenas_.size(); ++i) {
        ...
        bytes_found +=
                arena->FreeSystemMemory(bytes_needed, bytes_needed, slow_but_sure ? &arena_locks[i] : nullptr).second;
...
void BufferPool::BufferAllocator::AddCleanPage(const std::unique_lock<std::mutex>& client_lock, Page* page) {
...
bool BufferPool::BufferAllocator::RemoveCleanPage(const std::unique_lock<std::mutex>& client_lock, bool claim_buffer,
                                                  Page* page) {
    page->client->DCheckHoldsLock(client_lock);
...
std::string BufferPool::BufferAllocator::DebugString() {
    ...
    ss << "<BufferAllocator> " << this << " min_buffer_len: " << min_buffer_len_
       ...
       << " clean_page_bytes_remaining: " << clean_page_bytes_remaining_.load(std::memory_order_acquire) << "\n";
    for (int i = 0; i < per_core_arenas_.size(); ++i) {
...
bool BufferPool::FreeBufferArena::PopFreeBuffer(int64_t buffer_len, BufferHandle* buffer) {
    ...
    buffer->Unpoison();
    lists->num_free_buffers.fetch_add(-1, std::memory_order_release);
...
*/
std::pair<int64_t, int64_t> BufferPool::FreeBufferArena::FreeSystemMemory(int64_t target_bytes_to_free,
    ...
    for (int i = NumBufferSizes() - 1; i >= 0; --i) {
        ...
        int64_t buffers_to_free =
                std::min(free_buffers->Size(), BitUtil::Ceil(target_bytes_to_free - bytes_freed, buffer_len));
        ...
        lists->num_clean_pages.fetch_add(-num_pages_evicted, std::memory_order_release);
        parent_->clean_page_bytes_remaining_.fetch_add(page_bytes_evicted, std::memory_order_release);

        if (buffers_to_free > 0) {
            int64_t buffer_bytes_freed = parent_->FreeToSystem(free_buffers->GetBuffersToFree(buffers_to_free));
            DCHECK_EQ(buffer_bytes_to_free, buffer_bytes_freed);
            bytes_freed += buffer_bytes_to_free;
            lists->num_free_buffers.fetch_add(-buffers_to_free, std::memory_order_release);
    ...
    if (bytes_freed > bytes_claimed) {
        ...
        parent_->system_bytes_remaining_.fetch_add(bytes_freed - bytes_claimed, std::memory_order_release);
    }
    ...
    for (int i = 0; i < NumBufferSizes(); ++i) {
        ...
        if (lists->low_water_mark != 0) {
            ...
            parent_->system_bytes_remaining_.fetch_add(
                    ...
                    std::memory_order_release);
            lists->num_free_buffers.fetch_add(-num_to_free, std::memory_order_release);
        }
