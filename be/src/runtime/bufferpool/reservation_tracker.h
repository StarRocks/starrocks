...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/bufferpool/reservation_tracker.h
...
public:
    ...
    void InitChildTracker(RuntimeProfile* profile, ReservationTracker* parent, MemTracker* mem_tracker,
    ...
    void SetDebugDenyIncreaseReservation(double probability) { increase_deny_probability_ = probability; }
...
private:
    ...
    inline int64_t unused_reservation() const { return reservation_ - used_reservation_ - child_reservations_; }

    /// Returns the parent's memtracker if 'parent_' is non-NULL, or NULL otherwise.
    MemTracker* GetParentMemTracker() const { return parent_ == nullptr ? nullptr : parent_->mem_tracker_; }
    ...
    bool IncreaseReservationInternalLocked(int64_t bytes, bool use_existing_reservation, bool is_child_reservation);
    ...
    static bool lock_sibling_subtree_first(ReservationTracker* subtree1, ReservationTracker* subtree2) {
