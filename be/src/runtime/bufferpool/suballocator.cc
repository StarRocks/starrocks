...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/bufferpool/suballocator.cc
...
void Suballocator::Free(std::unique_ptr<Suballocation> allocation) {
...
std::unique_ptr<Suballocation> Suballocator::CoalesceBuddies(std::unique_ptr<Suballocation> b1,
    ...
    std::unique_ptr<Suballocation> parent = b1->parent_ != nullptr ? std::move(b1->parent_) : std::move(b2->parent_);
