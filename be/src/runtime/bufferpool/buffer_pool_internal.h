...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/bufferpool/buffer_pool_internal.h
...
#ifndef STARROCKS_BE_RUNTIME_BUFFER_POOL_INTERNAL_H
#define STARROCKS_BE_RUNTIME_BUFFER_POOL_INTERNAL_H
...
public:
    Page(Client* client, int64_t len) : client(client), len(len), pin_count(0), pin_in_flight(false) {}
...
private:
    ...
    void DCheckConsistency() {
        ...
        DCHECK_LE(pinned_pages_.size() + dirty_unpinned_pages_.size() + in_flight_write_pages_.size(), num_pages_);
