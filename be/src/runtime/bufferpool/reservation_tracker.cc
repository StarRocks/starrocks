...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/bufferpool/reservation_tracker.cc
...
void ReservationTracker::InitChildTracker(RuntimeProfile* profile, ReservationTracker* parent, MemTracker* mem_tracker,
...
void ReservationTracker::InitCounters(RuntimeProfile* profile, int64_t reservation_limit) {
    ...
    counters_.peak_used_reservation = profile->AddHighWaterMarkCounter("PeakUsedReservation", TUnit::BYTES);
...
bool ReservationTracker::IncreaseReservationInternalLocked(int64_t bytes, bool use_existing_reservation,
    ...
    if (reservation_increase == 0) {
        granted = true;
    } else if (increase_deny_probability_ != 0.0 && rand() < increase_deny_probability_ * (RAND_MAX + 1L)) {
...
std::string ReservationTracker::DebugString() {
    ...
    std::stringstream ss;
    ss << "<ReservationTracker>: reservation_limit " << reservation_limit_ << " reservation " << reservation_
       << " used_reservation " << used_reservation_ << " child_reservations " << child_reservations_ << " parent:\n"
       << parent_debug_string;
    return ss.str();
}
} // namespace starrocks
