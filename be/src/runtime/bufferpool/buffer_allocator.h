...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/bufferpool/buffer_allocator.h
...
#ifndef STARROCKS_BE_RUNTIME_BUFFER_ALLOCATOR_H
#define STARROCKS_BE_RUNTIME_BUFFER_ALLOCATOR_H
...
struct BufferPool::BufferAllocator {
    ...
    Status Allocate(ClientHandle* client, int64_t len, BufferPool::BufferHandle* buffer) WARN_UNUSED_RESULT;
    ...
    bool RemoveCleanPage(const std::unique_lock<std::mutex>& client_lock, bool claim_buffer, Page* page);
    ...
    int64_t GetSystemBytesAllocated() const { return system_bytes_limit_ - system_bytes_remaining_.load(); }
...
private:
    ...
    std::atomic<int64_t> system_bytes_remaining_{0};
    ...
    std::atomic<int64_t> clean_page_bytes_remaining_{0};
