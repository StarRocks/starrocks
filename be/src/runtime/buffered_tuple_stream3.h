...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/buffered_tuple_stream3.h
...
#ifndef STARROCKS_BE_RUNTIME_BUFFERED_TUPLE_STREAM_H
#define STARROCKS_BE_RUNTIME_BUFFERED_TUPLE_STREAM_H
...
public:
    ...
    BufferedTupleStream3(RuntimeState* state, const RowDescriptor* row_desc,
                         BufferPool::ClientHandle* buffer_pool_client, int64_t default_page_len, int64_t max_page_len,
    ...
    Status GetNext(RowBatch* batch, bool* eos, std::vector<FlatRowPtr>* flat_rows) WARN_UNUSED_RESULT;
    ...
    /// to false.
    Status GetRows(MemTracker* tracker, std::unique_ptr<RowBatch>* batch, bool* got_rows) WARN_UNUSED_RESULT;
...
private:
    ...
    bool is_read_page(const Page* page) const { return read_page_ != pages_.end() && &*read_page_ == page; }
    ...
    static bool CopyStrings(const Tuple* tuple, const std::vector<SlotDescriptor*>& string_slots, uint8_t** data,
    ...
    /// read iterator state.
    bool NeedReadReservation(bool stream_pinned, int64_t num_pages, bool has_read_iterator, bool has_read_page) const;

    /// Same as above, except assume the stream has 'num_pages' pages and a different
    /// write iterator state.
    static bool NeedReadReservation(bool stream_pinned, int64_t num_pages, bool has_read_iterator, bool has_read_page,
