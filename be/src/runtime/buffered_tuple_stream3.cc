...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/buffered_tuple_stream3.cc
...
BufferedTupleStream3::BufferedTupleStream3(RuntimeState* state, const RowDescriptor* row_desc,
                                           ...
                                           int64_t max_page_len, const std::set<SlotId>& ext_varlen_slots)
        : state_(state),
          desc_(row_desc),
          node_id_(-1),
...
string BufferedTupleStream3::DebugString() const {
    std::stringstream ss;
    ss << "BufferedTupleStream3 num_rows=" << num_rows_ << " rows_returned=" << rows_returned_ << " pinned=" << pinned_
       << " delete_on_read=" << delete_on_read_ << " closed=" << closed_ << "\n"
       << " bytes_pinned=" << bytes_pinned_ << " has_write_iterator=" << has_write_iterator_
       << " write_page=" << write_page_ << " has_read_iterator=" << has_read_iterator_ << " read_page=";
    if (read_page_ == pages_.end()) {
        ss << "<end>";
    } else {
        ss << &*read_page_;
...
bool BufferedTupleStream3::NeedWriteReservation(bool stream_pinned) const {
    return NeedWriteReservation(stream_pinned, num_pages_, has_write_iterator(), write_page_ != nullptr,
                                has_read_write_page());
}

bool BufferedTupleStream3::NeedWriteReservation(bool stream_pinned, int64_t num_pages, bool has_write_iterator,
...
bool BufferedTupleStream3::NeedReadReservation(bool stream_pinned) const {
    return NeedReadReservation(stream_pinned, num_pages_, has_read_iterator(), read_page_ != pages_.end());
}

bool BufferedTupleStream3::NeedReadReservation(bool stream_pinned, int64_t num_pages, bool has_read_iterator,
                                               bool has_read_page) const {
    return NeedReadReservation(stream_pinned, num_pages, has_read_iterator, has_read_page, has_write_iterator(),
                               write_page_ != nullptr);
}

bool BufferedTupleStream3::NeedReadReservation(bool stream_pinned, int64_t num_pages, bool has_read_iterator,
                                               bool has_read_page, bool has_write_iterator, bool has_write_page) {
...
Status BufferedTupleStream3::NewWritePage(int64_t page_len) noexcept {
    ...
    Page new_page;
    const BufferHandle* write_buffer;
    RETURN_IF_ERROR(buffer_pool_->CreatePage(buffer_pool_client_, page_len, &new_page.handle, &write_buffer));
...
Status BufferedTupleStream3::AdvanceWritePage(int64_t row_size, bool* got_reservation) noexcept {
    ...
    if (NeedWriteReservation(pinned_, num_pages_, true, write_page_ != nullptr, has_read_write_page()) &&
    ...
    if (NeedReadReservation(pinned_, num_pages_, has_read_iterator(), read_page_ != pages_.end(), true,
                            write_page_ != nullptr) &&
        !NeedReadReservation(pinned_, num_pages_ + 1, has_read_iterator(), read_page_ != pages_.end(), true, true)) {
    ...
    if (write_reservation_to_restore > 0) {
        buffer_pool_client_->RestoreReservation(&write_page_reservation_, write_reservation_to_restore);
    }
    if (read_reservation_to_restore > 0) {
        buffer_pool_client_->RestoreReservation(&read_page_reservation_, read_reservation_to_restore);
    }
    ResetWritePage();
...
void BufferedTupleStream3::InvalidateWriteIterator() {
    ...
    if (NeedReadReservation(pinned_, num_pages_, has_read_iterator(), read_page_ != pages_.end(), true,
                            write_page_ != nullptr) &&
        !NeedReadReservation(pinned_, num_pages_, has_read_iterator(), read_page_ != pages_.end(), false, false)) {
...
Status BufferedTupleStream3::NextReadPage() {
    ...
    if (!pinned_ && read_page_->len() > default_page_len_ &&
        ...
        err_stream << "Internal error: couldn't pin large page of " << read_page_->len() << " bytes, client only had "
    ...
    if (!NeedWriteReservation(pinned_, num_pages_, has_write_iterator(), write_page_ != nullptr, false) &&
...
Status BufferedTupleStream3::PrepareForRead(bool delete_on_read, bool* got_reservation) {
    ...
    *got_reservation = pinned_ || pages_.empty() || buffer_pool_client_->IncreaseReservationToFit(default_page_len_);
    if (!*got_reservation) return Status::OK();
...
*/
Status BufferedTupleStream3::GetRows(MemTracker* tracker, std::unique_ptr<RowBatch>* batch, bool* got_rows) {
...
Status BufferedTupleStream3::GetNextInternal(RowBatch* batch, bool* eos, vector<FlatRowPtr>* flat_rows) {
...
void BufferedTupleStream3::FixUpStringsForRead(const vector<SlotDescriptor*>& string_slots, Tuple* tuple) {
...
bool BufferedTupleStream3::DeepCopy(TupleRow* row, uint8_t** data, const uint8_t* data_end) noexcept {
...
bool BufferedTupleStream3::DeepCopyInternal(TupleRow* row, uint8_t** data, const uint8_t* data_end) noexcept {
    uint8_t* pos = *data;
    const uint64_t tuples_per_row = desc_->tuple_descriptors().size();
...
bool BufferedTupleStream3::CopyStrings(const Tuple* tuple, const vector<SlotDescriptor*>& string_slots, uint8_t** data,
...
void BufferedTupleStream3::GetTupleRow(FlatRowPtr flat_row, TupleRow* row) const {
    ...
    return has_nullable_tuple_ ? UnflattenTupleRow<true>(&data, row) : UnflattenTupleRow<false>(&data, row);
}
...
void BufferedTupleStream3::UnflattenTupleRow(uint8_t** data, TupleRow* row) const {
    ...
    if (has_nullable_tuple_) {
        ...
        for (int i = 0; i < tuples_per_row; ++i) {
            ...
            row->set_tuple(i, reinterpret_cast<Tuple*>(reinterpret_cast<uint64_t>(ptr) * is_not_null));
