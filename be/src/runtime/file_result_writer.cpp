...
Status FileResultWriter::_create_file_writer() {
    ...
    if (_file_opts->is_local_file) {
        ...
        _file_writer = new BrokerWriter(_state->exec_env(), _file_opts->broker_addresses, _file_opts->broker_properties,
                                        file_name, 0 /*start offset*/);
    }
...
Status FileResultWriter::_write_one_row_as_csv(TupleRow* row) {
    {
        ...
        _plain_text_outstream << _file_opts->row_delimiter;
    }
...
Status FileResultWriter::_flush_plain_text_outstream(bool eos) {
    ...
    RETURN_IF_ERROR(_file_writer->write(reinterpret_cast<const uint8_t*>(buf.c_str()), buf.size(), &written_len));
