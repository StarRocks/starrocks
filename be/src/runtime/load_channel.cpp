...
LoadChannel::LoadChannel(LoadChannelMgr* mgr, const UniqueId& load_id, int64_t timeout_s, MemTracker* mem_tracker,
                         Ownership mem_tracker_ownership)
        : _load_mgr(mgr),
          _load_id(load_id),
          _timeout_s(timeout_s),
          _mem_tracker(mem_tracker),
          _mem_tracker_ownership(mem_tracker_ownership),
          _last_update_time(time(nullptr)) {}
...
LoadChannel::~LoadChannel() {
    ...
    if (_mem_tracker_ownership == kTakesOwnership) {
        delete _mem_tracker;
    }
}
...
void LoadChannel::open(brpc::Controller* cntl, const PTabletWriterOpenRequest* request,
    ...
    _last_update_time.store(time(nullptr), std::memory_order_relaxed);
    ...
    Status st;
    {
        ...
        if (_tablets_channels.find(index_id) == _tablets_channels.end()) {
            ...
            scoped_refptr<TabletsChannel> channel(new TabletsChannel(this, key, _mem_tracker));
