...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/export_task_mgr.cpp
...
ExportTaskMgr::ExportTaskMgr(ExecEnv* exec_env)
        : _exec_env(exec_env), _success_tasks(LRU_MAX_CASH_TASK_NUM), _failed_tasks(LRU_MAX_CASH_TASK_NUM) {}
...
Status ExportTaskMgr::start_task(const TExportTaskRequest& request) {
    ...
    RETURN_IF_ERROR(_exec_env->fragment_mgr()->exec_plan_fragment(
            request.params, std::bind<void>(&ExportTaskMgr::finalize_task, this, std::placeholders::_1)));
...
Status ExportTaskMgr::finish_task(const TUniqueId& id, const Status& status, const ExportTaskResult& result) {
    std::lock_guard<std::mutex> l(_lock);
    auto it = _running_tasks.find(id);
    if (it == _running_tasks.end()) {
        std::stringstream ss;
    ...
    VLOG_EXPORT << "Move task(" << id << ") from running to " << (status.ok() ? "success tasks" : "failed tasks");
...
void ExportTaskMgr::report_to_master(PlanFragmentExecutor* executor) {
    ...
    FrontendServiceConnection client(_exec_env->frontend_client_cache(), master_address, config::thrift_rpc_timeout_ms,
                                     &status);
    if (!status.ok()) {
        std::stringstream ss;
    ...
    VLOG_ROW << "export updateExportTaskStatus. request  is " << apache::thrift::ThriftDebugString(request).c_str();
    ...
    try {
        ...
        std::stringstream ss;
        ss << "Fail to report export task to master(" << master_address.hostname << ":" << master_address.port
           << "). reason: " << e.what();
        LOG(WARNING) << ss.str();
    }
