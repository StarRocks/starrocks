...
>>>>>>> 91a4bc69e1 ([Enhancement] Support write iceberg row lineage fields into data files during compaction (#70128))
...
#include "util/runtime_profile.h"
    ...
    Status IcebergTableSink::send_chunk(RuntimeState * state, Chunk * chunk) { return Status::OK(); }
    ...
    Status IcebergTableSink::create_delete_sink_context(
        ...
        if (slots.size() != output_exprs.size()) {
            return Status::InternalError(fmt::format("Mismatched slot and output expression counts: {} vs {}",
