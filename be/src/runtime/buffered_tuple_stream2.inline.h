...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/buffered_tuple_stream2.inline.h
...
#ifndef STARROCKS_BE_SRC_RUNTIME_BUFFERED_TUPLE_STREAM2_INLINE_H
#define STARROCKS_BE_SRC_RUNTIME_BUFFERED_TUPLE_STREAM2_INLINE_H
...
inline void BufferedTupleStream2::get_tuple_row(const RowIdx& idx, TupleRow* row) const {
    ...
    if (_nullable_tuple) {
        ...
        for (int i = 0; i < tuples_per_row; ++i) {
            ...
            row->set_tuple(i, reinterpret_cast<Tuple*>(reinterpret_cast<uint64_t>(data) * is_not_null));
