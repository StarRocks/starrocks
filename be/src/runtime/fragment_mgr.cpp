...
void FragmentMgr::report_fragments_with_same_host(const std::vector<std::shared_ptr<FragmentExecState>>& need_report_exec_states,
...
void FragmentMgr::report_fragments(const std::vector<TUniqueId>& non_pipeline_need_report_fragment_ids) {
    ...
    for (int i = 0; i < need_report_exec_states.size(); i++) {
        if (reported[i] == false) {
            ...
            if (!fe_connection_status.ok()) {
                ...
                LOG(WARNING) << ss.str();
                fragment_exec_state->exec_env()->frontend_client_cache()->close_connections(fragment_exec_state->coord_addr());
                continue;
            }
            ...
            report_exec_status_params_vector.push_back(params);
            // todo: delete me
            LOG(WARNING) << "report fragment_instance_id" << fragment_exec_state->fragment_instance_id();

            std::vector<int32_t> cur_batch_report_indexes;
            cur_batch_report_indexes.push_back(i);

            report_fragments_with_same_host(need_report_exec_states, reported, fragment_exec_state->coord_addr(), report_exec_status_params_vector, cur_batch_report_indexes);
            ...
            VLOG_ROW << "debug: reportExecStatus params is " << apache::thrift::ThriftDebugString(params).c_str();
            // todo: delete me
            LOG(WARNING) << "debug: reportExecStatus params is " << apache::thrift::ThriftDebugString(params).c_str();
            LOG(WARNING) << "debug: coord id " << fragment_exec_state->coord_addr();
            try {
                try {
    ...
    for (const auto& fragment_instance_id: fragments_non_exist) {
        starrocks::StorageEngine::instance()->unregister_non_pipeline_load(fragment_instance_id);
    }
}
