...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/dpp_sink_internal.cpp
...
Status PartRange::from_thrift(ObjectPool* pool, const TPartitionRange& t_part_range, PartRange* range) {
...
Status PartitionInfo::from_thrift(ObjectPool* pool, const TRangePartition& t_partition, PartitionInfo* partition,
    ...
    if (t_partition.__isset.distributed_exprs) {
        ...
        RETURN_IF_ERROR(
                Expr::create_expr_trees(pool, t_partition.distributed_exprs, &partition->_distributed_expr_ctxs));
    }
    return Status::OK();
}
