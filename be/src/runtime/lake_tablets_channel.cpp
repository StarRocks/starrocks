...
void LakeTabletsChannel::add_chunk(Chunk* chunk, const PTabletWriterAddChunkRequest& request,
    ...
    if (request.eos()) {
        // flush shared data file at last.
        auto st = _shared_writable_file_context->close();
        if (!st.ok()) {
            context->update_status(st);
            LOG(WARNING) << "Fail to close shared data file: " << st
                         << ", file: " << _shared_writable_file_context->shared_file->filename();
        }
    }
