...
void LoadChannelMgr::open(brpc::Controller* cntl, const PTabletWriterOpenRequest* request,
    ...
    scoped_refptr<LoadChannel> channel;
    {
        ...
        if (it != _load_channels.end()) {
            ...
            int64_t timeout_s = calc_job_timeout_s(request->has_load_channel_timeout_s());
            if (request->load_mem_limit() > 0 && request->load_mem_limit() < _mem_tracker->limit()) {
                auto mem_tracker = new MemTracker(request->load_mem_limit(), load_id.to_string(), _mem_tracker, true);
                channel.reset(new LoadChannel(this, load_id, timeout_s, mem_tracker, kTakesOwnership));
            } else {
                channel.reset(new LoadChannel(this, load_id, timeout_s, _mem_tracker, kDontTakeOwnership));
            }
...
#ifndef BE_TEST
        constexpr long interval = 60;
#else
        constexpr long interval = 1;
#endif
...
Status LoadChannelMgr::_start_load_channels_clean() {
    ...
    time_t now = time(nullptr);
    {
        ...
        for (auto it = _load_channels.begin(); it != _load_channels.end(); /**/) {
            if (difftime(now, it->second->last_update_time()) >= it->second->timeout()) {
