...
class ProcessMemoryMetricValues {
public:
...
public:
    ...
    // Install process memory metrics to registry.
    void install(MetricRegistry* registry);

    // Update process memory metrics.
    void update();

    const ProcessMemoryMetricValues* memory_metrics() const { return _memory_metrics.get(); }
...
private:
    ...
    std::unique_ptr<ProcessMemoryMetricValues> _memory_metrics;
