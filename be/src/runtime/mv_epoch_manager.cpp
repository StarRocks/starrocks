...
Status MVEpochManager::update_epoch(const TUniqueId& fragment_instance_id, const EpochInfo& epoch_info,
...
Status MVEpochManager::update_binlog_offset(int64_t scan_node_id, int64_t tablet_id, BinlogOffset binlog_offset) {
    std::unique_lock<std::shared_mutex> l(_epoch_lock);
...
const BinlogOffset* MVEpochManager::get_binlog_offset(int64_t scan_node_id, int64_t tablet_id) const {
...
const TUniqueId& MVEpochManager::fragment_instance_id() const {
    std::shared_lock<std::shared_mutex> l(_epoch_lock);
    return _fragment_instance_id;
}

const EpochInfo& MVEpochManager::epoch_info() const {
    std::shared_lock<std::shared_mutex> l(_epoch_lock);
    return _epoch_info;
}

const std::unordered_map<int64_t, TabletId2BinlogOffset>& MVEpochManager::node_id_to_scan_ranges() const {
    std::shared_lock<std::shared_mutex> l(_epoch_lock);
    return _node_id_to_scan_ranges;
}

const BinlogOffset* MVEpochManager::_get_epoch_unlock(const TabletId2BinlogOffset& tablet_id_scan_ranges_mapping,
