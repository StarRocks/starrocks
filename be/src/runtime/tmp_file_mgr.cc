...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/tmp_file_mgr.cc
...
const std::string _s_tmp_sub_dir_name = "starrocks-scratch";
...
Status TmpFileMgr::init_custom(const vector<string>& tmp_dirs, bool one_dir_per_device, MetricRegistry* metrics) {
    DCHECK(!_initialized);
    if (tmp_dirs.empty()) {
    ...
    for (const auto& tmp_dir : tmp_dirs) {
        ...
        if (!status.ok()) {
            LOG(WARNING) << "Cannot use directory " << tmp_path.string() << " for scratch: " << status.get_error_msg();
            continue;
        }
        ...
        if (!one_dir_per_device || disk_id < 0 || !is_tmp_dir_on_disk[disk_id]) {
            ...
            RETURN_IF_ERROR(FileSystemUtil::get_space_available(tmp_path.string(), &available_space));
            if (available_space < _s_available_space_threshold_mb * 1024 * 1024) {
                LOG(WARNING) << "Filesystem containing scratch directory " << tmp_path << " has less than "
                             << _s_available_space_threshold_mb << "MB available.";
            }
            ...
            if (status.ok()) {
                ...
                LOG(INFO) << "Using scratch directory " << scratch_subdir_path.string() << " on disk " << disk_id;
                _tmp_dirs.push_back(Dir(scratch_subdir_path.string(), false));
            } else {
                LOG(WARNING) << "Could not remove and recreate directory " << scratch_subdir_path.string()
...
Status TmpFileMgr::get_file(const DeviceId& device_id, const TUniqueId& query_id, File** new_file) {
    ...
    file_name << std::hex << query_id.hi << '-' << query_id.lo << std::dec << "_" << unique_name;
    std::filesystem::path new_file_path(_tmp_dirs[device_id].path());
