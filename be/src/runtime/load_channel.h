...
public:
    LoadChannel(LoadChannelMgr* mgr, const UniqueId& load_id, int64_t timeout_s, MemTracker* mem_tracker,
                Ownership mem_tracker_ownership);
    ...
    time_t last_update_time() const { return _last_update_time.load(std::memory_order_relaxed); }
...
private:
    ...
    Ownership _mem_tracker_ownership;
...
inline std::ostream& operator<<(std::ostream& os, const LoadChannel& load_channel) {
    os << "LoadChannel(id=" << load_channel.load_id()
       << ", last_update_time=" << static_cast<uint64_t>(load_channel.last_update_time()) << ")";
    return os;
}
