...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/decimal_value.h
...
static const int32_t powers10[DIG_PER_DEC1 + 1] = {1,      10,      100,      1000,      10000,
...
public:
    ...
    friend int do_div_mod(const DecimalValue& value1, const DecimalValue& value2, DecimalValue* to, DecimalValue* mod);
    ...
    uint32_t hash_uint(uint32_t value, uint32_t seed) const { return HashUtil::hash(&value, sizeof(value), seed); }
...
namespace std {
template <>
struct hash<starrocks::DecimalValue> {
