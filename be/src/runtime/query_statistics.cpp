...
void QueryStatistics:: update_exec_stats_item(uint32_t node_id, int64_t push, int64_t pull, int64_t pred_filter, int64_t index_filter,
                                              int64_t rf_filter) {
    auto iter = _exec_stats_items.find(node_id);
    if (iter == _exec_stats_items.end()) {
        _exec_stats_items.insert({node_id, std::make_shared<NodeExecStats>(push, pull,
                                                                           pred_filter,
                                                                           index_filter,
                                                                           rf_filter)});
    } else {
...

    {
        ...
        for (const auto& [node_id, exec_stats_item] : other_exec_stats_items) {
            update_exec_stats_item(node_id, exec_stats_item->push_rows, exec_stats_item->pull_rows,
                                   ...
                                   exec_stats_item->index_filter_rows, exec_stats_item->rf_filter_rows);
        }
    }
}
