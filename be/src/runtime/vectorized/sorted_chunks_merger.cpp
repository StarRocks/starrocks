...
Status SortedChunksMerger::get_next(ChunkPtr* chunk, bool* eos) {
    ...
    *chunk = cursor->clone_empty_chunk(_state->batch_size());
    ...
    while (row_number < _state->batch_size() && !_min_heap.empty()) {
        cursor = _min_heap[0];
    ...
    *chunk = std::make_shared<Chunk>();
    ...
    for (;;) {
        ...
        if (_wait_for_data) {
            ...
            if (_row_number >= _state->batch_size() || _min_heap.empty()) {
        ...
        _cursor = _min_heap[0];
        if (!_row_number) {
            _result_chunk = _cursor->clone_empty_chunk(_state->batch_size());
            _current_chunk = _cursor->get_current_chunk();
            _selective_values.clear();
            _selective_values.reserve(_state->batch_size());
