...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/spill_sorter.cc
...
private:
    ...
    void collect_non_null_varslots(Tuple* src, vector<StringValue*>* var_len_values, int* total_var_len);
    ...
    // StringValue.
    void copy_var_len_data_convert_offset(char* dest, int64_t offset, const vector<StringValue*>& var_values);

    // Returns true if we have var-len slots and there are var-len blocks.
    inline bool has_var_len_blocks() const { return _has_var_len_slots && !_var_len_blocks.empty(); }
    ...
    int64_t _num_tuples_returned = 0;
    ...
    int _fixed_len_blocks_index = 0;
    int _var_len_blocks_index = 0;
    ...
    bool _pin_next_fixed_len_block = false;
    bool _pin_next_var_len_block = false;

    // Offset into the current fixed length data block being processed.
    int _fixed_len_block_offset = 0;
}; // class SpillSorter::Run
...
private:
    ...
    class TupleIterator {
    public:
        TupleIterator(TupleSorter* parent, int64_t index) : _parent(parent), _index(index), _current_tuple(NULL) {
            DCHECK_GE(index, 0);
...
Status SpillSorter::Run::init() {
    ...
    RETURN_IF_ERROR(_sorter->_block_mgr->get_new_block(_sorter->_block_mgr_client, NULL, &block));
    if (block == NULL) {
    ...
    if (_has_var_len_slots) {
        ...
        if (!_is_sorted) {
            RETURN_IF_ERROR(_sorter->_block_mgr->get_new_block(_sorter->_block_mgr_client, NULL, &_var_len_copy_block));
        ...
        for (int i = 0; i < tuples_remaining; ++i) {
            ...
            if (_materialize_slots) {
                new_tuple->materialize_exprs<has_var_len_data>(input_row, *_sort_tuple_desc,
                                                               _sorter->_sort_tuple_slot_expr_ctxs, NULL,
                                                               &string_values, &total_var_len);
                if (total_var_len > _sorter->_block_mgr->max_block_size()) {
                    std::stringstream error_msg;
                    error_msg << "Variable length data in a single tuple larger than block size " << total_var_len
                              << " > " << _sorter->_block_mgr->max_block_size();
            ...
            if (has_var_len_data) {
                ...
                // for (const auto* collection_slot :_sort_tuple_desc->collection_slots()) {
...
void SpillSorter::Run::transfer_resources(RowBatch* row_batch) {
    ...
    for (auto* block : _fixed_len_blocks) {
        if (block != NULL) {
            row_batch->add_block(block);
        }
    }
    _fixed_len_blocks.clear();
    for (auto* block : _var_len_blocks) {
        if (block != NULL) {
            row_batch->add_block(block);
        }
    }
...
Status SpillSorter::Run::unpin_all_blocks() {
    ...
    int total_var_len = 0;
    string_values.reserve(_sort_tuple_desc->string_slots().size());
    BufferedBlockMgr2::Block* cur_sorted_var_len_block = NULL;
    if (has_var_len_blocks()) {
        DCHECK(_var_len_copy_block != NULL);
    ...
    for (int i = 0; i < _fixed_len_blocks.size(); ++i) {
        ...
        if (has_var_len_blocks()) {
            for (int block_offset = 0; block_offset < cur_fixed_block->valid_data_len();
                ...
                Tuple* cur_tuple = reinterpret_cast<Tuple*>(cur_fixed_block->buffer() + block_offset);
    ...
    for (auto* var_block : _var_len_blocks) {
        var_block->del();
    }
    _var_len_blocks.clear();
...
Status SpillSorter::Run::get_next_batch(RowBatch** output_batch) {
    if (_buffered_batch.get() != NULL) {
        ...
        if (eos) {
            ...
            _fixed_len_blocks[_fixed_len_blocks.size() - 1] = NULL;
            if (has_var_len_blocks()) {
                _var_len_blocks.back()->del();
                _var_len_blocks[_var_len_blocks.size() - 1] = NULL;
            }
        }
    }
...
Status SpillSorter::Run::get_next(RowBatch* output_batch, bool* eos) {
    ...
    if (!_is_pinned) {
        ...
        if (_pin_next_fixed_len_block) {
            ...
            _fixed_len_blocks[_fixed_len_blocks_index - 1] = NULL;
            bool pinned;
        ...
        if (_pin_next_var_len_block) {
            ...
            _var_len_blocks[_var_len_blocks_index - 1] = NULL;
            bool pinned;
    ...
    while (!output_batch->at_capacity() && _fixed_len_block_offset < fixed_len_block->valid_data_len()) {
        DCHECK(fixed_len_block != NULL);
        Tuple* input_tuple = reinterpret_cast<Tuple*>(fixed_len_block->buffer() + _fixed_len_block_offset);
        ...
        if (convert_offset_to_ptr) {
            ...
            for (int i = 0; i < string_slots.size(); ++i) {
                ...
                StringValue* value = reinterpret_cast<StringValue*>(input_tuple->get_slot(slot_desc->tuple_offset()));
                ...
                if (block_index > _var_len_blocks_index) {
                    ...
                    value->ptr =
                            reinterpret_cast<char*>(_var_len_blocks[_var_len_blocks_index]->buffer() + block_offset);
                } // if (block_index > _var_len_blocks_index)
            }     // for (int i = 0; i < string_slots.size(); ++i)
...
void SpillSorter::Run::collect_non_null_varslots(Tuple* src, vector<StringValue*>* string_values, int* total_var_len) {
    string_values->clear();
    *total_var_len = 0;
    for (const auto* string_slot : _sort_tuple_desc->string_slots()) {
        if (!src->is_null(string_slot->null_indicator_offset())) {
            StringValue* string_val = reinterpret_cast<StringValue*>(src->get_slot(string_slot->tuple_offset()));
...
Status SpillSorter::Run::try_add_block(vector<BufferedBlockMgr2::Block*>* block_sequence, bool* added) {
    ...
    RETURN_IF_ERROR(_sorter->_block_mgr->get_new_block(_sorter->_block_mgr_client, last_block, &new_block));
    if (new_block != NULL) {
        *added = true;
...
// along the way.
void SpillSorter::TupleSorter::insertion_sort(const TupleIterator& first, const TupleIterator& last) {
    ...
    for (; insert_iter._index < last._index; insert_iter.next()) {
        ...
        while (_less_than_comp(_temp_tuple_row, reinterpret_cast<TupleRow*>(&iter._current_tuple))) {
...
void SpillSorter::TupleSorter::find_the_median(TupleSorter::TupleIterator& first, TupleSorter::TupleIterator& last,
...
SpillSorter::TupleSorter::TupleIterator SpillSorter::TupleSorter::partition(TupleIterator first, TupleIterator last,
                                                                            TupleIterator& mid) {
    ...
    while (true) {
        ...
        while (_less_than_comp(reinterpret_cast<TupleRow*>(&first._current_tuple), _temp_tuple_row)) {
            first.next();
        }
        while (_less_than_comp(_temp_tuple_row, reinterpret_cast<TupleRow*>(&last._current_tuple))) {
            last.prev();
        }
...
Status SpillSorter::init() {
    DCHECK(_unsorted_run == NULL) << "Already initialized";
    ...
    RETURN_IF_ERROR(_block_mgr->register_client(min_blocks_required, _mem_tracker, _state, &_block_mgr_client));
...
Status SpillSorter::add_batch(RowBatch* batch) {
    ...
    while (cur_batch_index < batch->num_rows()) {
        if (_has_var_len_slots) {
            ...
            RETURN_IF_ERROR(_unsorted_run->add_batch<false>(batch, cur_batch_index, &num_processed));
        }
...
Status SpillSorter::sort_run() {
    ...
    if (_has_var_len_slots) {
        DCHECK(_unsorted_run->_var_len_copy_block != NULL);
        ...
        if (last_block->valid_data_len() > 0) {
            ...
            if (_unsorted_run->_var_len_blocks.size() == 0) {
                ...
                _unsorted_run->_var_len_copy_block = NULL;
            }
        }
    }
    {
...
uint64_t SpillSorter::estimate_merge_mem(uint64_t available_blocks, RowDescriptor* row_desc, int merge_batch_size) {
...
Status SpillSorter::merge_intermediate_runs() {
    ...
    while (_sorted_runs.size() > max_runs_per_final_merge) {
        ...
        int num_runs_to_merge =
                std::min<int>(max_runs_per_intermediate_merge, _sorted_runs.size() - max_runs_per_intermediate_merge);
        ...
        Run* merged_run = _obj_pool.add(new Run(this, _output_row_desc->tuple_descriptors()[0], false));
        ...
        while (!eos) {
            ...
            if (_has_var_len_slots) {
                ...
                ret_status = merged_run->add_batch<false>(&intermediate_merge_batch, 0, &num_copied);
            }
...
Status SpillSorter::create_merger(int num_runs) {
    ...
    for (int i = 0; i < num_runs; ++i) {
        ...
        // from this run.
        merge_runs.emplace_back(std::bind<Status>(std::mem_fn(&Run::get_next_batch), run, std::placeholders::_1));
