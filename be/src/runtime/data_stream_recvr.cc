...
Status DataStreamRecvr::SenderQueue::_deserialize_chunk(const ChunkPB& pchunk, vectorized::Chunk* chunk,
    ...
    if (pchunk.compress_type() == CompressionTypePB::NO_COMPRESSION) {
        ...
        TRY_CATCH_BAD_ALLOC(
                if (!chunk->deserialize(&is, _chunk_meta)) return Status::Corruption("chunk deserialization failed"));
    } else {
        ...
        }
        {
            ...
            SCOPED_TIMER(_recvr->_deserialize_row_batch_timer);
            TRY_CATCH_BAD_ALLOC(if (!chunk->deserialize(&is, _chunk_meta)) return Status::Corruption(
                    "chunk deserialization failed"));
        }
    }
    return Status::OK();
}
