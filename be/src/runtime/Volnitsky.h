...
using Id = uint8_t;     /// Index of the string (within the array of multiple needles), must not be greater than 255.
...
static inline void putNGram(const uint8_t* const pos, const int offset, const Callback& putNGramBase) {
...
public:
    ...
    const char* search(const char* haystack, size_t haystack_size) const {
        return reinterpret_cast<const char*>(search(reinterpret_cast<const uint8_t*>(haystack), haystack_size));
    }
...
protected:
    void putNGramBase(const VolnitskyTraits::Ngram ngram, const int offset) {
        ...
        hash[cell_num] = static_cast<VolnitskyTraits::Offset>(offset);
    }
};
