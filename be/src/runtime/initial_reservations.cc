...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/initial_reservations.cc
...
InitialReservations::InitialReservations(ObjectPool* obj_pool, ReservationTracker* query_reservation,
                                         MemTracker* query_mem_tracker, int64_t initial_reservation_total_claims)
        : initial_reservation_mem_tracker_(
                  obj_pool->add(new MemTracker(-1, "Unclaimed reservations", query_mem_tracker, true))),
          remaining_initial_reservation_claims_(initial_reservation_total_claims) {
    initial_reservations_.InitChildTracker(nullptr, query_reservation, initial_reservation_mem_tracker_,
...
Status InitialReservations::Init(const TUniqueId& query_id, int64_t query_min_reservation) {
    ...
    if (!initial_reservations_.IncreaseReservation(query_min_reservation)) {
        std::stringstream ss;
        ss << "Minimum reservation unavaliable: " << query_min_reservation << " query id:" << query_id;
...
void InitialReservations::Return(BufferPool::ClientHandle* src, int64_t bytes) {
    ...
    int64_t excess_reservation = initial_reservations_.GetReservation() - remaining_initial_reservation_claims_;
