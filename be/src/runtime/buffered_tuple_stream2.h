...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/buffered_tuple_stream2.h
...
#ifndef STARROCKS_BE_SRC_RUNTIME_BUFFERED_TUPLE_STREAM2_H
#define STARROCKS_BE_SRC_RUNTIME_BUFFERED_TUPLE_STREAM2_H
...
public:
    ...
    struct RowIdx {
        ...
        uint64_t set(uint64_t block, uint64_t offset, uint64_t idx) {
            DCHECK_LE(block, BLOCK_MASK) << "Cannot have more than 2^16 = 64K blocks in a tuple stream.";
            DCHECK_LE(offset, OFFSET_MASK >> OFFSET_SHIFT) << "Cannot have blocks larger than 2^24 = 16MB";
            DCHECK_LE(idx, IDX_MASK >> IDX_SHIFT) << "Cannot have more than 2^24 = 16M rows in a block.";
...
private:
    ...
    bool copy_strings(const Tuple* tuple, const std::vector<SlotDescriptor*>& string_slots, int* bytes_allocated);
...
#endif // STARROCKS_BE_SRC_RUNTIME_BUFFERED_TUPLE_STREAM2_H
