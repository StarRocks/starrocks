...
>>>>>>> 39a4352ad9 ([Enhancement] Pause routine load job on non-retryable errors like primary key size exceed (#71161))
    ...
    static bool wait_txn_visible_until(const AuthInfo& auth, std::string_view db, std::string_view table,
                                       int64_t txn_id, int64_t deadline);
...
// submit this params
        ...
        auto st = _exec_env->fragment_mgr()->exec_plan_fragment(
                ...
                [ctx](PlanFragmentExecutor* executor) {
                    ...
                    if (status.ok()) {
                        ...
                        if (status.ok()) {
                            StarRocksMetrics::instance()->stream_receive_bytes_total.increment(
                                    ctx->total_receive_bytes);
                    ...
                    if (!executor->runtime_state()->get_rejected_record_file_path().empty()) {
                        ctx->rejected_record_path =
                                fmt::format("{}:{}", BackendOptions::get_localBackend().host,
    ...
    Status StreamLoadExecutor::commit_txn(StreamLoadContext * ctx) {
        ...
        while (true) {
            ...
            if (st.ok()) {
                ...
                ctx->need_rollback = false;
                bool visible = wait_txn_visible_until(ctx->auth, request.db, request.tbl, request.txnId,
                                                      ctx->load_deadline_sec);
                ...
                LOG(WARNING) << "commit transaction " << request.txnId
                             << " failed, will retry. errmsg=" << st.message();
    ...
    Status commit_txn_internal(const TLoadTxnCommitRequest& request, int32_t rpc_timeout_ms,
                               TLoadTxnCommitResult* result) {
        TNetworkAddress master_addr = get_master_address();
#ifndef BE_TEST
        RETURN_IF_ERROR(ThriftRpcHelper::rpc<FrontendServiceClient>(
