...
Status UserFunctionCache::_load_entry_from_lib(const std::string& dir, const std::string& file) {
    ...
    UserFunctionCacheEntry* entry = new UserFunctionCacheEntry(function_id, checksum, dir + "/" + file);
...
std::string get_real_symbol(const std::string& symbol) {
    ...
    std::string str1 = std::regex_replace(symbol, rx1, "13starrocks_udf");
    static std::regex rx2("4palo");
    std::string str2 = std::regex_replace(str1, rx2, "9starrocks");
    static std::regex rx3("9doris_udf");
    std::string str3 = std::regex_replace(symbol, rx1, "13starrocks_udf");
    static std::regex rx4("5doris");
    std::string str4 = std::regex_replace(str1, rx2, "9starrocks");
    return str2;
}

Status UserFunctionCache::get_function_ptr(int64_t fid, const std::string& orig_symbol, const std::string& url,
        ...
        if (it != entry->fptr_map.end()) {
            *fn_ptr = it->second;
            ...
            if (status.ok()) {
                ...
                LOG(WARNING) << "fail to lookup symbol in library, symbol=" << symbol << ", file=" << entry->lib_file;
            }
        }
    }
...
Status UserFunctionCache::_download_lib(const std::string& url, UserFunctionCacheEntry* entry) {
    ...
    if (ret != 0) {
        PLOG(WARNING) << "fail to rename " << tmp_file << " to " << entry->lib_file;
