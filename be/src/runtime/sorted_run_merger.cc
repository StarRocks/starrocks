...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/sorted_run_merger.cc
...
void SortedRunMerger::heapify(int parent_index) {
    ...
    if (right_index >= _min_heap.size() ||
        _compare_less_than(_min_heap[left_index]->current_row(), _min_heap[right_index]->current_row())) {
    ...
    // heapify recursively.
    if (_compare_less_than(_min_heap[least_child]->current_row(), _min_heap[parent_index]->current_row())) {
...
Status SortedRunMerger::get_next(RowBatch* output_batch, bool* eos) {
    ...
    while (!output_batch->at_capacity()) {
        ...
        if (_deep_copy_input) {
            ...
            memcpy(output_row, min->current_row(), _input_row_desc->tuple_descriptors().size() * sizeof(Tuple*));
        }
