...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/buffered_tuple_stream2.cc
...
string BufferedTupleStream2::debug_string() const {
    ...
    ss << "BufferedTupleStream2 num_rows=" << _num_rows << " rows_returned=" << _rows_returned
       << " pinned=" << (_pinned ? "true" : "false") << " delete_on_read=" << (_delete_on_read ? "true" : "false")
       << " closed=" << (_closed ? "true" : "false") << " num_pinned=" << _num_pinned << " write_block=" << _write_block
       << " _read_block=";
    ...
    ss << " blocks=[\n";
    for (list<BufferedBlockMgr2::Block*>::const_iterator it = _blocks.begin(); it != _blocks.end(); ++it) {
        ss << "{" << (*it)->debug_string() << "}";
        if (*it != _blocks.back()) {
            ss << ",\n";
        }
...
int64_t BufferedTupleStream2::bytes_in_mem(bool ignore_current) const {
    ...
    for (auto _block : _blocks) {
        ...
        if (!_block->is_max_size()) {
            continue;
        }
        if (_block == _write_block && ignore_current) {
            continue;
        }
        result += _block->buffer_len();
    }
    return result;
}
...
Status BufferedTupleStream2::new_block_for_write(int64_t min_size, bool* got_block) {
    ...
    *got_block = false;
    ...
    BufferedBlockMgr2::Block* new_block = nullptr;
    {
        ...
        RETURN_IF_ERROR(_block_mgr->get_new_block(_block_mgr_client, unpin_block, &new_block, block_len));
    }
...
Status BufferedTupleStream2::prepare_for_read(bool delete_on_read, bool* got_buffer) {
    ...
    for (auto& _block : _blocks) {
        if (!_block->is_pinned()) {
            ...
            RETURN_IF_ERROR(_block->pin(&current_pinned));
            if (!current_pinned) {
...
Status BufferedTupleStream2::pin_stream(bool already_reserved, bool* pinned) {
    ...
    for (auto& _block : _blocks) {
        ...
        {
            SCOPED_TIMER(_pin_timer);
            RETURN_IF_ERROR(_block->pin(pinned));
        }
        if (!*pinned) {
            VLOG_QUERY << "Should have been reserved." << std::endl << _block_mgr->debug_string(_block_mgr_client);
            return Status::OK();
        }
        ++_num_pinned;
        DCHECK_EQ(_num_pinned, num_pinned(_blocks));
    ...
    if (!_delete_on_read) {
        ...
        for (auto& _block : _blocks) {
            _block_start_idx.push_back(_block->buffer());
        }
    }
    *pinned = true;
    _pinned = true;
...
Status BufferedTupleStream2::get_next_internal(RowBatch* batch, bool* eos, vector<RowIdx>* indices) {
    DCHECK(!_closed);
    ...
    *eos = (_rows_returned == _num_rows);
    ...
    int rows_to_fill = std::min(static_cast<int64_t>(batch->capacity() - batch->num_rows()), rows_left);
    ...
    while (i < rows_to_fill) {
        ...
        (*indices)[i].set(_read_block_idx, _read_bytes + _null_indicators_read_block, last_read_row);
        if (HasNullableTuple) {
            for (int j = 0; j < tuples_per_row; ++j) {
                ...
                // false (0x0).
                row->set_tuple(j, reinterpret_cast<Tuple*>(reinterpret_cast<uint64_t>(_read_ptr) * is_not_null));
        ...
        for (auto& _string_slot : _string_slots) {
            ...
            read_strings(_string_slot.second, data_len, tuple);
        }
    ...
    _rows_returned += i;
    *eos = (_rows_returned == _num_rows);
    if ((!_pinned || _delete_on_read) && rows_returned_curr_block + i == (*_read_block)->num_rows()) {
...
void BufferedTupleStream2::read_strings(const vector<SlotDescriptor*>& string_slots, int data_len, Tuple* tuple) {
