...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/decimal_value.cpp
...
inline void fix_intg_frac_error(const int32_t len, int32_t* int_len, int32_t* frac_len, int32_t* error) {
...
int32_t do_add(const DecimalValue& value1, const DecimalValue& value2, DecimalValue* to) {
    ...
    int32_t first_big_digit_sum = intg1 > intg2
                                          ? value1._buffer[0]
...
int do_div_mod(const DecimalValue& value1, const DecimalValue& value2, DecimalValue* to, DecimalValue* mod) {
...
#pragma GCC diagnostic pop
    ...
    if (to->_buffer != first_no_zero) {
        memmove(to->_buffer, first_no_zero, (round_up(to->_int_length) + round_up(to->_frac_length)) * sizeof(int32_t));
    }
    return error;
}
