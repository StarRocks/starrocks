...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/disk_io_mgr_internal.h
...
#ifndef STARROCKS_BE_SRC_QUERY_RUNTIME_DISK_IO_MGR_INTERNAL_H
#define STARROCKS_BE_SRC_QUERY_RUNTIME_DISK_IO_MGR_INTERNAL_H
    ...
    inline void enqueue_context(RequestContext* worker) {
        {
            ...
            DCHECK(find(request_contexts.begin(), request_contexts.end(), worker) == request_contexts.end());
...
public:
    ...
    void decrement_disk_ref_count() {
        // STL doesn't let us dcheck that the reader lock is taken
...
private:
    ...
    public:
        ...
        const InternalQueue<ScanRange>* unstarted_scan_ranges() const { return &_unstarted_scan_ranges; }
        const InternalQueue<WriteRange>* unstarted_write_ranges() const { return &_unstarted_write_ranges; }
...
#endif // STARROCKS_BE_SRC_QUERY_RUNTIME_DISK_IO_MGR_INTERNAL_H
