...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/buffered_block_mgr2.cc
...
public:
    Client(BufferedBlockMgr2* mgr, int num_reserved_buffers, MemTracker* tracker, RuntimeState* state)
            : _mgr(mgr),
              _state(state),
              _tracker(tracker),
...
BufferedBlockMgr2::BufferedBlockMgr2(RuntimeState* state, TmpFileMgr* tmp_file_mgr, int64_t block_size)
...
Status BufferedBlockMgr2::create(RuntimeState* state, MemTracker* parent, RuntimeProfile* profile,
    ...
    block_mgr->reset();
    {
        ...
        if (*block_mgr == nullptr) {
            ...
            // all std::shared_ptr references have gone to 0 and it is in the process of
...
int64_t BufferedBlockMgr2::available_buffers(Client* client) const {
    int64_t unused_reserved =
            client->_num_reserved_buffers + client->_num_tmp_reserved_buffers - client->_num_pinned_buffers;
    return std::max<int64_t>(0, remaining_unreserved_buffers()) + std::max<int64_t>(0, unused_reserved);
}

int64_t BufferedBlockMgr2::remaining_unreserved_buffers() const {
    int64_t num_buffers = _free_io_buffers.size() + _unpinned_blocks.size() + _non_local_outstanding_writes;
    num_buffers += _mem_tracker->spare_capacity() / max_block_size();
    num_buffers -= _unfullfilled_reserved_buffers;
    return num_buffers;
}

Status BufferedBlockMgr2::register_client(int num_reserved_buffers, MemTracker* tracker, RuntimeState* state,
                                          Client** client) {
...
void BufferedBlockMgr2::clear_reservations(Client* client) {
    ...
    if (client->_num_pinned_buffers < client->_num_reserved_buffers) {
        _unfullfilled_reserved_buffers -= client->_num_reserved_buffers - client->_num_pinned_buffers;
    }
...
Status BufferedBlockMgr2::mem_limit_too_low_error(Client* client, int node_id) {
    ...
    error_msg << "The memory limit is set too low to initialize spilling operator (id=" << node_id
              ...
              << PrettyPrinter::print(client->_num_reserved_buffers * max_block_size(), TUnit::BYTES) << ".";
    return Status::MemoryLimitExceeded(error_msg.str());
}

Status BufferedBlockMgr2::get_new_block(Client* client, Block* unpin_block, Block** block, int64_t len) {
    ...
    *block = nullptr;
    ...
    DCHECK(!new_block->is_pinned() || new_block->_buffer_desc != nullptr) << new_block->debug_string();
...
BufferedBlockMgr2::~BufferedBlockMgr2() {
    {
        std::lock_guard<SpinLock> lock(_s_block_mgrs_lock);
    ...
    for (auto* buffer : _all_io_buffers) {
...
Status BufferedBlockMgr2::pin_block(Block* block, bool* pinned, Block* release_block, bool unpin) {
    ...
    *pinned = block->_is_pinned;
        ...
        if (block->_buffer_desc != nullptr) {
            {
                ...
                if (_free_io_buffers.contains(block->_buffer_desc)) {
                    DCHECK(!block->_is_pinned && !block->_in_write && !_unpinned_blocks.contains(block))
                            << endl
                            << block->debug_string();
    ...
    scan_range->reset(nullptr, block->_write_range->file(), block->_write_range->len(), block->_write_range->offset(),
                      block->_write_range->disk_id(), false, block, DiskIoMgr::ScanRange::NEVER_CACHE);
...
Status BufferedBlockMgr2::unpin_block(Block* block) {
    ...
    std::lock_guard<std::mutex> unpinned_lock(_lock);
    if (_is_cancelled) {
        return Status::Cancelled("Cancelled");
    }
...
Status BufferedBlockMgr2::write_unpinned_block(Block* block) {
    ...
    if (block->_write_range == nullptr) {
        ...
        DiskIoMgr::WriteRange::WriteDoneCallback callback =
                std::bind(std::mem_fn(&BufferedBlockMgr2::write_complete), this, block, std::placeholders::_1);
        block->_write_range =
                _obj_pool.add(new DiskIoMgr::WriteRange(tmp_file->path(), file_offset, disk_id, callback));
...
void BufferedBlockMgr2::write_complete(Block* block, const Status& write_status) {
    ...
    if (block->_is_pinned) {
        ...
        // hasn't.
        DCHECK(!block->_client_local) << "Client should be waiting. No one should have pinned this block.";
        if (write_status.ok() && !_is_cancelled && !state->is_cancelled()) {
            status = write_unpinned_blocks();
        }
    } else if (block->_client_local) {
        DCHECK(!block->_is_deleted) << "Client should be waiting. No one should have deleted this block.";
        block->_write_complete_cv.notify_one();
    } else {
        DCHECK_EQ(block->_buffer_desc->len, _max_block_size) << "Only io sized buffers should spill";
...
Status BufferedBlockMgr2::find_buffer_for_block(Block* block, bool* in_mem) {
    ...
    DCHECK(!block->_is_pinned && !block->_is_deleted) << "Pinned or deleted block " << endl << block->debug_string();
    *in_mem = false;
    ...
    if (block->_buffer_desc != nullptr) {
        ...
        DCHECK(_unpinned_blocks.contains(block) || block->_in_write || _free_io_buffers.contains(block->_buffer_desc));
        ...
        *in_mem = true;
        ...
        if (buffer_desc == nullptr) {
            ...
            if (VLOG_QUERY_IS_ON) {
                stringstream ss;
                ss << "Query id=" << _query_id << " was unable to get minimum required buffers." << endl
                   << debug_internal() << endl
                   << client->debug_string();
                VLOG_QUERY << ss.str();
            }
    ...
    DCHECK(block->_buffer_desc->len < max_block_size() || !block->_is_pinned)
            << "Trying to pin already pinned block. " << block->_buffer_desc->len << " " << block->_is_pinned;
    block->_is_pinned = true;
...
Status BufferedBlockMgr2::find_buffer(std::unique_lock<std::mutex>& lock, BufferDescriptor** buffer_desc) {
    *buffer_desc = nullptr;

    // First, try to allocate a new buffer.
    if (_free_io_buffers.size() < _block_write_threshold && _mem_tracker->try_consume(_max_block_size)) {
        uint8_t* new_buffer = new uint8_t[_max_block_size];
        *buffer_desc = _obj_pool.add(new BufferDescriptor(new_buffer, _max_block_size));
        (*buffer_desc)->all_buffers_it = _all_io_buffers.insert(_all_io_buffers.end(), *buffer_desc);
        return Status::OK();
    }
    ...
    for (auto* buffer : _all_io_buffers) {
        ...
        if (buffer->block != nullptr) {
            ...
            if (!buffer->block->validate()) {
                LOG(ERROR) << "buffer->block inconsistent." << endl << buffer->block->debug_string();
                return false;
            }

            if (is_free &&
                (buffer->block->_is_pinned || buffer->block->_in_write || _unpinned_blocks.contains(buffer->block))) {
                LOG(ERROR) << "Block with buffer in free list and"
                           << " _is_pinned = " << buffer->block->_is_pinned
                           << " _in_write = " << buffer->block->_in_write
                           << " _Unpinned_blocks.contains = " << _unpinned_blocks.contains(buffer->block) << endl
                           << buffer->block->debug_string();
                return false;
            }
        }
    ...
    while (block != nullptr) {
        ...
        if (block->_in_write || _free_io_buffers.contains(block->_buffer_desc)) {
            LOG(ERROR) << "Block in unpinned list with"
                       ...
                       << " _free_io_buffers.contains = " << _free_io_buffers.contains(block->_buffer_desc) << endl
                       << block->debug_string();
            return false;
        }
        block = block->next();
...
void BufferedBlockMgr2::init(DiskIoMgr* io_mgr, RuntimeProfile* parent_profile, MemTracker* parent_tracker,
                             int64_t mem_limit) {
    std::unique_lock<std::mutex> l(_lock);
    if (_initialized) {
        return;
    ...
    _outstanding_writes_counter = ADD_COUNTER(_profile.get(), "BlockWritesOutstanding", TUnit::UNIT);
    ...
    _mem_tracker = std::make_unique<MemTracker>(mem_limit, "Block Manager", parent_tracker);
