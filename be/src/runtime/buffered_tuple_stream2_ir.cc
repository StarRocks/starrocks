...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/buffered_tuple_stream2_ir.cc
...
bool BufferedTupleStream2::deep_copy_internal(TupleRow* row) {
    ...
    DCHECK(_write_block->is_pinned()) << debug_string() << std::endl << _write_block->debug_string();

    const uint64_t tuples_per_row = _desc.tuple_descriptors().size();
    if (UNLIKELY((_write_block->bytes_remaining() < _fixed_tuple_row_size) ||
                 (HasNullableTuple && (_write_tuple_idx + tuples_per_row > _null_indicators_write_block * 8)))) {
        return false;
    }
