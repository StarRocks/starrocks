...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/disk_io_mgr_reader_context.cc
...
void DiskIoMgr::RequestContext::cancel(const Status& status) {
    ...
    for (const auto& write_callback : write_callbacks) {
...
void DiskIoMgr::RequestContext::add_request_range(DiskIoMgr::RequestRange* range, bool schedule_immediately) {
...
string DiskIoMgr::RequestContext::debug_string() const {
    ...
    if (_state != RequestContext::Inactive) {
        ss << " _status=" << (_status.ok() ? "OK" : _status.get_error_msg()) << " #ready_buffers=" << _num_ready_buffers
           << " #used_buffers=" << _num_used_buffers << " #num_buffers_in_reader=" << _num_buffers_in_reader
           << " #finished_scan_ranges=" << _num_finished_ranges << " #disk_with_ranges=" << _num_disks_with_ranges
           << " #disks=" << _num_disks_with_ranges;
        for (int i = 0; i < _disk_states.size(); ++i) {
            ss << endl
               << "   " << i << ": "
               << "is_on_queue=" << _disk_states[i].is_on_queue() << " done=" << _disk_states[i].done()
...
bool DiskIoMgr::RequestContext::validate() const {
    ...
    for (int i = 0; i < _disk_states.size(); ++i) {
        ...
        if (num_reading_threads < 0) {
            LOG(WARNING) << "disk_id=" << i << "state.num_threads_in_read < 0: #threads=" << num_reading_threads;
            return false;
        }
        ...
        if (_state != RequestContext::Cancelled) {
            if (state.unstarted_scan_ranges()->size() + state.in_flight_ranges()->size() >
                state.num_remaining_ranges()) {
                LOG(WARNING) << "disk_id=" << i << " state.unstarted_ranges.size() + state.in_flight_ranges.size()"
                             << " > state.num_remaining_ranges:"
            ...
            if (!state.in_flight_ranges()->empty() && !on_queue && num_reading_threads == 0) {
                LOG(WARNING) << "disk_id=" << i << " reader has inflight ranges but is not on the disk queue."
                             << " #in_flight_ranges=" << state.in_flight_ranges()->size()
                             << " #reading_threads=" << num_reading_threads << " on_queue=" << on_queue;
                return false;
            }

            if (state.done() && num_reading_threads > 0) {
                LOG(WARNING) << "disk_id=" << i << " state set to done but there are still threads working."
        ...
        if (state.done() && on_queue) {
            LOG(WARNING) << "disk_id=" << i << " state set to done but the reader is still on the disk queue."
