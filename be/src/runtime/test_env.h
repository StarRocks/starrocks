...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/test_env.h
...
public:
    ...
    Status create_query_state(int64_t query_id, int max_buffers, int block_size, RuntimeState** runtime_state);
    ...
    Status create_query_states(int64_t start_query_id, int num_mgrs, int buffers_per_mgr, int block_size,
...
private:
    ...
    static std::unique_ptr<MetricRegistry> _s_static_metrics;
    std::unique_ptr<ExecEnv> _exec_env;
...
#endif // STARROCKS_BE_TEST_QUERY_RUNTIME_TEST_ENV_H
