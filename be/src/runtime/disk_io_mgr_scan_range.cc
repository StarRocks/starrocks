...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/disk_io_mgr_scan_range.cc
...
Status DiskIoMgr::ScanRange::get_next(BufferDescriptor** buffer) {
    *buffer = nullptr;
    ...
    std::unique_lock<std::mutex> reader_lock(_reader->_lock);
    if (_eosr_returned) {
        _reader->_total_range_queue_capacity += _ready_buffers_capacity;
        ++_reader->_num_finished_ranges;
        _reader->_initial_queue_capacity = _reader->_total_range_queue_capacity / _reader->_num_finished_ranges;
    }
...
void DiskIoMgr::ScanRange::cancel(const Status& status) {
    ...
    DCHECK(!status.ok());
    {
        ...
        std::unique_lock<std::mutex> scan_range_lock(_lock);
        std::unique_lock<std::mutex> hdfs_lock(_hdfs_lock);
        DCHECK(validate()) << debug_string();
        if (_is_cancelled) {
            return;
        }
...
Status DiskIoMgr::ScanRange::read(char* buffer, int64_t* bytes_read, bool* eosr) {
    ...
    *bytes_read = 0;
    ...
    int bytes_to_read = std::min(static_cast<int64_t>(_io_mgr->_max_buffer_size), _len - _bytes_read);
    ...
    *bytes_read = fread(buffer, 1, bytes_to_read, _local_file);
    ...
    if (*bytes_read < bytes_to_read) {
        if (ferror(_local_file) != 0) {
            string error_msg = get_str_err_msg();
            stringstream ss;
            ss << "Error reading from " << _file << " at byte offset: " << (_offset + _bytes_read) << ": " << error_msg;
            return Status::InternalError(ss.str());
        } else {
