...
//   https://github.com/apache/incubator-doris/blob/master/be/src/runtime/disk_io_mgr.cc
...
DiskIoMgr::WriteRange::WriteRange(const string& file, int64_t file_offset, int disk_id, WriteDoneCallback callback) {
    _file = file;
    _offset = file_offset;
    _disk_id = disk_id;
...
DiskIoMgr::DiskIoMgr(int num_local_disks, int threads_per_disk, int min_buffer_size, int max_buffer_size)
...
void DiskIoMgr::cancel_context(RequestContext* context, bool wait_for_disks_completion) {
    ...
    if (wait_for_disks_completion) {
        std::unique_lock<std::mutex> lock(context->_lock);
...
Status DiskIoMgr::add_scan_ranges(RequestContext* reader, const vector<ScanRange*>& ranges, bool schedule_immediately) {
    if (ranges.empty()) {
        return Status::OK();
    }
...
Status DiskIoMgr::read(RequestContext* reader, ScanRange* range, BufferDescriptor** buffer) {
    ...
    *buffer = nullptr;
    ...
    if (range->len() > _max_buffer_size) {
        ...
        error_msg << "Cannot perform sync read larger than " << _max_buffer_size << ". Request was " << range->len();
...
void DiskIoMgr::gc_io_buffers() {
    ...
    for (int idx = 0; idx < _free_buffers.size(); ++idx) {
        for (list<char*>::iterator iter = _free_buffers[idx].begin(); iter != _free_buffers[idx].end(); ++iter) {
    ...
    *range = nullptr;
    ...
    while (true) {
        *request_context = nullptr;
        ...
        std::unique_lock<std::mutex> request_lock((*request_context)->_lock);
        VLOG_FILE << "Disk (id=" << disk_id << ") reading for " << (*request_context)->debug_string();
...
void DiskIoMgr::handle_write_finished(RequestContext* writer, WriteRange* write_range, const Status& write_status) {
    ...
    {
        std::unique_lock<std::mutex> writer_lock(writer->_lock);
...
void DiskIoMgr::handle_read_finished(DiskQueue* disk_queue, RequestContext* reader, BufferDescriptor* buffer) {
    std::unique_lock<std::mutex> reader_lock(reader->_lock);
...
Status DiskIoMgr::write_range_helper(FILE* file_handle, WriteRange* write_range) {
    ...
    if (success != 0) {
        ...
        error_msg << "fseek(" << write_range->_file << ", " << write_range->offset()
                  << " SEEK_SET) failed with errno=" << errno << " description=" << get_str_err_msg();
        return Status::InternalError(error_msg.str());
    }
