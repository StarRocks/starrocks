...
public:
    ...
    Status write_block(const CacheId& cache_id, DiskBlockPtr block, off_t offset_in_block,
                       const IOBuf& buf) const;
    Status read_block(const CacheId& cache_id, DiskBlockPtr block, off_t offset_in_block, size_t size,
                      IOBuf* buf) const;
...
private:
    void _update_block_checksum(DiskBlockPtr block, off_t offset_in_block, const IOBuf& buf) const;

    bool _check_block_checksum(DiskBlockPtr block, off_t offset_in_block, const IOBuf& buf) const;
