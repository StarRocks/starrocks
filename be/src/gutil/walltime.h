...
//   https://github.com/apache/incubator-doris/blob/master/be/src/gutil/walltime.h
...
bool WallTime_Parse_Timezone(const char* time_spec, const char* format, const struct tm* default_time, bool local,
                             WallTime* result);
...
inline MicrosecondsInt64 GetThreadCpuTimeMicros() {
    ...
    kern_return_t result = thread_info(thread, THREAD_BASIC_INFO, reinterpret_cast<thread_info_t>(&thread_info_data),
                                       &thread_info_count);
