...
// column's ordinal index and page zone map as one contiguous run immediately before the short key
// index and the footer, instead of interleaving each column's indexes after that column's data
// pages. Both layouts are readable by any binary -- indexes are always located through absolute
...
// accessed column, and the page zone map of every predicate column, before it can read any data.
// In the legacy layout those live at N scattered offsets, so they cost N serial round trips to
// remote storage, each pulling a whole cache block. Gathered at the tail they share blocks, and
// the block that parsing the footer already fetched often covers them.
//
// Writers that finalize columns in several groups retain the deferred index builders until the
// footer is written, so vertical compaction and partial-update rewrites produce the layout too.
// That is not a corner case: compaction picks VERTICAL for any table wider than
// vertical_compaction_max_columns_per_group with more than one source rowset, so gating it out
