...
// Logical type used for the __cset__ load slot / chunk / .upt column. FE injects it as SMALLINT and
// the scanner writes / apply reads it as SIGNED int16, so a valid set-id must fit [0, 32767]; the
// interner is bounded to 2^15 distinct column-sets accordingly (CDC loads have a handful). Exceeding
// the bound fails the load fast (get_or_intern returns an error) rather than overflowing int16 negative.
