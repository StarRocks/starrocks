...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/topn_node.cpp
...
Status TopNNode::prepare(RuntimeState* state) {
    ...
    _runtime_profile->add_info_string("SortType", "TopN");
    ...
    _tuple_row_less_than.reset(new TupleRowComparator(_sort_exec_exprs, _is_asc_order, _nulls_first));
...
Status TopNNode::open(RuntimeState* state) {
    ...
    if (_priority_queue.get() == NULL) {
        _priority_queue.reset(
                new std::priority_queue<Tuple*, std::vector<Tuple*>, TupleRowComparator>(*_tuple_row_less_than));
    }

    // Allocate memory for a temporary tuple.
    _tmp_tuple = reinterpret_cast<Tuple*>(_tuple_pool->allocate(_materialized_tuple_desc->byte_size()));
    if (UNLIKELY(_tmp_tuple == nullptr)) {
...
void TopNNode::insert_tuple_row(TupleRow* input_row) {
    ...
    if (_priority_queue->size() < _offset + _limit) {
        insert_tuple = reinterpret_cast<Tuple*>(_tuple_pool->allocate(_materialized_tuple_desc->byte_size()));
        insert_tuple->materialize_exprs<false>(input_row, *_materialized_tuple_desc,
                                               _sort_exec_exprs.sort_tuple_slot_expr_ctxs(), _tuple_pool.get(), NULL,
                                               NULL);
    } else {
...
void TopNNode::push_down_predicate(RuntimeState* state, std::list<ExprContext*>* expr_ctxs, bool is_vectorized) {
