...
LookupJoinSeekOperator::LookupJoinSeekOperator(OperatorFactory* factory, int32_t id, int32_t plan_node_id,
        ...
        : SourceOperator(factory, id, "index_seek", plan_node_id, driver_sequence),
          ...
          _other_join_conjuncts(_lookup_join_context->params().other_join_conjunct_ctxs) {
    DCHECK(_lookup_join_context);
    _lookup_join_context->ref();
}

LookupJoinSeekOperator::~LookupJoinSeekOperator() {
    if (_reader) {
        _reader.reset();
    }
}

Status LookupJoinSeekOperator::prepare(RuntimeState* state) {
...
Status LookupJoinSeekOperator::_init_tablet_reader(RuntimeState* state, TabletReaderState& reader_state) {
    ...
    RETURN_IF_ERROR(_init_conjunct_ctxs(reader_state, _row_id));
    // init conjncts manager

    // Make sure clear states.
    RETURN_IF_ERROR(_init_conjuncts_manager(state, reader_state));
    // _init_reader_state
    RETURN_IF_ERROR(_init_reader_state(state, reader_state));
    ...
    RETURN_IF_ERROR(_reader->open(reader_state.params));
    _row_id++;

    return Status::OK();
}

void LookupJoinSeekOperator::_init_row_desc() {
...
Status LookupJoinSeekOperator::_get_tablet(const TInternalScanRange* scan_range) {
...
Status LookupJoinSeekOperator::_init_conjuncts_manager(RuntimeState* state, TabletReaderState& reader_state) {
    ...
    RETURN_IF_ERROR(cm->get_key_ranges(&(reader_state.key_ranges)));
    cm->get_not_push_down_conjuncts(&(reader_state.not_push_down_conjuncts));
    VLOG(1) << "reader_state.conjunct_ctxs:" << Expr::debug_string(reader_state.conjunct_ctxs);
    VLOG(1) << "reader_state.not_push_down_conjuncts:" << Expr::debug_string(reader_state.not_push_down_conjuncts);
...
Status LookupJoinSeekOperator::_init_scanner_columns(std::vector<uint32_t>& scanner_columns) {
...
void LookupJoinSeekOperator::close(RuntimeState* state) {
...
vectorized::ChunkPtr LookupJoinSeekOperator::_init_ouput_chunk(RuntimeState* state) {
...
void LookupJoinSeekOperator::_permute_output_chunk(RuntimeState* state, vectorized::ChunkPtr* output_chunk) {
...
Status LookupJoinSeekOperator::_seek_row(RuntimeState* state) {
    do {
        if (_cur_eos) {
...
StatusOr<vectorized::ChunkPtr> LookupJoinSeekOperator::pull_chunk(RuntimeState* state) {
    ...
    do {
        ...
        // output result
        _permute_output_chunk(state, &output_chunk);

        // filter other conjuncts.
        if (!_other_join_conjuncts.empty()) {
            RETURN_IF_ERROR(eval_conjuncts(_other_join_conjuncts, output_chunk.get(), nullptr));
        }
    } while (output_chunk->num_rows() == 0);
    return std::move(output_chunk);
}

Status LookupJoinSeekOperator::set_finishing(RuntimeState* state) {
    _is_finished = true;
    return Status::OK();
}
