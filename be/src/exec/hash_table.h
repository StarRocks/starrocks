...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/hash_table.h
...
#ifndef STARROCKS_BE_SRC_QUERY_EXEC_HASH_TABLE_H
#define STARROCKS_BE_SRC_QUERY_EXEC_HASH_TABLE_H
...
public:
    ...
    HashTable(const std::vector<ExprContext*>& build_exprs, const std::vector<ExprContext*>& probe_exprs,
              int num_build_tuples, bool stores_nulls, const std::vector<bool>& finds_nulls, int32_t initial_seed,
    ...
    int64_t byte_size() const { return _node_byte_size * _nodes_capacity + sizeof(Bucket) * _buckets.size(); }
    ...
    void* last_expr_value(int expr_idx) const { return _expr_values_buffer + _expr_values_buffer_offsets[expr_idx]; }
    ...
    public:
        ...
        bool operator==(const Iterator& rhs) { return _bucket_idx == rhs._bucket_idx && _node_idx == rhs._node_idx; }
...
private:
    ...
    void move_node(Bucket* from_bucket, Bucket* to_bucket, int64_t node_idx, Node* node, Node* previous_node);
