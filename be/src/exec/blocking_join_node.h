...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/blocking_join_node.h
...
#ifndef STARROCKS_BE_SRC_QUERY_EXEC_BLOCKING_JOIN_NODE_H
#define STARROCKS_BE_SRC_QUERY_EXEC_BLOCKING_JOIN_NODE_H
...
private:
    ...
    int _left_batch_pos = 0;     // current scan pos in _left_batch
    bool _left_side_eos = false; // if true, left child has no more rows to process
    TupleRow* _current_left_child_row = nullptr;
    ...
    RuntimeProfile::Counter* _build_timer = nullptr;            // time to prepare build side
    RuntimeProfile::Counter* _left_child_timer = nullptr;       // time to process left child batch
    RuntimeProfile::Counter* _build_row_counter = nullptr;      // num build rows
    RuntimeProfile::Counter* _left_child_row_counter = nullptr; // num left child rows
