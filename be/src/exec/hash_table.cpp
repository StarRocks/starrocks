...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/hash_table.cpp
...
HashTable::HashTable(const std::vector<ExprContext*>& build_expr_ctxs, const std::vector<ExprContext*>& probe_expr_ctxs,
                     int num_build_tuples, bool stores_nulls, const std::vector<bool>& finds_nulls,
                     int32_t initial_seed, MemTracker* mem_tracker, int64_t num_buckets)
    ...
    _results_buffer_size =
            Expr::compute_results_layout(_build_expr_ctxs, &_expr_values_buffer_offsets, &_var_result_begin);
