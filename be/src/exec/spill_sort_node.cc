...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/spill_sort_node.cc
...
Status SpillSortNode::prepare(RuntimeState* state) {
    ...
    _runtime_profile->add_info_string("SortType", "All");
...
Status SpillSortNode::open(RuntimeState* state) {
    ...
    if (_sorter.get() == NULL) {
        ...
        _sorter.reset(new SpillSorter(less_than, _sort_exec_exprs.sort_tuple_slot_expr_ctxs(), &_row_descriptor,
                                      mem_tracker(), runtime_profile(), state));
