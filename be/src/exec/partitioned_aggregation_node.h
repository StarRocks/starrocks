...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/partitioned_aggregation_node.h
...
#ifndef STARROCKS_BE_SRC_EXEC_NEW_PARTITIONED_AGGREGATION_NODE_H
#define STARROCKS_BE_SRC_EXEC_NEW_PARTITIONED_AGGREGATION_NODE_H
...
private:
    ...
    typedef Status (*ProcessBatchFn)(PartitionedAggregationNode*, RowBatch*, PartitionedHashTableCtx*);
    /// Jitted ProcessBatch function pointer. Null if codegen is disabled.
    ProcessBatchFn process_batch_fn_;

    typedef Status (*ProcessBatchStreamingFn)(PartitionedAggregationNode*, bool, RowBatch*, RowBatch*,
                                              PartitionedHashTableCtx*, int[PARTITION_FANOUT]);
    ...
    Status CopyStringData(const SlotDescriptor& slot_desc, RowBatch* row_batch, int first_row_idx, MemPool* pool);

    /// Constructs singleton output tuple, allocating memory from pool.
    Tuple* ConstructSingletonOutputTuple(const std::vector<NewAggFnEvaluator*>& agg_fn_evals, MemPool* pool);
    ...
    Tuple* ConstructIntermediateTuple(const std::vector<NewAggFnEvaluator*>& agg_fn_evals, BufferedTupleStream3* stream,
                                      Status* status);
    ...
    Tuple* ConstructIntermediateTuple(const std::vector<NewAggFnEvaluator*>& agg_fn_evals, MemPool* pool,
                                      Status* status);
    ...
    void InitAggSlots(const std::vector<NewAggFnEvaluator*>& agg_fn_evals, Tuple* intermediate_tuple);
    ...
    void UpdateTuple(NewAggFnEvaluator** agg_fn_evals, Tuple* tuple, TupleRow* row, bool is_merge = false);
    ...
    Tuple* GetOutputTuple(const std::vector<NewAggFnEvaluator*>& agg_fn_evals, Tuple* tuple, MemPool* pool);
    ...
    Status ProcessBatchStreaming(bool needs_serialize, RowBatch* in_batch, RowBatch* out_batch,
                                 PartitionedHashTableCtx* ht_ctx, int remaining_capacity[PARTITION_FANOUT]);
    ...
    void CleanupHashTbl(const std::vector<NewAggFnEvaluator*>& agg_fn_evals, PartitionedHashTable::Iterator it);
