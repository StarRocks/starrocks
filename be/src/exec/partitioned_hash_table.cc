...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/partitioned_hash_table.cc
...
static uint32_t SEED_PRIMES[] = {1, // First seed must be 1, level 0 is used by other operators in the fragment.
                                 1431655781, 1183186591, 622729787, 472882027, 338294347, 275604541,
                                 41161739,   29999999,   27475109,  611603,    16313357,  11380003,
                                 21261403,   33393119,   101,       71043403};
...
// to 255 bytes
static int64_t NULL_VALUE[] = {HashUtil::FNV_SEED, HashUtil::FNV_SEED, HashUtil::FNV_SEED, HashUtil::FNV_SEED,
...
PartitionedHashTableCtx::PartitionedHashTableCtx(const std::vector<Expr*>& build_exprs,
        ...
        : build_exprs_(build_exprs),
          ...
          finds_some_nulls_(std::accumulate(finds_nulls_.begin(), finds_nulls_.end(), false, std::logical_or<bool>())),
          level_(0),
          scratch_row_(NULL),
          mem_pool_(mem_pool),
...
Status PartitionedHashTableCtx::Init(ObjectPool* pool, RuntimeState* state, int num_build_tuples, MemTracker* tracker,
...
Status PartitionedHashTableCtx::Create(ObjectPool* pool, RuntimeState* state, const std::vector<Expr*>& build_exprs,
                                       const std::vector<Expr*>& probe_exprs, bool stores_nulls,
    ...
    ht_ctx->reset(new PartitionedHashTableCtx(build_exprs, probe_exprs, stores_nulls, finds_nulls, initial_seed,
                                              max_levels, mem_pool, expr_results_pool));
...
uint32_t PartitionedHashTableCtx::HashRow(const uint8_t* expr_values, const uint8_t* expr_values_null) const noexcept {
...
bool PartitionedHashTableCtx::EvalRow(TupleRow* row, const vector<ExprContext*>& ctxs, uint8_t* expr_values,
...
template bool PartitionedHashTableCtx::Equals<true>(TupleRow* build_row, const uint8_t* expr_values,
                                                    const uint8_t* expr_values_null) const;
template bool PartitionedHashTableCtx::Equals<false>(TupleRow* build_row, const uint8_t* expr_values,
                                                     const uint8_t* expr_values_null) const;
...
Status PartitionedHashTableCtx::ExprValuesCache::Init(RuntimeState* state, MemTracker* tracker,
    ...
    expr_values_bytes_per_row_ = Expr::compute_results_layout(build_exprs, &expr_values_offsets_, &var_result_offset_);
    ...
    capacity_ = std::max(1, std::min(state->batch_size(), MAX_EXPR_VALUES_ARRAY_SIZE / expr_values_bytes_per_row_));

    int mem_usage = MemUsage(capacity_, expr_values_bytes_per_row_, num_exprs_);
    if (UNLIKELY(!tracker->try_consume(mem_usage))) {
        capacity_ = 0;
        string details = Substitute("PartitionedHashTableCtx::ExprValuesCache failed to allocate $0 bytes.", mem_usage);
...
int PartitionedHashTableCtx::ExprValuesCache::MemUsage(int capacity, int expr_values_bytes_per_row, int num_exprs) {
...
PartitionedHashTable* PartitionedHashTable::Create(Suballocator* allocator, bool stores_duplicates,
    ...
    return new PartitionedHashTable(config::enable_quadratic_probing, allocator, stores_duplicates, num_build_tuples,
                                    tuple_stream, max_num_buckets, initial_num_buckets);
}

PartitionedHashTable::PartitionedHashTable(bool quadratic_probing, Suballocator* allocator, bool stores_duplicates,
                                           int num_build_tuples, BufferedTupleStream3* stream, int64_t max_num_buckets,
                                           int64_t num_buckets)
...
Status PartitionedHashTable::CheckAndResize(uint64_t buckets_to_fill, const PartitionedHashTableCtx* ht_ctx,
                                            bool* got_memory) {
    uint64_t shift = 0;
...
Status PartitionedHashTable::ResizeBuckets(int64_t num_buckets, const PartitionedHashTableCtx* ht_ctx,
                                           bool* got_memory) {
    DCHECK_EQ((num_buckets & (num_buckets - 1)), 0) << "num_buckets=" << num_buckets << " must be a power of 2";
    ...
    for (PartitionedHashTable::Iterator iter = Begin(ht_ctx); !iter.AtEnd();
        ...
        bool found = false;
        int64_t bucket_idx = Probe<true>(new_buckets, num_buckets, NULL, bucket_to_copy->hash, &found);
        DCHECK(!found);
...
void PartitionedHashTable::DebugStringTuple(std::stringstream& ss, HtData& htdata, const RowDescriptor* desc) {
    if (stores_tuples_) {
        ss << "(" << htdata.tuple << ")";
    } else {
        ss << "(" << htdata.flat_row << ")";
...
string PartitionedHashTable::DebugString(bool skip_empty, bool show_match, const RowDescriptor* desc) {
    std::stringstream ss;
    ss << std::endl;
...
string PartitionedHashTable::PrintStats() const {
    ...
    std::stringstream ss;
    ss << "Buckets: " << num_buckets_ << " " << num_filled_buckets_ << " " << curr_fill_factor << std::endl;
    ss << "Duplicates: " << num_buckets_with_duplicates_ << " buckets " << num_duplicate_nodes_ << " nodes"
       << std::endl;
