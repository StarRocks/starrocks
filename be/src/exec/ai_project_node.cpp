...
Status AIProjectNode::init(const TPlanNode& tnode, RuntimeState* state) {
    ...
    for (const auto& [id, model_config] : thrift_project.ai_model_configs) {
        ...
        if (source == TAIModelSource::SYSTEM) {
            ...
            if (model_config.__isset.provider_id || endpoint.__isset.api_key || endpoint.__isset.timeout_ms ||
            ...
            if (!model_config.__isset.provider_id || !pipeline::is_valid_ai_provider_id(model_config.provider_id) ||
            ...
            // Retired draft sources and unknown sources must never become Provider routes.
            return invalid_ai_project_plan();
        }
        configs.emplace(
                id,
                pipeline::AIProjectModelConfig{
                        ...
                        .source = source,
                        .provider_id = source == TAIModelSource::PROVIDER ? model_config.provider_id : std::string{},
