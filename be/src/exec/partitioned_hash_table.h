...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/partitioned_hash_table.h
...
#ifndef STARROCKS_BE_SRC_EXEC_NEW_PARTITIONED_HASH_TABLE_H
#define STARROCKS_BE_SRC_EXEC_NEW_PARTITIONED_HASH_TABLE_H
...
public:
    ...
    /// of other parameters.
    static Status Create(ObjectPool* pool, RuntimeState* state, const std::vector<Expr*>& build_exprs,
                         const std::vector<Expr*>& probe_exprs, bool stores_nulls, const std::vector<bool>& finds_nulls,
                         int32_t initial_seed, int max_levels, int num_build_tuples, MemPool* mem_pool,
                         MemPool* expr_results_pool, MemTracker* tracker, const RowDescriptor& row_desc,
    ...
    public:
        ...
        Status Init(RuntimeState* state, MemTracker* tracker, const std::vector<Expr*>& build_exprs);
        ...
        /// have been read.
        bool ALWAYS_INLINE AtEnd() const { return cur_expr_values_hash_ == cur_expr_values_hash_end_; }
        ...
        int ALWAYS_INLINE expr_values_offsets(int expr_idx) const { return expr_values_offsets_[expr_idx]; }
    ...
    private:
        ...
        int ALWAYS_INLINE CurIdx() const { return cur_expr_values_hash_ - expr_values_hash_array_.get(); }
        ...
        int expr_values_bytes_per_row_ = 0;
...
private:
    ...
    PartitionedHashTableCtx(const std::vector<Expr*>& build_exprs, const std::vector<Expr*>& probe_exprs,
    ...
    Status Init(ObjectPool* pool, RuntimeState* state, int num_build_tuples, MemTracker* tracker,
    ...
    uint32_t IR_NO_INLINE HashRow(const uint8_t* expr_values, const uint8_t* expr_values_null) const noexcept;
    ...
    /// codegen'd function.
    bool IR_NO_INLINE EvalBuildRow(TupleRow* row, uint8_t* expr_values, uint8_t* expr_values_null) noexcept {
    ...
    bool IR_NO_INLINE EvalProbeRow(TupleRow* row, uint8_t* expr_values, uint8_t* expr_values_null) noexcept {
...
public:
    ...
    ///    with.
    static PartitionedHashTable* Create(Suballocator* allocator, bool stores_duplicates, int num_build_tuples,
                                        BufferedTupleStream3* tuple_stream, int64_t max_num_buckets,
    ...
    bool IR_ALWAYS_INLINE Insert(PartitionedHashTableCtx* ht_ctx, BufferedTupleStream3::FlatRowPtr flat_row,
    ...
    int64_t size() const { return num_filled_buckets_ - num_buckets_with_duplicates_ + num_duplicate_nodes_; }
    ...
    Status CheckAndResize(uint64_t buckets_to_fill, const PartitionedHashTableCtx* ht_ctx, bool* got_memory);
    ...
    public:
        IR_ALWAYS_INLINE Iterator() : table_(NULL), scratch_row_(NULL), bucket_idx_(BUCKET_NOT_FOUND), node_(NULL) {}
...
private:
    ...
    ///    opposed to linear.
    PartitionedHashTable(bool quadratic_probing, Suballocator* allocator, bool stores_duplicates, int num_build_tuples,
                         BufferedTupleStream3* tuple_stream, int64_t max_num_buckets, int64_t initial_num_buckets);
    ...
    template <bool FORCE_NULL_EQUALITY>
    int64_t IR_ALWAYS_INLINE Probe(Bucket* buckets, int64_t num_buckets, PartitionedHashTableCtx* ht_ctx, uint32_t hash,
                                   bool* found);
