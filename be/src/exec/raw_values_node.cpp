...
pipeline::OpFactories RawValuesNode::decompose_to_pipeline(pipeline::PipelineBuilderContext* context) {
    ...
    const auto& dst_tuple_desc = context->fragment_context()->runtime_state()->desc_tbl().get_tuple_descriptor(_tuple_id);
    ...
    auto raw_values_source_op = std::make_shared<RawValuesSourceOperatorFactory>(
            context->next_operator_id(), id(), dst_slots, _constant_type,
            std::move(_long_values), std::move(_string_values));
