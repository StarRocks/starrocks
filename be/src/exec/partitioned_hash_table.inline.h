...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/partitioned_hash_table.inline.h
...
#ifndef STARROCKS_BE_SRC_EXEC_NEW_PARTITIONED_HASH_TABLE_INLINE_H
#define STARROCKS_BE_SRC_EXEC_NEW_PARTITIONED_HASH_TABLE_INLINE_H
...
inline int64_t PartitionedHashTable::Probe(Bucket* buckets, int64_t num_buckets, PartitionedHashTableCtx* ht_ctx,
    ...
    *found = false;
    ...
    do {
        ...
        if (hash == bucket->hash) {
            if (ht_ctx != NULL && ht_ctx->Equals<FORCE_NULL_EQUALITY>(GetRow(bucket, ht_ctx->scratch_row_))) {
                *found = true;
                return bucket_idx;
            }
...
inline PartitionedHashTable::HtData* PartitionedHashTable::InsertInternal(PartitionedHashTableCtx* ht_ctx,
                                                                          Status* status) {
    ++num_probes_;
    bool found = false;
...
inline bool PartitionedHashTable::Insert(PartitionedHashTableCtx* ht_ctx, BufferedTupleStream3::FlatRowPtr flat_row,
...
inline PartitionedHashTable::Iterator PartitionedHashTable::FindProbeRow(PartitionedHashTableCtx* ht_ctx) {
    ++num_probes_;
    bool found = false;
    uint32_t hash = ht_ctx->expr_values_cache()->CurExprValuesHash();
...
inline PartitionedHashTable::Iterator PartitionedHashTable::FindBuildRowBucket(PartitionedHashTableCtx* ht_ctx,
                                                                               bool* found) {
    ++num_probes_;
    uint32_t hash = ht_ctx->expr_values_cache()->CurExprValuesHash();
    int64_t bucket_idx = Probe<true>(buckets_, num_buckets_, ht_ctx, hash, found);
...
inline PartitionedHashTable::Iterator PartitionedHashTable::Begin(const PartitionedHashTableCtx* ctx) {
...
inline PartitionedHashTable::Iterator PartitionedHashTable::FirstUnmatched(PartitionedHashTableCtx* ctx) {
    int64_t bucket_idx = Iterator::BUCKET_NOT_FOUND;
    DuplicateNode* node = NULL;
    NextFilledBucket(&bucket_idx, &node);
...
inline PartitionedHashTable::DuplicateNode* PartitionedHashTable::InsertDuplicateNode(int64_t bucket_idx,
                                                                                      Status* status) {
    DCHECK_GE(bucket_idx, 0);
    DCHECK_LT(bucket_idx, num_buckets_);
    Bucket* bucket = &buckets_[bucket_idx];
...
inline void PartitionedHashTable::Iterator::Next() {
    DCHECK(!AtEnd());
    if (table_->stores_duplicates() && table_->buckets_[bucket_idx_].hasDuplicates && node_->next != NULL) {
        node_ = node_->next;
    } else {
...
inline int64_t PartitionedHashTable::NumInsertsBeforeResize() const {
    return std::max<int64_t>(0, static_cast<int64_t>(num_buckets_ * MAX_FILL_FACTOR) - num_filled_buckets_);
}
