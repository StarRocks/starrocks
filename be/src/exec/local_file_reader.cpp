...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/local_file_reader.cpp
...
int64_t LocalFileReader::size() {
    if (_file_size == -1) {
        ...
        if (ret) {
            LOG(WARNING) << "Get file size is error, errno: " << errno << ", msg " << strerror(errno);
            return -1;
        }
...
Status LocalFileReader::seek(int64_t position) {
    ...
    if (res != 0) {
        char err_buf[64];
        std::stringstream ss;
        ss << "Seek to start_offset failed. offset=" << position << ", error=" << strerror_r(errno, err_buf, 64);
        return Status::InternalError(ss.str());
    }
    return Status::OK();
}
