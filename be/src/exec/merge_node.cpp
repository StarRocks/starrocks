...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/merge_node.cpp
...
Status MergeNode::prepare(RuntimeState* state) {
    ...
    for (int i = 0; i < _const_result_expr_ctx_lists.size(); ++i) {
        RETURN_IF_ERROR(Expr::prepare(_const_result_expr_ctx_lists[i], state, row_desc(), expr_mem_tracker()));
    ...
    for (int i = 0; i < _result_expr_ctx_lists.size(); ++i) {
        RETURN_IF_ERROR(Expr::prepare(_result_expr_ctx_lists[i], state, child(i)->row_desc(), expr_mem_tracker()));
...
Status MergeNode::get_next(RuntimeState* state, RowBatch* row_batch, bool* eos) {
    ...
    if (UNLIKELY(tuple_buffer == nullptr)) {
    ...
    while (_const_result_expr_idx < _const_result_expr_ctx_lists.size()) {
        ...
        eval_and_materialize_exprs(_const_result_expr_ctx_lists[_const_result_expr_idx], true, &tuple, row_batch);
    ...
    while (_child_idx < _children.size()) {
        ...
        if (_child_row_batch.get() == NULL) {
            RETURN_IF_CANCELLED(state);
            _child_row_batch.reset(new RowBatch(child(_child_idx)->row_desc(), state->batch_size(), mem_tracker()));
            // Open child and fetch the first row batch.
            RETURN_IF_ERROR(child(_child_idx)->open(state));
            RETURN_IF_ERROR(child(_child_idx)->get_next(state, _child_row_batch.get(), &_child_eos));
            _child_row_idx = 0;
        }
        ...
        while (true) {
            ...
            if (eval_and_materialize_exprs(_result_expr_ctx_lists[_child_idx], false, &tuple, row_batch)) {
                *eos = reached_limit();
...
bool MergeNode::eval_and_materialize_exprs(const std::vector<ExprContext*>& ctxs, bool const_exprs, Tuple** tuple,
                                           RowBatch* row_batch) {
    ...
    do {
        ...
        for (int i = 0; i < ctxs.size(); ++i) {
            ...
            RawValue::write(ctxs[i]->get_value(child_row), *tuple, slot_desc, row_batch->tuple_data_pool());
        }
