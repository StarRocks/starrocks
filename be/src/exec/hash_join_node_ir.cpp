...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/hash_join_node_ir.cpp
...
// codegen.
int HashJoinNode::process_probe_batch(RowBatch* out_batch, RowBatch* probe_batch, int max_added_rows) {
    ...
    while (true) {
        ...
        if ((!_matched_probe && _match_all_probe) || ((!_matched_probe && _join_op == TJoinOp::LEFT_ANTI_JOIN))) {
