...
Status AvroCppScanner::create_column_readers() {
    if (!TimezoneUtils::find_cctz_time_zone(_state->timezone(), _timezone)) {
        return Status::InternalError(fmt::format("Can not find cctz time zone {}", _state->timezone()));
    }
...
StatusOr<AvroReaderUniquePtr> AvroCppScanner::open_avro_reader(const TBrokerRangeDesc& range_desc) {
    ...
    RETURN_IF_ERROR(avro_reader->init(std::make_unique<AvroBufferInputStream>(file, 1048576, _counter), range_desc.path,
                                      _state, _counter, &_src_slot_descriptors, &_column_readers,
                                      col_not_found_as_null));
