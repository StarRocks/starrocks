...
public:
    ...
    using ProbeKeyData = typename ProbeKeyConstructor::KeyData;
...
private:
    ...
    void _search_ht_impl(RuntimeState* state, const ImmBuffer<CppType> build_data, const ProbeKeyData data);

    // for one key inner join
    template <bool first_probe, bool is_collision_free_and_unique>
    void _probe_from_ht(RuntimeState* state, const ImmBuffer<CppType> build_data, const ProbeKeyData probe_data);
    ...
    void _probe_coroutine(RuntimeState* state, const ImmBuffer<CppType> build_data, const ProbeKeyData probe_data);
    ...
    HashTableProbeState::ProbeCoroutine _probe_from_ht_for_left_outer_join(RuntimeState* state,
                                                                           ...
                                                                           const ProbeKeyData probe_data);
    bool _contains_probe_row(RuntimeState* state, const ImmBuffer<CppType> build_data, const ProbeKeyData probe_data,
