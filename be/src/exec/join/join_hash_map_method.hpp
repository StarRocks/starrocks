...
// -- GermanStringBucketChainedJoinHashMap
template <LogicalType LT>
void GermanStringBucketChainedJoinHashMap<LT>::build_prepare(RuntimeState* state, JoinHashTableItems* table_items) {
    table_items->bucket_size = JoinHashMapHelper::calc_bucket_size(table_items->row_count + 1);
    table_items->log_bucket_size = __builtin_ctz(table_items->bucket_size);
    table_items->first.resize(table_items->bucket_size, 0);
    table_items->next.resize(table_items->row_count + 1, 0);
...
template <LogicalType LT>
void GermanStringBucketChainedJoinHashMap<LT>::construct_hash_table(JoinHashTableItems* table_items,
    ...
    if (is_nulls == nullptr) {
        ...
        const auto* __restrict is_nulls_data = is_nulls->data();
        auto need_calc_bucket_num = [&](const uint32_t index) { return is_nulls_data[index] == 0; };
...
template <LogicalType LT>
void GermanStringBucketChainedJoinHashMap<LT>::lookup_init(const JoinHashTableItems& table_items,
