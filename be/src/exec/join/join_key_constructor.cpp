...
void BuildKeyConstructorForSerialized::build_key(RuntimeState* state, JoinHashTableItems* table_items) {
    ...
    for (size_t i = 0; i < table_items->key_columns.size(); i++) {
        if (table_items->join_keys[i].is_null_safe_equal) {
            ...
            // Probe side wraps a non-nullable null-safe key in NullableColumn so its serialized Slice
            // carries a leading null byte. Mirror that here, otherwise build and probe keys serialize
            // to different byte lengths and the join misses every match on this variable-size path.
            if (col->is_nullable()) {
