...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/olap_scanner.cpp
...
OlapScanner::OlapScanner(RuntimeState* runtime_state, OlapScanNode* parent, bool aggregation, bool need_agg_finalize,
...
Status OlapScanner::prepare(const TInternalScanRange& scan_range, const std::vector<OlapScanRange*>& key_ranges,
...
Status OlapScanner::open() {
    ...
    if (res != OLAP_SUCCESS) {
        ...
        std::stringstream ss;
        ss << "failed to initialize storage reader. tablet=" << _params.tablet->full_name() << ", res=" << res
...
Status OlapScanner::_init_params(const std::vector<OlapScanRange*>& key_ranges, const std::vector<TCondition>& filters,
...
Status OlapScanner::get_batch(RuntimeState* state, RowBatch* batch, bool* eof) {
    ...
    uint8_t* tuple_buf = batch->tuple_data_pool()->allocate(state->batch_size() * _tuple_desc->byte_size());
    ...
    int64_t raw_rows_threshold = raw_rows_read() + config::doris_scanner_row_num;
    {
        ...
        while (true) {
            ...
            auto res = _reader->next_row_with_aggregation(&_read_row_cursor, mem_pool.get(), batch->agg_object_pool(),
                                                          eof);
            if (res != OLAP_SUCCESS) {
                std::stringstream ss;
                    ...
                    if (_num_rows_read > 32768) {
                        ...
                        if (pushdown_return_rate > config::doris_max_pushdown_conjuncts_return_rate) {
                            ...
                            VLOG(2) << "Stop Using PushDown Conjuncts. "
                                    ...
                                    << " MaxPushDownReturnRate: " << config::doris_max_pushdown_conjuncts_return_rate
                                    << "%";
                        }
                    }
                }
...
void OlapScanner::_convert_row_to_tuple(Tuple* tuple) {
    ...
    for (int i = 0; i < slots_size; ++i) {
        ...
        case TYPE_DECIMALV2: {
            if (_tablet->tablet_schema().column(cid).type() == OLAP_FIELD_TYPE_DECIMAL) {
                ...
                // Because memory address is not aligned, use memcopy instead
                DecimalV2Value* slot = tuple->get_decimalv2_slot(slot_desc->tuple_offset());
                memcpy(slot, ptr, sizeof(DecimalV2Value));
            }
            break;
        }
        case TYPE_DATETIME: {
            if (_tablet->tablet_schema().column(cid).type() == OLAP_FIELD_TYPE_DATETIME) {
                ...
                // Must be OLAP_FIELD_TYPE_TIMESTAMP
                // TODO(zc): make it a better implementation
                DateTimeValue* slot = tuple->get_datetime_slot(slot_desc->tuple_offset());
                const vectorized::TimestampValue* value = reinterpret_cast<const vectorized::TimestampValue*>(ptr);
                if (!slot->from_olap_datetime(value->to_timestamp_literal())) {
                    tuple->set_null(slot_desc->null_indicator_offset());
                }
            }
            break;
        ...
        case TYPE_DATE: {
            if (_tablet->tablet_schema().column(cid).type() == OLAP_FIELD_TYPE_DATE) {
                DateTimeValue* slot = tuple->get_datetime_slot(slot_desc->tuple_offset());
                uint64_t value = 0;
                ...
                // must be OLAP_FIELD_TYPE_DATE_V2
                DateTimeValue* slot = tuple->get_datetime_slot(slot_desc->tuple_offset());
                const vectorized::DateValue* value = reinterpret_cast<const vectorized::DateValue*>(ptr);
                if (!slot->from_olap_date(value->to_mysql_date())) {
                    tuple->set_null(slot_desc->null_indicator_offset());
                }
            }
            break;
...
void OlapScanner::update_counter() {
    ...
    COUNTER_UPDATE(_parent->_vec_cond_evaluate_timer, _reader->stats().vec_cond_evaluate_ns);
    COUNTER_UPDATE(_parent->_vec_cond_chunk_copy_timer, _reader->stats().vec_cond_chunk_copy_ns);
