...
Status ShortCircuitHybridScanNode::_process_key_chunk() {
    ...
    _key_chunk = ChunkBuilder::new_chunk(key_schema, _num_rows);
    _key_chunk->reset();
...
Status ShortCircuitHybridScanNode::_process_value_chunk(std::vector<bool>& found) {
    ...
    ChunkPtr value_chunk = ChunkBuilder::new_chunk(*(value_schema), _num_rows);
    // final value_chunk, order match key_chunk
    _value_chunk = ChunkBuilder::new_chunk(*(value_schema), _num_rows);
    ...
    for (int i = 0; i < _tablets.size(); ++i) {
        ...
        auto current_chunk = ChunkBuilder::new_chunk(*(value_schema), _num_rows);
