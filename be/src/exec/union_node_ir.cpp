...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/union_node_ir.cpp
...
void IR_ALWAYS_INLINE UnionNode::materialize_exprs(const std::vector<ExprContext*>& exprs, TupleRow* row,
    ...
    dst_tuple->materialize_exprs<false>(row, *_tuple_desc, exprs, dst_batch->tuple_data_pool(), nullptr, nullptr);
...
void UnionNode::materialize_batch(RowBatch* dst_batch, uint8_t** tuple_buf) {
    ...
    int num_rows_to_process =
            std::min(child_batch->num_rows() - _child_row_idx, dst_batch->capacity() - dst_batch->num_rows());
