...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/partitioned_aggregation_node.cc
...
Status PartitionedAggregationNode::init(const TPlanNode& tnode, RuntimeState* state) {
    ...
    RETURN_IF_ERROR(Expr::create(tnode.agg_node.grouping_exprs, row_desc, state, &grouping_exprs_, mem_tracker()));
    ...
    for (int i = 0; i < grouping_exprs_.size(); ++i) {
        ...
        SlotRef* build_expr = _pool->add(
                desc->type().type != TYPE_NULL ? new SlotRef(desc) : new SlotRef(desc, TypeDescriptor(TYPE_BOOLEAN)));
    ...
    for (int i = 0; i < tnode.agg_node.aggregate_functions.size(); ++i, ++j) {
        ...
        AggFn* agg_fn;
        RETURN_IF_ERROR(AggFn::Create(tnode.agg_node.aggregate_functions[i], row_desc, *intermediate_slot_desc,
                                      *output_slot_desc, state, &agg_fn));
...
Status PartitionedAggregationNode::prepare(RuntimeState* state) {
    ...
    largest_partition_percent_ = runtime_profile()->AddHighWaterMarkCounter("LargestPartitionPercent", TUnit::UNIT);
    ...
    if (is_streaming_preagg_) {
        ...
        preagg_estimated_reduction_ = ADD_COUNTER(runtime_profile(), "ReductionFactorEstimate", TUnit::DOUBLE_VALUE);
        preagg_streaming_ht_min_reduction_ =
                ADD_COUNTER(runtime_profile(), "ReductionFactorThresholdToExpand", TUnit::DOUBLE_VALUE);
    } else {
        ...
        max_partition_level_ = runtime_profile()->AddHighWaterMarkCounter("MaxPartitionLevel", TUnit::UNIT);
    }
    // TODO chenhao
    const RowDescriptor& row_desc = child(0)->row_desc();
    RETURN_IF_ERROR(NewAggFnEvaluator::Create(agg_fns_, state, _pool, agg_fn_pool_.get(), &agg_fn_evals_,
                                              expr_mem_tracker(), row_desc));
    ...
    if (!grouping_exprs_.empty()) {
        ...
        RETURN_IF_ERROR(PartitionedHashTableCtx::Create(
                _pool, state, build_exprs_, grouping_exprs_, true, vector<bool>(build_exprs_.size(), true),
                state->fragment_hash_seed(), MAX_PARTITION_DEPTH, 1, expr_mem_pool(), expr_results_pool_.get(),
...
Status PartitionedAggregationNode::open(RuntimeState* state) {
    ...
    if (grouping_exprs_.empty()) {
        ...
        if (ht_allocator_ == nullptr) {
            ...
            ht_allocator_.reset(new Suballocator(state_->exec_env()->buffer_pool(), &_buffer_pool_client,
                                                 _resource_profile.spillable_buffer_size));

            if (!is_streaming_preagg_ && needs_serialize_) {
                serialize_stream_.reset(new BufferedTupleStream3(state, &intermediate_row_desc_, &_buffer_pool_client,
                                                                 _resource_profile.spillable_buffer_size,
    ...
    do {
        ...
        RETURN_IF_ERROR(state->check_query_state("New partitioned aggregation, while getting next from child 0."));
...
Status PartitionedAggregationNode::HandleOutputStrings(RowBatch* row_batch, int first_row_idx) {
    ...
    for (const AggFn* agg_fn : agg_fns_) {
        ...
        if (is_in_subplan()) {
            ...
            // batches.
            RETURN_IF_ERROR(CopyStringData(slot_desc, row_batch, first_row_idx, row_batch->tuple_data_pool()));
        } else {
...
Status PartitionedAggregationNode::CopyStringData(const SlotDescriptor& slot_desc, RowBatch* row_batch,
...
Status PartitionedAggregationNode::GetNextInternal(RuntimeState* state, RowBatch* row_batch, bool* eos) {
    SCOPED_TIMER(_runtime_profile->total_time_counter());
...
void PartitionedAggregationNode::GetSingletonOutput(RowBatch* row_batch) {
    ...
    Tuple* output_tuple = GetOutputTuple(agg_fn_evals_, singleton_output_tuple_, row_batch->tuple_data_pool());
...
Status PartitionedAggregationNode::GetRowsFromPartition(RuntimeState* state, RowBatch* row_batch) {
    ...
    while (!output_iterator_.AtEnd()) {
        ...
        if ((count++ & (N - 1)) == 0) {
            RETURN_IF_CANCELLED(state);
            RETURN_IF_ERROR(
                    state->check_query_state("New partitioned aggregation, while getting rows from partition."));
        }
        ...
        Tuple* output_tuple =
                GetOutputTuple(output_partition_->agg_fn_evals, intermediate_tuple, row_batch->tuple_data_pool());
...
Status PartitionedAggregationNode::GetRowsStreaming(RuntimeState* state, RowBatch* out_batch) {
    ...
    if (child_batch_ == NULL) {
        child_batch_.reset(new RowBatch(child(0)->row_desc(), state->batch_size(), mem_tracker()));
    }

    do {
        DCHECK_EQ(out_batch->num_rows(), 0);
        RETURN_IF_CANCELLED(state);
        RETURN_IF_ERROR(state->check_query_state("New partitioned aggregation, while getting rows in streaming."));
        ...
        if (ht_needs_expansion && ShouldExpandPreaggHashTables()) {
            for (int i = 0; i < PARTITION_FANOUT; ++i) {
                ...
                if (remaining_capacity[i] < child_batch_->num_rows()) {
                    ...
                    bool resized;
                    RETURN_IF_ERROR(ht->CheckAndResize(child_batch_->num_rows(), ht_ctx_.get(), &resized));
                    if (resized) {
        ...
        if (process_batch_streaming_fn_ != NULL) {
            RETURN_IF_ERROR(process_batch_streaming_fn_(this, needs_serialize_, child_batch_.get(), out_batch,
                                                        ht_ctx_.get(), remaining_capacity));
        } else {
            RETURN_IF_ERROR(ProcessBatchStreaming(needs_serialize_, child_batch_.get(), out_batch, ht_ctx_.get(),
                                                  remaining_capacity));
        }
...
Status PartitionedAggregationNode::Partition::InitStreams() {
    ...
    NewAggFnEvaluator::ShallowClone(parent->partition_pool_.get(), agg_fn_pool.get(), parent->agg_fn_evals_,
                                    &agg_fn_evals);
    ...
    auto agg_slot = parent->intermediate_tuple_desc_->slots().begin() + parent->grouping_exprs_.size();
    ...
    aggregated_row_stream.reset(
            new BufferedTupleStream3(parent->state_, &parent->intermediate_row_desc_, &parent->_buffer_pool_client,
    ...
    if (!parent->is_streaming_preagg_) {
        unaggregated_row_stream.reset(new BufferedTupleStream3(
                ...
                parent->_resource_profile.spillable_buffer_size, parent->_resource_profile.max_row_buffer_size));
...
Status PartitionedAggregationNode::Partition::Spill(bool more_aggregate_rows) {
    ...
    if (more_aggregate_rows) {
        ...
        DCHECK(got_buffer) << "Accounted in min reservation" << parent->_buffer_pool_client.DebugString();
    }
...
Tuple* PartitionedAggregationNode::ConstructSingletonOutputTuple(const vector<NewAggFnEvaluator*>& agg_fn_evals,
                                                                 MemPool* pool) {
    DCHECK(grouping_exprs_.empty());
...
Tuple* PartitionedAggregationNode::ConstructIntermediateTuple(const vector<NewAggFnEvaluator*>& agg_fn_evals,
...
void PartitionedAggregationNode::CopyGroupingValues(Tuple* intermediate_tuple, uint8_t* buffer, int varlen_size) {
    ...
    for (int expr_idx : string_grouping_exprs_) {
        ...
        StringValue* sv = reinterpret_cast<StringValue*>(intermediate_tuple->get_slot(slot_desc->tuple_offset()));
...
void PartitionedAggregationNode::InitAggSlots(const vector<NewAggFnEvaluator*>& agg_fn_evals,
    ...
    for (int i = 0; i < agg_fn_evals.size(); ++i, ++slot_desc) {
        ...
        if ((agg_op == AggFn::MIN || agg_op == AggFn::MAX) && !agg_fn->intermediate_type().is_string_type() &&
...
void PartitionedAggregationNode::UpdateTuple(NewAggFnEvaluator** agg_fn_evals, Tuple* tuple, TupleRow* row,
                                             bool is_merge) {
...
Tuple* PartitionedAggregationNode::GetOutputTuple(const vector<NewAggFnEvaluator*>& agg_fn_evals, Tuple* tuple,
                                                  MemPool* pool) {
...
Status PartitionedAggregationNode::AppendSpilledRow(Partition* partition, TupleRow* row) {
    ...
    BufferedTupleStream3* stream =
            AGGREGATED_ROWS ? partition->aggregated_row_stream.get() : partition->unaggregated_row_stream.get();
...
void PartitionedAggregationNode::DebugString(int indentation_level, stringstream* out) const {
    ...
    *out << "PartitionedAggregationNode("
         << "intermediate_tuple_id=" << intermediate_tuple_id_ << " output_tuple_id=" << output_tuple_id_
         << " needs_finalize=" << needs_finalize_ << " grouping_exprs=" << Expr::debug_string(grouping_exprs_)
...
Status PartitionedAggregationNode::CreateHashPartitions(int level, int single_partition_idx) {
    ...
    if (UNLIKELY(level >= MAX_PARTITION_DEPTH)) {
        ...
        error_msg << "Cannot perform aggregation at hash aggregation node with id " << _id << '.'
                  << " The input data was partitioned the maximum number of " << MAX_PARTITION_DEPTH << " times."
...
Status PartitionedAggregationNode::CheckAndResizeHashPartitions(bool partitioning_aggregated_rows, int num_rows,
...
Status PartitionedAggregationNode::RepartitionSpilledPartition() {
    ...
    int64_t num_input_rows =
            partition->aggregated_row_stream->num_rows() + partition->unaggregated_row_stream->num_rows();
...
Status PartitionedAggregationNode::ProcessStream(BufferedTupleStream3* input_stream) {
    ...
    if (input_stream->num_rows() > 0) {
        ...
        bool eos = false;
        const RowDescriptor* desc = AGGREGATED_ROWS ? &intermediate_row_desc_ : &(_children[0]->row_desc());
...
Status PartitionedAggregationNode::MoveHashPartitions(int64_t num_input_rows) {
    DCHECK(!hash_partitions_.empty());
    std::stringstream ss;
    ss << "PA(node_id=" << id() << ") partitioned(level=" << hash_partitions_[0]->level << ") " << num_input_rows
       << " rows into:" << std::endl;
    for (int i = 0; i < hash_partitions_.size(); ++i) {
        Partition* partition = hash_partitions_[i];
        if (partition == nullptr) continue;
        ...
        ss << "  " << i << " " << (partition->is_spilled() ? "spilled" : "not spilled") << " (fraction=" << std::fixed
