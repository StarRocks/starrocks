...
public:
    ...
    Status init(const OlapTableSinkParams& params);
...
private:
    ...
    void mark_as_failed(const NodeChannel* ch) { _failed_channels.insert(ch->node_id()); }
    bool is_failed_channel(const NodeChannel* ch) { return _failed_channels.count(ch->node_id()) != 0; }
    bool has_intolerable_failure() { return _failed_channels.size() >= ((_num_replicas + 1) / 2); }
    ...
    // Store the output expr compute result column
    vectorized::ChunkPtr _output_chunk = nullptr;
