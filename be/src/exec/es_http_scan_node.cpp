...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/es_http_scan_node.cpp
...
Status EsHttpScanNode::get_next(RuntimeState* state, RowBatch* row_batch, bool* eos) {
    ...
    if (VLOG_ROW_IS_ON) {
        for (int i = 0; i < row_batch->num_rows(); ++i) {
            ...
            VLOG_ROW << "EsHttpScanNode output row: " << Tuple::to_string(row->get_tuple(0), *_tuple_desc);
        }
    }
...
Status EsHttpScanNode::close(RuntimeState* state) {
    ...
    //close exec node
    update_status(ConvertScanNode::close(state));
...
void EsHttpScanNode::scanner_worker(int start_idx, int length, std::promise<Status>& p_status) {
    ...
    std::unique_ptr<EsHttpScanner> scanner(new EsHttpScanner(_runtime_state, runtime_profile(), _tuple_id, properties,
                                                             scanner_expr_ctxs, &counter, doc_value_mode));
    status = scanner_scan(std::move(scanner), scanner_expr_ctxs, &counter);
    if (!status.ok()) {
        LOG(WARNING) << "Scanner[" << start_idx << "] process failed. status=" << status.get_error_msg();
    }
