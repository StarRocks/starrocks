...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/read_write_util.h
...
#ifndef STARROCKS_BE_SRC_QUERY_EXEC_READ_WRITE_UTIL_H
#define STARROCKS_BE_SRC_QUERY_EXEC_READ_WRITE_UTIL_H
...
inline int64_t ReadWriteUtil::get_long_int(const uint8_t* buf) {
    return (static_cast<int64_t>(buf[0]) << 56) | (static_cast<int64_t>(buf[1]) << 48) |
           (static_cast<int64_t>(buf[2]) << 40) | (static_cast<int64_t>(buf[3]) << 32) | (buf[4] << 24) |
