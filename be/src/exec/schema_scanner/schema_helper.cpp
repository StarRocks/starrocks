...
Status SchemaHelper::get_temporary_tables_info(const std::string& ip, const int32_t port, const TGetTemporaryTablesInfoRequest& request,
        TGetTemporaryTablesInfoResponse* response, const int timeout_ms) {
    return ThriftRpcHelper::rpc<FrontendServiceClient>(
        ip, port, [&request, &response](FrontendServiceConnection& client) {
