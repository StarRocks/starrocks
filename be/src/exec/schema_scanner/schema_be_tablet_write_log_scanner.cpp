...
Status SchemaBeTabletWriteLogScanner::get_next(ChunkPtr* chunk, bool* eos) {
    ...
    *chunk = ChunkHelper::new_chunk(_slot_descs, _runtime_state->chunk_size());
    ...
    for (; i < _runtime_state->chunk_size() && _log_index < _logs.size(); ++i, ++_log_index) {
        ...
        // be_id
        {
            ColumnHelper::get_data_column((*chunk)->get_column_by_index(col_idx++).get())
                    ->append_datum(Datum(log.backend_id));
        }
        // begin_time
        {
            DateTimeValue ts;
            ts.from_unixtime(log.begin_time / 1000, _ctz); // Convert ms to seconds
            fill_column_with_slot<TYPE_DATETIME>((*chunk)->get_column_by_index(col_idx++).get(), (void*)&ts);
        }
        // finish_time
        {
            DateTimeValue ts;
        ...
        {
            ColumnHelper::get_data_column((*chunk)->get_column_by_index(col_idx++).get())
                    ->append_datum(Datum(log.txn_id));
        }
        // tablet_id
        {
            ColumnHelper::get_data_column((*chunk)->get_column_by_index(col_idx++).get())
                    ->append_datum(Datum(log.tablet_id));
        }
        // table_id
        {
            ColumnHelper::get_data_column((*chunk)->get_column_by_index(col_idx++).get())
                    ->append_datum(Datum(log.table_id));
        }
        // partition_id
        {
            ColumnHelper::get_data_column((*chunk)->get_column_by_index(col_idx++).get())
                    ->append_datum(Datum(log.partition_id));
        }
        // log_type
        {
            std::string type_str = (log.log_type == lake::LogType::LOAD) ? "LOAD" : "COMPACTION";
            Slice s(type_str);
            ColumnHelper::get_data_column((*chunk)->get_column_by_index(col_idx++).get())->append_datum(Datum(s));
        }
        // input_rows
        {
            ColumnHelper::get_data_column((*chunk)->get_column_by_index(col_idx++).get())
                    ->append_datum(Datum(log.input_rows));
        }
        // input_bytes
        {
            ColumnHelper::get_data_column((*chunk)->get_column_by_index(col_idx++).get())
                    ->append_datum(Datum(log.input_bytes));
        }
        // output_rows
        {
            ColumnHelper::get_data_column((*chunk)->get_column_by_index(col_idx++).get())
                    ->append_datum(Datum(log.output_rows));
        }
        // output_bytes
        {
            ColumnHelper::get_data_column((*chunk)->get_column_by_index(col_idx++).get())
                    ->append_datum(Datum(log.output_bytes));
        }
        // input_segments
        {
            auto& column = (*chunk)->get_column_by_index(col_idx++);
            if (log.log_type == lake::LogType::COMPACTION) {
                column->append_datum(Datum(log.input_segments));
            } else {
        ...
        {
            ColumnHelper::get_data_column((*chunk)->get_column_by_index(col_idx++).get())
                    ->append_datum(Datum(log.output_segments));
        }
        // label
        {
            auto& column = (*chunk)->get_column_by_index(col_idx++);
        ...
        // compaction_score
        {
            auto& column = (*chunk)->get_column_by_index(col_idx++);
            if (log.log_type == lake::LogType::COMPACTION) {
                column->append_datum(Datum(log.compaction_score));
            } else {
                column->append_nulls(1);
            }
        }
