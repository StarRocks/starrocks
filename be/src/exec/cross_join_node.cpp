...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/cross_join_node.cpp
...
Status CrossJoinNode::construct_build_side(RuntimeState* state) {
    ...
    while (true) {
        RowBatch* batch =
                _build_batch_pool->add(new RowBatch(child(1)->row_desc(), state->batch_size(), mem_tracker()));
...
Status CrossJoinNode::get_next(RuntimeState* state, RowBatch* output_batch, bool* eos) {
    ...
    *eos = false;
    ...
    while (!_eos) {
        ...
        _num_rows_returned += process_left_child_batch(output_batch, _left_batch.get(), max_added_rows);
...
int CrossJoinNode::process_left_child_batch(RowBatch* output_batch, RowBatch* batch, int max_added_rows) {
