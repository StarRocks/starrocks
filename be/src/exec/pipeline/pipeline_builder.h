...
public:
    ...
    /// Local shuffle by key columns identified by slot id; rows whose key
    /// contains NULL are spread round-robin (see ColumnHashPartitioner).
    OpFactories interpolate_local_column_hash_partition_exchange(RuntimeState* state, int32_t plan_node_id,
                                                                 OpFactories& pred_operators,
                                                                 const std::vector<int32_t>& key_slot_ids,
                                                                 int num_receivers);
