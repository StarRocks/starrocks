...
#include "util/time.h"
    ...
    StatusOr<LookUpTaskPtr> LookUpProcessor::_create_task(const LookUpTaskContextPtr& ctx) {
        ...
        default:
            return Status::InternalError("unknown row position descriptor type: " +
                                         std::to_string(row_pos_desc->type()));
        }
    }
    ...
    bool LookUpOperator::is_finished() const { return _is_finished && _num_running_io_tasks == 0; }

    bool LookUpOperator::pending_finish() const { return !is_finished(); }
