...
// IndexSeekOperator
class IndexSeekOperator final : public SourceOperator {
public:
    IndexSeekOperator(OperatorFactory* factory, int32_t id,
...
private:
    ...
    Status _init_tablet_reader(RuntimeState* state, TabletReaderState& reader_params);
    Status _init_reader_params(RuntimeState* state, TabletReaderState& reader_params);
    Status _init_conjuncts_manager(RuntimeState* state, TabletReaderState& reader_params);
    Status _init_conjunct_ctxs(TabletReaderState& reader_params, uint32_t row_id);
    ...
    vectorized::ChunkPtr _permute_output_chunk(RuntimeState* state);
...
private:
    ...
    std::vector<LookupJoinKeyDesc> _join_key_descs;
    uint32_t _row_id{0};
...
class IndexSeekOperatorFactory final : public SourceOperatorFactory {
public:
    ...
    OperatorPtr create(int32_t degree_of_parallelism, int32_t driver_sequence) override {
        return std::make_shared<IndexSeekOperator>(this, _id,
