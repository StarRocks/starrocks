...
Status PartitionSortSinkOperatorFactory::prepare(RuntimeState* state) {
    ...
    RETURN_IF_ERROR(ExecExecutor::prepare(_analytic_partition_exprs, state));
    RETURN_IF_ERROR(ExecExecutor::open(_analytic_partition_exprs, state));
    return Status::OK();
}
...
void PartitionSortSinkOperatorFactory::close(RuntimeState* state) {
    ExecExecutor::close(_analytic_partition_exprs, state);
