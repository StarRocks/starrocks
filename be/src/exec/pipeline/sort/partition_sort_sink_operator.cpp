...
OperatorPtr PartitionSortSinkOperatorFactory::create(int32_t dop, int32_t driver_sequence) {
    ...
    if (_limit >= 0) {
        if (_topn_type == TTopNType::ROW_NUMBER && _limit <= ChunksSorter::USE_HEAP_SORTER_LIMIT_SZ) {
            ...
            chunks_sorter = ChunksSorterTopn::create(runtime_state(), &(_sort_exec_exprs.lhs_ordering_expr_ctxs()),
                                                     &_is_asc_order, &_is_null_first, _sort_keys, 0, _limit + _offset,
                                                     _topn_type, max_buffered_chunks);
        }
    } else {
