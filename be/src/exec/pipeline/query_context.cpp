...
void QueryContextManager::report_fragments_with_same_host(const std::vector<std::shared_ptr<FragmentContext>>& need_report_fragment_context,
    std::vector<bool>& reported, const TNetworkAddress& last_coord_addr, std::vector<TReportExecStatusParams>& report_exec_status_params_vector,
...
void QueryContextManager::report_fragments(const std::vector<PipeLineReportTaskKey>& pipeline_need_report_query_fragment_ids) {
    ...
    for (int i = 0; i < need_report_fragment_context.size(); i++) {
        if (reported[i] == false) {
            ...
            FrontendServiceConnection fe_connection(exec_env->frontend_client_cache(), 
                    fe_addr, &fe_connection_status);
            ...
            report_fragments_with_same_host(need_report_fragment_context, reported, fe_addr, report_exec_status_params_vector, cur_batch_report_indexes);
    ...
    for (const auto& key: fragment_context_non_exist) {
        StorageEngine::instance()->unregister_pipeline_load(key.query_id, key.fragment_instance_id);
    }
}
