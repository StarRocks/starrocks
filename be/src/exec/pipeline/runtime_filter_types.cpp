...
StatusOr<bool> PartialRuntimeFilterMerger::add_partial_filters(
        size_t idx, size_t ht_row_count, size_t ht_ndv, RuntimeInFilters&& partial_in_filters,
...
Status PartialRuntimeFilterMerger::merge_local_in_filters() {
    ...
    // Gate by estimated distinct keys, not rows: an IN-filter pushed to storage is bounded by its number
    // of distinct values. Local-shuffle partitions are disjoint, so per-builder NDVs sum to the merged IN
    // cardinality; a chained estimate is a lower bound, so this cap bounds pushdown cost, not correctness.
    ...
    for (auto i = 0; i < _ht_row_counts.size(); ++i) {
        ...
        // empty in-filter list is generated by non-empty hash tables whose estimated distinct count
        // exceeds the pushdown cap; in-filters can not be merged.
        if (in_filters.empty()) {
        ...
        num_distinct += _ht_ndv[i];
    }

    can_merge_in_filters =
            can_merge_in_filters && (num_distinct <= config::max_pushdown_conditions_per_column) && k >= 0;
...
Status PartialRuntimeFilterMerger::merge_singleton_local_bloom_filters() {
    ...
    // Size and gate by estimated distinct keys (NDV) rather than row count: bloom/bitset size and the byte
    // caps are driven by distinct keys. Local-shuffle partitions are disjoint, so per-instance NDVs sum.
    const size_t ndv_total = std::accumulate(_ht_ndv.begin(), _ht_ndv.end(), size_t{0});
    ...
    const bool maybe_use_bitset_filter =
            _enable_join_runtime_bitset_filter && is_version_v3 && ndv_total <= _global_rf_limit &&
    ...
    for (size_t i = 0; i < num_bloom_filters; ++i) {
        ...
        // skip if the estimated distinct count exceeds the limit, and it's only for local.
        if (!desc->has_remote_targets() && ndv_total > _local_rf_limit) continue;
        ...
        if (maybe_use_bitset_filter && _partial_bloom_filter_build_params[0][i].has_value()) {
            ...
            rf = RuntimeFilterFactory::create_join_filter(_pool, build_type, desc->join_mode(), param.columns,
                                                          kHashJoinKeyColumnOffset, ndv_total);
        } else {
        ...
        // set BF paramaters
        if (desc->has_remote_targets() && ndv_total > _global_rf_limit) {
            if (!is_version_v3) {
                rf->get_membership_filter()->clear_bf();
            } else {
                rf = RuntimeFilterFactory::to_empty_filter(_pool, rf);
                rf->get_membership_filter()->init(ndv_total);
            }
        } else {
            rf->get_membership_filter()->init(ndv_total);
        }
...
Status PartialRuntimeFilterMerger::merge_multi_partitioned_local_bloom_filters() {
    ...
    // Size and gate by estimated distinct keys (NDV), not rows; disjoint partitions sum.
    size_t ndv_total = 0;
    for (auto ndv : _ht_ndv) {
        ndv_total += ndv;
    }
