...
Status IcebergTableSinkOperator::push_chunk(RuntimeState* state, const ChunkPtr& chunk) {
    ...
    if (_writers.find(partition_location) == _writers.end()) {
        ...
        _writers.emplace(partition_location, writer);
        LOG(WARNING) << "==========[insert writer on [" << partition_location << "]=============== size["
                     << chunk->num_rows() << "]==";
    }

    _writers[partition_location]->append_chunk(chunk.get(), state);
...
Status IcebergTableSinkOperatorFactory::prepare(RuntimeState* state) {
    ...
    if (_file_format == "parquet") {
        for (const auto& field : t_iceberg_schema->fields) {
            field_ids.emplace_back(field.field_id);
        }
        _parquet_file_schema =
                ParquetBuilder::get_schema(_iceberg_table->full_column_names(), field_ids, _output_expr_ctxs);
    }
...
[[maybe_unused]] static void add_iceberg_commit_info(starrocks::parquet::AsyncFileWriter* writer, RuntimeState* state) {
    ...
    stats.__set_columnSizes(std::map<int32_t, int64_t>());
    stats.__set_valueCounts(std::map<int32_t, int64_t>());
    stats.__set_nullValueCounts(std::map<int32_t, int64_t>());
    stats.__set_lowerBounds(std::map<int32_t, std::string>());
    stats.__set_upperBounds(std::map<int32_t, std::string>());
    ...
    state->update_num_rows_load_from_sink(iceberg_data_file.record_count);
}
