...
class IcebergTableMergeSinkOperator final : public Operator {
public:
    IcebergTableMergeSinkOperator(OperatorFactory* factory, int32_t id, int32_t plan_node_id, int32_t driver_sequence,
...
public:
    IcebergTableMergeSinkOperatorFactory(int32_t id, FragmentContext* fragment_ctx, vector<TExpr> t_output_expr,
                                    IcebergTableDescriptor* iceberg_table,
                                    const TIcebergTableSink& t_iceberg_table_sink,
                                    std::vector<ExprContext*> partition_expr_ctxs);

    ~IcebergTableMergeSinkOperatorFactory() override = default;
