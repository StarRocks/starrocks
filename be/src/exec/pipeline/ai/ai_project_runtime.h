...
struct AIProjectExpressionOutput {
    SlotId slot_id = 0;
...
private:
    ...
    AIProjectExpressionProjection(std::vector<AIProjectExpressionOutput> outputs, std::vector<SlotId> common_slot_ids,
                                  std::vector<ExprContext*> common_expr_ctxs, std::string default_model);

    void _close_contexts(RuntimeState* state, std::vector<DriverExpressionContexts>* drivers) noexcept;

    const std::vector<AIProjectExpressionOutput> _outputs;
    const std::vector<SlotId> _common_slot_ids;
    const std::vector<ExprContext*> _common_expr_ctxs;
    const std::string _default_model;
