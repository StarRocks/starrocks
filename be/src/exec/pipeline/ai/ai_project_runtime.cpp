...
StatusOr<std::shared_ptr<AIProjectExpressionProjection>> AIProjectExpressionProjection::create(
        std::vector<AIProjectExpressionOutput> outputs, std::vector<SlotId> common_slot_ids,
        std::vector<ExprContext*> common_expr_ctxs, std::string default_model) {
    if (outputs.empty() || common_slot_ids.size() != common_expr_ctxs.size()) {
    ...
    try {
        ...
        slot_ids.reserve(outputs.size() + common_slot_ids.size());
        contexts.reserve(outputs.size() + common_expr_ctxs.size());

        for (size_t index = 0; index < common_slot_ids.size(); ++index) {
            ExprContext* context = common_expr_ctxs[index];
            if (context == nullptr || context->root() == nullptr || contains_ai_expression(context->root()) ||
                !slot_ids.emplace(common_slot_ids[index]).second || !contexts.emplace(context).second) {
                return invalid_projection();
            }
        }

        for (const AIProjectExpressionOutput& output : outputs) {
        ...
        if (requires_default_model && contains_only_ascii_whitespace(default_model)) {
            return invalid_projection();
        }

        return std::shared_ptr<AIProjectExpressionProjection>(new AIProjectExpressionProjection(
                std::move(outputs), std::move(common_slot_ids), std::move(common_expr_ctxs), std::move(default_model)));
    } catch (const std::bad_alloc&) {
        return Status::MemoryLimitExceeded("Failed to allocate AI project expression projection");
    }
}

AIProjectExpressionProjection::AIProjectExpressionProjection(std::vector<AIProjectExpressionOutput> outputs,
                                                             std::vector<SlotId> common_slot_ids,
                                                             std::vector<ExprContext*> common_expr_ctxs,
                                                             std::string default_model)
        : _outputs(std::move(outputs)),
          _common_slot_ids(std::move(common_slot_ids)),
...
Status AIProjectExpressionProjection::prepare(RuntimeState* state, int32_t dop) {
    if (state == nullptr || dop <= 0 || state->obj_pool() == nullptr) {
    ...
    try {
        output_expr_ctxs.reserve(_outputs.size());
        output_slot_ids.reserve(_outputs.size());
        common_expr_ctxs = _common_expr_ctxs;
        for (const AIProjectExpressionOutput& output : _outputs) {
        ...
        if (_lifecycle_status.ok()) {
            DictOptimizeParser::set_output_slot_id(&common_expr_ctxs, _common_slot_ids);
...
void AIProjectExpressionProjection::_close_contexts(RuntimeState* state,
    ...
    for (const AIProjectExpressionOutput& output : _outputs) {
        try {
            ExprExecutor::close(output.expr_ctx, state);
        } catch (...) {
        }
    }
    close_expr_contexts_noexcept(_common_expr_ctxs, state);
}
...
StatusOr<AIProjectPreparedSubchunk> AIProjectExpressionProjection::prepare_subchunk(RuntimeState* state,
    ...
    try {
        ...
        prepared.ai_outputs.reserve(_outputs.size());
        const size_t rows = input->num_rows();

        if (rows == 0) {
            for (size_t index = 0; index < _outputs.size(); ++index) {
                const AIProjectExpressionOutput& output = _outputs[index];
                MutableColumnPtr column = ColumnHelper::create_column(driver->outputs[index]->root()->type(),
                                                                      output.is_ai || output.nullable);
        ...
        for (size_t index = 0; index < _common_slot_ids.size(); ++index) {
            ...
            input->append_column(std::move(column), _common_slot_ids[index]);
        ...
        for (size_t index = 0; index < _outputs.size(); ++index) {
            ...
            ASSIGN_OR_RETURN(AIFunctionInputBatch batch,
                             ai_expression->build_input_batch(context, input.get(), _default_model));
