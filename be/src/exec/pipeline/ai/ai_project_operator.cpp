...
AIBufferSinkOperator::AIBufferSinkOperator(OperatorFactory* factory, int32_t id, int32_t plan_node_id,
                                           int32_t driver_sequence, std::shared_ptr<AIProjectProcessor> processor)
        : Operator(factory, id, "ai_buffer_sink", plan_node_id, false, driver_sequence),
          _processor(std::move(processor)) {}

Status AIBufferSinkOperator::prepare(RuntimeState* state) {
...
void AIBufferSinkOperator::close(RuntimeState* state) {
...
bool AIBufferSinkOperator::pending_finish() const {
    // Async work captures the fragment-shared processor, never this sink.
    // The paired source owns the fragment-lifetime barrier.
    return false;
}

StatusOr<ChunkPtr> AIBufferSinkOperator::pull_chunk(RuntimeState*) {
    return Status::InternalError("Shouldn't pull chunk from AI buffer sink operator");
}

Status AIBufferSinkOperator::push_chunk(RuntimeState*, const ChunkPtr& chunk) {
    if (!_status.ok()) {
        return _status;
    }
    if (_is_finishing || _is_finished) {
        return Status::InternalError("Cannot push a chunk after AI buffer sink finishing");
    }
    if (chunk == nullptr) {
        return Status::InvalidArgument("Cannot push a null chunk into AI buffer sink");
    }
    if (_pending_chunk != nullptr) {
        return Status::InternalError("Cannot push a chunk while AI buffer sink has pending input");
    }
...
Status AIBufferSinkOperator::set_finishing(RuntimeState*) {
    if (_is_finished) {
        return _status;
    }
    _is_finishing = true;
...
Status AIBufferSinkOperator::set_finished(RuntimeState*) {
    return _force_finish();
}

Status AIBufferSinkOperator::set_cancelled(RuntimeState*) {
    return _force_finish();
}

Status AIBufferSinkOperator::_advance_pending() const {
    if (!_status.ok() || _is_finished) {
        return _status;
    }
...
Status AIBufferSinkOperator::_record_status(const Status& status) const {
    if (status.ok()) {
        return Status::OK();
    }
...
Status AIBufferSinkOperator::_force_finish() const {
    if (_is_finished) {
        return _status;
    }
    _is_finishing = true;
...
AIBufferSinkOperatorFactory::AIBufferSinkOperatorFactory(int32_t id, int32_t plan_node_id,
                                                         std::shared_ptr<AIProjectProcessor> processor)
        : OperatorFactory(id, "ai_buffer_sink", plan_node_id), _processor(std::move(processor)) {}

OperatorPtr AIBufferSinkOperatorFactory::create(int32_t degree_of_parallelism, int32_t driver_sequence) {
    static_cast<void>(_processor->configure(degree_of_parallelism));
    return std::make_shared<AIBufferSinkOperator>(this, _id, _plan_node_id, driver_sequence, _processor);
}
