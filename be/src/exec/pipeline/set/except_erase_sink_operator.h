...
public:
    ExceptEraseSinkOperatorFactory(int32_t id, int32_t plan_node_id, std::shared_ptr<ExceptContext> except_ctx,
                                   const std::vector<ExprContext*>& dst_exprs)
    ...
    OperatorPtr create(int32_t degree_of_parallelism, int32_t driver_sequence) override {
        _except_ctx->create_one_erase_driver();
        return std::make_shared<ExceptEraseSinkOperator>(_id, _plan_node_id, _except_ctx, _dst_exprs);
    }
