...
public:
    RawValuesSourceOperator(OperatorFactory* factory, int32_t id, int32_t plan_node_id, int32_t driver_sequence,
                                   const std::vector<SlotDescriptor*>& dst_slots, const TypeDescriptor& constant_type,
...
private:
    ...
    const size_t _start_index;  // Starting index in the values array for this operator instance
    const size_t _rows_total;   // Total number of rows this operator should produce
    size_t _next_processed_row_index;  // Index of next row to process (relative to start_index)
};

class RawValuesSourceOperatorFactory final : public SourceOperatorFactory {
public:
    RawValuesSourceOperatorFactory(int32_t id, int32_t plan_node_id, 
    ...
    OperatorPtr create(int32_t degree_of_parallelism, int32_t driver_sequence) override {
        ...
        return std::make_shared<RawValuesSourceOperator>(
                this, _id, _plan_node_id, driver_sequence, _dst_slots, _constant_type,
                long_values_ptr, string_values_ptr, start_index, rows_count);
    }
