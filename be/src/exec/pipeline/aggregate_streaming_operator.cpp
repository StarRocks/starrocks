...
bool AggregateStreamingOperator::has_output() {
    ...
    // There are two cases where _curr_chunk is null, need to wait all the input chunks
...
Status AggregateStreamingOperator::push_chunk(RuntimeState* state, const vectorized::ChunkPtr& chunk) {
    ...
    if (_streaming_preaggregation_mode == TStreamingPreaggregationMode::FORCE_STREAMING) {
        ...
        return _push_chunk_by_force_preaggregation(chunk);
    } else {
        return _push_chunk_by_auto(chunk);
    }
}
...
Status AggregateStreamingOperator::_push_chunk_by_force_preaggregation(const vectorized::ChunkPtr& chunk) {
    size_t chunk_size = chunk->num_rows();
...
#undef HASH_MAP_METHOD
    ...
    (this->*_compute_agg_states)(chunk_size);
...
Status AggregateStreamingOperator::_push_chunk_by_auto(const vectorized::ChunkPtr& chunk) {
    size_t chunk_size = chunk->num_rows();
