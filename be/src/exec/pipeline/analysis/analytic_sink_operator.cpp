...
Status AnalyticSinkOperator::prepare(RuntimeState* state) {
    ...
    if (!_tnode.analytic_node.__isset.window) {
        _partition_finish = &AnalyticSinkOperator::_partition_finish_for_unbounded_frame;
        ...
        if (!window.__isset.window_end) {
            ...
            _partition_finish = &AnalyticSinkOperator::_partition_finish_for_unbounded_preceding_range_frame;
        }
    } else {
        if (!window.__isset.window_start && !window.__isset.window_end) {
            ...
            _partition_finish = &AnalyticSinkOperator::_partition_finish_for_unbounded_preceding_rows_frame;
        } else {
            _partition_finish = &AnalyticSinkOperator::_partition_finish_for_sliding_frame;
        }
    }
...
Status AnalyticSinkOperator::_maybe_partition_finish() {
    while (_analytor->has_output()) {
        ...
        (this->*_partition_finish)(chunk_size, is_new_partition);
...
void AnalyticSinkOperator::_partition_finish_for_unbounded_frame(size_t chunk_size, bool is_new_partition) {
    if (is_new_partition) {
...
void AnalyticSinkOperator::_partition_finish_for_unbounded_preceding_range_frame(size_t chunk_size,
...
void AnalyticSinkOperator::_partition_finish_for_unbounded_preceding_rows_frame(size_t chunk_size,
                                                                                bool is_new_partition) {
    while (_analytor->current_row_position() < _analytor->partition_end() &&
           _analytor->window_result_position() < chunk_size) {
        _analytor->update_window_batch(_analytor->partition_start(), _analytor->partition_end(),
...
void AnalyticSinkOperator::_partition_finish_for_sliding_frame(size_t chunk_size, bool is_new_partition) {
    while (_analytor->current_row_position() < _analytor->partition_end() &&
           _analytor->window_result_position() < chunk_size) {
