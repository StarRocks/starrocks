...
Status TableFunctionOperator::_process_table_function(RuntimeState* state) {
    ...
    // A table function (e.g. unnest) materializes the whole expansion of the input
    // chunk in this single call, with no intermediate memory-limit check. For wide
    // arrays this one step can allocate tens of GB and get the BE OOM-killed before
    // the next pipeline-driver limit check fires. Wrap it so allocations are checked
    // against the query/query_pool/process mem tracker per batch so that a runaway
    // query fails with MemoryLimitExceeded instead of crashing the whole BE.
