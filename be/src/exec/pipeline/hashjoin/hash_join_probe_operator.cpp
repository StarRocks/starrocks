...
bool HashJoinProbeOperator::need_input() const {
    ...
    if (_join_builder->is_build_done() && _join_prober != _join_builder) {
...
Status HashJoinProbeOperator::push_chunk(RuntimeState* state, const ChunkPtr& chunk) {
    RETURN_IF_ERROR(_reference_join_builder_once());
...
Status HashJoinProbeOperator::_reference_join_builder_once() {
    if (_join_prober == _join_builder) {
        return Status::OK();
    }
