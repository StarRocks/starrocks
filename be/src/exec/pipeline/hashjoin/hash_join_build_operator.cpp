...
HashJoinBuildOperator::HashJoinBuildOperator(OperatorFactory* factory, int32_t id, const string& name,
                                             int32_t plan_node_id, HashJoinerPtr build_hash_joiner,
                                             const std::vector<HashJoinerPtr>& only_probe_hash_joiners,
        ...
        : Operator(factory, id, name, plan_node_id),
          ...
          _only_probe_hash_joiners(only_probe_hash_joiners),
...
Status HashJoinBuildOperator::push_chunk(RuntimeState* state, const vectorized::ChunkPtr& chunk) {
    return _build_hash_joiner->append_chunk_to_ht(state, chunk);
}
...
Status HashJoinBuildOperator::prepare(RuntimeState* state) {
    ...
    for (auto& probe_hash_joiner : _only_probe_hash_joiners) {
        probe_hash_joiner->ref();
    }

    return _build_hash_joiner->prepare(state);
}
Status HashJoinBuildOperator::close(RuntimeState* state) {
    for (auto& probe_hash_joiner : _only_probe_hash_joiners) {
        RETURN_IF_ERROR(probe_hash_joiner->unref(state));
    }
...
void HashJoinBuildOperator::set_finishing(RuntimeState* state) {
    _is_finished = true;
    _build_hash_joiner->build_ht(state);
    ...
    _build_hash_joiner->create_runtime_filters(state);

    auto ht_row_count = _build_hash_joiner->get_ht_row_count();
    auto& partial_in_filters = _build_hash_joiner->get_runtime_in_filters();
    auto& partial_bloom_filter_build_params = _build_hash_joiner->get_runtime_bloom_filter_build_params();
    auto& partial_bloom_filters = _build_hash_joiner->get_runtime_bloom_filters();
    ...
    for (auto& probe_hash_joiner : _only_probe_hash_joiners) {
        probe_hash_joiner->reference_hash_table(_build_hash_joiner.get());
    }
    _build_hash_joiner->enter_probe_phase();
    for (auto& probe_hash_joiner : _only_probe_hash_joiners) {
        probe_hash_joiner->enter_probe_phase();
    }
}
...
OperatorPtr HashJoinBuildOperatorFactory::create(int32_t degree_of_parallelism, int32_t driver_sequence) {
    return std::make_shared<HashJoinBuildOperator>(this, _id, _name, _plan_node_id,
                                                   ...
                                                   _hash_joiner_factory->get_only_probe_hash_joiners(), driver_sequence,
