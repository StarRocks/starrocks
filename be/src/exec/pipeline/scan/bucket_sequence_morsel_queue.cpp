...
void BucketSequenceMorselQueue::set_key_ranges(const std::vector<std::unique_ptr<OlapScanRange>>& key_ranges) {
    _olap_morsel_queue()->set_key_ranges(key_ranges);
}

void BucketSequenceMorselQueue::set_key_ranges(const TabletReaderParams::RangeStartOperation& range_start_op,
...
void BucketSequenceMorselQueue::set_tablets(const std::vector<BaseTabletSharedPtr>& tablets) {
    _olap_morsel_queue()->set_tablets(tablets);
}

void BucketSequenceMorselQueue::set_tablet_rowsets(
        const std::vector<std::vector<BaseRowsetSharedPtr>>& tablet_rowsets) {
    _olap_morsel_queue()->set_tablet_rowsets(tablet_rowsets);
}

void BucketSequenceMorselQueue::set_ticket_checker(const query_cache::TicketCheckerPtr& ticket_checker) {
...
void BucketSequenceMorselQueue::set_tablet_schema(const TabletSchemaCSPtr& tablet_schema) {
...
OlapMorselQueue* BucketSequenceMorselQueue::_olap_morsel_queue() const {
    auto* queue = dynamic_cast<OlapMorselQueue*>(_morsel_queue.get());
