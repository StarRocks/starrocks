...
// but in the next round, if there is 5 drivers assigned scan ranges, then we have to expand vector to 5.
static void ensure_size_of_queue_per_drive_seq(std::vector<MorselQueuePtr>& queue_per_driver_seq, int driver_seq) {
    int size = queue_per_driver_seq.size();
    if (driver_seq >= size) {
        for (int i = 0; i < (driver_seq - size) + 1; i++) {
            queue_per_driver_seq.emplace_back(create_empty_morsel_queue());
        }
    }
}
