...
int ConnectorScanOperator::available_pickup_morsel_count() {
    ...
    P.gen_chunks_time = (now - P.op_start_time - P.cs_total_halt_time);
    ...
    double source_speed = _op_pull_chunks * 1e4 / (P.gen_chunks_time + 1);
    ...
    double cs_total_scan_bytes = P.cs_total_scan_bytes.load() / (1024 * 1024);
    double io_latency = cs_total_io_time / (cs_total_scan_bytes + 1e-3);

    // adjust routines.
    auto do_add_io_tasks = [&]() { io_tasks += config::connector_io_tasks_adjust_step; };
    auto do_sub_io_tasks = [&]() { io_tasks -= config::connector_io_tasks_adjust_step; };
    ...
    [[maybe_unused]] auto build_debug_string = [&]() {
        ...
        ss << ", cs = " << doround(source_speed) << "(" << _op_pull_chunks << "/" << (P.gen_chunks_time * 1e-4) << ")";
