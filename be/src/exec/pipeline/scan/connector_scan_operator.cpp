...
int ConnectorScanOperator::available_pickup_morsel_count() {
    ...
    auto moving_exp = [&](int64_t value) {
        // const double MOVING_EXP = 0.95;
        // return int64_t(value * MOVING_EXP + 0.5);
        return value;
    };

    // adjust io tasks according information collected
    int64_t cs_gen_chunks_time = (now - P.op_start_time - P.cs_total_halt_time);
    int64_t delta = cs_gen_chunks_time - P.cs_gen_chunks_time;
    P.cs_gen_chunks_time = delta + moving_exp(P.cs_gen_chunks_time);
...
Status ConnectorChunkSource::_read_chunk(RuntimeState* state, ChunkPtr* chunk) {
    ConnectorScanOperatorAdaptiveProcessor& P = *(_op->_adaptive_processor);

    DeferOp op([&]() { P.last_chunk_souce_finish_timestamp = GetCurrentTimeMicros(); });
