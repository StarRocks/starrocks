...
Status OlapChunkSource::_init_olap_reader(RuntimeState* runtime_state) {
    ...
    starrocks::vectorized::Schema child_schema = ChunkHelper::convert_schema(tablet_schema, reader_columns);
    ...
    if (reader_columns.size() == scanner_columns.size()) {
        ...
        starrocks::vectorized::Schema output_schema = ChunkHelper::convert_schema(tablet_schema, scanner_columns);
