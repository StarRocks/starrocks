...
Status HashPartitionContext::push_one_chunk_to_partitioner(RuntimeState* state, const vectorized::ChunkPtr& chunk) {
...
StatusOr<vectorized::ChunkPtr> HashPartitionContext::pull_one_chunk(RuntimeState* state) {
    if (!_chunks_partitioner->is_hash_map_eos()) {
        _chunks_partitioner->consume_from_hash_map([&](int32_t partition_idx, vectorized::ChunkPtr& chunk) {
