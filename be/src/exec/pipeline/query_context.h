...
public:
    ...
    void incr_sent_bytes(int64_t sent_bytes) { _total_sent_bytes += sent_bytes; }
    ...
    int64_t get_sent_bytes() { return _total_sent_bytes; }
    ...
    std::shared_ptr<QueryStatistics> intermediate_query_statistic(int64_t delta_sent_bytes,
                                                                  const TUniqueId& fragment_id, const std::string& log);
...
private:
    ...
    std::atomic<int64_t> _total_sent_bytes = 0;
