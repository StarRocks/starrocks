...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/partitioned_aggregation_node_ir.cc
...
template <bool AGGREGATED_ROWS>
void IR_ALWAYS_INLINE PartitionedAggregationNode::EvalAndHashPrefetchGroup(RowBatch* batch, int start_row_idx,
...
template <bool AGGREGATED_ROWS>
Status PartitionedAggregationNode::AddIntermediateTuple(Partition* partition, TupleRow* row, uint32_t hash,
    ...
    while (true) {
        ...
        Tuple* intermediate_tuple = ConstructIntermediateTuple(
                partition->agg_fn_evals, partition->aggregated_row_stream.get(), &process_batch_status_);
...
Status PartitionedAggregationNode::ProcessBatchStreaming(bool needs_serialize, RowBatch* in_batch, RowBatch* out_batch,
                                                         PartitionedHashTableCtx* ht_ctx,
    ...
    for (int group_start = 0; group_start < num_rows; group_start += cache_size) {
        ...
        FOREACH_ROW_LIMIT(in_batch, group_start, cache_size, in_batch_iter) {
            ...
            if (!expr_vals_cache->IsRowNull() &&
                !TryAddToHashTable(ht_ctx, hash_partitions_[partition_idx], GetHashTable(partition_idx), in_row, hash,
                ...
                Tuple* intermediate_tuple =
                        ConstructIntermediateTuple(agg_fn_evals_, out_batch->tuple_data_pool(), &process_batch_status_);
...
bool PartitionedAggregationNode::TryAddToHashTable(PartitionedHashTableCtx* ht_ctx, Partition* partition,
    ...
    if (found) {
        ...
        intermediate_tuple =
                ConstructIntermediateTuple(partition->agg_fn_evals, partition->aggregated_row_stream.get(), status);
        if (LIKELY(intermediate_tuple != NULL)) {
...
template Status PartitionedAggregationNode::ProcessBatch<false>(RowBatch*, PartitionedHashTableCtx*);
