...
StatusOr<LogicalType> JDBCScanner::_precheck_data_type(const std::string& java_class, SlotDescriptor* slot_desc) {
    ...
    if (java_class == "java.lang.Short") {
        ...
        if (type != TYPE_DATETIME) {
            return Status::NotSupported(fmt::format(
                    "Type mismatches on column[{}], JDBC result type is oracle Timestamp, please set the type to datetime",
                    slot_desc->col_name()));
        }
        return TYPE_VARCHAR;
        ...
        if (type != TYPE_DATETIME) {
            return Status::NotSupported(fmt::format(
                    "Type mismatches on column[{}], JDBC result type is oracle Timestamp ltz, please set the type to datetime",
                    slot_desc->col_name()));
        }
        return TYPE_VARCHAR;
        ...
        if (type != TYPE_DATETIME) {
            return Status::NotSupported(fmt::format(
                    "Type mismatches on column[{}], JDBC result type is oracle Timestamp tz, please set the type to datetime",
                    slot_desc->col_name()));
        }
        return TYPE_VARCHAR;
    } else {
