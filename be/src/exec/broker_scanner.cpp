...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/broker_scanner.cpp
...
BrokerScanner::BrokerScanner(RuntimeState* state, RuntimeProfile* profile, const TBrokerScanRangeParams& params,
...
Status BrokerScanner::open() {
    RETURN_IF_ERROR(FileScanner::open()); // base default function
...
Status BrokerScanner::create_decompressor(TFileFormatType::type type) {
    ...
    case TFileFormatType::FORMAT_JSON:
        compress_type = CompressionTypePB::NO_COMPRESSION;
        break;
    case TFileFormatType::FORMAT_CSV_GZ:
        compress_type = CompressionTypePB::GZIP;
        break;
    case TFileFormatType::FORMAT_CSV_BZ2:
        compress_type = CompressionTypePB::BZIP2;
        break;
    case TFileFormatType::FORMAT_CSV_LZ4_FRAME:
        compress_type = CompressionTypePB::LZ4_FRAME;
        break;
    case TFileFormatType::FORMAT_CSV_DEFLATE:
        compress_type = CompressionTypePB::DEFLATE;
        break;
    case TFileFormatType::FORMAT_CSV_ZSTD:
        compress_type = CompressionTypePB::ZSTD;
        break;
    default: {
        std::stringstream ss;
...
Status BrokerScanner::open_line_reader() {
    ...
    case TFileFormatType::FORMAT_CSV_DEFLATE:
    case TFileFormatType::FORMAT_CSV_ZSTD:
        _cur_line_reader = new PlainTextLineReader(_profile, _cur_file_reader, _cur_decompressor, size, _row_delimiter);
        break;
    default: {
        std::stringstream ss;
...
bool BrokerScanner::check_decimal_input(const Slice& slice, int precision, int scale, std::stringstream* error_msg) {
...
bool BrokerScanner::line_to_src_tuple(const Slice& line) {
    ...
    if (values.size() + columns_from_path.size() < _src_slot_descs.size()) {
        ...
        error_msg << "actual column number is less than schema column number. "
                  << "actual number: " << values.size() << " sep: " << _column_separator << ", "
