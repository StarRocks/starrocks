...
ExpectedTabletsByPartition TabletSinkSender::_expected_tablets_by_partition() const {
    ...
    for (const auto& [partition_id, logs] : _txn_log_map) {
        ...
        for (const auto& index : it->second->indexes) {
            ...
            // On this branch TOlapTableIndexTablets carries the id list in `tablets`; the rename to
            // `tablet_ids` (with `tablets` becoming a struct list) is main-only.
            tablets.insert(index.tablets.begin(), index.tablets.end());
        }
        if (!tablets.empty()) {
