...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/broker_reader.cpp
...
Status BrokerReader::open() {
    ...
    if (response.opStatus.statusCode != TBrokerOperationStatusCode::OK) {
        std::stringstream ss;
        ss << "Open broker reader failed, broker:" << broker_addr << " failed:" << response.opStatus.message;
        LOG(WARNING) << ss.str();
...
Status BrokerReader::readat(int64_t position, int64_t nbytes, int64_t* bytes_read, void* out) {
    ...
    if (response.opStatus.statusCode == TBrokerOperationStatusCode::END_OF_FILE) {
        ...
        *bytes_read = 0;
        ...
        std::stringstream ss;
        ss << "Read from broker failed, broker:" << broker_addr << " failed:" << response.opStatus.message;
        LOG(WARNING) << ss.str();
        return Status::InternalError(ss.str());
    }
...
void BrokerReader::close() {
    ...
    try {
        ...
        LOG(WARNING) << "Close broker reader failed, broker:" << broker_addr << " failed:" << e.what();
        return;
    }

    if (response.statusCode != TBrokerOperationStatusCode::OK) {
        LOG(WARNING) << "Open broker reader failed, broker:" << broker_addr << " failed:" << response.message;
        return;
    }
    _is_fd_valid = false;
}
