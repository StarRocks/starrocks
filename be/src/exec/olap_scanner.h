...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/olap_scanner.h
...
#ifndef STARROCKS_BE_SRC_QUERY_EXEC_OLAP_SCANNER_H
#define STARROCKS_BE_SRC_QUERY_EXEC_OLAP_SCANNER_H
...
class OlapScanner {
public:
    OlapScanner(RuntimeState* runtime_state, OlapScanNode* parent, bool aggregation, bool need_agg_finalize,
                const TInternalScanRange& scan_range, const std::vector<OlapScanRange*>& key_ranges);

    ~OlapScanner();

    Status prepare(const TInternalScanRange& scan_range, const std::vector<OlapScanRange*>& key_ranges,
...
private:
    Status _init_params(const std::vector<OlapScanRange*>& key_ranges, const std::vector<TCondition>& filters,
