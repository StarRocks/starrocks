...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/json_scanner.cpp
...
JsonScanner::JsonScanner(RuntimeState* state, RuntimeProfile* profile, const TBrokerScanRangeParams& params,
...
Status JsonScanner::open_next_reader() {
    ...
    case TFileType::FILE_BROKER: {
        BrokerReader* broker_reader =
                new BrokerReader(_state->exec_env(), _broker_addresses, _params.properties, range.path, start_offset);
...
////// class JsonReader
JsonReader::JsonReader(RuntimeState* state, ScannerCounter* counter, RuntimeProfile* profile, FileReader* file_reader,
...
void JsonReader::_close() {
    ...
    if (typeid(*_file_reader) == typeid(starrocks::BrokerReader) ||
        typeid(*_file_reader) == typeid(starrocks::LocalFileReader)) {
        _file_reader->close();
        delete _file_reader;
    }
    _closed = true;
...
Status JsonReader::_parse_json_doc(bool* eof) {
    ...
    std::unique_ptr<uint8_t[]> json_str = nullptr;
    size_t length = 0;
    ...
    if (_parsed_json_root.size() != 0) {
        _json_doc = JsonFunctions::get_json_object_from_parsed_json(_parsed_json_root, &_origin_json_doc,
...
void JsonReader::_fill_slot(Tuple* tuple, SlotDescriptor* slot_desc, MemPool* mem_pool, const uint8_t* value,
                            int32_t len) {
...
void JsonReader::_write_data_to_tuple(rapidjson::Value::ConstValueIterator value, SlotDescriptor* desc, Tuple* tuple,
    ...
    case rapidjson::Type::kStringType:
        ...
        _fill_slot(tuple, desc, tuple_pool, (uint8_t*)str_value, value->GetStringLength());
        break;
...
void JsonReader::_set_tuple_value(rapidjson::Value& objectValue, Tuple* tuple,
                                  const std::vector<SlotDescriptor*>& slot_descs, MemPool* tuple_pool, bool* valid) {
...
bool JsonReader::_write_values_by_jsonpath(rapidjson::Value& objectValue, MemPool* tuple_pool, Tuple* tuple,
                                           const std::vector<SlotDescriptor*>& slot_descs) {
    int nullcount = 0;
    bool valid = true;
    size_t column_num = slot_descs.size();
    size_t jsonpath_num = _parsed_jsonpaths.size();
    ...
    for (size_t i = 0; i < column_num; i++) {
        ...
        rapidjson::Value* json_values = JsonFunctions::get_json_array_from_parsed_json(
 ...
 */
Status JsonReader::_handle_nested_complex_json(Tuple* tuple, const std::vector<SlotDescriptor*>& slot_descs,
                                               MemPool* tuple_pool, bool* eof) {
    while (true) {
        Status st = _parse_json_doc(eof);
        if (st.is_data_quality_error()) {
 ...
 *      value2     30
 */
Status JsonReader::_handle_flat_array_complex_json(Tuple* tuple, const std::vector<SlotDescriptor*>& slot_descs,
                                                   MemPool* tuple_pool, bool* eof) {
    do {
