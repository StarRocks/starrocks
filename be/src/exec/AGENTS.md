...
- Treat existing targets such as `ExecPrimitive`, `ExecSinkCore`, `ExecSchemaScannerCore`, `ExecSchemaScanners`, `ExecJoinCore`, and the sorting/spill slices now owned by `ComputeEnv` as the model for future extractions.
