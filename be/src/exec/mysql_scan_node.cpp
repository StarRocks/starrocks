...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/mysql_scan_node.cpp
...
MysqlScanNode::MysqlScanNode(ObjectPool* pool, const TPlanNode& tnode, const DescriptorTbl& descs)
        : ConvertScanNode(pool, tnode, descs),
          _is_init(false),
...
Status MysqlScanNode::prepare(RuntimeState* state) {
    ...
    RETURN_IF_ERROR(ConvertScanNode::prepare(state));
    // get tuple desc
    ...
    // get mysql info
    const MySQLTableDescriptor* mysql_table = static_cast<const MySQLTableDescriptor*>(_tuple_desc->table_desc());
...
Status MysqlScanNode::write_text_slot(char* value, int value_length, SlotDescriptor* slot, RuntimeState* state) {
    if (!_text_converter->write_slot(slot, _tuple, value, value_length, true, false, _tuple_pool.get())) {
        if (slot->is_nullable()) {
...
Status MysqlScanNode::close(RuntimeState* state) {
    ...
    return ConvertScanNode::close(state);
}
