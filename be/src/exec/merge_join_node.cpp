...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/merge_join_node.cpp
...
Status MergeJoinNode::init(const TPlanNode& tnode, RuntimeState* state) {
    ...
    RETURN_IF_ERROR(
            Expr::create_expr_trees(_pool, tnode.merge_join_node.other_join_conjuncts, &_other_join_conjunct_ctxs));
    return Status::OK();
}
...
Status MergeJoinNode::prepare(RuntimeState* state) {
    ...
    RETURN_IF_ERROR(Expr::prepare(_left_expr_ctxs, state, child(0)->row_desc(), expr_mem_tracker()));
    RETURN_IF_ERROR(Expr::prepare(_right_expr_ctxs, state, child(1)->row_desc(), expr_mem_tracker()));
    ...
    _left_child_ctx.reset(new ChildReaderContext(row_desc(), state->batch_size(), state->instance_mem_tracker()));
    _right_child_ctx.reset(new ChildReaderContext(row_desc(), state->batch_size(), state->instance_mem_tracker()));
...
Status MergeJoinNode::get_input_row(RuntimeState* state, int child_idx) {
    ...
    while (!ctx->is_eos && ctx->row_idx >= ctx->batch.num_rows()) {
        ...
        if (child_idx == 0) {
            _left_child_ctx.reset(new ChildReaderContext(child(child_idx)->row_desc(), state->batch_size(),
                                                         state->instance_mem_tracker()));
            ctx = _left_child_ctx.get();
        } else {
            _right_child_ctx.reset(new ChildReaderContext(child(child_idx)->row_desc(), state->batch_size(),
                                                          state->instance_mem_tracker()));
            ctx = _right_child_ctx.get();
        }
...
void MergeJoinNode::debug_string(int indentation_level, std::stringstream* out) const {
    ...
    *out << "MergeJoin(eos=" << (_eos ? "true" : "false")
         ...
         << " _right_child_pos=" << (_right_child_ctx.get() ? _right_child_ctx->row_idx : -1) << " join_conjuncts=";
    *out << "Conjunct(";
