...
Status HdfsScanNode::init(const TPlanNode& tnode, RuntimeState* state) {
    ...
    if (tnode.hdfs_scan_node.__isset.min_max_conjuncts) {
        RETURN_IF_ERROR(Expr::create_expr_trees(_pool, tnode.hdfs_scan_node.min_max_conjuncts, &_min_max_conjunct_ctxs,
                                                state->batch_size()));
    }

    if (tnode.hdfs_scan_node.__isset.partition_conjuncts) {
        RETURN_IF_ERROR(Expr::create_expr_trees(_pool, tnode.hdfs_scan_node.partition_conjuncts,
                                                &_partition_conjunct_ctxs, state->batch_size()));
...
Status HdfsScanNode::_start_scan_thread(RuntimeState* state) {
    ...
    _chunks_per_scanner = config::doris_scanner_row_num / runtime_state()->batch_size();
    _chunks_per_scanner += static_cast<int>(config::doris_scanner_row_num % runtime_state()->batch_size() != 0);
...
void HdfsScanNode::_fill_chunk_pool(int count) {
    ...
    for (int i = 0; i < count; i++) {
        auto chunk = ChunkHelper::new_chunk(*_tuple_desc, runtime_state()->batch_size());
