...
AggregateBase::AggregateBase(const TPlanNode& tnode)
        : _tnode(tnode),
...
AggregateBase::~AggregateBase() = default;

Status AggregateBase::_init(const TPlanNode& tnode, RuntimeState* state) {
    RETURN_IF_ERROR(Expr::create_expr_trees(get_obj_pool0(), tnode.agg_node.grouping_exprs, &_group_by_expr_ctxs));
    // add profile attributes
    if (tnode.agg_node.__isset.sql_grouping_keys) {
        get_runtime_profile0()->add_info_string("GroupingKeys", tnode.agg_node.sql_grouping_keys);
    }
    if (tnode.agg_node.__isset.sql_aggregate_functions) {
        get_runtime_profile0()->add_info_string("AggregateFunctions", tnode.agg_node.sql_aggregate_functions);
    }
    ...
    for (int i = 0; i < agg_size; ++i) {
        ...
        for (int j = 0; j < desc.nodes[0].num_children; ++j) {
            ...
            RETURN_IF_ERROR(
                    Expr::create_tree_from_thrift(get_obj_pool0(), desc.nodes, nullptr, &node_idx, &expr, &ctx));
...
Status AggregateBase::_prepare(RuntimeState* state) {
    _get_results_timer = ADD_TIMER(get_runtime_profile0(), "GetResultsTime");
    _iter_timer = ADD_TIMER(get_runtime_profile0(), "ResultIteratorTime");
    _agg_append_timer = ADD_TIMER(get_runtime_profile0(), "ResultAggAppendTime");
    _group_by_append_timer = ADD_TIMER(get_runtime_profile0(), "ResultGroupByAppendTime");
    //TODO: split agg_compute_timer to cunstruct_ht_time + agg_func_compute_time
    _agg_compute_timer = ADD_TIMER(get_runtime_profile0(), "AggComputeTime");
    _streaming_timer = ADD_TIMER(get_runtime_profile0(), "StreamingTime");
    _expr_compute_timer = ADD_TIMER(get_runtime_profile0(), "ExprComputeTime");
    _expr_release_timer = ADD_TIMER(get_runtime_profile0(), "ExprReleaseTime");

    _input_row_count = ADD_COUNTER(get_runtime_profile0(), "InputRowCount", TUnit::UNIT);
    _hash_table_size = ADD_COUNTER(get_runtime_profile0(), "HashTableSize", TUnit::UNIT);
    _pass_through_row_count = ADD_COUNTER(get_runtime_profile0(), "PassThroughRowCount", TUnit::UNIT);

    SCOPED_TIMER(get_runtime_profile0()->total_time_counter());
    ...
    _mem_pool = std::make_unique<MemPool>(get_memtracker0());
    ...
    if (_group_by_expr_ctxs.empty()) {
        _compute_agg_states = &AggregateBase::_compute_single_agg_state;
    } else {
        _compute_agg_states = &AggregateBase::_compute_batch_agg_states;
    }

    // determine serialize or finalize function
    if (_needs_finalize) {
        _serialize_or_finalize = &AggregateBase::_finalize_to_chunk;
    } else {
        _serialize_or_finalize = &AggregateBase::_serialize_to_chunk;
    }
    return Status::OK();
}

Status AggregateBase::_close(RuntimeState* state) {
    if (is_closed0()) {
        return Status::OK();
    }
...
#undef HASH_MAP_METHOD
    ...
    get_memtracker0()->release(_last_agg_func_memory_usage);
    get_memtracker0()->release(_last_ht_memory_usage);
...
void AggregateBase::_evaluate_const_columns(int i) {
...
void AggregateBase::_init_agg_hash_variant(HashVariantType& hash_variant) {
...
void AggregateBase::_compute_single_agg_state(size_t chunk_size) {
...
void AggregateBase::_compute_batch_agg_states(size_t chunk_size) {
    for (size_t i = 0; i < _agg_fn_ctxs.size(); i++) {
        if (!_is_merge_funcs[i]) {
...
void AggregateBase::_compute_batch_agg_states(size_t chunk_size, const std::vector<uint8_t>& selection) {
    for (size_t i = 0; i < _agg_fn_ctxs.size(); i++) {
...
void AggregateBase::_evaluate_exprs(Chunk* chunk) {
    {
...
Columns AggregateBase::_create_agg_result_columns() {
...
Columns AggregateBase::_create_group_by_columns() {
...
void AggregateBase::_convert_to_chunk_no_groupby(ChunkPtr* chunk) {
    ...
    increase_num_rows_returned0(1);
    *chunk = std::move(result_chunk);
    _is_finished = true;
}

void AggregateBase::_serialize_to_chunk(ConstAggDataPtr state, const Columns& agg_result_columns) {
    for (size_t i = 0; i < _agg_fn_ctxs.size(); i++) {
...
void AggregateBase::_finalize_to_chunk(ConstAggDataPtr state, const Columns& agg_result_columns) {
    for (size_t i = 0; i < _agg_fn_ctxs.size(); i++) {
...
void AggregateBase::_output_chunk_by_streaming(ChunkPtr* chunk) {
    ChunkPtr result_chunk = std::make_shared<Chunk>();
    ...
    increase_num_rows_returned0(result_chunk->num_rows());
    *chunk = std::move(result_chunk);
    COUNTER_SET(_pass_through_row_count, _num_pass_through_rows);
}

void AggregateBase::_output_chunk_by_streaming(ChunkPtr* chunk, const std::vector<uint8_t>& filter) {
...
Status AggregateBase::_check_hash_map_memory_usage(RuntimeState* state) {
    if ((_num_input_rows & memory_check_batch_size) < config::vector_chunk_size) {
        ...
        get_memtracker0()->consume(delta_memory_usage);
        ...
        get_memtracker0()->consume(agg_func_memory_usage - _last_agg_func_memory_usage);
...
Status AggregateBase::_check_hash_set_memory_usage(RuntimeState* state) {
    if ((_num_input_rows & memory_check_batch_size) < config::vector_chunk_size) {
...
void AggregateBase::_try_convert_to_two_level_map() {
...
bool AggregateBase::_should_expand_preagg_hash_tables(size_t input_chunk_size, int64_t ht_mem, int64_t ht_rows) const {
    ...
    const int64_t input_rows = get_prev_num_rows_returned0() - input_chunk_size;
    const int64_t aggregated_input_rows = input_rows - get_num_rows_returned0();
