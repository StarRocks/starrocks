...
/// Two execute engine (vectorized engine and pipeline engine)
/// will coexist for a long time, so we need to extract
/// the common parts, so that code can be reused
/// TODO(hcf) This class should be removed as soon as vectorized engine goes offline
class AggregateBase {
protected:
    AggregateBase(const TPlanNode& tnode);
    ~AggregateBase();

    /// this following methods' implementation vary from vectorized engine and pipeline engine
    virtual MemTracker* get_memtracker0() = 0;
    virtual int64_t get_num_rows_returned0() const = 0;
    // get num rows returned of pre node/operator
    virtual int64_t get_prev_num_rows_returned0() const = 0;
    virtual int64_t increase_num_rows_returned0(int increment) = 0;
    virtual RuntimeProfile::Counter* get_rows_returned_counter0() = 0;
    virtual int64_t get_limit0() = 0;
    virtual bool reached_limit0() = 0;
    virtual ObjectPool* get_obj_pool0() = 0;
    virtual bool is_closed0() = 0;
    virtual RuntimeProfile* get_runtime_profile0() = 0;

    Status _init(const TPlanNode& tnode, RuntimeState* state);
    Status _prepare(RuntimeState* state);
    ...
    void _convert_hash_map_to_chunk(HashMapWithKey& hash_map_with_key, int32_t chunk_size, ChunkPtr* chunk) {
        ...
        increase_num_rows_returned0(read_index);
    ...
    void _process_limit(ChunkPtr* chunk) {
        if (reached_limit0()) {
            int64_t num_rows_over = get_num_rows_returned0() - get_limit0();
            (*chunk)->set_num_rows((*chunk)->num_rows() - num_rows_over);
            COUNTER_SET(get_rows_returned_counter0(), get_limit0());
            _is_finished = true;
            LOG(INFO) << "Aggregate Node ReachedLimit " << get_limit0();
        }
    }
    ...
    void (AggregateBase::*_serialize_or_finalize)(ConstAggDataPtr state, const Columns& agg_result_columns) = nullptr;

    void (AggregateBase::*_compute_agg_states)(size_t chunk_size) = nullptr;
