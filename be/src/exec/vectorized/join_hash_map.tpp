    ...
    if (!null_columns.empty()) {
        for (size_t i = 0; i < quo; i++) {
            _build_nullable_columns(table_items, probe_state, data_columns, null_columns,
                                    1 + state->batch_size() * i, state->batch_size());
        }
        _build_nullable_columns(table_items, probe_state, data_columns, null_columns,
                                1 + state->batch_size() * quo, rem);
    } else {
        for (size_t i = 0; i < quo; i++) {
            _build_columns(table_items, probe_state, data_columns,
                           1 + state->batch_size() * i, state->batch_size());
        }
        _build_columns(table_items, probe_state, data_columns, 1 + state->batch_size() * quo,
                       rem);
    }
    return Status::OK();
}
...
void JoinHashMap<PT, BuildFunc, ProbeFunc>::_search_ht_remain(RuntimeState* state) {
    ...
    for (; i < _probe_state->build_match_index.size(); i++) {
        if (_probe_state->build_match_index[i] == 0) {
            ...
            if (match_count >= state->batch_size()) {
                i++;
...
#define RETURN_IF_CHUNK_FULL()                                   \
    if (match_count > state->batch_size()) {               \
        ...
        _probe_state->count = state->batch_size();         \
        return;                                                  \
    }
...
#define PROCESS_PROBE_STAGE_FOR_RIGHT_JOIN_WITH_OTHER_CONJUNCT()                             \
    if constexpr (!first_probe) {                                                            \
        _probe_state->probe_index[0] = _probe_state->probe_index[state->batch_size()]; \
        _probe_state->build_index[0] = _probe_state->build_index[state->batch_size()]; \
        match_count = 1;                                                                     \
...
void JoinHashMap<PT, BuildFunc, ProbeFunc>::_probe_from_ht_for_left_outer_join_with_other_conjunct(RuntimeState* state, 
    ...
    if constexpr (!first_probe) {
        ...
        _probe_state->cur_row_match_count = 0;
        for (size_t j = 0; j < state->batch_size(); j++) {
