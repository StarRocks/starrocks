...
static Status decode_datetime_min_max(PrimitiveType ptype, const orc::proto::ColumnStatistics& colStats,
                                      int64_t offset_in_seconds, const ColumnPtr& min_col, const ColumnPtr& max_col) {
    if (colStats.has_timestampstatistics() && colStats.timestampstatistics().has_minimumutc() &&
        ...
        const cctz::time_zone utc_tzinfo = cctz::utc_time_zone();
        {
            ...
            orc_ts_to_native_ts(&min, utc_tzinfo, offset_in_seconds, secs, ns);
        }

        {
            ...
            int64_t secs = ms / 1000;
            ns += (ms - secs * 1000) * 1000000L;
            orc_ts_to_native_ts(&max, utc_tzinfo, offset_in_seconds, secs, ns);
        }
...
Status OrcScannerAdapter::decode_min_max_value(SlotDescriptor* slot, const orc::proto::ColumnStatistics& stats,
                                               ColumnPtr min_col, ColumnPtr max_col, int64_t offset_in_seconds) {
    if (slot->is_nullable()) {
    ...
    case PrimitiveType::TYPE_DATETIME:
        return decode_datetime_min_max(ptype, stats, offset_in_seconds, min_col, max_col);
