...
public:
    ...
    virtual Status build(const std::string& timezone, std::vector<ExprContext*> conjunct_ctxs, std::string& file_path,
                         int64_t file_length) = 0;
};
...
public:
    ParquetEqualityDeleteBuilder(FileSystem* fs, std::vector<SlotDescriptor*>& materialize_slots)
            : _fs(fs), _materialize_slots(materialize_slots) {}
    ~ParquetEqualityDeleteBuilder() override = default;

    Status build(const std::string& timezone, std::vector<ExprContext*> conjunct_ctxs, std::string& file_path,
                 int64_t file_length) override;
...
public:
    ...
    void build_parquet(const std::string& timezone, TIcebergDeleteFile* delete_file) {
        switch (delete_file->file_content) {
        case TIcebergFileContent::EQUALITY_DELETES:
            (new ParquetEqualityDeleteBuilder(_fs, _materialize_slots))
                    ->build(timezone, _conjunct_ctxs, delete_file->full_path, delete_file->length);
            break;
