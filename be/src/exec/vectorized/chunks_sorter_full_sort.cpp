...
ChunkPtr ChunksSorterFullSort::_late_materialize(const ChunkPtr& chunk) {
    auto use_64bit_ordinal = (_chunk_idx_bits + _offset_in_chunk_bits) > 32;
    if (use_64bit_ordinal) {
...
template <typename T>
ChunkPtr ChunksSorterFullSort::_late_materialize_tmpl(const ChunkPtr& sorted_eager_chunk) {
    static_assert(type_is_ordinal<T>, "T must be uint32_t or uint64_t");
