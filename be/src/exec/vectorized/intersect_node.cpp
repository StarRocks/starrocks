...
Status IntersectNode::get_next(RuntimeState* state, ChunkPtr* chunk, bool* eos) {
    ...
    *eos = false;
    ...
    _remained_keys.resize(runtime_state()->batch_size());
    while (_hash_set_iterator != _hash_set->end() && read_index < runtime_state()->batch_size()) {
    ...
    if (read_index > 0) {
        ...
        for (size_t i = 0; i < _types.size(); ++i) {
            ...
            result_columns[i]->reserve(runtime_state()->batch_size());
        }
    ...
    DCHECK_LE(result_chunk->num_rows(), runtime_state()->batch_size());
