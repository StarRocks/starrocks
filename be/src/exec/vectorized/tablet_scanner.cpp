...
<<<<<<< HEAD:be/src/exec/vectorized/tablet_scanner.cpp
    PredicateParser parser(_tablet->tablet_schema());
    std::vector<PredicatePtr> preds;
    RETURN_IF_ERROR(_parent->_conjuncts_manager.get_column_predicates(&parser, &preds));
=======
>>>>>>> bea3d6a39 (Change storage chunk size when no predicate for non-pipeline engine (#20809)):be/src/exec/tablet_scanner.cpp
    for (auto& p : preds) {
