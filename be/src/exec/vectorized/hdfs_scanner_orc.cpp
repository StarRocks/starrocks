...
Status HdfsOrcScanner::do_get_next(RuntimeState* runtime_state, ChunkPtr* chunk) {
    ...
    for (;;) {
        ...
        if (_orc_adapter->get_cvb_size() != 0) {
            ...
            _file_read_param.append_not_exised_columns_to_chunk(&temp_chunk, temp_chunk->num_rows());
            _file_read_param.append_partition_column_to_chunk(&temp_chunk, temp_chunk->num_rows());
