...
Status CrossJoinNode::get_next_internal(RuntimeState* state, ChunkPtr* chunk, bool* eos,
    ...
    *chunk = nullptr;
    ...
    for (;;) {
        ...
        size_t row_count = runtime_state()->batch_size() - (*chunk)->num_rows();
        ...
        if ((*chunk)->num_rows() < runtime_state()->batch_size()) {
            continue;
        }
...
Status CrossJoinNode::_build(RuntimeState* state) {
    ...
    if (_build_chunk != nullptr) {
        ...
        _build_chunks_size = (_number_of_build_rows / runtime_state()->batch_size()) * runtime_state()->batch_size();
    }
...
void CrossJoinNode::_init_chunk(ChunkPtr* chunk) {
    ...
    *chunk = std::move(new_chunk);
    (*chunk)->reserve(runtime_state()->batch_size());
}
