...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/set_operation_node.cpp
...
void SetOperationNode::create_output_row(TupleRow* input_row, RowBatch* row_batch, uint8_t* tuple_buf) {
    ...
    dst_tuple->materialize_exprs<false>(input_row, *_tuple_desc, exprs, row_batch->tuple_data_pool(), nullptr, nullptr);
...
Status SetOperationNode::open(RuntimeState* state) {
    ...
    _hash_tbl.reset(new HashTable(_child_expr_lists[0], _child_expr_lists[1], _build_tuple_size, true, _find_nulls,
                                  id(), mem_tracker(), 1024));
    ...
    while (!eos) {
        ...
        for (int i = 0; i < build_batch.num_rows(); ++i) {
            VLOG_ROW << "build row: " << get_row_output_string(build_batch.get_row(i), child(0)->row_desc());
