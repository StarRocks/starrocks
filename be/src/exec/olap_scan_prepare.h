...
template <typename E>
concept ExprContainer = std::is_same_v<E, RawExprContainer> || std::is_same_v<E, ExprContextContainer>;

template <ExprContainer E>
class ChunkPredicateBuilder {
public:
    enum class Type : uint8_t { AND, OR };

    ChunkPredicateBuilder(const OlapScanConjunctsManagerOptions& opts, Type type, std::vector<E> exprs,
                          bool allow_partial_normalized);

    StatusOr<bool> parse_conjuncts();

    StatusOr<ChunkPredicatePtr> get_chunk_predicate(PredicateParser* parser);
...
private:
    ...
    StatusOr<std::vector<ColumnPredicatePtr>> _get_column_predicates(PredicateParser* parser);
...
private:
    ...
    ChunkPredicateBuilder<ExprContextContainer> _root_builder;
};
