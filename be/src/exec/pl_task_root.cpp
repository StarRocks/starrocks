...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/pl_task_root.cpp
...
Status ExchangeNode::prepare(RuntimeState* state) {
    ...
    _stream_recvr = state->create_recvr(_row_descriptor, _id, _num_senders, config::exchg_node_buffer_size_bytes,
                                        runtime_profile());
    return Status::OK();
}
...
Status ExchangeNode::get_next(RuntimeState* state, RowBatch* output_batch, bool* eos) {
    ...
    while (true) {
        ...
        if (is_cancelled) {
            return Status::Cancelled("Cancelled ExchangeNode::get_next");
        }
