    ...
    if (cache_value.version == version) {
        ...
        if (!status.ok()) {
            ...
            if (all_rs_empty) {
                ...
                // for now, HIT_PARTIAL is processed just as PLBS_MISS.
...
Status CacheOperator::reset_lane(LaneOwnerType lane_owner) {
    DCHECK(_multilane_operators.size() > 0);
    for (auto i = 0; i < _multilane_operators.size() - 1; ++i) {
        RETURN_IF_ERROR(_multilane_operators[i]->reset_lane(lane_owner, {}));
    }
    // TODO(by satanson): multi-version cache mechanism needs to push partial-hit result to
    //  the predecessor operator of the cache operator.
    RETURN_IF_ERROR(_multilane_operators.back()->reset_lane(lane_owner, {}));
    return Status::OK();
}
