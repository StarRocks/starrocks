...
//   https://github.com/apache/incubator-doris/blob/master/be/src/exec/aggregation_node.cpp
...
Status AggregationNode::init(const TPlanNode& tnode, RuntimeState* state) {
    ...
    RETURN_IF_ERROR(Expr::create_expr_trees(_pool, tnode.agg_node.grouping_exprs, &_probe_expr_ctxs));
...
Status AggregationNode::prepare(RuntimeState* state) {
    ...
    _hash_table_load_factor_counter = ADD_COUNTER(runtime_profile(), "LoadFactor", TUnit::DOUBLE_VALUE);
    ...
    for (int i = 0; i < _aggregate_evaluators.size(); ++i, ++j) {
        ...
        RETURN_IF_ERROR(_aggregate_evaluators[i]->prepare(state, child(0)->row_desc(), _tuple_pool.get(),
                                                          intermediate_slot_desc, output_slot_desc, mem_tracker(),
                                                          &_agg_fn_ctxs[i]));
...
Status AggregationNode::open(RuntimeState* state) {
    ...
    while (true) {
        ...
        if (VLOG_ROW_IS_ON) {
            for (int i = 0; i < batch.num_rows(); ++i) {
                ...
                VLOG_ROW << "id=" << id() << " input row: " << row->to_string(_children[0]->row_desc());
            }
        }
    ...
    VLOG_ROW << "id=" << id() << " aggregated " << num_input_rows << " input rows into " << num_agg_rows
             << " output rows";
...
void AggregationNode::push_down_predicate(RuntimeState* state, std::list<ExprContext*>* expr_ctxs, bool is_vectorized) {
