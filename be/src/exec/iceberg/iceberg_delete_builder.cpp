...
Status ParquetPositionDeleteBuilder::build(const std::string& timezone, const std::string& delete_file_path,
    ...
    update_delete_file_io_counter(scanner_params.profile->runtime_profile, app_scan_stats, fs_scan_stats,
                                  cache_input_stream, shared_buffered_input_stream);
    return Status::OK();
}

void ParquetPositionDeleteBuilder::update_delete_file_io_counter(
