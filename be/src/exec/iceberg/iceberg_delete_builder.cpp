...
Status ParquetPositionDeleteBuilder::build(const std::string& timezone, const std::string& delete_file_path,
    ...
    for (int64_t offset = 0; offset < file_size;) {
        const int64_t remain_length =
                std::min(static_cast<int64_t>(config::io_coalesce_read_max_buffer_size), file_size - offset);
    ...
    if (scanner_params.datacache_options.enable_datacache) {
        cache_input_stream = std::make_shared<io::CacheInputStream>(shared_buffered_input_stream, filename, file_size,
                                                                    scanner_params.datacache_options.modification_time);
        cache_input_stream->set_enable_populate_cache(scanner_params.datacache_options.enable_populate_datacache);
        cache_input_stream->set_enable_async_populate_mode(scanner_params.datacache_options.enable_datacache_async_populate_mode);
        cache_input_stream->set_enable_cache_io_adaptor(scanner_params.datacache_options.enable_datacache_io_adaptor);
    ...
    try {
        reader = std::make_unique<parquet::FileReader>(state->chunk_size(), file.get(), file->get_size().value(), scanner_params.datacache_options);
