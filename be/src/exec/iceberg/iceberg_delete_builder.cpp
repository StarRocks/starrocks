...
Status ParquetPositionDeleteBuilder::build(const std::string& timezone, const std::string& delete_file_path,
    ...
    if (scanner_params.use_block_cache) {
        ...
        cache_input_stream->set_enable_populate_cache(scanner_params.enable_populate_block_cache);
        cache_input_stream->set_enable_block_buffer(config::block_cache_block_buffer_enable);
    ...
    try {
        reader = std::make_unique<parquet::FileReader>(state->chunk_size(), file.get(), file->get_size().value(), shared_buffered_input_stream.get(), nullptr);
...
Status ParquetEqualityDeleteBuilder::build(const std::string& timezone, const std::string& file_path,
    ...
    try {
        reader = std::make_unique<parquet::FileReader>(state->chunk_size(), file.get(), file->get_size().value(),
                                                       shared_buffered_input_stream.get(), nullptr);
    } catch (std::exception& e) {
        const auto s = strings::Substitute(
...
void ParquetPositionDeleteBuilder::update_v2_io_counter(
    ...
    if (cache_input_stream) {
        static const char* prefix = "MOR_BlockCache";
        ADD_CHILD_COUNTER(parent_profile, prefix, TUnit::NONE, ICEBERG_TIMER);

        RuntimeProfile::Counter* block_cache_read_counter =
                ADD_CHILD_COUNTER(parent_profile, "MOR_BlockCacheReadCounter", TUnit::UNIT, prefix);
        RuntimeProfile::Counter* block_cache_read_bytes =
                ADD_CHILD_COUNTER(parent_profile, "MOR_BlockCacheReadBytes", TUnit::BYTES, prefix);
        RuntimeProfile::Counter* block_cache_read_timer = ADD_CHILD_TIMER(parent_profile, "MOR_BlockCacheReadTimer", prefix);
        RuntimeProfile::Counter* block_cache_write_counter =
                ADD_CHILD_COUNTER(parent_profile, "MOR_BlockCacheWriteCounter", TUnit::UNIT, prefix);
        RuntimeProfile::Counter* block_cache_write_bytes =
                ADD_CHILD_COUNTER(parent_profile, "MOR_BlockCacheWriteBytes", TUnit::BYTES, prefix);
        RuntimeProfile::Counter* block_cache_write_timer = ADD_CHILD_TIMER(parent_profile, "MOR_BlockCacheWriteTimer", prefix);
        RuntimeProfile::Counter* block_cache_write_fail_counter =
                ADD_CHILD_COUNTER(parent_profile, "MOR_BlockCacheWriteFailCounter", TUnit::UNIT, prefix);
        RuntimeProfile::Counter* block_cache_write_fail_bytes =
                ADD_CHILD_COUNTER(parent_profile, "MOR_BlockCacheWriteFailBytes", TUnit::BYTES, prefix);
        RuntimeProfile::Counter* block_cache_read_block_buffer_counter =
                ADD_CHILD_COUNTER(parent_profile, "MOR_BlockCacheReadBlockBufferCounter", TUnit::UNIT, prefix);
        RuntimeProfile::Counter* block_cache_read_block_buffer_bytes =
                ADD_CHILD_COUNTER(parent_profile, "MOR_BlockCacheReadBlockBufferBytes", TUnit::BYTES, prefix);

        const io::CacheInputStream::Stats& stats = cache_input_stream->stats();
        COUNTER_UPDATE(block_cache_read_counter, stats.read_cache_count);
        COUNTER_UPDATE(block_cache_read_bytes, stats.read_cache_bytes);
        COUNTER_UPDATE(block_cache_read_timer, stats.read_cache_ns);
        COUNTER_UPDATE(block_cache_write_counter, stats.write_cache_count);
        COUNTER_UPDATE(block_cache_write_bytes, stats.write_cache_bytes);
        COUNTER_UPDATE(block_cache_write_timer, stats.write_cache_ns);
        COUNTER_UPDATE(block_cache_write_fail_counter, stats.write_cache_fail_count);
        COUNTER_UPDATE(block_cache_write_fail_bytes, stats.write_cache_fail_bytes);
        COUNTER_UPDATE(block_cache_read_block_buffer_counter, stats.read_block_buffer_count);
        COUNTER_UPDATE(block_cache_read_block_buffer_bytes, stats.read_block_buffer_bytes);
