...
StatusOr<std::unique_ptr<RandomAccessFile>> HdfsScanner::create_random_access_file(
    ...
    return connector::open_scan_random_access_file(shared_buffered_input_stream, cache_input_stream, options);
}

Status HdfsScanner::open_random_access_file() {
    connector::ScanFileOpenOptions options{.fs = _scanner_ctx->fs,
