...
Status IcebergSinkBuilder::build(pipeline::OpFactories prev_operators, const TDataSink& thrift_sink,
    ...
    if (table_desc == nullptr) {
        return Status::InternalError("Missing Iceberg table descriptor");
    }
...
Status IcebergSinkBuilder::create_delete_sink_context(const TDataSink& thrift_sink, RuntimeState* runtime_state,
    ...
    const auto& output_exprs = _output_exprs;
...
Status IcebergSinkBuilder::create_data_sink_context(const TDataSink& thrift_sink, RuntimeState* runtime_state,
                                                    pipeline::PipelineBuilderContext* /*context*/,
                                                    IcebergTableDescriptor* iceberg_table_desc,
                                                    std::unique_ptr<connector::ConnectorSinkProvider>& sink_provider,
                                                    std::vector<TExpr>& partition_expr) const {
    ...
    data_sink_ctx->column_evaluators = ColumnExprEvaluator::from_exprs(_output_exprs, runtime_state);
    ...
    if (iceberg_table_desc->is_unpartitioned_table()) {
        ...
        for (const auto& index : iceberg_table_desc->partition_source_index_in_schema()) {
            if (index < 0 || index >= _output_exprs.size()) {
                return Status::InternalError(fmt::format("Invalid partition index: {}", index));
            }
            partition_expr.push_back(_output_exprs[index]);
        }
        ...
        if (partition_expr.empty()) {
            auto output_expr = _output_exprs;
        ...
        for (auto& part_expr : partition_expr) {
            ...
            auto slot_ref = _output_exprs[index];
...
Status IcebergSinkBuilder::create_row_delta_sink_context(
...
Status IcebergSinkBuilder::update_partition_expr_slot_refs_by_map(
