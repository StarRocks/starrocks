...
Status HiveSinkBuilder::build(pipeline::OpFactories prev_operators, const TDataSink& thrift_sink,
    ...
    if (num_data_columns > output_exprs.size()) {
        return Status::InternalError("Hive sink has more data columns than output expressions");
    }
