...
Status Analytor::_add_chunk(const ChunkPtr& chunk) {
...

    {
        ...
        for (size_t i = 0; i < _partition_ctxs.size(); i++) {
            ...
            // Upgrade BinaryColumn to LargeBinaryColumn if it exceeds 4GB
            auto upgrade_ret = _partition_columns[i]->upgrade_if_overflow();
            if (!upgrade_ret.ok()) {
                return upgrade_ret.status();
            }
            if (upgrade_ret.value() != nullptr) {
                _partition_columns[i] = std::move(upgrade_ret.value());
            }
        ...
        for (size_t i = 0; i < _order_ctxs.size(); i++) {
            ...
            // Upgrade BinaryColumn to LargeBinaryColumn if it exceeds 4GB
            auto upgrade_ret = _order_columns[i]->upgrade_if_overflow();
            if (!upgrade_ret.ok()) {
                return upgrade_ret.status();
            }
            if (upgrade_ret.value() != nullptr) {
                _order_columns[i] = std::move(upgrade_ret.value());
            }
