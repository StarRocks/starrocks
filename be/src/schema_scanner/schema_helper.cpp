...
Status SchemaHelper::get_policy_references(const SchemaScannerState& state, const TGetPolicyReferencesRequest& request,
                                           TGetPolicyReferenceResponse* response) {
    return _call_rpc(state, [&request, &response](FrontendServiceConnection& client) {
        client->getPolicyReference(*response, request);
    });
}
