...
message(STATUS "Generating thrift ${CODEGEN_SOURCE}")
execute_process(
...
if(NOT _generate_result EQUAL 0)
    message(FATAL_ERROR "Failed to generate thrift ${CODEGEN_SOURCE}\n${_generate_stdout}\n${_generate_stderr}")
endif()

if(CODEGEN_PATCH_FILE)
    if(NOT EXISTS "${CODEGEN_PATCH_FILE}")
        message(FATAL_ERROR "Missing thrift patch file ${CODEGEN_PATCH_FILE}")
    endif()
    if(NOT EXISTS "${CODEGEN_PATCH_HEADER}")
        message(FATAL_ERROR "Missing thrift patch header ${CODEGEN_PATCH_HEADER}")
    endif()

    file(READ "${CODEGEN_PATCH_HEADER}" _patch_header_contents)
    string(FIND "${_patch_header_contents}" "TApplicationException" _patch_required_index)
    if(NOT _patch_required_index EQUAL -1)
        execute_process(
            COMMAND patch -p0 -d "${CODEGEN_OUTPUT_DIR}" -i "${CODEGEN_PATCH_FILE}"
            RESULT_VARIABLE _patch_result
            OUTPUT_VARIABLE _patch_stdout
            ERROR_VARIABLE _patch_stderr)
        if(NOT _patch_result EQUAL 0)
            message(FATAL_ERROR
                    "Failed to patch generated thrift header ${CODEGEN_PATCH_HEADER}\n${_patch_stdout}\n${_patch_stderr}")
        endif()
    endif()
endif()
...
_starrocks_outputs_missing(_missing_outputs)
if(_missing_outputs)
    message(FATAL_ERROR "Thrift generation completed without producing all outputs for ${CODEGEN_SOURCE}")
endif()
