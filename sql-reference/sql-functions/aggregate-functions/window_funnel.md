...
- `mode`：事件链的筛选模式，类型为INT。取值范围：0，1，2，4。
  - 默认值为`0`，表示执行最一般的漏斗计算。
  - 模式为`1`时（bits 设置第1位）表示`DEDUPLICATION`模式，即筛选出的事件链不能有重复的事件。假设`array`参数为`[event_type='A', event_type='B', event_type='C', event_type='D']`，原事件链为"A-B-C-B-D"，由于事件B重复，那么筛选出的事件链只能是"A-B-C"。
  - 模式为`2`时（bits 设置第2位）表示`FIXED`模式，即筛选出的事件链不能有跳跃的事件，假设`array`参数如上不变，原事件链为"A-B-D-C"，由于事件D跳跃，那么筛选出的事件链只能是"A-B"。
...
**示例四：筛选出`uid`对应的最大连续事件数，窗口为1900s，分别计算筛选模式为`0`(时间戳不需要严格递增)和`4`(时间戳需要严格递增)的结果。**

假设有表`action3`，数据以`time`排序：

```Plaintext
mysql> select * from action3 order by time;
+------+------------+---------------------+
| uid  | event_type | time                |
+------+------------+---------------------+
| 1    | 浏览       | 2020-01-02 11:00:00 |
...
```Plaintext
mysql> select uid, window_funnel(1900,time,0,[event_type='浏览', event_type='点击',
        event_type='下单', event_type='下单']) AS level from action3 group by uid order by uid;
+------+-------+
| uid  | level |
+------+-------+
| 1    |     4 |
...
可以看到对于uid=1，输出为`4`，因为第3、4两个条件都是"下单"，所以时间戳`2020-01-02 11:00:31`所对应的事件应用了两次。

将`mode`改为`4`，再次执行SQL：

```Plaintext
mysql> select uid, window_funnel(1900,time,4,[event_type='浏览', event_type='点击',
        event_type='下单', event_type='下单']) AS level from action3 group by uid order by uid;
+------+-------+
| uid  | level |
+------+-------+
...
输出为`3`，因为指定了时间戳严格递增，所以“下单”事件只算作第三个条件，第四个条件被忽略。
