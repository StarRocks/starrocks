...
该语句用于按条件删除指定表的基本索引 (Base Index) 分区中的数据。

该操作会同时删除和此基本索引相关的物化视图索引的数据。
...
> 说明：方括号 [] 中内容如无需指定的话可省略不写。

### 注意事项

- `op` 的可选类型包括：`=`、`>`、`<`、`>=`、`<=`、`!=`、`IN` 和 `NOT IN`。
...
在数据库 `test` 中创建一张名为 `users` 的表，如下所示：
...
查询 `users` 表中 `country` 列值为 `China` 的数据行，然后删除 `score_board` 表中与 `users` 表里里查询到的数据行具有相同 `name` 列值的所有数据行。
