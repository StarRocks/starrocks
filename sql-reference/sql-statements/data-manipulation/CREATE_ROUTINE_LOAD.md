...
Routine Load 支持持续消费 Apache Kafka® 的消息并导入至 StarRocks 中。Routine Load 支持 Kafka 中消息的格式为 CSV 和 JSON，并且并且访问 Kafka 时，支持多种安全协议，包括 `plaintext`、`ssl`、`sasl_plaintext` 和 `sasl_ssl`。
