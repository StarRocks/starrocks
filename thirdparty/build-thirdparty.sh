...
build_aws_cpp_sdk() {
    ...
    $CMAKE_CMD -Bbuild -DBUILD_ONLY="core;s3;s3-crt" -DCMAKE_BUILD_TYPE=RelWithDebInfo -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX=${TP_INSTALL_DIR} -DENABLE_TESTING=OFF
    cd build
    make -j$PARALLEL && make install
}
