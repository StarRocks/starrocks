...
public class StarrocksStreamLoader {
    ...
    private HttpURLConnection getConnection(String urlStr, String label) throws IOException {
        ...
        conn.addRequestProperty("columns", "queryId, timestamp, clientIp, user, authorizedUser, resourceGroup, catalog, db, state, errorCode," +
    ...
    private String toCurl(HttpURLConnection conn) {
        ...
        sb.append("-H \"").append("columns\":").append("\"queryId, timestamp, clientIp, user, authorizedUser, resourceGroup, catalog, db, state, errorCode," +
                "queryTime, scanBytes, scanRows, returnRows, cpuCostNs, memCostBytes, stmtId, isQuery, feIp, stmt, digest, planCpuCosts, planMemCosts\" \\\n  ");
